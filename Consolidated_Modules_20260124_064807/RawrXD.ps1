...
${script:ErrorNotificationConfig} = @{
    ...
    EmailRecipient           = ${env:RAWRXD_EMAIL_RECIPIENT}
    SMTPServer               = ${env:RAWRXD_SMTP_SERVER}
...
function Search-Marketplace {
    ...
    if ($IncludeRemote) {
        ...
        foreach ($entry in $catalog) {
            & $evaluateEntry $entry (if ($entry.Source) { $entry.Source } else { 'Marketplace' })
        }
    }
