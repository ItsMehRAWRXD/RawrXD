...
#ifndef GGML_BF16_T_DEFINED
#define GGML_BF16_T_DEFINED 1
typedef struct { std::uint16_t bits; } ggml_bf16_t;
#endif

class VulkanCompute;

// Compression type enumeration for GGUF tensors
enum class GGUFCompressionType : uint32_t {
    NONE = 0,
    DEFLATE = 1,
    BRUTAL_GZIP = 2,
    ZLIB = 3,
...
public:
    ...
    bool IsCompressed() const { return compression_type_ != GGUFCompressionType::NONE; }
    GGUFCompressionType GetCompressionType() const { return compression_type_; }
    bool SetCompressionType(GGUFCompressionType type);
...
private:
    ...
    GGUFCompressionType compression_type_ = GGUFCompressionType::NONE;
