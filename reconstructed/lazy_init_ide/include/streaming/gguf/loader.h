...
public:
    ...
    // ============================================================================
    // Returns a view directly into zone memory. NO memcpy, NO allocation.
    // Caller MUST NOT hold this span across zone unloads.
    // Returns empty span if zone not loaded (call PrefetchTensorAsync first).
    ...
    // Check if tensor's zone is currently resident (for polling pattern)
    bool IsTensorResident(const std::string& tensor_name) const;
    
    // Get raw pointer to zone data (for advanced users)
