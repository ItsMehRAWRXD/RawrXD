...
extern "C" bool DirectIO_Prefetch(DirectIOContext* ctx, uint64_t offset, size_t size, void* dst) {
    ...
    HRESULT hr = BuildIoRingReadFile(ctx->hRing, 
                                    ...
                                    (uint32_t)size, 
                                    offset, 
                                    (uint64_t)offset, // Use offset as ID for simplicity in trace
                                    IOSQE_FLAGS_NONE);
...
extern "C" TensorMetadata GetTensorMetadata(uint32_t tensor_id) {
    // Dummy metadata for validation
    return { (uint64_t)tensor_id * 4096, 4096 };
}
