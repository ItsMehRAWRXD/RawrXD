...
bool HFDownloader::DownloadFile(const std::string& url, const std::string& output_path,
    ...
    current_progress_.current_file = output_path;
    QNetworkReply* reply = manager.get(request);
    QEventLoop loop;
    QTimer cancelTimer;
    ...
    QObject::connect(reply, &QNetworkReply::downloadProgress, [&](qint64 received, qint64 total) {
        current_progress_.bytes_downloaded = static_cast<uint64_t>(received);
        current_progress_.total_bytes = static_cast<uint64_t>(total);
        if (total > 0) {
            current_progress_.progress_percent = static_cast<float>(received * 100.0 / total);
        }
        if (callback) {
            callback(current_progress_);
        }
    });
