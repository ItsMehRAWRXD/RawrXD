...
#include "agentic/RawrXD_AgentLoop.h"
...
#define WM_AGENT_STATUS         (WM_USER + 300)
#define WM_AGENT_PERCEPTION     (WM_USER + 310)

// Digest-specific numeric error codes (shared with MASM side)
#define E_DIGEST_INVALIDARG     87   // ERROR_INVALID_PARAMETER
#define E_DIGEST_FILENOTFOUND   2    // ERROR_FILE_NOT_FOUND
#define E_DIGEST_NOMEMORY       8    // ERROR_NOT_ENOUGH_MEMORY
#define E_DIGEST_THREADCREATE   8    // Reuse NOMEMORY for thread failures
#define S_DIGEST_OK             0

static std::atomic<DWORD> g_digestionTaskCounter{1};
static std::atomic<DWORD> g_lastDigestionTaskId{0};

#pragma pack(push, 1)
struct DIGESTION_CTX {
    DWORD   cbSize;
...
using PDIGESTION_CTX = DIGESTION_CTX*;

constexpr char kRawrXDIDEClass[] = "RawrXD_IDE_Class";

// Forward declarations
static std::wstring utf8_to_wide(const std::string& s);

static void __stdcall DigestionProgressCallback(DWORD percent, DWORD taskId)
{
    {
        char dbg[128];
        sprintf_s(dbg, "[IDE-Digest] DigestionProgressCallback: Task %lu, %lu%%\n", taskId, percent);
        OutputDebugStringA(dbg);
    }
    
    HWND hwndIDE = FindWindowA(kRawrXDIDEClass, nullptr);
    if (!hwndIDE) {
        OutputDebugStringA("[IDE-Digest] ERROR: Could not find IDE window in callback\n");
        return;
    }
    
    BOOL posted = PostMessageW(hwndIDE, WM_DIGESTION_PROGRESS,
                 static_cast<WPARAM>(taskId), MAKELPARAM(percent, 0));
    if (!posted) {
        OutputDebugStringA("[IDE-Digest] ERROR: PostMessageW failed in callback\n");
    }
}
...
{
    OutputDebugStringA("[IDE-Digest] DigestionThreadProc ENTRY\n");
    
    auto* pCtx = static_cast<PDIGESTION_CTX>(lpParam);
    if (!pCtx) {
        OutputDebugStringA("[IDE-Digest] ERROR: DigestionThreadProc pCtx is NULL\n");
        return 0;
    }

    {
        char dbg[256];
        sprintf_s(dbg, "[IDE-Digest] Thread started - Task %lu, Source: %S, Output: %S\n",
                  pCtx->dwTaskId, pCtx->szSource, pCtx->szOutput);
        OutputDebugStringA(dbg);
    }
    ...
    DWORD dwThreads = pCtx->dwThreads;
    if (dwThreads == 0) {
        dwThreads = si.dwNumberOfProcessors ? si.dwNumberOfProcessors : 1;
    }
    dwThreads = std::max<DWORD>(1, std::min<DWORD>(dwThreads, 64));

    DWORD dwChunk = pCtx->dwChunkSize ? pCtx->dwChunkSize : 65536;
    dwChunk = std::min<DWORD>(dwChunk, 64u * 1024u * 1024u);

    {
        char dbg[128];
        sprintf_s(dbg, "[IDE-Digest] Calling RawrXD_DigestionEngine_Avx512 with threads=%lu\n", dwThreads);
        OutputDebugStringA(dbg);
    }

    DWORD dwResult = RawrXD_DigestionEngine_Avx512(
        pCtx->szSource,
        pCtx->szOutput,
        dwChunk,
        dwThreads,
        pCtx->dwFlags,
        DigestionProgressCallback
    );

    {
        char dbg[128];
        sprintf_s(dbg, "[IDE-Digest] Engine returned: %lu\n", dwResult);
        OutputDebugStringA(dbg);
    }

    HWND hwndNotify = pCtx->hNotify ? pCtx->hNotify : FindWindowA(kRawrXDIDEClass, nullptr);
    if (hwndNotify) {
        OutputDebugStringA("[IDE-Digest] Posting WM_DIGESTION_COMPLETE\n");
        PostMessageW(hwndNotify, WM_DIGESTION_COMPLETE,
                     static_cast<WPARAM>(pCtx->dwTaskId), static_cast<LPARAM>(dwResult));
    } else {
        OutputDebugStringA("[IDE-Digest] WARNING: hwndNotify is NULL, cannot post completion message\n");
    }
    
    if (pCtx->hCompletionEvent) {
        SetEvent(pCtx->hCompletionEvent);
    }
    
    HeapFree(GetProcessHeap(), 0, pCtx);
    OutputDebugStringA("[IDE-Digest] DigestionThreadProc EXIT\n");
    return static_cast<unsigned>(dwResult);
}
...
#define IDM_AGENT_TOGGLE_WATCHER 4106
#define IDM_AGENT_TOGGLE_CONSENT 4107
#define IDM_AGENT_WATCHER_SENSITIVITY_HIGH 4108
#define IDM_AGENT_WATCHER_SENSITIVITY_MEDIUM 4109
#define IDM_AGENT_WATCHER_SENSITIVITY_LOW 4110
#define IDM_AGENT_CONSENT_AUTO 4111
#define IDM_AGENT_CONSENT_PROMPT 4112
#define IDM_AGENT_CONSENT_DENY 4113
...
Win32IDE::Win32IDE(HINSTANCE hInstance)
    ...
    , m_watcherSensitivity(1)
    , m_lastWatcherNotification(std::chrono::steady_clock::time_point::min())
{
    ...
    try {
        ...
        agentConfig.enableAutoDiagnostics = true;
        agentConfig.enableBeaconing = true;
        agentConfig.enableSelfHealing = true;
        agentConfig.beaconLogPath = "C:\\RawrXD_Agent_Beacons.log";
        agentConfig.diagnosticReportPath = "C:\\RawrXD_Diagnostics.json";
        
        AutonomousAgent::Initialize(agentConfig);
        LOG_INFO("Autonomous agent initialized");
    } catch (const std::exception& e) {
        LOG_ERROR(std::string("Failed to initialize autonomous agent: ") + e.what());
        OutputDebugStringA("ERROR: Autonomous agent initialization failed\n");
    }
}
...
Win32IDE::~Win32IDE()
{
    ...
    try {
        ...
        LOG_INFO("Autonomous agent shutdown complete");
    } catch (...) {
        OutputDebugStringA("ERROR: Autonomous agent shutdown failed\n");
    }
...
bool Win32IDE::createWindow()
{
    ...
    // Create accelerator table for hotkeys
    ACCEL accels[1];
    accels[0].fVirt = FVIRTKEY | FCONTROL | FSHIFT;
    accels[0].key = 'D';
    accels[0].cmd = IDM_RUN_DIGESTION;
    m_hAccelTable = CreateAcceleratorTableA(accels, 1);

    // Register IDE window with autonomous agent
    try {
        auto* agent = AutonomousAgent::Instance();
        agent->SetIDEWindow(m_hwndMain);
        agent->SetIDEProcessId(GetCurrentProcessId());
        agent->Start();
        LOG_INFO("Autonomous agent started and monitoring IDE");
    } catch (const std::exception& e) {
        LOG_ERROR(std::string("Failed to start autonomous agent: ") + e.what());
    }
...
int Win32IDE::runMessageLoop()
{
    ...
    while (GetMessage(&msg, nullptr, 0, 0)) {
                if (m_hAccelTable && TranslateAcceleratorW(m_hwndMain, m_hAccelTable, &msg)) {
            continue;
        }
...
LRESULT Win32IDE::handleMessage(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
        {
            {
                char dbg[256];
                sprintf_s(dbg, "[IDE-Digest] WM_KEYDOWN: wParam=0x%IX (char=%c)\n",
                          static_cast<UINT_PTR>(wParam),
                          (wParam >= 32 && wParam < 127) ? static_cast<int>(wParam) : '?');
                OutputDebugStringA(dbg);
            }
            ...
            {
                char dbg[128];
                sprintf_s(dbg, "[IDE-Digest] Modifiers: ctrl=%d, shift=%d, alt=%d\n", ctrl, shift, alt);
                OutputDebugStringA(dbg);
            }
            ...
            if (ctrl && shift && wParam == 'D') { // Ctrl+Shift+D run digestion engine
                OutputDebugStringA("[IDE-Digest] HOTKEY: Ctrl+Shift+D detected\n");
                
                // Notify autonomous agent
                AutonomousAgent::Instance()->OnDigestionHotkeyPressed();

                if (m_currentFile.empty()) {
                    OutputDebugStringA("[IDE-Digest] ERROR: No current file open\n");
                    MessageBoxA(m_hwndMain, "Open a file before running digestion.", "Digestion", MB_OK | MB_ICONWARNING);
                    return 0;
                }

                {
                    char dbg[256];
                    sprintf_s(dbg, "[IDE-Digest] Current file: %s\n", m_currentFile.c_str());
                    OutputDebugStringA(dbg);
                }

                std::wstring source = utf8_to_wide(m_currentFile);
                std::wstring output = source + L".digest";
                
                OutputDebugStringA("[IDE-Digest] About to call queueDigestionJob\n");
    ...
    case WM_AGENT_STATUS:
        {
            const char* msg = reinterpret_cast<const char*>(lParam);
            if (msg) {
                appendToOutput(std::string("Agent: ") + msg + "\n", "Output", OutputSeverity::Info);
                free(const_cast<char*>(msg));
            }
            return 0;
        }

    case WM_AGENT_PERCEPTION:
        {
            const char* msg = reinterpret_cast<const char*>(lParam);
            if (msg) {
                std::string payload = msg;
                if (m_autonomyManager) {
                    m_autonomyManager->addObservation(payload);
                }
                if (m_agentLoop) {
                    m_agentLoop->TriggerPerception(RawrXD::Agentic::PerceptionType::FileChange, payload);
                }
                free(const_cast<char*>(msg));
            }
            return 0;
    ...
    case WM_RUN_DIGESTION:
        {
            OutputDebugStringA("IDE-Digest: WM_RUN_DIGESTION received\n");
            OutputDebugStringA("[IDE-Digest] WM_RUN_DIGESTION handler ENTRY\n");
            
            // Notify agent loop of digestion event (before processing digestion)
            if (m_agentLoop) {
                const char* payload = reinterpret_cast<const char*>(lParam);
                if (!payload) payload = "(digestion started)";
                m_agentLoop->TriggerPerception(RawrXD::Agentic::PerceptionType::CodeChange, 
                    std::string("digestion_started: task_id=") + std::to_string((DWORD)wParam));
                OutputDebugStringA("[IDE-Digest] Notified agent loop of digestion perception\n");
            }
            
            // New contract: wParam = task id, lParam = HEAP-allocated DIGESTION_CTX*
            if (!lParam) {
                OutputDebugStringA("[IDE-Digest] ERROR: lParam is NULL\n");
                return static_cast<LRESULT>(E_DIGEST_INVALIDARG);
            }

            auto* pIncoming = reinterpret_cast<PDIGESTION_CTX>(lParam);
            if (!pIncoming || pIncoming->cbSize != sizeof(DIGESTION_CTX)) {
                OutputDebugStringA("[IDE-Digest] ERROR: pIncoming null or cbSize mismatch\n");
                if (pIncoming) {
                    HeapFree(GetProcessHeap(), 0, pIncoming);
                }
                return static_cast<LRESULT>(E_DIGEST_INVALIDARG);
            }

            if (pIncoming->szSource[0] == L'\0' || pIncoming->szOutput[0] == L'\0') {
                OutputDebugStringA("[IDE-Digest] ERROR: Source or output path is empty\n");
                HeapFree(GetProcessHeap(), 0, pIncoming);
                return static_cast<LRESULT>(E_DIGEST_INVALIDARG);
            }

            DWORD dwAttr = GetFileAttributesW(pIncoming->szSource);
            if (dwAttr == INVALID_FILE_ATTRIBUTES) {
                OutputDebugStringA("[IDE-Digest] ERROR: Source file not found\n");
                HeapFree(GetProcessHeap(), 0, pIncoming);
                return static_cast<LRESULT>(E_DIGEST_FILENOTFOUND);
            }

            SYSTEM_INFO si{};
            GetSystemInfo(&si);
            DWORD threads = pIncoming->dwThreads ? pIncoming->dwThreads : (si.dwNumberOfProcessors ? si.dwNumberOfProcessors : 1);
            if (threads > 64) threads = 64;

            auto* pCtx = static_cast<PDIGESTION_CTX>(HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(DIGESTION_CTX)));
            if (!pCtx) {
                OutputDebugStringA("[IDE-Digest] ERROR: HeapAlloc failed for pCtx in handler\n");
                HeapFree(GetProcessHeap(), 0, pIncoming);
                return static_cast<LRESULT>(E_DIGEST_NOMEMORY);
            }

            CopyMemory(pCtx, pIncoming, sizeof(DIGESTION_CTX));
            pCtx->hNotify = hwnd;
            pCtx->dwThreads = threads;

            DWORD incomingTaskId = static_cast<DWORD>(wParam);
            if (incomingTaskId != 0) {
                pCtx->dwTaskId = incomingTaskId;
            }

            // Free the initial heap-allocated context passed by the caller
            HeapFree(GetProcessHeap(), 0, pIncoming);

            OutputDebugStringA("[IDE-Digest] Calling _beginthreadex\n");
            
            HANDLE hThread = reinterpret_cast<HANDLE>(_beginthreadex(nullptr, 0, DigestionThreadProc, pCtx, 0, nullptr));
            if (!hThread) {
                OutputDebugStringA("[IDE-Digest] ERROR: _beginthreadex failed\n");
                HeapFree(GetProcessHeap(), 0, pCtx);
                return static_cast<LRESULT>(E_DIGEST_THREADCREATE);
            }

            OutputDebugStringA("[IDE-Digest] Thread created successfully\n");
            
            // Notify autonomous agent of thread spawn
            AutonomousAgent::Instance()->OnDigestionThreadSpawned(pCtx->dwTaskId, hThread);
            
            CloseHandle(hThread);
            g_lastDigestionTaskId.store(pCtx->dwTaskId, std::memory_order_relaxed);

            if (m_hwndStatusBar) {
                WCHAR szBuf[128];
                wsprintfW(szBuf, L"Digestion Task %u: starting...", pCtx->dwTaskId);
                SetWindowTextW(m_hwndStatusBar, szBuf);
                OutputDebugStringA("[IDE-Digest] Status bar updated\n");
            }

            {
                char msg[256];
                sprintf_s(msg, "[Digestion] Task %lu started (threads=%lu, chunk=%lu)\n",
                          pCtx->dwTaskId, pCtx->dwThreads, pCtx->dwChunkSize);
                appendToOutput(msg, "Output", OutputSeverity::Info);
            }

            OutputDebugStringA("[IDE-Digest] WM_RUN_DIGESTION handler EXIT SUCCESS\n");
            return static_cast<LRESULT>(S_DIGEST_OK);
        }

    case WM_DIGESTION_PROGRESS:
        {
            DWORD taskId = static_cast<DWORD>(wParam);
            DWORD percent = static_cast<DWORD>(LOWORD(lParam));
            {
                char dbg[128];
                sprintf_s(dbg, "[IDE-Digest] WM_DIGESTION_PROGRESS Task %lu -> %lu%%\n", taskId, percent);
                OutputDebugStringA(dbg);
            }
            
            // Notify autonomous agent
            AutonomousAgent::Instance()->OnDigestionProgress(taskId, percent);
            ...
            if (m_hwndStatusBar) {
                WCHAR szBuf[64];
                wsprintfW(szBuf, L"Digestion Task %u: %u%%", taskId, percent);
                SetWindowTextW(m_hwndStatusBar, szBuf);
                OutputDebugStringA("[IDE-Digest] WM_DIGESTION_PROGRESS status bar updated\n");
            }
            return 0;
        }

    case WM_DIGESTION_COMPLETE:
        {
            DWORD taskId = static_cast<DWORD>(wParam);
            DWORD result = static_cast<DWORD>(lParam);
            WCHAR szBuf[128];
            {
                char dbg[128];
                sprintf_s(dbg, "[IDE-Digest] WM_DIGESTION_COMPLETE Task %lu result %lu\n", taskId, result);
                OutputDebugStringA(dbg);
            }
            
            // Notify autonomous agent
            if (result == S_DIGEST_OK) {
                AutonomousAgent::Instance()->OnDigestionComplete(taskId, result);
            } else {
                AutonomousAgent::Instance()->OnDigestionError(taskId, result, "Digestion engine returned error");
            }
            ...
            if (result == S_DIGEST_OK) {
                wsprintfW(szBuf, L"Digestion Task %u: SUCCESS", taskId);
            } else {
                wsprintfW(szBuf, L"Digestion Task %u: FAILED (%lu)", taskId, result);
            }
            if (m_hwndStatusBar) {
                SetWindowTextW(m_hwndStatusBar, szBuf);
                OutputDebugStringA("[IDE-Digest] WM_DIGESTION_COMPLETE status bar updated\n");
            }
            {
                char msg[256];
                sprintf_s(msg, "[Digestion] Task %lu %s (%lu)\n",
                          taskId, (result == S_DIGEST_OK) ? "COMPLETED" : "FAILED", result);
                appendToOutput(msg, "Output", (result == S_DIGEST_OK) ? OutputSeverity::Info : OutputSeverity::Error);
            }
            return 0;
        }
...
bool Win32IDE::queueDigestionJob(const std::wstring& source, const std::wstring& output)
{
    // DEBUG: Confirm entry
    char buf[256];
    snprintf(buf, sizeof(buf), "IDE-Digest: queueDigestionJob called, hwnd=%p\n", (void*)m_hwndMain);
    OutputDebugStringA(buf);

    OutputDebugStringA("[IDE-Digest] queueDigestionJob ENTRY\n");
    
    if (!IsWindow(m_hwndMain)) {
        OutputDebugStringA("[IDE-Digest] ERROR: m_hwndMain is not a valid window\n");
        MessageBoxA(nullptr, "Digestion is unavailable (main window lost).", "Digestion", MB_OK | MB_ICONERROR);
        return false;
    }

    auto* ctxHeap = static_cast<PDIGESTION_CTX>(HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(DIGESTION_CTX)));
    if (!ctxHeap) {
        OutputDebugStringA("[IDE-Digest] ERROR: HeapAlloc failed for DIGESTION_CTX\n");
        MessageBoxA(m_hwndMain, "Failed to allocate memory for digestion context.", "Digestion", MB_OK | MB_ICONERROR);
        return false;
    }
    
    OutputDebugStringA("[IDE-Digest] DIGESTION_CTX allocated\n");

    ctxHeap->cbSize = sizeof(DIGESTION_CTX);
    ctxHeap->dwChunkSize = 65536;
    ctxHeap->dwThreads = 0;
    ctxHeap->dwFlags = 0x1 | 0x2;
    ctxHeap->hCompletionEvent = nullptr;
    ctxHeap->hNotify = m_hwndMain;
    ctxHeap->dwTaskId = g_digestionTaskCounter.fetch_add(1, std::memory_order_relaxed);
    const DWORD taskId = ctxHeap->dwTaskId;

    // Notify autonomous agent
    AutonomousAgent::Instance()->OnDigestionQueued(taskId, source);

    if (wcsncpy_s(ctxHeap->szSource, _countof(ctxHeap->szSource), source.c_str(), _TRUNCATE) != 0 ||
        wcsncpy_s(ctxHeap->szOutput, _countof(ctxHeap->szOutput), output.c_str(), _TRUNCATE) != 0) {
        OutputDebugStringA("[IDE-Digest] ERROR: Failed to copy source/output paths\n");
        HeapFree(GetProcessHeap(), 0, ctxHeap);
        MessageBoxA(m_hwndMain, "Failed to prepare digestion paths.", "Digestion", MB_OK | MB_ICONERROR);
        return false;
    }
    
    {
        char dbg[512];
        sprintf_s(dbg, "[IDE-Digest] Paths copied - Task %lu, Source: %S, Output: %S\n", 
                  taskId, ctxHeap->szSource, ctxHeap->szOutput);
        OutputDebugStringA(dbg);
    }

    OutputDebugStringA("[IDE-Digest] SENDING WM_RUN_DIGESTION message\n");
    
    LRESULT result = SendMessageW(
        m_hwndMain,
        WM_RUN_DIGESTION,
        static_cast<WPARAM>(taskId),
        reinterpret_cast<LPARAM>(ctxHeap));

    {
        char dbg[128];
        sprintf_s(dbg, "[IDE-Digest] SendMessageW returned: %Id\n", static_cast<INT_PTR>(result));
        OutputDebugStringA(dbg);
    }

    if (result != S_DIGEST_OK) {
        // WM_RUN_DIGESTION is responsible for freeing ctxHeap on all paths
        WCHAR szMsg[256];
        wsprintfW(szMsg, L"Failed to start digestion (%lu).", static_cast<DWORD>(result));
        OutputDebugStringA("[IDE-Digest] ERROR: WM_RUN_DIGESTION returned non-OK result\n");
        MessageBoxW(m_hwndMain, szMsg, L"Digestion", MB_OK | MB_ICONERROR);
        return false;
    }

    {
        WCHAR szDbg[128];
        wsprintfW(szDbg, L"[IDE-Digest] Digestion Task %u: STARTED\n", taskId);
        OutputDebugStringW(szDbg);
    }

    appendToOutput("Digestion job queued\n", "Output", OutputSeverity::Info);
    OutputDebugStringA("[IDE-Digest] queueDigestionJob EXIT SUCCESS\n");
    return true;
}

// Public wrapper to trigger digestion from external hooks (e.g., CLI startup)
bool Win32IDE::triggerAutoDigest(const std::wstring& source, const std::wstring& output)
{
    return queueDigestionJob(source, output);
}
...
void Win32IDE::onCreate(HWND hwnd)
{
    ...
    m_hwndSplitter = CreateWindowExA(0, "STATIC", "",
        ...
        0, 0, 100, 4, hwnd, HMENU_FROM_ID(IDC_SPLITTER), m_hInstance, nullptr);
    ...
    if (!m_agentLoop) {
        m_agentLoop = std::make_unique<RawrXD::Agentic::AgentLoop>();
        m_agentLoop->SetBridge(m_agenticBridge.get());
        m_agentLoop->Start();
    ...
    if (!m_fileWatcher) {
        ...
        m_fileWatcher->SetCallback([this](const std::string& payload) {
            if (!shouldDeliverWatcherEvent()) {
                LOG_DEBUG("File watcher event throttled due to sensitivity settings");
                return;
            }
            if (m_hwndMain) {
                PostMessageA(m_hwndMain, WM_AGENT_PERCEPTION, 0, reinterpret_cast<LPARAM>(_strdup(payload.c_str())));
            }
        });
        std::wstring watchPath = L".\\src";
        if (!m_fileWatcher->Start(watchPath)) {
            LOG_WARNING("IocpFileWatcher failed to start");
        }
    }

    RawrXD::Agent::ToolRegistry::Instance().SetConsentCallback([this](const RawrXD::Agent::ToolDefinition& tool, const nlohmann::json&) {
        std::string message = "Allow tool execution?\n\nTool: " + tool.name + "\nCategory: " + tool.category;
        return ShowConsentPrompt(m_hwndMain, message);
    });
    
    // Update status bar with agent status indicators
...
void Win32IDE::onCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
    ...
    // Tools menu
    case IDM_RUN_DIGESTION:
        {
            OutputDebugStringA("[IDE-Digest] Menu Command: IDM_RUN_DIGESTION triggered\n");
            if (m_currentFile.empty()) {
                MessageBoxA(m_hwndMain, "Open a file before running digestion.", "Digestion", MB_OK | MB_ICONWARNING);
                return;
            }
    ...
    case IDM_AGENT_STOP:
        onAgentStop();
        break;
    case IDM_AGENT_TOGGLE_WATCHER:
        onAgentToggleWatcher();
        break;
    case IDM_AGENT_TOGGLE_CONSENT:
        onAgentToggleConsent();
        break;
    case IDM_AGENT_WATCHER_SENSITIVITY_HIGH:
        setWatcherSensitivity(0);
        appendToOutput("✓ File watcher sensitivity set to HIGH (every event triggers the agent)\n", "Output", OutputSeverity::Info);
        break;
    case IDM_AGENT_WATCHER_SENSITIVITY_MEDIUM:
        setWatcherSensitivity(1);
        appendToOutput("✓ File watcher sensitivity set to MEDIUM (batched events)\n", "Output", OutputSeverity::Info);
        break;
    case IDM_AGENT_WATCHER_SENSITIVITY_LOW:
        setWatcherSensitivity(2);
        appendToOutput("✓ File watcher sensitivity set to LOW (coalesced, 1s throttle)\n", "Output", OutputSeverity::Info);
        break;
    case IDM_AGENT_CONSENT_AUTO:
        appendToOutput("✓ Consent level set to AUTOMATIC. Agent will run tools without prompting.\n", "Output", OutputSeverity::Info);
        appendToOutput("⚠️ WARNING: Automatic consent grants full autonomy - use with caution.\n", "Output", OutputSeverity::Warning);
        setConsentLevel(0);
        updateAgentStatusIndicators();
        break;
    case IDM_AGENT_CONSENT_PROMPT:
        appendToOutput("✓ Consent level set to PROMPT. Agent will ask before executing tools.\n", "Output", OutputSeverity::Info);
        setConsentLevel(1);
        updateAgentStatusIndicators();
        break;
    case IDM_AGENT_CONSENT_DENY:
        appendToOutput("✓ Consent level set to DENY. Agent will not execute tools.\n", "Output", OutputSeverity::Info);
        setConsentLevel(2);
        updateAgentStatusIndicators();
        break;
...
void Win32IDE::createMenuBar(HWND hwnd)
{
    ...
    AppendMenuA(hToolsMenu, MF_STRING, IDM_RUN_DIGESTION, "Run &Digestion...\tCtrl+Shift+D");
    ...
    AppendMenuA(hAgentMenu, MF_STRING, IDM_AGENT_TOGGLE_WATCHER, "Toggle &File Watcher");
    AppendMenuA(hAgentMenu, MF_STRING, IDM_AGENT_TOGGLE_CONSENT, "Toggle &Consent Prompts");
    // Watcher sensitivity submenu
    HMENU hWatcherSensitivityMenu = CreatePopupMenu();
    AppendMenuA(hWatcherSensitivityMenu, MF_STRING, IDM_AGENT_WATCHER_SENSITIVITY_HIGH, "High (every change)");
    AppendMenuA(hWatcherSensitivityMenu, MF_STRING, IDM_AGENT_WATCHER_SENSITIVITY_MEDIUM, "Medium (batch frequent events)");
    AppendMenuA(hWatcherSensitivityMenu, MF_STRING, IDM_AGENT_WATCHER_SENSITIVITY_LOW, "Low (throttle to 1s)");
    AppendMenuA(hAgentMenu, MF_POPUP, (UINT_PTR)hWatcherSensitivityMenu, "Watcher Sensitivity");
    // Consent level submenu
    HMENU hConsentLevelMenu = CreatePopupMenu();
    AppendMenuA(hConsentLevelMenu, MF_STRING, IDM_AGENT_CONSENT_AUTO, "Automatic (no prompts)");
    AppendMenuA(hConsentLevelMenu, MF_STRING, IDM_AGENT_CONSENT_PROMPT, "Prompt before tool execution");
    AppendMenuA(hConsentLevelMenu, MF_STRING, IDM_AGENT_CONSENT_DENY, "Deny all tool execution");
    AppendMenuA(hAgentMenu, MF_POPUP, (UINT_PTR)hConsentLevelMenu, "Consent Level");
...
void Win32IDE::createTitleBarControls()
{
    ...
    m_hwndTitleLabel = CreateWindowExA(0, "STATIC", "RawrXD IDE", labelStyle,
                                      0, 0, 200, 24, m_hwndToolbar, HMENU_FROM_ID(IDC_TITLE_TEXT), m_hInstance, nullptr);
    ...
    auto createButton = [&](HWND& target, int controlId, const char* caption) {
        target = CreateWindowExA(0, "BUTTON", caption, buttonStyle,
                                 0, 0, 32, 24, m_hwndToolbar, HMENU_FROM_ID(controlId), m_hInstance, nullptr);
    };
...
void Win32IDE::createEditor(HWND hwnd)
{
    ...
    m_hwndEditor = CreateWindowExA(WS_EX_CLIENTEDGE, RICHEDIT_CLASSA, "",
                                  ...
                                  0, 0, 0, 0, hwnd, HMENU_FROM_ID(IDC_EDITOR), m_hInstance, nullptr);
    if (!m_hwndEditor) {
...
void Win32IDE::createTerminal(HWND hwnd)
{
    ...
    m_hwndCommandInput = CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", "",
                                        ...
                                        0, 0, 0, 0, hwnd, HMENU_FROM_ID(IDC_COMMAND_INPUT), m_hInstance, nullptr);
...
void Win32IDE::createStatusBar(HWND hwnd)
{
    ...
    m_hwndStatusBar = CreateWindowExA(0, STATUSCLASSNAMEA, "",
                                     WS_CHILD | WS_VISIBLE,
                                     0, 0, 0, 0, hwnd, HMENU_FROM_ID(IDC_STATUS_BAR), m_hInstance, nullptr);
    if (!m_hwndStatusBar) {
...
void Win32IDE::createOutputTabs()
{
    ...
    m_hwndOutputTabs = CreateWindowExA(0, WC_TABCONTROLA, "",
        ...
        m_hwndMain, HMENU_FROM_ID(IDC_OUTPUT_TABS), m_hInstance, nullptr);

    // Add severity filter dropdown
    m_hwndSeverityFilter = CreateWindowExA(0, "COMBOBOX", "",
        WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL,
        client.right - 145, 2, 140, 100,
        m_hwndMain, HMENU_FROM_ID(IDC_SEVERITY_FILTER), m_hInstance, nullptr);
...
void Win32IDE::showFindDialog()
{
    ...
    if (!m_hwndFindDialog) {
        ...
        CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", m_lastSearchText.c_str(),
            ...
            hwndDlg, HMENU_FROM_ID(IDC_FIND_TEXT), m_hInstance, nullptr);
        
        CreateWindowExA(0, "BUTTON", "Case sensitive", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
            10, 45, 120, 20, hwndDlg, HMENU_FROM_ID(IDC_CASE_SENSITIVE), m_hInstance, nullptr);
        CreateWindowExA(0, "BUTTON", "Whole word", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
            140, 45, 100, 20, hwndDlg, HMENU_FROM_ID(IDC_WHOLE_WORD), m_hInstance, nullptr);
        CreateWindowExA(0, "BUTTON", "Regex", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
            250, 45, 70, 20, hwndDlg, HMENU_FROM_ID(IDC_USE_REGEX), m_hInstance, nullptr);
        
        CreateWindowExA(0, "BUTTON", "Find Next", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON,
            10, 80, 90, 28, hwndDlg, HMENU_FROM_ID(IDC_BTN_FIND_NEXT), m_hInstance, nullptr);
        CreateWindowExA(0, "BUTTON", "Close", WS_CHILD | WS_VISIBLE,
            110, 80, 90, 28, hwndDlg, HMENU_FROM_ID(IDC_BTN_CLOSE), m_hInstance, nullptr);
    }
...
void Win32IDE::showReplaceDialog()
{
    ...
    CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", m_lastReplaceText.c_str(),
        ...
        hwndDlg, HMENU_FROM_ID(IDC_REPLACE_TEXT), m_hInstance, nullptr);
    
    CreateWindowExA(0, "BUTTON", "Case sensitive", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
        10, 75, 120, 20, hwndDlg, HMENU_FROM_ID(IDC_CASE_SENSITIVE), m_hInstance, nullptr);
    CreateWindowExA(0, "BUTTON", "Whole word", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
        140, 75, 100, 20, hwndDlg, HMENU_FROM_ID(IDC_WHOLE_WORD), m_hInstance, nullptr);
    CreateWindowExA(0, "BUTTON", "Regex", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
        250, 75, 70, 20, hwndDlg, HMENU_FROM_ID(IDC_USE_REGEX), m_hInstance, nullptr);
    
    CreateWindowExA(0, "BUTTON", "Find Next", WS_CHILD | WS_VISIBLE,
        10, 110, 90, 28, hwndDlg, HMENU_FROM_ID(IDC_BTN_FIND_NEXT), m_hInstance, nullptr);
    CreateWindowExA(0, "BUTTON", "Replace", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON,
        110, 110, 90, 28, hwndDlg, HMENU_FROM_ID(IDC_BTN_REPLACE), m_hInstance, nullptr);
    CreateWindowExA(0, "BUTTON", "Replace All", WS_CHILD | WS_VISIBLE,
        210, 110, 90, 28, hwndDlg, HMENU_FROM_ID(IDC_BTN_REPLACE_ALL), m_hInstance, nullptr);
    CreateWindowExA(0, "BUTTON", "Close", WS_CHILD | WS_VISIBLE,
        310, 110, 70, 28, hwndDlg, HMENU_FROM_ID(IDC_BTN_CLOSE), m_hInstance, nullptr);
...
void Win32IDE::showSnippetManager()
{
    ...
    HWND hwndList = CreateWindowExA(WS_EX_CLIENTEDGE, "LISTBOX", "",
        ...
        10, 35, 150, 400, hwndDlg, HMENU_FROM_ID(IDC_SNIPPET_LIST_DLG), m_hInstance, nullptr);
    ...
    CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", "",
        WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
        230, 8, 350, 22, hwndDlg, HMENU_FROM_ID(IDC_SNIPPET_NAME), m_hInstance, nullptr);
    
    CreateWindowExA(0, "STATIC", "Description:", WS_CHILD | WS_VISIBLE,
        175, 40, 70, 20, hwndDlg, nullptr, m_hInstance, nullptr);
    CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", "",
        WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
        175, 60, 405, 22, hwndDlg, HMENU_FROM_ID(IDC_SNIPPET_DESC), m_hInstance, nullptr);
    ...
    CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", "",
        ...
        175, 115, 405, 280, hwndDlg, HMENU_FROM_ID(IDC_SNIPPET_CODE), m_hInstance, nullptr);
    
    // Buttons
    CreateWindowExA(0, "BUTTON", "Insert", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON,
        175, 410, 90, 28, hwndDlg, HMENU_FROM_ID(IDC_BTN_INSERT_SNIPPET), m_hInstance, nullptr);
    CreateWindowExA(0, "BUTTON", "New", WS_CHILD | WS_VISIBLE,
        275, 410, 90, 28, hwndDlg, HMENU_FROM_ID(IDC_BTN_NEW_SNIPPET), m_hInstance, nullptr);
    CreateWindowExA(0, "BUTTON", "Delete", WS_CHILD | WS_VISIBLE,
        375, 410, 90, 28, hwndDlg, HMENU_FROM_ID(IDC_BTN_DELETE_SNIPPET), m_hInstance, nullptr);
    CreateWindowExA(0, "BUTTON", "Save & Close", WS_CHILD | WS_VISIBLE,
        475, 410, 105, 28, hwndDlg, HMENU_FROM_ID(IDC_BTN_SAVE_SNIPPETS), m_hInstance, nullptr);
...
void Win32IDE::createFileExplorer(HWND hwndParent)
{
    ...
    m_hwndFileExplorer = CreateWindowExA(
        ...
        hwndParent,
        HMENU_FROM_ID(IDC_FILE_EXPLORER),
...
void Win32IDE::showCommitDialog()
{
    ...
    HWND hwndCommitBtn = CreateWindowExA(0, "BUTTON", "Commit", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON,
        10, 145, 100, 30, hwndDlg, HMENU_FROM_ID(1), m_hInstance, nullptr);
    
    HWND hwndCancelBtn = CreateWindowExA(0, "BUTTON", "Cancel", WS_CHILD | WS_VISIBLE,
        120, 145, 100, 30, hwndDlg, HMENU_FROM_ID(2), m_hInstance, nullptr);
...
std::string Win32IDE::generateResponse(const std::string& prompt)
{
    ...
    auto performOllama = [&](const std::string& promptText) -> std::string {
        ...
        appendToOutput("Ollama raw response: " + raw, "Debug", OutputSeverity::Debug);
...
void Win32IDE::createChatPanel() {
    ...
    m_hwndSecondarySidebar = CreateWindowExA(
        ...
        WS_CHILD | WS_VISIBLE,
        0, 0, 300, 600,
        m_hwndMain, HMENU_FROM_ID(IDC_SECONDARY_SIDEBAR), m_hInstance, nullptr);
    ...
    m_hwndModelSelector = CreateWindowExA(
        ...
        60, 35, 235, 200,
        m_hwndSecondarySidebar, HMENU_FROM_ID(IDC_COPILOT_SEND_BTN), m_hInstance, nullptr);
    ...
    m_hwndMaxTokensSlider = CreateWindowExA(
        ...
        5, 80, 290, 25,
        m_hwndSecondarySidebar, HMENU_FROM_ID(IDC_COPILOT_CLEAR_BTN), m_hInstance, nullptr);
    ...
    m_hwndCopilotChatOutput = CreateWindowExA(
        ...
        5, 110, 290, 300,
        m_hwndSecondarySidebar, HMENU_FROM_ID(IDC_COPILOT_CHAT_OUTPUT), m_hInstance, nullptr);
    ...
    m_hwndCopilotChatInput = CreateWindowExA(
        ...
        WS_CHILD | WS_VISIBLE | ES_MULTILINE | ES_AUTOVSCROLL | ES_WANTRETURN | WS_VSCROLL,
        5, 415, 290, 85,
        m_hwndSecondarySidebar, HMENU_FROM_ID(IDC_COPILOT_CHAT_INPUT), m_hInstance, nullptr);
...
void Win32IDE::onAgentToggleWatcher() {
    if (!m_fileWatcher) {
        appendToOutput("File watcher not initialized", "Output", OutputSeverity::Warning);
        return;
    }
    
    bool isRunning = m_fileWatcher->IsRunning();
    
    if (isRunning) {
        m_fileWatcher->Stop();
        appendToOutput("✓ File watcher STOPPED\n", "Output", OutputSeverity::Info);
        MessageBoxA(m_hwndMain, "File watcher stopped. Agent will not be triggered by file changes.", "Watcher Control", MB_OK | MB_ICONINFORMATION);
        updateAgentStatusIndicators();
    } else {
        // Show folder browser dialog to let user select watch directory
        BROWSEINFOW bi = {};
        bi.hwndOwner = m_hwndMain;
        bi.ulFlags = BIF_RETURNONLYFSDIRS | BIF_NEWDIALOGSTYLE;
        bi.lpszTitle = L"Select directory to monitor for file changes:";
        
        PIDLIST_ABSOLUTE pidl = SHBrowseForFolderW(&bi);
        std::wstring watchPath = L".\\src"; // Default fallback
        ...
        if (pidl) {
            ...
            if (SHGetPathFromIDListW(pidl, szPath)) {
                ...
                CoTaskMemFree(pidl);
                appendToOutput("✗ Failed to get path from folder browser\n", "Errors", OutputSeverity::Error);
                return;
            }
        } else {
            // User cancelled - use default
            appendToOutput("ℹ File watcher dialog cancelled, using default path (.\\src)\n", "Output", OutputSeverity::Info);
        }
        
        if (m_fileWatcher->Start(watchPath)) {
            std::string pathStr(watchPath.begin(), watchPath.end());
            std::string msg = "✓ File watcher STARTED (monitoring " + pathStr + ")\n";
            appendToOutput(msg, "Output", OutputSeverity::Info);
            
            std::string msgBox = "File watcher started.\nMonitoring: " + pathStr;
            MessageBoxW(m_hwndMain, (L"File watcher started.\nMonitoring: " + watchPath).c_str(), 
                       L"Watcher Control", MB_OK | MB_ICONINFORMATION);
            
            // Save path and update status
            std::string narrowPath(watchPath.begin(), watchPath.end());
            setWatchPath(narrowPath);
            updateAgentStatusIndicators();
        } else {
            appendToOutput("✗ Failed to start file watcher on selected path\n", "Errors", OutputSeverity::Error);
            MessageBoxA(m_hwndMain, "Failed to start file watcher on selected path", "Error", MB_OK | MB_ICONERROR);
        }
    }
}

void Win32IDE::onAgentToggleConsent() {
    // Use TaskDialog for a more modern, styled dialog
    TASKDIALOGCONFIG config = {};
    config.cbSize = sizeof(config);
    config.hwndParent = m_hwndMain;
    config.hInstance = m_hInstance;
    config.dwFlags = TDF_ALLOW_DIALOG_CANCELLATION | TDF_USE_COMMAND_LINKS;
    config.pszTitle = L"Agent Consent Level";
    config.pszMainInstruction = L"Select Agent Autonomy Level";
    config.pszContent = L"Choose how the autonomous agent should handle tool execution:\n\n"
                        L"🟢 Automatic: Execute tools immediately without prompting (fastest)\n"
                        L"🟡 Prompt: Ask for confirmation before each tool execution (safe)\n"
                        L"🔴 Deny: Block all tool executions (most restrictive)";
    ...
    config.cButtons = 3;
    config.nDefaultButton = 101; // Default to Prompt
    
    int nClickedBtn = 101;
    if (SUCCEEDED(TaskDialogIndirect(&config, &nClickedBtn, nullptr, nullptr))) {
        // Map button to consent level
        int newLevel = -1;
        switch (nClickedBtn) {
            case 100: newLevel = 0; break; // Automatic
            case 101: newLevel = 1; break; // Prompt
            case 102: newLevel = 2; break; // Deny
        }
        
        if (newLevel >= 0) {
            const char* levels[] = {"Automatic (no prompts)", "Prompt for each tool", "Always deny (blocked)"};
            std::string msg = "Consent level changed to: " + std::string(levels[newLevel]) + "\n";
            appendToOutput(msg, "Output", OutputSeverity::Info);
            
            if (newLevel == 0) {
                appendToOutput("⚠️  WARNING: Agent will execute tools WITHOUT prompting. Use with caution!\n", "Output", OutputSeverity::Warning);
            } else if (newLevel == 1) {
                appendToOutput("✓ Agent will prompt for confirmation before executing tools\n", "Output", OutputSeverity::Info);
            } else {
                appendToOutput("✓ Agent tool execution is BLOCKED (consent level: deny)\n", "Output", OutputSeverity::Info);
            }
            
            // Save consent level and update status
...
std::string Win32IDE::getSettingsPath() const
{
    // Get AppData local folder for settings
    wchar_t szPath[MAX_PATH] = {};
    if (SUCCEEDED(SHGetFolderPathW(nullptr, CSIDL_LOCAL_APPDATA, nullptr, 0, szPath))) {
        std::wstring appDataPath(szPath);
        appDataPath += L"\\RawrXD";
        
        // Create directory if it doesn't exist
        CreateDirectoryW(appDataPath.c_str(), nullptr);
        
        // Convert to string and return
        std::string result(appDataPath.begin(), appDataPath.end());
        result += "\\agent_settings.json";
        return result;
    }
    return "agent_settings.json"; // Fallback to current directory
}
...
void Win32IDE::loadSettings()
{
    ...
    if (!file.is_open()) {
        ...
        m_consentLevel = 1; // Default to Prompt
        m_watchPath = ".\\src";
        m_watcherSensitivity = 1;
        appendToOutput("ℹ Settings file not found - using defaults\n", "Output", OutputSeverity::Info);
        return;
    }
    ...
    try {
        ...
        // Simple JSON parsing (if nlohmann::json is available)
        // For now, use a simpler string-based approach
        size_t consentPos = content.find("\"consentLevel\":");
        if (consentPos != std::string::npos) {
            size_t valueStart = content.find(':', consentPos) + 1;
            size_t valueEnd = content.find(',', valueStart);
            std::string consentStr = content.substr(valueStart, valueEnd - valueStart);
            // Trim whitespace
            consentStr.erase(0, consentStr.find_first_not_of(" \t\n\r"));
            consentStr.erase(consentStr.find_last_not_of(" \t\n\r") + 1);
            m_consentLevel = std::stoi(consentStr);
        } else {
            m_consentLevel = 1;
        }
        
        size_t pathPos = content.find("\"watchPath\":");
        if (pathPos != std::string::npos) {
            size_t valueStart = content.find('\"', pathPos + 14) + 1;
            size_t valueEnd = content.find('\"', valueStart);
            m_watchPath = content.substr(valueStart, valueEnd - valueStart);
        } else {
            m_watchPath = ".\\src";
        }

        size_t sensitivityPos = content.find("\"watcherSensitivity\":");
        if (sensitivityPos != std::string::npos) {
            size_t valueStart = content.find(':', sensitivityPos) + 1;
            size_t valueEnd = content.find_first_of(",}", valueStart);
            if (valueEnd == std::string::npos) {
                valueEnd = content.size();
            }
            std::string sensitivityStr = content.substr(valueStart, valueEnd - valueStart);
            sensitivityStr.erase(0, sensitivityStr.find_first_not_of(" \t\n\r"));
            sensitivityStr.erase(sensitivityStr.find_last_not_of(" \t\n\r") + 1);
            try {
                int level = std::stoi(sensitivityStr);
                if (level < 0) level = 0;
                if (level > 2) level = 2;
                m_watcherSensitivity = level;
            } catch (...) {
                m_watcherSensitivity = 1;
            }
        } else {
        ...
        appendToOutput("✓ Settings loaded successfully\n", "Output", OutputSeverity::Info);
    } catch (const std::exception& e) {
        appendToOutput(std::string("✗ Error loading settings: ") + e.what() + "\n", "Errors", OutputSeverity::Error);
        m_consentLevel = 1;
        m_watchPath = ".\\src";
        m_watcherSensitivity = 1;
    }
...
void Win32IDE::saveSettings()
{
    ...
    try {
        ...
        if (!file.is_open()) {
            appendToOutput("✗ Failed to open settings file for writing\n", "Errors", OutputSeverity::Error);
            return;
        }
        
        // Write JSON settings
        file << "{\n";
        file << "  \"consentLevel\": " << m_consentLevel << ",\n";
        file << "  \"watcherSensitivity\": " << m_watcherSensitivity << ",\n";
        file << "  \"watchPath\": \"" << m_watchPath << "\",\n";
        file << "  \"lastSaved\": \"" << std::chrono::system_clock::now().time_since_epoch().count() << "\"\n";
        file << "}\n";
        file.close();
        
        appendToOutput("✓ Settings saved to: " + settingsPath + "\n", "Output", OutputSeverity::Info);
    } catch (const std::exception& e) {
        appendToOutput(std::string("✗ Error saving settings: ") + e.what() + "\n", "Errors", OutputSeverity::Error);
    }
}

int Win32IDE::getConsentLevel() const
{
    return m_consentLevel;
}

void Win32IDE::setConsentLevel(int level)
{
    if (level >= 0 && level <= 2) {
...
int Win32IDE::getWatcherSensitivity() const
{
    return m_watcherSensitivity;
}

void Win32IDE::setWatcherSensitivity(int level)
{
    if (level < 0 || level > 2) {
        return;
    }
...
bool Win32IDE::shouldDeliverWatcherEvent()
{
    using namespace std::chrono;
    int thresholdMs = 0;
    switch (m_watcherSensitivity) {
        case 2: thresholdMs = 1000; break; // Low sensitivity, throttle to 1s
        case 1: thresholdMs = 250; break;  // Medium sensitivity
        default: thresholdMs = 0; break;   // High sensitivity, no throttling
    }

    auto now = steady_clock::now();
    std::lock_guard<std::mutex> guard(m_watcherThrottleMutex);
    if (thresholdMs == 0 || m_lastWatcherNotification == steady_clock::time_point::min()) {
        m_lastWatcherNotification = now;
        return true;
    }

    auto elapsedMs = duration_cast<milliseconds>(now - m_lastWatcherNotification).count();
...
std::string Win32IDE::getWatchPath() const
{
    return m_watchPath;
}

void Win32IDE::setWatchPath(const std::string& path)
{
    m_watchPath = path;
    saveSettings();
}

void Win32IDE::updateAgentStatusIndicators()
{
    if (!m_hwndStatusBar) return;
    
    std::string watcherStatus;
    if (m_fileWatcher && m_fileWatcher->IsRunning()) {
        watcherStatus = "👁️ Watcher: ON";
    } else {
        watcherStatus = "🚫 Watcher: OFF";
    }
    
    const char* consentLevels[] = {"Automatic", "Prompt", "Deny"};
    std::string consentStatus = "🔒 Consent: ";
    if (m_consentLevel >= 0 && m_consentLevel <= 2) {
        consentStatus += consentLevels[m_consentLevel];
    } else {
        consentStatus += "Unknown";
    }

    const char* sensitivityLabels[] = {"High", "Medium", "Low"};
    std::string sensitivityStatus = "⚙️ Sensitivity: ";
    if (m_watcherSensitivity >= 0 && m_watcherSensitivity <= 2) {
        sensitivityStatus += sensitivityLabels[m_watcherSensitivity];
    } else {
        sensitivityStatus += "Custom";
    }
    
    // Update status bar part 2 (index 2) with agent status
    std::string fullStatus = watcherStatus + " | " + sensitivityStatus + " | " + consentStatus;
    SendMessageA(m_hwndStatusBar, SB_SETTEXT, 2, (LPARAM)fullStatus.c_str());
}
