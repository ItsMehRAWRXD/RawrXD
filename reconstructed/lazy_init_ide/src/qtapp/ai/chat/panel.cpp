...
AgentChatBreadcrumb* AIChatPanel::getBreadcrumb() const {
    return m_breadcrumb;
}
...
void AIChatPanel::setSelectedModel(const QString& modelName)
{
    ...
    if (m_inputField) {
        const QString label = m_selectedModel.isEmpty()
            ? tr("Ask AI anything...")
            : tr("Ask %1...").arg(m_selectedModel);
        m_inputField->setPlaceholderText(label);
    }
}
