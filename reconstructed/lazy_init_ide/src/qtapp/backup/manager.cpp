...
#if defined(HAVE_ZLIB) && __has_include(<zlib.h>)
...
#if defined(_MSC_VER)
#pragma message("zlib not found - BackupManager will store uncompressed backups")
#else
...
bool copyRecursively(const QString& srcPath, const QString& dstPath) {
    ...
    if (srcInfo.isDir()) {
        ...
        for (const QFileInfo& entry : entries) {
            const QString dstEntry = dstPath + "/" + entry.fileName();
            if (!copyRecursively(entry.filePath(), dstEntry)) {
                return false;
            }
        }
        return true;
...
bool BackupManager::compressBackup(const QString& srcPath, const QString& dstPath) {
    ...
    QFileInfo srcInfo(srcPath);
    if (!srcInfo.exists()) {
        qWarning() << "[BackupManager] Source path missing:" << srcPath;
        return false;
    }

    qInfo() << "[BackupManager] zlib unavailable - storing uncompressed backup at" << dstPath;
...
bool BackupManager::decompressBackup(const QString& srcPath, const QString& dstPath) {
    ...
    QFileInfo srcInfo(srcPath);
    if (!srcInfo.exists()) {
        qWarning() << "[BackupManager] Backup path missing:" << srcPath;
        return false;
    }

    qInfo() << "[BackupManager] zlib unavailable - restoring uncompressed backup from" << srcPath;
    return copyRecursively(srcPath, dstPath);
#endif
}
