...
void MetricsCollector::incrementCounter(const QString& name, qint64 value) {
    if (!m_enabled) return;

    QMutexLocker locker(&m_mutex);
    m_counters[name] = m_counters.value(name, 0) + value;
    emit metricsUpdated();
}

void MetricsCollector::recordLatency(const QString& name, qint64 milliseconds) {
    if (!m_enabled) return;

    QMutexLocker locker(&m_mutex);
    // Store latency as a counter for now - could be extended with histogram support
    m_counters[name + "_total"] = m_counters.value(name + "_total", 0) + milliseconds;
    m_counters[name + "_count"] = m_counters.value(name + "_count", 0) + 1;
    emit metricsUpdated();
}
...
QString MetricsCollector::exportToJson() const {
    ...
    if (!m_counters.isEmpty()) {
        ...
        for (auto it = m_counters.constBegin(); it != m_counters.constEnd(); ++it) {
            countersObj.insert(it.key(), static_cast<qint64>(it.value()));
        }
        root["counters"] = countersObj;
    }
