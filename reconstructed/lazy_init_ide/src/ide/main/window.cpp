...
IDEMainWindow::IDEMainWindow(QWidget *parent)
    ...
    // Load settings
    loadSettings();
    // Initialize Overclock settings (CLI parity) - use default config paths
    Settings::LoadCompute(ocState, "compute.ini");
    Settings::LoadOverclock(ocState, "overclock.ini");
...
void IDEMainWindow::onSaveOverclockSettings() {
    Settings::SaveCompute(ocState, "compute.ini");
    Settings::SaveOverclock(ocState, "overclock.ini");
