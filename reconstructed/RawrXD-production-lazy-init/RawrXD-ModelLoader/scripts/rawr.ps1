...
function Invoke-Repl {
    ...
    while ($true) {
        ...
        switch ($cmd) {
            ...
            "mission" { Invoke-Mission ($args -join " ") }
            "plan" { Invoke-Plan ($args -join " ") }
            "execute_plan" { Invoke-ExecutePlan }
            "registry" { Invoke-Registry }
            "hotpatch" { Invoke-Hotpatch $args }
            "selftest" { Invoke-SelfTest }
            "telemetry" { Invoke-Telemetry }
            "governor" { Invoke-Governor $args }
            "settings" { Invoke-Settings }
            "verbose" { Set-Verbose $args[0] }
            # Original tool commands
...
if ($Command) {
    ...
    switch ($cmd) {
        ...
        # New agentic commands
        "mission" { Invoke-Mission ($Arguments -join " ") }
        "plan" { Invoke-Plan ($Arguments -join " ") }
        "execute_plan" { Invoke-ExecutePlan }
        "registry" { Invoke-Registry }
        "hotpatch" { Invoke-Hotpatch $Arguments }
        "selftest" { Invoke-SelfTest }
        "telemetry" { Invoke-Telemetry }
        "governor" { Invoke-Governor $Arguments }
        "settings" { Invoke-Settings }
        "verbose" { Set-Verbose $Arguments[0] }
        # Original tool commands
        "tools" { Invoke-ToolList }
