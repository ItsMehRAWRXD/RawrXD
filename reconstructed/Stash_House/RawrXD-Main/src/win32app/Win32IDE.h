...
// AI Backend Debug / Test command IDs (4115–4119)
#define IDM_DEBUG_TEST_AI 4115
...
#define IDC_AI_RESULT 1001
#define IDD_AI_TEST 200
...
public:
    ...
    // ========================================================================
    // AI BACKEND VERIFICATION & TESTING
    // Verify Ollama connectivity, list available models, test inference
    // ========================================================================
    bool m_aiAvailable = false;
    static constexpr UINT WM_AI_BACKEND_VERIFIED = WM_APP + 102;
    static constexpr UINT WM_AI_BACKEND_FAILED = WM_APP + 103;
    
    bool VerifyAIBackend();
    void runDebugTestAI();
    void onAIBackendVerified(bool success, const std::string& status);
    void updateToolsMenu();

    // ========================================================================
    // FULL SYSTEM INTEGRATION: Enable All 468+ Components & Wire Together
    // ========================================================================
    bool enableAllFeaturesAndWire();
    void wireAllSubsystems();
    void enableCoreSubsystems();
    void enableAISubsystems();
    void enableAgentSystems();
    void enableBuildSystems();
    void enableAdvancedFeatures();
    bool allFeaturesEnabled() const;
    std::string getLastFeatureError() const;

    // ========================================================================
    // DYNAMIC ORCHESTRATION: Flexible, Rewritable Subsystem Wiring
    // Untie rigid boundaries, swap implementations at runtime, redirect flows
    // ========================================================================
    void initializeDynamicOrchestration();
    void shutdownDynamicOrchestration();
    // Forward declare dynamic orchestrator (implementation in Win32IDE_DynamicOrchestration.cpp)
    class DynamicOrchestrator;
    std::unique_ptr<DynamicOrchestrator> m_dynamicOrch;

    // ========================================================================
    // CONTINUATION WEAVING: Detect & Bridge Incomplete Implementations
    // No artificial boundaries. No rewriting. Just seamless continuation.
    // ========================================================================
    void initializeContinuationWeaving();
    void shutdownContinuationWeaving();
    std::vector<std::string> getIncompleteComponents();
    bool continueIncompleteComponent(const std::string& componentName);
    // Forward declare helpers (implementation in Win32IDE_ContinuationWeaving.cpp)
    class IncompleteWorkDetector;
    class ContinuationWeaver;
    std::unique_ptr<IncompleteWorkDetector> m_incompleteDetector;
    std::unique_ptr<ContinuationWeaver> m_weaver;
    ...
    void openFilePublic(const std::string& filePath) { openFile(filePath); }
    std::string getCurrentFilePathPublic() const { return getCurrentFilePath(); }
