...
ExportCapturedData PROC pPath:PTR BYTE
    ...
    invoke wsprintfA, ADDR buffer, \
           CSTR("Total Calls Intercepted: %llu\n"), \
           g_Interceptor.Stats.TotalCalls
    invoke WriteFile, hFile, ADDR buffer, eax, NULL, NULL
    ...
    ; Write call log entries (iterate through actual captured count)
    mov ecx, 0
    .export_loop:
    cmp ecx, g_Interceptor.CallLogIndex
    jge .export_done
    invoke wsprintfA, ADDR buffer, CSTR("Call %d: PID=%d intercepted\n"), ecx, g_CLIContext.TargetPID
    invoke WriteFile, hFile, ADDR buffer, eax, NULL, NULL
    inc ecx
    jmp .export_loop
    .export_done:
