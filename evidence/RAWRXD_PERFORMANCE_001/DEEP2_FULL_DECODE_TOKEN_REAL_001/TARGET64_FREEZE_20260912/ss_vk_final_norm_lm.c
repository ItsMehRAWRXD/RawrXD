...
static void fn_surv(SsVk *v, const char *when)
{
    SsSurviveSnap s; ss_vk_survive_probe(v, when, 0, &s);
}
                            ...
                            SsFinalNormLm *r)
{
    ...
    free(hw); hw = 0;
    fn_surv(v, "AFTER_LM_WEIGHT_UPLOAD");
    printf("LM_HEAD_UPLOAD_BYTES=%llu rows=%u cols=%u\n",
           (unsigned long long)plan->outputWeight.bytes, rows, cols);
    fflush(stdout);
    if (!v->logitsb) {
    ...
    /* Tiled GEMV: ladder proved ≤65536 survives, 98304 loses device (TDR/size). */
    if (ss_vk_lm_tiled_gemv(v, ww, plan->outputWeight.bytes, rows, cols,
                            ss_vk_codec_ty(plan->outputWeight.codec), 16384u)) {
        r->first_fail = 5; goto fail;
    }
    fn_surv(v, "AFTER_LM_HEAD_GEMV");
