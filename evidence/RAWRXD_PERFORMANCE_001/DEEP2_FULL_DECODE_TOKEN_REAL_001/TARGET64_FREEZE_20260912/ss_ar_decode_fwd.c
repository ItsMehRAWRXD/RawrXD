...
{
    ArUser *u = (ArUser *)user; SsFullBlockLoop FL; SsFinalNormLm FN;
    SsSurviveSnap s; int rc; char when[40];
    if (!u || !u->v || !u->plan || !out_logits || !out_vocab) {
        if (u) return fail_pos(u, 0, "EMBD_FAILED");
        return 0;
    }
    /* #2: never reuse prior sealed logits — every position is a full forward. */
    u->v->logits_op = 0;
    u->sealed0 = 0; u->sealed1 = 0;
    if (pos == 0) u->pos0 = 0;
    if (pos == 1) u->pos1 = 1;
    ...
    if (rc) return fail_pos(u, pos, "EMBD_FAILED");
    u->embd_calls++;
    ...
    if (rc || !FL.pass) return fail_pos(u, pos, "BLOCK_LOOP_FAILED");
    ...
    if (rc || !FN.final_norm_real) return fail_pos(u, pos, "FINAL_RMS_FAILED");
    if (pos == 0) u->tok0_rms = 1;
    if (pos == 1) u->tok1_rms = 1;
    if (!FN.lm_head_real || !FN.logits_real) return fail_pos(u, pos, "LM_HEAD_FAILED");
    ...
    if (!copy_logits(u)) return fail_pos(u, pos, "LM_HEAD_FAILED");
    ...
    *out_vocab = u->vocab;
    ...
    if (!s.device_alive || !s.noop_submit_ok) {
        ...
        return fail_pos(u, pos, "DEVICE_LOST");
    }
    u->post_last_alive = 1;
