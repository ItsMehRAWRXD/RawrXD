...
inline bool GeometryReady(const ModelWeightsT& mw, std::string* why = nullptr) {
    ...
    for (std::size_t i = 0; i < mw.layers.size(); ++i) {
        ...
        const bool hasFfn = WtLive(lw.wDown) && WtLive(lw.wUp) && (WtLive(lw.wGate) || lw.wUp.rows == 2 * lw.wDown.cols);
        if (!hasSplitAttn && !hasFusedAttn) return fail("NO_ATTN_GEOM");
        if (!hasFfn) return fail("NO_FFN_GEOM");
        ++checked;
        if (checked >= 2) break; // enough to clear schema class; full layer checks happen in runtime.
    }
