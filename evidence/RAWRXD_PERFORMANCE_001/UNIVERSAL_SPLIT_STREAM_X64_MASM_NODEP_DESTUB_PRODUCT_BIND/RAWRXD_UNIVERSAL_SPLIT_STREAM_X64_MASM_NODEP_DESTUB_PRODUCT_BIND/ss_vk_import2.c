/* ss_vk_import2.c — second D3D12 HOT import (projection weight) on same device */
#include "ss_vk_api.h"
...
int ss_vk_import2(SsVk *v, void *nt, uint64_t bytes)
{
    ...
    bi.pNext = &ext;
    if (v->a.create_buf(v->dev, &bi, 0, &v->pbuf) != VK_SUCCESS) return 100;
    v->a.buf_req(v->dev, v->pbuf, &req);
    ...
    ded.buffer = v->pbuf; imp.pNext = &ded;
    ...
    if (v->a.alloc_mem(v->dev, &ai, 0, &v->pmem) != VK_SUCCESS) return 100;
    if (v->a.bind_mem(v->dev, v->pbuf, v->pmem, 0) != VK_SUCCESS) return 100;
    v->pbytes = bytes; v->proj_imported = 1;
    return 0;
}
