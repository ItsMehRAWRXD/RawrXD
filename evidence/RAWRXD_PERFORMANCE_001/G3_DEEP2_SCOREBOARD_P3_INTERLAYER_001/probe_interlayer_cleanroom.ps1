...
$EvDir = 'G:\~dev\rawrxd\evidence\RAWRXD_PERFORMANCE_001\G3_DEEP2_SCOREBOARD_P3_INTERLAYER_001'
$OutDir = 'C:\r1cert\smoke_p3_interlayer'
...
if ($runHits.Count -gt 0) {
    ...
    $idx = -1
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -eq 'GATE=G3_DEEP2_SCOREBOARD_P3_INTERLAYER_001') { $idx = $i }
    }
    if ($idx -ge 0) {
...
$block | Set-Content (Join-Path $EvDir 'INTERLAYER_SEAL_BLOCK.txt')
...
$depVal = Get-Field $block "N_PLUS_1_DEP_RELEASE_OBSERVED"
$rdyVal = Get-Field $block "N_PLUS_1_READYEXEC_FROM_SCOREBOARD"
$subVal = Get-Field $block "N_PLUS_1_SUBMIT_FROM_SCOREBOARD"
$ncoVal = Get-Field $block "N_COMPLETION_OBSERVED"
$winsVal = Get-Field $block "INTERLAYER_RELEASE_WINS"
$dupVal = Get-Field $block "INTERLAYER_DUPLICATES_DROPPED"
$seqVal = Get-Field $block "SEQUENTIAL_N_PLUS_1_ISSUE_COUNT"
$ilOwnVal = Get-Field $block "INTER_LAYER_PROGRESS_SCOREBOARD_OWNED"
$f3dVal = Get-Field $block "FENCE3_DEMOTED"
$f3fVal = Get-Field $block "FENCE3_FALLBACK_JOIN"
$f3aVal = Get-Field $block "FENCE3_ATTEMPTS"
$depN=0;[void][int]::TryParse($depVal,[ref]$depN)
$rdyN=0;[void][int]::TryParse($rdyVal,[ref]$rdyN)
$subN=0;[void][int]::TryParse($subVal,[ref]$subN)
$ncoN=0;[void][int]::TryParse($ncoVal,[ref]$ncoN)
$winsN=0;[void][int]::TryParse($winsVal,[ref]$winsN)
$seqN=0;[void][int]::TryParse($seqVal,[ref]$seqN)
$ilOwn = [int]($ilOwnVal -eq "1" -or ($ncoN -gt 0 -and $depN -gt 0 -and $rdyN -gt 0 -and $subN -gt 0 -and $winsN -gt 0))
...
$verdict =
    ...
    elseif ($ilOwn -eq 1 -and $pathLive -eq 1 -and $tok -eq 1 -and $exeMatch) {
        'PASS_P3_INTERLAYER_TIP_SCHEDULER_HELD'
    }
    elseif ($pathLive -eq 1 -and $tok -eq 1 -and $exeMatch) {
        'PASS_MULTIOP_INTERLAYER_PENDING'
    }
...
$receipt = @"
GATE=G3_DEEP2_SCOREBOARD_P3_INTERLAYER_001
KIND=P3_CLEANROOM_INTERLAYER_PROBE
RUN_ID=$runId
EXE_SHA16=$exeSha16
...
N_COMPLETION_OBSERVED=$ncoVal
N_PLUS_1_DEP_RELEASE_OBSERVED=$depVal
N_PLUS_1_READYEXEC_FROM_SCOREBOARD=$rdyVal
N_PLUS_1_SUBMIT_FROM_SCOREBOARD=$subVal
INTERLAYER_RELEASE_WINS=$winsVal
INTERLAYER_DUPLICATES_DROPPED=$dupVal
SEQUENTIAL_N_PLUS_1_ISSUE_COUNT=$seqVal
INTER_LAYER_PROGRESS_SCOREBOARD_OWNED=$ilOwn
FENCE3_ATTEMPTS=$f3aVal
FENCE3_DEMOTED=$f3dVal
FENCE3_FALLBACK_JOIN=$f3fVal
SEQUENTIAL_LAYER_ADVANCE_AUTHORITY=$(if($seqN -gt 0){1}else{0})
...
"@
$receipt | Set-Content (Join-Path $EvDir 'INTERLAYER_CLEANROOM_PROBE_RECEIPT.txt')
Get-Content $serr -Tail 120 | Set-Content (Join-Path $EvDir 'INTERLAYER_server.err.tail.txt')

Write-Output "RUN_ID=$runId EXE_MATCH=$([int]$exeMatch) BINDING=$binding VERDICT=$verdict"
Write-Output "RE=$reVal DEP=$depVal RDY=$rdyVal SUB=$subVal ILOWN=$ilOwn SEQ=$seqVal F3D=$f3dVal F3F=$f3fVal TOK=$tok SCHED=$sched"
Kill-Port
if ($verdict -eq 'ILLEGAL_LIVE_FLIP') { exit 5 }
if ($verdict -eq 'FAIL_GENERATE_HTTP') { exit 6 }
if ($verdict -eq 'P3_PROBE_PARTIAL') { exit 7 }
if ($verdict -eq 'PASS_MULTIOP_INTERLAYER_PENDING') { exit 8 }
exit 0
