...
inline void ClassifyTensorName(DynamicManifest& m, const std::string& name) {
    ...
    } else if (Contains(low, "proj.weight") && !Contains(low, "gate_proj") &&
               !Contains(low, "up_proj") && !Contains(low, "down_proj") &&
               !Contains(low, "o_proj")) {
        lf.projector = name;
        lf.qkv = name; // existing ABI slot can carry projector handle
...
inline void FinalizeLayerFacts(DynamicManifest& m) {
    ...
    for (LayerFact& lf : m.layerFacts) {
        if (lf.attention == AttentionTopology::Unknown) {
            ...
            else if (!lf.qkv.empty())
                lf.attention = AttentionTopology::FusedQKV;
            else if (!lf.projector.empty())
                lf.attention = AttentionTopology::ProjectorBlock;
            else
...
inline DynamicManifest DigestLoadedModel(const std::string& modelPath,
    ...
    // Requires ggufResult.tensors iteration with .name/.type/.dims in existing code.
    for (const auto& it : ggufResult.tensors) {
        const auto& wt = it.second;
        const std::string name = wt.name.empty() ? it.first : wt.name;
    ...
    const std::string out = ManifestCachePath(modelPath, m.fp);
