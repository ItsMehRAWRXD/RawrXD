...
with open(filepath, 'rb') as f:
    ...
    for i in range(metadata_count):
        ...
        if len(key_len_data) != 8:
            print(f"ERROR: Could not read key_len for metadata {i}")
            break
        key_len = struct.unpack('<Q', key_len_data)[0]
        if key_len > 10000:
            print(f"ERROR: key_len {key_len} is too large at metadata {i}, position {f.tell()}")
            break
        key = f.read(key_len)
        value_type_data = f.read(4)
        if len(value_type_data) != 4:
            print(f"ERROR: Could not read value_type for metadata {i}")
            break
    ...
    print(f'Position after metadata: {f.tell()}')
    ...
    # Print first 10
    for t in tensors[:10]:
        print(f"{t['name']}: shape={t['dims']}, dtype={t['dtype']}, offset={t['offset']}")
    ...
    print('Index written to D:\\rawrxd\\models\\test_model.index.bin')
