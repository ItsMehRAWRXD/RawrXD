...
function Resolve-InHouseLinker {
    $candidates = @(
        (Join-Path $rootDir "tools\\inhouse\\bin\\rawrxd_linker.exe"),
        (Join-Path $rootDir "tools\\inhouse\\rawrxd_linker.exe"),
...
function Invoke-LinkOnly {
    ...
    if ($UseInHouseLinker) {
        $inhouse = Resolve-InHouseLinker
        if (-not $inhouse) {
            Write-Host "ERROR: in-house linker not found (rawrxd_linker.exe)." -ForegroundColor Red
            exit 1
        }
        if (-not $EntryPoint) {
            Write-Host "ERROR: -EntryPoint required for -UseInHouseLinker." -ForegroundColor Red
            exit 1
        }

        $filtered = @($LinkInputs | Where-Object { $_ -notmatch '\\.res$' })
        $skippedRes = @($LinkInputs | Where-Object { $_ -match '\\.res$' })
        if ($skippedRes.Count -gt 0) {
            Write-Host "WARN: skipping .res inputs (not supported by in-house linker MVP)" -ForegroundColor Yellow
        }

        $extraLibs = @()
        if (-not $NoDefaultSystemLibs) {
            $linkExeForPaths = $LinkExe
            if (-not $linkExeForPaths) { $linkExeForPaths = Resolve-LinkTool }
            $searchPaths = @()
            $vcLibPath = Resolve-VcLibPath -LinkExe $linkExeForPaths
            if ($vcLibPath) { $searchPaths += $vcLibPath }
            $searchPaths += (Resolve-WindowsSdkLibPaths)

            foreach ($lib in $SystemLibs) {
                $found = $null
                foreach ($p in $searchPaths) {
                    $candidate = Join-Path $p $lib
                    if (Test-Path $candidate) { $found = $candidate; break }
                }
                if ($found) { $extraLibs += $found } else { Write-Host "WARN: system lib not found: $lib" -ForegroundColor Yellow }
            }
        }

        $args = @("--out", $OutputExe, "--entry", $EntryPoint, "--subsystem", $Subsystem, "--in") + $filtered + $extraLibs
        Write-Host "[LinkOnly/InHouse] $OutputExe" -ForegroundColor Cyan
        Write-Host "  linker: $inhouse" -ForegroundColor DarkGray
        & $inhouse @args
        if ($LASTEXITCODE -ne 0) {
            Write-Host "ERROR: in-house linker failed ($LASTEXITCODE)." -ForegroundColor Red
            exit $LASTEXITCODE
        }
