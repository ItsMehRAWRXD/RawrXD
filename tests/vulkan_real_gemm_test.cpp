// ============================================================================
// RawrXD Standalone Real-Dimension Vulkan GEMM Test
// Purpose: Prove compute shader GEMM at actual transformer dimensions without
//          any RawrXD dependencies. Uses synthetic data matching Qwen scales.
// ============================================================================
...
static bool createContext(VulkanContext& ctx) {
    ...
    appInfo.pApplicationName = "RawrXD Real GEMM Test";
...
bool RunRealGemmTest(uint32_t M, uint32_t N, uint32_t K) {
    printf("=================================================================\n");
    printf("RawrXD Real-Dimension Vulkan GEMM Test\n");
    printf("=================================================================\n");
    printf("[REAL-GEMM] Dimensions: A[%u×%u] × B[%u×%u] = C[%u×%u]\n", M, K, K, N, M, N);
    ...
    printf("[REAL-GEMM] CPU reference computed\n");
    ...
    // Record upload + compute + readback
    ...
    if (spirv.empty()) {
        printf("[REAL-GEMM] Failed to load SPIR-V\n");
        return false;
    }
    ...
    printf("[REAL-GEMM] GPU compute dispatched and completed\n");
    ...
    for (size_t i = 0; i < static_cast<size_t>(M) * N; ++i) {
        ...
        if (err > 1e-4f) {
            if (mismatchCount < 5) {
                printf("[REAL-GEMM] Mismatch at [%zu]: CPU=%.6f GPU=%.6f err=%.6f\n", i, cpuC[i], gpuC[i], err);
            }
            mismatchCount++;
            ok = false;
        }
    ...
    if (ok) {
        printf("[REAL-GEMM] Verification PASS: all %u elements match (max error=%.6e)\n", M * N, maxError);
    } else {
    ...
    printf("=================================================================\n");
    printf("REAL-DIMENSION GEMM TEST: %s\n", ok ? "PASS" : "FAIL");
    printf("  - Context creation\n");
    printf("  - Device-local buffer allocation\n");
    printf("  - Staging upload (A, B)\n");
    printf("  - SPIR-V compute pipeline creation\n");
    printf("  - Descriptor set + push constants\n");
    printf("  - vkCmdDispatch at %u×%u×%u\n", M, N, K);
    printf("  - Memory barrier + readback\n");
    printf("  - CPU reference comparison\n");
    printf("=================================================================\n");
    return ok;
}
...
int main(int argc, char** argv) {
    (void)argc; (void)argv;
    // Default: 4096×4096×4096 (realistic transformer layer GEMM)
    // Can override with args: vulkan_real_gemm_test.exe 32000 4096 1
