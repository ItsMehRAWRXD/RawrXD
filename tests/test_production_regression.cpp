...
using AEHRecoveryStrategy = AgenticErrorHandler::RecoveryStrategy;
...
static void test_observability() {
    ...
    TEST(tracing_start_end_span)
        auto& obs = AgenticObservability::instance();
        obs.setSpanCallback([](const std::string& /*spanId*/, void*) {
            spanCallbackFired = true;
        }, nullptr);
        std::string traceId = obs.startTrace("test-trace");
        ASSERT_TRUE(!traceId.empty());
        std::string spanId = obs.startSpan("test-span", traceId);
...
static void test_agentic_loop_state() {
    ...
    TEST(set_and_get_goal)
        AgenticLoopState state;
        state.startIteration("Fix all bugs");
        auto* it = state.getCurrentIteration();
        ASSERT_TRUE(it != nullptr);
        ASSERT_TRUE(it->goal == "Fix all bugs");
    PASS;
    ...
    TEST(constraints)
        ...
        bool valid = state.validateAgainstConstraints(nlohmann::json{{"max_tokens", "4096"}});
        ASSERT_TRUE(valid);
    PASS;
