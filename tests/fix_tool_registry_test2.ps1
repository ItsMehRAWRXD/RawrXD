...
$Script:newLines = New-Object System.Collections.Generic.List[string]
$Script:inOrphan = $false
$Script:orphanClosed = $false
...
$Script:oldBlock = '    // Verify logging
...
$Script:newBlock = '    // Logger writes to stdout directly (no virtual interface for mock interception).
...
$Script:c = $c.Replace($oldBlock, $newBlock)
...
$Script:c = $c -replace 'bool hasInfoLog = false;\s+for \(const auto& entry : ctx\.logger->entries\) \{[^}]+\}\s+assert\(hasInfoLog[^;]+;\s+', ''
$Script:c = $c -replace "ctx\.metrics->counters\[""tools_registered""\]", 'ctx.metrics->getCounter("tools_registered")'
$Script:c = $c -replace "ctx\.metrics->counters\[""tool_executions_total""\]", 'ctx.metrics->getCounter("tool_executions_total")'
$Script:c = $c -replace "ctx\.metrics->counters\[""tool_executions_successful""\]", 'ctx.metrics->getCounter("tool_executions_successful")'
$Script:c = $c -replace 'uint64_t totalExec = ', 'int64_t totalExec = '
$Script:c = $c -replace 'uint64_t successfulExec = ', 'int64_t successExec = '
$Script:c = $c -replace 'successfulExec ==', 'successExec =='
$Script:c = $c -replace '<< ctx\.logger->entries\.size\(\)', '<< "(see above)"'
$Script:c = $c -replace 'successfulExec$', 'successExec'
