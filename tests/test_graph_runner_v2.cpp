//==============================================================================
// test_graph_runner_v2.cpp
// SovereignGraphRunner v2 Integration Test
//
// Validates:
...
//==============================================================================
struct TestConfig {
    uint32_t hiddenSize{512};  // Small for fast testing
    uint32_t numHeads{8};
...
bool Test_ForwardPass() {
    ...
    // Run forward pass with validation
    auto result = runner.Forward(0, 0, ValidationMode::COMPARE);
    ...
    if (result.success) {
        ...
        if (!result.validationResults.empty()) {
            std::cout << "  Validation:\n";
            for (const auto& entry : result.validationResults) {
                std::cout << "    " << entry.backendName 
                          << ": max_error=" << std::scientific << entry.maxError
