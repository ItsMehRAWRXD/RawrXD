...
async function ensureModel(model: string): Promise<boolean> {
  ...
  try {
    const res = await (globalThis as any).fetch('http://127.0.0.1:11434/api/pull', {
      method: 'POST',
...
async function ollamaGenerate(model: string, prompt: string, params: any): Promise<string> {
  ...
  const res = await (globalThis as any).fetch('http://127.0.0.1:11434/api/generate', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
async function listModels(): Promise<string[]> {
  try {
    const res = await (globalThis as any).fetch('http://127.0.0.1:11434/api/tags', { method: 'GET' } as any);
    if (!res.ok) return [];
