...
public:
    ...
    void RegisterMetaCognitiveCallback(std::function<void(const std::string&)> callback);
    void TriggerMetaCognitiveEvent(const std::string& eventType);
...
private:
    ...
    std::vector<std::function<void(const std::string&)>> m_metaCognitiveCallbacks;
