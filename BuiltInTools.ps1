...
function Initialize-BuiltInTools {
    ...
    Register-AgentTool -Name "edit_file" -Description "Edit an existing file by replacing strings" `
        ...
        -Parameters @{
            path = @{ type = "string"; description = "Absolute path to file"; required = $true }
        ...
        -Handler {
            param($path, $old_string, $new_string)
            # Support legacy parameter name
            if (-not $path -and $PSBoundParameters.ContainsKey("file_path")) {
                $path = $PSBoundParameters["file_path"]
            }
            try {
                if (Test-Path $path) {
    ...
    Register-AgentTool -Name "create_directory" -Description "Create a new directory" `
        ...
        -Parameters @{
            path = @{ type = "string"; description = "Absolute path to directory"; required = $true }
        } `
        -Handler {
            param($path)
            # Support legacy parameter name
            if (-not $path -and $PSBoundParameters.ContainsKey("dir_path")) {
                $path = $PSBoundParameters["dir_path"]
            }
            try {
                if (-not (Test-Path $path)) {
                    New-Item -ItemType Directory -Path $path -Force | Out-Null
                    return @{ success = $true; message = "Directory created"; path = $path }
                }
                return @{ success = $true; message = "Directory already exists"; path = $path }
            } catch {
                return @{ success = $false; error = $_.Exception.Message }
            }
        }
    ...
    Register-AgentTool -Name "create_file" -Description "Create a new file with content" `
        ...
        -Handler {
            ...
            try {
                ...
                return @{ success = $true; message = "File created"; path = $path; size = $content.Length }
            } catch {
                return @{ success = $false; error = $_.Exception.Message }
            }
        }
    ...
    Register-AgentTool -Name "read_file" -Description "Read file contents" `
        ...
        -Handler {
            param($path, $start_line = 0, $end_line = 0)
            # Note: file_path is automatically normalized to path by Invoke-AgentTool
            # Check both path and file_path for maximum compatibility
            if (-not $path) {
                if ($PSBoundParameters.ContainsKey("file_path")) {
                    $path = $PSBoundParameters["file_path"]
            ...
            try {
                if (Test-Path $path) {
                    $content = if ($start_line -gt 0 -and $end_line -gt 0) {
                        Get-Content $path | Select-Object -Skip ($start_line - 1) -First ($end_line - $start_line + 1)
                    } else {
                        Get-Content $path
                    }
                    return @{ 
