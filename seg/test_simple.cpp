// ============================================================================
// Simple Transformer Runtime Test
// ============================================================================
...
void TestRMSNorm() {
    ...
    for (size_t i = 0; i < input.size(); i++) {
        ...
        if (std::abs(output[i] - expected) > 1e-5f) {
            pass = false;
            std::cout << "FAIL at index " << i << ": expected " << expected << ", got " << output[i] << "\n";
        }
    }
...
void BenchmarkSequenceLength(const TransformerConfig& config,
    ...
    for (uint32_t seq_len : seq_lengths) {
        ...
        for (uint32_t pos = 0; pos < seq_len; pos++) {
            layer.Forward(input_view, output_view, kv_cache, pos);
        }
        
        // Time next token
        auto start = std::chrono::high_resolution_clock::now();
        layer.Forward(input_view, output_view, kv_cache, seq_len);
        auto end = std::chrono::high_resolution_clock::now();
