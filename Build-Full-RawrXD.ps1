...
function Verify-Build {
    ...
    if (Test-Path $exePath) {
        ...
        Write-Status "  ✓ RawrXD-QtShell.exe: $([math]::Round($exeSize, 2)) MB" -Color $Color.Green
        return $true
    } else {
        Write-Status "  ✗ RawrXD-QtShell.exe not found" -Color $Color.Red
        return $false
    }
}
...
function Show-BuildSummary {
    ...
    Write-Status "  • Qt IDE Executable:" -Color $Color.White
    ...
    Write-Status "  1. Launch Qt IDE:" -Color $Color.White
    Write-Status "     .\build\bin\Release\RawrXD-QtShell.exe" -Color $Color.Cyan
...
function Main {
    ...
    if (Verify-Build) {
        ...
        if ($Run) {
            Write-Status "`n🎯 Launching RawrXD-QtShell..." -Color $Color.Yellow
