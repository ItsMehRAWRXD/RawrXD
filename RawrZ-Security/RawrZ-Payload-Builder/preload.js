...
// Expose protected, explicit capabilities to the rendering layer safely
contextBridge.exposeInMainWorld('SovereignCore', {
    triggerBeacon: (channel, payload) => {
        ipcRenderer.send('beacon-signal-outbound', { channel, payload });
    },
    onStatusUpdate: (callback) => {
        ipcRenderer.on('beacon-status-sync', (event, data) => callback(data));
    }
});

// Legacy bridge: maintain existing API surface under window.rawrz
