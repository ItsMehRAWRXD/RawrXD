...
PRIVATE ZeroDayIntegration_DetectExpertKeywords

; Count tokens in goal string (simple heuristic: spaces + 1)
; rcx = goal string (LPCSTR)
; Returns: rax = token count estimate
PRIVATE ZeroDayIntegration_CountTokens
...
ZeroDayIntegration_Initialize PROC FRAME
    ...
    LEA rcx, [REL ZeroDayIntegration_OnAgentStream]
    MOV [rax + 0], rcx

    LEA rcx, [REL ZeroDayIntegration_OnAgentComplete]
    MOV [rax + 8], rcx

    LEA rcx, [REL ZeroDayIntegration_OnAgentError]
    MOV [rax + 16], rcx
...
ZeroDayIntegration_RouteExecution PROC FRAME
    ...
    ; Get integration context (simplified: assume global context initialized)
    LEA rax, [REL gdwZeroDayIntegrationContext]
    MOV rcx, [rax]
    TEST rcx, rcx
...
ZeroDayIntegration_DetectExpertKeywords PROC FRAME
    ...
    MOV rcx, rbx
    LEA rdx, [REL szZeroKeyword]
    CALL asm_string_contains
    TEST eax, eax
    JNZ @expert_detected

    MOV rcx, rbx
    LEA rdx, [REL szMetaKeyword]
    CALL asm_string_contains
    TEST eax, eax
    JNZ @expert_detected
...
; ============================================================================
; String Helper (simple substring search)
; rcx = haystack string, rdx = needle string
; Returns eax = 1 if found, 0 otherwise
; ============================================================================
asm_string_contains PROC FRAME
    PUSH_REG rbx
    PUSH_REG r12
    .ALLOCSTACK 28h
    SUB rsp, 28h
...
@search_loop:
    ...
    MOV al, BYTE PTR [r12]  ; First char of needle
    CMP cl, al
    JNE @search_next

    ; Potential match - verify rest of needle
    MOV rcx, rbx
    MOV rdx, r12
...
@verify_loop:
    ...
    TEST al, al             ; Both null-terminated?
    JZ @search_found
