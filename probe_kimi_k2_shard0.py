#!/usr/bin/env python3
"""Kimi K2 Shard 0 Tensor Inventory Probe"""
...
# Block sizes for quantized types (bytes per 32-element block)
GGUF_TYPE_SIZES = {
...
def main():
    base = r'F:\OllamaModels\Kimi-K2-Instruct-0905-GGUF\Q4_K_M'
    shard = os.path.join(base, 'Kimi-K2-Instruct-0905-Q4_K_M-00001-of-00013.gguf')
    ...
    with open(shard, 'rb') as f:
        ...
        data_section_start = align_offset(data_section_start, 32)
        
        print(f'Data section start: {data_section_start}')
        print(f'File size: {os.path.getsize(shard)}')
        ...
        print('\n=== First 20 tensors ===')
        for t in tensors[:20]:
            print(f"{t['index']:3d} | {t['name']:50s} | dims={t['n_dims']} {str(t['dims']):30s} | type={t['type_name']:10s} | offset={t['offset']:12d} | size={t['byte_size']:15d}")
        ...
        if overlaps:
            ...
            for o in overlaps[:5]:
                print(f'  {o[0]} overlaps {o[1]} by {o[2]} bytes')
        else:
            print('No overlaps detected')
        
        total_data = max(t['end_offset'] for t in tensors) - data_section_start
        print(f'Total data section size: {total_data} bytes ({total_data / 1e9:.2f} GB)')
        print(f'File size: {os.path.getsize(shard)} bytes ({os.path.getsize(shard) / 1e9:.2f} GB)')
        ...
        for cat, items in categories.items():
            ...
            total_size += cat_size
            print(f'{cat:20s}: {len(items):4d} tensors | {cat_size / 1e9:8.3f} GB')
        print(f'{"total":20s}: {sum(len(v) for v in categories.values()):4d} tensors | {total_size / 1e9:8.3f} GB')
