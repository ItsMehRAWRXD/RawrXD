...
:: Remove old Deep2Engine.obj from library (output to temp)
lib.exe /nologo /out:InferenceEngine_temp.lib /remove:"InferenceEngine.dir\Release\Deep2Engine.obj" InferenceEngine.lib
if errorlevel 1 (
    echo FAILED: could not remove old Deep2Engine.obj
    exit /b 1
)

:: Add new Deep2Engine_new.obj
lib.exe /nologo /out:InferenceEngine_new.lib InferenceEngine_temp.lib "%BUILD%\Deep2Engine_new.obj"
if errorlevel 1 (
    echo FAILED: could not add new Deep2Engine_new.obj
    exit /b 1
)
...
del InferenceEngine_temp.lib 2>nul

echo --- Rebuilding test_generate_313_tokens.exe ---
...
cl.exe %FLAGS% %DEFS% %INC1% %INC2% "%SRC%\test_generate_313_tokens.cpp" /Fe"%OUT%\test_generate_313_tokens.exe" /link %LIBS%
if errorlevel 1 (
    echo FAILED: test_generate_313_tokens.exe
    exit /b 1
)
