...
public:
    ...
    void RegisterNeuralCallback(std::function<void(const std::string&)> callback);
    void TriggerNeuralEvent(const std::string& eventType);
...
private:
    ...
    std::vector<std::function<void(const std::string&)>> m_neuralCallbacks;
