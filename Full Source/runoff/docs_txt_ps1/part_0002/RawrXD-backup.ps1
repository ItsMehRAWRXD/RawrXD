...
# ============================================
# EMERGENCY STARTUP LOGGING (IMMEDIATE AVAILABILITY)
# ============================================
...
# Emergency logging function available immediately
...
Write-EmergencyLog "RawrXD Emergency Startup Logging Initialized" "SUCCESS"
Write-EmergencyLog "PowerShell Version: $($PSVersionTable.PSVersion)" "INFO"
Write-EmergencyLog "OS: $([System.Environment]::OSVersion.VersionString)" "INFO"
Write-EmergencyLog "User: ${env:USERNAME} on ${env:COMPUTERNAME}" "INFO"
Write-EmergencyLog "Process ID: $PID" "INFO"
...
function Show-FindDialog {
    ...
    $findNextBtn.Add_Click({
        ...
        if ($foundIndex -ge 0) {
            ...
            [System.Windows.Forms.MessageBox]::Show("Text not found: '$searchText'", "Find", "OK", "Information")
        }
    })
...
function Write-ErrorLog {
    ...
    if (Get-Command Write-ErrorReport -ErrorAction SilentlyContinue) {
        Write-ErrorReport -ErrorMessage $ErrorMessage -ErrorCategory $ErrorCategory -Severity $Severity -SourceFunction $SourceFunction -AdditionalData $AdditionalData -ShowToUser $ShowToUser
    }
    
    # Log to security system
    Write-SecurityLog "Error logged: $ErrorMessage" "ERROR" "Category: $ErrorCategory, Severity: $Severity, Source: $SourceFunction"
}
...
function Apply-WindowSettings {
    if (-not ${script:form} -or -not ${script:CurrentSettings}.ContainsKey('WindowState')) {
        return
    }
    ...
    $loginBtn.Add_Click({
            ...
            if ($username -and $validCredentials.ContainsKey($username) -and $validCredentials[$username] -eq $password) {
                ...
                if (${script:CurrentSession}.LoginAttempts -ge ${script:SecurityConfig}.MaxLoginAttempts) {
                    [System.Windows.Forms.MessageBox]::Show("Maximum login attempts exceeded. Application will exit.", "Security Alert", "OK", "Error")
                    $authForm.DialogResult = "Cancel"
                    $authForm.Close()
                    return
                }
            
                [System.Windows.Forms.MessageBox]::Show("Invalid credentials. Please try again.", "Authentication Failed", "OK", "Warning")
                $passwordBox.Clear()
                $passwordBox.Focus()
            }
        })
    ...
    $switchBtn.Add_Click({
            if ($serverGrid.SelectedRows -and $serverGrid.SelectedRows.Count -gt 0) {
                ...
                if ($serverKey) {
                    ...
                    [System.Windows.Forms.MessageBox]::Show("Switched to server: $serverName", "Server Switch", "OK", "Information")
                }
            }
        })
...
if (${script:SecurityConfig}.AuthenticationRequired) {
    ...
    if (-not $authResult) {
        ...
        [System.Windows.Forms.MessageBox]::Show("Authentication required to access this application.", "Access Denied", "OK", "Warning")
        exit
    }
        ...
        try {
            ...
            if ($node.Tag -and $node.Tag -ne "DUMMY") {
                ...
                if ($fileInfo.Length -gt 50MB) {
                    $result = [System.Windows.Forms.MessageBox]::Show("File is large ($([math]::Round($fileInfo.Length/1MB, 1))MB). This may slow down the editor. Continue?", "Large File Warning", "YesNo", "Question")
                    if ($result -ne "Yes") {
                        return
                    }
                }
                ...
                if ($extension -in $binaryExts) {
                    $result = [System.Windows.Forms.MessageBox]::Show("This appears to be a binary file ($extension). It may not display correctly as text. Open anyway?", "Binary File Warning", "YesNo", "Question")
                    if ($result -ne "Yes") {
                        return
                    }
                }
                ...
                try {
                    ...
                    if ($extension -eq '.secure') {
                        try {
                            ...
                            $result = [System.Windows.Forms.MessageBox]::Show("File appears to be encrypted but decryption failed. Show raw content?", "Decryption Error", "YesNo", "Question")
                            if ($result -ne "Yes") {
                                return
                            }
                        }
                    ...
                    if (${script:editor}) {
                        ...
                        [System.Windows.Forms.MessageBox]::Show("Editor is not properly initialized. Please restart RawrXD.", "Editor Error", "OK", "Error")
                    }
            ...
            [System.Windows.Forms.MessageBox]::Show("Unexpected error: $($_.Exception.Message)", "Handler Error", "OK", "Error")
        }
        ...
        if ($selectedNode -and $selectedNode.Tag -and $selectedNode.Tag -ne "DUMMY") {
            if (Test-Path $selectedNode.Tag) {
                try {
                    ...
                    [System.Windows.Forms.MessageBox]::Show("Failed to open file: $($_.Exception.Message)", "Open Error", "OK", "Error")
                }
            }
        }
    })
...
$openFolderItem.Add_Click({
        ...
        if ($selectedNode -and $selectedNode.Tag -and $selectedNode.Tag -ne "DUMMY") {
            try {
                ...
                [System.Windows.Forms.MessageBox]::Show("Failed to open folder: $($_.Exception.Message)", "Folder Error", "OK", "Error")
            }
        }
    })
...
"@
                    [System.Windows.Forms.MessageBox]::Show($props, "Properties - $($item.Name)", "OK", "Information")
                }
            }
            catch {
                Write-DevConsole "Failed to get properties: $_" "ERROR"
                [System.Windows.Forms.MessageBox]::Show("Failed to get properties: $($_.Exception.Message)", "Properties Error", "OK", "Error")
            }
        }
    })
...
$openItem.Add_Click({
        ...
        if (-not (Test-SessionSecurity)) {
            ...
            [System.Windows.Forms.MessageBox]::Show("Session security check failed. Please restart the application.", "Security Alert", "OK", "Warning")
            return
        }
        ...
        if ($dlg.ShowDialog() -eq "OK") {
            try {
                ...
                if (-not (Test-InputSafety -Input $fileName -Type "FilePath")) {
                    ...
                    [System.Windows.Forms.MessageBox]::Show("File path contains potentially dangerous content.", "Security Alert", "OK", "Warning")
                    return
                }
                ...
                if ($fileInfo.Length -gt 10MB) {
                    ...
                    [System.Windows.Forms.MessageBox]::Show("File is too large (>10MB) for security reasons.", "Security Alert", "OK", "Warning")
                    return
                }
                ...
                if ($extension -in $dangerousExtensions) {
                    ...
                    $result = [System.Windows.Forms.MessageBox]::Show("This file type ($extension) could be potentially dangerous. Continue anyway?", "Security Warning", "YesNo", "Warning")
                    if ($result -ne "Yes") {
                        return
                    }
                }
                ...
                if (-not (Test-InputSafety -Input $content -Type "FileContent")) {
                    ...
                    $result = [System.Windows.Forms.MessageBox]::Show("File content contains potentially dangerous patterns. Continue anyway?", "Security Warning", "YesNo", "Warning")
                    if ($result -ne "Yes") {
                        return
                    }
                }
                ...
                if ($extension -eq '.secure' -and ${script:SecurityConfig}.EncryptSensitiveData) {
                    try {
                        ...
                        [System.Windows.Forms.MessageBox]::Show("Failed to decrypt file. It may not be encrypted or use a different key.", "Decryption Error", "OK", "Error")
                        return
                    }
                }
                ...
                [System.Windows.Forms.MessageBox]::Show("Error opening file: $($_.Exception.Message)", "File Error", "OK", "Error")
            }
        }
    })
        ...
        if (${global:currentFile}) {
            try {
                ...
                if (-not (Test-InputSafety -Input $content -Type "FileContent")) {
                    ...
                    $result = [System.Windows.Forms.MessageBox]::Show("Content contains potentially dangerous patterns. Save anyway?", "Security Warning", "YesNo", "Warning")
                    if ($result -ne "Yes") {
                        return
                    }
                }
                ...
                [System.Windows.Forms.MessageBox]::Show("Error saving file: $($_.Exception.Message)", "File Error", "OK", "Error")
            }
        }
        else {
...
function Show-ModelSettings {
    ...
    if ($settingsForm.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        ${global:settings}.MaxChatTabs = $tabsNumeric.Value  # Use same value for chat tabs by default
...
function Optimize-UIPerformance {
    try {
        ...
        if ($form) {
            ...
            $form.SetStyle([System.Windows.Forms.ControlStyles]::ResizeRedraw, $true)
        ...
        try {
            if (${script:editor}) {
                ${script:editor}.SetStyle([System.Windows.Forms.ControlStyles]::AllPaintingInWmPaint, $true)
                ${script:editor}.SetStyle([System.Windows.Forms.ControlStyles]::DoubleBuffer, $true)
            }
        }
        catch {
            Write-DevConsole "Text editor optimization partial: $_" "WARNING"
        }
    ...
    if (${script:SecurityConfig}.LogSecurityEvents) {
        ...
        ${script:securityTimer}.add_Tick({
                if (-not (Test-SessionSecurity)) {
                    ...
                    if (${script:SecurityConfig}.AuthenticationRequired) {
                        $result = [System.Windows.Forms.MessageBox]::Show("Session security check failed. Do you want to re-authenticate?", "Security Alert", "YesNo", "Warning")
                        if ($result -eq "Yes") {
                            $authResult = Show-AuthenticationDialog
                            if (-not $authResult) {
