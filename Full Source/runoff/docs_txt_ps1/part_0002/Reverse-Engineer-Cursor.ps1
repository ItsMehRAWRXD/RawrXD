...
<#
.SYNOPSIS
    Comprehensive Reverse Engineering Script for Cursor IDE
.DESCRIPTION
    Fully reverse engineers Cursor IDE to extract:
    - Binary analysis and architecture
    - API endpoints and protocols
    - Configuration and settings
    - Plugin system and extensions
    - AI integration logic
    - Agentic capabilities architecture
...
function Analyze-CursorBinary {
    ...
    # Save binary analysis with increased depth
    $binaryInfo | ConvertTo-Json -Depth 20 | Out-File -FilePath "$OutputDirectory\Binary\binary_analysis.json" -Encoding UTF8        Write-ColorOutput "✓ Binary analysis complete" "Success"
...
function Extract-APIEndpoints {
    ...
    # Common Cursor API endpoints (based on known information)
    $knownEndpoints = @(
        @{
            Name = "Cursor API"
    ...
    # Save API analysis with increased depth
    $apiInfo | ConvertTo-Json -Depth 20 | Out-File -FilePath "$OutputDirectory\API\api_analysis.json" -Encoding UTF8
...
function Analyze-CursorConfiguration {
        ...
        if (Test-Path $location) {
            ...
            foreach ($file in $jsonFiles) {
                try {
                    ...
                    Write-ColorOutput "✓ Analyzed: $($file.Name)" "Success"
                }
                catch {
    ...
    # Save configuration analysis with increased depth
    $configInfo | ConvertTo-Json -Depth 20 | Out-File -FilePath "$OutputDirectory\Config\config_analysis.json" -Encoding UTF8
...
function Extract-PluginsAndExtensions {
            ...
            foreach ($extDir in $extensionDirs) {
                ...
                if (Test-Path $packageJsonPath) {
                    try {
                        ...
                        Write-ColorOutput "✓ Extension found: $($extensionInfo.DisplayName) v$($extensionInfo.Version)" "Success"
                    }
                    catch {
    ...
    # Save plugins analysis with increased depth
    $pluginsInfo | ConvertTo-Json -Depth 20 | Out-File -FilePath "$OutputDirectory\Plugins\plugins_analysis.json" -Encoding UTF8
...
"@
    ...
    # Save AI analysis with increased depth
    $aiInfo | ConvertTo-Json -Depth 20 | Out-File -FilePath "$OutputDirectory\AI\ai_analysis.json" -Encoding UTF8
...
function Analyze-AgenticCapabilities {
    ...
    # Save agent analysis with increased depth
    $agentInfo | ConvertTo-Json -Depth 20 | Out-File -FilePath "$OutputDirectory\Agents\agent_analysis.json" -Encoding UTF8
...
function Analyze-MCPImplementation {
    ...
    # Save MCP analysis with increased depth
    $mcpInfo | ConvertTo-Json -Depth 20 | Out-File -FilePath "$OutputDirectory\MCP\mcp_analysis.json" -Encoding UTF8
...
function Analyze-SecurityImplementation {
    ...
    # Save security analysis with increased depth
    $securityInfo | ConvertTo-Json -Depth 20 | Out-File -FilePath "$OutputDirectory\Security\security_analysis.json" -Encoding UTF8
