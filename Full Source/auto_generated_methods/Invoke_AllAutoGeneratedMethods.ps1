...
$Script:loggingModule = Join-Path $scriptRoot 'RawrXD.Logging.psm1'
$Script:configModule = Join-Path $scriptRoot 'RawrXD.Config.psm1'
...
$Script:methodFiles = Get-ChildItem -Path "D:/lazy init ide/auto_generated_methods" -Filter "*_AutoMethod.ps1"
...
$Script:results = @()
foreach ($file in $methodFiles) {
$Script:baseName = $file.BaseName -replace '_AutoMethod$',''
$Script:funcName = "Invoke-${baseName}Auto"
    if (Get-Command $funcName -ErrorAction SilentlyContinue) {
$Script:result = & $funcName
...
$Script:outPath = Join-Path (Split-Path -Parent $PSScriptRoot) 'InvokeAllResults.json'
