...
function Write-ExtensionLog {
    ...
    if (Get-Command Write-DevConsole -ErrorAction SilentlyContinue) {
        Write-ExtensionLog $Message $Level
    } else {
        $color = switch ($Level) {
            "ERROR" { "Red" }
            "WARNING" { "Yellow" }
...
try {
    ...
    Write-ExtensionLog "⚠️ System.Web.Extensions not available, using native JSON handling" "WARNING"
}
...
function Initialize-RawrXDChatBotExtension {
    ...
    try {
        Write-ExtensionLog "🤖 Initializing RawrXD ChatBot Extension..." "INFO"
        ...
        Write-ExtensionLog "✅ ChatBot Extension initialized successfully" "SUCCESS"
        Write-ExtensionLog "   - Language: $($script:supportedLanguages[$script:chatBotLanguage].Name)" "INFO"
        Write-ExtensionLog "   - Intents registered: $($script:chatBotIntents.Count)" "INFO"
        Write-ExtensionLog "   - Entities registered: $($script:chatBotEntities.Count)" "INFO"

        return $true
    }
    catch {
        Write-ExtensionLog "❌ Failed to initialize ChatBot Extension: $($_.Exception.Message)" "ERROR"
        return $false
    }
}
...
function Register-ChatBotIntent {
    ...
    try {
        ...
        Write-ExtensionLog "✅ Registered intent: $Name" "SUCCESS"
        return $true
    }
    catch {
        Write-ExtensionLog "❌ Failed to register intent $Name : $($_.Exception.Message)" "ERROR"
        return $false
    }
}
...
function Register-ChatBotEntity {
    ...
    try {
        ...
        Write-ExtensionLog "✅ Registered entity extractor: $Name" "SUCCESS"
        return $true
    }
    catch {
        Write-ExtensionLog "❌ Failed to register entity $Name : $($_.Exception.Message)" "ERROR"
        return $false
    }
}
...
function Generate-ChatBotResponse {
    ...
    try {
        ...
        Write-ExtensionLog "❌ Error generating response: $($_.Exception.Message)" "ERROR"
...
function Invoke-ChatBotTerminalCommand {
    ...
    try {
        Write-ExtensionLog "🔧 Executing terminal command: $Command" "INFO"
        ...
        Write-ExtensionLog "❌ Error executing command: $($_.Exception.Message)" "ERROR"
...
function Invoke-ChatBotCliCommand {
    ...
    try {
        Write-ExtensionLog "🔧 Executing RawrXD CLI command: $Command" "INFO"
        ...
        # Execute the command
        Write-ExtensionLog "Executing: $rawrxdScript $($psArgs -join ' ')" "INFO"
        ...
        Write-ExtensionLog "❌ Error executing CLI command: $($_.Exception.Message)" "ERROR"
...
function Process-ChatBotMessage {
    ...
    try {
        ...
        Write-ExtensionLog "❌ Error processing message: $($_.Exception.Message)" "ERROR"
...
function Start-ChatBotHttpServer {
    ...
    try {
        ...
        Write-ExtensionLog "🌐 HTTP server started on port $Port" "SUCCESS"
        ...
        Write-ExtensionLog "❌ Failed to start HTTP server: $($_.Exception.Message)" "ERROR"
...
function Stop-ChatBotHttpServer {
    ...
    try {
        ...
        $script:chatBotHttpServerRunning = $false
        Write-ExtensionLog "🛑 HTTP server stopped" "INFO"
        return "✅ HTTP server stopped"
    }
    catch {
        Write-ExtensionLog "❌ Error stopping HTTP server: $($_.Exception.Message)" "ERROR"
        return "❌ Error: $($_.Exception.Message)"
    }
}
...
function Set-ChatBotLanguage {
    ...
    if ($script:supportedLanguages.ContainsKey($langCode)) {
        $script:chatBotLanguage = $langCode
        $script:chatBotState.Language = $langCode
        Write-ExtensionLog "🌐 Language changed to $($script:supportedLanguages[$langCode].Name)" "SUCCESS"
        return $true
    }
    else {
        Write-ExtensionLog "❌ Unsupported language code: $langCode" "ERROR"
        return $false
    }
}
...
function Invoke-ChatBotFromRawrXD {
    ...
    try {
        ...
        Write-ExtensionLog "❌ Error in ChatBot integration: $($_.Exception.Message)" "ERROR"
        if ($ChatBox -and -not $ChatBox.IsDisposed) {
...
function Remove-RawrXDChatBotExtension {
    ...
    try {
        ...
        Write-ExtensionLog "✅ ChatBot Extension removed" "SUCCESS"
        return $true
    }
    catch {
        Write-ExtensionLog "❌ Error removing ChatBot Extension: $($_.Exception.Message)" "ERROR"
        return $false
    }
}
