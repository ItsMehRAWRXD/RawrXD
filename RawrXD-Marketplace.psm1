...
function Update-MarketplaceData {
    ...
    try {
        ...
        # Fetch from Open VSX Registry
        if ($Script:MarketplaceConfig.realMarketplaceSources.openVSX.enabled) {
            Write-Host "   📦 Fetching from Open VSX Registry..." -ForegroundColor Cyan
            $openVSXExtensions = Get-OpenVSXRegistryExtensions
            $allExtensions += $openVSXExtensions
            Write-Host "   ✅ Found $($openVSXExtensions.Count) Open VSX extensions" -ForegroundColor Green
        }
