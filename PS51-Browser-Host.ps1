...
function Initialize-BrowserForm {
    ...
    $script:BrowserForm.Text = "RawrXD Browser (PS5.1 Host) - Login Ready"
    $script:BrowserForm.Size = New-Object System.Drawing.Size(1800, 900)
    ...
    $script:BrowserForm.MinimumSize = New-Object System.Drawing.Size(1000, 600)
    ...
    # Quick Login Buttons
    $loginButtons = @(
        @{ Text = "📧 Gmail"; Url = "https://mail.google.com"; Color = [System.Drawing.Color]::FromArgb(234, 67, 53); X = 1020 },
        @{ Text = "🐙 GitHub"; Url = "https://github.com/login"; Color = [System.Drawing.Color]::FromArgb(36, 41, 46); X = 1105 },
        @{ Text = "🤖 ChatGPT"; Url = "https://chat.openai.com"; Color = [System.Drawing.Color]::FromArgb(10, 132, 255); X = 1190 },
        @{ Text = "✨ Kimi"; Url = "https://kimi.moonshot.cn"; Color = [System.Drawing.Color]::FromArgb(139, 69, 19); X = 1275 },
        @{ Text = "🧠 DeepSeek"; Url = "https://chat.deepseek.com"; Color = [System.Drawing.Color]::FromArgb(0, 122, 204); X = 1360 },
        @{ Text = "📝 Cursor"; Url = "https://cursor.sh"; Color = [System.Drawing.Color]::FromArgb(0, 122, 204); X = 1445 }
    )
    
    $script:LoginButtons = @()
    foreach ($btnInfo in $loginButtons) {
        $btn = New-Object System.Windows.Forms.Button
        $btn.Text = $btnInfo.Text
        $btn.Location = New-Object System.Drawing.Point($btnInfo.X, 5)
        ...
        $url = $btnInfo.Url
        $btn.Add_Click({ $script:WebBrowser.Navigate($url) })
        $navPanel.Controls.Add($btn)
        $script:LoginButtons += $btn
    }
    
    # YouTube Button (moved after login buttons)
    $btnYouTube = New-Object System.Windows.Forms.Button
    $btnYouTube.Text = "▶ YouTube"
    $btnYouTube.Location = New-Object System.Drawing.Point(1530, 5)
    ...
    $script:StatusLabel.Location = New-Object System.Drawing.Point(1620, 10)
    ...
    # Enable JavaScript and modern features for login flows
    ...
    # Enable modern rendering (IE11 mode for best compatibility)
    try {
        ...
        # 11001 = IE11 mode with full modern features
        ...
        $cookiePath = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\3"
        if (Test-Path $cookiePath) {
            # Allow cookies (0x00000001 = Allow)
            Set-ItemProperty -Path $cookiePath -Name "1A00" -Value 0 -Type DWord -ErrorAction SilentlyContinue
            # Allow third-party cookies (needed for OAuth)
            Set-ItemProperty -Path $cookiePath -Name "1A05" -Value 0 -Type DWord -ErrorAction SilentlyContinue
            # Allow session cookies
            Set-ItemProperty -Path $cookiePath -Name "1A06" -Value 0 -Type DWord -ErrorAction SilentlyContinue
            # Allow authentication
            Set-ItemProperty -Path $cookiePath -Name "1A02" -Value 0 -Type DWord -ErrorAction SilentlyContinue
            # Allow automatic logon
            Set-ItemProperty -Path $cookiePath -Name "1A00" -Value 0 -Type DWord -ErrorAction SilentlyContinue
        }
        
        # Enable JavaScript
        $jsPath = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\3"
        if (Test-Path $jsPath) {
            Set-ItemProperty -Path $jsPath -Name "1400" -Value 0 -Type DWord -ErrorAction SilentlyContinue
        }
        
        # Enable popups (needed for OAuth flows)
        $popupPath = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\3"
        if (Test-Path $popupPath) {
            Set-ItemProperty -Path $popupPath -Name "1809" -Value 0 -Type DWord -ErrorAction SilentlyContinue
        }
        
        Write-Host "✅ Browser configured for login flows (cookies, JavaScript, popups enabled)" -ForegroundColor Green
    } catch {
        Write-Host "⚠️ Browser configuration warning: $($_.Exception.Message)" -ForegroundColor Yellow
    }
    ...
    # Resize handler for URL box and buttons
    $script:BrowserForm.Add_Resize({
        $formWidth = $script:BrowserForm.ClientSize.Width
        # Adjust URL box width to fit available space
        $script:UrlBox.Width = [Math]::Max(400, $formWidth - 750)
        $btnGo.Location = New-Object System.Drawing.Point(($script:UrlBox.Right + 10), 5)
        
        # Adjust login buttons position if needed
        $currentX = $btnGo.Right + 10
        foreach ($btn in $script:LoginButtons) {
            $btn.Location = New-Object System.Drawing.Point($currentX, 5)
            $currentX += 85
        }
        
        $btnYouTube.Location = New-Object System.Drawing.Point($currentX, 5)
