...
private:
    ...
    // ---- Phase 29.2: GSI Hash Table (O(1) symbol lookup) ----
    // Forward-declared; defined in pdb_gsi_hash.h
    // These are heap-allocated in parsePublicSymbolStream() and freed in unload().
    void*             m_gsiHashTable    = nullptr;  // Actually GSIHashTable*
    void*             m_tpiParser       = nullptr;  // Actually TPIStreamParser*
    uint8_t*          m_symRecordData   = nullptr;  // Assembled symbol record stream
    uint32_t          m_symRecordSize   = 0;
    uint8_t*          m_gsiStreamData   = nullptr;  // Assembled GSI hash stream
    uint32_t          m_gsiStreamSize   = 0;
};
