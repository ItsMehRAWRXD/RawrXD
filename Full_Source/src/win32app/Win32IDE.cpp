...
Win32IDE::Win32IDE(HINSTANCE hInstance)
    ...
    // File Explorer
    m_hwndFileExplorer(nullptr), m_hImageList(nullptr), m_currentExplorerPath(""),
    // Model Chat
    m_chatMode(false),
    // PowerShell Panel
...
void Win32IDE::populateFileTree()
{
    ...
    // Add root directories for model browsing based on configured paths.
    std::vector<std::string> modelPaths = getConfiguredModelDirectories();
    ...
    // Expand first model root by default if any root exists.
...
std::vector<std::string> Win32IDE::getConfiguredModelDirectories() const {
    ...
    auto expandEnv = [](const std::string& input) {
        if (input.empty()) return input;
        char expanded[32768] = {0};
        DWORD n = ExpandEnvironmentStringsA(input.c_str(), expanded, (DWORD)sizeof(expanded));
        if (n == 0 || n > sizeof(expanded)) return input;
        return std::string(expanded);
    };

    auto addPath = [&](const std::string& candidate) {
        std::string p = trim(expandEnv(candidate));
        if (p.empty()) return;
        ...
        if (!(attr & FILE_ATTRIBUTE_DIRECTORY)) {
            ...
            p = p.substr(0, slash);
            attr = GetFileAttributesA(p.c_str());
        ...
        if (seen.insert(key).second) roots.push_back(p);
    };

    for (const auto& raw : splitPathList(m_settings.aiModelPath)) {
        addPath(raw);
    }

    if (!m_currentExplorerPath.empty()) {
        addPath(m_currentExplorerPath);
    }

    addPath("%USERPROFILE%\\OllamaModels");
    addPath("%USERPROFILE%\\.ollama\\models");
    addPath("%LOCALAPPDATA%\\Ollama\\models");
    addPath("D:\\OllamaModels");
    addPath("C:\\OllamaModels");

    return roots;
}

std::vector<std::string> Win32IDE::collectFilesystemModelNames(const std::vector<std::string>& modelDirs,
                                                               size_t maxCount) const {
    namespace fs = std::filesystem;
    std::vector<std::string> models;
    std::unordered_set<std::string> seen;

    auto addModel = [&](std::string name) {
    ...
    for (const auto& root : modelDirs) {
        ...
        for (fs::recursive_directory_iterator it(base, fs::directory_options::skip_permission_denied, ec), end;
             it != end && models.size() < maxCount; it.increment(ec)) {
            if (ec) continue;
            const fs::directory_entry& entry = *it;
            ...
            if (ext == ".gguf") {
                ...
                if (!stem.empty()) addModel(stem);
                continue;
            }

            // Ollama manifest layout:
            // ...\models\manifests\<registry>\<namespace>\<model>\<tag>
            std::string norm = entry.path().string();
            std::replace(norm.begin(), norm.end(), '/', '\\');
            std::string normLower = norm;
            std::transform(normLower.begin(), normLower.end(), normLower.begin(), [](unsigned char c) {
                return static_cast<char>(std::tolower(c));
            });
            const std::string needle = "\\manifests\\";
            const size_t mpos = normLower.find(needle);
            if (mpos != std::string::npos) {
                fs::path p = entry.path();
                std::string tag = p.filename().string();
                std::string model = p.parent_path().filename().string();
                if (!model.empty() && !tag.empty()) {
                    addModel(model + ":" + tag);
                }
            }
        }
...
void Win32IDE::populateModelSelector() {
    ...
    auto addModel = [&](const std::string& model) {
        ...
        if (seen.insert(key).second) m_availableModels.push_back(model);
    };

    // 1) Live Ollama tags from configured endpoint.
    try {
        const std::string endpoint = m_ollamaBaseUrl.empty() ? "http://localhost:11434" : m_ollamaBaseUrl;
        ModelConnection connection(endpoint);
        for (const auto& model : connection.getAvailableModels()) addModel(model);
    } catch (...) {
        // Keep going with filesystem discovery fallback.
    }

    // 2) Filesystem model scan from configured and known model roots.
    const auto dirs = getConfiguredModelDirectories();
    const auto fsModels = collectFilesystemModelNames(dirs);
    for (const auto& model : fsModels) addModel(model);

    // 3) Last-resort defaults for first-run UX.
...
void Win32IDE::removeTab(int index) {
    ...
    if (index == m_activeTabIndex) {
        if (m_editorTabs.size() > 1) {
            ...
            // Last tab - create empty untitled
            newFile();
            if (m_editorTabs.size() > 1) setActiveTab(0);
        }
    }
...
void Win32IDE::openModelFromOllama() {
    ...
    if (blobs.empty()) {
        MessageBoxW(m_hwndMain,
            ...
            L"Searched standard Ollama model directories and configured model paths.\n"
            L"Set Settings > AI/Model > Model Path to include custom directories.\n\n"
...
LRESULT CALLBACK Win32IDE::EditorSubclassProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (pThis) {
        ...
        case WM_KEYDOWN:
            // Ctrl+Space → Manual AI completion trigger
            if (wParam == VK_SPACE && (GetKeyState(VK_CONTROL) & 0x8000)) {
                // Immediately request ghost text (bypass debounce timer)
                pThis->dismissGhostText();  // Clear any stale ghost text
                pThis->onGhostTextTimer();  // Fire completion request now
                return 0;  // Consumed - prevent space from being typed
            }
