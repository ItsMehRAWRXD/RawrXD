...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR lpCmdLine, int nCmdShow) {
    ...
    // ========================================================================
    // CLI MODE — Phase 40: TITAN Local Engine (verified GOLD path)
    // If --mode is present, route to the standalone CLI inference pipeline.
    // This bypasses all GUI/WebView2 initialization and runs the proven
    // CPUInferenceEngine path that was verified in RawrXD_CLI_Final.exe.
    // ========================================================================
    if (lpCmdLine && strstr(lpCmdLine, "--mode")) {
        AllocConsole();
        freopen("CONOUT$", "w", stdout);
        freopen("CONOUT$", "w", stderr);
        freopen("CONIN$", "r", stdin);
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            else if (arg == "--chat" && i + 1 < argc) chat = argv[++i];
        }

        fprintf(stdout, "[TITAN] %s Mode Init\n", mode == "local" ? "Local" : "Distributed");

        // Initialize the verified TITAN engine
        auto* engine = RawrXD::CPUInferenceEngine::getInstance();
        if (!model.empty() && engine->LoadModel(model)) {
            fprintf(stdout, "[TITAN] Model loaded successfully\n");
            if (!chat.empty()) {
                fprintf(stdout, "[TITAN] Generate: \"%s...\"\n", chat.c_str());
                auto tokens = engine->Tokenize(chat);
                auto result = engine->Detokenize(tokens);
                fprintf(stdout, "%s\n", result.c_str());
            } else {
                // Interactive REPL mode
                fprintf(stdout, "[TITAN] Interactive mode. Type 'exit' to quit.\n");
                std::string line;
                while (fprintf(stdout, ">>> "), std::getline(std::cin, line)) {
            ...
            fprintf(stderr, "[TITAN] Error: --model <path.gguf> required\n");
            FreeConsole();
            return 1;
        } else {
