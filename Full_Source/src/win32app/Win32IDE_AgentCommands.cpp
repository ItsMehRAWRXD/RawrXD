...
void Win32IDE::onAgentConfigureModel() {
    ...
    // Retrieve available models from both Ollama tags and configured model directories.
    ...
    try {
        ...
        for (const auto& model : connection.getAvailableModels()) {
            addModel(model);
        }
    } catch (...) {
    ...
    try {
        const auto roots = getConfiguredModelDirectories();
        for (const auto& model : collectFilesystemModelNames(roots)) {
            addModel(model);
        }
    } catch (...) {
        // Keep defaults fallback if both discovery methods fail.
    }

    if (availableModels.empty()) {
        std::ostringstream diag;
        diag << "No models discovered.\n\n";
        diag << "Checked:\n";
        diag << "  - Ollama endpoint: "
             << (m_ollamaBaseUrl.empty() ? "http://localhost:11434" : m_ollamaBaseUrl) << "\n";
        diag << "  - Configured model directories:\n";
        const auto roots = getConfiguredModelDirectories();
        if (roots.empty()) {
            diag << "      (none)\n";
        } else {
        ...
        diag << "\nSet Settings -> AI/Model -> Model Path to your GGUF/Ollama models directory.";
        MessageBoxA(m_hwndMain, diag.str().c_str(), "Agent Model Configuration Failed", MB_OK | MB_ICONWARNING);
        appendToOutput("Agent model configuration failed: no models available.\n", "Errors", OutputSeverity::Warning);
        return;
    }
