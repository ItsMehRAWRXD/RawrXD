...
// ============================================================================
// FRONTEND: /models — list loaded model + scan configured model directories
// ============================================================================
...
void Win32IDE::handleModelsEndpoint(SOCKET client) {
    ...
    auto normalizePathKey = [](std::string p) {
    ...
    auto addModelRow = [&](const std::string& name,
                           const std::string& type,
                           const std::string& size,
                           const std::string& path) {
        if (path.empty()) return;
        std::string key = normalizePathKey(path);
        if (!seenPaths.insert(key).second) return;

        if (count > 0) j << ",";
        j << "{\"name\":\"" << LocalServerUtil::escapeJson(name)
          << "\",\"type\":\"" << LocalServerUtil::escapeJson(type)
          << "\",\"size\":\"" << LocalServerUtil::escapeJson(size)
    ...
    if (m_nativeEngine && m_nativeEngine->IsModelLoaded() && !m_loadedModelPath.empty()) {
        ...
        addModelRow(displayName, "gguf", "loaded", m_loadedModelPath);
    }

    // 2) Scan configured model roots recursively for GGUF and large Ollama blobs.
    const auto modelRoots = getConfiguredModelDirectories();
    for (const auto& root : modelRoots) {
        ...
        for (fs::recursive_directory_iterator it(base, fs::directory_options::skip_permission_denied, ec), end;
            ...
            if (ec) bytes = 0;
            double sizeGB = static_cast<double>(bytes) / (1024.0 * 1024.0 * 1024.0);
            ...
            if (ext == ".gguf") {
                ...
                addModelRow(displayName, "gguf", sizeFmt.str(), fullPath);
                continue;
            }

            // Ollama manifest entries:
            ...
            const std::string manifestNeedle = "\\manifests\\";
            if (normLower.find(manifestNeedle) != std::string::npos) {
                std::string tag = p.filename().string();
                std::string model = p.parent_path().filename().string();
                if (!model.empty() && !tag.empty()) {
                    addModelRow(model + ":" + tag, "manifest", sizeFmt.str(), fullPath);
                }
                continue;
            }

            const std::string file = p.filename().string();
            const bool isBlob = file.rfind("sha256-", 0) == 0;
            if (!isBlob || sizeGB < 0.1) continue; // keep metadata noise out

            std::string displayName = "blob:" + file.substr(0, std::min<size_t>(19, file.size()));
            addModelRow(displayName, "blob", sizeFmt.str(), fullPath);
        }
    }
