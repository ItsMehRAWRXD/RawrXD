...
#include "../../include/native_memory.hpp"
#include "../../include/ExtensionLoader.hpp"
...
// Window Class Name
// ============================================================================
static const wchar_t* kWindowClassName = L"RawrXD_IDE_MainWindow";
...
bool Win32IDE::createWindow() {
    ...
    m_hwndMain = CreateWindowExW(
        WS_EX_APPWINDOW,
        kWindowClassName,
        L"RawrXD IDE - Native Win32 AI Development Environment",
...
void Win32IDE::deferredHeavyInitBody() {
        ...
        // Initialize AI Backend Manager (Ollama, OpenAI, Claude, Gemini, Local)
        try {
            initBackendManager();
            OutputDebugStringA("[deferredHeavyInit] Backend manager initialized\n");
        } catch (...) {
            OutputDebugStringA("ERROR: initBackendManager failed\n");
        }

        // Probe all AI backends for availability
        try {
            probeAllBackendsAsync();
            OutputDebugStringA("[deferredHeavyInit] Backend health probes started\n");
        } catch (...) {
            OutputDebugStringA("ERROR: probeAllBackendsAsync failed\n");
        }
