...
void TransparentRenderer::DrawTextW(const std::wstring& text, int x, int y, uint32_t color)
{
    ...
    D2D1_RECT_F layoutRect = D2D1::RectF((float)x, (float)y, (float)x + 1000.0f, (float)y + 100.0f);
...
void TransparentRenderer::DrawRect(int x, int y, int w, int h, uint32_t color)
{
    ...
    D2D1_RECT_F rect = D2D1::RectF((float)x, (float)y, (float)x + (float)w, (float)y + (float)h);
