#pragma once
#ifndef RAWRXD_NATIVE_AGENT_HPP
#define RAWRXD_NATIVE_AGENT_HPP
...
private:
    ...
    std::string BuildPrompt(const std::string& userQuery) {
        ...
        system += "\n\nYou have access to the following tools. To use a tool, /* emit */ the "
                  ...
                  "11. **list_ide_commands** — List all IDE commands (your command palette). TOOL:list_ide_commands:{}\n\n"
                  "12. **execute_ide_command** — Run an IDE command by id or name. TOOL:execute_ide_command:{\"id\":2002} or {\"name\":\"File: Open\"}\n\n"
                  "Use list_ide_commands to browse available IDE actions; use execute_ide_command to run them (navigate, show terminal, etc.).\n"
