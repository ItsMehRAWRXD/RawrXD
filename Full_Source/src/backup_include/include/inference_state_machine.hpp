...
struct StateMachineStats {
    ...
    StateMachineStats() = default;
    StateMachineStats(const StateMachineStats& other)
        : totalTransitions(other.totalTransitions.load(std::memory_order_relaxed))
        , rejectedTransitions(other.rejectedTransitions.load(std::memory_order_relaxed))
        , faultCount(other.faultCount.load(std::memory_order_relaxed))
        , recoveryCount(other.recoveryCount.load(std::memory_order_relaxed))
        , hotpatchPauses(other.hotpatchPauses.load(std::memory_order_relaxed))
        , avgTransitionUs(other.avgTransitionUs)
        , maxTransitionUs(other.maxTransitionUs)
        {
            std::memcpy(stateResidencyUs, other.stateResidencyUs, sizeof(stateResidencyUs));
        }
    StateMachineStats& operator=(const StateMachineStats& other) {
        totalTransitions.store(other.totalTransitions.load(std::memory_order_relaxed), std::memory_order_relaxed);
        rejectedTransitions.store(other.rejectedTransitions.load(std::memory_order_relaxed), std::memory_order_relaxed);
        faultCount.store(other.faultCount.load(std::memory_order_relaxed), std::memory_order_relaxed);
        recoveryCount.store(other.recoveryCount.load(std::memory_order_relaxed), std::memory_order_relaxed);
        hotpatchPauses.store(other.hotpatchPauses.load(std::memory_order_relaxed), std::memory_order_relaxed);
        avgTransitionUs = other.avgTransitionUs;
        maxTransitionUs = other.maxTransitionUs;
        std::memcpy(stateResidencyUs, other.stateResidencyUs, sizeof(stateResidencyUs));
        return *this;
