...
struct WorkspaceSnapshot {
    ...
    static WorkspaceSnapshot fromJson(const nlohmann::json& j) {
        ...
        if (j.contains("files") && j["files"].is_array()) {
            const nlohmann::json& filesArr = j["files"];
            for (size_t i = 0; i < filesArr.size(); ++i) {
                const nlohmann::json& fj = filesArr[i];
                FileSnapshot fs;
                fs.path = fj.value("path", std::string(""));
...
struct ReplayConfig {
    ...
    bool            enableAutoRecovery = false;  // T4: trigger recovery on divergence
...
public:
    ...
    // ---- T4: Autonomous recovery on divergence ----
    // When enableAutoRecovery is set in config, divergences are
    // routed to AutonomousRecoveryOrchestrator::executeRecovery().
    // This callback fires after each recovery attempt.
    using RecoveryCallback = void(*)(const DivergenceEvent& div,
                                      bool recoverySuccess,
                                      void* userData);
    void SetRecoveryCallback(RecoveryCallback cb, void* userData);
...
private:
    ...
    // ---- T4: Auto-recovery dispatch ----
    bool AttemptAutoRecovery(const DivergenceEvent& event);
    ...
    // T4: Recovery integration
    RecoveryCallback            m_recoveryCallback = nullptr;
    void*                       m_recoveryUserData = nullptr;
