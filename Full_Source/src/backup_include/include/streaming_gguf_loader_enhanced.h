...
public:
    ...
    // Returns pointer to zone memory with automatic prefetch of next predicted zones
    // Returns nullptr on failure. Caller must not modify or free the returned pointer.
    // Memory is valid until the zone is evicted from cache.
    const uint8_t* GetTensorViewPtr(
...
private:
    ...
    // ---- Zone Tracking ----
    std::unordered_map<uint32_t, std::pair<uint64_t, uint64_t>> zone_offsets_;  // zone_id -> (offset, size)
