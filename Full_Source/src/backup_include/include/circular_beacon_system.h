...
enum class BeaconKind : uint32_t {
    ...
    // Dynamic agent/panel range: agents and models register here when they appear
    // and unregister when they disappear. No hard-coded map — UI shows whatever
    // is currently registered (built-in + dynamic).
    DynamicBase         = 0x3000,
    DynamicEnd          = 0x3FFE,
...
public:
    ...
    // ── Dynamic agents/panels (no hard-coded map: appear when needed, disappear when done) ──
    BeaconKind registerDynamicBeacon(const std::string& id, const char* name, void* instance, BeaconHandler handler);
    bool unregisterDynamicBeacon(const std::string& id);
    bool unregisterDynamicBeacon(BeaconKind dynamicKind);
    std::vector<std::pair<BeaconKind, std::string>> getDynamicBeacons() const;
...
private:
    ...
    // Dynamic agent/panel registry: id → kind (so agents can appear/disappear without fixed map)
    std::unordered_map<std::string, BeaconKind>          m_dynamicIdToKind;
    std::unordered_map<uint32_t, std::string>            m_dynamicKindToId;
    uint32_t                                             m_nextDynamicKind{static_cast<uint32_t>(BeaconKind::DynamicBase)};
};
