...
public:
    ...
    // ── Generation ──
    // Convenience method: Generate responses synchronously and return the session
    // This is a simpler API that wraps startSession() + generateAll()
    MultiResponseResult generate(const std::string& prompt,
                                 int maxResponses = 4,
                                 const std::string& context = "",
