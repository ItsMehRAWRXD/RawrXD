...
inline bool IsMainPaneControlId(int id) {
    return (id >= IDC_FILE_EXPLORER_MIN && id <= IDC_FILE_EXPLORER_MAX) ||
           (id >= IDC_PANEL_MIN         && id <= IDC_PANEL_MAX)         ||
           (id >= IDC_EDITOR_MIN        && id <= IDC_EDITOR_MAX)        ||
...
inline int GetMainPaneFromControlId(int id) {
    if (id >= IDC_FILE_EXPLORER_MIN && id <= IDC_FILE_EXPLORER_MAX) return static_cast<int>(MainPane::FileExplorer);
    if (id >= IDC_PANEL_MIN         && id <= IDC_PANEL_MAX)         return static_cast<int>(MainPane::TerminalDebug);
    if (id >= IDC_EDITOR_MIN        && id <= IDC_EDITOR_MAX)        return static_cast<int>(MainPane::Editor);
    if (id >= IDC_COPILOT_MIN       && id <= IDC_COPILOT_MAX)       return static_cast<int>(MainPane::AIChat);
    return -1;
}
