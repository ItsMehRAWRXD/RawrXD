...
private:
    ...
    void* m_nvmeTempSeries[5];       // Line series (was void*)
    void* m_gpuTempSeries;           // Line series (was void*)
    void* m_cpuTempSeries;           // Line series (was void*)
    void* m_predictedTempSeries;     // Dotted prediction line (was void*)
    void* m_predictionRangeSeries;   // Confidence band (was void*)
    void* m_thresholdMarkers;        // Scatter series (was void*)
    void* m_tempXAxis;               // Value axis (was void*)
    void* m_tempYAxis;               // Value axis (was void*)
    
    // Load balancer chart
    void* m_loadChartView;           // Chart view handle
    void* m_loadChart;               // Chart object handle
    void* m_loadSeries[5];           // Line series (was void*)
    void* m_selectedDriveSeries;     // Line series (was void*)
    
    // Prediction chart
    void* m_predChartView;           // Chart view handle
    void* m_predChart;               // Chart object handle
    void* m_ewmaSeries;              // Line series (was void*)
    void* m_slopeSeries;             // Line series (was void*)
    void* m_confidenceSeries;        // Line series (was void*)
    
    // ═══════════════════════════════════════════════════════════════════════════
    // UI Components - Config Tab
    // ═══════════════════════════════════════════════════════════════════════════
    
    void* m_alphaSpinBox;            // Double spin (was void*)
    void* m_historySizeSpinBox;
    void* m_thermalThresholdSpinBox;  // Double spin (was void*)
    void* m_emergencyThresholdSpinBox; // Double spin (was void*)
    ...
    void* m_driveSelectionTable;  // HWND list-view control (was void*)
...
private:
    ...
    // Compact widget runtime state (queried by Win32/web shell adapters).
    float m_lastMaxTemp;
    int m_lastThrottlePercent;
    double m_lastPredictedTemp;
    double m_lastPredictionConfidence;
    int64_t m_lastPredictionHorizonMs;
    int m_hottestDriveIndex;
    bool m_expandRequestedFlag;
