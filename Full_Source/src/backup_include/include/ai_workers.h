...
class AITrainingWorker
{
  ...
  public:
    ...
    // Control methods
    void startTraining(const AIDigestionDataset& dataset, const std::string& modelName, const std::string& outputPath,
...
class AIWorkerManager
{
  ...
  public:
    ...
    void startTrainingWorker(AITrainingWorker* worker, const AIDigestionDataset& dataset, const std::string& modelName,
                             const std::string& outputPath, const AITrainingWorker::TrainingConfig& config);
...
// Native invoke queue (replaces QMetaObject::invokeMethod). Call from main/UI thread to drain.
