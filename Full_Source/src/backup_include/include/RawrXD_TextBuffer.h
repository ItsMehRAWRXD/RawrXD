...
private:
    ...
    bool validateTree(Node* node, int& totalChars, int& totalLines) const;
    void dumpTree(Node* node, int depth, std::wstringstream& ss) const;
    
    // UTF-16 surrogate pair handling
    bool isHighSurrogate(wchar_t c) const { return (c >= 0xD800 && c <= 0xDBFF); }
    bool isLowSurrogate(wchar_t c) const { return (c >= 0xDC00 && c <= 0xDFFF); }
    bool isValidCodePoint(int pos) const;
    ...
    // Helper for balance
    Node* buildTreeFromLeaves(const std::vector<Node*>& leaves, int start, int end);
