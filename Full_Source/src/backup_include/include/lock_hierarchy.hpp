...
public:
    ...
    struct AuditorStats {
        ...
        AuditorStats() = default;
        AuditorStats(const AuditorStats& other)
            : totalAcquisitions(other.totalAcquisitions.load(std::memory_order_relaxed))
            , totalReleases(other.totalReleases.load(std::memory_order_relaxed))
            , violations(other.violations.load(std::memory_order_relaxed))
            , maxConcurrentHeld(other.maxConcurrentHeld.load(std::memory_order_relaxed))
            {}
        AuditorStats& operator=(const AuditorStats& other) {
            totalAcquisitions.store(other.totalAcquisitions.load(std::memory_order_relaxed), std::memory_order_relaxed);
            totalReleases.store(other.totalReleases.load(std::memory_order_relaxed), std::memory_order_relaxed);
            violations.store(other.violations.load(std::memory_order_relaxed), std::memory_order_relaxed);
            maxConcurrentHeld.store(other.maxConcurrentHeld.load(std::memory_order_relaxed), std::memory_order_relaxed);
            return *this;
        }
    };
