...
struct JournalStats {
    ...
    JournalStats(const JournalStats& other)
        : totalWrites(other.totalWrites.load(std::memory_order_relaxed))
        , totalCommits(other.totalCommits.load(std::memory_order_relaxed))
        , totalRollbacks(other.totalRollbacks.load(std::memory_order_relaxed))
        , totalRecoveries(other.totalRecoveries.load(std::memory_order_relaxed))
        , checkpointCount(other.checkpointCount.load(std::memory_order_relaxed))
        , diskBytesUsed(other.diskBytesUsed)
        , oldestSequenceId(other.oldestSequenceId)
        , newestSequenceId(other.newestSequenceId)
        {}
    JournalStats& operator=(const JournalStats& other) {
        totalWrites.store(other.totalWrites.load(std::memory_order_relaxed), std::memory_order_relaxed);
        totalCommits.store(other.totalCommits.load(std::memory_order_relaxed), std::memory_order_relaxed);
        totalRollbacks.store(other.totalRollbacks.load(std::memory_order_relaxed), std::memory_order_relaxed);
        totalRecoveries.store(other.totalRecoveries.load(std::memory_order_relaxed), std::memory_order_relaxed);
        checkpointCount.store(other.checkpointCount.load(std::memory_order_relaxed), std::memory_order_relaxed);
        diskBytesUsed = other.diskBytesUsed;
        oldestSequenceId = other.oldestSequenceId;
        newestSequenceId = other.newestSequenceId;
        return *this;
