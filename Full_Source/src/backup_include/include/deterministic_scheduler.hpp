...
struct SchedulerStats {
    ...
    SchedulerStats(const SchedulerStats& other)
        : tasksEnqueued(other.tasksEnqueued.load(std::memory_order_relaxed))
        , tasksExecuted(other.tasksExecuted.load(std::memory_order_relaxed))
        , barriersHit(other.barriersHit.load(std::memory_order_relaxed))
        , ticksAdvanced(other.ticksAdvanced.load(std::memory_order_relaxed))
        , replaySteps(other.replaySteps.load(std::memory_order_relaxed))
        , currentTick(other.currentTick)
        , mode(other.mode)
        {}
    SchedulerStats& operator=(const SchedulerStats& other) {
        tasksEnqueued.store(other.tasksEnqueued.load(std::memory_order_relaxed), std::memory_order_relaxed);
        tasksExecuted.store(other.tasksExecuted.load(std::memory_order_relaxed), std::memory_order_relaxed);
        barriersHit.store(other.barriersHit.load(std::memory_order_relaxed), std::memory_order_relaxed);
        ticksAdvanced.store(other.ticksAdvanced.load(std::memory_order_relaxed), std::memory_order_relaxed);
        replaySteps.store(other.replaySteps.load(std::memory_order_relaxed), std::memory_order_relaxed);
        currentTick = other.currentTick;
        mode = other.mode;
        return *this;
    }
