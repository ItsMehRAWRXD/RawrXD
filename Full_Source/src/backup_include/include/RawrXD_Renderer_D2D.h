...
public:
    ...
    // Full per-token colored text rendering via custom IDWriteTextRenderer
    void drawStyledTextColored(const Point& p, const String& text, const Font& font,
                               const std::vector<TextRun>& runs, const Color& defaultColor = Color::Black);

    // DirectWrite advanced text layout with font fallback + ligature control
    void drawTextAdvanced(const Point& p, const String& text, const Font& font, const Color& color,
                          float maxWidth = 10000.0f, bool enableLigatures = true);
