...
inline void stroke_rect(Canvas& c, int x, int y, int w, int h, float thickness, const Color& col) {
    int t = static_cast<int>(thickness * 0.5f);
    // Top
    fill_rect(c, x - t, y - t, w + 2 * t, static_cast<int>(thickness), col);
    // Bottom
    fill_rect(c, x - t, y + h - t, w + 2 * t, static_cast<int>(thickness), col);
    // Left
    fill_rect(c, x - t, y, static_cast<int>(thickness), h, col);
    // Right
    fill_rect(c, x + w - t, y, static_cast<int>(thickness), h, col);
}
