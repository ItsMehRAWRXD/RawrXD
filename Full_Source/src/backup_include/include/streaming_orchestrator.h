...
// Stream model weights from disk (opens file, memory maps, initializes layer states). Returns 1/0.
int       SO_StreamWeights(const char* modelPath);

// Close weight stream and release resources. Returns 1/0.
int       SO_CloseWeightStream(void);

// Get pointer to weights for a specific layer (may trigger remapping). Returns ptr or NULL.
void*     SO_GetLayerWeights(uint64_t layerId, uint64_t* outSize);

// Get the number of layers in the currently streamed model.
uint32_t  SO_GetStreamedLayerCount(void);

// Get the total file size of the streamed model.
