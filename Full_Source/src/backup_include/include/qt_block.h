...
protected:
    ...
    static QString tr(const char* s, const char* /*disambiguation*/ = nullptr, int /*n*/ = -1) { return QString(s ? s : ""); }
private:
    QString objectName_;
};
// Q_ARG / invokeMethod stubs for Qt-free build (no MOC)
struct QGenericArgument { const char* name; const void* data; QGenericArgument(const char* n = nullptr, const void* d = nullptr) : name(n), data(d) {} };
#define Q_ARG(type, data) QGenericArgument(#type, static_cast<const void*>(&data))
class QMetaObject { public: struct Connection {}; static Connection connectSlotsByName(QObject*) { return {}; }
    static bool invokeMethod(QObject* obj, const char* method, int type, ...) { (void)obj; (void)method; (void)type; return true; }
};
#ifndef QTR_H
#define QTR_H
inline QString tr(const char* s) { return QString(s ? s : ""); }
#endif
#endif
...
#ifndef QBUFFER_H
#define QBUFFER_H
class QBuffer : public QIODevice { std::vector<uint8_t> data_; public: QBuffer(QObject* p = nullptr) : QIODevice(p) {} void setData(const std::vector<uint8_t>& d) { data_ = d; } std::vector<uint8_t> data() const { return data_; } bool open(int mode) { (void)mode; return true; } };
#endif
#ifndef QAUDIO_H
#define QAUDIO_H
namespace QAudio { enum State { ActiveState, IdleState, StoppedState, SuspendedState }; }
#endif
#ifndef QSETTINGS_H
#define QSETTINGS_H
...
class QAudioFormat { public: enum ChannelConfig { ChannelConfigMono, ChannelConfigStereo }; enum SampleFormat { Unknown, Int16, Float }; QAudioFormat()=default; void setSampleRate(int){} void setChannelConfig(int){} void setChannelCount(int){} void setSampleFormat(int){} int sampleRate()const{return 16000;} int channelCount()const{return 1;} int sampleFormat()const{return 0;} bool isValid()const{return true;} };
class QAudioDevice { public: QAudioDevice()=default; QString description()const{return {};} bool isFormatSupported(const QAudioFormat&)const{return true;} QAudioFormat preferredFormat()const{return {};} };
class QAudioDeviceInfo{};class QAudioSource{public:QAudioSource(QObject*=nullptr){} void start(QIODevice*){}};
