...
// ============================================================================
// llama.cpp dynamic-bridge exports (implemented in src/asm/RawrXD_LlamaBridge.asm)
// Only available when MASM is enabled in the build.
// ============================================================================
#if defined(RAWR_HAS_MASM) && (RAWR_HAS_MASM == 1)
extern "C" {
    void* LlamaBridge_CreateA(const char* modelPath);
    void  LlamaBridge_Destroy(void* bridge);
    int   LlamaBridge_GenerateStreamA(void* bridge, const char* prompt,
                                      void (*cb)(const char* piece, int len, void* user),
                                      void* user);
    void  LlamaBridge_RequestStop(void* bridge);
}
#endif
...
PatchResult NativeInferencePipeline::Init(const PipelineConfig& cfg) {
    ...
    m_config = cfg;
    m_backend.store(InferenceBackend::None, std::memory_order_release);
    m_llamaBridge = nullptr;

    // Initialize the core engine
    PatchResult r = m_core.Init();
    m_coreInitialized = r.success;
    if (!m_coreInitialized) {
        // Still allow the pipeline to run with llama.dll backend only.
        m_state.store(PipelineState::Idle);
    }

    // Set thread count
    if (m_coreInitialized && cfg.inferenceThreads > 0) {
    ...
    return PatchResult::ok(m_coreInitialized
        ? "Pipeline: initialized"
        : "Pipeline: initialized (llama-only; LocalAICore init failed)");
}
...
PatchResult NativeInferencePipeline::Shutdown() {
    ...
    // Unload llama backend if active
    if (m_llamaBridge) {
#if defined(RAWR_HAS_MASM) && (RAWR_HAS_MASM == 1)
        LlamaBridge_Destroy(m_llamaBridge);
#endif
        m_llamaBridge = nullptr;
        m_backend.store(InferenceBackend::None, std::memory_order_release);
    }
    ...
    // Unload model
    if (m_coreInitialized && m_core.IsModelLoaded()) {
        m_core.UnloadModel();
    }
...
PatchResult NativeInferencePipeline::LoadModel(const char* ggufPath) {
    ...
    if (!ggufPath || ggufPath[0] == '\0') {
        return PatchResult::error("Pipeline: invalid model path");
    }

    if (m_llamaBridge || (m_coreInitialized && m_core.IsModelLoaded())) {
    ...
    // Prefer llama.dll backend (no service/network deps); fall back to LocalAICore.
    void* bridge = nullptr;
#if defined(RAWR_HAS_MASM) && (RAWR_HAS_MASM == 1)
    bridge = LlamaBridge_CreateA(ggufPath);
#endif
    if (bridge) {
        m_llamaBridge = bridge;
        m_backend.store(InferenceBackend::LlamaDll, std::memory_order_release);

        strncpy_s(m_config.modelPath, ggufPath, sizeof(m_config.modelPath) - 1);
        m_state.store(PipelineState::Ready);
        return PatchResult::ok("Pipeline: model loaded (llama.dll backend)");
    }

    if (!m_coreInitialized || !m_core.IsReady()) {
        m_state.store(PipelineState::Error);
        if (m_config.targetHWND && m_config.postMessages) {
            PostMessageA((HWND)m_config.targetHWND, WM_NATIVE_AI_ERROR, 0, 0);
        }
        return PatchResult::error("Pipeline: llama.dll backend unavailable and LocalAICore not initialized");
    }
    ...
    m_backend.store(InferenceBackend::LocalAICore, std::memory_order_release);
...
PatchResult NativeInferencePipeline::UnloadModel() {
    ...
    if (m_llamaBridge) {
        ...
        m_backend.store(InferenceBackend::None, std::memory_order_release);
        m_state.store(PipelineState::Idle);
        return PatchResult::ok("Pipeline: model unloaded");
    }
    ...
    return PatchResult::ok("Pipeline: no model loaded");
}
...
void NativeInferencePipeline::InferenceWorker(const char* prompt,
    ...
    const InferenceBackend backend = m_backend.load(std::memory_order_acquire);

    if (backend == InferenceBackend::LlamaDll && m_llamaBridge) {
#if defined(RAWR_HAS_MASM) && (RAWR_HAS_MASM == 1)
        int ok = LlamaBridge_GenerateStreamA(
            m_llamaBridge, prompt,
            &NativeInferencePipeline::LlamaTokenCallbackTrampoline,
            this);
        ...
        if (ok) {
            ...
            if (m_config.targetHWND && m_config.postMessages) {
                float tps = CurrentTokensPerSec();
                uint32_t bits = 0;
                static_assert(sizeof(bits) == sizeof(tps), "float bits size mismatch");
                memcpy(&bits, &tps, sizeof(bits));
    ...
    if (result.success) {
        ...
        if (m_config.targetHWND && m_config.postMessages) {
            // Note: existing UI interprets lParam as float bits.
            uint32_t bits = 0;
            static_assert(sizeof(bits) == sizeof(result.tokensPerSec), "float bits size mismatch");
            memcpy(&bits, &result.tokensPerSec, sizeof(bits));
            PostMessageA((HWND)m_config.targetHWND,
                        WM_NATIVE_AI_COMPLETE,
...
void NativeInferencePipeline::LlamaTokenCallbackTrampoline(const char* piece, int len, void* userData) {
    auto* self = static_cast<NativeInferencePipeline*>(userData);
    if (!self) return;

    // Best-effort cancellation for llama backend.
    if (self->m_stopRequested.load(std::memory_order_relaxed)) {
        if (self->m_llamaBridge) {
#if defined(RAWR_HAS_MASM) && (RAWR_HAS_MASM == 1)
            LlamaBridge_RequestStop(self->m_llamaBridge);
#endif
        }
        return;
    }
    ...
    self->OnToken(0, piece, (uint32_t)len);
}
...
PatchResult NativeInferencePipeline::StopInference() {
    ...
    if (m_backend.load(std::memory_order_acquire) == InferenceBackend::LlamaDll && m_llamaBridge) {
#if defined(RAWR_HAS_MASM) && (RAWR_HAS_MASM == 1)
        LlamaBridge_RequestStop(m_llamaBridge);
#endif
    }
...
PatchResult NativeInferencePipeline::ResetContext() {
    ...
    if (m_backend.load(std::memory_order_acquire) == InferenceBackend::LlamaDll) {
        // Not currently supported by the minimal ASM bridge; treat as a no-op.
        return PatchResult::ok("Pipeline: context reset not supported (llama.dll backend)");
    }
...
uint32_t NativeInferencePipeline::ContextLength() const {
    ...
    return m_coreInitialized ? m_core.ContextLength() : 0;
}
...
void NativeInferencePipeline::GetDiagnostics(char* buf, size_t bufLen) const {
    ...
    const char* backendName =
        (backend == InferenceBackend::LlamaDll) ? "llama.dll" :
        (backend == InferenceBackend::LocalAICore) ? "LocalAICore" : "None";

    if (backend == InferenceBackend::LlamaDll) {
        snprintf(buf, bufLen,
...
void NativeInferencePipeline::GetModelInfo(char* buf, size_t bufLen) const {
    ...
    if (m_backend.load(std::memory_order_acquire) == InferenceBackend::LlamaDll) {
        ...
        snprintf(buf, bufLen,
            "Backend: llama.dll | Model: %s\n"
            "Sampling: greedy argmax (bridge baseline)\n",
            m_config.modelPath[0] ? m_config.modelPath : "(unknown)"
        );
        return;
    }
