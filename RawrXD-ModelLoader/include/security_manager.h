...
public:
     ...
     */
    QString refreshToken(const QString& username);
...
signals:
    /// Emitted when encryption key is rotated successfully
    void keyRotationCompleted(const QString& newKeyId);

    /// Emitted when token refresh fails
    void tokenRefreshFailed(const QString& username, const QString& reason);

    /// Emitted on security event (suspicious activity, unauthorized access)
    void securityEventOccurred(const QString& eventType, const QString& details);

    /// Emitted when certificate pinning fails
    void certificatePinningFailed(const QString& domain);

    /// Emitted when unauthorized access attempt detected
    void unauthorizedAccessAttempt(const QString& username, const QString& resource);
