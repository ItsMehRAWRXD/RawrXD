...
// Slow scalar reference GEMM (decode every access)
...
// Pack fp32 weights in range [-8,7] to Q4_0 with given scale (simple packer for test)
...
int main() {
    ...
    const int K = 128;  // shared
    const int N = 64;   // cols of B/C
    ...
    for (auto& v : Bfp32) v = std::round(dist(rng) * 7.0f); // stay within [-7,7]
    ...
    ggml_gemm_q4_0_avx2(M, N, K, A.data(), Bq4.data(), scale, Copt.data());
