...
class Engine:
    ...
    async def step(self) -> None:
        ...
        # /ask fallback: require actual findings, not merely "some bot claimed supports"
        if event.kind == "llm.question" and not answered and not handed_off:
            mode = (event.payload or {}).get("mode", "response_gen")
            if not produced:
                if mode == "response_gen":
                    await self._start_response_generation(event)
                else:
                    self._run_default_ask(event)
                return
            # Bots ran but produced nothing usable → still generate / fallback
            if mode == "response_gen":
                await self._start_response_generation(event)
            else:
                self._run_default_ask(event)
    ...
    async def _start_response_generation(self, event: Event) -> None:
        """Weightless contact path when no bot produced a finding (Response Gen)."""
        question = event.payload.get("question", "")
        request_id = event.payload.get("request_id", "")
        # Emit plan topology as findings — not neural weights
        self.history.append(
            Finding(
                bot="HexMag-Engine",
                score=0.6,
                labels={LABEL_PARTIAL, "response.plan"},
                rationale="response.plan: seed topology on contact",
                data={
    ...
    def _run_default_ask(self, event: Event) -> None:
        ...
        finding = Finding(
            bot="HexMag-Engine",
            score=1.0,
            labels={LABEL_ANSWER, LABEL_LLM_ANSWER, "response.final"},
            rationale="Direct answer generated by HexMag engine (oracle not required)",
            data={
                "answer": answer,
                "question": question,
                "request_id": event.payload.get("request_id"),
