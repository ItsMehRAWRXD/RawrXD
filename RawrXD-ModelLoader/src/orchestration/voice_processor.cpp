...
void VoiceProcessor::setupAudioFormat()
{
    ...
    m_audioFormat.setSampleSize(m_config.sampleSize);
    m_audioFormat.setCodec("audio/pcm");
    m_audioFormat.setByteOrder(m_config.byteOrder);
    m_audioFormat.setSampleType(m_config.sampleType);
...
bool VoiceProcessor::startRecording()
{
    ...
    try {
        ...
        if (!inputDevice.isFormatSupported(m_audioFormat)) {
            logStructured("ERROR", "Audio format not supported by device", QJsonObject{{"device", inputDevice.deviceName()}});
        ...
        connect(m_audioInput, &QAudioInput::stateChanged, this, &VoiceProcessor::handleAudioStateChanged);
