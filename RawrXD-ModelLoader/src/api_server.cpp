...
bool APIServer::Start(uint16_t port) {
    if (is_running_) {  // Scalar: no .load() needed
    ...
    // Create HTTP server (synchronous, scalar operation)
    ...
    svr->Get("/api/tags", [](const httplib::Request& req, httplib::Response& res) {
        ...
        std::string json_response = "{\"models\":[{\"name\":\"loaded-model\",\"model\":\"loaded-model\",\"modified_at\":\"2025-12-01T00:00:00Z\",\"size\":25463410912,\"digest\":\"scalar-cpu\",\"details\":{\"format\":\"gguf\",\"family\":\"llama\",\"parameter_size\":\"69B\",\"quantization_level\":\"Q2_K\"}}]}";
        res.set_content(json_response, "application/json");
    });
    
    // Start server synchronously (scalar, no background thread)
    std::cout << "API Server started on port " << port << " (scalar mode)" << std::endl;
    std::cout << "Endpoints:" << std::endl;
    std::cout << "  POST /api/generate" << std::endl;
    std::cout << "  POST /v1/chat/completions" << std::endl;
    std::cout << "  GET  /api/tags" << std::endl;
    
    // Scalar: server runs synchronously in main thread
...
bool APIServer::Stop() {
    is_running_ = false;
    std::cout << "API Server stopped (scalar mode)" << std::endl;
    return true;
}
