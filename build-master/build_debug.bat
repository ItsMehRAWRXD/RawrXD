...
echo [BUILD] RawrXD Complete - Debug Mode
echo ====================================
...
echo /DLL > link_debug.rsp
echo /DEBUG >> link_debug.rsp
echo /INCREMENTAL:NO >> link_debug.rsp
echo /ENTRY:DllMain >> link_debug.rsp
echo /OUT:"%BIN_DIR%\RawrXD_Unified.dll" >> link_debug.rsp
echo /PDB:"%BIN_DIR%\RawrXD_Unified.pdb" >> link_debug.rsp

REM Add all objects from build-master
cd /d "%BUILD_DIR%"
set "OBJ_COUNT=0"
for %%f in (*.obj) do (
    echo "%BUILD_DIR%\%%f" >> "%DEBUG_PIPE%\link_debug.rsp"
    set /a OBJ_COUNT+=1
)

echo kernel32.lib >> "%DEBUG_PIPE%\link_debug.rsp"

echo.
echo Objects in response file: %OBJ_COUNT%
...
echo.
echo [LINK] Linking...
"%LINK%" @link_debug.rsp /LIBPATH:"%LIBPATH1%" /LIBPATH:"%LIBPATH2%" /LIBPATH:"%LIBPATH3%"
...
REM DO NOT cleanup - keep for inspection
echo.
echo Response file preserved at: %DEBUG_PIPE%\link_debug.rsp
