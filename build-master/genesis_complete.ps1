...
# PowerShell Edition with proper directory handling
...
$rspPath = Join-Path $DEBUG_PIPE "link.rsp"
$rspContent = @"
/DLL
/DEBUG
/INCREMENTAL:NO
...
/PDB:"$BIN_DIR\RawrXD_Unified.pdb"
"@

# Add all objects
Get-ChildItem "$BUILD_DIR\*.obj" | ForEach-Object {
    $rspContent += "`n`"$($_.FullName)`""
}

$rspContent += "`nkernel32.lib"

Set-Content -Path $rspPath -Value $rspContent -Force

Write-Host "Response file created with $objCount objects"

# Run linker from debug-pipeline directory
Push-Location $DEBUG_PIPE

$linkArgs = "@link.rsp", "/LIBPATH:`"$LIBPATH1`"", "/LIBPATH:`"$LIBPATH2`"", "/LIBPATH:`"$LIBPATH3`""
$process = Start-Process -FilePath $LINK -ArgumentList $linkArgs -Wait -PassThru -NoNewWindow
