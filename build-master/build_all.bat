...
echo [BUILD] RawrXD Full Unified Build
echo =================================
...
REM Create response file for objects
echo. > objects.rsp
for %%f in (*.obj) do (
    echo "%BUILD_DIR%\%%f" >> objects.rsp
)

"%LINK%" @objects.rsp /DLL /OUT:"%BIN_DIR%\RawrXD_Full.dll" /PDB:"%BIN_DIR%\RawrXD_Full.pdb" /LIBPATH:"%LIBPATH1%" /LIBPATH:"%LIBPATH2%" /LIBPATH:"%LIBPATH3%" kernel32.lib user32.lib gdi32.lib advapi32.lib shell32.lib ole32.lib /SUBSYSTEM:WINDOWS /MACHINE:X64 /LARGEADDRESSAWARE /DEBUG /INCREMENTAL:NO
