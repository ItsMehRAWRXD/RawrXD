...
echo [BUILD] RawrXD Final - Complete System
echo ======================================
...
echo /DLL > link_final.rsp
echo /DEBUG >> link_final.rsp
echo /INCREMENTAL:NO >> link_final.rsp
echo /ENTRY:DllMain >> link_final.rsp
echo /OUT:"%BIN_DIR%\RawrXD_Unified.dll" >> link_final.rsp
echo /PDB:"%BIN_DIR%\RawrXD_Unified.pdb" >> link_final.rsp

REM Add all objects from build-master
cd /d "%BUILD_DIR%"
for %%f in (*.obj) do (
    echo "%BUILD_DIR%\%%f" >> "%DEBUG_PIPE%\link_final.rsp"
)

echo kernel32.lib >> "%DEBUG_PIPE%\link_final.rsp"

cd /d "%DEBUG_PIPE%"

echo.
echo Linking with response file...
"%LINK%" @link_final.rsp /LIBPATH:"%LIBPATH1%" /LIBPATH:"%LIBPATH2%" /LIBPATH:"%LIBPATH3%"
...
if exist "%BIN_DIR%\RawrXD_Unified.dll" (
    ...
    REM Calculate total object size
    cd /d "%BUILD_DIR%"
    set "TOTAL_SIZE=0"
    for %%f in (*.obj) do (
        for %%z in ("%%f") do set /a TOTAL_SIZE+=%%~zz
    )
    echo Total object size: %TOTAL_SIZE% bytes
) else (
    echo FAILED: RawrXD_Unified.dll not created
)

REM Cleanup
del link_final.rsp 2>nul
