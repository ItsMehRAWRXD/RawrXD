...
bool ToolRegistry::LoadFromDisk(const std::wstring& path) {
    ...
    std::ifstream file(std::filesystem::path(m_registryPath));
    if (!file.is_open()) {
        LOG_ERROR("ToolRegistry: failed to open registry: " + ToUtf8(m_registryPath));
        return false;
    }
...
ToolResult ToolRegistry::HandleBuildProject(const json& args, std::string& output) {
    ...
    std::filesystem::path root = m_projectRoot.empty() ? std::filesystem::current_path() : m_projectRoot;
