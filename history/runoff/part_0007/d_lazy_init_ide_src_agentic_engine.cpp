...
// Ensure TOOL_REGISTRY_INIT_IMPLEMENTED is always defined for production builds
#ifdef NDEBUG
#ifndef TOOL_REGISTRY_INIT_IMPLEMENTED
#define TOOL_REGISTRY_INIT_IMPLEMENTED 1
#endif
#endif
...
void AgenticEngine::initialize() {
    ...
    // Initialize observability sink for persistent logging/metrics
    if (!m_observabilitySink) {
        m_observabilitySink = new RawrXD::ObservabilitySink(this);
        
        // Configure from production config
        RawrXD::ProductionConfigManager& config = RawrXD::ProductionConfigManager::instance();
        config.loadConfig();
        if (config.isFeatureEnabled("structured_logging")) {
            m_observabilitySink->enableFileRotation(true);
            qInfo() << "[AgenticEngine] Structured logging enabled with file rotation";
        }
        
        if (config.isFeatureEnabled("otlp_export")) {
            QString endpoint = config.value("otlp_endpoint", "http://localhost:4317").toString();
            m_observabilitySink->setOtlpEndpoint(endpoint);
            m_observabilitySink->enableOtlp(true);
            qInfo() << "[AgenticEngine] OTLP export enabled to" << endpoint;
        }
        
        if (config.isFeatureEnabled("prometheus_export")) {
            QString endpoint = config.value("prometheus_endpoint", "http://localhost:9090").toString();
            m_observabilitySink->setPrometheusEndpoint(endpoint);
            m_observabilitySink->enablePrometheus(true);
            qInfo() << "[AgenticEngine] Prometheus export enabled to" << endpoint;
        }
        
        qInfo() << "[AgenticEngine] Observability sink initialized";
    }
    
    // Observability settings are configured via ProductionConfigManager
    ...
    if (!m_toolRegistry) {
        ...
        toolsRegistered = initializeAllTools(m_toolRegistry);
        if (toolsRegistered) {
    ...
    // === Enhanced Tool Registry Initialization ===
    // Ensure mandatory tool registration
    if (!toolsRegistered) {
        qCritical() << "[AgenticEngine] Critical failure: Mandatory tools not registered. Aborting initialization.";
        delete m_toolRegistry;
        m_toolRegistry = nullptr;
        throw std::runtime_error("Tool registry initialization failed: Mandatory tools missing.");
    }
    
    // Connect observability sink to automatically persist logs and metrics
    if (m_observabilitySink) {
        connect(this, &AgenticEngine::logWritten, [this](const AgenticObservability::LogEntry& entry) {
            m_observabilitySink->writeLog(entry);
            m_logWritten = true;
        });

        connect(this, &AgenticEngine::metricRecorded, [this](const QString& metricName) {
            // Metrics are already written via recordMetric calls
            m_metricRecorded = true;
        });
        
        qInfo() << "[AgenticEngine] Observability sink connected to persistence layer";
    }
...
bool AgenticEngine::loadModelAsync(const std::string& modelPath) {
    try {
        ...
        // === Enhanced GGUF Quantization Validation ===
        // Validate against ggml reference for supported quantization types
        const std::set<std::string> supportedQuantizationTypes = {"Q2_K", "Q4_0", "Q4_1", "Q5_0", "Q5_1", "Q8_0"};
        
        // Find quantization version key
        int qver = gguf_find_key(ctx, "general.quantization_version");
        
        if (qver >= 0) {
            const char *qt = gguf_get_val_str(ctx, qver);
            qInfo() << "Quantization type:" << qt;

            if (supportedQuantizationTypes.find(qt) == supportedQuantizationTypes.end()) {
                qCritical() << "Unsupported quantization type:" << qt;
                gguf_free(ctx);
                m_modelLoaded = false;
                emit modelLoadingFinished(false, QString::fromStdString(modelPath));
                return false;
