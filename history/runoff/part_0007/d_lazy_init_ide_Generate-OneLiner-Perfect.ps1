# RawrXD One-Liner Generator - Perfect Version
...
param(
    ...
    [Parameter(Mandatory=$false)]
    [string]$OutputFile = "D:\lazy init ide\OneLiner-AutonomousAgent-Perfect.txt",
...
Write-Host "║                                                                   ║" -ForegroundColor Magenta
Write-Host "║         RawrXD ONE-LINER GENERATOR - PERFECT                    ║" -ForegroundColor Magenta
Write-Host "║                    Version 3.0.3 - Production Ready               ║" -ForegroundColor Magenta
Write-Host "║                                                                   ║" -ForegroundColor Magenta
Write-Host "╚═══════════════════════════════════════════════════════════════════╝" -ForegroundColor Magenta
Write-Host ""
...
# Completely remove param block and replace with $args parsing
Write-Host "Removing param block and adding `$args parsing..." -ForegroundColor Cyan
try {
    # Find and remove the entire param block including all parameters
    # This matches from "param(" to the closing ")" including all content in between
    $paramPattern = '(?s)param\s*\([^\)]+Mandatory=\$false[^\)]+\)\s*\n\s*\[int\]\$MaxIterations[^\n]+\n\s*\[Parameter[^\)]+\)\s*\n\s*\[int\]\$SleepIntervalMs[^\n]+\n\s*\[Parameter[^\)]+\)\s*\n\s*\[switch\]\$WhatIf[^\n]+\n\s*\)\s*'
    $scriptContent = $scriptContent -replace $paramPattern, ''
    
    # Add $args parsing at the very beginning of the script
    $argsParsing = @"
...
Write-Host "Generate-OneLiner-Perfect.ps1" -ForegroundColor Green -NoNewline
