...
function Write-DeploymentLog {
    ...
    if (${Script:DeploymentState}.Config -and ${Script:DeploymentState}.Config.deployment.logPath) {
        ...
        try {
            ...
            # If we can't write to the log file, skip file logging
        }
    }
}
...
function Start-UltimateDeployment {
    ...
    # In dry run mode, we bypass admin check
    if ($DryRun) {
        $adminCheckFailed = -not $prerequisites.AdminRights
        if ($adminCheckFailed) {
            $failedPrerequisites = $failedPrerequisites - 1
            Write-DeploymentLog -Message "Admin check bypassed for DryRun" -Level Warning
        }
    }
...
try {
    ...
    $result = Start-UltimateDeployment -ConfigPath $ConfigPath -Mode $Mode -SkipTesting:$SkipTesting -SkipOptimization:$SkipOptimization -SkipSecurity:$SkipSecurity -DryRun:$DryRun
    exit 0
} catch {
    exit 1
}