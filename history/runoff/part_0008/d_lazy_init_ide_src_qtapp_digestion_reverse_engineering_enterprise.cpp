...
QStringList DigestionReverseEngineeringSystem::collectFiles(const QString &rootDir, int maxFiles, const QStringList &excludes) {
    QStringList files;
    QDirIterator it(rootDir, QDir::Files | QDir::Dirs | QDir::NoDotAndDotDot, QDirIterator::Subdirectories);
    ...
    while (it.hasNext()) {
        ...
        for (const QString &pattern : excludes) {
            if (file.contains(pattern) || info.fileName().contains(pattern, Qt::CaseInsensitive)) {
                excluded = true;
                break;
            }
        }
...
QString DigestionReverseEngineeringSystem::generatePatchFile(const QString &originalDir, const QString &modifiedDir) {
    QString patch;
    QDirIterator it(modifiedDir, QDir::Files | QDir::Dirs | QDir::NoDotAndDotDot, QDirIterator::Subdirectories);
