...
bool VulkanCompute::DispatchMatMulHost(const float* input_a,
                                       const float* input_b,
                                       float* output,
                                       uint32_t M,
                                       uint32_t K,
    ...
    if (!input_a || !input_b || !output) {
        std::cerr << "DispatchMatMulHost received null pointers" << std::endl;
        return false;
    }

    if (!EnsureMatMulPipeline(spirv_path)) {
        std::cerr << "MatMul pipeline unavailable for host dispatch" << std::endl;
        return false;
    }
    ...
    VkBuffer bufferA = nullptr, bufferB = nullptr, bufferC = nullptr;
    VkDeviceMemory memoryA = nullptr, memoryB = nullptr, memoryC = nullptr;

    auto cleanup = [&](bool logErrors = false) {
        if (bufferA) vkDestroyBuffer(device_, bufferA, nullptr);
        if (memoryA) vkFreeMemory(device_, memoryA, nullptr);
        if (bufferB) vkDestroyBuffer(device_, bufferB, nullptr);
        if (memoryB) vkFreeMemory(device_, memoryB, nullptr);
        if (bufferC) vkDestroyBuffer(device_, bufferC, nullptr);
        if (memoryC) vkFreeMemory(device_, memoryC, nullptr);
        if (logErrors) {
            std::cerr << "DispatchMatMulHost cleaned up after failure" << std::endl;
        }
    };

    if (!AllocateBuffer(sizeA, bufferA, memoryA) ||
        !AllocateBuffer(sizeB, bufferB, memoryB) ||
        !AllocateBuffer(sizeC, bufferC, memoryC)) {
        cleanup(true);
        return false;
    }

    if (!CopyHostToBuffer(const_cast<float*>(input_a), bufferA, sizeA) ||
        !CopyHostToBuffer(const_cast<float*>(input_b), bufferB, sizeB)) {
        cleanup(true);
        return false;
    }

    // Reuse the descriptor/pipeline setup from the device-buffer variant
    std::vector<VkDescriptorBufferInfo> buffer_infos(3);
    std::vector<VkWriteDescriptorSet> writes(3);

    VkBuffer buffers[3] = { bufferA, bufferB, bufferC };
    size_t sizes[3] = { sizeA, sizeB, sizeC };
    ...
    VkDescriptorSet descriptor_set = nullptr;
    if (vkAllocateDescriptorSets(device_, &alloc_info, &descriptor_set) != VK_SUCCESS) {
        std::cerr << "Failed to allocate descriptor set for host MatMul" << std::endl;
        cleanup(true);
        return false;
    }
    ...
    auto it = shaders_.find("matmul");
    if (it == shaders_.end()) {
        std::cerr << "MatMul shader not loaded after EnsureMatMulPipeline" << std::endl;
        vkFreeDescriptorSets(device_, matmul_descriptor_pool_, 1, &descriptor_set);
        cleanup(true);
        return false;
    }
    ...
    bool success = ExecuteSingleTimeCommands([&](VkCommandBuffer cmd_buffer) {
        ...
        const uint32_t TILE_SIZE = 16;
        const uint32_t group_count_x = (N + TILE_SIZE - 1) / TILE_SIZE;
        const uint32_t group_count_y = (M + TILE_SIZE - 1) / TILE_SIZE;
        vkCmdDispatch(cmd_buffer, group_count_x, group_count_y, 1);
    });
    ...
    if (success) {
        success = CopyBufferToHost(bufferC, output, sizeC);
    }
