...
public:
    ...
    void startTestRun(const QString& testPattern = QString()) {
        setTestRunActive(true);
        m_currentTestPattern = testPattern;
        // Actual test execution would be triggered here
        emit testRunStarted(testPattern);
    }
    
    void finishTestRun(bool success, int passed, int failed) {
        setTestRunActive(false);
        emit testRunFinished(success, passed, failed);
    }

    void finishTestRun() {
        setTestRunActive(false);
        emit testRunFinished(true, 0, 0);
    }

signals:
    void testRunStarted(const QString& pattern);
...
public:
    ...
    // Connection handling
    void onConnectionEstablished(const QString& connectionInfo = QString()) {
        m_isConnected = true;
        if (queryEdit_) {
...
signals:
    void connectionEstablished(const QString& connectionInfo);
    void connectionLost();
