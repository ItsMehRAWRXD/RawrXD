...
void test_tool_execution() {
    ...
    toolDef.handler = [](const json& params) {
        const double a = params.contains("a") ? params["a"].get<double>() : 0.0;
        const double b = params.contains("b") ? params["b"].get<double>() : 0.0;
        json result;
        result["sum"] = a + b;
        return result;
    };
    ...
    std::cout << "✓ Tool execution successful: 5 + 3 = " << result.data["sum"].get<int>() << std::endl;
}
...
void test_logging_and_metrics() {
    ...
    for (int i = 0; i < 3; ++i) {
        ctx.registry->executeTool("test_metrics", json::object_type());
    }
...
void test_error_handling_and_retry() {
    ...
    auto result = ctx.registry->executeTool("test_retry", json::object_type());
...
void test_caching() {
    ...
    toolDef.handler = [&callCount](const json& params) {
        callCount++;
        json result = json::object_type();
        result["call_number"] = callCount;
        return result;
    };
...
void test_configuration_management() {
    ...
    auto result = ctx.registry->executeTool("test_config", json::object_type());
    assert(result.executionContext.status == ToolExecutionStatus::SkippedByToggle && 
           "Tool not disabled");
    
    // Re-enable
    ctx.registry->setToolEnabled("test_config", true);
    result = ctx.registry->executeTool("test_config", json::object_type());
...
void test_batch_execution() {
    ...
    batch.emplace_back("batch_tool_1", json::object_type());
    batch.emplace_back("batch_tool_2", json::object_type());
    batch.emplace_back("batch_tool_3", json::object_type());
...
void test_statistics_and_monitoring() {
    ...
    // Execute multiple times
    for (int i = 0; i < 5; ++i) {
        ctx.registry->executeTool("test_stats", json::object_type());
    }
    ...
    std::cout << "✓ Tool statistics: " << stats["total_executions"].get<int>() << " executions, "
              << stats["success_rate"].get<double>() << " success rate" << std::endl;
    std::cout << "✓ Metrics snapshot captured: " << snapshot["total_executions"].get<int>() << " total" << std::endl;
...
void test_execution_context_and_tracing() {
    ...
    auto result = ctx.registry->executeToolWithTrace("test_trace", json::object_type(),
