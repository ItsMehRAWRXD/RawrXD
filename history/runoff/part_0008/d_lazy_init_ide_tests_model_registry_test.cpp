...
{
    // Clear database state BEFORE creating registry to ensure test isolation
    // This ensures the registry starts with a clean slate
    {
        // Temporarily open connection to clear data
        QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE", "TestCleanupConnection");
        db.setDatabaseName(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + "/model_registry.db");
        if (db.open()) {
            QSqlQuery query(db);
            query.exec("DELETE FROM models");
            // Also reset auto-increment so IDs start at 1 again
            query.exec("DELETE FROM sqlite_sequence WHERE name='models'");
            db.close();
        }
        QSqlDatabase::removeDatabase("TestCleanupConnection");
    }
    ...
    // MUST call initialize() to set up the database connection (lazy init pattern)
...
void ModelRegistryTest::testRegisterModelWithAllFields()
{
    ...
    // Get the model by looking up the registered models (don't assume id=1)
    QVector<ModelVersion> models = m_registry->getAllModels();
    QCOMPARE(models.size(), 1);
    ModelVersion retrieved = models.first();
...
void ModelRegistryTest::testLoadModelsFromDatabase()
{
    ...
    // Note: Need to call initialize() to connect to database (lazy init pattern)
...
void ModelRegistryTest::testGetModelById()
{
    ...
    // Get the model via getAllModels to find its actual id
    QVector<ModelVersion> models = m_registry->getAllModels();
    QCOMPARE(models.size(), 1);
    
    // Now use the actual id to retrieve
    ModelVersion retrieved = m_registry->getModel(models.first().id);
...
void ModelRegistryTest::testStatusLabelOnFilter()
{
    ...
    // Set filter and process events to trigger the slot
    m_registry->m_filterCombo->setCurrentIndex(0);
    QCoreApplication::processEvents();  // Process the signal

    QString status = m_registry->m_statusLabel->text();
    // After filter change, status shows either "Showing" or "Loaded"
    QVERIFY(status.contains("Showing") || status.contains("Loaded") || status.contains("models"));
}
...
void ModelRegistryTest::testModelPersistenceAcrossInstances()
{
    ...
    // Note: Must call initialize() to load models from database
    ModelRegistry* registry2 = new ModelRegistry();
    registry2->initialize();
