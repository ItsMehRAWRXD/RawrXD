...
#ifdef USE_INFERENCE_ENGINE_STUB
...
namespace {
inline QString asQString(const QString& value) { return value; }
inline QString asQString(const std::string& value) { return QString::fromStdString(value); }
}
...
void GGUFServer::handleTagsRequest(HttpResponse& response) {
    ...
    if (m_engine && m_engine->isModelLoaded()) {
        QJsonObject model;
        model["name"] = asQString(m_engine->modelPath());
...
void GGUFServer::handleHealthRequest(HttpResponse& response) {
    ...
    if (m_engine && m_engine->isModelLoaded()) {
        responseObj["model_path"] = asQString(m_engine->modelPath());
    }
