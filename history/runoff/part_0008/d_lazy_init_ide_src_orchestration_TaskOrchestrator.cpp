...
RawrXD::Backend::ToolResult TaskOrchestrator::executeTool(const QString& toolName, const QJsonObject& params)
{
    ...
    std::string paramsJson = doc.toJson(QJsonDocument::Compact).toStdString();
    return m_toolExecutor.executeTool(toolName.toStdString(), paramsJson);
}
...
void TaskOrchestrator::createExecutionTab(const TaskDefinition& task)
{
    ...
    m_activeTasks[task.id].parameters["progressBar"] = QVariant::fromValue<quintptr>(
        reinterpret_cast<quintptr>(progressBar));
    m_activeTasks[task.id].parameters["contentArea"] = QVariant::fromValue<quintptr>(
