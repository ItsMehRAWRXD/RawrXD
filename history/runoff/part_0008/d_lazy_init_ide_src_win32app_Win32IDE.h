...
#ifndef HMENU_FROM_ID
#define HMENU_FROM_ID(x) reinterpret_cast<HMENU>(static_cast<INT_PTR>(x))
#endif
...
namespace RawrXD { namespace Agentic { class AgentLoop; } }
...
public:
    ...
    // Auto-digest test hook (public wrapper)
    bool triggerAutoDigest(const std::wstring& source, const std::wstring& output);
    ...
    std::unique_ptr<RawrXD::Agentic::AgentLoop> m_agentLoop;
...
private:
    ...
    bool queueDigestionJob(const std::wstring& source, const std::wstring& output);
    ...
    HACCEL m_hAccelTable;     // Accelerator table for hotkeys
...
private:
    ...
    void loadSettings();
    void saveSettings();
    int getConsentLevel() const;
    void setConsentLevel(int level);
    std::string getWatchPath() const;
    void setWatchPath(const std::string& path);
    int getWatcherSensitivity() const;
    void setWatcherSensitivity(int level);
    bool shouldDeliverWatcherEvent();
    
    // Status indicators
    void updateAgentStatusIndicators();
    ...
    int m_watcherSensitivity;
    std::chrono::steady_clock::time_point m_lastWatcherNotification;
    std::mutex m_watcherThrottleMutex;
};
