...
void Win32IDE::newFileInExplorer()
{
    ...
    try {
        ...
        newFile();  // newFile() handles its own success/failure reporting
        
        qInfo() << "[Win32IDE] INFO: New file creation initiated from Explorer";
...
void Win32IDE::newFolderInExplorer()
{
    ...
    // Simple folder creation using MessageBox for confirmation
    if (MessageBoxA(m_hwndMain, 
                    "Create a new folder named 'NewFolder' in the current directory?",
                    "Create New Folder", 
                    MB_OKCANCEL | MB_ICONQUESTION) == IDOK) {
        std::string newPath = m_explorerRootPath + "\\NewFolder";
        
        // If NewFolder exists, append a number
        int counter = 1;
        std::string basePath = newPath;
        while (fs::exists(newPath)) {
            newPath = basePath + "_" + std::to_string(counter++);
        }
