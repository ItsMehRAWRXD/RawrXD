...
# Start with core working modules only
$Script:workingFiles = @('masm_main','engine_debug','window','config_manager','orchestra','tab_control_minimal','file_tree_following_pattern','ui_layout')
$Script:compiledObjs = @()
...
if ($compiledObjs.Count -gt 0) {
    ...
    & $link /SUBSYSTEM:WINDOWS /OUT:"$out\AgenticIDEWin.exe" /LIBPATH:"$libPath" @compiledObjs `
        kernel32.lib user32.lib gdi32.lib 2>&1 | Select-String "error|warning|Creating"
