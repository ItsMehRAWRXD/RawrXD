...
# Resolve beacon relative to config directory if needed
$configDir = Split-Path -Parent (Resolve-Path $ConfigPath)
try {
  $resolvedBeacon = Resolve-Path -Path $beaconPath -ErrorAction Stop
} catch {
  $resolvedBeacon = Join-Path $configDir $beaconPath
}

if (!(Test-Path $resolvedBeacon)) {
  Write-Host "Beacon not found: $resolvedBeacon" -ForegroundColor Red
  exit 4
}

try {
  $beacon = Get-Content $resolvedBeacon -Raw | ConvertFrom-Json
} catch {
