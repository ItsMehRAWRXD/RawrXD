$Script:ML64 = "C:\VS2022Enterprise\VC\Tools\MSVC\14.50.35717\bin\Hostx64\x64\ml64.exe"
$Script:LINK = "C:\VS2022Enterprise\VC\Tools\MSVC\14.50.35717\bin\Hostx64\x64\link.exe"
$Script:LIB_PATH = "C:\Program Files (x86)\Windows Kits\10\Lib\10.0.22621.0\um\x64"
$Script:CDB = "C:\Program Files (x86)\Windows Kits\10\Debuggers\x64\cdb.exe"

Set-Location "D:\RawrXD-ExecAI"

# Assemble
& $ML64 /c /Zi RawrXD-GGUFAnalyzer-Complete.asm
if ($LASTEXITCODE -ne 0) { Write-Error "Assembly failed"; exit }

# Link
& $LINK /nologo /subsystem:console /entry:main /debug RawrXD-GGUFAnalyzer-Complete.obj /LIBPATH:"$LIB_PATH" kernel32.lib /out:RawrXD-GGUFAnalyzer-Complete.exe
if ($LASTEXITCODE -ne 0) { Write-Error "Linking failed"; exit }

Write-Host "Build successful. Running..."

# Run 
.\RawrXD-GGUFAnalyzer-Complete.exe
