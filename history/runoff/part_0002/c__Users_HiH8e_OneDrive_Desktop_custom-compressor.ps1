...
function Compress-Custom {
  param($File)
  # Optional dynamic chunk size adjustment so File.Length % ChunkSize == 0 ("x%0" interpretation)
  $effectiveChunk = $ChunkSize
  if($AlignExactZero){
    for($try=$ChunkSize; $try -ge 256; $try--){
      if(($File.Length % $try) -eq 0){ $effectiveChunk = $try; break }
    }
  }
  $out = $File.FullName + '.cust'
  $in  = [System.IO.File]::Open($File.FullName,'Open','Read','Read')
  $outStream = [System.IO.File]::Create($out)
  $header = New-HeaderBytes -FoldId $foldId -SentinelFlag $SentinelFlag -ChunkSize $effectiveChunk -ExpansionFactor $ExpansionFactor -OrigSize $File.Length
  $outStream.Write($header,0,$header.Length)
  $buf = New-Object byte[] $effectiveChunk
  $totalRead = 0
  $sw = [System.Diagnostics.Stopwatch]::StartNew()
  while(($read = $in.Read($buf,0,$effectiveChunk)) -gt 0){
    $totalRead += $read
  ...
  $ratio = $comp / $orig
  Write-Host "Compressed $($File.Name) -> $(Split-Path -Leaf $out) | ratio $(('{0:P6}' -f $ratio)) | ChunkSize=$effectiveChunk | time $([int]$sw.Elapsed.TotalSeconds)s" -ForegroundColor Cyan
  if($VerboseStats){
    $chunksProcessed = [Math]::Ceiling($orig / $effectiveChunk)
    Write-Host "Original bytes: $orig  Compressed bytes: $comp  Chunks processed: $chunksProcessed  AlignExactZero=$($AlignExactZero.IsPresent)" -ForegroundColor DarkCyan
    if($AlignExactZero -and ($effectiveChunk -ne $ChunkSize)){
      Write-Host "Adjusted chunk size from $ChunkSize to $effectiveChunk to satisfy fileSize % chunkSize == 0" -ForegroundColor DarkYellow
    }
  }
}
...
# ---- main ----
if($FilePath){
  $target = Get-Item -LiteralPath $FilePath -ErrorAction Stop
  if($target.Length -lt $MinSize){ Write-Host "Skipping (below MinSize) override disabled for single file: $($target.Name)" -ForegroundColor Yellow }
  if($Action -eq 'compress'){ Compress-Custom $target } else { Decompress-Custom $target }
} elseif($Folder){
  Get-ChildItem $Folder -File | Where-Object { $_.Length -ge $MinSize } | ForEach-Object {
    if($Action -eq 'compress') { Compress-Custom $_ } else { Decompress-Custom $_ }
  }
} else {
  throw "Provide -Folder or -FilePath"
}
Write-Host 'Done.'