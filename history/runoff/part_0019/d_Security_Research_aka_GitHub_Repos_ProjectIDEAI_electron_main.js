...
function safeWrite(stream, data) {
  ...
  try {
    ...
    if (error.code === 'EPIPE' || error.code === 'ENOTCONN' ||
        error.message?.includes('EPIPE') || error.message?.includes('broken pipe')) {
...
// Disable console output on process exit to prevent EPIPE errors
process.on('exit', () => {
  consoleDisabled = true;
});
...
// Plugin Ollama API bridge
ipcMain.handle('plugin:ollama:generate', async (event, model, prompt, options = {}) => {
  try {
    const response = await fetchWithLogging('http://localhost:11434/api/generate', {
      method: 'POST',
    ...
    console.error('[Plugin Ollama] Generate error:', error);
    return { success: false, error: 'Make sure Ollama is running' };
  }
});
