...
@@topp_done:
    ...
    ; Write token to ring buffer with proper circular index management
    mov rcx, g_pRingBuffer
    mov edx, g_BufferIndex
    
    ; Compute write offset: (index % RING_SIZE) * 4
    mov eax, edx
    and eax, 0FFh                  ; RING_SIZE = 256 entries (mask)
    mov [rcx + rax*4], rbx         ; store token at ring[index % 256]
    
    inc edx
    mov g_BufferIndex, edx         ; advance write pointer
