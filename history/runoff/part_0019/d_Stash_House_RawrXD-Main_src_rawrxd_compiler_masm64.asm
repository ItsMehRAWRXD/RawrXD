...
CompilerEngine_StageSemantic proc frame engine:dq, options:dq, result:dq
    ...
    ; Walk AST and resolve symbols
    ; RSI = pASTRoot, RAX = pSymbolTable
    push rax                         ; save symbol table pointer
    mov rcx, rsi                     ; AST root node
    mov rdx, rax                     ; symbol table base
    xor r8d, r8d                     ; scope depth = 0
    call SemanticWalkNode
    pop rax
...
;-----------------------------------------------------------------------------
; SemanticWalkNode - Recursive AST visitor for symbol resolution
; RCX = pNode (AST_NODE*), RDX = pSymTable, R8D = scopeDepth
;-----------------------------------------------------------------------------
SemanticWalkNode proc
    push rbx
    push rsi
    push rdi
    push r12
    ...
    mov rbx, rcx                     ; pNode
    mov r12, rdx                     ; pSymTable
    mov r13d, r8d                    ; scopeDepth

    test rbx, rbx
    jz @@sw_done

    ; Check token type - if IDENTIFIER (1) or KEYWORD (2), register in symbol table
    mov eax, (AST_NODE ptr [rbx]).token.type
    cmp eax, 1                       ; TOKEN_IDENTIFIER
    je @@sw_register
    cmp eax, 2                       ; TOKEN_KEYWORD (function/class/var decl)
    je @@sw_check_keyword
    jmp @@sw_visit_children

@@sw_check_keyword:
    ; Only register if child[0] is an identifier (e.g., "function foo")
    cmp (AST_NODE ptr [rbx]).childCount, 0
    je @@sw_visit_children
    jmp @@sw_visit_children          ; keywords themselves aren't symbols

@@sw_register:
    ; Add identifier to symbol table
    ; Layout: [count(4)][capacity(4)][entries...]
    ; Each entry: SYMBOL_ENTRY (name[256], type(4), scope(4), location(...), flags(4), next(8))
    mov eax, dword ptr [r12]         ; current count
    cmp eax, dword ptr [r12+4]       ; >= capacity?
    jge @@sw_visit_children          ; table full, skip

    ; Compute entry offset: 8 + count * sizeof(SYMBOL_ENTRY)
    ; sizeof(SYMBOL_ENTRY) ~ 548 but use SIZEOF if available
    push rdx
    mov ecx, eax
    imul ecx, 548                    ; entry_size (conservative)
    add ecx, 8                       ; skip header
    lea rdi, [r12 + rcx]            ; target entry address

    ; Copy token name → entry.name (up to 256 bytes)
    lea rsi, (AST_NODE ptr [rbx]).token.value
    push rcx
    mov ecx, 255
@@sw_copy_name:
    mov al, byte ptr [rsi]
...
@@sw_name_done:
    ...
    pop rcx
    lea rdi, [r12 + rcx]            ; re-base to entry start

    ; Set entry fields
    mov (SYMBOL_ENTRY ptr [rdi]).type, 0       ; generic type
    mov eax, r13d
    mov (SYMBOL_ENTRY ptr [rdi]).scope, eax    ; current scope depth
    mov (SYMBOL_ENTRY ptr [rdi]).flags, 0
...
@@sw_visit_children:
    ...
    mov r14d, (AST_NODE ptr [rbx]).childCount
    test r14d, r14d
    jz @@sw_done

    xor esi, esi                     ; child index
@@sw_child_loop:
    cmp esi, r14d
    jge @@sw_done

    ; Load child pointer: children[esi]
    mov rcx, (AST_NODE ptr [rbx]).children[rsi*8]
    test rcx, rcx
    jz @@sw_next_child

    mov rdx, r12                     ; pSymTable
    mov r8d, r13d
    inc r8d                          ; child scope = parent + 1
    call SemanticWalkNode
