...
bool EngineManager::SwitchEngine(const std::string& engine_id) {
    ...
    if (it != engines_.end() && it->second->loaded) {
        ...
        s_logger.info("Switched to engine: " + engine_id);
        return true;
    }
    return false;
}
...
bool EngineManager::Load800BModel(const std::string& model_name) {
    if (current_engine_id_ != "800b-5drive") {
        s_logger.error("Error: Current engine doesn't support 800B models");
        return false;
    }
    
    // Verify 5-drive setup
    if (!VerifyDriveSetup()) {
        s_logger.error("Error: 5-drive setup not properly configured");
        return false;
    }
    
    auto drives = GetDrivePaths();
    if (drives.size() != 5) {
        s_logger.error("Error: Expected 5 drives, found " + std::to_string(drives.size()));
        return false;
    }
    ...
    for (size_t i = 0; i < 5; i++) {
        ...
        if (!std::filesystem::exists(part_path)) {
            s_logger.error("Error: Model part not found: " + part_path);
            return false;
        }
    ...
    s_logger.info("Successfully loaded 800B model: " + model_name);
    return true;
}
...
bool EngineManager::Setup5DriveLayout(const std::string& base_dir) {
    ...
    s_logger.info("5-drive layout created at: " + base_dir);
    return true;
}
...
bool EngineManager::CompileWithEngine(const std::string& engine_id, const std::string& source_file) {
    if (engine_id != "rawrxd-compiler") {
        s_logger.error("Error: Only rawrxd-compiler engine supports compilation");
        return false;
    }
