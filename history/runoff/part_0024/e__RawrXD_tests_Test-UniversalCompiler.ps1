...
function Invoke-Compiler {
    ...
    if ($TargetArch -ne "native") {
        $compArgs += "--target", $TargetArch
    }

    # ADJUSTMENT: rawrxd supports --target (one arg) or separate?
    # rawrxd ArgumentParser has --target for arch.
    # It does NOT have --target-os.
    # It attempts to parse --target-os in mock script, but real rawrxd only has --target.
    
    Write-Host "DEBUG EXEC: $compiler $compArgs" -ForegroundColor Magenta
