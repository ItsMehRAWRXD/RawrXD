    ...
    foreach ($path in $sdkPaths) {
        if (Test-Path $path) {
            ...
            if (Test-Path $includeDir) {
                # Prefer SDK versions that have UCRT
...
if ($windowsSdkDir -and $windowsSdkVersion) {
    ...
    if (Test-Path $umPath)     { $includePaths += $umPath }
    if (Test-Path $sharedPath) { $includePaths += $sharedPath }
    if (Test-Path $ucrtPath)   { $includePaths += $ucrtPath }
}

# Add VC tools include path
$vcIncludePath = Join-Path $vsPath "VC\Tools\MSVC\14.44.35207\include"
if (Test-Path $vcIncludePath) {
    Write-Host "  VC Include: $vcIncludePath (exists: True)"
    $includePaths += $vcIncludePath
} else {
    Write-Host "  VC Include: $vcIncludePath (exists: False)"
}
...
# Set LIB environment variable for link.exe
$libPaths = @()
if ($windowsSdkDir -and $windowsSdkVersion) {
    ...
    if (Test-Path $libUmPath)   { $libPaths += $libUmPath }
    if (Test-Path $libUcrtPath) { $libPaths += $libUcrtPath }
}

# Add VC tools library path
$vcLibPath = Join-Path $vsPath "VC\Tools\MSVC\14.44.35207\lib\x64"
if (Test-Path $vcLibPath) {
    Write-Host "  VC Lib: $vcLibPath (exists: True)"
    $libPaths += $vcLibPath
} else {
    Write-Host "  VC Lib: $vcLibPath (exists: False)"
}
...
$CPP | ForEach-Object {
    $cmdLine = "$($CL.Source) " + ($CPPFLAGS -join " ") + " /c /Fo`"$OBJ\$($_.BaseName).obj`" `"$($_.FullName)`""
    Write-Host "CL Command: $cmdLine"
