...
param(
    ...
    [string]$ReportPath = "FeatureSyncReport.txt"
)
...
"@
        ...
        # Insert loading code defensively to avoid 0..-1 slices
        if ($insertIndex -le 0) {
            $newContent = $loadingCode + "`n" + $content
        } elseif ($insertIndex -ge $lines.Count) {
            $newContent = $content + "`n" + $loadingCode
        } else {
            $newContent = ($lines[0..($insertIndex-1)] + $loadingCode + $lines[$insertIndex..($lines.Count-1)]) -join "`n"
        }
...
if ($SyncFeatures) {
    Sync-AllFeatures
    Ensure-CLIOptimizedLoading
}

if ($GenerateReport) {
    Export-SyncReport -OutputPath $ReportPath
}
