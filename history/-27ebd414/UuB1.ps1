...
$sources = Get-ChildItem -Path $SourceDir -Filter "*.asm" -Recurse | Where-Object {
    $_.Name -notmatch "_test\.asm$" -and
    # Skip UASM-only files (require OPTION WIN64:3 / masm64 SDK)
    $_.Name -notin @("cursor_host_replacement.asm", "RawrXD_AgentHost.asm") -and
    # Skip NASM-syntax files (section .data instead of .data segment)
    $_.Name -notin @("solo_standalone_compiler.asm")
}
...
foreach ($src in $sources) {
    ...
    $asmExe = $assembler
    $incDir = $src.DirectoryName   # -I points at source dir for include resolution
    
    $job = Start-Job -ScriptBlock {
        param($asmExe, $asmPath, $obj, $srcName, $incDir)
        try {
            $output = & $asmExe /c /Fo"$obj" /W3 /Zi /Zd /I"$incDir" "$asmPath" 2>&1
    ...
    } -ArgumentList $asmExe, $asmPath, $obj, $src.Name, $incDir
...
if ($failed -gt 0 -and $compiled -eq 0) {
    throw "Phase 1 failed: all $failed files had compilation errors"
} elseif ($failed -gt 0) {
    Write-Phase1Log "Warning: $failed files failed, but $compiled succeeded — continuing to link" "Warning"
}
