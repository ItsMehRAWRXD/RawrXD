...
$Script:linkArgs = $linkArgsBase + "@`"$responseFile`""
    ...
    # Redirect to temp files via PowerShell native call
$Script:stdoutFile = Join-Path $OutDir "link_stdout.txt"
$Script:stderrFile = Join-Path $OutDir "link_stderr.txt"
    
    # Build a batch file to avoid quoting hell
$Script:batchFile = Join-Path $OutDir "run_link.bat"
$Script:batchContent = "@echo off`r`n`"$Linker`" $($linkArgs -join ' ') 1>`"$stdoutFile`" 2>`"$stderrFile`"`r`nexit /b %errorlevel%"
    Set-Content -Path $batchFile -Value $batchContent -Encoding ASCII
    
$Script:cmdProc = Start-Process -FilePath "cmd.exe" -ArgumentList "/c", "`"$batchFile`"" -NoNewWindow -Wait -PassThru
