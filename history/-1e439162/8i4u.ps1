...
# Ensure PoshLLM module is loaded
$poshLLMModulePath = Join-Path $PSScriptRoot "..\extensions\PoshLLM.psm1"
if (-not (Get-Module -Name "PoshLLM")) {
    try {
        Import-Module $poshLLMModulePath -Force -ErrorAction Stop
        Write-Host "✅ PoshLLM module loaded successfully" -ForegroundColor Green
    }
    catch {
        Write-Host "❌ Failed to load PoshLLM module: $($_.Exception.Message)" -ForegroundColor Red
        exit 1
    }
}
...
function Invoke-PoshLLMListHandler {
    try {
        ...
        foreach ($modelName in $models.Keys) {
            ...
            Write-Host "   Vocab Size: $([SimpleTokenizer]::VocabSize)" -ForegroundColor Gray
