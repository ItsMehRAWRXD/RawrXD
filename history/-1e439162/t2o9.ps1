...
function Invoke-PoshLLMSaveHandler {
    ...
    try {
        $model = Get-PoshLLMModel -Name $Name
        if (-not $model) {
            Write-Host "Error: Model '$Name' not found." -ForegroundColor Red
            return 1
        }

        Write-Host "Saving model '$Name' to $Path..." -ForegroundColor Cyan
        Save-PoshLLMModel -Model $model -Path $Path
...
function Invoke-PoshLLMLoadHandler {
    ...
    try {
        ...
        $model = Load-PoshLLMModel -Path $Path
        Register-PoshLLMModel -Name $Name -Model $model
