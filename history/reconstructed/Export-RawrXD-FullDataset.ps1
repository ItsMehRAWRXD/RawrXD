...
# Recurse into src/, include/, test/ but exclude known vendor dirs
$RecurseDirs = @($srcDir, $incDir, $testDir)
$ExcludeDirs = @('build','rawrxd_dataset','.git','node_modules','third_party','external','vendor','__pycache__')
$Extensions = @("*.cpp","*.c","*.hpp","*.h","*.asm","*.inc","*.rc","*.ps1","*.cmake")

foreach ($dir in $RecurseDirs) {
    if (!(Test-Path $dir)) { continue }
    foreach ($ext in $Extensions) {
        $found = Get-ChildItem -Path $dir -Filter $ext -Recurse -ErrorAction SilentlyContinue |
            Where-Object { $dominated = $false; foreach ($ex in $ExcludeDirs) { if ($_.FullName -match "\\$ex\\") { $dominated = $true; break } }; -not $dominated }
        foreach ($f in $found) { [void]$AllFiles.Add($f) }
    }
}
# Root-level files only (no recurse)
foreach ($ext in $Extensions) {
    $found = Get-ChildItem -Path $Root -Filter $ext -ErrorAction SilentlyContinue
    foreach ($f in $found) { [void]$AllFiles.Add($f) }
}
...
# ============================================================
# 8. RANDOM / ENTROPY CATCHER (Fast string-split approach)
# ============================================================
Write-Header "PHASE 7: ENTROPY ANALYSIS (RANDOM BUCKET)"

# Build word-frequency via fast string splitting (no regex on huge content)
Write-Host "Building word frequency index..."
$wordFreq = @{}
$p7filesProcessed = 0

for ($p7fi = 0; $p7fi -lt $AllContentParts.Count; $p7fi++) {
    $p7content = $AllContentParts[$p7fi]
    if ([string]::IsNullOrEmpty($p7content)) { continue }
    $p7filesProcessed++
    
    # Fast: split by non-identifier chars, filter
    $tokens = $p7content -split '[^A-Za-z_0-9]+'
    foreach ($tok in $tokens) {
        if ($tok.Length -ge 3 -and $tok[0] -match '[A-Za-z_]') {
            if ($wordFreq.ContainsKey($tok)) {
                $wordFreq[$tok] = $wordFreq[$tok] + 1
            } else {
                $wordFreq[$tok] = 1
            }
        }
    ...
    if ($p7filesProcessed % 500 -eq 0) {
        Write-Host "  ...$p7filesProcessed files, $($wordFreq.Count) unique"
    }
}
Write-Host "Indexed $($wordFreq.Count) unique identifiers from $p7filesProcessed files"

# Collect function definitions from project files only (skip ggml*/nlohmann)
$allFunctions = [System.Collections.Generic.HashSet[string]]::new()
$fnPat = '(?:void|int|bool|BOOL|LRESULT|HRESULT|size_t|float|double|uint32_t|int64_t|std::string)\s+(\w+)\s*\('
foreach ($file in $AllFiles) {
    if ($file.FullName -match '\\ggml|\\nlohmann|\\sqlite3') { continue }
    if ($file.Extension -notmatch '\.(cpp|c|h|hpp)$') { continue }
    $raw = Get-Content $file.FullName -Raw -ErrorAction SilentlyContinue
    if (!$raw) { continue }
    $ms = [regex]::Matches($raw, $fnPat)
    for ($i = 0; $i -lt $ms.Count; $i++) { [void]$allFunctions.Add($ms[$i].Groups[1].Value) }
}
Write-Host "  Function definitions found: $($allFunctions.Count)"
...
$wellKnown = [System.Collections.Generic.HashSet[string]]::new(
    [string[]]@("main","WinMain","wWinMain","DllMain","wmain","_tmain","sizeof","memcpy",
                ...
                "CreateWindowExA","DefWindowProcA","PostQuitMessage","DispatchMessageA",
                "TranslateMessage","GetMessageA","RegisterClassExA","ShowWindow",
                "UpdateWindow","GetLastError","CloseHandle","CreateFile",
                "ReadFile","WriteFile","GetModuleHandleA")
)
...
# Check function usage via word frequency (O(1) lookup)
...
# Unreferenced ASM exports — check via word frequency
...
# Mystery structs — per-file scan (skip vendored)
foreach ($file in $AllFiles) {
    if ($file.FullName -match '\\ggml|\\nlohmann|\\sqlite3') { continue }
    if ($file.Extension -notmatch '\.(cpp|c|h|hpp)$') { continue }
    $raw = Get-Content $file.FullName -Raw -ErrorAction SilentlyContinue
    if (!$raw) { continue }
    $sms = [regex]::Matches($raw, '(?:struct|class)\s+(\w{3,})\s*[{:]')
    for ($i = 0; $i -lt $sms.Count; $i++) {
        $name = $sms[$i].Groups[1].Value
        $freq = 0
        if ($wordFreq.ContainsKey($name)) { $freq = $wordFreq[$name] }
        if ($freq -le 1) {
