  ...
  "flags": 950
        ...
        try {
            ...
            $response = Invoke-RestMethod -Uri "https://marketplace.visualstudio.com/_apis/public/gallery/extensionquery" `
                -Method POST -ContentType "application/json" -Body $marketplaceBody -Headers $headers -ErrorAction Stop
        ...
        try {
            # VSIX is ZIP format — rename to .zip for Expand-Archive compatibility
            $zipPath = $vsixPath -replace '\.vsix$', '.zip'
            Copy-Item $vsixPath $zipPath -Force
            Expand-Archive -LiteralPath $zipPath -DestinationPath $installPath -Force -ErrorAction Stop
