<#
.SYNOPSIS
    Real Integration Test Suite for RawrXD Built-In Tools
.DESCRIPTION
    Tests all 40+ built-in tools with actual RawrXD runtime context
    Run this AFTER RawrXD.ps1 has launched the main IDE window
#>
...
function Initialize-TestEnvironment {
    Write-TestHeader "REAL RUNTIME INITIALIZATION"
    ...
    # Verify Register-AgentTool exists
    if (-not (Get-Command Invoke-AgentTool -ErrorAction SilentlyContinue)) {
        Write-TestFail "Invoke-AgentTool not found" "RawrXD runtime not initialized"
        Write-TestInfo "Make sure RawrXD.ps1 is running first!"
        return $false
    }
    
    # Verify Write-DevConsole exists
    if (-not (Get-Command Write-DevConsole -ErrorAction SilentlyContinue)) {
        Write-TestFail "Write-DevConsole not found" "RawrXD context not available"
        return $false
    }
    
    Write-TestPass "RawrXD runtime context detected"
    Write-TestPass "Invoke-AgentTool available"
    Write-TestPass "Write-DevConsole available"
...
function Test-FileOperations {
    ...
    try {
        ...
        if ($result.success -and (Test-Path $testFile)) {
            ...
            if ($content -eq "Test content for file creation") {
                ...
                Write-TestFail "File created but content mismatch" "Expected: 'Test content for file creation', Got: '$content'"
            }
        } else {
    ...
    try {
        ...
        if ($result.success -and $result.content) {
            ...
            Write-TestInfo "Read lines 2-4: $($result.content.Substring(0, [Math]::Min(50, $result.content.Length)))..."
        } else {
    ...
    try {
        ...
        1..3 | ForEach-Object {
            Set-Content -Path (Join-Path ${script:TestDirectory} "file$_.txt") -Value "test content $_"
        }
        ...
        if ($result.success) {
            Write-TestPass "Directory listing successful - found $($result.count) items" $result
        } else {
            Write-TestFail "Directory listing failed" $result.error
        }
    } catch {
    ...
    try {
        ...
        if ($result.success -and $result.count -eq 2) {
            Write-TestPass "Multiple files edited successfully - $($result.count) edits" $result
        } else {
...
function Test-SearchTools {
    ...
    try {
        # Create test files
        Set-Content -Path (Join-Path ${script:TestDirectory} "search1.ps1") -Value "# PowerShell script"
        Set-Content -Path (Join-Path ${script:TestDirectory} "search2.ps1") -Value "# Another script"
        Set-Content -Path (Join-Path ${script:TestDirectory} "search3.txt") -Value "Text file"
        ...
        if ($result.success -and $result.count -ge 2) {
            Write-TestPass "File search found $($result.count) .ps1 files" $result
        } else {
            Write-TestFail "File search failed" $result.error
        }
    } catch {
    ...
    try {
        ...
        Set-Content -Path $testFile -Value "function MyFunction { }`nfunction AnotherFunction { }`nfunction ThirdFunction { }"
        ...
        if ($result.success -and $result.count -gt 0) {
            Write-TestPass "Text search found $($result.count) matches" $result
        } else {
            Write-TestFail "Text search failed or no matches" $result.error
        }
    } catch {
    ...
    try {
        ...
        Set-Content -Path $testFile -Value "function TestSymbol { }`nTestSymbol`nTestSymbol`nif (`$TestSymbol)"
        ...
        if ($result.success) {
            Write-TestPass "Code usages search completed - $($result.count) found" $result
        } else {
    ...
    try {
        ...
        if ($result.success) {
            Write-TestPass "Semantic search executed" $result
        } else {
...
function Test-TerminalTools {
    ...
    try {
        ...
        if ($result.success -and $result.output) {
            ...
            Write-TestInfo "Command output: $($result.output.Substring(0, 30))..."
        } else {
    ...
    try {
        $result = Invoke-AgentTool -ToolName "run_in_terminal" -Arguments @{
            command    = "Start-Sleep -Seconds 2; Write-Output 'Job completed'"
            background = $true
        }
        
        if ($result.success -and $result.job_id) {
            Write-TestPass "Background job started with ID $($result.job_id)" $result
    ...
    try {
        ...
        if ($result.success -and $result.command) {
            ...
            Write-TestInfo "Last command: $($result.command.Substring(0, 50))..."
        } else {
    ...
    try {
        ...
        if ($result.success -and (Test-Path $notebookPath)) {
            ...
            if ($summaryResult.success) {
                Write-TestPass "Notebook summary retrieved - $($summaryResult.cell_count) cells" $summaryResult
            } else {
...
function Test-GitTools {
    ...
    try {
        ...
        if ($result.success) {
            ...
            Write-TestInfo "Status: $($result.status.Substring(0, 50))..."
        } else {
    ...
    try {
        ...
        if ($result.success) {
            Write-TestPass "GitHub PR tool executed" $result
        } else {
...
function Test-WorkspaceTools {
    ...
    try {
        ...
        if ($result.success) {
            Write-TestPass "Project setup info retrieved - Type: $($result.type)" $result
        } else {
...
function Test-BrowserTools {
    ...
    try {
        Write-TestInfo "Attempting to fetch example.com..."
        $result = Invoke-AgentTool -ToolName "fetch_webpage" -Arguments @{
            url = "https://www.example.com"
        }
        
        if ($result.success -and $result.status_code -eq 200) {
            Write-TestPass "Webpage fetched successfully - $($result.content_length) bytes" $result
        } else {
            Write-TestFail "Webpage fetch returned status code $($result.status_code)" $result.error
        }
    } catch {
...
function Test-TaskTools {
    ...
    try {
        $result = Invoke-AgentTool -ToolName "manage_todos" -Arguments @{
            action = "add"
            task   = "Test task 1 - Validate tool"
        }
        
        if ($result.success -and $result.id) {
            Write-TestPass "Todo item added successfully with ID $($result.id)" $result
            $todoId = $result.id
            ...
            if ($listResult.success) {
                Write-TestPass "Todos listed successfully - found $($listResult.todos.Count) items" $listResult
    ...
    try {
        ...
        if ($result.success -and $result.output) {
            ...
            Write-TestInfo "Task output: $($result.output.Substring(0, 40))..."
        } else {
...
function Test-APIAndDiagnosticTools {
    ...
    try {
        ...
        if ($result.success) {
            Write-TestPass "Problem detection tool executed" $result
        } else {
    ...
    try {
        ...
        if ($result.success) {
            Write-TestPass "VS Code API documentation tool executed" $result
        } else {
    ...
    try {
        ...
        if ($result.success) {
            Write-TestPass "VS Code command execution initiated" $result
        } else {
...
function Test-SubagentTools {
    ...
    try {
        $result = Invoke-AgentTool -ToolName "run_subagent" -Arguments @{
            task_description = "Analyze built-in tools functionality"
            timeout_seconds  = 30
        }
...
function Generate-TestReport {
    ...
    if (${script:TestFailed} -eq 0) {
        ...
        Write-Host "║ ⚠️  MOST TESTS PASSED - $passRate% SUCCESS RATE                    ║" -ForegroundColor $Colors.Yellow
    } else {
        Write-Host "║ ❌ SEVERAL TESTS FAILED - REVIEW RESULTS ABOVE                ║" -ForegroundColor $Colors.Red
    }
    Write-Host "╚════════════════════════════════════════════════════════════════╝" -ForegroundColor $Colors.Cyan
    Write-Host "`n"
}
...
Write-Host "║  RawrXD Built-In Tools - REAL INTEGRATION TEST SUITE v1.0    ║" -ForegroundColor $Colors.Magenta
Write-Host "║  Testing 40+ tools with actual RawrXD runtime                 ║" -ForegroundColor $Colors.Magenta
Write-Host "║  All tests execute against REAL tool implementations          ║" -ForegroundColor $Colors.Magenta
Write-Host "╚════════════════════════════════════════════════════════════════╝" -ForegroundColor $Colors.Magenta
Write-Host "`nℹ️  NOTE: Make sure RawrXD.ps1 is running in another terminal!" -ForegroundColor $Colors.Yellow
Write-Host "`n"

# Initialize test environment
if (-not (Initialize-TestEnvironment)) {
    Write-Host "`n❌ RawrXD runtime context not found." -ForegroundColor $Colors.Red
    Write-Host "❌ Test environment initialization failed. Exiting." -ForegroundColor $Colors.Red
    Write-Host "`nMake sure RawrXD.ps1 is running first!" -ForegroundColor $Colors.Yellow
    exit 1
}

Write-Host "`n✅ RawrXD runtime context verified. Starting test execution...`n" -ForegroundColor $Colors.Green
...
Write-Host "✅ Real integration tests completed! Check results above.`n" -ForegroundColor $Colors.Green
