...
function Get-SourceFiles {
    ...
    foreach ($dir in $SourceDirs) {
        ...
        Get-ChildItem -Path $dir -Recurse -Filter "*.asm" | ForEach-Object {
            $obj = Join-Path $ObjDir ($_.BaseName + ".obj")
            $files += [SourceFile]@{
                Path = $_.FullName
                Type = "ASM"
                ObjPath = $obj
...
function Invoke-MasmCompile {
    ...
    $flags = @(
        ...
        "/Zi",                   # Debug info
        "/Fo`"$($File.ObjPath)`"",  # Output object
        "/W3",                   # Warning level
        "/nologo"
    )
    
    if ($Config -eq "Release") {
        $flags += "/O2"          # Optimize for speed
        $flags += "/DNDEBUG"
    }
    else {
        $flags += "/Od"          # No optimization (debug)
        $flags += "/DDEBUG"
        $flags += "/Zd"          # Line numbers only
    }
    ...
    # Define RAWRXD version
    $flags += "/DRAWXD_VER=`"14.2.0`""
