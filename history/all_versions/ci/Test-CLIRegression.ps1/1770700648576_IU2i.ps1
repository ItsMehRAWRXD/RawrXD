...
<#
.SYNOPSIS
    RawrXD Unified CLI — CI Regression Harness v1.0
.DESCRIPTION
    One-file CI job that validates all CLI modes against the frozen contract.
...
Write-Host "  RawrXD CI Regression Harness v1.0" -ForegroundColor Cyan
...
function Test-Mode {
    ...
    try {
        ...
        if ($sw.ElapsedMilliseconds -gt $LatencyThresholdMs) {
            throw "LATENCY: ${sw.ElapsedMilliseconds}ms exceeds threshold ${LatencyThresholdMs}ms"
        }
...
Write-Host ""
Write-Host "─── Phase 2: Demonstration Modes ───" -ForegroundColor White
...
Write-Host ""
Write-Host "─── Phase 3: Usage-Error Modes (EXIT 0) ───" -ForegroundColor White
...
Write-Host ""
Write-Host "─── Phase 4: Agent Mode ───" -ForegroundColor White

Test-Mode -Name "agent" -Switch "-agent"

Write-Host ""
Write-Host "─── Phase 5: Privileged Modes ───" -ForegroundColor White
...
Write-Host ""
Write-Host "─── Phase 6: Log File Validation ───" -ForegroundColor White
...
Write-Host ""
Write-Host "─── Phase 7: Artifact Integrity ───" -ForegroundColor White
