...
# Also adding Windows SDK bin path explicitly because vcvars64.bat seems to fail to add it in this specific environment
# And manually patching INCLUDE and LIB because vcvars fails to find the SDK
...
# Note: We append to LIB/INCLUDE after vcvars runs
$batContent = "@echo off`r`nset PATH=C:\Windows\System32;C:\Windows;C:\Windows\System32\Wbem;$sdkBin`r`ncall `"$vcvarsPath`"`r`nset LIB=%LIB%;$sdkLib\um\x64;$sdkLib\ucrt\x64`r`nset INCLUDE=%INCLUDE%;$sdkInc\um;$sdkInc\ucrt;$sdkInc\shared`r`n%*"
