...
$batContent = "@echo off`r`ncall `"$vcvarsPath`" > NUL`r`n%*"
...
$clTest = & $wrapperBat cl.exe 2>&1 | Select-String "Microsoft"
if (-not $clTest) {
...
& $wrapperBat cmake $cmakeArgs *>> $logFile
...
& $wrapperBat cmake $buildArgs *>> $logFile
