...
$Script:pattern = '(?mi)(return\s+(false|true|nullptr|0|"");?\s*//.*(stub|todo|placeholder|fixme|not.?impl)|//\s*(TODO|STUB|FIXME|PLACEHOLDER|NOT.?IMPL))'

Get-ChildItem $srcDir -Filter '*.cpp' | ForEach-Object {
$Script:content = [IO.File]::ReadAllText($_.FullName)
...
$Script:lineNum = ($content.Substring(0, $m.Index) -split "`n").Count
$Script:line = $m.Value.Trim()
