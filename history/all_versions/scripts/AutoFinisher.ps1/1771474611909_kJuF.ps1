...
function Split-LongByteStrings {
    ...
    # Find BYTE continuation blocks with many lines
    # Pattern: label BYTE \  (then many continuation lines ending with ,\)
    $Content = [regex]::Replace($Content,
        '(?m)^(\w+)\s+(BYTE)\s+\\[\r\n]+((?:\s+"[^"]*"(?:,13,10)?(?:,\\)?[\r\n]+)*\s+"[^"]*"(?:,13,10)?,0)',
        {
            ...
            $contLines = ($fullBlock -split "`n").Count
            if ($contLines -le 10) {
                return $fullBlock  # short enough, leave it
            }

            $splits++

            # Split into chunks of 8 continuation lines max
            $allLines = $fullBlock -split "`n"
            $result = @()
            $chunkNum = 0
            $currentChunk = @()
            $labelUsed = $false
            ...
            foreach ($line in $allLines) {
                ...
                if ($currentChunk.Count -ge 8) {
                    # Close this chunk and start a new label
                    # Remove trailing ,\ from last line in chunk and add ,0
                    $lastLine = $currentChunk[-1]
                    $lastLine = $lastLine -replace ',\\$', ''
                    $lastLine = $lastLine.TrimEnd()
                    if ($lastLine -notmatch ',0$') {
                        $lastLine += ",0"
                    }
                    $currentChunk[-1] = $lastLine

                    $result += $currentChunk
                    $chunkNum++
                    $chunkLabel = "${label}_p${chunkNum}"
                    $currentChunk = @("$chunkLabel BYTE \")
                }
            }
