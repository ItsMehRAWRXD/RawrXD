...
#==============================================================================
# PHASE 4b: SPLIT LONG BYTE CONTINUATIONS
# ml64 has ~512 char line limit and max continuation complexity
# Split multi-line BYTE definitions that exceed limits
#==============================================================================
function Split-LongByteStrings {
    param([string]$Content)
    ...
    while ($i -lt $lines.Count) {
        ...
        # Detect start of a BYTE continuation block: label BYTE \
        if ($line -match '^(\w+)\s+(BYTE)\s+\\$') {
            $label = $matches[1]
            $contLines = @()
            $i++
            ...
            while ($i -lt $lines.Count) {
                $cl = $lines[$i].TrimEnd("`r")
                $contLines += $cl
                $i++

                # Last line of continuation: doesn't end with ,\ or just \
                if ($cl -notmatch ',\\$' -and $cl -notmatch '^\s*\\$') {
                    break
                }
            }

            # If block is short enough (<= 8 continuation lines), keep as-is
            if ($contLines.Count -le 8) {
                $result += "$label BYTE \"
                $result += $contLines
            } else {
                $splits++
                # Split into chunks of 6 continuation lines
                $chunkSize = 6
                $chunkNum = 0

                for ($c = 0; $c -lt $contLines.Count; $c += $chunkSize) {
                    $end = [math]::Min($c + $chunkSize, $contLines.Count)
                    $chunk = $contLines[$c..($end-1)]

                    if ($chunkNum -eq 0) {
                        $chunkLabel = $label
                    } else {
                        $chunkLabel = "${label}_c${chunkNum}"
                    }

                    # Fix last line of non-final chunk: need to terminate with ,0
                    if ($end -lt $contLines.Count) {
                        $lastLine = $chunk[-1]
                        # Remove trailing ,\ and add ,0
        ...
        # Also handle: label BYTE "...",13,10,\  (continuation starts on same line)
        if ($line -match '^(\w+)\s+(BYTE)\s+(".+",?\s*\\)$') {
            $label = $matches[1]
            $firstPart = $matches[3] -replace '\s*\\$', ''
            $contLines = @($firstPart)
            $i++
    ...
    if ($splits -gt 0) {
        Write-Status "  Split $splits overly-long BYTE continuation(s)" "Green"
    }
