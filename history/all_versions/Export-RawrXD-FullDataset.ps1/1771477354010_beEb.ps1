...
Write-Host "  AllContentParts count: $($AllContentParts.Count)"
Write-Host "  AllContent length: $($AllContent.Length)"
$wordFreq = [System.Collections.Generic.Dictionary[string,int]]::new()

$p7filesProcessed = 0
$p7totalWordMatches = 0
# Process each file individually — use indexed access to avoid enumeration quirks
for ($p7fi = 0; $p7fi -lt $AllContentParts.Count; $p7fi++) {
    $p7content = $AllContentParts[$p7fi]
    if ([string]::IsNullOrEmpty($p7content)) { continue }
    $p7filesProcessed++
    
    # Use [regex]::Matches on each file content directly (files are small enough)
    ...
    for ($p7mi = 0; $p7mi -lt $p7matches.Count; $p7mi++) {
        ...
        if ($wordFreq.ContainsKey($p7word)) { $wordFreq[$p7word]++ } else { $wordFreq[$p7word] = 1 }
    }
    
    if ($p7filesProcessed -eq 1) {
        Write-Host "  After file 1 (idx $p7fi, $($p7content.Length) chars): $($wordFreq.Count) unique, $p7totalWordMatches matches"
        $p7sample = ($wordFreq.Keys | Select-Object -First 10) -join ", "
        Write-Host "  Sample keys: $p7sample"
    }
    if ($p7filesProcessed % 100 -eq 0) {
        Write-Host "  ...processed $p7filesProcessed files, $($wordFreq.Count) unique, $p7totalWordMatches matches"
    }
}
Write-Host "  Files processed: $p7filesProcessed"
Write-Host "  Total matches: $p7totalWordMatches"
