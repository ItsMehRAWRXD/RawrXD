...
# ============================================================
# 8. RANDOM / ENTROPY CATCHER
# ============================================================
Write-Header "PHASE 7: ENTROPY ANALYSIS (RANDOM BUCKET)"

# All function definitions
$allFunctions = [System.Collections.Generic.HashSet[string]]::new()
$fnMatches = [regex]::Matches($AllContent, "(?:void|int|bool|BOOL|LRESULT|HRESULT|size_t|float|double|uint32_t|int64_t|std::string)\s+(\w+)\s*\(")
foreach ($m in $fnMatches) { [void]$allFunctions.Add($m.Groups[1].Value) }

# Exclude well-known functions
$wellKnown = [System.Collections.Generic.HashSet[string]]::new(
    [string[]]@("main","WinMain","wWinMain","DllMain","wmain","_tmain")
)

# Functions that are defined but never called elsewhere
...
foreach ($fn in $allFunctions) {
    ...
    if ($fn.Length -le 2) { continue }

    # Count occurrences (definition + calls)
    $callCount = ([regex]::Matches($AllContent, "\b$([regex]::Escape($fn))\s*\(")).Count
    if ($callCount -le 1) {
...
# Unreferenced ASM exports (not declared as extern "C" in C++ side)
$cppExternNames = [System.Collections.Generic.HashSet[string]]::new()
foreach ($s in $Dataset.AsmBridges.Stubs) { [void]$cppExternNames.Add($s.Name) }

# Also check raw content for any reference
foreach ($exp in $Dataset.AsmBridges.Exports) {
    $name = $exp.Name
    # Check if referenced in any C++ file
    $cppContent = ($AllFiles | Where-Object { $_.Extension -match "\.(cpp|c|h|hpp)$" } | ForEach-Object {
        Get-Content $_.FullName -Raw -ErrorAction SilentlyContinue
    }) -join "`n"

    if ($cppContent -notmatch "\b$([regex]::Escape($name))\b") {
...
# Structs defined once and never used
$structMatches = [regex]::Matches($AllContent, "(?:struct|class)\s+(\w+)\s*[{:]")
foreach ($s in $structMatches) {
    $name = $s.Groups[1].Value
    if ($name.Length -le 2) { continue }
    $usageCount = ([regex]::Matches($AllContent, "\b$([regex]::Escape($name))\b")).Count
    if ($usageCount -le 1) {
