...
foreach ($part in $AllContentParts) {
    ...
    foreach ($line in ($part -split "`n")) {
        ...
        $totalWordMatches += $lineMatches.Count
        foreach ($wm in $lineMatches) {
            $w = $wm.Groups[1].Value
            if ($wordFreq.ContainsKey($w)) { $wordFreq[$w]++ } else { $wordFreq[$w] = 1 }
        }
    }
    if ($filesProcessed -eq 1) {
        Write-Host "  After file 1: $($wordFreq.Count) unique, $totalWordMatches matches"
        Write-Host "  First 10 keys: $($wordFreq.Keys | Select-Object -First 10)"
    }
    if ($filesProcessed % 100 -eq 0) {
        Write-Host "  ...processed $filesProcessed files, $($wordFreq.Count) unique identifiers, $totalWordMatches total matches"
    }
}
Write-Host "  Files actually processed in Phase 7: $filesProcessed"
Write-Host "  Total word matches: $totalWordMatches"
