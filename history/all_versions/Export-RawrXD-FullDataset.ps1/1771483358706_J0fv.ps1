...
for ($p7fi = 0; $p7fi -lt $AllContentParts.Count; $p7fi++) {
    ...
    $p7matches = [regex]::Matches($p7content, '\b([A-Za-z_]\w{2,})\b')
    $p7totalWordMatches += $p7matches.Count
    for ($p7mi = 0; $p7mi -lt $p7matches.Count; $p7mi++) {
        $p7word = $p7matches[$p7mi].Groups[1].Value
        if ($wordFreq.ContainsKey($p7word)) {
            $wordFreq[$p7word] = $wordFreq[$p7word] + 1
        } else {
            $wordFreq[$p7word] = 1
        }
    }
    
    if ($p7filesProcessed % 200 -eq 0) {
        Write-Host "  ...$p7filesProcessed files, $($wordFreq.Count) unique, $p7totalWordMatches matches"
    }
}
Write-Host "Indexed $($wordFreq.Count) unique identifiers ($p7totalWordMatches total matches)"
