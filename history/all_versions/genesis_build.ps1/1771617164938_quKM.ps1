...
$objFiles = Get-ChildItem -Path $Root -Filter "*.obj" -Recurse -ErrorAction SilentlyContinue | Where-Object {
    ...
    if($name -match "RawrXD_P2PRelay\.obj" -or $name -match "rawrxd-scc-nasm64\.obj" -or $name -match "RawrXD_CLI_Titan\.obj" -or $name -match "proof\.obj") { return $false }
...
$linkArgs = @(
    ...
    "/FORCE:UNRESOLVED", # MAGIC BULLET: Ignore missing standard libraries
...
$linkArgs += "kernel32.lib", "user32.lib", "gdi32.lib", "shell32.lib", "ole32.lib", "advapi32.lib", "crypt32.lib", "psapi.lib", "shlwapi.lib", "ws2_32.lib", "ntdll.lib", "msvcrt.lib", "vcruntime.lib", "ucrt.lib"

Write-Host "`n[LINKING] Forging Monolithic Executable: $finalExe..." -Fore Yellow
Write-Host "[DEBUG] Linker command: $Linker $($linkArgs -join ' ')" -Fore Cyan
