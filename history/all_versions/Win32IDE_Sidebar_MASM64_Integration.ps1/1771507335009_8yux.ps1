# Win32IDE_Sidebar_MASM64_Integration.ps1
# Pure MASM64 Sidebar Implementation + IDE Patcher
# Zero Qt. Zero C++. Pure x64 Win32 API.
...
$PATCHER_SCRIPT = @'
# Sidebar_MASM_Integration.ps1 - Wires pure ASM sidebar into IDE

param(
    [string]$IdeSrcPath = "D:\rawrxd\src",
    [string]$BuildPath = "D:\rawrxd\build"
)

Write-Host "=== RawrXD Sidebar MASM64 Integration ===" -ForegroundColor Cyan

# 1. Write ASM source
$AsmFile = "$IdeSrcPath\Win32IDE_Sidebar.asm"
$MASM_SOURCE | Out-File -FilePath $AsmFile -Encoding ASCII
Write-Host "[+] Generated $AsmFile ($($MASM_SOURCE.Length) chars)" -ForegroundColor Green
...
if (!$Ml64) {
    Write-Host "[!] ML64.exe not found. Please install Visual Studio Build Tools with MASM support:" -ForegroundColor Red
    Write-Host "   - Download Visual Studio Build Tools 2022" -ForegroundColor Yellow
    Write-Host "   - Select 'Desktop development with C++' workload" -ForegroundColor Yellow
    Write-Host "   - Include 'MSVC v143 - VS 2022 C++ x64/x86 build tools'" -ForegroundColor Yellow
    Write-Host "   - Include 'Windows 11 SDK' or latest Windows SDK" -ForegroundColor Yellow
    Write-Host "   - Or install via winget: winget install Microsoft.VisualStudio.2022.BuildTools --override '--add Microsoft.VisualStudio.Workload.VCTools --includeRecommended'" -ForegroundColor Yellow
    exit 1
}

Write-Host "[*] Using ML64: $Ml64" -ForegroundColor Cyan
Write-Host "[*] Assembling x64 object..." -ForegroundColor Yellow
& $Ml64 /c /Fo"$BuildPath\Win32IDE_Sidebar.obj" /W3 /Zi "$AsmFile" 2>&1 | Tee-Object -Variable AsmOutput

if ($LASTEXITCODE -ne 0) {
    Write-Host "[!] Assembly failed!" -ForegroundColor Red
    Write-Host "Assembly output:" -ForegroundColor Red
    $AsmOutput | ForEach-Object { Write-Host "  $_" -ForegroundColor Red }
    exit 1
}

# 3. Generate header for C++ integration (if needed for bridge)
$HeaderContent = @"
...
$HeaderContent | Out-File -FilePath "$IdeSrcPath\Win32IDE_Sidebar.h" -Encoding ASCII
Write-Host "[+] Generated header file" -ForegroundColor Green

# 4. Patch CMakeLists.txt or build script to link the OBJ
$CMakeFile = "$IdeSrcPath\CMakeLists.txt"
if (Test-Path $CMakeFile) {
    $CMakeContent = Get-Content $CMakeFile -Raw
    
    # Check if already patched
    if ($CMakeContent -notmatch "Win32IDE_Sidebar.obj") {
        # Find target_link_libraries or add_executable section and insert obj
        $ObjPath = "$BuildPath\Win32IDE_Sidebar.obj".Replace('\','/')
        $Patch = @"

# MASM64 Sidebar Integration
target_link_libraries(`${PROJECT_NAME} PRIVATE "$ObjPath")
"@
        
        # Append before last line or find appropriate spot
        $CMakeContent += $Patch
        $CMakeContent | Out-File -FilePath $CMakeFile -Encoding ASCII
        Write-Host "[+] Patched CMakeLists.txt" -ForegroundColor Green
    } else {
        Write-Host "[=] CMakeLists.txt already patched" -ForegroundColor Yellow
    }
}

# 5. Create replacement stub for old Qt sidebar
$ReplacementStub = @"
...
$ReplacementStub | Out-File -FilePath "$IdeSrcPath\Win32IDE_Sidebar.cpp" -Encoding ASCII -Force
Write-Host "[+] Replaced Qt sidebar with MASM stub" -ForegroundColor Green

# 6. Verify and report
Write-Host "`n=== Integration Summary ===" -ForegroundColor Cyan
Write-Host "Object file: $BuildPath\Win32IDE_Sidebar.obj"
Write-Host "Header file: $IdeSrcPath\Win32IDE_Sidebar.h"
Write-Host "Source stub: $IdeSrcPath\Win32IDE_Sidebar.cpp (Qt replacement)"

# Check dependencies
$Deps = @("comctl32.lib", "shell32.lib", "dwmapi.lib")
Write-Host "`nRequired linker libs: $($Deps -join ', ')" -ForegroundColor Yellow

Write-Host "`nBuild command:" -ForegroundColor White
Write-Host "cmake --build $BuildPath --config Release" -ForegroundColor Gray

Write-Host "`n[✓] Sidebar MASM64 integration complete!" -ForegroundColor Green
'@

# Execute the patcher
Invoke-Expression $PATCHER_SCRIPT
