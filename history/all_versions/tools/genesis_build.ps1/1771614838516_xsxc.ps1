...
function Resolve-WindowsSdkLibPaths {
    ...
    $versions = Get-ChildItem -Path $sdkRoot -Directory -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Name
    $best = $null
    foreach ($v in $versions) {
        try {
            $verObj = [version]$v
            if (-not $best -or $verObj -gt $best) { $best = $verObj }
        } catch {
            continue
        }
    }
    ...
    $bestDir = Join-Path $sdkRoot $best.ToString()
    $paths = @(
        (Join-Path $bestDir "um\x64"),
        (Join-Path $bestDir "ucrt\x64")
    )
