    ...
    hidden [void] InitializeProcessors() {
        $this.specializedProcessors = @(
            ...
            [SpecializedProcessor]::new(
                ...
                {
                    param($request, $context)
                    # Specialized compiler logic - creates standalone executables without runtimes
                    return $this.ProcessWithCompilerCapabilities($request)
                },
                @("compile", "build", "executable", "binary", "standalone", "native", "aot")
            )
        )
    }
...
```"
    ...
    hidden [object] ProcessWithSecondaryModel([object] $request) {
        ...
        try {
            ...
            $mockResponse = @{
                result = @"
Here's how to set up an AWS Lambda function with proper IAM roles:
...
"@
    ...
    # Process with compiler capabilities (standalone executables, no runtime dependencies)
    hidden [object] ProcessWithCompilerCapabilities([object] $request) {
        $response = [BaseResponse]::new()
        
        # Generate standalone executable code
        if ($request.Content -match "compile|build|executable") {
            $response.Content = "```c
...
int main(int argc, char* argv[]) {
    printf(\"Compiled executable ready\\n\");
    return 0;
}

// Compile with: gcc -static -o app app.c
```

```powershell
# PowerShell native compilation
Add-Type -TypeDefinition @'
public class NativeApp {
    public static void Main() {
        System.Console.WriteLine(\"Native compiled application\");
    }
}'

# Compile to native: dotnet publish -c Release -r win-x64 --self-contained true
```"
        } else {
            $response.Content = "Compiler specialist ready. I create standalone executables without runtime dependencies using AOT compilation, static linking, and native code generation."
        }
...
```"
    ...
    [hashtable] GetCapabilities() {
        return @{
            ...
            "CompilerSpecialist" = $true
            "CreatesStandaloneExecutables" = $true
