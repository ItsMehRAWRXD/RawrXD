...
$agentToolsPaths = @(
    ...
    (Join-Path (Get-Item $PSScriptRoot).Parent "agents\AgentTools.ps1")
)
...
if ($agentToolsPath) {
    . $agentToolsPath
    Write-Host "✅ AgentTools.ps1 loaded from: $agentToolsPath" -ForegroundColor Green
}
else {
    Write-Host "❌ AgentTools.ps1 not found in any location:" -ForegroundColor Red
    foreach ($path in $agentToolsPaths) {
