...
with open(path, 'rb') as f:
    ...
    data_offset = struct.unpack('<Q', f.read(8))[0] if version >= 3 else 0
    print(f'version={version} nt={nt} nm={nm} data_offset={data_offset}')
    # skip metadata
    for mi in range(nm):
    ...
    print('verify: data section starts at', data_offset)
