// rmsnorm_stage_v2.cpp
// RMSNorm layer validation for VAL-019 with invariant checks
...
Invariants check_invariants(const Tensor<float>& output, 
...
public:
    ...
    bool execute(const Tensor<float>& input, Tensor<float>& output,
        ...
        if (hidden_dim != config_.hidden_dim) {
            std::cerr << "[ERROR] Hidden dim mismatch" << std::endl;
            return false;
        }
    ...
    std::string get_kernel_version() const { return "rmsnorm_v1.0_native"; }
...
RMSNormResult validate_rmsnorm_stage(
    ...
    // Load input
    Tensor<float> input;
    if (!load_tensor(input_path, input, {1, 10, 4096})) {
    ...
    // Check invariants
    result.invariants = check_invariants(actual_output, expected_output.shape, 
                                          result.min_rms, result.max_rms);
    
    auto end = std::chrono::high_resolution_clock::now();
    result.runtime_ms = std::chrono::duration<double, std::milli>(end - start).count();
    
    // Determine PASS/FAIL
    bool numerical_pass = (result.max_error <= tolerance);
    bool invariant_pass = result.invariants.all_passed();
    
    if (numerical_pass && invariant_pass) {
        result.success = true;
    } else {
        result.success = false;
        std::stringstream ss;
        ...
        if (!invariant_pass) {
            if (!ss.str().empty()) ss << "; ";
            ss << "Invariant check failed";
            if (result.invariants.has_nan) ss << " (NaN detected)";
            if (result.invariants.has_inf) ss << " (Inf detected)";
            if (!result.invariants.output_shape_matches) ss << " (shape mismatch)";
            if (!result.invariants.rms_range_valid) ss << " (RMS out of range)";
        }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "  (with invariant checks)" << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "RMS stats:       mean=" << result.mean_rms 
              << ", min=" << result.min_rms 
              << ", max=" << result.max_rms << std::endl;
    std::cout << std::endl;
    
    // Invariant checks
    std::cout << "INVARIANT CHECKS:" << std::endl;
    std::cout << "  has_nan:              " << (result.invariants.has_nan ? "FAIL" : "PASS") << std::endl;
    std::cout << "  has_inf:              " << (result.invariants.has_inf ? "FAIL" : "PASS") << std::endl;
    std::cout << "  output_shape_matches: " << (result.invariants.output_shape_matches ? "PASS" : "FAIL") << std::endl;
    std::cout << "  rms_range_valid:      " << (result.invariants.rms_range_valid ? "PASS" : "FAIL") << std::endl;
    if (result.invariants.nan_count > 0) {
        std::cout << "  NaN count:            " << result.invariants.nan_count << std::endl;
    }
    if (result.invariants.inf_count > 0) {
        std::cout << "  Inf count:            " << result.invariants.inf_count << std::endl;
    }
    std::cout << std::endl;
    ...
    std::cout << "    \"has_nan\": " << (result.invariants.has_nan ? "true" : "false") << "," << std::endl;
    std::cout << "    \"has_inf\": " << (result.invariants.has_inf ? "true" : "false") << "," << std::endl;
    std::cout << "    \"output_shape_matches\": " << (result.invariants.output_shape_matches ? "true" : "false") << "," << std::endl;
    std::cout << "    \"rms_range_valid\": " << (result.invariants.rms_range_valid ? "true" : "false") << "," << std::endl;
    std::cout << "    \"nan_count\": " << result.invariants.nan_count << "," << std::endl;
    std::cout << "    \"inf_count\": " << result.invariants.inf_count << std::endl;
    std::cout << "  }," << std::endl;
