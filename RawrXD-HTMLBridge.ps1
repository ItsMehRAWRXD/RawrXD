...
function Setup-EnhancedWebView {
    ...
    try {
        ...
        try {
            ...
            Write-BridgeLog "WebView2 load failed, falling back to legacy WebBrowser: $($_.Exception.Message)" 'Warn'
        }
        ...
        Write-BridgeLog "Setup-EnhancedWebView failed: $($_.Exception.Message)" 'Error'
        return $null
    }
}
...
function Setup-WebView2Bridge {
    ...
    $WebView.add_NavigationCompleted({
            Write-BridgeLog "WebView navigation completed - injecting bridge" 'Debug'
...
"@
    ...
    $WebView.add_WebMessageReceived({
            ...
            try {
                ...
                Write-BridgeLog "Received WebMessage action=$($message.action)" 'Debug'
                ...
                Write-BridgeLog "WebMessage handler failed: $($_.Exception.Message)" 'Error'
                $errorResponse = @{
                    id      = $message.id
                    success = $false
                    error   = $_.Exception.Message
    ...
    $WebView.add_NavigationCompleted({
            ...
            ")
                Write-BridgeLog "Injected AI-First CSS from $cssPath" 'Debug'
            }

            if (Test-Path $jsPath) {
                $js = Get-Content $jsPath -Raw
                $WebView.ExecuteScriptAsync($js.Replace('`', '\`').Replace('$', '\$'))
                Write-BridgeLog "Injected AI-First JS from $jsPath" 'Debug'
            }
...
'@
            ...
            Write-BridgeLog "Extensions UI injected" 'Debug'
        })
}
...
function Setup-WebBrowserBridge {
    ...
    Write-BridgeLog "Using legacy WebBrowser bridge" 'Warn'
...
"@
    ...
    try {
        ...
        Write-BridgeLog "Legacy bridge setup failed: $($_.Exception.Message)" 'Error'
    }
...
"@
                ...
                Write-BridgeLog "Legacy bridge script injection failed: $($_.Exception.Message)" 'Error'
            }
        })
}
...
function Handle-PowerShellCall {
    ...
    try {
        Write-BridgeLog "Handle-PowerShellCall invoked: action=$Action" 'Debug'
        ...
        Write-BridgeLog "Handle-PowerShellCall failed for action=$Action : $($_.Exception.Message)" 'Error'
        return @{
            id      = $Id
            success = $false
            error   = $_.Exception.Message
...
function Start-HTMLPowerShellIDE {
    Write-BridgeLog "Start-HTMLPowerShellIDE invoked (WxH=${Width}x${Height}, FullScreen=$FullScreen)" 'Info'
    ...
    if (-not $webView) {
        ...
        Write-BridgeLog "WebView creation failed, launching default browser" 'Error'
        Start-Process $HtmlPath
        Write-Host "✅ HTML IDE opened in default browser. Use it directly!" -ForegroundColor Green
        return
    }

    Write-BridgeLog "WebView created successfully" 'Debug'
    ...
    if (Test-Path $HtmlPath) {
        ...
        Write-BridgeLog "Loading HTML from $HtmlPath" 'Info'
        ...
        if (-not $fileUri) {
            ...
            Start-Process $HtmlPath
            Write-BridgeLog "Failed to build file URI, opened in default browser" 'Error'
        }
        else {
            ...
            if ($webViewTypeName -eq "WebView2") {
                ...
                Write-BridgeLog "WebView2 navigating to $($fileUri.AbsoluteUri)" 'Debug'
            }
            else {
                $webView.Navigate($fileUri.AbsoluteUri)
                Write-BridgeLog "Legacy WebBrowser navigating to $($fileUri.AbsoluteUri)" 'Debug'
            }
        }
    }
    else {
        ...
        Write-BridgeLog "HtmlPath not found, generating minimal IDE" 'Warn'
...
'@
        Write-BridgeLog "Minimal IDE HTML injected" 'Info'
...
try {
    ...
    Write-BridgeLog "Start-HTMLPowerShellIDE crashed: $($_.Exception.Message)" 'Error'
}
finally {
...
function Cleanup-HTMLBridge {
    try {
        if ($global:IDEBridge.WebView -ne $null) {
            ...
            Write-BridgeLog "WebView disposed by Cleanup-HTMLBridge" 'Debug'
        }
        ...
        if ($global:IDEBridge.Form -ne $null) {
            ...
            Write-BridgeLog "Form disposed by Cleanup-HTMLBridge" 'Debug'
        }
    }
    finally {
        ...
        Write-BridgeLog "Cleanup-HTMLBridge completed" 'Info'
    }
}
