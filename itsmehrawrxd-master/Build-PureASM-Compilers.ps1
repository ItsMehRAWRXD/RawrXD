...
function Get-AsmFiles {
  Get-ChildItem -Path $PSScriptRoot -Filter '*_compiler_from_scratch.asm' -File -Recurse
}
...
$files = Get-AsmFiles
if ($Name) {
  $files = $files | Where-Object { $_.Name -like "${Name}_compiler_from_scratch.asm" }
}
if (-not $files) { Write-Host "No *_compiler_from_scratch.asm files found." -ForegroundColor Red; exit 1 }

Write-Host ("Found {0} ASM compiler sources" -f $files.Count) -ForegroundColor Green
Make-Manifest $files
