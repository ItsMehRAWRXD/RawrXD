...
# Pre-fix: Always scan for and fix literal escape sequences in source files
$sourceFiles = @(
    (Join-Path $RepoRoot "src\win32app\Win32IDE.h"),
    (Join-Path $RepoRoot "src\win32app\Win32IDE.cpp"),
    (Join-Path $RepoRoot "src\win32app\Win32IDE_Sidebar.cpp"),
    (Join-Path $RepoRoot "src\win32app\Win32IDE_VSCodeUI.cpp")
)

foreach ($srcFile in $sourceFiles) {
    if (Test-Path $srcFile) {
        $srcContent = Get-Content -Path $srcFile -Raw
        # Fix literal backtick-r-backtick-n sequences that should be actual newlines
        if ($srcContent -match '``r``n') {
            Write-Host "[AUTOFIX] Fixing literal escape sequences in $srcFile" -ForegroundColor Yellow
            $fixedContent = $srcContent -replace '``r``n', "`r`n"
            if (-not $DryRun -and $AutoFix) {
                Set-Content -Path $srcFile -Value $fixedContent -Encoding UTF8
                Write-Host "[OK] Fixed literal escapes in $srcFile" -ForegroundColor Green
            }
        }
    }
}
...
foreach ($err in $errors) {
    ...
    if ($err -match "C1070.*mismatched.*#if.*#endif") {
        Write-Host "[AUTOFIX] Mismatched #if/#endif detected - scanning for literal escape sequences" -ForegroundColor Yellow
        # Already handled by the pre-fix above
        continue
    }
