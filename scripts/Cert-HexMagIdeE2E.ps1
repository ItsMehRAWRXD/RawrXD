<#
.SYNOPSIS
  HEXMAG_IDE_E2E_001 — IDE send path → controller → tuner → FinalizePolicy.
  Does not rewrite the controller. Uses the same ideHexMagSendPath entry as Win32IDE.
#>
param(
...
if (-not $OutDir) { $OutDir = Join-Path $Root "build\hexmag_ide_e2e" }
if (-not $EvidenceDir) { $EvidenceDir = Join-Path $Root "evidence\HEXMAG_IDE_E2E_001" }
...
$exe = Join-Path $OutDir "hexmag_ide_e2e_cert.exe"
...
$srcs = @(
    (Join-Path $Root "tests\hexmag_ide_e2e_cert.cpp"),
