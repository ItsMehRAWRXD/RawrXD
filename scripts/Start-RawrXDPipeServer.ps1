...
if ($AsJob) {
    $serverFunc = ${function:Invoke-RawrXDByteServer}
    $sessionFunc = ${function:Invoke-RawrXDSession}
    $responseFunc = ${function:Write-PipeResponse}

    Start-Job -Name 'RawrXD-PipeServer' -ArgumentList $PipeName, $ModulePath, $serverFunc, $sessionFunc, $responseFunc -ScriptBlock {
        param($PipeName, $ModulePath, $ServerFunc, $SessionFunc, $ResponseFunc)
        Set-Item -Path Function:Write-PipeResponse -Value $ResponseFunc
        Set-Item -Path Function:Invoke-RawrXDSession -Value $SessionFunc
        & $ServerFunc -PipeName $PipeName -ModulePath $ModulePath
    } | Out-Null
