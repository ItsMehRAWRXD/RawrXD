...
; Buffers
MAX_SRC		equ 0100000h	; 1MB source
MAX_TOK		equ 0010000h	; 64KB tokens
MAX_CODE	equ 0010000h	; 64KB code
MAX_DATA	equ 0010000h	; 64KB data
MAX_PE		equ 0020000h	; 128KB PE buffer
...
write_pe64 proc
	...
	mov byte ptr [rdi+2], 0Eh
	; ImageBase = 0x400000 (4MB)
	mov dword ptr [rdi+24], 0400000h
	mov dword ptr [rdi+28], 0
...
emit_x86_epilogue proc
	...
	lea rsi, g_code
	; mov esp, ebp (8B E4)
	mov byte ptr [rsi+rbx], 08Bh
	mov byte ptr [rsi+rbx+1], 0E4h
	; pop ebp (5D)
	mov byte ptr [rsi+rbx+2], 05Dh
	; ret (C3)
	mov byte ptr [rsi+rbx+3], 0C3h
	add g_code_ptr, 4
	pop rsi
	pop rbx
	ret
...
emit_x64_prologue proc
	...
	mov byte ptr [g_code+1], 48h
	mov byte ptr [g_code+2], 89h
	mov byte ptr [g_code+3], 0E5h
	mov g_code_ptr, 4
	ret
...
emit_x64_epilogue proc
	...
	mov byte ptr [rsi+rbx], 48h
	mov byte ptr [rsi+rbx+1], 89h
	mov byte ptr [rsi+rbx+2], 0ECh
	; pop rbp (5D)
	mov byte ptr [rsi+rbx+3], 05Dh
	; ret (C3)
	mov byte ptr [rsi+rbx+4], 0C3h
	add g_code_ptr, 5
	pop rsi
	pop rbx
	ret
