@echo off
REM ============================================================================
REM Build All 69 Compilers - FINAL WORKING VERSION
REM ============================================================================
...
if %fail%==0 (
    ...
    REM Create registry header
    echo #pragma once > "%OUTDIR%\compiler_registry.h"
    echo #include ^<string^> >> "%OUTDIR%\compiler_registry.h"
    echo #include ^<vector^> >> "%OUTDIR%\compiler_registry.h"
    echo. >> "%OUTDIR%\compiler_registry.h"
    echo struct CompilerInfo { >> "%OUTDIR%\compiler_registry.h"
    echo     std::string name; >> "%OUTDIR%\compiler_registry.h"
    echo     std::string displayName; >> "%OUTDIR%\compiler_registry.h"
    echo     std::string version; >> "%OUTDIR%\compiler_registry.h"
    echo     std::string category; >> "%OUTDIR%\compiler_registry.h"
    echo     std::string path; >> "%OUTDIR%\compiler_registry.h"
    echo }; >> "%OUTDIR%\compiler_registry.h"
    echo. >> "%OUTDIR%\compiler_registry.h"
    echo inline std::vector^<CompilerInfo^> GetAllCompilers() { >> "%OUTDIR%\compiler_registry.h"
    echo     return { >> "%OUTDIR%\compiler_registry.h"
    
    REM Add all compilers to registry
    for %%%%f in ("%OUTDIR%\*.exe") do (
        for /f "tokens=*" %%%%a in ("%%%%~nf") do (
            echo         {"%%%%~nf", "%%%%~nf", "1.0", "Compiler", "%%%%f"}, >> "%OUTDIR%\compiler_registry.h"
        )
    )
    ...
    echo   - compiler_registry.h created
    echo   - All executables in: %OUTDIR%
)
...
REM Link
"%LINK%" /LIBPATH:"%SDK_LIB%" /SUBSYSTEM:CONSOLE /ENTRY:mainCRTStartup kernel32.lib "%OBJ%" /OUT:"%EXE%" 2> nul
if errorlevel 1 (
    echo   [FAIL] Link
    set /a fail+=1
    goto :eof
