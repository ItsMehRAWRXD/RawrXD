...
function Compile-MASM($src,$outDir){
  ...
  $linkCmd = '"{0}" /nologo /subsystem:{1} /entry:{2} {3} /LIBPATH:"{4}" /LIBPATH:"{5}" /LIBPATH:"{6}" kernel32.lib user32.lib gdi32.lib comdlg32.lib comctl32.lib /out:"{7}"' -f $tools.link, $sub, $entry, $linkObjsStr, $tools.lib, $kits.ucrt, $kits.um, $exe
...
function Compile-NASM($src,$outDir){
  ...
  $linkCmd = '"{0}" /nologo /subsystem:{1} /entry:{2} {3} /LIBPATH:"{4}" /LIBPATH:"{5}" /LIBPATH:"{6}" kernel32.lib user32.lib /out:"{7}"' -f $tools.link, $sub, $entry, $linkObjsStr, $tools.lib, $kits.ucrt, $kits.um, $exe
  cmd /c $linkCmd; if ($LASTEXITCODE -ne 0) { throw "link failed" }
  return $exe
}
