/*
 * REAL Java Compiler - Fixed version with proper path escaping
...
int compile_java(const char* input_file, const char* class_name) {
    ...
    // Build command line: "javac.exe" "input.java"
    char cmdline[MAX_CMD_LEN];
    snprintf(cmdline, sizeof(cmdline), "\"%s\" \"%s\"", javac_path, input_file);
    
    // Use CreateProcessA to avoid cmd.exe quoting issues
    STARTUPINFOA si = {0};
    si.cb = sizeof(si);
...
void escape_path(const char* input, char* output, size_t output_size) {
    size_t j = 0;
...
int create_wrapper(const char* class_name, const char* output_file) {
    ...
    // Escape backslashes for C string
    escape_path(java_path, escaped_path, sizeof(escaped_path));
