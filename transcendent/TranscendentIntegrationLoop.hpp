...
public:
    ...
    void RegisterTranscendentCallback(std::function<void(const std::string&)> callback);
    void TriggerTranscendentEvent(const std::string& eventType);
...
private:
    ...
    std::vector<std::function<void(const std::string&)>> m_transcendentCallbacks;
