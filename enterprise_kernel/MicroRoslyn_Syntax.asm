...
; Fixes: TOKEN_INVALID defined, keyword matching, message length, ABI compliance
; =======================================================================================
...
; Nesting stacks (for error reporting)
...
; Keyword table (null-terminated strings)
...
ErrMsg_UnclosedParen    BYTE "Unclosed parenthesis '('", 0
ErrMsg_UnclosedBracket  BYTE "Unclosed bracket '['", 0
ErrMsg_MismatchedBrace  BYTE "Mismatched brace '}'", 0
ErrMsg_MismatchedParen  BYTE "Mismatched parenthesis ')'", 0
ErrMsg_MismatchedBracket BYTE "Mismatched bracket ']'", 0
ErrMsg_InvalidChar      BYTE "Invalid character in source", 0
ErrMsg_UnclosedString   BYTE "Unclosed string literal", 0
ErrMsg_UnclosedChar     BYTE "Unclosed character literal", 0
ErrMsg_StackOverflow    BYTE "Brace/paren nesting too deep", 0
ErrMsg_MissingSemicolon BYTE "Missing semicolon after statement", 0
ErrMsg_InvalidToken     BYTE "Invalid token encountered", 0
...
; =======================================================================================
; Helper: StrCmpA - Compare two ASCII strings
; RCX = string 1
; RDX = string 2
...
; =======================================================================================
; Helper: IsKeyword - Check if identifier is a C# keyword
...
IsKeyword PROC FRAME
    ...
    mov r12, rcx          ; r12 = identifier
    mov r13, OFFSET KeywordTable
    
IsKeyword_loop:
    mov al, byte ptr [r13]
    test al, al
    jz IsKeyword_not_found    ; End of table
...
; =======================================================================================
; Helper: AddDiagnostic - Add syntax error to diagnostics array
; RCX = line number
; RDX = column number
; R8 = error code
; R9 = pointer to error message string
; =======================================================================================
AddDiagnostic PROC FRAME
    ...
    mov r12, rcx          ; r12 = line
    mov r13, rdx          ; r13 = column
    mov r14, r8           ; r14 = error code
    mov r15, r9           ; r15 = message pointer
    
    ; Check if diagnostic array is full
    cmp DiagnosticCount, 32
    jge AddDiagnostic_done
    
    ; Calculate diagnostic slot
    ...
    ; Copy message with actual length (not fixed 127)
    mov rcx, r15
    call StrLenA
    cmp rax, 127
...
Rawrxd_ParseCSharpSyntax PROC FRAME
    ...
    mov rsi, rcx        ; rsi = Source cursor
    mov rdi, rdx        ; rdi = Diagnostic output struct
...
ScanLoop:
    ...
    cmp InComment, 1
    jne ProcessChar
    mov InComment, 0    ; End line comment
    jmp NextChar
    
CheckCarriageReturn:
    cmp al, 13          ; '\r'
    je NextChar         ; Ignore CR
    
ProcessChar:
    ; --- Handle Comments ---
    cmp InComment, 1
    je NextChar         ; Skip everything inside line comment
    ...
    mov InComment, 1    ; Enter Line Comment
    inc rsi
    inc CurrentColumn
    jmp NextChar
...
CheckBlockCommentStart:
    cmp bl, '*'
    jne CheckStrings
    mov InBlockComment, 1 ; Enter Block Comment
    inc rsi
    inc CurrentColumn
    jmp NextChar
...
CheckEndBlockComment:
    ...
    cmp bl, '/'
    jne NextChar
    mov InBlockComment, 0 ; Exit Block Comment
    inc rsi
    inc CurrentColumn
    jmp NextChar
...
CheckIdentifier:
    ; Check if we need a semicolon before this token
    cmp ExpectSemicolon, 1
    jne CheckKeyword
    ; Only enforce for certain tokens
    cmp al, '}'
    je NextChar           ; Semicolon not required before }
    cmp al, ')'
    je NextChar           ; Semicolon not required before )
    cmp al, ','
    je NextChar           ; Semicolon not required before ,
    cmp al, ';'
    je NextChar           ; Already have semicolon
    
    ; Missing semicolon
    mov rcx, CurrentLine
    mov rdx, CurrentColumn
    mov r8, 7               ; Error code 7 = Missing semicolon
...
CheckKeyword:
    ; Check if this is start of identifier
    cmp al, '_'
    jl CheckNumber
    cmp al, 'z'
    jg CheckNumber
...
StartIdentifier:
    ...
    lea rcx, CurrentToken.Value
    call IsKeyword
    test rax, rax
    jz NotKeyword
    mov CurrentToken.Type, TOKEN_KEYWORD
    
    ; Check if this keyword expects a semicolon after
    lea rcx, CurrentToken.Value
    call NeedsSemicolon
    test rax, rax
    jz NextChar
...
NotKeyword:
    mov CurrentToken.Type, TOKEN_IDENTIFIER
    
    ; Check if identifier is end of statement
    lea rcx, CurrentToken.Value
    call NeedsSemicolon
    test rax, rax
    jz NextChar
...
StartNumber:
    mov CurrentToken.Type, TOKEN_NUMBER
    jmp NextChar
...
IsOperator:
    mov CurrentToken.Type, TOKEN_OPERATOR
    jmp NextChar
...
InvalidChar:
    mov rcx, CurrentLine
    mov rdx, CurrentColumn
    mov r8, 6               ; Error code 6 = Invalid char
...
StackOverflow:
    mov rcx, CurrentLine
    mov rdx, CurrentColumn
    mov r8, 8               ; Error code 8
    lea r9, ErrMsg_StackOverflow
    call AddDiagnostic
    ; Halt parsing on stack overflow
    jmp EOF
...
EOF:
    ...
    ; Report unclosed brace at position where it was opened
    mov rax, BraceStackPtr
    test rax, rax
    jz CheckUnclosedParen
    dec rax
    mov ebx, BraceLineStack[rax * 4]
    mov ecx, BraceColStack[rax * 4]
    mov rcx, rbx
    mov rdx, rcx
    mov r8, 2               ; Error code 2
...
CheckUnclosedParen:
    ...
    dec rax
    mov ebx, ParenLineStack[rax * 4]
    mov ecx, ParenColStack[rax * 4]
    mov rcx, rbx
    mov rdx, rcx
    mov r8, 4               ; Error code 4
...
SyntaxErrorFound:
    ; Copy first diagnostic to output struct
...
; =======================================================================================
; Helper: ParseIdentifier - Extract identifier from source
; RCX = destination token value buffer
; RDX = source cursor
; =======================================================================================
...
; =======================================================================================
; Helper: NeedsSemicolon - Check if token requires semicolon after
; RCX = pointer to token string
; Returns: RAX = 1 (needs semicolon), 0 (doesn't)
; =======================================================================================
NeedsSemicolon PROC FRAME
    ...
    ; Check for return/break/continue/throw
    lea rdx, szReturn
    call StrCmpA
    test rax, rax
    jz NeedsSemi_yes
    ...
    ; Check for simple assignment/declaration patterns
    ; (In real implementation, would check token context)
...
; =======================================================================================
; Rawrxd_GetDiagnosticCount - Return number of diagnostics
; Returns: RAX = count
; =======================================================================================
...
; =======================================================================================
; Rawrxd_GetDiagnostic - Get specific diagnostic
; RCX = index
...
; =======================================================================================
; String data for NeedsSemicolon
; =======================================================================================
.data
...
; =======================================================================================
; Helper: PrintInt - Print integer as decimal
; RCX = integer value
; =======================================================================================
PrintInt PROC FRAME
    push rbp
    ...
    lea rbx, [rsp + 30h]  ; rbx = buffer end
    mov byte ptr [rbx], 0
    dec rbx
...
; =======================================================================================
.data
TestSource              BYTE "class Test { int x; }", 0
