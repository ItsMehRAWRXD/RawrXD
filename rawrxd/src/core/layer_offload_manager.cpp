...
PatchResult LayerOffloadManager::initialize(const char* modelPath,
    ...
    // NOTE: ModelSharding feature not yet in license manifest; gate on Enterprise tier instead
    if (lic.currentTier() < RawrXD::License::LicenseTierV2::Enterprise) {
    ...
    if (isPyre) {
        ...
        for (uint32_t t = 0; t < header.numTensors; t++) {
            ...
            if (layerIdx >= 0 && layerIdx < static_cast<int>(m_layerCount)) {
                ...
                strncpy_s(slice.name, sizeof(slice.name), entry.name, sizeof(slice.name) - 1);
                slice.fileOffset = header.dataOffset + entry.offset;
                slice.byteSize = entry.byteSize;
                // Determine per-tensor quant format
                switch (static_cast<uint32_t>(entry.dtype)) {
                    case static_cast<uint32_t>(PyreDataType::Q2_K): slice.format = QuantFormat::Q2_K; break;
                    case static_cast<uint32_t>(PyreDataType::Q4_0): slice.format = QuantFormat::Q4_0; break;
                    case static_cast<uint32_t>(PyreDataType::Q8_0): slice.format = QuantFormat::Q8_0; break;
                    case static_cast<uint32_t>(PyreDataType::F16): slice.format = QuantFormat::F16;  break;
