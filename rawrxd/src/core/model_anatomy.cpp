                              ...
                              std::string* errorOut)
{
    ...
    for (uint64_t i = 0; i < tensorCount; ++i)
    {
        ...
        std::vector<uint8_t> nameBa(static_cast<size_t>(nameLen) + 1);
        if (ds.readRawData(reinterpret_cast<char*>(nameBa.data()), static_cast<int>(nameLen)) != static_cast<int>(nameLen))
        {
            if (errorOut)
            {
                *errorOut = "truncated tensor name";
            }
            return false;
        }
        nameBa[static_cast<size_t>(nameLen)] = 0;
        te.name.assign(reinterpret_cast<const char*>(nameBa.data()), static_cast<size_t>(nameLen));
        uint32_t nDims = 0;
        ds >> nDims;
