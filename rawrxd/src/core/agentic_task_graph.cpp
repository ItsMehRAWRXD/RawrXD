...
static int run_shell_command(const std::string& cmd) {
    ...
    AgentToolRegistry& authority = RequireAgentToolAuthority();
    ToolRequest req;
    req.tool_id = "run-shell";
    req.stdin_text = "{\"cmd\":\"" + cmd + "\"}";
    ToolContext ctx{};
...
AgenticTaskGraph::AgenticTaskGraph()
    : nextGraphId_(1), nextTaskId_(10000), shutdownRequested_(false), eventListenerCount_(0), checkpointRunning_(false)
{
...
// Graph Lifecycle
// ============================================================================
uint64_t AgenticTaskGraph::createGraph(const std::string& goalDescription, const TaskGraphConfig& config)
{
...
TaskResult AgenticTaskGraph::addDependency(uint64_t graphId, uint64_t taskId, uint64_t dependsOnId)
{
    std::lock_guard<std::mutex> lock(graphMutex_);
...
// ============================================================================
TaskResult AgenticTaskGraph::topologicalSort(const Graph& g, std::vector<uint64_t>& sorted) const
{
    sorted.clear();
    ...
    auto cmp = [&g](uint64_t a, uint64_t b)
    {
        ...
        if (ait == g.tasks.end() || bit == g.tasks.end())
            return false;
        return static_cast<uint8_t>(ait->second.priority) > static_cast<uint8_t>(bit->second.priority);
    };
...
std::vector<uint64_t> AgenticTaskGraph::findReadyTasks(const Graph& g) const
{
    ...
    for (auto& [id, task] : g.tasks)
    {
        ...
        for (auto depId : task.dependsOn)
        {
            ...
            if (dit->second.state != TaskState::SUCCEEDED && dit->second.state != TaskState::SKIPPED)
            {
                allDeps = false;
                break;
            }
...
TaskResult AgenticTaskGraph::executeTask(Graph& g, uint64_t taskId)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (g.config.enableAutoRollback && task.rollback != RollbackStrategy::NONE)
            {
            ...
            while (!skipQueue.empty())
            {
                ...
                if (sit != g.tasks.end() && sit->second.state == TaskState::PENDING)
                {
    ...
    {
        if (t.state != TaskState::SUCCEEDED && t.state != TaskState::SKIPPED && t.state != TaskState::FAILED &&
            t.state != TaskState::ROLLED_BACK && t.state != TaskState::CANCELLED)
        {
            allDone = false;
            break;
        }
    ...
    if (allDone)
    {
        ...
        doneEvt.type = anyFailed ? TaskGraphEvent::GRAPH_FAILED : TaskGraphEvent::GRAPH_COMPLETED;
        doneEvt.graphId = g.id;
        doneEvt.taskId = 0;
        doneEvt.progressPercentage = 100;
        doneEvt.detail = anyFailed ? "Graph completed with failures" : "Graph completed successfully";
        ...
        {
            std::string path = g.config.persistPath + "graph_" + std::to_string(g.id) + ".json";
            std::string json;
...
TaskResult AgenticTaskGraph::performRollback(Graph& g, uint64_t taskId)
{
    ...
    switch (task.rollback)
    {
        ...
        case RollbackStrategy::GIT_REVERT:
        {
            ...
            {
                // Route through canonical AgentToolRegistry
        ...
        case RollbackStrategy::UNDO_LOG:
        {
            ...
            if (!task.rollbackData.empty())
            {
                ...
                while (offset + 5 <= task.rollbackData.size())
                {
                    ...
                    if (offset + opLen > task.rollbackData.size())
                        break;
                    ...
                    else if (opType == 1)
                    {
                        ...
                        if (opLen >= 4)
                        {
                            ...
                            {
                                std::string filePath(task.rollbackData.begin() + offset + 4,
                                                     task.rollbackData.begin() + offset + 4 + pathLen);
                                BytePatch bp;
                                memcpy(&bp.offset, &task.rollbackData[offset + 4 + pathLen], sizeof(uint64_t));
                                size_t dataStart = offset + 4 + pathLen + 8;
                                size_t dataLen = opLen - 4 - pathLen - 8;
                                bp.data.assign(task.rollbackData.begin() + dataStart,
                                               task.rollbackData.begin() + dataStart + dataLen);
                                bp.description = "undo_log rollback";
                                UnifiedHotpatchManager::instance().apply_byte_patch(filePath.c_str(), bp);
                            }
                        }
                    }
...
TaskResult AgenticTaskGraph::rollbackToCheckpoint(uint64_t graphId, uint64_t checkpointId)
{
    std::lock_guard<std::mutex> lock(graphMutex_);
    ...
    for (int i = (int)sorted.size() - 1; i > checkpointPos; --i)
    {
        ...
        if (task.state == TaskState::SUCCEEDED || task.state == TaskState::FAILED || task.state == TaskState::RUNNING)
        {
    ...
    {
        auto& task = g.tasks[sorted[i]];
        if (task.state == TaskState::ROLLED_BACK || task.state == TaskState::CANCELLED)
        {
            task.state = TaskState::PENDING;
    ...
    char msg[128];
    snprintf(msg, sizeof(msg), "Rolled back %u tasks to checkpoint %llu", rolledBack, (unsigned long long)checkpointId);
    return TaskResult::ok(msg);
}
...
// ============================================================================
TaskState AgenticTaskGraph::getTaskState(uint64_t graphId, uint64_t taskId) const
{
    std::lock_guard<std::mutex> lock(graphMutex_);
    auto git = graphs_.find(graphId);
    if (git == graphs_.end())
...
void AgenticTaskGraph::updateTaskProgress(uint64_t graphId, uint64_t taskId, uint32_t progressPercentage)
{
    std::lock_guard<std::mutex> lock(graphMutex_);
    auto git = graphs_.find(graphId);
    if (git == graphs_.end())
...
// ============================================================================
TaskResult AgenticTaskGraph::serializeGraph(const Graph& g, std::string& jsonOut) const
{
...
TaskResult AgenticTaskGraph::deserializeGraph(const std::string& json, Graph& g)
{
    ...
    auto parseIntArray = [&](const std::string& arr) -> std::vector<uint64_t>
    {
        ...
        while (pos < inner.size())
        {
            ...
            if (!num.empty())
                result.push_back(static_cast<uint64_t>(std::strtoll(num.c_str(), nullptr, 10)));
            pos = end;
        }
        return result;
    };
...
TaskResult AgenticTaskGraph::saveGraph(uint64_t graphId, const char* filepath)
{
    ...
    {
        path = it->second.config.persistPath + "graph_" + std::to_string(graphId) + ".json";
    }
...
std::vector<std::string> AgenticTaskGraph::listSavedGraphs() const
{
    ...
    if (dir)
    {
        ...
        while ((entry = readdir(dir)) != nullptr)
        {
            ...
            if (name.size() > 5 && name.substr(name.size() - 5) == ".json" && name.substr(0, 6) == "graph_")
            {
...
// LLM-Guided Planning
// ============================================================================
uint64_t AgenticTaskGraph::planFromGoal(const std::string& goal, const TaskGraphConfig& config)
{
    // Create the graph
...
TaskResult AgenticTaskGraph::replanFromFailure(uint64_t graphId, uint64_t failedTaskId)
{
    std::lock_guard<std::mutex> lock(graphMutex_);
    ...
    replacement.description = "LLM-generated alternative for failed task: " + tit->second.description;
...
// Statistics
// ============================================================================
AgenticTaskGraph::GraphStats AgenticTaskGraph::getGraphStats(uint64_t graphId) const
{
    std::lock_guard<std::mutex> lock(graphMutex_);
    GraphStats stats = {};
    ...
    for (auto& [id, task] : g.tasks)
    {
        switch (task.state)
        {
            ...
            case TaskState::FAILED:
                stats.failedTasks++;
                break;
            case TaskState::ROLLED_BACK:
                stats.rolledBackTasks++;
                break;
    ...
    {
        uint32_t done = stats.completedTasks + stats.failedTasks + stats.rolledBackTasks + stats.skippedTasks;
    ...
    {
        stats.elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(now - g.startedAt);
    }
...
// Event System
// ============================================================================
void AgenticTaskGraph::addEventListener(TaskGraphEventCallback callback, void* userData)
{
    uint32_t idx = eventListenerCount_.load();
    if (idx >= MAX_EVENT_LISTENERS)
        return;
...
void AgenticTaskGraph::workerThread()
{
    while (!shutdownRequested_.load())
    {
...

        {
            ...
            workCV_.wait_for(lock, std::chrono::milliseconds(100),
                             [this] { return !workQueue_.empty() || shutdownRequested_.load(); });
...
TaskResult AgenticTaskGraph::executor_codeRefactor(TaskNode* self, void* context)
{
    (void)context;
    if (!self)
        return TaskResult::error("Null task node", -1);
    ...
    RefactorResult rr = engine.executeSafeRefactor(self->name, targetFiles, verifyConfig, applyFn);
    ...
    summary << "Refactor '" << self->name << "' applied: " << rr.diff.filesChanged << " files, +"
            << rr.diff.totalLinesAdded << "/-" << rr.diff.totalLinesRemoved << " lines";
...
TaskResult AgenticTaskGraph::executor_symbolIndex(TaskNode* self, void* context)
{
    ...
    std::ostringstream summary;
    summary << "Indexed " << indexer.indexedFileCount() << " files, " << indexer.totalChunks() << " chunks in "
            << indexPath.string();
    self->outputSummary = summary.str();
...
TaskResult AgenticTaskGraph::executor_runTests(TaskNode* self, void* context)
{
    ...
    if (!self)
        return TaskResult::error("Null task node", -1);
    // Route through canonical AgentToolRegistry
    int rc = run_shell_command("cmake --build . --config Release --target self_test_gate 2>&1");
    if (rc != 0)
        return TaskResult::error("Tests failed", rc);
...
TaskResult AgenticTaskGraph::executor_buildProject(TaskNode* self, void* context)
{
    ...
    if (!self)
        return TaskResult::error("Null task node", -1);
    int rc = run_shell_command("cmake --build . --config Release 2>&1");
    if (rc != 0)
        return TaskResult::error("Build failed", rc);
...
TaskResult AgenticTaskGraph::executor_agenticLoop(TaskNode* self, void* context)
{
    ...
    if (!self->checkpointData.empty())
    {
        ...
        {
            if (!line.empty())
                config.openFiles.push_back(line);
        }
    }
...
TaskResult AgenticTaskGraph::executor_hotpatch(TaskNode* self, void* context)
{
    ...
    if (!self->checkpointData.empty())
    {
        ...
        if (patchType == 0)
        {
            ...
            auto ur = uhm.apply_memory_patch(reinterpret_cast<void*>(addr), patchSize, patchData);
            if (!ur.result.success)
            {
            ...
            std::ostringstream ss;
            ss << "Memory patch applied at 0x" << std::hex << addr << " (" << std::dec << patchSize
               << " bytes, seq=" << ur.sequenceId << ")";
            self->outputSummary = ss.str();
        }
        else if (patchType == 1)
        {
            ...
            bp.data.assign(self->checkpointData.begin() + headerEnd + 8, self->checkpointData.end());
            ...
            std::ostringstream ss;
            ss << "Byte patch applied to " << filename << " at offset " << bp.offset << " (" << bp.data.size()
               << " bytes, seq=" << ur.sequenceId << ")";
            self->outputSummary = ss.str();
        }
        else if (patchType == 2)
        {
            ...
            self->outputSummary = std::string("Server patch '") + (shp->name ? shp->name : "unnamed") +
...
TaskResult AgenticTaskGraph::executor_swarmCompile(TaskNode* self, void* context)
{
    ...
    if (!self->checkpointData.empty())
    {
        ...
        profileName = std::string(self->checkpointData.begin(), self->checkpointData.end());
    }
    ...
    // Execute the actual build via canonical AgentToolRegistry
    int buildRc = run_shell_command("cmake --build . --config Release 2>&1");
    ...
    swarm.recordStep("compiler",                    // agentId
    ...
    {
        std::ostringstream ss;
        ss << "Swarm compilation failed (rc=" << buildRc << ", " << trace.entries.size() << " trace entries, "
    ...
    std::ostringstream summary;
    summary << "Swarm compile complete: " << trace.entries.size() << " steps, " << std::fixed << std::setprecision(1)
            << buildMs << "ms"
            << ", deterministic=" << (replayResult.reproducible ? "yes" : "no") << ", trace=" << trace.traceId;
    self->outputSummary = summary.str();
...
void AgenticTaskGraph::shutdown()
{
    ...
    for (auto& [gid, g] : graphs_)
    {
        if (g.config.persistToDisk && (g.isRunning || !g.isCompleted))
        {
            std::string json;
            serializeGraph(g, json);
            std::string path = g.config.persistPath + "graph_" + std::to_string(gid) + "_shutdown.json";
            std::ofstream out(path);
            if (out.is_open())
            {
                out << json;
