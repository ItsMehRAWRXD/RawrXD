                               ...
                               const uint8_t* symbolStream, uint32_t symbolStreamSize)
{
    if (!streamData || streamSize < sizeof(GSIHashHeader)) {
        return PDBResult::error("GSI stream too small", PDB_ERR_INVALID_FORMAT);
    }
    if (!symbolStream || symbolStreamSize == 0) {
        return PDBResult::error("Null symbol stream", PDB_ERR_INVALID_FORMAT);
    }
    ...
    if (hdr->verSignature != GSI_HASH_SIGNATURE || hdr->verHdr != GSI_HASH_V70) {
        ...
        return PDBResult::error("Unsupported GSI hash version", PDB_ERR_INVALID_FORMAT);
    }

    uint32_t hrSize = hdr->hrSize;
    if (hrSize == 0 || hrSize % GSI_HASH_RECORD_SIZE != 0) {
        return PDBResult::error("Invalid GSI hash record size", PDB_ERR_INVALID_FORMAT);
    }
    ...
    if (afterRecords + bitmapSize > streamSize) {
        return PDBResult::error("GSI stream truncated (no bitmap)", PDB_ERR_INVALID_FORMAT);
    }
...
PDBResult TPIStreamParser::parse(const uint8_t* streamData, uint32_t streamSize) {
    if (!streamData || streamSize < sizeof(TPIStreamHeader)) {
        return PDBResult::error("TPI stream too small", PDB_ERR_INVALID_FORMAT);
    }
    ...
    if (hdr->version != 20040203) {
        ...
        snprintf(msg, sizeof(msg), "Unsupported TPI version: %u (expected 20040203)",
                 hdr->version);
        return PDBResult::error(msg, PDB_ERR_INVALID_FORMAT);
    }
    ...
    if (dataStart + typeRecordBytes > streamSize) {
        return PDBResult::error("TPI type record data extends past stream", PDB_ERR_INVALID_FORMAT);
    }
...
{
    if (!out || maxLen < 32) {
        return PDBResult::error("Buffer too small", PDB_ERR_INVALID_FORMAT);
    }
    ...
    if (leafKind != LF_PROCEDURE && leafKind != LF_MFUNCTION) {
        snprintf(out, maxLen, "<type 0x%X is not a procedure (leaf=0x%04X)>",
                 typeIndex, leafKind);
        return PDBResult::error("Not a procedure type", PDB_ERR_INVALID_FORMAT);
    }

    if (recSize < sizeof(CVTypeProcedure)) {
        snprintf(out, maxLen, "<truncated procedure type>");
        return PDBResult::error("Truncated procedure record", PDB_ERR_INVALID_FORMAT);
    }
