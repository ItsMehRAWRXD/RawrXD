...
// ============================================================================
// Local helper: route a shell/build command through the canonical
// AgentToolRegistry. Every agent-originated process action must cross the
...
WorkflowStageResult AutonomousWorkflowEngine::executeBuild(const WorkflowPolicy& policy) {
    ...
    int buildRc = run_shell_command(cmd);
    ...
    r.success = (buildRc == 0);
    r.detail = r.success ? "Build succeeded" : "Build failed with exit code " + std::to_string(buildRc);
    r.durationMs = static_cast<double>(elapsed);
...
WorkflowStageResult AutonomousWorkflowEngine::executeTest(const WorkflowPolicy& policy) {
    ...
    int testRc = run_shell_command(cmd);
