...
struct ModelProbeResult {
    ...
    bool        ollama_identity_verified = false; // Exact model identity confirmed
    bool        ollama_generation_valid = false; // Ollama generated tokens
    bool        cpu_loadable     = false; // CPUInferenceEngine can open it
    bool        cpu_tokenizer_valid = false; // Tokenizer test passed
    bool        cpu_generation_valid = false; // Actual generation produced tokens
    bool        native_loadable  = false; // NativeInferencePipeline can open it
    bool        native_generation_valid = false; // Native pipeline generated tokens
    bool        token_generated  = false; // At least 1 token produced (any backend)
    ...
    bool        cli_e2e_certified = false; // End-to-end CLI generation verified
    bool        gui_e2e_certified = false; // End-to-end GUI generation verified
    bool        html_e2e_certified = false; // End-to-end HTML generation verified
...
public:
    ...
    /** Get last scan results (cached from most recent BruteForceAll). Returns a snapshot copy. */
    std::vector<ModelProbeResult> GetLastResults() const;

    /** Get last scan progress (for async polling). Returns a snapshot copy. */
