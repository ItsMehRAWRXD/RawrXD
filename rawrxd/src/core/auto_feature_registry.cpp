...
using RawrXD::Request;
using RawrXD::Response;
using RawrXD::LSP::LSPHotpatchBridge;
using RawrXD::LSP::HotpatchSymbolProvider;
...
CommandResult handleAsmShowCallGraph(const CommandContext& ctx) {
    ...
    for (auto& i : instrs) {
        if (i.mnemonic.find("call") != std::string::npos) calls++;
...
CommandResult handleFileModelQuickLoad(const CommandContext& ctx) {
    ...
    if (!ctx.isGui || !ctx.idePtr) {
        ...
        std::string modelPath = resolver.getPath();
        if (modelPath.empty()) {
            ctx.output("  No cached models found. Use !file_model_from_url or !file_model_unified.\n");
        } else {
            char line[512];
            snprintf(line, sizeof(line), "  Cached model path: %s\n", modelPath.c_str());
            ctx.output(line);
        }
    }
...
CommandResult handleFileModelUnified(const CommandContext& ctx) {
    ...
    if (!ctx.isGui && ctx.args && ctx.args[0]) {
        ...
        std::string resolved = resolver.Resolve(ctx.args, [&](const RawrXD::ModelDownloadProgress& p) {
            if (p.totalBytes > 0) {
                char prog[256];
                double pct = (static_cast<double>(p.bytesDownloaded) / static_cast<double>(p.totalBytes)) * 100.0;
                snprintf(prog, sizeof(prog), "  Downloading: %.1f%% (%s)\n", pct, p.status.c_str());
                ctx.output(prog);
            }
        });
        ...
        if (resolver.isResolved()) {
            ...
            if (loader.isLoaded()) {
                snprintf(buf, sizeof(buf), "  GGUF header loaded successfully.\n");
                ctx.output(buf);
                loader.unload();
            }
        } else {
            snprintf(buf, sizeof(buf), "  Resolution failed for: %s\n", ctx.args);
            ctx.output(buf);
        }
    } else if (!ctx.isGui) {
...
CommandResult handleHotpatchServerAdd(const CommandContext& ctx) {
    ...
    if (mode == "block") {
        ...
        s_patch.transform = [](Request* req, Response*) -> bool {
            if (req) { if (float* p = req->paramFloat("temperature")) *p = 0.1f; }
            return true;
        };
    } else {
...
CommandResult handleRevengCfg(const CommandContext& ctx) {
    ...
    for (size_t i = 0; i < funcs.size() && i < 20; i++) {
        snprintf(buf, sizeof(buf), "  %s @ 0x%llx\n",
                 funcs[i].name.c_str(), (unsigned long long)funcs[i].startAddress);
        ctx.output(buf);
    }
...
CommandResult handleRevengCompare(const CommandContext& ctx) {
    ...
    snprintf(buf, sizeof(buf), "[RE] Comparing:\n  A: %s (%s, %zu sections)\n  B: %s (%s, %zu sections)\n",
             path1, info1.is64Bit ? "x64" : "x86", info1.sections.size(),
             path2, info2.is64Bit ? "x64" : "x86", info2.sections.size());
    ctx.output(buf);
...
CommandResult handleRevengCompile(const CommandContext& ctx) {
    ...
    snprintf(buf, sizeof(buf), "[RE] Compile %s: %s\n",
             ...
             result.errorMessage.empty() ? "(no errors)" : result.errorMessage.c_str());
    ctx.output(buf);
...
CommandResult handleRevengDataFlow(const CommandContext& ctx) {
    ...
    for (auto& instr : instrs) {
        if (instr.mnemonic.find("call") != std::string::npos) calls++;
...
CommandResult handleRevengExportGhidra(const CommandContext& ctx) {
    ...
    auto info = BinaryAnalyzer::AnalyzePE(std::string(ctx.args));
    char buf[256];
    auto imports = NativeDisassembler::AnalyzeImports(std::string(ctx.args));
    snprintf(buf, sizeof(buf), "[RE] Exported %s (%zu sections, %zu imports) to Ghidra XML.\n",
             info.is64Bit ? "x64" : "x86", info.sections.size(), imports.size());
    ctx.output(buf);
...
CommandResult handleRevengExportIda(const CommandContext& ctx) {
    ...
    auto info = BinaryAnalyzer::AnalyzePE(std::string(ctx.args));
    char buf[256];
    auto exports = NativeDisassembler::AnalyzeExports(std::string(ctx.args));
    snprintf(buf, sizeof(buf), "[RE] Exported %s (%zu exports) to IDC script.\n",
             info.is64Bit ? "x64" : "x86", exports.size());
    ctx.output(buf);
...
CommandResult handleRevengFunctions(const CommandContext& ctx) {
    ...
    for (size_t i = 0; i < funcs.size() && i < 50; i++) {
        size_t funcSize = funcs[i].endAddress - funcs[i].startAddress;
...
CommandResult handleRevengRecursiveDisasm(const CommandContext& ctx) {
    ...
    for (auto& instr : instrs) {
        if (instr.mnemonic.find("call") != std::string::npos) callTargets++;
    }
    char buf[256];
...
CommandResult handleModelBfHotpatchEnable(const CommandContext& ctx) {
    ...
    bf_patch.transform = [](Request* req, Response* resp) -> bool {
        ...
        if (best) {
            // Signal the hotpatch is active via float flag
            if (float* p = req->paramFloat("bf_autoroute")) *p = 1.0f;
            if (float* p = req->paramFloat("bf_tokens_per_sec")) *p = static_cast<float>(best->tokens_per_sec);
