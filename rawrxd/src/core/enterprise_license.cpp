...
// Bridge macros: IDELogger exposes static methods, not LOG_* macros
#define LOG_INFO(...) IDELogger::info(__VA_ARGS__)
#define LOG_WARNING(...) IDELogger::warn(__VA_ARGS__)
#define LOG_ERROR(...) IDELogger::error(__VA_ARGS__)
...
void appendFeature(uint64_t mask, uint64_t bit, const char* name, std::vector<std::string>& out)
{
    if ((mask & bit) != 0)
...
bool loadAzureADConfig(AzureADConfig* out)
{
    ...
    if (out->provider != "azure-ad")
        return false;
    if (out->clientId.empty() || out->clientId == "your-client-id-here")
        return false;
    return true;
}
...
bool readRegistryLicensePath(std::wstring& outPath)
{
    ...
    if (RegQueryValueExW(key, kLicenseRegistryValue, nullptr, &type, reinterpret_cast<BYTE*>(&buffer[0]), &size) !=
        ERROR_SUCCESS)
    {
        RegCloseKey(key);
        return false;
...
void writeRegistryLicensePath(const std::wstring& path)
{
    ...
    HKEY key = nullptr;
    DWORD disp = 0;
    if (RegCreateKeyExW(HKEY_CURRENT_USER, kLicenseRegistryKey, 0, nullptr, REG_OPTION_NON_VOLATILE, KEY_WRITE, nullptr,
    ...
    RegSetValueExW(key, kLicenseRegistryValue, 0, REG_SZ, reinterpret_cast<const BYTE*>(path.c_str()), size);
    RegCloseKey(key);
}
#endif
}  // namespace
...
bool EnterpriseLicense::Initialize()
{
    ...
    LOG_INFO("[EnterpriseLicense] Initializing license subsystem...");
    ...
    if (readRegistryLicensePath(storedPath))
    {
        ...
        if (GetState() == LicenseState::Invalid)
        {
            ...
            {
                LOG_INFO("[EnterpriseLicense] Loaded license from registry path");
            }
        }
    }
#endif
    ...
    if (loadAzureADConfig(&aad))
    {
        ...
        telemetry.recordAudit(AuditEventType::ConfigChange, "system", "azure_ad", "loaded",
    ...
    switch (newState)
    {
        case LicenseState::ValidEnterprise:
        {
            ...
            oss << "[EnterpriseLicense] Enterprise license ACTIVE — Features: 0x" << std::hex << GetFeatureMask()
                << std::dec;
            LOG_INFO(oss.str());
            LOG_INFO(std::string("[EnterpriseLicense] 800B Dual-Engine: ") +
        ...
        case LicenseState::ValidTrial:
            ...
            break;
        case LicenseState::Expired:
            LOG_INFO("[EnterpriseLicense] License EXPIRED — running Community mode");
            break;
        case LicenseState::HardwareMismatch:
            LOG_INFO("[EnterpriseLicense] Hardware mismatch — running Community mode");
            break;
        case LicenseState::Tampered:
            LOG_INFO("[EnterpriseLicense] TAMPER DETECTED — degraded Community mode");
            break;
        default:
        {
            std::ostringstream oss;
...
{
    if (!m_initialized)
        return "RawrXD Community (License system not initialized)";
...
uint64_t EnterpriseLicense::GetMaxModelSizeGB() const
{
    ...
    {
        case LicenseState::ValidEnterprise:
            return 800;  // 800B parameter models
        case LicenseState::ValidOEM:
            return 800;
        case LicenseState::ValidTrial:
            return 180;  // Up to 180B trial
        default:
            return 70;  // Community: 70B max
    }
}
...
uint32_t EnterpriseLicense::GetShieldState() const
{
    ...
    if (!m_initialized)
        return 0;
    if (m_lastState == LicenseState::Tampered)
        return 0;
...
bool EnterpriseLicense::InstallLicense(const void* licenseBlob, size_t blobSize, const void* signature)
{
    if (!m_initialized)
    {
        LOG_ERROR("[EnterpriseLicense] Cannot install — system not initialized");
        return false;
    }
    ...
    int64_t result = Enterprise_InstallLicense(licenseBlob, static_cast<uint64_t>(blobSize), signature);
    ...
    LOG_INFO(std::string("[EnterpriseLicense] Features: ") + GetFeatureString());
    std::ostringstream oss2;
    oss2 << "[EnterpriseLicense] Max Model: " << GetMaxModelSizeGB() << "GB | Max Context: " << GetMaxContextLength()
         << " tokens";
    LOG_INFO(oss2.str());
...
void EnterpriseLicense::notifyStateChange(LicenseState oldState, LicenseState newState)
{
    ...
    EnterpriseStateChanged(static_cast<uint32_t>(oldState), static_cast<uint32_t>(newState));
    ...
    {
        telemetry.recordAudit(AuditEventType::LicenseCheck, "system", "license", "expired", "state=Expired");
        telemetry.incrementCounter("license.expired.count", 1.0);
    }

    if (oldState == LicenseState::Expired &&
        (newState == LicenseState::ValidEnterprise || newState == LicenseState::ValidPro ||
         newState == LicenseState::ValidTrial || newState == LicenseState::ValidOEM))
    {
        telemetry.recordAudit(AuditEventType::LicenseCheck, "system", "license", "renewed", "state=Renewed");
...
void EnterpriseLicense::updateTelemetry(LicenseState state, const char* action)
{
    ...
    info.valid = (state == LicenseState::ValidEnterprise || state == LicenseState::ValidPro ||
                  state == LicenseState::ValidTrial || state == LicenseState::ValidOEM);
    ...
    telemetry.recordAudit(AuditEventType::LicenseCheck, "system", "license", action ? action : "update", detail);
...
// LicenseGuard RAII Implementation
// ============================================================================
LicenseGuard::LicenseGuard(EnterpriseFeature required) : m_granted(false), m_required(required)
{
