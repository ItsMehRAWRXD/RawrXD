...
struct CorrectionResult {
    ...
    static CorrectionResult ok(const std::string& output, PuppeteerFailure failure);
    static CorrectionResult error(PuppeteerFailure failureType, const std::string& diagnostic);
};

class AgenticPuppeteer {
public:
    using FailureType = PuppeteerFailure;

    AgenticPuppeteer();
    ~AgenticPuppeteer();

    CorrectionResult correctResponse(const std::string& originalResponse,
                                      const std::string& userPrompt = "");
    CorrectionResult correctJsonResponse(const std::string& response,
...
public:
    ...
    CorrectionResult bypassRefusal(const std::string& refusedResponse,
...
public:
    ...
    CorrectionResult detectAndCorrectHallucination(
...
public:
    ...
    void setRequiredJsonSchema(const std::string& schema);
