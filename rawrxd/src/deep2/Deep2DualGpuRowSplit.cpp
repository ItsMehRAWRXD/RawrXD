...
// DEEP2_SPLIT_RATIO_CACHE_001 v2: set when the frozen-split gate fires;
// the ratio controller stops updating and every split serves from the
// frozen value (stable geometry keeps RCU residency keys valid).
std::atomic<uint32_t> gSplitFrozen{0};
...
static double envInitialAsyncRowRatio() noexcept {
    const char* e=std::getenv("DEEP2_SPLIT_INITIAL_RATIO");
    if(!e||!*e) return 0.50;
    char* end=nullptr;
    const double v=std::strtod(e,&end);
    return end!=e && v>=0.20 && v<=0.80 ? v : 0.50;
}
...
void updateAsyncRowRatio(uint32_t rows0, uint64_t ns0, uint32_t rows1, uint64_t ns1) noexcept {
    ...
    // One-time seed: allow env override of initial ratio for faster
    // convergence to the true device capability ratio.
    static std::atomic<uint32_t> initDone{0};
    if(initDone.load(std::memory_order_relaxed)==0) {
        gAsyncRowRatio.store(envInitialAsyncRowRatio(), std::memory_order_relaxed);
        initDone.store(1, std::memory_order_relaxed);
    }
    ...
    // DEEP2_SPLIT_RATIO_CACHE_001 v2: WARMUP takes large steps toward the
    // ideal so the ratio genuinely converges BEFORE the freeze fires;
    // once frozen the controller stops updating entirely (measured-window
    // geometry stays stable, RCU residency keys stay valid, residency
    // stays hot — re-splitting mid-measure is what collapsed TPS 17x).
    if (gSplitFrozen.load(std::memory_order_relaxed) == 0) {
        double target = 0.5*r + 0.5*ideal;
        target = std::max(r - 0.20, std::min(r + 0.20, target));
...
struct SplitCacheKey {
    ...
    uint32_t speed0m=0;
    uint32_t speed1m=0;
    // DEEP2_SPLIT_RATIO_CACHE_001: async-ratio bucket (16-permille
    // granularity) — a converged controller re-derives plans instead of
    // serving splits frozen at the first-call ratio.
    uint32_t ratioBucket=31;
...
struct SplitCacheHash {
    size_t operator()(const SplitCacheKey& k) const noexcept {
        ...
        h^=(uint64_t)k.ratioBucket*0x9e3779b97f4a7c15ull;
        return (size_t)h;
    }
};
...
RowSplitPlan cachedThroughputSplit(
    const WeightTensor& wt,const VulkanCompute& g0,const VulkanCompute& g1)
{
    // DEEP2_SPLIT_RATIO_CACHE_001: the plan cache is RATIO-AWARE. The
    // pre-fix cache keyed only on env-derived speeds, so the first call
    // for a tensor froze its split at whatever gAsyncRowRatio was at
    // that moment — the controller kept learning but NO cached plan
    // ever followed, and the measured GPU-time split stayed pinned at
    // its warmup value (R9700 12.2s vs 7800XT 19.7s across the whole
    // 5-run series). Keying on the CURRENT ratio permille (16-permille
    // buckets: ~1.6% ratio granularity) means a converged controller
    // re-derives each tensor's plan once per bucket transition, and
    // steady state is a pure cache hit.
    const long double s0=envRowThroughput("DEEP2_ROW_GPU0_THROUGHPUT_WEIGHT","DEEP2_GPU0_THROUGHPUT_WEIGHT");
    const long double s1=envRowThroughput("DEEP2_ROW_GPU1_THROUGHPUT_WEIGHT","DEEP2_GPU1_THROUGHPUT_WEIGHT");
    ...
    if(asyncSplitControl()&&gAsyncSamples.load(std::memory_order_relaxed)>0)
        r=gAsyncRowRatio.load(std::memory_order_relaxed);
    const uint32_t rBucket=
        (uint32_t)std::llround(r*1000.0/32.0); // 32-permille buckets (3.2%)
    const SplitCacheKey key{
        wt.data,(uint32_t)wt.rows,(uint32_t)wt.cols,wt.type,m0,m1,rBucket};
    static std::mutex mu;
...
RowSplitPlan chooseThroughputSplit(
    uint32_t rows,const VulkanCompute& g0,const VulkanCompute& g1) noexcept
{
    // Frozen-split gate: DEEP2_SPLIT_RATIO_CACHE_001 v2. The measured
    // window must NEVER re-split — each re-split changes every slice's
    // geometry, which changes RCU residency keys, which sends every GEMV
    // through the cold-row race (whole-tensor target + range uploads)
    // and collapses TPS ~17x (measured 0.297 vs 5.18). Convergence is a
    // WARMUP-ONLY activity: the controller takes large steps until it
    // settles, the freeze fires on EMA MOVEMENT stability (window
    // movement < 0.008 — tolerant of per-call measurement noise), and
    // from then on the frozen ratio serves every split with stable
    // geometry: zero re-derivation, residency stays hot.
    static std::atomic<uint64_t> frozenSamples{0};
    static std::atomic<double> frozenRatio{-1.0};
    static std::atomic<double> windowStartRatio{0.0};
    static std::atomic<uint64_t> windowStartSamples{0};
    static std::atomic<uint64_t> freezeWindow{0};
    uint64_t windowSz=freezeWindow.load(std::memory_order_relaxed);
    if(windowSz==0) {
        const char* e=std::getenv("DEEP2_SPLIT_FREEZE_WINDOW");
        windowSz=e&&*e?std::strtoul(e,nullptr,10):128;
        if(windowSz==0) windowSz=128;
        freezeWindow.store(windowSz,std::memory_order_relaxed);
    }
    ...
    if(freezeEnabled&&asyncSplitControl()&&samples>0) {
        if(frozenSamples.load(std::memory_order_relaxed)==0) {
            ...
            if(windowStartSamples.load(std::memory_order_relaxed)==0) {
                windowStartSamples.store(samples,
                                         std::memory_order_relaxed);
                windowStartRatio.store(cur,
                                      std::memory_order_relaxed);
            } else if(samples-
                     windowStartSamples.load(std::memory_order_relaxed)
                     >=windowSz) {
                const double moved=std::fabs(cur-
                    windowStartRatio.load(std::memory_order_relaxed));
                if(moved<0.008) {
                    // EMA settled: freeze the converged ratio.
                    frozenSamples.store(samples,
                                        std::memory_order_relaxed);
                    frozenRatio.store(cur,
                                      std::memory_order_relaxed);
                    gSplitFrozen.store(1,
                                       std::memory_order_relaxed);
                    std::fprintf(stderr,
                        "[SPLIT_FROZEN] ratio=%.4f samples=%llu\n",
                        cur,
                        static_cast<unsigned long long>(samples));
                    std::fflush(stderr);
                }
                // Roll the stability window.
                windowStartSamples.store(samples,
                                         std::memory_order_relaxed);
                windowStartRatio.store(cur,
                                      std::memory_order_relaxed);
    ...
    if(asyncSplitControl()&&gAsyncSamples.load(std::memory_order_relaxed)>0&&
       gSplitFrozen.load(std::memory_order_relaxed)==0) {
