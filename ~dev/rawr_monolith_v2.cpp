...
// Braid transient weight system
#include "rawrxd/src/rawr_braid_integration.hpp"
...
struct MMapFile {
    ...
    bool open(const char* path) {
        ...
        size_t lock_len = len < (size_t)256*1024*1024 ? len : (size_t)256*1024*1024;
        lock_memory(addr, lock_len); // Lock first 256MB
#else
        ...
        #endif
        size_t lock_len2 = len < (size_t)256*1024*1024 ? len : (size_t)256*1024*1024;
        lock_memory(addr, lock_len2);
#endif
        return true;
    }
...
struct GGUFModel {
    ...
    // GGUF type sizes — full table for native packed support.
    // FP32-only applies to computation surfaces with no sealed packed kernel.
    ...
    // Optional validation: true if all tensors are F32.
    bool is_fp32_only() const {
    ...
    // Generic raw-byte accessor for packed quantized tensors.
    const void* get_tensor_data(const std::string& name) const {
        auto* t = get_tensor(name);
        if (t && t->data_ptr) return t->data_ptr;
        return nullptr;
    }
...
// Must be included outside namespace to avoid std::queue resolution issues
}} // close namespace rawrxd::monolith
#include "rawr_gemm_avx512.h"
namespace rawrxd { namespace monolith {
...
// Braid-aware matmul: resolves execution weight on demand
static inline std::vector<float> matmul_braid(
    const float* w,
    const std::vector<float>& x,
    int rows, int cols,
    rawrxd::braid::TensorRole role
) {
    using namespace rawrxd::braid;
    BraidInstrumentation& inst = GetBraidInstrumentation();

    // Count the call
    inst.matmulCalls.fetch_add(1);
    inst.roleCalls[static_cast<int>(role)].fetch_add(1);

    // For small matrices or when braid is not beneficial, use direct GEMM
    if (rows * cols < 4096) {
        inst.fallbackCalls.fetch_add(1);
        return rawrxd::gemm::matmul_avx512(w, x, rows, cols);
    }

    // Classify and potentially use transient quantized path
    WeightFormat fmt = ClassifyBlock(w, std::min((uint32_t)cols, (uint32_t)BRAID_BLOCK_WEIGHTS));

    // Role-based policy enforcement
    uint8_t minBits = MinimumBitsForRole(role);
    if (FormatBits(fmt) < minBits) {
        fmt = (minBits >= 4) ? WF_Q4 : (minBits >= 3) ? WF_Q3 : WF_T3;
    }

    // Count precision usage
    switch (fmt) {
        case WF_ZERO: inst.precisionCalls[0].fetch_add(1); break;
        case WF_B1:   inst.precisionCalls[1].fetch_add(1); break;
        case WF_T3:   inst.precisionCalls[2].fetch_add(1); break;
        case WF_Q3:   inst.precisionCalls[3].fetch_add(1); break;
        case WF_Q4:   inst.precisionCalls[4].fetch_add(1); break;
        case WF_RAW:  inst.precisionCalls[5].fetch_add(1); break;
        default:      inst.precisionCalls[5].fetch_add(1); break;
    }

    // Simulate transient bytes tracking (would be actual packed size with real kernels)
    uint32_t packedBytes = FormatBlockBytes(fmt);
    uint64_t prev = inst.transientBytesCurrent.fetch_add(packedBytes);
    uint64_t current = prev + packedBytes;
    uint64_t peak = inst.transientBytesPeak.load();
    while (current > peak && !inst.transientBytesPeak.compare_exchange_weak(peak, current)) {}

    // Execute via standard GEMM (packed kernels would go here)
    auto result = rawrxd::gemm::matmul_avx512(w, x, rows, cols);

    // Decrement transient bytes (simulating discard after use)
    inst.transientBytesCurrent.fetch_sub(packedBytes);
...
struct Transformer {
    ...
    std::vector<float> forward_layer(const std::vector<float>& x, int layer, int pos,
        ...
        // QKV projections with braid-aware execution weights
        std::vector<float> q, k, v;
        if (w_q[layer]) q = matmul_braid(w_q[layer], normed, dim, dim, rawrxd::braid::TensorRole::ATTN_Q);
        else q = normed;
        if (w_k[layer]) k = matmul_braid(w_k[layer], normed, dim, dim, rawrxd::braid::TensorRole::ATTN_K);
        else k = normed;
        if (w_v[layer]) v = matmul_braid(w_v[layer], normed, dim, dim, rawrxd::braid::TensorRole::ATTN_V);
        else v = normed;
        ...
        // Output projection with braid-aware execution weight
        std::vector<float> attn_proj(dim, 0);
        if (w_o[layer]) attn_proj = matmul_braid(w_o[layer], attn_out, dim, dim, rawrxd::braid::TensorRole::ATTN_O);
        ...
        // SwiGLU FFN with braid-aware execution weights
        std::vector<float> gate(dim), up(dim);
        if (w_ffn_gate[layer]) gate = matmul_braid(w_ffn_gate[layer], ffn_normed, dim, dim, rawrxd::braid::TensorRole::FFN_GATE);
        else gate = ffn_normed;
        if (w_ffn_up[layer]) up = matmul_braid(w_ffn_up[layer], ffn_normed, dim, dim, rawrxd::braid::TensorRole::FFN_UP);
        else up = ffn_normed;
        ...
        // Down projection with braid-aware execution weight
        std::vector<float> ffn_out(dim, 0);
        if (w_ffn_down[layer]) ffn_out = matmul_braid(w_ffn_down[layer], gate, dim, dim, rawrxd::braid::TensorRole::FFN_DOWN);
        else ffn_out = gate;
    ...
    std::vector<float> logits(const std::vector<float>& hidden) {
        if (w_output) {
            return matmul_braid(w_output, hidden, model->n_vocab, dim, rawrxd::braid::TensorRole::OUTPUT);
        }
        // Fallback
...
struct Engine {
    ...
    // Braid transient weight runtime (fixed-size, model-size-independent)
    rawrxd::braid::BraidRuntime braid;
    ...
    void run_inference(const std::string& prompt, int max_tokens = 50) {
        ...
        if (scheduler.fetch(req)) {
            ...
            // BRAID_LIVE_FORWARD_001: Dump telemetry
            rawrxd::braid::GetBraidInstrumentation().Dump();
