# Production Integration Complete - PowerShell Script
...
foreach ($compiler in $compilers) {
    $exePath = Join-Path $compilerDir $compiler
    Write-Host "  Testing $compiler..." -ForegroundColor Gray
    
    $output = & $exePath 2>&1
    $outputString = $output -join " "
    
    if ($outputString -like "*[TEST] PASS*") {
        "[PASS] $compiler" | Add-Content -Path $testOutput
        $passCount++
    } else {
        "[FAIL] $compiler" | Add-Content -Path $testOutput
    }
}
...
Get-Content -Path $testOutput | ForEach-Object { Write-Host "  $_" -ForegroundColor Gray }
