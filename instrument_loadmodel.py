...
# Verify the function signature we expect
if 'bool Deep2Engine::loadModel(const std::string& ggufPath)' not in text:
    raise RuntimeError('loadModel signature not found or already modified')
...
replacements = [
    ...
    (r'''        std::fprintf\(stderr, "\[Deep2Engine\] GGUF load failed: %s\\n",
                     loader->error\(\)\.c_str\(\)\);
        return false;''',
    ...
    (r'''        std::fprintf\(stderr, "\[Deep2Engine\] GGUF missing general\.architecture\\n"\);
        return false;''',
    ...
    (r'''        std::fprintf\(stderr, "\[Deep2Engine\] GGUF missing token embedding tensor\\n"\);
        return false;''',
    ...
    (r'''        std::fprintf\(stderr, "\[Deep2Engine\] embedding geometry mismatch\\n"\);
        return false;''',
    ...
    (r'''        std::fprintf\(stderr, "\[Deep2Engine\] invalid transformer head/layer geometry\\n"\);
        return false;''',
    ...
    (r'''        std::fprintf\(stderr, "\[Deep2Engine\] GGUF missing layer-norm epsilon\\n"\);
        return false;''',
    ...
    (r'''        std::fprintf\(stderr, "\[Deep2Engine\] final norm / LM-head topology invalid\\n"\);
        return false;''',
...
# Apply plain-text replacements first
for old, new in replacements:
    if old not in text:
        # try regex
        m = re.search(old, text)
        if not m:
...
# Now handle the per-layer fprintf blocks with regex (they contain %zu variables)
layer_patterns = [
...
for pat, repl in layer_patterns:
    m = re.search(pat, text)
    if not m:
        raise RuntimeError(f'Layer pattern not found: {pat[:100]}...')
    text = text[:m.start()] + repl + text[m.end():]

# Remaining non-layer fprintf blocks
remaining = [
...
for pat, repl in remaining:
    m = re.search(pat, text)
    if not m:
        raise RuntimeError(f'Remaining pattern not found: {pat[:100]}...')
    text = text[:m.start()] + repl + text[m.end():]

# Update function signature
if 'bool Deep2Engine::loadModel(const std::string& ggufPath) {' in text:
    text = text.replace(
    ...
    raise RuntimeError('loadModel signature replacement failed')

with open(path, 'w', encoding='utf-8') as f:
    f.write(text)

print('Instrumentation complete. Stages 1-28 added.')
