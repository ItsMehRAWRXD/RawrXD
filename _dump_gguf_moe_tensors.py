...
with open(path, 'rb') as f:
    ...
    for i in range(n_tensors):
        ...
        f.seek(4, 1)  # type
        f.seek(8, 1)  # offset
        namelen = struct.unpack('<Q', f.read(8))[0]
        name = f.read(namelen).decode('utf-8', errors='replace')
        if 'expert' in name.lower() or 'moe' in name.lower() or 'gate' in name.lower() or 'ffn' in name.lower():
            print(name)
