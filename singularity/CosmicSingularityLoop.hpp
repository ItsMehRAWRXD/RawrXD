...
public:
    ...
    void RegisterSingularityCallback(std::function<void(const std::string&)> callback);
    void TriggerSingularityEvent(const std::string& eventType);
...
private:
    ...
    std::vector<std::function<void(const std::string&)>> m_singularityCallbacks;
