...
${Script:BlacklistedNames} = @(
    ...
    "RawrXD_P2PRelay.obj",
    "RawrXD_CLI_Titan.obj",
    "RawrXD_HttpChatServer.obj",
    "lsp_jsonrpc.obj",
    "mmap_loader.obj",
    "dequant_simd.obj",
    "kv_cache_mgr.obj",
    "genesis.obj",
    "RawrXD_LSP_Handshake_Ext.obj",
    "RawrXD_System_Primitives.obj",
    "Titan_Streaming_Orchestrator_Fixed.obj",
    "gpu_cuda_impl.obj",
    "masm_RawrXD_HttpChatServer.obj",
    "thermal_masm_ghost_paging_v2.obj",
    "thermal_masm_nvme_oracle_service.obj",
    "thermal_masm_nvme_service.obj",
...
# ==========================================================
# .pdata Section Validator (catches LNK1223 pre-link)
# ==========================================================
# Uses dumpbin /headers to detect corrupt .pdata contributions.
# Objects with malformed .pdata cause fatal error LNK1223 at
# link time. This catches them BEFORE linking.
function Test-ValidPdata {
    param([string]$Path)
    ...
    if (-not $dumpbin) { return $true }  # Can't check, assume OK
    
    try {
        $output = & $dumpbin /headers $Path 2>&1 | Out-String
        # If dumpbin itself reports corruption, the object is bad
        if ($output -match 'corrupt|invalid|error') {
            return $false
        }
        return $true
    } catch {
