// ============================================================================
// [SOURCE] win32app\OSExplorerInterceptor.cpp
// FILE: D:\rawrxd\src\win32app\OSExplorerInterceptor.cpp
// ============================================================================

// [CONSOLIDATED] #include "OSShellIntegration.h"
...
// [SYSINCLUDE] #include <memory>
// [SYSINCLUDE] #include <mutex>
// [SYSINCLUDE] #include <atomic>
// [SYSINCLUDE] #include <thread>
// [SYSINCLUDE] #include <sstream>
// [SYSINCLUDE] #include <iomanip>

// Global interceptor instance
std::unique_ptr<OSShellIntegration> g_osInterceptor = nullptr;

// Constructor
OSShellIntegration::OSShellIntegration() 
...
bool OSShellIntegration::Initialize(DWORD targetPID, OSInterceptorCallback callback) {
    LOG_FUNCTION();
    ...
    m_interceptor = (OSIntegrationCtx*)VirtualAlloc(NULL, sizeof(OSIntegrationCtx), 
    ...
    m_interceptor->magic = OS_INTEGRATION_MAGIC;
    ...
    // Allocate hook table
    m_interceptor->hookTable = VirtualAlloc(NULL, sizeof(OSCallbackTable), 
                                            MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
...
// Start interception
bool OSShellIntegration::StartInterception() {
    LOG_FUNCTION();
    ...
    m_stopLogThread = false;
    m_logThread = std::make_unique<std::thread>(&OSShellIntegration::ProcessLogQueue, this);
...
// Stop interception
bool OSShellIntegration::StopInterception() {
    LOG_FUNCTION();
...
// Get statistics
InterceptionStats OSShellIntegration::GetStatistics() {
...
// Process log queue
void OSShellIntegration::ProcessLogQueue() {
    LOG_FUNCTION();
...
// Hook OS APIs
bool OSShellIntegration::RegisterOSCallbacks() {
    LOG_FUNCTION();
    ...
    OSCallbackTable* hookTable = (OSCallbackTable*)m_interceptor->hookTable;
    ...
    if (pCreateFileW) {
        hookTable->CreateFileW = SilentCallback(pCreateFileW, MyCreateFileWHook);
    ...
    if (pReadFile) {
        hookTable->ReadFile = SilentCallback(pReadFile, MyReadFileHook);
    ...
    if (pWriteFile) {
        hookTable->WriteFile = SilentCallback(pWriteFile, MyWriteFileHook);
    ...
    if (pRegOpenKeyExW) {
        hookTable->RegOpenKeyExW = SilentCallback(pRegOpenKeyExW, MyRegOpenKeyExWHook);
    ...
    if (pRegQueryValueExW) {
        hookTable->RegQueryValueExW = SilentCallback(pRegQueryValueExW, MyRegQueryValueExWHook);
    ...
    if (pWSAConnect) {
        hookTable->WSAConnect = SilentCallback(pWSAConnect, MyWSAConnectHook);
    ...
    if (pSend) {
        hookTable->send = SilentCallback(pSend, MySendHook);
        if (!hookTable->send) {
    ...
    if (pRecv) {
        hookTable->recv = SilentCallback(pRecv, MyRecvHook);
        if (!hookTable->recv) {
...
// Unhook OS APIs
bool OSShellIntegration::UnregisterOSCallbacks() {
    LOG_FUNCTION();
    ...
    // Implementation depends on SilentCallback mechanism

    return true;
}

// Log intercepted call
void OSShellIntegration::LogCall(CallLogEntry* entry) {
...
void OSShellIntegration::StreamCall(CallLogEntry* entry) {
    if (m_callback) {
        m_callback(entry);
    }
}
...
void* SilentCallback(void* targetFunction, void* hookFunction) {
...
bool InitOSExplorerInterceptor(DWORD targetPID, OSInterceptorCallback callback) {
    if (!g_osInterceptor) {
        g_osInterceptor = std::make_unique<OSShellIntegration>();
    }
...
void* LoadModuleManual(HANDLE hProcess, void* dllData, size_t dllSize) {
    ...
    void* pMapped = RXD_VirtualAllocEx(hProcess, NULL, ntHeaders->OptionalHeader.SizeOfImage,
                                   MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    if (!pMapped) {
    ...
    // Copy headers
    RXD_WriteProcessMemory(hProcess, pMapped, dllData, ntHeaders->OptionalHeader.SizeOfHeaders, NULL);
    
    // Copy sections
    IMAGE_SECTION_HEADER* sections = IMAGE_FIRST_SECTION(ntHeaders);
    for (int i = 0; i < ntHeaders->FileHeader.NumberOfSections; i++) {
        RXD_WriteProcessMemory(hProcess,
    ...
    // Fix relocations
    ApplyRelocations(hProcess, pMapped, dllData);
    
    // Resolve imports
    BindImports(hProcess, pMapped, dllData);
    
    // Change protection to RX
    DWORD oldProtect;
    RXD_VirtualProtectEx(hProcess, pMapped, ntHeaders->OptionalHeader.SizeOfImage,
    ...
    while (relocationSize > 0 && relocation->VirtualAddress) {
        ...
        for (int i = 0; i < numRelocs; i++) {
            ...
            if (type == IMAGE_REL_BASED_DIR64) {
                ...
                ULONGLONG value;
                RXD_ReadProcessMemory(hProcess, address, &value, sizeof(value), NULL);
                value += (ULONGLONG)mappedBase - ntHeaders->OptionalHeader.ImageBase;
                RXD_WriteProcessMemory(hProcess, address, &value, sizeof(value), NULL);
            }
        }
