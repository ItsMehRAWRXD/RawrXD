...
<<<<<<< HEAD
                    ...
                    int textLen = GetWindowTextLengthW(g_Tabs[i].hEdit);
                    if (textLen > 0) {
                        wchar_t* buf = (wchar_t*)malloc((textLen + 1) * sizeof(wchar_t));
                        if (buf) {
                            GetWindowTextW(g_Tabs[i].hEdit, buf, textLen + 1);
                            ...
                            if (utf8Buf) {
                                ...
                                HANDLE hFile = CreateFileW(backupPath, GENERIC_WRITE, 0, NULL,
...
static void OnTreeViewItemClick(HWND hTree, NMTREEVIEW* pnm) {
    ...
    cf.yHeight = 220;  // 11pt
    SendMessageW(g_Tabs[idx].hEdit, EM_SETCHARFORMAT, SCF_ALL, (LPARAM)&cf);                // Convert content to wide
...
static void NewFile(void) {
    ...
    cf2.dwMask = CFM_COLOR | CFM_FACE | CFM_SIZE;
    cf2.crTextColor = RGB(212, 212, 212);
    wcscpy(cf2.szFaceName, L"Consolas");
    cf2.yHeight = 220;
    SendMessageW(g_Tabs[idx].hEdit, EM_SETCHARFORMAT, SCF_ALL, (LPARAM)&cf2);
...
static void OpenFile(void) {
    ...
    if (GetOpenFileNameW(&ofn)) {
        ...
        if (g_nTabCount < MAX_TABS) {
            ...
            CHARFORMAT2W cf3;
            ZeroMemory(&cf3, sizeof(cf3));
            cf3.cbSize = sizeof(cf3);
            cf3.dwMask = CFM_COLOR | CFM_FACE | CFM_SIZE;
            cf3.crTextColor = RGB(212, 212, 212);
            wcscpy(cf3.szFaceName, L"Consolas");
            cf3.yHeight = 220;
            SendMessageW(g_Tabs[idx].hEdit, EM_SETCHARFORMAT, SCF_ALL, (LPARAM)&cf3);
...
static void CreateNewTerminal(void) {
    ...
    CHARFORMAT2W cfTerm;
    ZeroMemory(&cfTerm, sizeof(cfTerm));
    cfTerm.cbSize = sizeof(cfTerm);
    cfTerm.dwMask = CFM_COLOR | CFM_FACE | CFM_SIZE;
    cfTerm.crTextColor = RGB(0, 255, 0);
    wcscpy(cfTerm.szFaceName, L"Consolas");
    cfTerm.yHeight = 200;
    SendMessageW(g_Terminals[idx].hOutput, EM_SETCHARFORMAT, SCF_ALL, (LPARAM)&cfTerm);
...
static void CreateChatPanel(HWND hWnd) {
    ...
    CHARFORMAT2W cfChat;
    ZeroMemory(&cfChat, sizeof(cfChat));
    cfChat.cbSize = sizeof(cfChat);
    cfChat.dwMask = CFM_COLOR | CFM_FACE | CFM_SIZE;
    cfChat.crTextColor = RGB(212, 212, 212);
    wcscpy(cfChat.szFaceName, L"Segoe UI");
    cfChat.yHeight = 200;
    SendMessageW(g_hChatHistory, EM_SETCHARFORMAT, SCF_ALL, (LPARAM)&cfChat);
...
<<<<<<< HEAD
    ...
    int utf8Len = WideCharToMultiByte(CP_UTF8, 0, prompt, -1, NULL, 0, NULL, NULL);
    char* utf8Prompt = (char*)malloc(utf8Len);
    if (!utf8Prompt) {
        AddChatMessage(L"System", L"Memory allocation failed.");
        return;
    }
    WideCharToMultiByte(CP_UTF8, 0, prompt, -1, utf8Prompt, utf8Len, NULL, NULL);
    
    // Try inference engine DLL chain
    BOOL gotResponse = FALSE;
    
    // 1. Try RawrXD_InferenceEngine_Win32.dll (full async inference)
    typedef void* (__stdcall *CreateEngine_t)();
    typedef uint32_t (__stdcall *SubmitInference_t)(void*, const char*, size_t);
    typedef int (__stdcall *GetResult_t)(void*, uint32_t, char*, size_t);
    typedef int (__stdcall *IsLoaded_t)(void*);
    
    HMODULE hInfEngine = LoadLibraryW(L"RawrXD_InferenceEngine_Win32.dll");
    if (!hInfEngine) {
        ...
        if (slash) {
            wcscpy(slash + 1, L"RawrXD_InferenceEngine_Win32.dll");
            hInfEngine = LoadLibraryW(exePath);
        }
    }
    
    if (hInfEngine) {
        auto pfnCreate = (CreateEngine_t)GetProcAddress(hInfEngine, "CreateInferenceEngine");
        auto pfnSubmit = (SubmitInference_t)GetProcAddress(hInfEngine, "InferenceEngine_SubmitInference");
        auto pfnGet = (GetResult_t)GetProcAddress(hInfEngine, "InferenceEngine_GetResult");
        auto pfnLoaded = (IsLoaded_t)GetProcAddress(hInfEngine, "InferenceEngine_IsModelLoaded");
        
        if (pfnCreate && pfnSubmit && pfnGet) {
            void* engine = pfnCreate();
            if (engine && (!pfnLoaded || pfnLoaded(engine))) {
                uint32_t reqId = pfnSubmit(engine, utf8Prompt, 512);
                if (reqId > 0) {
                    // Poll for result with timeout
                    char resultBuf[8192] = {0};
                    for (int attempt = 0; attempt < 100; attempt++) {
                        Sleep(100);
                        if (pfnGet(engine, reqId, resultBuf, sizeof(resultBuf))) {
                            // Convert result to wide string
                            int wLen = MultiByteToWideChar(CP_UTF8, 0, resultBuf, -1, NULL, 0);
                            wchar_t* wResult = (wchar_t*)malloc(wLen * sizeof(wchar_t));
                            if (wResult) {
                                MultiByteToWideChar(CP_UTF8, 0, resultBuf, -1, wResult, wLen);
                                AddChatMessage(L"Assistant", wResult);
                                free(wResult);
                                gotResponse = TRUE;
                            }
                            break;
    ...
    // 2. Try Titan Kernel DLL
    if (!gotResponse && g_hTitanDLL) {
        typedef int (*RunInference_t)(int, const char*, int);
        RunInference_t pfnInfer = (RunInference_t)GetProcAddress(g_hTitanDLL, "Titan_RunInference");
        if (pfnInfer) {
            int result = pfnInfer(0, utf8Prompt, 512);
            if (result >= 0) {
                AddChatMessage(L"Assistant", L"[Response via Titan Kernel]");
                gotResponse = TRUE;
            }
        }
    }
    
    // 3. Try Native Model Bridge for forward pass
    if (!gotResponse && g_hBridgeDLL) {
        typedef int (*ForwardPass_t)(void*, int*, int);
        ForwardPass_t pfnForward = (ForwardPass_t)GetProcAddress(g_hBridgeDLL, "ForwardPass");
        if (pfnForward) {
            // Tokenize prompt as char codes
            int tokens[2048] = {0};
            int nTokens = 0;
            for (int i = 0; utf8Prompt[i] && nTokens < 2048; i++) {
                tokens[nTokens++] = (unsigned char)utf8Prompt[i];
            }
            int fwdResult = pfnForward(NULL, tokens, nTokens);
            if (fwdResult >= 0) {
                AddChatMessage(L"Assistant", L"[Forward pass completed via Native Bridge - wire sampling for text output]");
                gotResponse = TRUE;
            }
        }
    }
    ...
    if (!gotResponse) {
        AddChatMessage(L"System", L"No inference engine responded. Ensure a .gguf model is loaded and the inference DLL is available.");
    }
...
static void AppendRichText(HWND hRich, const wchar_t* text, COLORREF color) {
    ...
    // Set color
    CHARFORMAT2W cfAppend;
    ZeroMemory(&cfAppend, sizeof(cfAppend));
    cfAppend.cbSize = sizeof(cfAppend);
    cfAppend.dwMask = CFM_COLOR;
    cfAppend.crTextColor = color;
    SendMessageW(hRich, EM_SETCHARFORMAT, SCF_SELECTION, (LPARAM)&cfAppend);
