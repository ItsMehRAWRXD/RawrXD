...
$Script:shipDir ="D:\rawrxd\Ship"; $srcFile=Join-Path $shipDir "RawrXD_Win32_IDE.cpp"; $exeFile=Join-Path $shipDir "RawrXD_IDE.exe"
...
$Script:src =Get-Content $srcFile -Raw
# G2: WM_CREATE
Write-Host "`n[2] WM_CREATE 4-Pane" -ForegroundColor White
$Script:wmC =[regex]::Match($src,'case WM_CREATE:\s*\{(.+?)(?=case WM_SIZE:)',[System.Text.RegularExpressions.RegexOptions]::Singleline).Groups[1].Value
...
$Script:ci1 =$wmC.IndexOf('g_hwndChatInput = CreateWindowExW');$ci2=$wmC.IndexOf('SendMessageW(g_hwndChatInput')
...
$Script:wmS =[regex]::Match($src,'case WM_SIZE:\s*\{(.+?)break;\s*\}',[System.Text.RegularExpressions.RegexOptions]::Singleline).Groups[1].Value
...
$Script:wmCmd =[regex]::Match($src,'case WM_COMMAND:\s*\{(.+?)(?=case WM_USER)',[System.Text.RegularExpressions.RegexOptions]::Singleline).Groups[1].Value
...
$Script:chatC =[regex]::Match($wmCmd,'case IDM_VIEW_CHAT:(.+?)break;',[System.Text.RegularExpressions.RegexOptions]::Singleline).Groups[1].Value
...
$Script:gc =([regex]::Matches($wmCmd,'case IDM_AI_GENERATE:')).Count
