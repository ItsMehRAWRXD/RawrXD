...
std::wstring utf8ToWide(const std::string& s)
{
    ...
    MultiByteToWideChar(CP_UTF8, 0, s.c_str(), len, w.data(), n);
    return w;
}
}  // namespace
...
const char* PipelineStageName(PipelineStage stage)
{
    switch (stage)
    {
        case PipelineStage::IDE_UI:
            return "IDE_UI";
        case PipelineStage::ChatService:
            return "ChatService";
        case PipelineStage::PromptBuilder:
            return "PromptBuilder";
        case PipelineStage::LLM_API:
            return "LLM_API";
        case PipelineStage::TokenStream:
            return "TokenStream";
        case PipelineStage::Renderer:
            return "Renderer";
        default:
            return "Unknown";
    }
...
struct AutonomousAgenticPipelineCoordinator::Impl
{
    ...
    void logInfo(const std::string& msg) { log(L"INFO:", msg); }
    void logWarn(const std::string& msg) { log(L"WARN:", msg); }
    void logError(const std::string& msg) { log(L"ERROR:", msg); }
    ...
    void autonomousLoopBody()
    {
        while (autonomousRunning.load(std::memory_order_relaxed))
        {
            ...
            if (!taskDesc.empty())
            {
                const std::string msg = wideToUtf8(taskDesc);
...
AutonomousAgenticPipelineCoordinator::AutonomousAgenticPipelineCoordinator() : m_impl(std::make_unique<Impl>())
{
...
PipelineResult<std::string> AutonomousAgenticPipelineCoordinator::Impl::runPipelineInternal(
    const std::string& userMessage)
{
    ...
    {
        std::string streamed;
        bool ok = routeLLMStreaming(prompt,
                                    [this, &streamed](const std::string& token, bool isFinal)
                                    {
                                        streamed += token;
                                        if (onToken)
...
void AutonomousAgenticPipelineCoordinator::startAutonomousLoop()
{
    ...
    m_impl->autonomousThread = std::thread([this]() { m_impl->autonomousLoopBody(); });
    m_impl->logInfo("Autonomous loop started");
}

void AutonomousAgenticPipelineCoordinator::stopAutonomousLoop()
{
    if (!m_impl->autonomousRunning.exchange(false))
        return;
...
int AutonomousAgenticPipelineCoordinator::submitAgentTask(const std::wstring& description, int priority)
{
    ...
    m_impl->logInfo("Task submitted: " + wideToUtf8(description));
    return ++taskId;
}
...
void AutonomousAgenticPipelineCoordinator::Impl::reportPipelineFailureInternal(PipelineStage stage,
                                                                               ...
                                                                               const std::string& failedMessage)
{
    ...
    lastFailedMessage = utf8ToWide(failedMessage);
}

void AutonomousAgenticPipelineCoordinator::triggerAutoFixCycle()
{
    const std::string msg = wideToUtf8(m_impl->lastFailedMessage);
...
void AutonomousAgenticPipelineCoordinator::Impl::triggerAutoFixCycleInternal(const std::string& failedMessage)
{
    ...
    if (lastFailedRetryCount < static_cast<int>(config.maxAutoFixRetries))
    {
        ...
        logInfo("AUTO-FIX: re-submitted last failed task (retry " + std::to_string(lastFailedRetryCount) + "/" +
                std::to_string(config.maxAutoFixRetries) + ")");
    }
    else
    {
...
extern "C"
{
    ...
    void RawrXD_AutonomousPipeline_StopAutonomous(RawrXD_PipelineCoordinatorHandle h)
    {
        auto* p = static_cast<RawrXD::AutonomousAgenticPipelineCoordinator*>(h);
        if (p)
            p->stopAutonomousLoop();
    }

    int RawrXD_AutonomousPipeline_RunPipeline(RawrXD_PipelineCoordinatorHandle h, const char* userMessage,
                                              char* outBuffer, int outBufferSize)
    {
        if (!h || !userMessage || !outBuffer || outBufferSize <= 0)
            return -1;
        auto* p = static_cast<RawrXD::AutonomousAgenticPipelineCoordinator*>(h);
        ...
        if (len >= (size_t)outBufferSize)
            len = (size_t)outBufferSize - 1;
