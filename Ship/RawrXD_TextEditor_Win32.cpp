...
public:
    ...
    void InsertText(const char* text, size_t length) {
        ...
        std::string before = line.text.substr(0, m_currentCol);
        std::string after = line.text.substr(m_currentCol);
        line.text = before + std::string(text, length) + after;
        m_currentCol += length;
    ...
    void DeleteChar(size_t count = 1) {
        ...
        if (m_currentLine < m_lines.size()) {
            LineInfo& line = m_lines[m_currentLine];
            if (m_currentCol > 0) {
                size_t delCount = count < m_currentCol ? count : m_currentCol;
                std::string before = line.text.substr(0, m_currentCol - delCount);
                std::string after = line.text.substr(m_currentCol);
    ...
    void GetText(char* buffer, size_t bufSize) const {
        ...
        size_t copyLen = bufSize - 1 > result.size() ? result.size() : bufSize - 1;
    ...
    bool GetLine(size_t lineNum, char* buffer, size_t bufSize) const {
        ...
        size_t copyLen = bufSize - 1 > line.text.size() ? line.text.size() : bufSize - 1;
    ...
    void SetCursorPosition(size_t line, size_t col) {
        ...
        if (line < m_lines.size()) {
            m_currentLine = line;
            size_t maxCol = m_lines[line].text.length();
            m_currentCol = col < maxCol ? col : maxCol;
        }
