...
masm_byte_patch_find_pattern PROC
    ...
    push r14
    push r15
    sub rsp, 4096           ; Stack buffer for file reading
    ...
    ; Read file into buffer (simplified: assume file < 4KB for now)
    ; In production, use chunked reading
    mov rcx, [rbx]          ; file handle
    lea rdx, [rsp + 32]     ; buffer
    mov r8d, 4000           ; bytes to read
    ...
    ; Boyer-Moore search (simplified naive search for now)
...
find_loop:
    ...
    xor r9, r9              ; r9 = pattern offset
    
find_compare:
    cmp r9, r13
    jge find_match_found
    
    mov al, [r12 + r9]      ; pattern byte
    lea r10, [r14 + r9]
    mov cl, [r8 + r10]      ; file byte
    
    cmp al, cl
    jne find_next_position
...
apply_rotate:
    ; Circular bit rotation (simplified: rotate left by 1 bit)
    mov rcx, [rbx]
    lea rdx, [rsp + 32]
    mov r8d, [rbx + 40]
    lea r9, [rsp + 16]
