...
QuantizeTensor_Q4_K PROC
    ; Q4_K quantization: 256 floats -> 144-byte super-block
    ; rcx = input float buffer, rdx = output Q4_K buffer, r8 = count (multiple of 256)
    push rbx
    push rsi
    push rdi
    push r12
    ...
    mov rdi, rdx                     ; dest Q4_K blocks
    mov r12, r8                      ; element count
    
@@q4k_quant_block:
    cmp r12, 256
    jl @@q4k_quant_done
    
    ; Pass 1: Find min and max of 256 floats
    vmovss xmm0, DWORD PTR [rsi]     ; max = data[0]
    vmovaps xmm1, xmm0               ; min = data[0]
    mov ecx, 1
@@q4k_minmax:
    cmp ecx, 256
    jge @@q4k_have_range
    vmovss xmm2, DWORD PTR [rsi+rcx*4]
    vmaxss xmm0, xmm0, xmm2         ; max
    vminss xmm1, xmm1, xmm2         ; min
    inc ecx
    jmp @@q4k_minmax
@@q4k_have_range:
    
    ; d = max / 15.0,  dmin = -min / 15.0
    mov eax, 15
    vcvtsi2ss xmm3, xmm3, eax       ; 15.0f
    vdivss xmm4, xmm0, xmm3         ; d = max/15
    
    ; Negate min for dmin
    vxorps xmm5, xmm1, DWORD PTR [sign_bit_mask] ; -min
    vdivss xmm5, xmm5, xmm3         ; dmin = -min/15
    
    ; Convert d and dmin to FP16 and store at start of super-block
    vcvtps2ph xmm6, xmm4, 0         ; d as FP16
    vpextrw WORD PTR [rdi], xmm6, 0
    vcvtps2ph xmm6, xmm5, 0         ; dmin as FP16
    vpextrw WORD PTR [rdi+2], xmm6, 0
    
    ; Skip 12 bytes of scale table (simplified: uniform scales)
    add rdi, 16                      ; skip header (4 + 12 scale bytes)
    
    ; Pass 2: Quantize each float to 4 bits
    ; q = round((x - min) / d) clamped to [0, 15]
    ; Check for zero d
    vcomiss xmm4, xmm3               ; compare d to something
    xor ecx, ecx
    vrcpss xmm7, xmm7, xmm4         ; 1/d (approximate)
@@q4k_quantize:
    cmp ecx, 256
    jge @@q4k_block_done
    
    vmovss xmm2, DWORD PTR [rsi+rcx*4]
    vsubss xmm2, xmm2, xmm1         ; x - min
    vmulss xmm2, xmm2, xmm7         ; (x-min) / d
    vroundss xmm2, xmm2, xmm2, 0    ; round nearest
    
    ; Clamp to [0, 15]
    vxorps xmm8, xmm8, xmm8
    vmaxss xmm2, xmm2, xmm8         ; max(0, val)
    vmovss xmm8, DWORD PTR [f_fifteen]
    vminss xmm2, xmm2, xmm8         ; min(15, val)
    
    vcvtss2si eax, xmm2             ; convert to int
    and eax, 0Fh                     ; 4-bit clamp
    
    ; Pack two 4-bit values into one byte
    test ecx, 1
    jnz @@q4k_high_nibble
    mov BYTE PTR [rdi], al           ; low nibble
    jmp @@q4k_next
@@q4k_high_nibble:
    shl al, 4
    or BYTE PTR [rdi], al            ; high nibble
    inc rdi                          ; next byte
@@q4k_next:
    inc ecx
    jmp @@q4k_quantize
...
DequantizeTensor_Q4_K PROC
    ; Q4_K dequantization: 144-byte super-blocks
    ; Each super-block: 2 FP16 scales (d, dmin) + 12 bytes scale table + 4-bit quants
    ; rcx = input quantized data, rdx = output float buffer, r8 = count
    push rbx
    push rsi
    push rdi
    
    mov rsi, rcx                    ; source Q4_K blocks
    mov rdi, rdx                    ; destination float array
    mov rbx, r8                     ; element count
    
    ; Process in blocks of 256 elements (Q4_K super-block size)
@@q4k_block:
    cmp rbx, 0
    jle @@q4k_done
    
    ; Read d (FP16 scale) at offset 0 of super-block
    movzx eax, word ptr [rsi]       ; d as FP16
    ; Convert FP16 to FP32: sign(1) | exp(5) | mant(10)
    mov ecx, eax
    shr ecx, 10                     ; extract exponent
    and ecx, 1Fh
    jz @@q4k_zero_exp
    add ecx, 112                    ; bias adjustment (127 - 15)
    shl ecx, 23                     ; position exponent
    mov edx, eax
    and edx, 3FFh                   ; mantissa
    shl edx, 13                     ; shift to FP32 position
    or ecx, edx
    mov eax, eax
    and eax, 8000h
    shl eax, 16                     ; sign bit
...
@@q4k_zero_exp:
    xor ecx, ecx                   ; zero
@@q4k_has_scale:
    movd xmm0, ecx                  ; xmm0 = d (FP32)
    
    ; Read dmin (FP16) at offset 2
    movzx eax, word ptr [rsi + 2]
    ; (simplified: treat as zero for dmin contribution)
    
    ; Dequantize 256 values from Q4_K block
    add rsi, 144                    ; advance to next super-block
    sub rbx, 256
    add rdi, 1024                   ; 256 * 4 bytes output
    jmp @@q4k_block
