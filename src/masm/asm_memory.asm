...
; Test harness console printer (for diagnostics)
EXTERN print_string:PROC
EXTERN g_console_handle:QWORD
...
asm_malloc PROC
    ...
    push rbx
    push r12
    sub rsp, 56             ; 32-byte shadow space + 24 for alignment (keeps RSP 16-byte aligned)
...
asm_free PROC
    ...
    push rbx
    push r12
    sub rsp, 40             ; align stack for WinAPI calls
...
asm_get_process_heap PROC
    ...
    sub rsp, 32             ; 32-byte shadow space for alignment
    call GetProcessHeap
    add rsp, 32
...
asm_heap_alloc PROC
    ...
    sub rsp, 40             ; ensure 16-byte alignment
    mov r8, rdx             ; r8 = size
    xor rdx, rdx            ; rdx = flags
    call HeapAlloc
    add rsp, 40
