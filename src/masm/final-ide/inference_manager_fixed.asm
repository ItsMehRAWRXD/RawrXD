...
inference_manager_init PROC PUBLIC
    ...
    call HeapAlloc
    test rax, rax
    jz inference_init_queue_oom_local
    ...
    ; Removed RIP-relative - using immediate 1.0  ; Load 1.0 double
    mov dword ptr [rbx + 68], 3f800000h  ; 1.0f      ; temperature = 1.0
    mov dword ptr [rbx + 72], 3f666666h  ; 0.9f      ; top_k_prob = 0.9 (simplified)
    
    mov byte ptr [rbx + 84], 0      ; model_loaded = false
    
    mov rax, rbx                ; Return INFERENCE_MANAGER*
    jmp inference_init_done_local
    
inference_init_queue_oom_local:
    mov rcx, rbx
    call HeapFree
    
inference_init_oom_local:
    xor rax, rax
    
inference_init_done_local:
    pop r12
    pop r13
    pop rbp
    ret
...
inference_manager_shutdown PROC PUBLIC
    ...
    test rbx, rbx
    jz inference_shutdown_invalid_local
    ...
    cmp byte ptr [rbx + 84], 1
    jne inference_shutdown_no_model_local
    
    mov rcx, [rbx + 0]
    call HeapFree               ; Free model_handle
    
inference_shutdown_no_model_local:
    ; Free KV cache
    cmp qword ptr [rbx + 24], 0
    je inference_shutdown_no_kv_local
    
    mov rcx, [rbx + 24]
    call HeapFree
    
inference_shutdown_no_kv_local:
    ; Free embedding cache
    cmp qword ptr [rbx + 32], 0
    je inference_shutdown_no_embed_local
    
    mov rcx, [rbx + 32]
    call HeapFree
    
inference_shutdown_no_embed_local:
    ; Free batch queue
    cmp qword ptr [rbx + 40], 0
    je inference_shutdown_no_queue_local
    
    mov rcx, [rbx + 40]
    call HeapFree
    
inference_shutdown_no_queue_local:
    ...
    xor rax, rax
    jmp inference_shutdown_done_local
    
inference_shutdown_invalid_local:
    mov rax, 1
    
inference_shutdown_done_local:
    pop rbx
    pop rbp
    ret
...
load_model PROC PUBLIC
    ...
    cmp byte ptr [rbx + 84], 1
    je load_model_already_loaded_local
    
    ; Allocate model path string buffer
    mov rcx, 512
    call HeapAlloc
    test rax, rax
    jz load_model_oom_local
    ...
    xor rax, rax
    jmp load_model_done_local
    
load_model_already_loaded_local:
    mov rcx, [rbx + 76]
    call ReleaseMutex
    mov rax, 1
    jmp load_model_done_local
    
load_model_kv_fail_local:
    mov rcx, [rbx + 8]
    call HeapFree
    mov rcx, [rbx + 76]
    call ReleaseMutex
...
load_model_oom_local:
    mov rcx, [rbx + 76]
    call ReleaseMutex
    mov rax, 2
    
load_model_done_local:
    pop rbx
    pop r12
    pop rbp
    ret
...
unload_model PROC PUBLIC
    ...
    je unload_model_not_loaded_local
    
    ; Free model handle
    cmp qword ptr [rbx + 0], 0
    je unload_model_no_handle_local
    
    mov rcx, [rbx + 0]
    call HeapFree
    
unload_model_no_handle_local:
    ; Reset flag
    mov byte ptr [rbx + 84], 0
    
    xor rax, rax
    jmp unload_model_release_local
    
unload_model_not_loaded_local:
    mov rax, 1
    
unload_model_release_local:
    mov rcx, [rbx + 76]
    call ReleaseMutex
