...
hpatch_reset_stats PROC
    ...
    ; Get global manager
    mov rbx, [g_unified_manager_ptr]
    test rbx, rbx
    jz reset_done
    
    ; Lock manager
    mov rcx, [rbx + 48]     ; mutex_handle
    call asm_mutex_lock
    ...
    mov qword ptr [rbx + 80], 0  ; successful_operations
    mov qword ptr [rbx + 88], 0  ; failed_operations
    mov qword ptr [rbx + 16], 0  ; memory_patch_count
    mov qword ptr [rbx + 32], 0  ; byte_patch_count
    mov qword ptr [rbx + 40], 0  ; server_hotpatch_count
    
    ; Reset global metrics
    mov qword ptr [g_patch_count], 0
    mov qword ptr [g_patches_successful], 0
    mov qword ptr [g_patches_failed], 0
    mov qword ptr [g_total_latency], 0
    mov qword ptr [g_max_latency], 0
    mov qword ptr [g_min_latency], 0FFFFFFFFFFFFFFFFh  ; Set to max value
    mov qword ptr [g_operation_start_time], 0
    mov qword ptr [g_last_error_time], 0
