...
TEXT_EDITOR STRUCT
    ...
    visible_lines   DWORD ?         ; Lines visible in viewport
    char_width      DWORD ?         ; Character width (pixels, monospace font)
...
FLAG_HAS_SELECTION  EQU 0x0004
FLAG_READONLY       EQU 0x0008

; Text colors (RGB)
COLOR_TEXT          EQU 0x000000    ; Black
COLOR_BACKGROUND    EQU 0xFFFFFF    ; White
COLOR_SELECTION     EQU 0xC0C0C0    ; Light gray
COLOR_KEYWORD       EQU 0x0000FF    ; Blue
COLOR_STRING        EQU 0x008000    ; Green
COLOR_COMMENT       EQU 0x808080    ; Dark gray
...
g_editor_global     QWORD 0         ; Global editor instance
g_clipboard_data    QWORD 0         ; Clipboard text pointer
g_clipboard_size    DWORD 0         ; Clipboard size
...
text_editor_create PROC
    ...
    call malloc_asm                     ; rax = editor ptr
    test rax, rax
    jz .error
    ...
    mov qword ptr [rcx + OBJECT_BASE.vmt], rax  ; vmt = &text_editor_vmt
    mov qword ptr [rcx + OBJECT_BASE.hwnd], 0   ; hwnd = NULL (will set later)
    ...
    mov rcx, 512
    call malloc_asm                    ; rax = path buffer
...
text_editor_load_file PROC
    ...
    mov rcx, rdi
    mov rdx, 512
    call copy_string_asm               ; Copy path to buffer
    ...
    call GetFileSizeEx                 ; Get file size
    test eax, eax
    jz .close_file
    ...
    mov rcx, r13                       ; rcx = file size
    add rcx, 1                         ; Add 1 for null terminator
    call malloc_asm                    ; rax = file buffer
    test rax, rax
    jz .close_file
    ...
    mov r8, r13d                       ; nNumberOfBytesToRead (lower 32-bit)
...
.create_line:
    ...
    ; Copy line text
    mov rsi, r14
    add rsi, r15                       ; rsi = source
    mov rdi, rax                       ; rdi = destination
    mov rcx, rdx                       ; rcx = length
    call copy_memory_asm               ; Copy rdx bytes
...
.write_lines:
    ...
    mov byte ptr [rsp + 50], 0x0D      ; CR
    mov byte ptr [rsp + 51], 0x0A      ; LF
...
text_editor_insert_text PROC
    ...
    ; Find the cursor line in linked list
...
.found_line:
    ...
    ; r11d = cursor_col (position in line)
    ; r12 = text to insert
    ; r13d = text length
    ...
    ; Check for newlines in inserted text
...
.no_newline:
    ; Simple case: no newlines, just insert text in current line
    ...
    call malloc_asm                    ; rax = new buffer
    test rax, rax
    jz .error
    
    mov rcx, [r14 + TEXT_LINE.text_ptr]
    mov rdx, [r14 + TEXT_LINE.text_len]
    mov r8, rax
    call copy_memory_asm               ; Copy old text
...
.bounds_ok:
    ...
    mov rdx, r14
    add rdx, r8d
    add rdx, r13d                      ; rdx = source (cursor_col + count)
    mov rcx, r14
    add rcx, r8d                       ; rcx = destination (cursor_col)
    
    mov r12d, r11d
    sub r12d, r8d
    sub r12d, r13d                     ; r12d = bytes to move
    
    ; Use memmove_asm to shift left
    call memmove_asm                   ; memmove(dest, src, count)
...
.copy_lines:
    ...
    ; copy_memory_asm(dest=rdx, src=rcx, size=r8)
    call copy_memory_asm
    ...
    ; Copy CRLF
    mov byte ptr [r14 + r15], 0x0D
    mov byte ptr [r14 + r15 + 1], 0x0A
    add r15d, 2
...
.update_cursor:
    ...
    shl r8d, 3                         ; r8d = col * 8 (char width)
...
.alloc_global:
    ...
    mov r8, rdx                        ; r8 = size
    ; GlobalAlloc(GMEM_MOVEABLE, size)
    ; Note: GlobalAlloc is typically called via LocalAlloc in recent Win32
    ...
    mov rcx, r13                       ; dest
    mov rdx, r12                       ; src
    mov r8, r10                        ; size (r10 still points past null)
    sub r8, r12
    call copy_memory_asm
...
.do_insert:
    ...
    ; Restore editor ptr to rcx (was passed in)
...
text_editor_paint PROC
    ...
    mov r13d, [rsp + 8]                ; r13d = rect.right (width)
...
.draw_lines:
    ...
    mov qword ptr [rsp + 32], [r10 + TEXT_LINE.text_len]  ; text length
...
; Draw line number in left margin
...
; =============== Stub malloc_asm ===============
; Simple allocator stub (will be linked with foundation)
malloc_asm PROC
    ; rcx = size
    ; TODO: Call HeapAlloc or similar
    mov rax, rcx
    ret
malloc_asm ENDP
