...
EXTERN g_hMainWnd:QWORD
EXTERN g_animation_active:DWORD
...
; JSON helper functions (implemented locally)
; These functions are implemented at the end of this file
...
gui_toggle_pane_visibility PROC
    ...
    mov eax, [rsi + PANE.visible]
    xor eax, 1
    mov [rsi + PANE.visible], eax
...
gui_create_component PROC
    ...
    mov [rdi + COMPONENT.visible], 1
    mov [rdi + COMPONENT.is_enabled], 1
    mov DWORD PTR [rdi + COMPONENT.opacity], 1065353216 ; 1.0f
    
    ; Set default dimensions
    mov DWORD PTR [rdi + COMPONENT.width], 1120403456   ; 100.0f
    mov DWORD PTR [rdi + COMPONENT.height], 1120403456  ; 100.0f
...
gui_apply_style PROC
    ...
    ; Parse style JSON into a temporary style struct
    lea rcx, [rsp + 32] ; temp style struct on stack
    mov rdx, r13        ; style_json
    call ParseStyleJson
    test eax, eax
    jz style_parse_failed
    
    ; Apply the parsed style to the component
    mov ecx, r12d       ; component_id
    lea rdx, [rsp + 32] ; temp style struct
...
ApplyComponentStyle PROC
    ...
    mov eax, [rsi + STYLE.background_color]
    mov [rdi + COMPONENT.custom_data], rax ; Using custom_data as proxy for bg_color in component
    
    ; Apply foreground color
    mov eax, [rsi + STYLE.text_color]
    ; ...
    ...
    ; Apply border width
    movss xmm0, [rsi + STYLE.border_width]
    movss [rdi + COMPONENT.min_width], xmm0 ; Proxy
    
    ; Apply corner radius
    movss xmm0, [rsi + STYLE.border_radius]
    movss [rdi + COMPONENT.min_height], xmm0 ; Proxy
    
    ; Apply font size
    movss xmm0, [rsi + STYLE.font_size]
    movss [rdi + COMPONENT.max_width], xmm0 ; Proxy
    ...
    ; Apply padding
    movss xmm0, [rsi + STYLE.padding_left]
    movss [rdi + COMPONENT.padding_left], xmm0
    movss xmm0, [rsi + STYLE.padding_top]
    movss [rdi + COMPONENT.padding_top], xmm0
    movss xmm0, [rsi + STYLE.padding_right]
    movss [rdi + COMPONENT.padding_right], xmm0
    movss xmm0, [rsi + STYLE.padding_bottom]
    movss [rdi + COMPONENT.padding_bottom], xmm0
    
    ; Apply margin
    movss xmm0, [rsi + STYLE.margin_left]
    movss [rdi + COMPONENT.margin_left], xmm0
    movss xmm0, [rsi + STYLE.margin_top]
    movss [rdi + COMPONENT.margin_top], xmm0
    movss xmm0, [rsi + STYLE.margin_right]
    movss [rdi + COMPONENT.margin_right], xmm0
    movss xmm0, [rsi + STYLE.margin_bottom]
    movss [rdi + COMPONENT.margin_bottom], xmm0
    
    ; Apply opacity
    movss xmm0, [rsi + STYLE.transition_duration] ; Proxy
    movss [rdi + COMPONENT.opacity], xmm0
    
    ; Apply shadow
    movss xmm0, [rsi + STYLE.shadow_blur]
    ; ...
    movss xmm0, [rsi + STYLE.shadow_offset_x]
    ; ...
    movss xmm0, [rsi + STYLE.shadow_offset_y]
    ; ...
    mov eax, [rsi + STYLE.shadow_color]
    ; ...
    
    ; Apply visibility
    mov eax, [rsi + STYLE.id] ; Proxy
...
UpdateComponentAppearance PROC
    ...
    mov rcx, rsi
    cvtss2si edx, [rbx + COMPONENT.x]
    cvtss2si r8d, [rbx + COMPONENT.y]
    cvtss2si r9d, [rbx + COMPONENT.width]
    cvtss2si eax, [rbx + COMPONENT.height]
    mov [rsp + 32], eax
...
ParseLayoutJson PROC
    ...
    mov r14, rax            ; Use r14 for string pointer
    lea rdx, szLayoutGrid
    mov rcx, r14
    call StringFind
    test rax, rax
    jne set_grid
...
skip_delay:
    ...
    jz skip_property
    call SkipJsonWhitespace
    mov r12, rax            ; Use r12 for property string pointer
...
set_prop_opacity:
    mov DWORD PTR [rsi + ANIMATION.property], ANIMATION_PROPERTY_OPACITY
    jmp skip_property

set_prop_height:
    mov DWORD PTR [rsi + ANIMATION.property], ANIMATION_PROPERTY_HEIGHT
    jmp skip_property

set_prop_width:
    mov DWORD PTR [rsi + ANIMATION.property], ANIMATION_PROPERTY_WIDTH
    jmp skip_property

set_prop_y:
    mov DWORD PTR [rsi + ANIMATION.property], ANIMATION_PROPERTY_Y
    jmp skip_property
...
skip_property:
    ...
    jz skip_easing
    call SkipJsonWhitespace
    mov r12, rax            ; Use r12 for easing string pointer
    lea rdx, szEaseIn
    mov rcx, r12
    call StringFind
    test rax, rax
...
set_ease_in:
    mov DWORD PTR [rsi + ANIMATION.easing], EASING_EASE_IN
    jmp skip_easing

set_ease_out:
    mov DWORD PTR [rsi + ANIMATION.easing], EASING_EASE_OUT
    jmp skip_easing

set_ease_inout:
    mov DWORD PTR [rsi + ANIMATION.easing], EASING_EASE_IN_OUT
...
set_prop_width:
    movss [rax + COMPONENT.width], xmm4
    jmp update_elapsed

set_prop_height:
    movss [rax + COMPONENT.height], xmm4
    jmp update_elapsed
...
class_found:
    ...
    mov [rsp + 40], rax
    cvtss2si eax, [rbx + COMPONENT.width]
    mov [rsp + 48], rax
    cvtss2si eax, [rbx + COMPONENT.height]
    mov [rsp + 56], rax
...
first_comp:
    ...
    mov byte ptr [rdi], ':'
    add rdi, 1
    movss xmm0, [rsi + COMPONENT.width]
    call append_float
    mov byte ptr [rdi], ','
    add rdi, 1
    ...
    mov byte ptr [rdi], ':'
    add rdi, 1
    movss xmm0, [rsi + COMPONENT.height]
    call append_float
    mov byte ptr [rdi], ','
    add rdi, 1

    ; "style":...
    lea rcx, szStyleKey_Val
    call append_string
    mov byte ptr [rdi], ':'
    add rdi, 1
...
;==========================================================================
; JSON HELPER FUNCTIONS (Local implementations)
;==========================================================================
...
append_bool PROC
    ...
    ; Append "true"
    mov dword ptr [rdi], 'eurt'
    mov byte ptr [rdi+4], 0
    jmp append_bool_done
    
append_false:
    ; Append "false"
    mov dword ptr [rdi], 'slaf'
...
write_json_to_file PROC
    ...
    mov r11d, FILE_ATTRIBUTE_NORMAL
    call CreateFileA
...
; Parses JSON style properties and populates style struct
; find_json_key(json: rcx, key: rdx) -> rax
find_json_key PROC
    ; Simple implementation - just call strstr_masm
    call strstr_masm
    ret
find_json_key ENDP

; parse_json_int(json_value: rcx) -> eax
parse_json_int PROC
...
; parse_json_bool(json_value: rcx) -> eax
parse_json_bool PROC
    ; Check for "true" or "false"
    cmp dword ptr [rcx], 'eurt'  ; "true"
    je return_true
    
    cmp dword ptr [rcx], 'slaf'  ; "fals"
    jne return_false
...
.data
    ...
    ; JSON Keys for AddComponentToJson
    szIdKey_Val     BYTE "id",0
    szTypeKey_Val   BYTE "type",0
    szXKey_Val      BYTE "x",0
    szYKey_Val      BYTE "y",0
    szWKey_Val      BYTE "w",0
    szHKey_Val      BYTE "h",0
    szStyleKey_Val  BYTE "style",0
    szBgColorKey_Val BYTE "bg_color",0
    szFgColorKey_Val BYTE "fg_color",0
    szBorderColorKey_Val BYTE "border_color",0
    szBorderWidthKey_Val BYTE "border_width",0
    szBorderRadiusKey_Val BYTE "border_radius",0
    szFontSizeKey_Val BYTE "font_size",0
    szFontWeightKey_Val BYTE "font_weight",0
    szPaddingLeftKey_Val BYTE "padding_left",0
    szPaddingTopKey_Val BYTE "padding_top",0
    szPaddingRightKey_Val BYTE "padding_right",0
    szPaddingBottomKey_Val BYTE "padding_bottom",0
    szMarginLeftKey_Val BYTE "margin_left",0
    szMarginTopKey_Val BYTE "margin_top",0
    szMarginRightKey_Val BYTE "margin_right",0
    szMarginBottomKey_Val BYTE "margin_bottom",0
    szOpacityKey_Val BYTE "opacity",0
    szShadowBlurKey_Val BYTE "shadow_blur",0
    szShadowOffsetXKey_Val BYTE "shadow_offset_x",0
    szShadowOffsetYKey_Val BYTE "shadow_offset_y",0
    szShadowColorKey_Val BYTE "shadow_color",0
    szVisibleKey_Val BYTE "visible",0
    szEnabledKey_Val BYTE "enabled",0
...
; JSON keys
szDurationKey_Val    BYTE "duration",0
szEasingKey_Val      BYTE "easing",0
szEaseIn_Val         BYTE "easeIn",0
szEaseOut_Val        BYTE "easeOut",0
szEaseInOut_Val      BYTE "easeInOut",0
