...
@find_last:
    ...
    cmp al, '\'
    jne @next
    mov rdx, rbx ; last backslash index
@next:
    inc rbx
    jmp @find_last

@found_end:
    ; Copy from last backslash + 1
    inc rdx
    add rsi, rdx
    mov rcx, rdi
    mov rdx, rsi
...
load_dataset_stats PROC
    ...
    ; rcx = dataset ptr
    mov dword ptr [rcx + DATASET.samples], 1000
    mov dword ptr [rcx + DATASET.features], 128
    mov dword ptr [rcx + DATASET.classes], 10
...
@training_loop:
    ...
    call asm_mutex_lock
    movzx eax, g_training_studio.training_active
    mov rcx, g_training_studio.hMutex
...
training_studio_wnd_proc PROC hWnd:HWND, uMsg:UINT, wParam:WPARAM, lParam:LPARAM
    ...
    cmp rax, 15 ; WM_PAINT
    je @on_paint
    cmp rax, 275 ; WM_TIMER
    je @on_timer
    cmp rax, 273 ; WM_COMMAND
    je @on_command
...
@on_create:
    mov g_training_studio.hWindow, rcx
    call create_dataset_list
    call create_model_list
    call create_experiment_list
    call create_metrics_chart
...
@on_timer:
    ; Trigger updates for charts
    mov rcx, g_training_studio.hMetricsChart
    xor rdx, rdx
    mov r8d, 1
    call InvalidateRect
    
    mov rcx, g_training_studio.hResourceChart
    xor rdx, rdx
    mov r8d, 1
    call InvalidateRect
...
@on_destroy:
    mov rcx, g_training_studio.update_timer
    test rcx, rcx
    jz @no_timer
    mov rdx, rcx
    mov rcx, hWnd
...
@on_paint:
    ...
    mov rsi, rax ; experiment ptr
    mov edi, [rsi + EXPERIMENT.metrics_count]
    test edi, edi
    jz @no_experiment
...
@draw_loss_loop:
    ...
    ; Calculate X: index * width / metrics_count
    mov eax, r13d
    imul eax, [rect_m.right]
    xor rdx, rdx
    div edi
    mov r14d, eax ; X
    
    ; Calculate Y: height - (loss * height)
    movss xmm0, [rsi + EXPERIMENT.train_loss + r13*4]
    cvtsi2ss xmm1, [rect_m.bottom]
    mulss xmm0, xmm1
    cvtss2si eax, xmm0
    mov edx, [rect_m.bottom]
    sub edx, eax ; Y
