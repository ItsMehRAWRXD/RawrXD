...
asm_mutex_destroy PROC
    ...
    ; Free the memory (rcx still holds pointer)
    call asm_free
...
asm_event_create PROC
    ...
    mov r12, rcx            ; r12 = manual_reset flag (0/1)
...
asm_event_set PROC
    ...
    ; Get Windows handle from structure and call SetEvent(handle)
    mov rax, [rcx]
    mov rcx, rax
    sub rsp, 20h
    call SetEvent
    ...
    ; Update state flag
    mov qword ptr [rcx + 16], 1  ; Set state = signaled
...
asm_event_reset PROC
    ...
    ; Get Windows handle and call ResetEvent(handle)
    mov rax, [rcx]
    mov rcx, rax
    sub rsp, 20h
    call ResetEvent
    ...
    ; Update state flag
    mov qword ptr [rcx + 16], 0  ; Set state = unsignaled
...
asm_event_wait PROC
    ...
    mov rcx, r8             ; hHandle
    ; rdx already contains timeout_ms
    sub rsp, 20h
...
asm_event_destroy PROC
    ...
    ; Free event structure memory (rcx already holds struct pointer)
    call asm_free
...
asm_initialize_critical_section PROC
    ; Production implementation: Call kernel32!InitializeCriticalSection
...
asm_enter_critical_section PROC
    ; Production implementation: Call kernel32!EnterCriticalSection
    ; RCX = pointer to CRITICAL_SECTION
    sub rsp, 28h
...
asm_leave_critical_section PROC
    ; Production implementation: Call kernel32!LeaveCriticalSection
    ; RCX = pointer to CRITICAL_SECTION
    sub rsp, 28h
...
asm_delete_critical_section PROC
    ; Production implementation: Call kernel32!DeleteCriticalSection
    ; RCX = pointer to CRITICAL_SECTION
    sub rsp, 28h
...
asm_create_event PROC
    ; Wrapper to CreateEventW for convenience
    ; RCX = bManualReset (BOOL)
    ; RDX = bInitialState (BOOL)
    ; R8  = lpName (LPCWSTR or NULL)
    
    ; Rearrange to WinAPI calling convention
    ...
    ; R8  already will be set below
    mov r8, rdx              ; move bInitialState to R8 (we need original RDX)
    mov rdx, rdx             ; keep bManualReset
    mov r9, rbx              ; lpName
...
ALIGN 16
asm_set_event PROC
    ; Production implementation: Call kernel32!SetEvent
    ; RCX = hEvent handle
    sub rsp, 28h
    call SetEvent
    add rsp, 28h
...
asm_reset_event PROC
    ; Production implementation: Call kernel32!ResetEvent
    ; RCX = hEvent handle
    sub rsp, 28h
    call ResetEvent
    add rsp, 28h
...
asm_wait_for_event PROC
    ; Wrapper to WaitForSingleObject
    ; RCX = hHandle, RDX = dwMilliseconds
    sub rsp, 20h
    call WaitForSingleObject
    add rsp, 20h
    ret
...
asm_close_handle PROC
    ; Production implementation: Call kernel32!CloseHandle
    ; RCX = hObject handle
    sub rsp, 28h
    call CloseHandle
    add rsp, 28h
