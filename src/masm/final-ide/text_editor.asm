...
;==========================================================================
; UNDO_OPERATION structure
;==========================================================================
UNDO_OPERATION STRUCT
    op_type     DWORD ?     ; 0=insert, 1=delete
    position    QWORD ?     ; Position in buffer
    length      QWORD ?     ; Length of operation
    text_data   QWORD ?     ; Saved text (for delete ops)
UNDO_OPERATION ENDS

;==========================================================================
; editor_push_undo(buffer: RCX, op_type: EDX, pos: R8, len: R9, text: [rsp+40])
;==========================================================================
ALIGN 16
editor_push_undo PROC
    ...
    ; Check if undo stack is full
    mov rax, [g_editor_state.undo_count]
    cmp rax, [g_editor_state.max_undo]
    jge undo_push_done
    
    ; Get undo stack entry
    mov rbx, [g_editor_state.undo_stack]
    mov rsi, SIZEOF UNDO_OPERATION
    imul rax, rsi
    add rbx, rax
    
    ; Fill operation
    mov [rbx + UNDO_OPERATION.op_type], edx
    mov [rbx + UNDO_OPERATION.position], r8
    mov [rbx + UNDO_OPERATION.length], r9
    
    ; Save text data if delete operation
    cmp edx, 1
    jne undo_push_no_text
    
    mov rcx, r9
    call asm_malloc
    mov [rbx + UNDO_OPERATION.text_data], rax
    
undo_push_no_text:
    inc [g_editor_state.undo_count]
...
editor_undo PROC
    ...
    ; Get last operation
    dec rax
    mov [g_editor_state.undo_count], rax
    ...
    mov edx, [rbx + UNDO_OPERATION.op_type]
    cmp edx, 0  ; Insert operation
    je undo_insert
    
    ; Undo delete - reinsert text
    mov rdx, [rbx + UNDO_OPERATION.position]
    mov r8, [rbx + UNDO_OPERATION.text_data]
    mov r9, [rbx + UNDO_OPERATION.length]
    call editor_buffer_insert
    
    ; Free saved text
    mov rcx, [rbx + UNDO_OPERATION.text_data]
    call asm_free
    jmp undo_success
...
undo_insert:
    ...
    mov rdx, [rbx + UNDO_OPERATION.position]
    mov r8, [rbx + UNDO_OPERATION.length]
    call editor_buffer_delete
...
editor_redo PROC
    ...
    mov rax, [g_editor_state.redo_count]
    test rax, rax
    jz redo_fail
    
    ; Get last redo operation
    dec rax
    mov [g_editor_state.redo_count], rax
    
    mov rbx, [g_editor_state.redo_stack]
    mov rsi, SIZEOF UNDO_OPERATION
    imul rax, rsi
    add rbx, rax
