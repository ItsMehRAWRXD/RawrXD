...
; Search commands and update results list with fuzzy matching
;==========================================================================
perform_fuzzy_search PROC
    ...
    ; Clear results array
    mov g_paletteState.resultCount, 0
    
    ; Search through all commands
    xor r12d, r12d          ; command index
    mov r13d, g_paletteState.commandCount
...
search_loop:
    ...
    ; Calculate fuzzy score for label
    mov rcx, r15            ; filter
    lea rdx, [rbx + COMMAND_ENTRY.label]
    call calculate_fuzzy_score
    ...
    mov ecx, g_paletteState.resultCount
    cmp ecx, MAX_RESULTS
    jge search_done
    
    ; Add to results array
    imul rcx, rcx, sizeof SEARCH_RESULT
    lea rdi, g_paletteState.results
    add rdi, rcx
    
    mov [rdi + SEARCH_RESULT.commandIndex], r12d
    mov [rdi + SEARCH_RESULT.fuzzyScore], eax
    
    inc g_paletteState.resultCount
...
;==========================================================================
; calculate_fuzzy_score(filter: rcx, text: rdx) -> score (eax)
; Calculate fuzzy match score (0 = no match, higher = better match)
;==========================================================================
calculate_fuzzy_score PROC
    ...
    mov rcx, rsi
    call string_length
    mov r14d, eax           ; filter length
    
    ; If filter is empty, return high score
    test r14d, r14d
    jz high_score
    
    ; Convert both to lowercase for case-insensitive matching
    xor rbx, rbx            ; text index
    
match_loop:
    ; Check if we've matched all filter characters
    cmp r13d, r14d
    jge match_complete
    ...
    test al, al
    jz match_fail           ; End of text, no complete match
    
    ; Convert to lowercase
    call to_lowercase
    mov cl, al
    
    ; Get current filter character
    ...
    ; Match found
    inc r13d                ; Advance filter
    add r12d, 10            ; Base score for match
    
    ; Bonus for consecutive matches
    cmp rbx, 0
    je next_text_char
    
    ; Check if previous character also matched
    ; (simplified - just give bonus for early matches)
    cmp rbx, 3
    jg next_text_char
    add r12d, 5             ; Early match bonus
...
match_complete:
    ; All filter characters matched
    mov eax, r12d
    jmp score_done
    
high_score:
    mov eax, 1000           ; High score for empty filter
    jmp score_done
...
sort_results_by_score PROC
    ...
    mov ecx, g_paletteState.resultCount
    cmp ecx, 2
    jb sort_done            ; Less than 2 elements
...
inner_loop:
    ...
    ; Compare results[i] and results[i+1]
    imul rsi, rbx, sizeof SEARCH_RESULT
    lea rdi, g_paletteState.results
    add rsi, rdi
    
    mov eax, [rsi + SEARCH_RESULT.fuzzyScore]
    mov edx, [rsi + sizeof SEARCH_RESULT + SEARCH_RESULT.fuzzyScore]
    
    cmp eax, edx
    jge no_swap
    
    ; Swap results[i] and results[i+1]
    mov r8d, [rsi + SEARCH_RESULT.commandIndex]
    mov r9d, [rsi + sizeof SEARCH_RESULT + SEARCH_RESULT.commandIndex]
    
    mov [rsi + SEARCH_RESULT.commandIndex], r9d
    mov [rsi + sizeof SEARCH_RESULT + SEARCH_RESULT.commandIndex], r8d
    
    mov [rsi + SEARCH_RESULT.fuzzyScore], edx
    mov [rsi + sizeof SEARCH_RESULT + SEARCH_RESULT.fuzzyScore], eax
...
populate_results_listbox PROC
    ...
    xor rbx, rbx            ; result index
    mov esi, g_paletteState.resultCount
...
pop_loop:
    ...
    ; Get result entry
    imul rax, rbx, sizeof SEARCH_RESULT
    lea rcx, g_paletteState.results
    add rcx, rax
    
    ; Get command index
    mov eax, [rcx + SEARCH_RESULT.commandIndex]
    
    ; Get command entry
    imul rax, rax, sizeof COMMAND_ENTRY
    lea rdx, g_paletteState.commands
    add rdx, rax
    
    ; Add label to listbox
    mov rcx, g_paletteState.hResultsList
    mov r8d, 180h           ; LB_ADDSTRING
    xor r9, r9
    lea r9, [rdx + COMMAND_ENTRY.label]
    mov rdx, r8
    mov r8, r9
    xor r9, r9
    call SendMessageA
