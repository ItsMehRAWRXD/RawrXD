...
hash_loop:
    ...
    ; rax = rax * 0x9E3779B97F4A7C15 (mixing constant)
    mov r10, 09E3779B97F4A7C15h
    mul r10                    ; RDX:RAX = RAX * r10
    xor rax, rdx               ; fold high bits for diffusion
    inc r8
    jmp hash_loop
...
rawr_deflate_compress PROC
    ...
    mov byte ptr [r8], 78h     ; CMF: deflate, default compression
    mov byte ptr [r8 + 1], 9Ch ; FLG
...
deflate_finalize:
    ...
    mov byte ptr [r8 + r10], 01h  ; BFINAL=1, BTYPE=00
...
rawr_metric_record PROC
    ...
    lea r9, metrics_base        ; address of metrics_base
    mov r8, qword ptr [r9]      ; load base pointer value
    cmp r8, 0
    je metric_skip
...
rawr_atomic_increment PROC
    ...
    xor rax, rax
    mov rdx, 1
    lock xadd qword ptr [rcx], rdx        ; atomic increment
...
rawr_atomic_compare_swap PROC
    ...
    lock cmpxchg qword ptr [rcx], r8      ; atomic CAS
