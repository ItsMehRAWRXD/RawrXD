...
;==============================================================================
; PUBLIC: HotpatchApply(pModelName: rcx, zoneId: edx) -> eax
;==============================================================================
...
HotpatchApply PROC
    ...
    cmp [g_HotpatchCoord.is_initialized], 1
    jne hotpatch_apply_fail
    
    ; Find target address (simulated)
    ; In production, this would use pattern matching or symbol lookup
    mov r14, 0x140000000 ; Dummy target address
    
    ; Generate patch code
    cmp r13d, HOTPATCH_ZONE_VRAM_CHECK
    je apply_vram_bypass
    jmp hotpatch_apply_fail
    
apply_vram_bypass:
    call BypassVRAMCheck
    test rax, rax
    jz hotpatch_apply_fail
    mov r15, rax        ; patch code
    
    ; Change memory protection
    mov rcx, r14        ; lpAddress
    mov rdx, HOTPATCH_CODE_SIZE ; dwSize
    mov r8d, PAGE_EXECUTE_READWRITE ; flNewProtect
    lea r9, oldProtect  ; lpflOldProtect
    call VirtualProtect
    ...
    mov rdi, r14
    mov rsi, r15
    mov rcx, 8          ; Copy 8 bytes (mov rax, 1; ret)
    rep movsb
    
    ; Restore protection
    mov rcx, r14
    mov rdx, HOTPATCH_CODE_SIZE
    mov r8d, oldProtect
    lea r9, oldProtect
    call VirtualProtect
...
;==============================================================================
; EXTERNAL: GetCurrentProcess() -> rax
;==============================================================================
