...
szCorrectionAttempt BYTE "[AGENT] Attempting correction for failure type %d", 13, 10, 0
szRefusal1          BYTE "I can't", 0
szRefusal2          BYTE "I cannot", 0
szRefusal3          BYTE "cannot help", 0
szHallucin1         BYTE "made-up", 0
szHallucin1         BYTE "I don't know", 0
szTimeout1          BYTE "timeout", 0
szTimeout2          BYTE "timed out", 0
szTokenLimit1       BYTE "token limit", 0
szTokenLimit2       BYTE "exceed max", 0
szApology1          BYTE "[APOLOGY: Request could not be processed. Please rephrase.]", 0
szHallucinDisc      BYTE " [DISCLAIMER: Information confidence is low. Verify independently.]", 0
szTimeoutRetry      BYTE " [RETRY: Please wait and retry the request.]", 0
szTokenLimitCont    BYTE " [CONTINUATION: Output truncated. Retry with smaller request.]", 0
...
agent_correct_response PROC
    ...
    mov r12, rcx            ; response text
    mov ebx, edx            ; failure type
    
    ; Log correction attempt
    lea rcx, [szCorrectionAttempt]
    mov edx, ebx
    call console_log
    
    ; Create corrected response
    lea rdi, [correction_buffer]
...
correct_refusal:
    ...
    lea rdx, [szRefusal1]
    lea r8, [szApology1]
    call replace_string_safe
    jmp correction_done
    
correct_hallucination:
    lea rcx, [rdi]
    lea rdx, [szHallucinDisc]
    call append_string_safe
    jmp correction_done
    
correct_timeout:
    lea rcx, [rdi]
    lea rdx, [szTimeoutRetry]
    call append_string_safe
    jmp correction_done
    
correct_token_limit:
    lea rcx, [rdi]
    lea rdx, [szTokenLimitCont]
    call append_string_safe
...
agent_detect_failure PROC
    ...
    mov rsi, rcx            ; response text
    test rsi, rsi
    jz detect_no_failure
    
    ; Check for refusal patterns (failure type 1)
    lea rcx, [rsi]
    lea rdx, [szRefusal1]
    call strstr_custom
    test rax, rax
    ...
    ; Check for hallucination patterns (failure type 2)
    lea rcx, [rsi]
    lea rdx, [szHallucin1]
    call strstr_custom
    test rax, rax
    jnz detect_hallucination
    
    ; Check for timeout patterns (failure type 3)
    lea rcx, [rsi]
    lea rdx, [szTimeout1]
    call strstr_custom
    test rax, rax
    ...
    ; Check for token limit patterns (failure type 5)
    lea rcx, [rsi]
    lea rdx, [szTokenLimit1]
    call strstr_custom
    test rax, rax
    jnz detect_token_limit
