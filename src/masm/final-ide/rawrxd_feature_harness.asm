...
.data
    ...
    ; JSON parsing keys
    szFeaturesKey           BYTE '"features"',0
    szTrue                  BYTE "true",0
    szFalse                 BYTE "false",0
    
    ; Registry keys
    szPolicyRegKey          BYTE "SOFTWARE\\RawrXD\\Policy\\Features",0
    szFeatureRegFormat      BYTE "Feature%d",0
...
LoadUserFeatureConfiguration PROC
    ...
    mov DWORD PTR [rsp + 48], FILE_ATTRIBUTE_NORMAL
    mov QWORD PTR [rsp + 56], 0   ; hTemplateFile
    call CreateFileA
    ...
    ; Null terminate
    mov eax, DWORD PTR [rsp + 60] ; bytes read
    mov ecx, eax
    ...
    ; Format: {"features": {"feature_id": true/false, ...}}
    mov r12, rdi        ; buffer pointer
    
    ; Find "features" key
    lea rsi, szFeaturesKey
    mov rcx, r12
    mov rdx, rsi
    call StringFind
    test rax, rax
    jz parse_config_done
    
    mov r13, rax
    add r13, 10         ; Skip past "features"
    
    ; Parse feature entries
    ; Look for pattern: "feature_id": true/false
parse_feature_loop:
    ; Find next feature ID (numeric)
    mov rcx, r13
    call FindNextFeatureId
    test rax, rax
    jz parse_config_done
    
    mov r13, rax        ; feature ID string
    mov rcx, r13
    call StringToInt
    mov ebx, eax        ; feature_id
    
    ; Find value (true/false)
    mov rcx, r13
    call FindNextFeatureValue
    test rax, rax
    jz parse_feature_loop
    ...
    call StringCompare
    test eax, eax
    jz set_feature_enabled
    
    ; Feature disabled
    mov ecx, ebx
    xor edx, edx
    call feature_toggle
    jmp parse_feature_loop
...
;==========================================================================
; Helper: FindNextFeatureId(rcx=json_string) -> rax (pointer to ID or NULL)
;==========================================================================
...
;==========================================================================
; Helper: FindNextFeatureValue(rcx=json_string) -> rax (pointer to value or NULL)
;==========================================================================
...
;==========================================================================
; Helper: StringToInt(rcx=string) -> eax (integer value)
;==========================================================================
StringToInt PROC
    push rbx
...
ApplyEnterpriseFeaturePolicy PROC
    ...
    ; Check Windows Registry for enterprise policy
    ; HKEY_LOCAL_MACHINE\SOFTWARE\RawrXD\Policy\Features
    
    ; Open registry key
    lea rcx, szPolicyRegKey
    mov rdx, KEY_READ
    lea r8, [rsp + 32]  ; hKey (output)
    call RegOpenKeyExA
    test eax, eax
    jnz check_security_defaults  ; If key doesn't exist, use defaults
    
    mov r12, [rsp + 32] ; hKey
    
    ; Read policy values for each feature
    xor ebx, ebx
    lea rdi, FeatureStatusArray
...
policy_loop:
    ...
    ; Build registry value name: "Feature%d"
    lea rcx, [rsp + 64] ; buffer
    lea rdx, szFeatureRegFormat
    mov r8d, ebx
    call wsprintfA
    
    ; Read registry value
    mov rcx, r12        ; hKey
    lea rdx, [rsp + 64] ; value name
    mov r8, 0           ; Reserved
    lea r9, [rsp + 40]  ; lpType
    lea rax, [rsp + 48] ; lpData
    mov [rsp + 32], rax
    mov dword ptr [rsp + 40], 4 ; cbData (DWORD)
    call RegQueryValueExA
    test eax, eax
    jnz check_security_feature
    
    ; Value exists - apply policy
    mov eax, [rsp + 48]
    test eax, eax
    jz disable_feature_policy
    mov [rsi + FEATURE_STATUS.feature_enabled], 1
    jmp next_policy_feature
    
disable_feature_policy:
    ; Check if mandatory - can't disable
    cmp [rsi + FEATURE_STATUS.feature_mandatory], 1
    je next_policy_feature
    mov [rsi + FEATURE_STATUS.feature_enabled], 0
...
policy_reg_done:
    ...
    mov rcx, r12
    call RegCloseKey
    jmp check_security_defaults
    
check_security_feature:
    ; No registry value - check security defaults
    cmp [rsi + FEATURE_STATUS.feature_category], FEATURE_CATEGORY_SECURITY
    jne next_policy_feature
    ; Force enable security features by default
    mov [rsi + FEATURE_STATUS.feature_enabled], 1
    jmp next_policy_feature
    
check_security_defaults:
    ; Ensure all security features are enabled (default policy)
    xor ebx, ebx
    lea rdi, FeatureStatusArray
...
InitializeFeaturePerformanceMonitoring PROC
    ...
    ; Allocate performance monitoring structure
    ; Structure: [feature_id (4 bytes)][call_count (8 bytes)][total_time (8 bytes)][avg_time (8 bytes)] * 32 features
    mov rcx, 32 * 28    ; 32 features * 28 bytes each
    call asm_malloc
    test rax, rax
    jz perf_mon_fail
    ...
    ; Initialize all feature performance counters
    xor ebx, ebx
...
perf_init_loop:
    ...
    mov [rdi], ebx              ; feature_id
    mov qword ptr [rdi + 4], 0  ; call_count
    mov qword ptr [rdi + 12], 0 ; total_time (nanoseconds)
    mov qword ptr [rdi + 20], 0 ; avg_time (nanoseconds)
    
    add rdi, 28                 ; Next entry
    inc ebx
    jmp perf_init_loop
    
perf_mon_success:
    ; Query performance frequency for accurate timing
    lea rcx, [rsp + 32]         ; lpFrequency
    call QueryPerformanceFrequency
    test eax, eax
    jz perf_mon_fail            ; If QPF fails, still return success but timing won't work
