//============================================================================
// nevm_kernel_provenance.hpp
// RawrXD N-EVM - Kernel Provenance Tracking
// Tracks compiler, flags, ISA path, registry version per benchmark
//============================================================================
...
struct KernelProvenance {
    ...
    // ISA path
    std::string isa_path;  // e.g., "AVX2", "AVX512", "AVX512-VNNI"
    
    // Kernel registry
    std::string registry_version;
    std::string kernel_hash;  // SHA256 of kernel binary
    ...
    static KernelProvenance FromJSON(const Json::Value& json) {
    ...
    // Detect current build provenance
    static KernelProvenance DetectCurrent() {
        KernelProvenance prov;
...
public:
    void Register(const std::string& kernel_name, const KernelProvenance& prov) {
        registry_[kernel_name] = prov;
    }
    
    KernelProvenance Get(const std::string& kernel_name) const {
    ...
    bool Has(const std::string& kernel_name) const {
    ...
    // Compare two provenance records for compatibility
    static bool IsCompatible(const KernelProvenance& a, const KernelProvenance& b) {
...
private:
    std::unordered_map<std::string, KernelProvenance> registry_;
};
