// ============================================================================
// ai_model_caller.cpp - FIXED VERSION
// Uses Deep2 Discovery as primary backend, removes hardcoded Ollama fallback
// ============================================================================
...
// Helper: Call Deep2 API
static std::string CallDeep2API(const std::string& prompt, const std::string& modelPath, int maxTokens) {
    // Use Deep2 Discovery to get the correct endpoint
    auto backend = Deep2::Deep2Discovery::GetPreferredBackend();
    
    if (!backend.available) {
        return "// Error: Deep2 backend not available";
    }
    
    // Parse URL to extract host and port
    std::string url = backend.url;
    std::string host = "localhost";
    int port = 11436; // Default Deep2 port
    ...
    std::wstring wpath = L"/api/generate";
    
    std::string resp = WinHttpPost(whost, port, wpath, body);
    
    if (resp.empty()) {
        return "// Error: Failed to connect to Deep2 API at " + backend.url;
    }
    
    // Parse Deep2 response
    size_t rpos = resp.find("\"response\":\"");
    if (rpos != std::string::npos) {
        rpos += 12;
        size_t endq = resp.find("\"", rpos);
...
static std::string CallNativeHost(const std::string& prompt) {
    HANDLE hPipe;
    char buffer[8192];
...
std::string ModelCaller::callModel(const std::string& prompt, const GenerationParams& params) {
    try {
        // PRIORITY 1: Try Native IPC (Production Path)
        std::string nativeResp = CallNativeHost(prompt);
        if (!nativeResp.empty()) {
            return nativeResp;
        }
        
        // PRIORITY 2: Try Deep2 Discovery
        auto backend = Deep2::Deep2Discovery::GetPreferredBackend();
        if (backend.available && backend.type == "deep2") {
            ...
            return CallDeep2API(prompt, modelPath, params.max_tokens);
        }
        
        // PRIORITY 3: Local GGUF via CPUInferenceEngine
        AgenticConfiguration& config = AgenticConfiguration::getInstance();
                ...
                if (fs::exists(modelPath)) {
                    if (engine->LoadModel(modelPath)) {
                     ...
                     if (fs::exists(fallbackPath)) {
                         ...
                         return "// Error: Model file not found. Please configure a valid model path.";
                     }
                }
            }
        ...
        return "// Error: No inference backend available. Please configure Deep2 or load a GGUF model.";
