...
//   2. AgentSelfRepair (scan-only MASM64 kernel + LiveBinaryPatcher redirection)
//   3. UnifiedHotpatchManager (three-layer coordination)
//
// Pipeline: detect bug → classify → verify CRC → redirect function via
//           LiveBinaryPatcher trampoline → CRC verify → log
//           → (rollback if verification fails)
//
// Enterprise-safe: NO byte-level mutation. All repairs are function-level
// redirections. The MASM64 kernel is used for scanning ONLY.
//
...
struct SelfHealAction {
    enum Type : uint8_t {
        None              = 0,
        PatternFix        = 1,  // Detected a known bug pattern (scan-only)
        TrampolineRedirect = 2, // Redirected a buggy function via LiveBinaryPatcher
...
public:
    ...
    //   1. Scan .text for all registered bug signatures (MASM64 kernel, read-only)
    //   2. Verify CRC of registered RepairableFunctions (software CRC32)
    //   3. Redirect degraded functions via LiveBinaryPatcher trampoline
    //   4. CRC32 verify each redirected function's integrity
    //   5. If any verification fails: rollback via LiveBinaryPatcher revert
    //   6. Fire callbacks, update statistics, telemetry
