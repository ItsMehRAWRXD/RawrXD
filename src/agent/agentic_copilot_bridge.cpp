...
AgenticCopilotBridge::AgenticCopilotBridge() {
    ...
    if (backend.native && backend.type == "deep2") {
        ...
        fprintf(stderr, "[AgenticCopilotBridge] Deep2 not available, using Ollama fallback on %s\n", backend.url.c_str());
        m_modelInvoker->setLLMBackend("ollama", backend.url, "");
    } else {
        fprintf(stderr, "[AgenticCopilotBridge] No backend detected, defaulting to Deep2 port 11436\n");
        m_modelInvoker->setLLMBackend("deep2", "http://localhost:11436", "");
    }
