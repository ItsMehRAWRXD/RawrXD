...
BulkFixResult BulkFixResult::fail(const std::string& id, const std::string& msg) {
    BulkFixResult r;
    r.success = false;
    r.bulkFixId = id;
    r.error = msg;
...
BulkFixResult AutonomousSubAgent::execute(const std::string& parentId) {
    ...
    if (m_targets.empty()) {
        ...
        return BulkFixResult::fail(m_bulkFixId, "No targets provided");
    }

    if (m_strategy.promptTemplate.empty()) {
        m_running.store(false);
        return BulkFixResult::fail(m_bulkFixId, "No strategy prompt template set");
    }
...
bool AutonomousSubAgent::processTarget(BulkFixTarget& target,
    ...
    for (int attempt = 0; attempt <= m_strategy.maxRetries; attempt++) {
        ...
        if (m_manager) {
            ...
            target.result = m_engine->processQuery(prompt);
        } else {
            target.status = BulkFixTarget::Status::Failed;
        ...
        if (m_strategy.selfHeal && m_detector) {
            auto failure = m_detector->detectFailure(target.result, target.id);
