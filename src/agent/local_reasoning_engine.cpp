...
bool LocalReasoningEngine::detectMissingStackAlignment(const std::string& asmCode, std::string& evidence) {
    ...
    std::string before = asmCode.substr(0, callPos);
...
bool LocalReasoningEngine::checkShadowSpace(const std::string& asmCode) {
...
bool LocalReasoningEngine::checkRegisterPreservation(const std::string& asmCode, const std::vector<std::string>& nonVolatile) {
    // Check if non-volatile registers are pushed at start and popped at end
    for (const auto& reg : nonVolatile) {
        size_t modifyPos = asmCode.find(reg + ",");  // Modified if used as destination
        if (modifyPos == std::string::npos) continue;

        // Check for push at start
        std::string start = asmCode.substr(0, 100);
        bool hasPush = (start.find("push " + reg) != std::string::npos);

        // Check for pop at end
        std::string end = asmCode.substr(asmCode.length() > 100 ? asmCode.length() - 100 : 0);
...
bool LocalReasoningEngine::detectIneffientInstruction(const std::string& asmCode, std::string& evidence) {
    // Detect common inefficiencies
    
    // xor reg, reg instead of mov reg, 0 is better
    if (asmCode.find("mov ") != std::string::npos && asmCode.find(", 0") != std::string::npos) {
        size_t pos = asmCode.find(", 0");
        size_t start = asmCode.rfind("mov ", pos);
        if (start != std::string::npos) {
            size_t end = asmCode.find('\n', pos);
            evidence = asmCode.substr(start, end - start) + " → use xor instead";
            return true;
        }
    }
    ...
    std::smatch match;
    if (std::regex_search(asmCode, match, imulPattern)) {
