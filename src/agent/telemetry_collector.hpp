...
public:
     ...
     */
    void trackFeatureUsage(const std::string& featureName, const nlohmann::json& metadata = {});
     ...
     */
    nlohmann::json getAllTelemetryData() const;
...
private:
    ...
    std::vector<nlohmann::json> m_events;  ///< Buffered events for batch sending
    ...
    void sendTelemetry(const nlohmann::json& payload);
