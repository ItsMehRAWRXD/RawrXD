...
PatchResult AgentSelfRepair::redirectFunction(uint32_t slotId) {
    ...
    if (result.success) {
        // Build a tiny relay trampoline: mov rax, <fallback>; jmp rax
        // This goes into the LiveBinaryPatcher's executable trampoline page.
        uintptr_t fallbackAddr = reinterpret_cast<uintptr_t>(target->fallbackImpl);
        uint8_t relay[12];
        relay[0] = 0x48; relay[1] = 0xB8; // REX.W MOV RAX, imm64
        std::memcpy(relay + 2, &fallbackAddr, 8);
