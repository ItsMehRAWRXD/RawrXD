...
using PFN_MapViewOfFile3 = PVOID(WINAPI*)(HANDLE, HANDLE, PVOID, ULONG64, SIZE_T, ULONG, ULONG, MEM_EXTENDED_PARAMETER*,
                                          ULONG);
...
GgufMapTelemetry& GetGgufMapTelemetry()
{
    ...
    std::call_once(init_once,
                   ...
                   {
                       char value[8] = {};
                       const DWORD len = GetEnvironmentVariableA("RAWRXD_GGUF_MAP_TELEMETRY", value,
                                                                 static_cast<DWORD>(sizeof(value)));
                       telemetry_ptr->enabled = len > 0 && len < sizeof(value) && value[0] != '0';
                       telemetry_ptr->remaining_decisions.store(
                           telemetry_ptr->enabled ? kGgufMapTelemetryDecisionBudget : 0, std::memory_order_relaxed);
                   });
    return telemetry;
}
...
bool ShouldRecordGgufMapDecision()
{
    ...
    while (remaining > 0)
    {
        if (telemetry.remaining_decisions.compare_exchange_weak(remaining, remaining - 1, std::memory_order_relaxed,
...
void FlushGgufMapTelemetrySummary()
{
    ...
    if (reuses == 0 && remaps == 0 && prefetch_bytes == 0 && align_large_2mb == 0 && align_sys_64kb == 0 &&
        align_legacy_reserve == 0 && align_direct_map == 0)
    {
        return;
    }
    ...
    oss << "GGUF_MAP_STATS: reuses=" << reuses << ", remaps=" << remaps
        << ", prefetch_kb=" << (prefetch_bytes / 1024ULL) << ", avg_win=" << avg_window_kb << "KB"
        << ", ALIGN_MODE=" << alignMode << ", ALIGN_MASK=0x" << std::hex << alignMask << std::dec << "\n";
}
}
...
NativeInferenceApi& GetNativeInferenceApi()
{
    ...
    std::call_once(
        ...
        [&]()
        {
            const wchar_t* kCandidates[] = {L"RawrXD_Titan.dll", L"RawrXD_InferenceEngine_Win32.dll",
            ...
            auto loadCandidate = [](const wchar_t* candidate) -> HMODULE
            {
                ...
                *(slash + 1) = L'\0';
                ...
                const DWORD envNativeLen = GetEnvironmentVariableW(L"RAWRXD_NATIVE_DLL_DIR", envNativeDir,
                                                                   static_cast<DWORD>(std::size(envNativeDir)));
            ...
            for (const wchar_t* candidate : kCandidates)
            {
                ...
                PFN_SubmitInference legacySubmit =
                    reinterpret_cast<PFN_SubmitInference>(GetProcAddress(module, "SubmitInference"));
                PFN_GetResult legacyGetResult = reinterpret_cast<PFN_GetResult>(GetProcAddress(module, "GetResult"));
                ...
                PFN_InferenceEngineLoadModel loadModel =
                    reinterpret_cast<PFN_InferenceEngineLoadModel>(GetProcAddress(module, "InferenceEngine_LoadModel"));
                ...
                PFN_InferenceEngineGetResult getResultEngine =
                    reinterpret_cast<PFN_InferenceEngineGetResult>(GetProcAddress(module, "InferenceEngine_GetResult"));
                ...
                if (loadModel && submitEngine && getResultEngine)
                {
                    ...
                    api.createEngine =
                        reinterpret_cast<PFN_CreateInferenceEngine>(GetProcAddress(module, "CreateInferenceEngine"));
                    api.destroyEngine =
                        reinterpret_cast<PFN_DestroyInferenceEngine>(GetProcAddress(module, "DestroyInferenceEngine"));
...
bool looksLikeBinaryInferencePayload(const std::string& text)
{
    ...
    for (size_t i = 0; i < text.size(); ++i)
    {
        ...
        if (i + 2 < text.size() && static_cast<unsigned char>(text[i]) == 0xEF &&
            static_cast<unsigned char>(text[i + 1]) == 0xBF && static_cast<unsigned char>(text[i + 2]) == 0xBD)
        {
            ++replacementTriples;
            i += 2;
        }
                            ...
                            std::string& metadata, std::string& error)
{
    ...
    // -------------------------------------------------------------------------
    {
        ...
        {
            // Scaffoldless model handoff: push UTF-8 path over the Titan pipe so TitanHost
            // loads weights explicitly (not only via inherited RAWRXD_NATIVE_MODEL_PATH).
            {
                std::string titanPathUtf8;
                wchar_t wModel[2048];
                const DWORD envLen = GetEnvironmentVariableW(L"RAWRXD_NATIVE_MODEL_PATH", wModel,
                                                             static_cast<DWORD>(sizeof(wModel) / sizeof(wModel[0])));
                if (envLen > 0 && envLen < static_cast<DWORD>(sizeof(wModel) / sizeof(wModel[0])))
                {
                    const int need = WideCharToMultiByte(CP_UTF8, 0, wModel, -1, nullptr, 0, nullptr, nullptr);
                    if (need > 1)
                    {
                        titanPathUtf8.resize(static_cast<size_t>(need - 1));
                        WideCharToMultiByte(CP_UTF8, 0, wModel, -1, titanPathUtf8.data(), need, nullptr, nullptr);
                    }
                }
                if (titanPathUtf8.empty())
                {
                    titanPathUtf8 = BackendOrchestrator::Instance().GetPreferredLoadedModelPath();
                }
                if (!titanPathUtf8.empty())
                {
                    std::string lmErr;
                    if (!RawrXD::TitanProxy::instance().loadModel(titanPathUtf8, lmErr))
                    {
                        error = lmErr.empty() ? "TitanHost load_model failed" : lmErr;
                        return false;
                    }
                }
            }
    ...
    if (win32ApiReady && api.loadModel && !api.modelLoaded)
    {
        ...
        const DWORD len =
            GetEnvironmentVariableW(L"RAWRXD_NATIVE_MODEL_PATH", modelPath, static_cast<DWORD>(sizeof(modelPath)));
        ...
        else
        {
            ...
            if (!preferredModel.empty())
            {
                ...
                const int converted = MultiByteToWideChar(CP_UTF8, 0, preferredModel.c_str(), -1, preferredWide,
        ...
        if (!api.modelLoaded)
        {
            ...
            {
                error = "InferenceEngine_LoadModel failed (no env model path and preferred loaded model path "
    ...
    while (true)
    {
        ...
        {
            gotResult =
                api.getResultEngine(api.engineHandle, static_cast<uint32_t>(requestId), buffer.data(), buffer.size());
        }
...
bool discoverModelShards(const std::string& model_path, std::vector<std::string>& shards, std::string& reason)
{
    namespace fs = std::filesystem;
...
bool reserveSlidingAperture(SIZE_T requested_size, void*& out_view, size_t& out_reserved_size,
...
bool mapShardFileReadOnly(const std::string& path, int device_index, MappedShardFile& out, std::string& reason)
{
    ...
    out.hFile = CreateFileA(path.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL,
                            nullptr);
...
bool mapSlidingWindow(MappedShardFile& file, uint64_t offset, size_t window_size, void*& window_ptr,
                      std::string& reason)
{
    window_ptr = nullptr;
    const bool allowDirectMap =
    ...
    else if (file.active_window && offset >= file.active_window_offset &&
             requested_end <= file.active_window_offset + file.active_window_size)
    {
    ...
    auto map_one_window = [&](uint64_t map_offset, size_t one_map_size, void* one_base, void*& mapped_out,
                              std::string& map_reason) -> bool
    {
        mapped_out = nullptr;
        ...
        if (PFN_MapViewOfFile3 mapViewOfFile3 = GetMapViewOfFile3Ptr())
        {
            ...
            void* mapped_view = mapViewOfFile3(file.hMapping, GetCurrentProcess(),
                                               file.has_placeholder_reservation ? one_base : nullptr, map_offset,
                                               one_map_size, allocationType, PAGE_READONLY, nullptr, 0);
            if (mapped_view)
            {
        ...
        if (!mapped_out)
        {
            ...
            if (mapped_out)
            {
                ...
                oss << "[LoaderTrace] MapViewOfFileEx mode="
                    << (file.has_placeholder_reservation ? "reserved-base" : "direct") << " base=" << mapped_out
                    << " offset=" << map_offset << " size=" << one_map_size << "\n";
    ...
    for (int i = 0; i < stripe_target; ++i)
    {
        ...
        const uint64_t stripe_offset =
            (stripe_offset_raw / sysInfo.dwAllocationGranularity) * sysInfo.dwAllocationGranularity;
        ...
        if (PFN_PrefetchVirtualMemory prefetchVirtualMemory = GetPrefetchVirtualMemoryPtr())
        {
            ...
            if (prefetch_start < stripe_end)
            {
                const size_t prefetch_size =
                    static_cast<size_t>(std::min<uint64_t>(stripe_end - prefetch_start, prefetch_budget));
                if (prefetch_size > 0)
                {
                    WIN32_MEMORY_RANGE_ENTRY range{};
                    range.VirtualAddress =
                        static_cast<char*>(mapped_view) + static_cast<size_t>(prefetch_start - stripe_offset);
...
{
    m_failover_order = {BackendKind::Vulkan, BackendKind::HIP, BackendKind::DML, BackendKind::CPU};
...
#if defined(_WIN32)
    // Always: headless may have started TitanHost even if orchestrator init failed early.
...
{
    if (m_forced_backend != BackendKind::Count)
        return m_forced_backend;
...
{
    std::unique_lock<std::shared_mutex> lk(m_backend_mtx);
    if (!m_health[(int)target].available)
        return false;
...
bool BackendOrchestrator::ShardModel(const std::string& model_path, const std::vector<int>& device_indices)
{
    ...
    std::stable_sort(shard_order.begin(), shard_order.end(),
                     [&](size_t a, size_t b)
                     {
...
void BackendOrchestrator::SetKVCacheGPUBudgetMB(float mb)
{
    m_kv_gpu_budget_mb.store(mb);
}
void BackendOrchestrator::SetKVCacheCPUBudgetMB(float mb)
{
    m_kv_cpu_budget_mb.store(mb);
}
float BackendOrchestrator::GetKVCacheGPUUsedMB() const
{
    return m_kv_gpu_used_mb.load();
}
float BackendOrchestrator::GetKVCacheCPUUsedMB() const
{
    return m_kv_cpu_used_mb.load();
}
...
bool BackendOrchestrator::ForensicMapProbe(const std::string& path, uint64_t offset, size_t window_size,
                                           std::string* out_reason)
{
    ...
    void* mapped_view =
        MapViewOfFile(hMapping, FILE_MAP_READ, static_cast<DWORD>(aligned_offset >> 32),
                      static_cast<DWORD>(aligned_offset & 0xFFFFFFFFULL), static_cast<SIZE_T>(desired_window));
    if (!mapped_view)
    {
...
bool BackendOrchestrator::UnloadModel(const std::string& tag)
{
    ...
    if (m_active_model_tag == tag)
        m_active_model_tag.clear();
...
std::vector<std::string> BackendOrchestrator::GetLoadedModelTags() const
{
    ...
    for (const auto& [t, _] : m_model_paths)
        tags.push_back(t);
    return tags;
}
...
void BackendOrchestrator::Cancel(uint64_t request_id)
{
    ...
    for (auto& q : m_queues)
    {
        q.erase(std::remove_if(q.begin(), q.end(), [=](const InferRequest& r) { return r.id == request_id; }), q.end());
    }
...
bool BackendOrchestrator::RunInference(const InferRequest& req)
{
    ...
    if (!ok)
    {
        ...
        if (req.complete_cb)
        {
            req.complete_cb("[BackendError] " + normalizedError, "{\"error\":\"native_inference_failed\"}");
        }
        return false;
    }
    ...
    // Native path returns full completion synchronously; stream_cb is one shot (not per-token HTTP NDJSON).
...
bool BackendOrchestrator::LookupCache(const std::string& prompt, std::string& out_completion) const
{
    if (!m_cache_enabled)
        return false;
    m_cache_lookups.store(m_cache_lookups.load(std::memory_order_relaxed) + 1, std::memory_order_relaxed);
    ...
    const_cast<SemanticCacheEntry&>(it->second).last_accessed = std::chrono::steady_clock::now();
    m_cache_hits.store(m_cache_hits.load(std::memory_order_relaxed) + 1, std::memory_order_relaxed);
    return true;
}

void BackendOrchestrator::InsertCache(const std::string& prompt, const std::string& completion)
{
    if (!m_cache_enabled)
        return;
    std::string key = HashPrompt(prompt);
    ...
    if (m_cache.size() >= m_cache_max)
    {
        ...
        auto victim = std::min_element(m_cache.begin(), m_cache.end(), [](const auto& a, const auto& b)
...
std::string BackendOrchestrator::CompressContext(const std::string& raw_context) const
{
    ...
    if (win < total - sink)
        out += "[...compressed...] ";
    int start = total - win;
    for (int i = std::max(start, sink); i < total; ++i)
        out += words[i] + " ";
    return out;
}
...
int BackendOrchestrator::EstimateCompressedTokenCount(int raw_tokens) const
{
    if (!m_ctx_comp.enabled)
        return raw_tokens;
    return std::min(raw_tokens, m_ctx_comp.sink_tokens + m_ctx_comp.rolling_window + m_ctx_comp.summary_budget);
}
...
void BackendOrchestrator::RefreshBackendHealth(BackendKind k)
{
    ...
    switch (k)
    {
        ...
        case BackendKind::Vulkan:
        {
            ...
            {
                FreeLibrary(vk);
                h.vram_total_mb = 16384.f; /* RX 7800 XT */
            }
            break;
        }
...
{
    if (m_health_running.exchange(true))
        return;
    m_health_thread = std::thread([this, interval]() { HealthCheckLoop(interval); });
}
...
void BackendOrchestrator::SetMaxBatchSize(int n)
{
    m_max_batch_size = n;
}
int BackendOrchestrator::GetCurrentBatchSize() const
{
    return m_cur_batch_size.load();
}
void BackendOrchestrator::FlushBatch()
{
    m_queue_cv.notify_all();
}
...
{
    m_metrics_path = path;
    if (m_metrics_running.exchange(true))
        return;
...
int BackendOrchestrator::GetRecommendedPrefetchDepth() const
{
    ...
    const float thermal_peak =
        std::max(m_cpu_temp_c.load(std::memory_order_relaxed), m_nvme_temp_c.load(std::memory_order_relaxed));
...
namespace RawrXD
{  // reopen for RegisterTitanToolDispatch
