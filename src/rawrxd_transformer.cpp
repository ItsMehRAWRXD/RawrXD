...
std::vector<float> RawrXDTransformer::Forward(const std::vector<uint32_t>& tokens, int start_pos)
{
    ...
    for (int t = 0; t < T; t++)
    {
        ...
        if (!row_ok)
        {
            printf("[Forward] FATAL: Missing token embedding tensor (tried: token_embd.weight, model.embed_tokens.weight, embeddings.weight, input.weight, v.token_embd.weight, v.embeddings.weight, v.input.weight)\n");
            return {};
        }
