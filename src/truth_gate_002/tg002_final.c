/* tg002_final.c - Final working version */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdint.h>
...
/* Skip metadata - just find tensor section */
static size_t skip_metadata(uint8_t* data, uint64_t count) {
    size_t pos = 24; /* After header */
    ...
    for (uint64_t i = 0; i < count; i++) {
        ...
        switch (val_type) {
            ...
            case 12: {
                ...
                pos += 8;
                size_t elem_size = (arr_type >= 4 && arr_type <= 6) ? 4 : 
                                   (arr_type >= 7 && arr_type <= 9) ? 8 : 1;
...
int gguf_open(const char* path, gguf_context_t* ctx) {
    ...
    /* Skip metadata */
    size_t pos = skip_metadata(ctx->data, metadata_count);
...
int main(int argc, char* argv[]) {
    ...
    if (tok_emb) {
        printf("Found token_embd: %llu elements, type %u, dims=[%llu, %llu]\n",
               (unsigned long long)tok_emb->n_elements, tok_emb->type,
               (unsigned long long)tok_emb->dims[0], (unsigned long long)tok_emb->dims[1]);
    } else {
