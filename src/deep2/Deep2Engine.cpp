...
void Deep2Engine::deallocateBuffers() {
    ...
    alignedFree(gateBuf);
    alignedFree(upBuf);
    alignedFree(ssmInnerBuf);
    alignedFree(ssmConvBuf);
    alignedFree(ssmDtBuf);
    alignedFree(ssmABuf);
    alignedFree(ssmOutBuf);
    alignedFree(ssmTempBuf);
    alignedFree(ssmState);
    hiddenStates = attentionOutput = ffnOutput = nullptr;
    logits = qProj = kProj = vProj = gateBuf = upBuf = nullptr;
    ssmInnerBuf = ssmConvBuf = ssmDtBuf = ssmABuf = nullptr;
    ssmOutBuf = ssmTempBuf = ssmState = nullptr;
    ssmStateInitialized = false;
}
...
bool Deep2Engine::loadModel(const std::string& ggufPath) {
    ...
    // SSM metadata — for hybrid architectures like Nemotron-H
    if (meta.ssmStateSize > 0) {
        modelWeights.isSSM          = true;
        modelWeights.ssmStateSize   = meta.ssmStateSize;
        modelWeights.ssmConvKernel  = meta.ssmConvKernel;
        modelWeights.ssmInnerSize   = meta.ssmInnerSize;
        modelWeights.ssmGroupCount  = meta.ssmGroupCount > 0 ? meta.ssmGroupCount : 1;
        modelWeights.ssmTimeStepRank = meta.ssmTimeStepRank;
        printf("[Deep2Engine] SSM hybrid model: state=%zu conv=%zu inner=%zu groups=%zu dtRank=%zu\n",
               modelWeights.ssmStateSize, modelWeights.ssmConvKernel,
               modelWeights.ssmInnerSize, modelWeights.ssmGroupCount,
               modelWeights.ssmTimeStepRank);
    }
    ...
    for (const auto& t : ggufResult.tensors) {
        ...
        if (name == "token_embd.weight") {
            ...
            // SSM (State Space Model) tensor mapping — for hybrid architectures like Nemotron-H
            if (name.find("ssm_in") != std::string::npos) {
                lw.ssmIn = wt;
                lw.hasSSM = true;
            }
            else if (name.find("ssm_out") != std::string::npos)
                lw.ssmOut = wt;
            else if (name.find("ssm_norm") != std::string::npos)
                lw.ssmNorm = wt;
...
void Deep2Engine::forwardLayer(size_t layer, const float* input, float* output, size_t seqLen) {
    ...
    // ── SSM (State Space Model) block path ──────────────────────────────
    ...
    if (lw.hasSSM && lw.ssmIn.data && modelWeights.isSSM) {
        ...
        // 2. SSM block: conv1d -> state update -> output projection
        computeSSMBlock(layer, attentionOutput, output, seqLen);
...
// ============================================================================
// Reset SSM state — clears all SSM hidden states for new conversation
// ============================================================================
