// deep2_k2_gpu_stream_mla_cert.cpp — K2_GPU_STREAM_MLA_001
// Live MLA Q4_K GEMV via DispatchGEMVPacked; CPU fallback parity.
...
struct RunOut {
    ...
    int32_t genId = -1;
    std::string text;
    uint64_t mlaOps = 0, mlaFail = 0, copyOps = 0;
};
...
static RunOut RunOnce(Deep2Engine& e, const char* prompt, uint32_t depth,
    ...
    o.copyOps = GpuTransfer_Snapshot().copyOps;
    return o;
}
...
int main() {
    ...
    _putenv_s("DEEP2_WEIGHT_SLOTS", "16");
    ...
    CreateDirectoryA("G:\\~dev\\rawrxd\\evidence\\K2_GPU_STREAM_MLA_001", nullptr);
    printf("K2_GPU_STREAM_MLA_001\nMODEL=%s\n", dir);
    if (!fs::is_directory(dir)) {
        printf("K2_GPU_STREAM_MLA_001=SKIP\n"); return 0;
    }
    ...
    if (!eng.initialize(cfg) || !eng.openK2ShardDirectory(dir)) {
        printf("K2_GPU_STREAM_MLA_001=FAIL open\n"); return 2;
    }

    printf("\n--- CPU MLA baseline (d=1 t=1) ---\n");
    ...
    printf("\n--- GPU MLA live path (d=1 t=1) ---\n");
    RunOut gpu = RunOnce(eng, kPrompt, 1, 1, true);
    printf("OK=%d GEN=%d MLA_OPS=%llu MLA_FAIL=%llu COPY_OPS=%llu\n",
           gpu.ok ? 1 : 0, (int)gpu.genId,
           (unsigned long long)gpu.mlaOps, (unsigned long long)gpu.mlaFail,
           (unsigned long long)gpu.copyOps);
    K2GpuStreamCopy_Emit(stdout);
    GpuTransfer_Emit(stdout);

    const bool usedGpu = gpu.mlaOps > 0 && gpu.mlaFail == 0;
    const bool parity = cpu.ok && gpu.ok && cpu.genId == gpu.genId &&
                        cpu.text == gpu.text;
    const bool pass = usedGpu && parity;
    printf("MLA_GPU_USED=%u OUTPUT_PARITY=%u\n", usedGpu ? 1u : 0u,
           parity ? 1u : 0u);
    printf("K2_GPU_STREAM_MLA_001=%s\n", pass ? "PASS" : "FAIL");
    FILE* f = fopen(
        "G:\\~dev\\rawrxd\\evidence\\K2_GPU_STREAM_MLA_001\\GATE_STATUS.txt", "w");
    if (f) {
        fprintf(f, "cpu_gen=%d gpu_gen=%d mla_ops=%llu mla_fail=%llu copy=%llu\n",
        ...
        fprintf(f, "K2_GPU_STREAM_MLA_001=%s\n", pass ? "PASS" : "FAIL");
        fprintf(f, "NOTE=Live MLA Q4_K gemvDispatchTransposed → DispatchGEMVPacked.\n");
        fclose(f);
    }
    fflush(stdout);
    _exit(pass ? 0 : 2);
