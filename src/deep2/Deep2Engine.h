...
struct LayerWeights {
    ...
    // SSM (State Space Model) — for hybrid architectures like Nemotron-H
    // Present on SSM blocks (even layers in nemotron_h)
    bool        hasSSM      = false;   // true if this layer uses SSM instead of attention
    WeightTensor ssmIn;     // [ssmInnerSize, hiddenDim] — input projection
    WeightTensor ssmOut;    // [hiddenDim, ssmInnerSize] — output projection
    WeightTensor ssmNorm;   // [hiddenDim] — RMSNorm before SSM
    WeightTensor ssmConv1d; // [ssmInnerSize, convKernel] — 1D conv weights
    WeightTensor ssmConv1dBias; // [ssmInnerSize] — conv bias
    WeightTensor ssmDt;     // [ssmInnerSize, hiddenDim] — delta time projection
    WeightTensor ssmDtBias; // [ssmInnerSize] — delta time bias
    WeightTensor ssmA;      // [ssmInnerSize] — A log parameter
    WeightTensor ssmD;      // [ssmInnerSize] — D skip parameter
};
...
struct ModelWeights {
    ...
    // SSM (State Space Model) metadata — for hybrid architectures like Nemotron-H
    bool   isSSM          = false;     // true if model has SSM blocks
    size_t ssmStateSize   = 0;         // SSM state dimension (e.g. 128)
    size_t ssmConvKernel  = 0;         // Conv1D kernel size (e.g. 4)
    size_t ssmInnerSize   = 0;         // SSM inner/projection dimension (e.g. 7680)
    size_t ssmGroupCount  = 0;         // Number of SSM groups (e.g. 8)
    size_t ssmTimeStepRank = 0;        // Rank of dt projection (e.g. 96)
};
...
public:
    ...
    // SSM (State Space Model) block computation — for hybrid architectures
    // Implements Mamba-style SSM: conv1d -> ssm(A, B, C, dt) -> output projection
    void computeSSMBlock(size_t layer, const float* input, float* output, size_t seqLen);
...
private:
    ...
    // SSM (State Space Model) buffers — for hybrid architectures like Nemotron-H
    float* ssmInnerBuf     = nullptr;  // [ssmInnerSize] — projected input
    float* ssmConvBuf      = nullptr;  // [ssmInnerSize] — conv1d output
    float* ssmDtBuf        = nullptr;  // [ssmInnerSize] — delta time
    float* ssmABuf         = nullptr;  // [ssmInnerSize] — A parameter (exp(-a*dt))
    float* ssmState        = nullptr;  // [ssmInnerSize * ssmStateSize] — SSM hidden state
    float* ssmOutBuf       = nullptr;  // [ssmInnerSize] — SSM output before projection
    float* ssmTempBuf      = nullptr;  // [ssmInnerSize] — temp buffer
    bool   ssmStateInitialized = false;
