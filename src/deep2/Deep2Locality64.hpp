...
// Cert sets active ordinal before each measured decode step (0..63).
inline std::atomic<uint64_t>& Locality64_ActiveOrdinal() {
    static std::atomic<uint64_t> v{UINT64_MAX};
    return v;
}
inline void Locality64_SetActiveOrdinal(uint64_t o) {
    Locality64_ActiveOrdinal().store(o, std::memory_order_release);
}
