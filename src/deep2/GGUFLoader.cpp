...
bool GGUFLoader::ParseMetadataKV(FILE* fp, uint64_t kvCount, ModelMetadata& metadata,
    ...
    for (uint64_t i = 0; i < kvCount; ++i) {
        ...
        if (key == "general.architecture") {
            ...
            if (key.find("ssm.state_size") != std::string::npos) {
                metadata.ssmStateSize = (uint32_t)strtoul(valueStr.c_str(), nullptr, 10);
            }
            if (key.find("ssm.conv_kernel") != std::string::npos) {
                metadata.ssmConvKernel = (uint32_t)strtoul(valueStr.c_str(), nullptr, 10);
            }
            if (key.find("ssm.inner_size") != std::string::npos) {
                metadata.ssmInnerSize = (uint32_t)strtoul(valueStr.c_str(), nullptr, 10);
            }
            if (key.find("ssm.group_count") != std::string::npos) {
                metadata.ssmGroupCount = (uint32_t)strtoul(valueStr.c_str(), nullptr, 10);
            }
            if (key.find("ssm.time_step_rank") != std::string::npos) {
                metadata.ssmTimeStepRank = (uint32_t)strtoul(valueStr.c_str(), nullptr, 10);
            }
        }
    }
    return true;
