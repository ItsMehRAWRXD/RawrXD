// endurance_live_cert.cpp — DEEP2_ENDURANCE_001 (same-process multi-gen)
// Soft-reset continuity. No authority retries. PROMOTE=0 TIP_CLIMB=HOLD.
...
static void WriteReceipt(const char* path, int runtime, int pass, uint32_t reqs,
                         ...
                         int mock, uint64_t wup_d, int retries) {
    FILE* f = nullptr;
    fopen_s(&f, path, "wb");
    if (!f) return;
    const int live = pass && runtime && !mock && (ok == reqs) && (reqs >= 100) &&
                     (toks > 0) && dual && gpu_res && (wup_d == 0) &&
                     (retries == 0);
    ...
    fprintf(f, "GENERATION_RETRIES=%d\n", retries);
    ...
    fprintf(f, "WEIGHT_UPLOAD_DELTA=%llu\n", (unsigned long long)wup_d);
...
int main(int argc, char** argv) {
    ...
    printf("GENERATION_RETRIES=0 MODEL_REOPEN_MIDRUN=0 SAME_PROCESS=1\n");
    ...
    if (!s || !d2_session_bind_deep2_engine_ptr(s, &engine) ||
        ...
        WriteReceipt(receipt, 0, 0, reqs, 0, 0, 0, 0, 1, 0, 0);
        return 1;
    }
    ...
    req.temperature = 0.f;
    req.seed = 42;
    req.continue_context = 0; /* soft reset between requests */
    uint32_t ok = 0;
    uint64_t toks = 0;
    uint64_t wA = 0;
    int haveA = 0;
    for (uint32_t i = 0; i < reqs; ++i) {
        if (i > 0) {
            req.max_tokens = 2;
            /* Point A: after prior gen return, before session reset */
            engine.emitContinuitySnap(stderr, i);
            fprintf(stderr, "CONTINUITY_POINT=AFTER_GEN%u_RETURN\n", i - 1);
        }
        /* Session generate calls fn_reset when continue_context=0 */
        if (i > 0) {
            /* Point B captured inside generate after reset via snap below */
        }
        engine.emitContinuitySnap(stderr, i);
        fprintf(stderr, "CONTINUITY_POINT=BEFORE_GEN%u_GENERATE\n", i);
        const int okGen = d2_session_generate(s, &req, on_tok, nullptr);
        engine.emitContinuitySnap(stderr, i);
        fprintf(stderr, "CONTINUITY_POINT=AFTER_GEN%u_GENERATE ok=%d\n", i,
                okGen);
        if (!okGen) {
            /* Authority path: no retry. Diagnostic-only retries forbidden for PASS. */
            printf("ENDURANCE_FAIL at=%u GENERATION_RETRIES=0\n", i);
            break;
        }
        const uint64_t got = d2_session_last_generated(s);
        toks += got;
        ++ok;
        if (!haveA) {
            wA = Wup(engine);
        ...
        if ((i + 1u) % 10u == 0u || reqs <= 10u) {
            printf("ENDURANCE_PROGRESS %u/%u toks=%llu linked=%u packed=%u\n",
                   i + 1u, reqs, (unsigned long long)toks,
                   engine.ssVkDecodeBindWindow() ? 1u : 0u,
                   engine.EnsurePersistentDecodeBinding() ? 1u : 0u);
            fflush(stdout);
        }
    }
    ...
    const int dual =
        (engine.vulkanDeviceCount() >= 2 &&
         ((gf.forwardSlot[0] > 0 && gf.forwardSlot[1] > 0) ||
          (ds.forwardCallsGpu0 > 0 && ds.forwardCallsGpu1 > 0)))
            ? 1
            : 0;
    ...
    WriteReceipt(receipt, 1, conj ? 1 : 0, reqs, ok, toks, dual, gpu_res, 0,
                 wup_d, 0);
    printf("ENDURANCE=%s ok=%u/%u toks=%llu dual=%d wup_d=%llu "
           "GENERATION_RETRIES=0 PROMOTE=0\n",
