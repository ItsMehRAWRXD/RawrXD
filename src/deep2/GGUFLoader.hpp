...
struct ModelMetadata {
    ...
    uint32_t ssmStateSize   = 0;   // SSM state dimension (e.g. 128)
    uint32_t ssmConvKernel  = 0;   // Conv1D kernel size (e.g. 4)
    uint32_t ssmInnerSize   = 0;   // SSM inner/projection dimension (e.g. 7680)
    uint32_t ssmGroupCount  = 0;   // Number of SSM groups (e.g. 8)
    uint32_t ssmTimeStepRank = 0;  // Rank of dt projection (e.g. 96)
    ...
    void Print() const {
        ...
        if (ssmStateSize > 0) {
            printf("[GGUF] SSM: state=%u conv=%u inner=%u groups=%u dtRank=%u\n",
                   ssmStateSize, ssmConvKernel, ssmInnerSize,
                   ssmGroupCount, ssmTimeStepRank);
        }
    }
};
