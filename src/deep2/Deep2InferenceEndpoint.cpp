        ...
        std::function<void(const std::string&)> scan = [&](const std::string& dir) {
            ...
            do {
                ...
                if (findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
                    ...
                    if (name.size() > 5 && 
                        _stricmp(name.substr(name.size() - 5).c_str(), ".gguf") == 0) {
                        ModelInfo info;
                        info.id = name.substr(0, name.size() - 5);
                        info.name = info.id;
                        info.path = fullPath;
                        info.format = "GGUF";
                        ...
                        FILE* fp = nullptr;
                        if (fopen_s(&fp, fullPath.c_str(), "rb") == 0 && fp) {
                            char magic[4];
                            if (fread(magic, 1, 4, fp) == 4 && 
                                memcmp(magic, "GGUF", 4) == 0) {
                                // Basic GGUF v3 header parse for metadata
                                uint32_t version = 0;
                                fread(&version, 4, 1, fp);
                                uint64_t tensorCount = 0, kvCount = 0;
                                fread(&tensorCount, 8, 1, fp);
                                fread(&kvCount, 8, 1, fp);
                                
                                // Scan KV pairs for architecture and context length
                                for (uint64_t i = 0; i < kvCount && i < 256; ++i) {
                                    uint64_t keyLen = 0;
                                    fread(&keyLen, 8, 1, fp);
                                    if (keyLen > 256) { fseek(fp, (long)keyLen, SEEK_CUR); }
                                    else {
                                        std::string key(keyLen, '\0');
                                        fread(&key[0], 1, (size_t)keyLen, fp);
                                        ...
                                        if (key == "general.architecture") {
                                            ...
                                            fread(&val[0], 1, (size_t)valLen, fp);
                                            ...
                                            } else { fseek(fp, 8, SEEK_CUR); }
                                        } else {
                                            // Skip value based on type
                                            static const size_t typeSizes[] = {8,8,4,8,8,4,8,8,8,8,8};
                                            if (type < 11) fseek(fp, (long)typeSizes[(size_t)type], SEEK_CUR);
                                        }
                                    }
                                }
                            }
                        ...
                        info.capabilities = {"generate", "chat"};
    ...
    std::string HandleChatCompletions(const std::string& body, SOCKET clientSocket) {
        ...
        try {
            ...
            if (engine_ && engine_->isModelLoaded()) {
                ...
                double latencyMs = std::chrono::duration<double, std::milli>(
                    endTime - startTime).count();
                result.tokensPerSecond = (latencyMs > 0) ? (result.tokensGenerated * 1000.0 / latencyMs) : 0.0;
