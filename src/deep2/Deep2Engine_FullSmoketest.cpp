...
// Extern declarations for Deep2 kernel functions (defined in Deep2Engine.cpp)
extern "C" {
    int Deep2_HasAVX2();
    int Deep2_HasAVX512();
...
TestResult Test_Autonomous_GoalSystem() {
    ...
    Deep2::GoalManager goals;
    
    Deep2::Goal g;
    g.id = "1";
    g.name = "Load model and generate text";
    g.description = "Load model and generate text";
    g.priority = Deep2::Priority::High;
    g.status = Deep2::Goal::Status::Pending;
    
    goals.CreateGoal(g.name, g.description, g.priority);
    goals.SetProgress("1", 0.5f);
    goals.SetStatus("1", Deep2::Goal::Status::Completed);
...
TestResult Test_Autonomous_TrailBrake() {
    ...
    Deep2::TrailBrake brake;
    Deep2::TrailBrakeConfig config;
    
    brake.Initialize(config);
    
    bool shouldProceed = brake.ShouldProceed();
    
    printf("    Should proceed: %s\n", shouldProceed ? "PASS" : "THROTTLE");
    
    char details[256];
    snprintf(details, sizeof(details), "Proceed:%s", shouldProceed ? "OK" : "THROTTLE");
    result.details = details;
    
    TEST_ASSERT(shouldProceed, "TrailBrake blocked execution");
