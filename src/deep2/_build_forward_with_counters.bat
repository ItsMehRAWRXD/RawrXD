...
echo === Recompiling Deep2Engine.cpp with Batch 21 instrumentation ===
cl.exe %CLFLAGS% /c Deep2Engine.cpp
if %ERRORLEVEL% NEQ 0 (
    echo Deep2Engine COMPILE FAILED
    exit /b %ERRORLEVEL%
)

echo === Recompiling QuantKernelRegistry.cpp with Batch 21 counters ===
...
if %ERRORLEVEL% NEQ 0 (
    echo Forward test COMPILE FAILED
    exit /b %ERRORLEVEL%
)
