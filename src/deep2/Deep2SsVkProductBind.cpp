...
bool SsVkProductBind::dispatchQ2K(const SsVkQ2KRequest& in) noexcept {
    ...
    if (rc != 0 || !proofAuthoritative(p)) {
        ...
        ::fprintf(stderr,
            "BATCH2_DISPATCH_FAIL rc=%d linked=%u packed=%u overlap=%u "
            "agg=%u g0=%u g1=%u compact=%u parity=%u\n",
            rc, p.productLinked, p.packedQ2KLive, p.materialSameTokenOverlap,
            p.aggregateBwAuthority, p.gpu0RealForwards, p.gpu1RealForwards,
            p.compactMergeReal, p.outputParity);
        return false;
    }
