...
/* Call once on first QKV shared-x hit inside production decode. */
inline void NoteQkvLiveTune(uint32_t rows, bool parityOk, uint64_t wallUs) noexcept {
    if (!parityOk) return;
    const int cur = QkvWinnerRows().load();
    if (cur < 0) {
        QkvWinnerRows().store((int)rows);
        QkvParity().store(1);
        std::printf("QKV_LIVE_TUNE ROWS=%u PARITY=1 WALL_US=%llu KEEP_GOING=1\n",
...
inline void NoteKvaLiveTune(uint32_t rows, bool parityOk, uint64_t wallUs) noexcept {
    if (!parityOk) return;
    if (KvaWinnerRows().load() < 0) {
        KvaWinnerRows().store((int)rows);
        KvaParity().store(1);
        std::printf("KVA_LIVE_TUNE ROWS=%u PARITY=1 WALL_US=%llu KEEP_GOING=1\n",
                    rows, (unsigned long long)wallUs);
    }
    (void)wallUs;
}
...
inline void EmitActualE2EFooter(uint64_t genTok, uint64_t wallNs,
                                uint64_t budgetNs, int teardownOk) noexcept {
    ...
    std::printf("TERMINATION_CLASS=%s TEARDOWN_WITNESS=%d\n",
                genTok > 0 ? "COMPLETED" : "FAILED_BEFORE_STREAM", teardownOk);
    const int within = (genTok > 0 && wallNs > 0 && wallNs <= budgetNs) ? 1 : 0;
    std::printf("STREAM_COMPLETE=%u ACTUAL_E2E_GENERATION_001=%s\n",
                genTok > 0 ? 1u : 0u, genTok > 0 ? "PASS" : "FAIL");
