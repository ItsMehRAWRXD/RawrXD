...
public:
    ImmutableAuditLog(const std::string& logPath = "audit.log") 
        ...
        licensed = LICENSE_CHECK(RawrXD::License::FeatureID::ImmutableAuditLogs);
    ...
    bool logEvent(const std::string& event, const std::string& actor, 
        ...
        if (!logChain.empty()) {
            ...
            static const char genesis[] = "RawrXD_AuditLog_Genesis";
            uint8_t ghash[32];
            RawrXD::License::AntiTampering::sha256(
                reinterpret_cast<const uint8_t*>(genesis), sizeof(genesis) - 1, ghash);
            std::stringstream gs;
            gs << std::hex << std::setfill('0');
            for (int i = 0; i < 32; ++i) gs << std::setw(2) << static_cast<unsigned>(ghash[i]);
            entry.previousHash = gs.str();
        }
...
private:
    // Compute SHA-256 hash of entry (tamper-evident chain)
    std::string computeHash(const AuditEntry& entry) const {
        ...
        uint8_t hash[32];
        RawrXD::License::AntiTampering::sha256(buf, len, hash);

        std::stringstream hexStr;
        hexStr << std::hex << std::setfill('0');
        for (int i = 0; i < 32; ++i)
            hexStr << std::setw(2) << static_cast<unsigned>(hash[i]);
        return hexStr.str();
    }
