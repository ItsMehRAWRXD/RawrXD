...
UnifiedResult UnifiedHotpatchManager::patchGGUFMetadata(const QString& key, const QVariant& value)
{
    ...
    // Convert metadata value to appropriate byte format
    QByteArray metadataBytes;
    
    if (value.type() == QVariant::Int) {
        qint32 intVal = value.toInt();
        metadataBytes = QByteArray(reinterpret_cast<const char*>(&intVal), sizeof(intVal));
    } else if (value.type() == QVariant::Double) {
        double doubleVal = value.toDouble();
        metadataBytes = QByteArray(reinterpret_cast<const char*>(&doubleVal), sizeof(doubleVal));
    } else {
        metadataBytes = value.toString().toUtf8();
    }
    
    // Patch metadata key-value in GGUF file via byte-level hotpatcher
    BytePatch metaPatch;
    metaPatch.name = QString("metadata_%1").arg(key);
    metaPatch.length = metadataBytes.size();
    
    if (!m_byteHotpatch->addPatch(metaPatch)) {
        return UnifiedResult::failureResult("patchGGUFMetadata", 
            QString("Failed to patch metadata key: %1").arg(key), PatchLayer::Byte);
    }
    
    m_stats.totalPatchesApplied++;
    m_stats.totalBytesModified += metadataBytes.size();
    
    emit patchApplied(QString("metadata_%1").arg(key), PatchLayer::Byte);
    
    qInfo() << "[UnifiedHotpatch] Patched GGUF metadata:" << key << "=" << value;
    return UnifiedResult::successResult("patchGGUFMetadata", PatchLayer::Byte,
        QString("Patched GGUF metadata: %1").arg(key));
}
