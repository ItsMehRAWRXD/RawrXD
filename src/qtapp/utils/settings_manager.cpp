...
void SettingsManager::initializeDefaults() {
    ...
    m_settings["compression"] = QJsonObject{
        {"preferred_type", 2},  // BRUTAL_GZIP
        {"enable_stats", true},
        {"max_decomp_bytes", 10737418240}  // 10GB
    };
}
...
SettingsManager::CompressionSettings SettingsManager::compressionSettings() const {
    CompressionSettings settings;
    settings.preferred_type = value("compression/preferred_type", 2).toInt();
    settings.enable_stats = value("compression/enable_stats", true).toBool();
    settings.max_decomp_bytes = value("compression/max_decomp_bytes", 10737418240ULL).toULongLong();
    return settings;
}
