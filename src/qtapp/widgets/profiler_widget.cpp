...
void ProfilerWidget::analyzeMemoryLeaks() {
    ...
    for (const MemoryAllocation& alloc : m_allocations) {
        if (alloc.freed) continue;  // Not a leak
        ...
        item->setText(3, "LEAK");
        item->setForeground(3, QColor("#f44747"));
        
        // Add stack trace as children
        for (const QString& frame : alloc.stackTrace) {
...
void ProfilerWidget::updateStatsPanel() {
    ...
    quint64 leaks = 0;
    for (const MemoryAllocation& a : m_allocations) {
        if (!a.freed) leaks++;
    }
...
QVector<MemoryAllocation> ProfilerWidget::getAllocations() const {
    QMutexLocker lock(const_cast<QMutex*>(&m_dataMutex));
