...
@@NotStats:
    ...
    ; EAX = pattern type, XMM0 = confidence
    mov r12d, eax               ; Save pattern type
    
    ; Check for match
    test eax, eax
    jz @@NoMatch
    inc qword ptr [qwTotalMatches]
@@NoMatch:
    ...
    cmp eax, 8
    ja @@UseUnknown
    lea rcx, [PatternNames]
    mov r8, [rcx + rax*8]       ; Pattern name string
    jmp @@FormatResult
@@UseUnknown:
    lea r8, [szPatternUnknown]
    
@@FormatResult:
    ; Convert confidence to integer parts (0.88 -> 88)
    cvtsd2si eax, xmm0
    imul eax, 100
    mov r9d, eax                ; Whole part * 100
    
    ; Format result
    lea rcx, [SendBuffer]
    lea rdx, [szResultTmpl]
    ; r8 already has pattern name
    ; r9d has confidence * 100
    mov eax, r9d
    xor edx, edx
    mov ecx, 100
    div ecx                     ; EAX = whole, EDX = frac
    mov r9d, eax
    mov [rsp+32], rdx           ; Fractional part
    mov dword ptr [rsp+40], 1   ; Line number (placeholder)
    mov dword ptr [rsp+48], 1   ; Priority (placeholder)
    lea rcx, [SendBuffer]
    lea rdx, [szResultTmpl]
    call wsprintfA
    jmp @@Done
