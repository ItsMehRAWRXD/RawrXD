...
TestResult TestStackAlignment() {
    ...
    for (int i = 0; i < 100; i++) {
        DecodeStatus status = ReferenceDecoder_Decode(ArchType::ARM_64, nop, 4, &instr);
        if (status != DecodeStatus::SUCCESS) {
            result.passed = false;
...
TestResult TestNullPointerHandling() {
    ...
    __try {
        DecodeStatus status = ReferenceDecoder_Decode(ArchType::ARM_64, nop, 4, nullptr);
...
TestResult TestTruncatedInstruction() {
    ...
    DecodeStatus status = ReferenceDecoder_Decode(ArchType::ARM_64, partial, 2, &instr);
...
TestResult TestAllArchitectures() {
    ...
    for (int i = 0; i < 10; i++) {
        memset(&instr, 0, sizeof(instr));
        DecodeStatus status = ReferenceDecoder_Decode(archTypes[i], nop, 4, &instr);
