...
extern "C" __declspec(dllexport) bool OmegaBridge_ExecutePowerShell(const char* command, char* output, uint32_t outputSize) {
    if (!RawrXD::Bridge::g_bridgeExecutor || !command || !output || outputSize == 0) {
        return false;
    }
    return RawrXD::Bridge::g_bridgeExecutor->ExecuteCommand(command, output, outputSize);
}

extern "C" __declspec(dllexport) bool OmegaBridge_LoadModule(const char* moduleName) {
    if (!RawrXD::Bridge::g_bridgeExecutor || !moduleName) return false;
    return RawrXD::Bridge::g_bridgeExecutor->LoadModule(moduleName);
}
...
extern "C" __declspec(dllexport) bool OmegaBridge_InvokeModule(const char* moduleName, const char* function, char* output, uint32_t outputSize) {
    if (!RawrXD::Bridge::g_bridgeExecutor || !moduleName || !function || !output || outputSize == 0) {
        return false;
    }
    return RawrXD::Bridge::g_bridgeExecutor->InvokeModuleFunction(moduleName, function, output, outputSize);
}
