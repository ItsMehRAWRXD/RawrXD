    ...
    while (step < m_config.maxSteps) {
        ...
        if (response.hasToolCall) {
            ...
            if (m_config.autoVerify && !m_config.dryRun &&
                ...
                // Gate: RAWRXD_AGENT_TOOL_AUTHORITY_001 — route diagnostics
                // through the authority, not AgentToolHandlers directly.
                RawrXD::Agent::AgentToolRequest diagReq;
                diagReq.tool_name = "get_diagnostics";
                diagReq.args = diagArgs;
                diagReq.caller_surface = "agentic_loop";
                auto diagAuthResult = RawrXD::Agent::AgentToolAuthority::Dispatch(diagReq);

                if (diagAuthResult.success && !diagAuthResult.output.empty() &&
                    diagAuthResult.output != "No diagnostics") {
                    // Inject diagnostics as additional context
                    json diagMsg;
                    diagMsg["role"] = "system";
                    diagMsg["content"] = "Post-edit diagnostics for " + toolResult.filePath +
                                         ":\n" + diagAuthResult.output;
...
ToolCallResult BoundedAgentLoop::DispatchTool(const std::string& name, const json& args) {
    // Gate: RAWRXD_AGENT_TOOL_AUTHORITY_001 — route through the single
    // AgentToolAuthority instead of calling AgentToolHandlers directly.
    // The AgenticLoop surface reach counter is incremented by the authority.
    RawrXD::Agent::AgentToolRequest req;
    req.tool_name = name;
    req.args = args;
    req.caller_surface = "agentic_loop";

    auto authResult = RawrXD::Agent::AgentToolAuthority::Dispatch(req);

    // Adapt AgentToolResult → ToolCallResult
    if (authResult.success) {
        return ToolCallResult::Ok(authResult.output);
    }
    // Map unknown-tool errors to NotFound, everything else to ExecutionError
    if (authResult.output.find("Unknown tool:") != std::string::npos)
        return ToolCallResult::NotFound(name);
    return ToolCallResult::Error(authResult.output.empty() ? "tool failed" : authResult.output);
}
