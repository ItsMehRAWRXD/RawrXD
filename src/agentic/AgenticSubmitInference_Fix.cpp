    ...
    const RuntimeConfig& runtime)
{
    ...
    for (int step = 0; step < maxIterations; ++step) {
        ...
        for (size_t i = 0; i < llmResult.tool_calls.size(); ++i) {
            ...
            // Gate: RAWRXD_AGENT_TOOL_AUTHORITY_001 — route through the
            // single AgentToolAuthority instead of AgentToolHandlers directly.
            Agent::ToolCallResult toolResult;
            try {
                RawrXD::Agent::AgentToolRequest req;
                req.tool_name = toolCall.first;
                req.args = toolCall.second;
                req.caller_surface = "agentic_loop";

                auto authResult = RawrXD::Agent::AgentToolAuthority::Dispatch(req);
                if (authResult.success) {
                    toolResult = Agent::ToolCallResult::Ok(authResult.output);
                } else {
                    toolResult = Agent::ToolCallResult::Error(
                        authResult.output.empty() ? "tool failed" : authResult.output);
                }
            } catch (const std::exception& e) {
