...
json AgenticChatSession::ExecuteTool(const std::string& name, const json& args)
{
    ...
    // Build a ToolCallResult-compatible JSON from the normalized result
    json result;
    result["success"] = authResult.success;
    result["output"] = authResult.output;
    if (!authResult.success) {
        result["error"] = authResult.output;
        result["outcome"] = "ExecutionError";
    } else {
        result["outcome"] = "Success";
    }
    if (authResult.elapsed_ms > 0) {
        result["duration_ms"] = static_cast<int64_t>(authResult.elapsed_ms);
    }
    return result;
}
