    ...
    : m_registry(AgentToolRegistry::Instance())
{
    ...
    // Register this registry as the single tool-dispatch authority (CAS).
    // Gate: RAWRXD_AGENT_TOOL_AUTHORITY_001 — AgenticLoop surface.
    AgentToolAuthority::Register(nullptr, &m_registry);
    ...
    StepCallback on_step)
{
    ...
    for (size_t i = 0; i < result.tool_calls.size(); ++i) {
        ...
        // Execute through the single authority (normalized contract).
        // Gate: RAWRXD_AGENT_TOOL_AUTHORITY_001 — AgenticLoop surface reach.
        AgentToolRequest toolReq;
        toolReq.tool_name = name;
        toolReq.args = args;
        toolReq.caller_surface = "agentic_loop";
        toolReq.session_id = session.session_id;

        auto execStart = std::chrono::high_resolution_clock::now();
        AgentToolResult authResult = AgentToolAuthority::Dispatch(toolReq);
        auto execEnd = std::chrono::high_resolution_clock::now();

        // Adapt normalized result to legacy ToolExecResult for session tracking
        ToolExecResult execResult;
        execResult.success = authResult.success;
        execResult.output = authResult.output;
        execResult.exit_code = authResult.exit_code;
        execResult.elapsed_ms = authResult.elapsed_ms;
