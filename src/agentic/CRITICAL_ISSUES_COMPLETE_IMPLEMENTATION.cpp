...
bool AIModelCaller_LoadWeights(TransformerModel* model, const char* path)
{
    ...
    // Check Magic: "RAWR"
    uint32_t magic;
    if (fread(&magic, sizeof(magic), 1, f) != 1 || magic != 0x52415752) {
        // Fallback: If not our custom format, maybe try GGUF? 
        // For this implementation we require our RawrXD format which is just a flat dump of tensors.
        fclose(f);
        return false;
    }
    
    // Helper to read tensor data
    auto read_tensor = [&](ggml_tensor* tensor) {
        if (!tensor) return false;
        size_t size = ggml_nbytes(tensor);
        if (fread(tensor->data, 1, size, f) != size) return false;
        return true;
    };
    
    // Read Embeddings
    if (!read_tensor(model->tok_embeddings)) goto error;
    if (!read_tensor(model->norm)) goto error;
    if (!read_tensor(model->output)) goto error;
    
    // Read Layers
    for (int i = 0; i < model->n_layers; i++) {
        LayerWeights& l = model->layers[i];
        if (!read_tensor(l.attn_norm)) goto error;
        if (!read_tensor(l.wq)) goto error;
        if (!read_tensor(l.wk)) goto error;
        if (!read_tensor(l.wv)) goto error;
        if (!read_tensor(l.wo)) goto error;
        if (!read_tensor(l.ffn_norm)) goto error;
        if (!read_tensor(l.w1)) goto error;
        if (!read_tensor(l.w2)) goto error;
        if (!read_tensor(l.w3)) goto error;
    }
...
=======
    ...
    // Initialize Tensor Structure (Weights must be loaded via LoadWeights)
    // Tensors are allocated in compute context but data is zeroed or uninitialized until loaded.
