...
// -----------------------------------------------------------------------
// Legacy adapter wrappers — absorb RawrXD_ToolRegistry capabilities
// -----------------------------------------------------------------------
ToolExecResult HandleCodeEdit(const json& args) {
    auto& legacy = RawrXD::Agent::ToolRegistry::Instance();
    std::string output;
    auto result = legacy.Execute("code_edit", args.dump(), output);
    if (result == RawrXD::Agent::ToolResult::Success) {
        return ToolExecResult::ok(output);
    }
    return ToolExecResult::error("[code_edit] Legacy adapter failed: " + output);
}

ToolExecResult HandleBuildProject(const json& args) {
    auto& legacy = RawrXD::Agent::ToolRegistry::Instance();
    std::string output;
    auto result = legacy.Execute("build_project", args.dump(), output);
    if (result == RawrXD::Agent::ToolResult::Success) {
        return ToolExecResult::ok(output);
    }
    return ToolExecResult::error("[build_project] Legacy adapter failed: " + output);
}

ToolExecResult HandleStaticAnalysis(const json& args) {
    auto& legacy = RawrXD::Agent::ToolRegistry::Instance();
    std::string output;
    auto result = legacy.Execute("static_analysis", args.dump(), output);
    if (result == RawrXD::Agent::ToolResult::Success) {
        return ToolExecResult::ok(output);
    }
    return ToolExecResult::error("[static_analysis] Legacy adapter failed: " + output);
}

ToolExecResult HandleGitOperation(const json& args) {
    auto& legacy = RawrXD::Agent::ToolRegistry::Instance();
    std::string output;
    auto result = legacy.Execute("git_operation", args.dump(), output);
    if (result == RawrXD::Agent::ToolResult::Success) {
        return ToolExecResult::ok(output);
    }
    return ToolExecResult::error("[git_operation] Legacy adapter failed: " + output);
}

// -----------------------------------------------------------------------
// New handlers: SSA lift, byte patch, memory patch
// -----------------------------------------------------------------------
ToolExecResult HandleSSALift(const json& args) {
    std::string binary_path = args.value("binary_path", "");
    std::string function_start_str = args.value("function_start", "");
    if (binary_path.empty() || function_start_str.empty())
        return ToolExecResult::error("Missing required parameters: binary_path, function_start");

    uint64_t function_start = 0;
    sscanf(function_start_str.c_str(), "%llx", &function_start);
    if (function_start == 0)
        return ToolExecResult::error("Invalid function_start address: " + function_start_str);

    RawrXD::ReverseEngineering::RawrCodex codex;
    if (!codex.LoadBinary(binary_path)) {
        return ToolExecResult::error("Failed to load binary: " + binary_path);
    }

    auto result = codex.LiftToSSA(function_start);
    if (!result.success) {
        return ToolExecResult::error("SSA lift failed for function at " + function_start_str);
    }

    std::ostringstream oss;
    oss << "[ssa_lift] Success for " << function_start_str << "\n";
    oss << "Variables: " << result.totalVars << "\n";
    oss << "Instructions: " << result.instructions.size() << "\n";
    oss << "PHI nodes: " << result.phiNodes.size() << "\n";
    return ToolExecResult::ok(oss.str());
}

ToolExecResult HandleBytePatch(const json& args) {
    std::string file = args.value("file", "");
    std::string pattern_hex = args.value("pattern_hex", "");
    std::string replacement_hex = args.value("replacement_hex", "");
    if (file.empty() || pattern_hex.empty() || replacement_hex.empty())
        return ToolExecResult::error("Missing required parameters: file, pattern_hex, replacement_hex");

    json wrapped;
    wrapped["layer"] = "byte";
    wrapped["target"] = file;
    wrapped["data"] = pattern_hex + ":" + replacement_hex;
    return HandleApplyHotpatch(wrapped);
}

ToolExecResult HandleMemoryPatch(const json& args) {
    std::string address_hex = args.value("address_hex", "");
    std::string data_hex = args.value("data_hex", "");
    if (address_hex.empty() || data_hex.empty())
        return ToolExecResult::error("Missing required parameters: address_hex, data_hex");

    json wrapped;
    wrapped["layer"] = "memory";
    wrapped["target"] = address_hex;
    wrapped["data"] = data_hex;
    return HandleApplyHotpatch(wrapped);
}
...
void AgentToolRegistry::InitDescriptors() {
    ...
    // code_edit
    setParam("code_edit", "path", "string", "File path to edit");
    setParam("code_edit", "old_string", "string", "Text to find");
    setParam("code_edit", "new_string", "string", "Replacement text");

    // build_project
    setParamWithDefault("build_project", "target", "string", "Build target (default: all)", "all");
    setParamWithDefault("build_project", "config", "string", "Build configuration", "Release");

    // static_analysis
    setParam("static_analysis", "file", "string", "Source file to analyze");
    setParamWithDefault("static_analysis", "rules", "string", "Rule set or config file", "");

    // git_operation
    setParam("git_operation", "command", "string", "Git subcommand to execute");
    setParamWithDefault("git_operation", "args", "string", "Additional arguments", "");

    // ssa_lift
    setParam("ssa_lift", "binary_path", "string", "Path to binary file");
    setParam("ssa_lift", "function_start", "string", "Hex address of function start (e.g. 0x140001000)");

    // byte_patch
    setParam("byte_patch", "file", "string", "Target file path");
    setParam("byte_patch", "pattern_hex", "string", "Hex-encoded bytes to find");
    setParam("byte_patch", "replacement_hex", "string", "Hex-encoded replacement bytes");

    // memory_patch
    setParam("memory_patch", "address_hex", "string", "Hex memory address (e.g. 0x7ff12345)");
    setParam("memory_patch", "data_hex", "string", "Hex-encoded patch data");
    ...
    RegisterHandler("ssa_lift",        HandleSSALift);
    RegisterHandler("byte_patch",      HandleBytePatch);
    RegisterHandler("memory_patch",    HandleMemoryPatch);

    // Legacy adapters — absorb RawrXD_ToolRegistry capabilities
    RegisterLegacyAdapter("code_edit",       HandleCodeEdit);
    RegisterLegacyAdapter("build_project",   HandleBuildProject);
    RegisterLegacyAdapter("static_analysis", HandleStaticAnalysis);
    RegisterLegacyAdapter("git_operation",   HandleGitOperation);
}
...
ToolExecResult AgentToolRegistry::Dispatch(const std::string& tool_name, const json& args) {
    ...
    // Fallback to legacy adapter if no direct handler
    bool usedLegacy = false;
    if (!td.handler) {
        auto legacyIt = m_legacyAdapters.find(tool_name);
        if (legacyIt != m_legacyAdapters.end()) {
            usedLegacy = true;
        } else {
            GetObs().logError(kRegistryComponent, "Dispatch: no handler or legacy adapter", {{"tool", tool_name}});
            return ToolExecResult::error("No handler or legacy adapter registered for tool: " + tool_name);
        }
    }
    ...
    if (td.handler) {
        ...
        result = m_legacyAdapters[tool_name](args);
    }
    auto end = std::chrono::high_resolution_clock::now();
...
void AgentToolRegistry::RegisterLegacyAdapter(
    const std::string& tool_name,
    std::function<ToolExecResult(const json&)> adapter)
{
    auto it = m_nameIndex.find(tool_name);
    if (it != m_nameIndex.end()) {
        m_legacyAdapters[tool_name] = std::move(adapter);
    }
}

AgentToolResult AgentToolRegistry::Dispatch(const AgentToolRequest& request) {
    // Parse caller surface string to enum for metrics tracking
    AgentToolSurface surface = AgentToolSurface::Unknown;
    if (request.caller_surface == "cli") surface = AgentToolSurface::CLI;
    else if (request.caller_surface == "gui") surface = AgentToolSurface::GUI;
    else if (request.caller_surface == "headless") surface = AgentToolSurface::Headless;
    else if (request.caller_surface == "swarm") surface = AgentToolSurface::Swarm;
    else if (request.caller_surface == "agentic_loop") surface = AgentToolSurface::AgenticLoop;

    // Increment reach counter for this surface
    switch (surface) {
        case AgentToolSurface::CLI:           ++m_authorityMetrics.reach_cli; break;
        case AgentToolSurface::GUI:           ++m_authorityMetrics.reach_gui; break;
        case AgentToolSurface::Headless:      ++m_authorityMetrics.reach_headless; break;
        case AgentToolSurface::Swarm:         ++m_authorityMetrics.reach_swarm; break;
        case AgentToolSurface::AgenticLoop:   ++m_authorityMetrics.reach_agentic_loop; break;
        default: break;
    }

    // Unknown tool detection before dispatch
    if (m_nameIndex.find(request.tool_name) == m_nameIndex.end()) {
        ++m_authorityMetrics.unknown_tool_dispatches;
        return AgentToolResult::error("Unknown tool: " + request.tool_name);
    }

    // Validate args using schema
    std::string validationError;
    if (!ValidateArgs(request.tool_name, request.args, validationError)) {
        ++m_authorityMetrics.validation_failures;
        return AgentToolResult::error("Validation failed: " + validationError);
    }

    // Attempt direct handler dispatch first
    auto it = m_nameIndex.find(request.tool_name);
    auto& td = m_tools[it->second];
    ToolExecResult execResult;
    bool usedLegacy = false;

    if (td.handler) {
        execResult = td.handler(request.args);
    } else {
        // Fallback to legacy adapter
        auto legacyIt = m_legacyAdapters.find(request.tool_name);
        if (legacyIt != m_legacyAdapters.end()) {
            execResult = legacyIt->second(request.args);
            usedLegacy = true;
        } else {
            return AgentToolResult::error("No handler or legacy adapter for: " + request.tool_name);
        }
    }

    ++m_authorityMetrics.total_dispatches;

    AgentToolResult result;
    result.success    = execResult.success;
    result.output     = execResult.output;
    result.exit_code  = execResult.exit_code;
    result.elapsed_ms = execResult.elapsed_ms;
    result.kind       = request.kind;
    result.tool_name  = request.tool_name;

    // Log to observability
    if (!execResult.success) {
        GetObs().logWarn(kRegistryComponent, "Normalized dispatch failed", {
        ...
        GetObs().logDebug(kRegistryComponent, "Normalized dispatch succeeded", {
            {"tool", request.tool_name},
            {"surface", request.caller_surface},
            {"used_legacy", usedLegacy},
