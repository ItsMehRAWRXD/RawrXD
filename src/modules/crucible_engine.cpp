...
CrucibleStageResult CrucibleEngine::stage_SP_AcquireTarget() {
    ...
    if (m_config.useSyntheticTarget) {
        ...
        // Real target: look up function in loaded modules
        // Strategy: PE export table walking (Windows) or dlsym (POSIX)
        if (m_config.targetFunctionName.empty()) {
            return CrucibleStageResult::fail(CrucibleStage::SP_AcquireTarget,
                "Non-synthetic target requires targetFunctionName in config");
        }
        ...
        if (dbgHelp) {
            ...
            auto pSymInit = (PFN_SymInitialize)GetProcAddress(dbgHelp, "SymInitialize");
            auto pSymFromName = (PFN_SymFromName)GetProcAddress(dbgHelp, "SymFromName");

            if (pSymInit && pSymFromName) {
                HANDLE hProc = GetCurrentProcess();
                pSymInit(hProc, nullptr, TRUE);

                // SYMBOL_INFO is variable-size — allocate on stack buffer
                alignas(8) char symBuf[sizeof(SYMBOL_INFO) + MAX_SYM_NAME];
                SYMBOL_INFO* sym = reinterpret_cast<SYMBOL_INFO*>(symBuf);
                memset(sym, 0, sizeof(symBuf));
                sym->SizeOfStruct = sizeof(SYMBOL_INFO);
                sym->MaxNameLen = MAX_SYM_NAME;

                if (pSymFromName(hProc, m_config.targetFunctionName.c_str(), sym)) {
                    funcAddr = reinterpret_cast<void*>(sym->Address);
                    funcSize = static_cast<size_t>(sym->Size);
                    if (funcSize == 0) funcSize = 256; // Default if symbol doesn't report size
                }
            }
            FreeLibrary(dbgHelp);
        }

        // Fallback: Walk PE export table of all loaded modules
        if (!funcAddr) {
            HMODULE hMods[256];
            DWORD cbNeeded = 0;
            HANDLE hProc = GetCurrentProcess();
            ...
            auto pEnum = psapi ? (PFN_EnumProcessModules)GetProcAddress(psapi, "EnumProcessModules") : nullptr;

            if (pEnum && pEnum(hProc, hMods, sizeof(hMods), &cbNeeded)) {
                DWORD modCount = cbNeeded / sizeof(HMODULE);
                for (DWORD m = 0; m < modCount && !funcAddr; ++m) {
                    FARPROC proc = GetProcAddress(hMods[m], m_config.targetFunctionName.c_str());
                    if (proc) {
                        funcAddr = reinterpret_cast<void*>(proc);
                        funcSize = 256; // Conservative default
                    }
                }
            }
...
#else
        // POSIX: try dlsym with RTLD_DEFAULT
        funcAddr = dlsym(RTLD_DEFAULT, m_config.targetFunctionName);
        if (funcAddr) funcSize = 256;
#endif

        if (!funcAddr) {
            return CrucibleStageResult::fail(CrucibleStage::SP_AcquireTarget,
                "Could not locate function in loaded modules");
        }

        // Make the memory readable (should already be, but ensure)
        m_targetFunctionAddr = funcAddr;
        m_targetFunctionSize = funcSize;
        m_originalBytes.assign(
            reinterpret_cast<uint8_t*>(funcAddr),
            reinterpret_cast<uint8_t*>(funcAddr) + funcSize);

        char msg[256];
        snprintf(msg, sizeof(msg), "Acquired real target '%s' at %p (%zu bytes)",
                 m_config.targetFunctionName, funcAddr, funcSize);

        return CrucibleStageResult::pass(CrucibleStage::SP_AcquireTarget,
            msg, timer.elapsedMs(), 1);
    }
}
