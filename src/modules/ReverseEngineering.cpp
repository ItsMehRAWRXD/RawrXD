...
std::vector<NativeDisassembler::Instruction> NativeDisassembler::DisassembleX64(
    ...
    while (offset < size) {
        ...
        size_t instLength = 0;
        DecodeX64Instruction(data + offset, instLength, inst);
        ...
        else if (instLength > size - offset) {
             inst.mnemonic = "??";
             instLength = size - offset;
        }
...
void NativeDisassembler::DecodeX64Instruction(const uint8_t* bytes, size_t& length, Instruction& inst) {
    ...
    if (bytes[pos] >= 0x40 && bytes[pos] <= 0x4F) {
        ...
        rexR = (bytes[pos] & 0x04) != 0;
        rexX = (bytes[pos] & 0x02) != 0;
        rexB = (bytes[pos] & 0x01) != 0;
        pos++;
    }
    
    uint8_t opcode = bytes[pos++];
    
    const char* regs64[] = {"rax", "rcx", "rdx", "rbx", "rsp", "rbp", "rsi", "rdi", "r8", "r9", "r10", "r11", "r12", "r13", "r14", "r15"};
    const char* regs32[] = {"eax", "ecx", "edx", "ebx", "esp", "ebp", "esi", "edi", "r8d", "r9d", "r10d", "r11d", "r12d", "r13d", "r14d", "r15d"};
    
    auto getRegName = [&](int reg) { return rexW ? regs64[reg % 16] : regs32[reg % 16]; };

    // 2. Opcodes
    if (opcode == 0x90) { inst.mnemonic = "nop"; }
    else if (opcode == 0xC3) { inst.mnemonic = "ret"; inst.isCall = false; }
    else if (opcode == 0xCC) { inst.mnemonic = "int3"; }
    else if (opcode == 0x0F && bytes[pos] == 0x05) { inst.mnemonic = "syscall"; pos++; }
    ...
    // MOV ModRM (88-8B)
        ...
        std::string src = getRegName(reg);
        std::string dst = getRegName(rm); // Simplified, assumes mod=11 (reg) or proper pointer
        
        if ((modrm >> 6) == 3) { // Reg, Reg
            if (opcode & 2) inst.operands = dst + ", " + src; // MOV r/m, r
    ...
    // CALL/JMP Rel32
        ...
        if (opcode == 0xE8) inst.isCall = true; else inst.isJump = true;
        
        // Read rel32
        int32_t rel = *(int32_t*)(bytes + pos);
        pos += 4;
        
        uint64_t target = inst.address + pos + rel;
        inst.jumpTarget = target;
        inst.operands = "0x" + std::to_string(target);
    }
    // Short Jumps (7x)
    else if ((opcode & 0xF0) == 0x70) {
        inst.mnemonic = "jcc"; // Simplified
        inst.isJump = true;
        int8_t rel = (int8_t)bytes[pos++];
        inst.operands = "short " + std::to_string(rel);
    }
    // PUSH/POP Reg
    else if ((opcode & 0xF8) == 0x50) {
        inst.mnemonic = "push";
        inst.operands = getRegName((opcode & 7) + (rexB ? 8 : 0));
    }
    else if ((opcode & 0xF8) == 0x58) {
        inst.mnemonic = "pop";
        inst.operands = getRegName((opcode & 7) + (rexB ? 8 : 0));
    }
    // Immediate Move (B8+ reg)
...
std::string RECodex::AnalyzeWithAI(const std::string& assembly, const std::string& context) {
    std::stringstream report;
    report << "=== Reference Heuristic Analysis ===\n";
    
    int riskScore = 0;
    std::vector<std::string> findings;
    
    // Simple heuristic scanning
    if (assembly.find("int3") != std::string::npos) {
        riskScore += 10;
        findings.push_back("[HIGH] Debug breakpoint trap (int3) found - common in packers/anti-debug");
    }
    if (assembly.find("syscall") != std::string::npos) {
        riskScore += 20;
        findings.push_back("[CRITICAL] Direct syscall usage - typical of shellcode or EDR evasion");
    }
    if (assembly.find("fs:[") != std::string::npos || assembly.find("gs:[") != std::string::npos) {
        riskScore += 15;
        findings.push_back("[HIGH] TEB/PEB Access - possible shellcode resolving imports manually");
    }
    if (assembly.find("rdtsc") != std::string::npos) {
        riskScore += 5;
        findings.push_back("[MEDIUM] RDTSC found - possible timing attack or anti-debug");
    }
    if (assembly.find("xor") != std::string::npos && assembly.find("ret") != std::string::npos && assembly.size() < 50) {
        riskScore += 5;
        findings.push_back("[LOW] Small XOR function - possible encryption/decryption routine");
    }
    
    report << "Risk Score: " << riskScore << "/100\n";
    report << "Verdict: " << (riskScore > 30 ? "MALICIOUS" : (riskScore > 10 ? "SUSPICIOUS" : "CLEAN")) << "\n\n";
    ...
    if (findings.empty()) {
        report << "No specific threats detected via static heuristics.\n";
    }
