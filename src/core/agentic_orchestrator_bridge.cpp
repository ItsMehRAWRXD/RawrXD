...
extern "C" {
    ...
    int RawrXD_InferenceEngine_Run(void* input, void* output) { 
        ...
        // Wire to BackendOrchestrator for actual inference
        // Input is a prompt string, output is the completion buffer
        const char* prompt = static_cast<const char*>(input);
        char* completion = static_cast<char*>(output);
        
        // Use RawrXD native inference pipeline
        RawrXD::InferRequest req;
        req.id = 1;
        req.prompt = prompt;
        req.max_tokens = 256;
        req.tenant_id = "agentic_bridge";
        
        auto completion_promise = std::make_shared<std::promise<std::string>>();
        req.complete_cb = [completion_promise](const std::string& text, const std::string&) {
            completion_promise->set_value(text);
        };
        
        auto& bo = RawrXD::BackendOrchestrator::Instance();
        if (!bo.IsInitialized()) {
            strncpy(completion, "[Error] Backend not initialized", 256);
            return -3;
        }
        ...
        if (future.wait_for(std::chrono::seconds(120)) != std::future_status::ready) {
            bo.Cancel(req_id);
            strncpy(completion, "[Error] Inference timeout", 256);
            return -4;
        }
        
        std::string result = future.get();
        strncpy(completion, result.c_str(), 256);
        completion[255] = '\0';
