        ...
        JS_NewCFunction(cx, [](JSContext* c, JSValue this_val, int argc, JSValue* argv) -> JSValue {
            ...
            JS_SetPropertyStr(c, config, "get",
                JS_NewCFunction(c, [](JSContext* ctx2, JSValue this2, int ac, JSValue* av) -> JSValue {
                    ...
                    // Bridge to VSCodeExtensionAPI via public getConfiguration API
                    auto cfg = vsapi().getConfiguration(sectionStr2 ? sectionStr2 : "");
                    const char* value = nullptr;
                    if (cfg.getStringFn) {
                        value = cfg.getStringFn(fullKey.c_str(), cfg.configCtx);
                    }
            ...
            JS_SetPropertyStr(c, config, "update",
                JS_NewCFunction(c, [](JSContext* ctx2, JSValue this2, int ac, JSValue* av) -> JSValue {
                    ...
                    if (key && value) {
                        ...
                        // Bridge to VSCodeExtensionAPI config update via public API
                        auto cfg = vsapi().getConfiguration(sectionStr2 ? sectionStr2 : "");
                        if (cfg.updateFn) {
                            cfg.updateFn(fullKey.c_str(), value, target, cfg.configCtx);
                        }
                    }
