...
// ---------------------------------------------------------------------------
static bool g_avx2_supported = false;  // Trivial, safe
static bool g_avx512_supported = false;  // Trivial, safe
static bool g_features_detected = false;  // Trivial, safe

// LAZY SINGLETON PATTERN: Avoid SIOF - std::atomic with non-trivial constructor
#define LAZY_ATOMIC(name, type, init) \
    inline std::atomic<type>& Get##name() { \
        static std::atomic<type>* inst = new std::atomic<type>(init); \
        return *inst; \
    }

LAZY_ATOMIC(StubFeatureDetectCalls, uint32_t, 0)
#define g_stub_feature_detect_calls GetStubFeatureDetectCalls()
LAZY_ATOMIC(StubRequestHookCalls, uint32_t, 0)
#define g_stub_request_hook_calls GetStubRequestHookCalls()
LAZY_ATOMIC(StubStreamChunkCalls, uint32_t, 0)
#define g_stub_stream_chunk_calls GetStubStreamChunkCalls()
LAZY_ATOMIC(StubReasoningCalls, uint32_t, 0)
#define g_stub_reasoning_calls GetStubReasoningCalls()
LAZY_ATOMIC(StubTransformCalls, uint32_t, 0)
#define g_stub_transform_calls GetStubTransformCalls()
LAZY_ATOMIC(StubIntegrityCalls, uint32_t, 0)
#define g_stub_integrity_calls GetStubIntegrityCalls()
LAZY_ATOMIC(StubPatternMatchCalls, uint32_t, 0)
#define g_stub_pattern_match_calls GetStubPatternMatchCalls()
...
extern "C"
{
    ...
    MasmOperationResult masm_memory_atomic_exchange(void* target, uint64_t new_val, uint64_t* old_val)
    {
        ...
        if (old_val)
            *old_val = static_cast<uint64_t>(prev);
#else
        ...
        *found_count = 0;
        ...
        for (int64_t i = pat_len - 1, j = pat_len - 1; i >= 0; --i)
        {
            ...
            else
            {
                if (i < j)
                    j = i;
                while (j >= 0 && pat[j] == pat[pat_len - 1 - (i - j)])
                    --j;
                suffix[i] = i - j;
            }
        }
    ...
    MasmOperationResult masm_byte_atomic_mutation_rotate(void* target, size_t size, uint32_t rotate_bits)
    {
        ...
        for (size_t i = 0; i < num_chunks; ++i)
        {
            ...
            for (int byte = 0; byte < 8; ++byte)
            {
                ...
                uint8_t rotated_byte =
                    static_cast<uint8_t>((byte_val << rotate_bits) | (byte_val >> (8 - rotate_bits)));
        ...
        for (size_t i = num_chunks * chunk_size; i < size; ++i)
        {
            ...
            uint8_t new_byte =
                static_cast<uint8_t>((original_byte << rotate_bits) | (original_byte >> (8 - rotate_bits)));
            data[i] = new_byte;
        }
    ...
    MasmOperationResult masm_byte_simd_compare_regions(const void* region1, const void* region2, size_t size)
    {
        if (!region1 || !region2)
            return MasmOperationResult::error(s_fail, -1);
        return memcmp(region1, region2, size) == 0 ? MasmOperationResult::ok(s_ok)
                                                   : MasmOperationResult::error(s_fail, 1);
    }
                                                        ...
                                                        void (*transform)(void*, void*))
    {
        ...
        const char* injection_markers[] = {
            "User-Agent:", "Authorization:", "Content-Type:", "Accept:", "X-API-Key:", "X-Request-ID:", "X-Trace-ID:"};
        ...
        {
            for (size_t marker_idx = 0; marker_idx < sizeof(injection_markers) / sizeof(injection_markers[0]);
                 ++marker_idx)
            {
                const char* marker = injection_markers[marker_idx];
                size_t marker_len = strlen(marker);

                if (marker_len + hook_len + 2 >= buffer_size)
                ...
                for (size_t i = 0; i + 31 < buffer_size - marker_len; i += 32)
                {
                    ...
                    if (mask != 0)
                    {
                            ...
                            memcmp(buffer + match_pos, marker, marker_len) == 0)
                        {
                            ...
                            {
                                memmove(buffer + match_pos + marker_len + hook_len, buffer + match_pos + marker_len,
                                                         ...
                                                         size_t* bytes_processed)
    {
        ...
        if (chunk_size > 10 * 1024 * 1024 || output_size > 10 * 1024 * 1024)
        {  // 10MB limit
            *bytes_processed = 0;
        ...
        if (g_avx2_supported && chunk_size >= 32)
        {
            ...
            for (; i + 31 < chunk_size && written + 32 < output_size; i += 32)
            {
                ...
                __m256i start_mask =
                    _mm256_or_si256(_mm256_cmpeq_epi8(chunk, json_start), _mm256_cmpeq_epi8(chunk, array_start));
                __m256i end_mask =
                    _mm256_or_si256(_mm256_cmpeq_epi8(chunk, json_end), _mm256_cmpeq_epi8(chunk, array_end));
                __m256i quote_mask = _mm256_cmpeq_epi8(chunk, quote);
                __m256i newline_mask =
    ...
    MasmOperationResult masm_agent_failure_detect_simd(const AgentMasmContext* context, const void* response_data,
                                                       size_t data_size, AgentFailureEvent* detected_failures,
    ...
    MasmOperationResult masm_agent_reasoning_accelerate(AgentMasmContext* context, const void* input_reasoning,
                                                        size_t input_size, void* output_reasoning, size_t output_size,
                                                        size_t* reasoning_cycles)
    {
        ...
        if (g_avx2_supported && num_floats >= 8)
        {
            ...
            for (; i + 7 < num_floats; i += 8)
            {
                ...
                __m256 inner = _mm256_mul_ps(const_sqrt_2_pi, _mm256_fmadd_ps(const_0_044715, x3, x));
                ...
                __m256 result = _mm256_mul_ps(const_0_5, _mm256_mul_ps(x, _mm256_add_ps(const_1, exp_2inner)));
                                                             ...
                                                             void* target_response, size_t response_size)
    {
        ...
        if (code_size > 1024 * 1024 || response_size > 10 * 1024 * 1024)
        {  // 1MB code, 10MB response limit
        ...
        while (bytecode_offset + 4 < code_size)
    ...
    MasmOperationResult masm_ai_tensor_simd_process(const AiMasmInferenceContext* context, const void* input,
                                                    size_t in_sz, void* output, size_t out_sz)
    {
    ...
    MasmOperationResult masm_ai_memory_mapped_inference(const AiMemoryMappedRegion* region, uint64_t offset,
                                                        size_t length, void* result_buffer, size_t result_size)
    {
        ...
        HANDLE hFile = CreateFileA(region->filename, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
                                                            ...
                                                            uint32_t transformation_flags)
    {
        ...
        // Security bounds checking
        if (completion_size > 10 * 1024 * 1024 || output_size > 10 * 1024 * 1024)
        {  // 10MB limits
        ...
        if (g_avx2_supported && completion_size >= 32)
        {
                ...
                for (size_t j = 0; j < 32 && i + j < completion_size && output_pos < output_size; ++j)
                {
                    ...
                    if (filter_special)
                    {
                        ...
                        for (const char* token : special_tokens)
                        {
                            ...
                            if (i + j + token_len <= completion_size && memcmp(input + i + j, token, token_len) == 0)
                            {
                                skip_token = true;
            ...
            for (; i < completion_size && output_pos < output_size; ++i)
            {
                ...
                if (filter_special)
                {
                    ...
                    {
                        size_t token_len = strlen(token);
                        if (i + token_len <= completion_size && memcmp(input + i, token, token_len) == 0)
                        {
                            skip_token = true;
                            i += token_len - 1;
                            break;
        ...
        if (compress_repeats && output_pos > 0)
        {
            ...
            for (size_t i = 1; i < output_pos; ++i)
            {
                ...
                {
                    if (count > 3)
                    {  // Only compress runs longer than 3
    ...
    MasmOperationResult masm_validate_memory_integrity(const void* memory, size_t size, uint64_t expected_checksum)
    {
        ...
        if (g_avx512_supported && size >= 64)
        {
            ...
            for (; i + 63 < size; i += 64)
            {
                ...
                uint64_t chunk_sum = _mm_cvtsi128_si64(sum128) + (_mm_cvtsi128_si64(_mm_srli_si128(sum128, 8)));
    ...
    MasmOperationResult masm_cognitive_pattern_scan_avx512(void* ctx, const void* input, size_t input_size,
                                                           void* pattern_db, size_t db_size, void* matches,
                                                           size_t max_matches, void* match_count)
    {
        ...
        *count = 0;
        ...
        PatternEntry patterns[] = {
            {"because", 7, 1},     {"therefore", 9, 1}, {"thus", 4, 1},     {"hence", 5, 1},
            {"maybe", 5, 2},       {"perhaps", 7, 2},   {"possibly", 8, 2}, {"might", 5, 2},
            {"definitely", 10, 3}, {"certainly", 9, 3}, {"clearly", 7, 3},
        };
    ...
    MasmOperationResult masm_reasoning_chain_accelerate(void* ctx, const void* chain, size_t chain_size,
                                                        void* accelerated, size_t accel_size, size_t* out_size,
                                                        size_t* cycles)
    {
    ...
    MasmOperationResult masm_semantic_memory_lookup(void* ctx, const void* query, size_t query_size, void* results,
                                                    float threshold, size_t* result_count)
    {
        ...
        *result_count = 0;
        ...
        if (q_dim == 0)
            return MasmOperationResult::ok("Empty query");
    ...
    MasmOperationResult masm_attention_compute_avx512(void* ctx, const void* queries, const void* keys, void* output,
                                                      size_t seq_len, size_t head_dim)
    {
        ...
        else
        {
            ...
            for (size_t i = 0; i < seq_len; ++i)
            {
                ...
                for (size_t j = 1; j < seq_len; ++j)
                {
                    if (O[i * seq_len + j] > max_val)
                        max_val = O[i * seq_len + j];
                }
                float sum = 0.0f;
                for (size_t j = 0; j < seq_len; ++j)
                {
    ...
    // ---------------------------------------------------------------------------
    MasmOperationResult masm_text_pattern_simd_match(const void* text, size_t text_size, const void* pattern,
                                                     size_t pattern_size, uint64_t* match_offsets, size_t max_matches,
                                                     size_t* match_count)
    {
    ...
    {
        const uint64_t feature_detect =
            static_cast<uint64_t>(g_stub_feature_detect_calls.load(std::memory_order_relaxed) & 0xFFFFu);
        const uint64_t request_hook =
            static_cast<uint64_t>(g_stub_request_hook_calls.load(std::memory_order_relaxed) & 0xFFFFu);
        const uint64_t stream_chunk =
            static_cast<uint64_t>(g_stub_stream_chunk_calls.load(std::memory_order_relaxed) & 0xFFFFu);
        const uint64_t reasoning =
            static_cast<uint64_t>(g_stub_reasoning_calls.load(std::memory_order_relaxed) & 0xFFFFu);
    ...
    uint64_t masm_ai_agent_stub_get_stats_ext(void)
    {
        const uint64_t transform =
            static_cast<uint64_t>(g_stub_transform_calls.load(std::memory_order_relaxed) & 0xFFFFu);
        const uint64_t integrity =
            static_cast<uint64_t>(g_stub_integrity_calls.load(std::memory_order_relaxed) & 0xFFFFu);
        const uint64_t pattern_match =
            static_cast<uint64_t>(g_stub_pattern_match_calls.load(std::memory_order_relaxed) & 0xFFFFu);
    ...
    // ============================================================================
    // NLShell_ValidateCommand — C++ implementation for the Phase 46 MASM symbol.
    // RawrXD_NLShell.asm is linked for Win32IDE only; RawrEngine/Gold use this TU.
    // Only define if ASM version is not being linked (Win32IDE links the ASM version).
    // ============================================================================
#ifndef RAWRXD_LINK_NLSHELL_ASM
    uint64_t NLShell_ValidateCommand(const char* cmd, uint64_t len)
    {
        ...
        // Patterns that indicate a destructive or privileged command.
