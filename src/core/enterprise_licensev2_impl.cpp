...
// V1 MASM bridge (same directory as this TU; not include/enterprise_license.h V2 manifest).
...
namespace
{
constexpr uint32_t kLicenseKeyV2Magic = 0x5258444C;  // "RXDL"

void mergeV1FeatureMaskIntoV2Key(LicenseKeyV2& key, uint64_t v1Mask)
{
    using ID = FeatureID;
    if (v1Mask & RawrXD::LicenseFeature::DualEngine800B)
        key.features.lo |= (1ULL << static_cast<uint32_t>(ID::DualEngine800B));
    if (v1Mask & RawrXD::LicenseFeature::AVX512Premium)
        key.features.lo |= (1ULL << static_cast<uint32_t>(ID::AVX512Acceleration));
    if (v1Mask & RawrXD::LicenseFeature::FlashAttention)
        key.features.lo |= (1ULL << static_cast<uint32_t>(ID::FlashAttention));
    if (v1Mask & RawrXD::LicenseFeature::MultiGPU)
        key.features.lo |= (1ULL << static_cast<uint32_t>(ID::MultiGPULoadBalance));
}
}  // namespace
...
{
    // First pass: HWID + empty key. Later passes: refresh tier/800B from V1 and persisted RXDL without
    // returning early (License Creator "Refresh", init order vs g_800B_Unlock, etc.).
    const bool firstTime = !m_initialized;
    if (firstTime)
    {
    ...
    // Re-try loading a validated RXDL key whenever we do not hold one (hot refresh, new registry install).
    if (m_currentKey.magic != kLicenseKeyV2Magic)
    {
        (void)loadKeyFromRegistry();
        if (m_currentKey.magic == 0)
        {
#ifdef _WIN32
            const char* appData = std::getenv("APPDATA");
            if (appData && appData[0] != '\0')
            {
                const std::string path = std::string(appData) + "\\RawrXD\\license_v2.key";
                (void)loadKeyFromFile(path.c_str());
            }
#endif
        }
    }

    RawrXD::EnterpriseLicense& v1 = RawrXD::EnterpriseLicense::Instance();

    if (m_currentKey.magic != kLicenseKeyV2Magic)
    {
        std::memset(&m_currentKey, 0, sizeof(m_currentKey));
        if (v1.IsEnterprise() || v1.GetState() == RawrXD::LicenseState::ValidOEM)
        {
            m_tier = LicenseTierV2::Enterprise;
            m_currentKey.features = TierPresets::Enterprise();
        }
        else if (v1.GetState() == RawrXD::LicenseState::ValidPro || v1.GetState() == RawrXD::LicenseState::ValidTrial)
        {
            m_tier = LicenseTierV2::Professional;
            m_currentKey.features = TierPresets::Professional();
        }
        else
        {
            m_tier = LicenseTierV2::Community;
            m_currentKey.features = TierPresets::Community();
        }
        m_currentKey.tier = static_cast<uint32_t>(m_tier);
        mergeV1FeatureMaskIntoV2Key(m_currentKey, v1.GetFeatureMask());
        if (v1.Is800BUnlocked())
        {
            m_currentKey.features.lo |= (1ULL << static_cast<uint32_t>(FeatureID::DualEngine800B));
        }
    }
    else if (m_tier == LicenseTierV2::Community)
    {
        if (v1.IsEnterprise() || v1.GetState() == RawrXD::LicenseState::ValidOEM)
        {
            m_tier = LicenseTierV2::Enterprise;
            m_currentKey.features = m_currentKey.features | TierPresets::Enterprise();
        }
        else if (v1.GetState() == RawrXD::LicenseState::ValidPro || v1.GetState() == RawrXD::LicenseState::ValidTrial)
        {
            m_tier = LicenseTierV2::Professional;
            m_currentKey.features = m_currentKey.features | TierPresets::Professional();
        }
        m_currentKey.tier = static_cast<uint32_t>(m_tier);
        mergeV1FeatureMaskIntoV2Key(m_currentKey, v1.GetFeatureMask());
        if (v1.Is800BUnlocked())
    ...
    if (devEnv && std::strcmp(devEnv, "1") == 0)
    {
        ...
        m_currentKey.features = TierPresets::Sovereign();
        m_currentKey.tier = static_cast<uint32_t>(m_tier);
    }

    m_initialized = true;

    return LicenseResult::ok(firstTime ? "Initialized" : "Refreshed");
}
...
{
    if (!m_initialized && id == FeatureID::DualEngine800B)
    {
        if (RawrXD::g_800B_Unlocked != 0)
            return true;
        const auto& v1 = RawrXD::EnterpriseLicense::Instance();
        if (v1.HasFeatureMask(RawrXD::LicenseFeature::DualEngine800B))
            return true;
        return v1.Is800BUnlocked();
    }
    if (!m_initialized)
        return false;
    ...
    if (bit < 64)
    {
        if ((m_currentKey.features.lo & (1ULL << bit)) != 0)
            return true;
    }
    else if (bit < 128)
    {
        if ((m_currentKey.features.hi & (1ULL << (bit - 64))) != 0)
            return true;
    }

    // V1 MASM / ASM global — DualEngine800B is not always present on synthetic V2 keys (e.g. Pro tier preset0..26).
    if (id == FeatureID::DualEngine800B)
    {
        if (RawrXD::g_800B_Unlocked != 0)
            return true;
        const auto& v1 = RawrXD::EnterpriseLicense::Instance();
...
{
    return TierLimits::forTier(currentTier());
}
...
LicenseResult EnterpriseLicenseV2::loadKeyFromFile(const char* path)
{
    ...
    if (!fp)
        return LicenseResult::error("File not found or unreadable");
    LicenseKeyV2 key = {};
...
LicenseResult EnterpriseLicenseV2::saveKeyToRegistry(const LicenseKeyV2& key)
{
    ...
    HKEY hKey;
    DWORD disposition;
    if (RegCreateKeyExA(HKEY_CURRENT_USER, "Software\\RawrXD\\License", 0, nullptr, REG_OPTION_NON_VOLATILE, KEY_WRITE,
                        nullptr, &hKey, &disposition) != ERROR_SUCCESS)
    {
        return LicenseResult::error("Failed to open registry key for writing");
    }

    LONG result = RegSetValueExA(hKey, "KeyData", 0, REG_BINARY, reinterpret_cast<const BYTE*>(&key), sizeof(key));
    RegCloseKey(hKey);
...
// ============================================================================
static const uint8_t g_VerificationSecret[] = {0xDE, 0xAD, 0xBE, 0xEF, 0xCA, 0xFE, 0xBA, 0xBE, 0x01, 0x02, 0x03,
                                               0x04, 0x05, 0x06, 0x07, 0x08, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66,
...
LicenseResult EnterpriseLicenseV2::validateKey(const LicenseKeyV2& key) const
{
    ...
    if (key.magic != 0x5258444C)
        return LicenseResult::error("Invalid magic");
    if (key.version != 2)
        return LicenseResult::error("Invalid version");
    if (key.tier > static_cast<uint32_t>(LicenseTierV2::Sovereign))
        return LicenseResult::error("Invalid tier");

    // Time-based validation
    uint32_t now = static_cast<uint32_t>(std::time(nullptr));
    if (key.issueDate > now)
        return LicenseResult::error("Future issue date");
    if (key.expiryDate != 0 && key.expiryDate < now)
        return LicenseResult::error("Expired");
...
LicenseResult EnterpriseLicenseV2::requestAzureADLicense(const char* tenantId, const char* clientId)
{
    ...
    if (!tenantId || !clientId)
        return LicenseResult::error("Invalid Azure AD parameters");

    // 1. Initialize WinHTTP
    HINTERNET hSession = WinHttpOpen(L"RawrXD-License-Agent/2.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
                                     WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
    if (!hSession)
        return LicenseResult::error("Failed to initialize HTTP session");
    ...
    BOOL bResults = WinHttpSendRequest(hRequest, L"Content-Type: application/x-www-form-urlencoded", (DWORD)-1L,
                                       (LPVOID)payload, payloadLen, payloadLen, 0);
...
void EnterpriseLicenseV2::signKey(LicenseKeyV2& key, const char* secret) const
{
    ...
    RawrXD::License::AntiTampering::computeHMAC_SHA256(reinterpret_cast<const uint8_t*>(&key), dataSize,
...
{
    if (!outKey || !signingSecret)
        return LicenseResult::error("Invalid parameters");
