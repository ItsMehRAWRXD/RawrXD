...
extern "C"
{
    ...
    bool HotSwapModelA(const char* model_id)
    {
    ...
    // Win32IDE model router expects the MASM signature:
    //   int HotSwapModel(const wchar_t* newPath, char preserveKV);
    // Provide a compatibility shim when the monolithic MASM pack is not linked.
    int HotSwapModel(const wchar_t* newPath, char preserveKV)
    {
        (void)preserveKV;
...
#ifdef _WIN32
        char utf8[512] = {};
        const int n = ::WideCharToMultiByte(CP_UTF8, 0, newPath, -1, utf8, (int)sizeof(utf8) - 1, nullptr, nullptr);
        if (n <= 0)
        {
            return 0;
        }
        return HotSwapModelA(utf8) ? 1 : 0;
#else
        return 0;
#endif
    }

    // Universal model router expects these legacy entrypoints.
    int ModelLoaderInit()
    {
        return 0;
    }
    ...
    {
        return HotSwapModel(path, /*preserveKV=*/0);
    }
