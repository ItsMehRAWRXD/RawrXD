...
void RawrXD_StreamToGPU_AVX512(void* dst, const void* src, size_t bytes) {
    ...
    if (rawr_cpu_has_avx512()) {
        ...
        // Use AVX-512 non-temporal stores (vmovntdq)
        for (size_t i = 0; i < alignedBytes; i += 64) {
            __m512i v = _mm512_loadu_si512(s + i);
            _mm512_stream_si512(d + i, v);
        }
        
        // Handle remaining bytes with standard memcpy
        size_t remainder = bytes - alignedBytes;
        if (remainder > 0) {
            memcpy(d + alignedBytes, s + alignedBytes, remainder);
        }
