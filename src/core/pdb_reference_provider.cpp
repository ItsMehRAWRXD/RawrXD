...
static uint32_t pdbProcGetIndexedCount(void* provider) {
    (void)provider;
    // Procedures are a subset of public symbols — use the same indexed count
    // from PDBManager. A separate procedure-only counter would require a full
    // enumeration pass, which is too expensive for an info-only query.
    PDBManager& pdb = PDBManager::instance();
    PDBManager::Stats stats = pdb.getStats();
    return static_cast<uint32_t>(stats.symbolsIndexed);
}
...
// ============================================================================
//
// Walks PE import tables to find all modules that import a given symbol.
// Enumerates IMAGE_IMPORT_DESCRIPTOR entries and matches against query.
//

struct ImportTableProviderState {
    bool initialized;
    uint32_t totalImportsIndexed;
};

// ---------------------------------------------------------------------------
// walkPEImportTable — Enumerates IMAGE_IMPORT_DESCRIPTOR entries in a PE
// loaded at the given base address. For each imported function name that
// matches the query, adds a ReferenceLocation to the result set.
// ---------------------------------------------------------------------------
static uint32_t walkPEImportTable(const uint8_t* imageBase, size_t imageSize,
                                   const char* modulePath,
                                   const ReferenceQuery* query,
                                   ReferenceResult* result) {
    if (!imageBase || imageSize < sizeof(IMAGE_DOS_HEADER)) return 0;

    const auto* dosHeader = reinterpret_cast<const IMAGE_DOS_HEADER*>(imageBase);
    if (dosHeader->e_magic != IMAGE_DOS_SIGNATURE) return 0;

    if (static_cast<size_t>(dosHeader->e_lfanew) + sizeof(IMAGE_NT_HEADERS64) > imageSize) return 0;

    const auto* ntHeaders = reinterpret_cast<const IMAGE_NT_HEADERS64*>(imageBase + dosHeader->e_lfanew);
    if (ntHeaders->Signature != IMAGE_NT_SIGNATURE) return 0;

    // Get import directory RVA from the data directory
    DWORD importDirRVA = 0;
    DWORD importDirSize = 0;

    if (ntHeaders->FileHeader.Machine == IMAGE_FILE_MACHINE_AMD64) {
        const auto* opt64 = &ntHeaders->OptionalHeader;
        if (opt64->NumberOfRvaAndSizes > IMAGE_DIRECTORY_ENTRY_IMPORT) {
            importDirRVA  = opt64->DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress;
            importDirSize = opt64->DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].Size;
        }
    } else {
        // 32-bit fallback
        const auto* nt32 = reinterpret_cast<const IMAGE_NT_HEADERS32*>(imageBase + dosHeader->e_lfanew);
        const auto* opt32 = &nt32->OptionalHeader;
        if (opt32->NumberOfRvaAndSizes > IMAGE_DIRECTORY_ENTRY_IMPORT) {
            importDirRVA  = opt32->DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress;
            importDirSize = opt32->DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].Size;
        }
    }

    if (importDirRVA == 0 || importDirSize == 0) return 0;
    if (static_cast<size_t>(importDirRVA) + importDirSize > imageSize) return 0;

    const auto* importDesc = reinterpret_cast<const IMAGE_IMPORT_DESCRIPTOR*>(imageBase + importDirRVA);
    uint32_t matchCount = 0;

    // Walk each IMAGE_IMPORT_DESCRIPTOR (null-terminated array)
    for (; importDesc->Characteristics != 0; ++importDesc) {
        // Bounds check the descriptor itself
        size_t descOffset = reinterpret_cast<const uint8_t*>(importDesc) - imageBase;
        if (descOffset + sizeof(IMAGE_IMPORT_DESCRIPTOR) > imageSize) break;

        // Get the imported DLL name
        if (importDesc->Name == 0 || static_cast<size_t>(importDesc->Name) >= imageSize) continue;
        const char* dllName = reinterpret_cast<const char*>(imageBase + importDesc->Name);

        // Walk the Import Name Table (INT) or Import Lookup Table (ILT)
        DWORD iltRVA = importDesc->OriginalFirstThunk ? importDesc->OriginalFirstThunk : importDesc->FirstThunk;
        if (iltRVA == 0 || static_cast<size_t>(iltRVA) >= imageSize) continue;

        const auto* thunkData = reinterpret_cast<const IMAGE_THUNK_DATA64*>(imageBase + iltRVA);
        uint32_t ordinalIndex = 0;

        for (; thunkData->u1.AddressOfData != 0; ++thunkData, ++ordinalIndex) {
            size_t thunkOffset = reinterpret_cast<const uint8_t*>(thunkData) - imageBase;
            if (thunkOffset + sizeof(IMAGE_THUNK_DATA64) > imageSize) break;

            // Skip ordinal imports (high bit set)
            if (IMAGE_SNAP_BY_ORDINAL64(thunkData->u1.Ordinal)) continue;

            DWORD hintNameRVA = static_cast<DWORD>(thunkData->u1.AddressOfData);
            if (static_cast<size_t>(hintNameRVA) + sizeof(IMAGE_IMPORT_BY_NAME) > imageSize) continue;

            const auto* importByName = reinterpret_cast<const IMAGE_IMPORT_BY_NAME*>(imageBase + hintNameRVA);
            const char* funcName = reinterpret_cast<const char*>(importByName->Name);

            // Match against the query symbol name (case-insensitive exact + substring)
            bool match = (_stricmp(funcName, query->symbolName) == 0);
            if (!match && query->crossModuleSearch) {
                const char* needle = query->symbolName;
                uint32_t needleLen = query->symbolNameLen;
            ...
            // Build a ReferenceLocation for this import entry
            ReferenceLocation loc{};
            strncpy_s(loc.moduleName, sizeof(loc.moduleName), dllName, _TRUNCATE);
            strncpy_s(loc.symbolName, sizeof(loc.symbolName), funcName, _TRUNCATE);

            // The IAT entry RVA is relative to the importing module
            DWORD iatRVA = importDesc->FirstThunk + ordinalIndex * sizeof(IMAGE_THUNK_DATA64);
            loc.rva = iatRVA;
            loc.section = 0; // Import table section resolved at load time
            loc.sectionOffset = 0;
            loc.size = 0;
            loc.isFunction = true;
            loc.kind = ReferenceKind::Import;
            loc.providerTag = "import";

            // Exact match gets higher confidence
            if (_stricmp(funcName, query->symbolName) == 0) {
                loc.confidence = 0.90f;
            } else {
                loc.confidence = 0.65f;
            }

            snprintf(loc.detail, sizeof(loc.detail),
                     "Imported from %s (hint %u, IAT RVA 0x%lX)",
                     dllName,
                     static_cast<unsigned>(importByName->Hint),
                     static_cast<unsigned long>(iatRVA));
...
static PDBResult importTableFindReferences(void* provider,
    ...
    if (!query->includeImports) {
        return PDBResult::ok("Import references disabled in query");
    }

    // Walk each loaded module's PE import table
    PDBManager& pdb = PDBManager::instance();
    uint32_t moduleCount = pdb.getLoadedModuleCount();
    ...
    for (uint32_t i = 0; i < moduleCount; ++i) {
        ...
        // If contextModule is set, only search that module
        if (query->contextModule && _stricmp(query->contextModule, moduleName) != 0) {
            continue;
        }

        // Try to open the PE file for this module to read its import table
        wchar_t widePath[MAX_PATH] = {};
        MultiByteToWideChar(CP_ACP, 0, moduleName, -1, widePath, MAX_PATH);
        ...
        if (!GetFileSizeEx(hFile, &fileSize) || fileSize.QuadPart < 64 ||
            fileSize.QuadPart > 512 * 1024 * 1024) { // Cap at 512MB
            CloseHandle(hFile);
            continue;
        }
        ...
        uint32_t matches = walkPEImportTable(base, static_cast<size_t>(fileSize.QuadPart),
                                              moduleName, query, result);
        totalMatches += matches;
        if (state) state->totalImportsIndexed += matches;

        UnmapViewOfFile(base);
        CloseHandle(hMap);
        CloseHandle(hFile);

        if (query->maxResults > 0 && result->count >= query->maxResults) break;
    }

    char msg[128];
    snprintf(msg, sizeof(msg), "Import table search complete (%u matches across %u modules)",
             totalMatches, moduleCount);
    return PDBResult::ok(msg);
}

static uint32_t importTableGetIndexedCount(void* provider) {
    auto* state = static_cast<ImportTableProviderState*>(provider);
    return state ? state->totalImportsIndexed : 0;
}
...
IReferenceProvider* createImportTableProvider() {
    ...
    state->totalImportsIndexed = 0;

    p->tag = "import";
    p->description = "PE import table cross-references (IMAGE_IMPORT_DESCRIPTOR walker)";
...
void ReferenceRouter::initBuiltinProviders() {
    ...
    // 3. Import Table (PE IMAGE_IMPORT_DESCRIPTOR walker)
