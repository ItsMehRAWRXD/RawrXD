...
static std::string reverseTraceHotspotsReport(size_t topN)
{
    ...
    std::sort(rows.begin(), rows.end(), [](const Row& a, const Row& b) { return a.stats.totalUs > b.stats.totalUs; });
    ...
    {
        const auto& r = rows[i];
        oss << "    [" << (i + 1) << "] " << r.name << " calls=" << r.stats.calls << " total=" << r.stats.totalUs
            << "us"
            << " avg=" << r.avgUs << "us"
...
static bool reverseTraceExportCsv(const std::string& outPath)
{
    ...
    for (const auto& r : rows)
    {
        ...
        fprintf(f, "\"%s\",%llu,%llu,%llu,%llu,%llu\n", safe.c_str(), static_cast<unsigned long long>(r.stats.calls),
                static_cast<unsigned long long>(r.stats.totalUs), static_cast<unsigned long long>(r.avgUs),
                static_cast<unsigned long long>(r.stats.maxUs), static_cast<unsigned long long>(r.stats.lastUs));
    }
...
static const char* themeNameFromId(uint32_t cmdId)
{
    switch (cmdId)
    {
        ...
        case 3112:
            return "theme.rawrxdCrimson";
        case 3113:
            return "theme.highContrast";
        case 3114:
            return "theme.oneDarkPro";
        case 3115:
            return "theme.synthwave84";
        case 3116:
            return "theme.abyss";
        default:
            return nullptr;
...
static CommandResult routeToIde(const CommandContext& ctx, uint32_t cmdId, const char* name)
{
    ...
    // CLI visual-mode support for theme/transparency/monaco commands.
    {
        ...
        if (cmdId == 3210)
        {
            ...
            const bool saved = saveCliVisualStateLocked(v);
            std::ostringstream oss;
            oss << "[VIEW] Transparency custom set to " << v.transparencyPct << "% (CLI profile, valid range 20-100)\n"
                << "  State: " << (saved ? "persisted" : "persist failed") << "\n";
            ctx.output(oss.str().c_str());
            return CommandResult::ok(name);
        }
        if (cmdId == 3211)
        {
            ...
            const bool saved = saveCliVisualStateLocked(v);
            std::ostringstream oss;
            oss << "[VIEW] Transparency " << (v.transparencyEnabled ? "enabled" : "disabled") << " in CLI profile";
            if (v.transparencyEnabled)
                oss << " at " << v.transparencyPct << "%";
            oss << "\n  State: " << (saved ? "persisted" : "persist failed") << "\n";
            ctx.output(oss.str().c_str());
            return CommandResult::ok(name);
        }
...
static std::string summarizeEmbeddedGgufProbe(const std::string& resolvedPath)
{
    ...
    char buf[256];
    snprintf(buf, sizeof(buf), "  Embedded GGUF probe: OK (mmap=%s, mapped=%llu MB, tensors=%llu, metadata=%llu)\n",
             mmapped ? "yes" : "no", static_cast<unsigned long long>(mappedBytes / (1024ull * 1024ull)),
             static_cast<unsigned long long>(tensorCount), static_cast<unsigned long long>(metadataCount));
...
static bool saveLspClientConfigToPath(const LspClientRuntimeConfig& cfg, const std::string& path)
{
    ...
    fprintf(f,
            ...
            "  \"lastSavedPath\": \"%s\"\n"
            "}\n",
            cfg.server.c_str(), cfg.serverArgs.c_str(), cfg.workspaceRoot.c_str(),
            cfg.diagnosticsOnSave ? "true" : "false", cfg.strictMode ? "true" : "false", cfg.maxReferences,
            path.c_str());
    fclose(f);
    return true;
}
...
static bool resolveBinaryPath(const std::string& requested, std::string& outPath)
{
    ...
    if (requested.empty())
        return false;
    bool hasPathSeparators = requested.find('\\') != std::string::npos || requested.find('/') != std::string::npos ||
...
static bool saveGovernorStateLocked(const SsoGovernorState& st)
{
    ...
    fprintf(f,
            ...
            "  \"preferredBackend\": \"%s\"\n"
            "}\n",
            st.powerProfile.c_str(), st.powerPercent, st.maxConcurrent,
            static_cast<unsigned long long>(st.defaultTimeoutMs), st.preferredBackend.c_str());
    fclose(f);
    return true;
}
...
static std::string detectBackendFromCommand(const std::string& command)
{
    ...
    if (lower.find("vulkan") != std::string::npos)
        return "vulkan";
    if (lower.find("cuda") != std::string::npos || lower.find("nvcuda") != std::string::npos)
        return "cuda";
    if (lower.find("rocm") != std::string::npos || lower.find("hip") != std::string::npos)
        return "rocm";
...
static const char* governorTaskStateName(GovernorTaskState state)
{
    switch (state)
    {
        case GovernorTaskState::Pending:
            return "pending";
        case GovernorTaskState::Running:
            return "running";
...
static bool parsePowerProfile(const std::string& raw, std::string& profile, int& percent, int& maxConcurrent)
{
    ...
    const bool isNumeric =
        !value.empty() && std::all_of(value.begin(), value.end(), [](unsigned char c) { return c >= '0' && c <= '9'; });
...
static bool hasLspSearchExt(const std::string& path)
{
    ...
    return ext == ".c" || ext == ".cc" || ext == ".cpp" || ext == ".cxx" || ext == ".h" || ext == ".hh" ||
           ext == ".hpp" || ext == ".hxx" || ext == ".ixx" || ext == ".inl" || ext == ".ipp" || ext == ".asm" ||
           ext == ".inc";
}

static void collectLspSearchFilesRecursive(const std::string& root, std::vector<std::string>& out, size_t maxFiles)
{
    if (out.size() >= maxFiles || !pathIsDirectory(root))
        return;
    std::string pattern = root + "\\*";
    WIN32_FIND_DATAA fd{};
    HANDLE hFind = FindFirstFileA(pattern.c_str(), &fd);
...
{
    if (pos > text.size() || len == 0 || pos + len > text.size())
        return false;
    if (pos > 0 && isIdentCharAscii(static_cast<unsigned char>(text[pos - 1])))
        return false;
    if (pos + len < text.size() && isIdentCharAscii(static_cast<unsigned char>(text[pos + len])))
        return false;
    return true;
}
...
{
    if (line.empty() || symbol.empty())
        return false;
    const std::string lower = toLowerAsciiCopy(line);
    const std::string sym = toLowerAsciiCopy(symbol);
    if (lower.find("class " + sym) != std::string::npos)
        return true;
    if (lower.find("struct " + sym) != std::string::npos)
        return true;
    if (lower.find("enum " + sym) != std::string::npos)
        return true;
    if (lower.find("typedef") != std::string::npos && lower.find(sym) != std::string::npos)
        return true;
    if (lower.find(sym + "(") != std::string::npos && lower.find(';') == std::string::npos)
        return true;
    return false;
}

static void collectSymbolMatches(const std::string& symbol, const std::vector<std::string>& files,
                                 std::vector<LspSymbolMatch>& out, size_t maxMatches)
{
    out.clear();
    if (symbol.empty())
        return;
    ...
    for (const auto& path : files)
    {
        if (out.size() >= maxMatches)
            break;
        std::string text;
        if (!readTextFileLimited(path, text, 8 * 1024 * 1024))
            continue;
...
static size_t replaceWholeWordOccurrences(std::string& text, const std::string& oldSym, const std::string& newSym)
{
    if (oldSym.empty() || oldSym == newSym)
        return 0;
    size_t replaced = 0;
    size_t pos = 0;
    while (pos < text.size())
...
static bool hasModelArtifactExt(const std::string& fileName)
{
    ...
    return endsWith(lower, ".gguf") || endsWith(lower, ".bin") || endsWith(lower, ".pt") ||
...
static void collectModelArtifactsFromDirectory(const std::string& dir, std::vector<std::string>& out)
{
    ...
    do
    {
        if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
            continue;
        if (!hasModelArtifactExt(fd.cFileName))
            continue;
        std::string full = dir;
...
static std::vector<std::string> discoverModelArtifacts()
{
    ...
    const std::array<const char*, 4> roots = {"models", ".rawrxd\\models", ".rawrxd\\training_output", "."};
    for (const char* root : roots)
    {
...
static RawrXD::Training::DatasetFormat inferDatasetFormat(const std::string& datasetPath,
                                                          const std::string& explicitFormat)
{
    const std::string fmt = toLowerAsciiCopy(trimAscii(explicitFormat));
    if (fmt == "jsonl" || fmt == "json")
        return RawrXD::Training::DatasetFormat::JSONL;
    if (fmt == "csv")
        return RawrXD::Training::DatasetFormat::CSV;
    if (fmt == "alpaca")
        return RawrXD::Training::DatasetFormat::Alpaca;
    if (fmt == "sharegpt")
        return RawrXD::Training::DatasetFormat::ShareGPT;
    if (fmt == "text" || fmt == "txt" || fmt == "markdown")
        return RawrXD::Training::DatasetFormat::PlainText;
    if (fmt == "tokenized" || fmt == "bin")
        return RawrXD::Training::DatasetFormat::CustomTokenized;
    if (fmt == "code" || fmt == "codefiles")
        return RawrXD::Training::DatasetFormat::CodeFiles;

    const std::string lowerPath = toLowerAsciiCopy(datasetPath);
    if (lowerPath.find(".jsonl") != std::string::npos)
        return RawrXD::Training::DatasetFormat::JSONL;
    if (lowerPath.find(".csv") != std::string::npos)
        return RawrXD::Training::DatasetFormat::CSV;
...
static bool parseQuantTypeArg(const std::string& raw, RawrXD::Training::QuantType& outType, std::string& outCanonical)
{
...
static bool readBinaryFile(const std::string& path, std::vector<uint8_t>& out)
{
    ...
    size_t read = 0;
    if (!out.empty())
        read = fread(out.data(), 1, out.size(), f);
    fclose(f);
    if (read != out.size())
    {
        out.clear();
...
static bool extractImageDimensions(const std::vector<uint8_t>& bytes, int& w, int& h)
{
    ...
    if (kind == "JPEG")
        return parseJpegDimensions(bytes, w, h);
    return false;
}
...
CommandResult handleBackendSwitchLocal(const CommandContext& ctx)
{
    ...
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5037, "backend.switchLocal");
...
CommandResult handleBackendSwitchOllama(const CommandContext& ctx)
{
    ...
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5038, "backend.switchOllama");
...
CommandResult handleBackendSwitchOpenAI(const CommandContext& ctx)
{
    ...
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5039, "backend.switchOpenAI");
...
CommandResult handleBackendSwitchClaude(const CommandContext& ctx)
{
    ...
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5040, "backend.switchClaude");
...
CommandResult handleBackendSwitchGemini(const CommandContext& ctx)
{
    ...
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5041, "backend.switchGemini");
...
CommandResult handleBackendShowStatus(const CommandContext& ctx)
{
    ...
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5042, "backend.showStatus");
...
CommandResult handleBackendShowSwitcher(const CommandContext& ctx)
{
    ...
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5043, "backend.showSwitcher");
...
CommandResult handleBeaconStatus(const CommandContext& ctx)
{
    ...
    {
        if (state == 0)
            ctx.output("[BEACON] Idle (no half-pulses seen).\n");
        if (state == 1)
            ctx.output("[BEACON] AVX2 half only (Half-Beat mode).\n");
        if (state == 2)
            ctx.output("[BEACON] AVX512 half only (Half-Beat mode).\n");
    }
...
CommandResult handleBackendConfigure(const CommandContext& ctx)
{
    ...
    if (value.empty())
        return CommandResult::error("Usage: !backend_config <key> <value>");
    ...
    ctx.output(msg.c_str());
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5044, "backend.configure");
    return CommandResult::ok("backend.configure");
}
...
CommandResult handleBackendHealthCheck(const CommandContext& ctx)
{
    ...
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5045, "backend.healthCheck");
...
CommandResult handleBackendSetApiKey(const CommandContext& ctx)
{
    ...
    std::string msg =
        "[BACKEND] API key set for: " + backend + " (" + key.substr(0, std::min<size_t>(8, key.size())) + "...)\n";
    ctx.output(msg.c_str());
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5046, "backend.setApiKey");
...
CommandResult handleBackendSaveConfigs(const CommandContext& ctx)
{
    ...
    if (!out.good())
        return CommandResult::error("Failed to write .rawrxd_backend_config.json");
    out << json;
    out.close();
    ctx.output("[BACKEND] Saved backend config to .rawrxd_backend_config.json\n");
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5047, "backend.saveConfigs");
...
CommandResult handleRouterEnable(const CommandContext& ctx)
{
    ...
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5048, "router.enable");
...
CommandResult handleRouterDisable(const CommandContext& ctx)
{
    ...
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5049, "router.disable");
...
CommandResult handleRouterStatus(const CommandContext& ctx)
{
    ...
    ctx.output(buf);
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5050, "router.status");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5088, "asm.analyzeBlock");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5089, "asm.callGraph");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5093, "asm.clearSymbols");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5090, "asm.dataFlow");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5091, "asm.detectConvention");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5084, "asm.findRefs");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5083, "asm.goto");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5086, "asm.instructionInfo");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5082, "asm.parseSymbols");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5087, "asm.registerInfo");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5092, "asm.showSections");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5085, "asm.symbolTable");
...
CommandResult handleConfidenceSetPolicy(const CommandContext& ctx)
{
    ...
    if (policy.empty())
        return CommandResult::error("Usage: !confidence policy <aggressive|conservative>");
    ...
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5131, "confidence.setPolicy");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5171, "dbg.addWatch");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5158, "dbg.attach");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5164, "dbg.break");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5169, "dbg.clearBps");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5159, "dbg.detach");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5176, "dbg.disasm");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5168, "dbg.enableBp");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5180, "dbg.evaluate");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5160, "dbg.go");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5165, "dbg.kill");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5157, "dbg.launch");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5170, "dbg.listBps");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5175, "dbg.memory");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5177, "dbg.modules");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5173, "dbg.registers");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5167, "dbg.removeBp");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5172, "dbg.removeWatch");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5182, "dbg.searchMemory");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5181, "dbg.setRegister");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5174, "dbg.stack");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5184, "dbg.status");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5162, "dbg.stepInto");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5163, "dbg.stepOut");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5161, "dbg.stepOver");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5179, "dbg.switchThread");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5183, "dbg.symbolPath");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5178, "dbg.threads");
...
CommandResult handleDiskListDrives(const CommandContext& ctx)
{
    ...
    if (n == 0 || n >= sizeof(drives))
        return CommandResult::error("Failed to enumerate drives");
    ...
    for (char* p = drives; *p; p += std::strlen(p) + 1)
    {
        ...
        if (t == DRIVE_FIXED)
            type = "fixed";
        else if (t == DRIVE_REMOVABLE)
            type = "removable";
        else if (t == DRIVE_REMOTE)
            type = "network";
        else if (t == DRIVE_CDROM)
            type = "cdrom";
        else if (t == DRIVE_RAMDISK)
            type = "ramdisk";
        std::string line = "  - " + std::string(p) + " (" + type + ")\n";
        ctx.output(line.c_str());
    }
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 11100, "disk.listDrives");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 209, "edit.multicursorAdd");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 210, "edit.multicursorRemove");
...
CommandResult handleEmbeddingEncode(const CommandContext& ctx)
{
    ...
    if (text.empty())
        return CommandResult::error("Usage: !embedding_encode <text>");
    ...
    ctx.output(buf);
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 11400, "embedding.encode");
...
CommandResult handleFileAutoSave(const CommandContext& ctx)
{
    ...
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 105, "file.autoSave");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 106, "file.closeFolder");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 110, "file.closeTab");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 109, "file.newWindow");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 108, "file.openFolder");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 11201, "governor.setPowerLevel");
    ...
    if (profile == "eco")
        scheme = "SCHEME_MAX";
    else if (profile == "performance" || profile == "turbo" || profile == "max")
        scheme = "SCHEME_MIN";
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5118, "gov.status");
    ...
    oss << "[GOVERNOR] Runtime status\n"
        ...
        << "  backends: vulkan=" << (hasVulkan ? "yes" : "no") << ", cuda=" << (hasCuda ? "yes" : "no")
        << ", rocm=" << (hasRocm ? "yes" : "no") << "\n";
    if (!active.empty())
    {
        oss << "  active_tasks:\n";
...
CommandResult handleGovSubmitCommand(const CommandContext& ctx)
{
    ...
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5119, "gov.submit");

    std::string cmd = trimAscii(ctx.args ? std::string(ctx.args) : std::string());
    if (cmd.empty())
        return CommandResult::error("Usage: !gov submit <command>");
    ...
    if (backend == "auto" && cmdBackend != "auto")
        backend = cmdBackend;

    std::ostringstream desc;
    desc << "gov.submit [" << backend << "] " << cmd.substr(0, 80);
    const GovernorTaskId id = execGov.submitCommand(cmd, timeoutMs, GovernorRiskTier::High, desc.str(), nullptr);
...
CommandResult handleGovTaskList(const CommandContext& ctx)
{
    ...
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5121, "gov.taskList");
    ...
    for (auto it = order.rbegin(); it != order.rend(); ++it)
    {
        ...
        if (state != GovernorTaskState::Pending && state != GovernorTaskState::Running)
        {
            ...
            {
                oss << "  rc=" << result.exitCode << "  dur_ms=" << std::fixed << std::setprecision(1)
                    << result.durationMs;
            }
        }
        oss << "\n";
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5097, "hybrid.analyzeFile");
    std::string file = getArgsTrimmed(ctx);
    if (file.empty())
        file = pickDefaultHybridFile();
    if (file.empty())
    {
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5102, "hybrid.annotateDiag");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5098, "hybrid.autoProfile");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5094, "hybrid.complete");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5105, "hybrid.correctionLoop");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5095, "hybrid.diagnostics");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5101, "hybrid.explainSymbol");
    ...
    if (lower.find("class " + lowerSymbol) != std::string::npos)
        kind = "class";
    else if (lower.find("struct " + lowerSymbol) != std::string::npos)
        kind = "struct";
    else if (lower.find("enum " + lowerSymbol) != std::string::npos)
        kind = "enum";
    else if (lower.find(lowerSymbol + "(") != std::string::npos)
        kind = "function";

    size_t refs = matches.size();
    if (refs > 0 && def.definitionLike)
        refs -= 1;
    std::ostringstream out;
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5104, "hybrid.semanticPrefetch");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5096, "hybrid.smartRename");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5099, "hybrid.status");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5103, "hybrid.streamAnalyze");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5100, "hybrid.symbolUsage");
    const std::string symbol = ssoGetArg(ctx, 0);
    if (symbol.empty())
    {
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5067, "lsp.clearDiag");
    ensureDirectory(".rawrxd");
    ensureDirectory(".rawrxd\\lsp");
    const std::array<const char*, 3> files = {".rawrxd\\lsp\\diagnostics_latest.txt",
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5069, "lsp.configure");
    ...
    {
        if (value.empty())
            return CommandResult::error("lsp.configure: server value missing");
        cfg.server = value;
        changed = true;
    }
    else if (key == "args")
    {
        if (value.empty())
            return CommandResult::error("lsp.configure: args value missing");
    ...
    {
        bool parsed = false;
        if (!parseBoolWord(value, parsed))
            return CommandResult::error("lsp.configure: expected bool value");
    ...
    {
        if (value.empty())
            return CommandResult::error("lsp.configure: max_refs value missing");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5065, "lsp.diagnostics");
    ensureDirectory(".rawrxd");
    ensureDirectory(".rawrxd\\lsp");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5066, "lsp.restart");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5070, "lsp.saveConfig");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5068, "lsp.symbolInfo");
    const std::string symbol = ssoGetArg(ctx, 0);
    if (symbol.empty())
    {
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 11301, "marketplace.install");
    ...
    if (!mod)
    {
        ...
        {
            std::ostringstream oss;
            oss << "[MARKETPLACE] LoadLibrary failed (error " << firstErr << "), LoadLibraryEx failed (error "
                << GetLastError() << ").\n";
            ctx.output(oss.str().c_str());
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 11300, "marketplace.list");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 11005, "model.finetune");
    ...
    if (outputDir.empty())
        outputDir = ".rawrxd\\training_output";
    ensureDirectory(".rawrxd");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 11002, "model.load");
...

    {
        ...
        if (!seen)
            st.discoveredModels.push_back(resolved);
    }
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 11004, "model.quantize");
    ...
    if (outputPath.empty())
    {
        ...
        outputPath = ".rawrxd\\models\\" + stemNoExt(fileNameFromPath(inputPath)) + "." + canonicalScheme + ".gguf";
    }
    ...
    const RawrXD::Training::TrainingResult qr = qe.quantizeModel(inputPath.c_str(), outputPath.c_str(), quant, arch);
    if (!qr.success)
    {
        std::ostringstream oss;
        oss << "[MODEL] Quantization failed: " << (qr.detail ? qr.detail : "unknown error") << " (code=" << qr.errorCode
            << ")\n";
        ctx.output(oss.str().c_str());
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 11003, "model.unload");
    const std::string requested = getArgsTrimmed(ctx);
    std::string unloaded;
    {
        auto& st = ModelRuntimeState::instance();
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5117, "multi.applyPreferred");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5116, "multi.clearHistory");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5109, "multi.compare");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5106, "multi.generate");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5108, "multi.selectPreferred");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5107, "multi.setMax");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5114, "multi.latest");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5113, "multi.prefs");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5110, "multi.stats");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5115, "multi.status");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5111, "multi.templates");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5112, "multi.toggleTemplate");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5208, "plugin.configure");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5201, "plugin.load");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5204, "plugin.refresh");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5205, "plugin.scanDir");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5200, "plugin.panel");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5206, "plugin.status");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5207, "plugin.toggleHotload");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5202, "plugin.unload");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5203, "plugin.unloadAll");
...
CommandResult handlePromptClassifyContext(const CommandContext& ctx)
{
    ...
    std::array<Bucket, 7> buckets{
        {{"debug", 0}, {"refactor", 0}, {"test", 0}, {"performance", 0}, {"security", 0}, {"docs", 0}, {"general", 1}}};
    ...
    bump(0, "error", 4);
    bump(0, "crash", 5);
    bump(0, "stack trace", 5);
    bump(0, "fix", 2);
    bump(1, "refactor", 5);
    bump(1, "cleanup", 3);
    bump(1, "rename", 3);
    bump(1, "extract", 3);
    bump(1, "restructure", 4);
    bump(2, "test", 4);
    bump(2, "unit test", 5);
    bump(2, "integration", 3);
    bump(2, "assert", 3);
    bump(2, "coverage", 4);
    bump(3, "optimize", 5);
    bump(3, "perf", 4);
    ...
    const double confidence =
        (totalScore > 0) ? (static_cast<double>(buckets[bestIdx].score) / static_cast<double>(totalScore)) : 0.0;
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5129, "replay.checkpoint");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5128, "replay.export");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5127, "replay.showLast");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5126, "replay.status");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 8101, "reveng.decompile");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 8100, "reveng.disassemble");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 8102, "reveng.findVulns");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5053, "router.capabilities");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5051, "router.decision");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5077, "router.ensembleDisable");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5076, "router.ensembleEnable");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5078, "router.ensembleStatus");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5054, "router.fallbacks");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5072, "router.pinTask");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5057, "router.resetStats");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5056, "router.routePrompt");
    std::string prompt = ctx.args ? std::string(ctx.args) : std::string();
    if (prompt.empty())
        return CommandResult::error("Usage: !router route <prompt>");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5055, "router.saveConfig");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5052, "router.setPolicy");
    std::string policy = ssoGetArg(ctx, 0);
    if (policy.empty())
        return CommandResult::error("Usage: !router policy <cost|speed|quality|latency>");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5081, "router.costStats");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5075, "router.heatmap");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5074, "router.showPins");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5079, "router.simulate");
    std::string prompt = ctx.args ? std::string(ctx.args) : std::string();
    if (prompt.empty())
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5080, "router.simulateLast");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5073, "router.unpinTask");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5071, "router.whyBackend");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5123, "safety.resetBudget");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5124, "safety.rollback");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5125, "safety.violations");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 5122, "safety.status");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 505, "tools.build");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 501, "tools.commandPalette");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 506, "tools.debug");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 503, "tools.extensions");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 502, "tools.settings");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 504, "tools.terminal");
...
{
    // GUI: run inline (7004 is Win32 Build/Clean id in resource.h — never PostMessage it for Unity)
...
{
    // GUI: run inline (7003 is Win32 Build > Rebuild — do not route there)
    ...
    const char* candidates[] = {"RawrXD_UnityBridge.dll", "plugins\\RawrXD_UnityBridge.dll",
...
{
    // GUI: run inline (7002 collides with Win32 Build menu)
    ...
    if (queried)
    {
        ...
        const bool looksUnreal = (lower.find("unreal") != std::string::npos) ||
                                 (lower.find("ue4") != std::string::npos) || (lower.find("ue5") != std::string::npos);
        oss << "  Image: " << imagePath << "\n";
        if (!looksUnreal)
        {
...
{
    // GUI: run inline (7001 is Win32 Build > Compile in resource.h)
    ...
    const char* candidates[] = {"RawrXD_UnrealBridge.dll", "plugins\\RawrXD_UnrealBridge.dll",
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 304, "view.toggleFullscreen");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 303, "view.toggleOutput");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 301, "view.toggleSidebar");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 302, "view.toggleTerminal");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 305, "view.zoomIn");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 306, "view.zoomOut");
...
{
    if (ctx.isGui && ctx.idePtr)
        routeToIde(ctx, 307, "view.zoomReset");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 11500, "vision.analyze");
    ...
    if (path.empty())
    {
        const char* patterns[] = {"*.png",  "*.jpg",         "*.jpeg",        "*.bmp",         "*.gif",
                                  "*.webp", "images\\*.png", "images\\*.jpg", "assets\\*.png", "assets\\*.jpg"};
        for (const char* pat : patterns)
        {
            if (findFirstFileByPattern(pat, path))
                break;
        }
        if (!path.empty())
        {
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 2013, "edit.copyFormat");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 2014, "edit.pastePlain");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 2015, "edit.clipboardHistory");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 2019, "edit.findPrev");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 2027, "view.vulkanRenderer");
    ...
    auto pEnumVersion =
        reinterpret_cast<PFN_vkEnumerateInstanceVersion>(GetProcAddress(vk, "vkEnumerateInstanceVersion"));
...
static size_t estimateContextReserveBytes(uint32_t tokenCount)
{
    ...
    if (bytes < kMinReserve)
        bytes = kMinReserve;
    if (bytes > kMaxReserve)
        bytes = kMaxReserve;
...
static size_t estimateContextCommitBytes(size_t reserveBytes)
{
    ...
    if (commitBytes < kMinCommit)
        commitBytes = kMinCommit;
    if (commitBytes > kMaxCommit)
        commitBytes = kMaxCommit;
...
static bool saveAIContextStateLocked(const AIContextRuntimeState& st)
{
    ...
    fprintf(f,
            ...
            "  \"poolBase\": \"%p\"\n"
            "}\n",
            st.label.c_str(), st.tokenCount, static_cast<unsigned long long>(st.reservedBytes),
            static_cast<unsigned long long>(st.committedBytes), st.poolBase);
    fclose(f);
    return true;
}

static bool configureAIContextMemoryLocked(AIContextRuntimeState& st, uint32_t tokenCount, const char* label,
                                           std::string& err)
{
    SYSTEM_INFO si{};
    GetSystemInfo(&si);
...
static CommandResult setAIContextWindow(const CommandContext& ctx, uint32_t cmdId, const char* label,
                                        uint32_t tokenCount)
{
    if (ctx.isGui && ctx.idePtr)
    {
    ...
    if (cfg.max_tokens > cfg.num_ctx)
        cfg.max_tokens = cfg.num_ctx;
    ...
    if (f)
    {
        fprintf(f, "{\"context_window\":%u,\"label\":\"%s\",\"reserved_bytes\":%llu,\"committed_bytes\":%llu}\n",
                tokenCount, label, reservedBytes, committedBytes);
        fclose(f);
    }
...
{
    return setAIContextWindow(ctx, 4210, "ai.context4k", 4096);
}
CommandResult handleAICtx32K(const CommandContext& ctx)
{
    return setAIContextWindow(ctx, 4211, "ai.context32k", 32768);
}
CommandResult handleAICtx64K(const CommandContext& ctx)
{
    return setAIContextWindow(ctx, 4212, "ai.context64k", 65536);
}
CommandResult handleAICtx128K(const CommandContext& ctx)
{
    return setAIContextWindow(ctx, 4213, "ai.context128k", 131072);
}
CommandResult handleAICtx256K(const CommandContext& ctx)
{
    return setAIContextWindow(ctx, 4214, "ai.context256k", 262144);
}
CommandResult handleAICtx512K(const CommandContext& ctx)
{
    return setAIContextWindow(ctx, 4215, "ai.context512k", 524288);
}
CommandResult handleAICtx1M(const CommandContext& ctx)
{
    return setAIContextWindow(ctx, 4216, "ai.context1m", 1048576);
}
...
CommandResult handleRECompare(const CommandContext& ctx)
{
    ...
    if (pipe)
    {
        char buf[512];
        int n = 0;
        while (fgets(buf, sizeof(buf), pipe) && n < 100)
        {
            ctx.output("  ");
            ctx.output(buf);
            n++;
...
CommandResult handleREDetectVulns(const CommandContext& ctx)
{
    ...
    if (pipe)
    {
        ...
        while (fgets(buf, sizeof(buf), pipe))
        {
            if (strstr(buf, "Dynamic base"))
                hasASLR = true;
            if (strstr(buf, "NX compatible"))
                hasDEP = true;
            if (strstr(buf, "Guard"))
                hasCFG = true;
            if (strstr(buf, "No structured exception"))
                hasSEH = true;
        }
        _pclose(pipe);
...
CommandResult handleREFunctions(const CommandContext& ctx)
{
    ...
    if (ctx.args && ctx.args[0])
    {
        ...
        if (pipe)
        {
            ...
            _pclose(pipe);
            char msg[64];
            snprintf(msg, sizeof(msg), "  Total symbols with (): %d\n", count);
            ctx.output(msg);
        }
        else
        {
...
CommandResult handleRERecursiveDisasm(const CommandContext& ctx)
{
    ...
    if (pipe)
    {
        char buf[512];
        int n = 0;
        while (fgets(buf, sizeof(buf), pipe) && n < 200)
        {
            ctx.output(buf);
            n++;
        }
...
CommandResult handleRETypeRecovery(const CommandContext& ctx)
{
    ...
    if (pipe)
    {
        ...
        _pclose(pipe);
        char msg[64];
        snprintf(msg, sizeof(msg), "  Found %d type-related entries.\n", n);
        ctx.output(msg);
    }
    else
        ctx.output("  dumpbin not found.\n");
...
CommandResult handleREDataFlow(const CommandContext& ctx)
{
    ...
    if (pipe)
    {
        ...
        _pclose(pipe);
        char msg[64];
        snprintf(msg, sizeof(msg), "  %d relocation entries shown.\n", n);
        ctx.output(msg);
    }
    else
        ctx.output("  dumpbin not found.\n");
...
CommandResult handleREDecompilerView(const CommandContext& ctx)
{
    ...
    if (pipe)
    {
        char buf[512];
        int n = 0;
        while (fgets(buf, sizeof(buf), pipe) && n < 150)
        {
            ctx.output(buf);
            n++;
        }
...
CommandResult handleSwarmStartLeader(const CommandContext& ctx)
{
    ...
    if (f)
    {
        ...
        ctx.output("[SWARM] Failed to create config.\n");
    return CommandResult::ok("swarm.startLeader");
}
CommandResult handleSwarmStartWorker(const CommandContext& ctx)
{
    ...
    if (f)
    {
        ...
        ctx.output("[SWARM] Failed to create worker config.\n");
    return CommandResult::ok("swarm.startWorker");
}
CommandResult handleSwarmStartHybrid(const CommandContext& ctx)
{
    ...
    if (f)
    {
        ...
        ctx.output("[SWARM] Config write failed.\n");
    return CommandResult::ok("swarm.startHybrid");
}
CommandResult handleSwarmRemoveNode(const CommandContext& ctx)
{
    ...
    if (removed)
    {
        char buf[256];
        snprintf(buf, sizeof(buf),
                 "[SWARM] Node %u removed from cluster.\n"
...
CommandResult handleSwarmBlacklist(const CommandContext& ctx)
{
    ...
    if (f)
    {
        ...
        ctx.output("[SWARM] Failed to update blacklist.\n");
    return CommandResult::ok("swarm.blacklistNode");
}
#ifndef RAWR_AUTO_FEATURE_REGISTRY_PROVIDES_HANDLERS
CommandResult handleSwarmBuildSources(const CommandContext& ctx)
{
    ...
    char buf[64];
    snprintf(buf, sizeof(buf), "[SWARM] %d source files found.\n", count);
    ctx.output(buf);
    return CommandResult::ok("swarm.buildSources");
}
#endif
...
CommandResult handleSwarmBuildCmake(const CommandContext& ctx)
{
    ...
    if (pipe)
    {
        char buf[512];
        int n = 0;
        while (fgets(buf, sizeof(buf), pipe) && n < 50)
        {
            ctx.output(buf);
            n++;
        }
        int rc = _pclose(pipe);
        char msg[64];
        snprintf(msg, sizeof(msg), "[SWARM] CMake exit: %d\n", rc);
        ctx.output(msg);
    }
    else
        ctx.output("[SWARM] cmake not found.\n");
    return CommandResult::ok("swarm.buildCmake");
}
#endif
#ifndef RAWR_AUTO_FEATURE_REGISTRY_PROVIDES_HANDLERS
CommandResult handleSwarmStartBuild(const CommandContext& ctx)
{
    ...
    if (pipe)
    {
        ...
        int rc = _pclose(pipe);
        char msg[64];
        snprintf(msg, sizeof(msg), "[SWARM] Build exit: %d\n", rc);
        ctx.output(msg);
    }
    else
        ctx.output("[SWARM] Build failed to start.\n");
    return CommandResult::ok("swarm.startBuild");
}
...
CommandResult handleSwarmConfig(const CommandContext& ctx)
{
    ...
    HANDLE h = CreateFileA(".rawrxd_swarm.json", GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
...
CommandResult handleSwarmDiscovery(const CommandContext& ctx)
{
    ...
    if (pipe)
    {
        char buf[512];
        int n = 0;
        while (fgets(buf, sizeof(buf), pipe) && n < 30)
        {
            ctx.output("  ");
            ctx.output(buf);
            n++;
        ...
        _pclose(pipe);
        char msg[64];
        snprintf(msg, sizeof(msg), "[SWARM] %d ARP entries found.\n", n);
        ctx.output(msg);
    }
    else
        ctx.output("[SWARM] ARP scan failed.\n");
    return CommandResult::ok("swarm.discovery");
}
CommandResult handleSwarmTaskGraph(const CommandContext& ctx)
{
    ...
    if (pipe)
    {
        char buf[512];
        int n = 0;
        while (fgets(buf, sizeof(buf), pipe) && n < 40)
        {
            ctx.output("  ");
            ctx.output(buf);
            n++;
        ...
        ctx.output("  CMake not configured. Run !swarm_cmake first.\n");
    return CommandResult::ok("swarm.taskGraph");
}
...
CommandResult handleSwarmWorkerStatus(const CommandContext& ctx)
{
    ...
    snprintf(buf, sizeof(buf),
             "  Processors:  %lu\n  Arch:        %s\n  RAM:         %llu MB total\n  Load:        %lu%%\n",
             si.dwNumberOfProcessors, si.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_AMD64 ? "x64" : "other",
...
CommandResult handleSwarmWorkerConnect(const CommandContext& ctx)
{
    ...
    if (pipe)
    {
        ...
        while (fgets(buf, sizeof(buf), pipe))
        {
            if (strstr(buf, "Reply from") || strstr(buf, "bytes="))
                ok = true;
        }
        _pclose(pipe);
...
CommandResult handleSwarmFitness(const CommandContext& ctx)
{
    ...
    snprintf(buf, sizeof(buf),
             ...
             "  Free RAM:     %llu MB\n"
             "  Fitness:      %s\n",
             ms, std::thread::hardware_concurrency(), mem.ullAvailPhys / (1024 * 1024),
...
CommandResult handleHotpatchServerRemove(const CommandContext& ctx)
{
    ...
    if (ctx.args && ctx.args[0])
    {
        ...
        std::ostringstream oss;
        oss << "[Hotpatch] Server patch '" << ctx.args << "': " << (r.result.success ? "removed" : r.result.detail)
            << "\n";
        ctx.output(oss.str().c_str());
    }
    else
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5062, "lsp.findRefs");
    const std::string symbol = ssoGetArg(ctx, 0);
    if (symbol.empty())
    {
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5063, "lsp.rename");
    ...
    {
        std::ostringstream oss;
        oss << preview.str() << "  mode: preview-only\n"
    ...
    {
        std::string text;
        if (!readTextFileLimited(kv.first, text, 8 * 1024 * 1024))
            continue;
        const size_t replaced = replaceWholeWordOccurrences(text, oldSym, newSym);
        if (replaced == 0)
            continue;
        if (!writeTextFile(kv.first, text))
            continue;
        ++changedFiles;
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 5064, "lsp.hover");
    const std::string symbol = ssoGetArg(ctx, 0);
    if (symbol.empty())
    {
...
CommandResult handleLspSrvStart(const CommandContext& ctx)
{
    ...
    if (requestedServer.empty())
        requestedServer = "clangd";
    ...
    const BOOL launched = CreateProcessA(nullptr, mutableCmd.data(), nullptr, nullptr, FALSE, CREATE_NO_WINDOW, nullptr,
                                         nullptr, &si, &pi);
...
CommandResult handleLspSrvStatus(const CommandContext& ctx)
{
    ...
    for (auto s : servers)
    {
        ...
        ctx.output(s);
        ctx.output(": ");
        ctx.output(found ? "installed" : "not found");
        ctx.output("\n");
    }
    return CommandResult::ok("lspServer.status");
}
...
CommandResult handleLspSrvConfig(const CommandContext& ctx)
{
    ...
    HANDLE h = CreateFileA("compile_commands.json", GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
                           FILE_ATTRIBUTE_NORMAL, nullptr);
    if (h != INVALID_HANDLE_VALUE)
    {
        LARGE_INTEGER sz;
...
CommandResult handleLspSrvExportSymbols(const CommandContext& ctx)
{
    ...
    std::string cmd =
        "findstr /s /r /n \"class \\|struct \\|void \\|int \\|enum \" src\\core\\*.hpp src\\core\\*.h 2>&1";
    FILE* pipe = _popen(cmd.c_str(), "r");
...
CommandResult handlePdbFetch(const CommandContext& ctx)
{
    ...
    if (ctx.args && ctx.args[0])
    {
        std::string cmd = "symchk /s srv*C:\\Symbols*https://msdl.microsoft.com/download/symbols \"" +
                          std::string(ctx.args) + "\" 2>&1";
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 9403, "pdb.cacheClear");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 9404, "pdb.enable");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 9405, "pdb.resolve");
    if (!ctx.args || !ctx.args[0])
    {
    ...
    std::string cmd = "symchk /r \"" + std::string(ctx.args) +
                      "\" /s SRV*C:\\SymCache*https://msdl.microsoft.com/download/symbols 2>&1";
...
CommandResult handlePdbImports(const CommandContext& ctx)
{
    ...
    if (ctx.args && ctx.args[0])
    {
        ...
        ctx.output("Usage: !pdb_imports <binary>\n");
    return CommandResult::ok("pdb.imports");
}
CommandResult handlePdbExports(const CommandContext& ctx)
{
    ...
    if (ctx.args && ctx.args[0])
    {
        ...
        ctx.output("Usage: !pdb_exports <binary>\n");
    return CommandResult::ok("pdb.exports");
}
CommandResult handlePdbIatStatus(const CommandContext& ctx)
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 9412, "pdb.iatStatus");
    if (ctx.args && ctx.args[0])
    {
        std::string cmd =
            "dumpbin /imports \"" + std::string(ctx.args) + "\" 2>&1 | findstr /C:\"Import Address Table\"";
...
CommandResult handleAuditRunFull(const CommandContext& ctx)
{
    ...
    std::ostringstream oss;
    oss << "  Orchestrator: " << orchStats.repairsAttempted << " repairs, " << orchStats.anomaliesDetected
        << " anomalies\n"
        << "  Hotpatch:     " << hpStats.totalOperations.load() << " applied\n"
        << "  Proxy:        biases=" << pStats.biasesApplied << " rewrites=" << pStats.rewritesApplied << "\n"
        << reverseTraceHotspotsReport(8) << "  Audit complete.\n";
    ctx.output(oss.str().c_str());
    return CommandResult::ok("audit.runFull");
}
...
CommandResult handleAuditQuickStats(const CommandContext& ctx)
{
    ...
    if (!args.empty())
    {
        ...
        if (lowerArgs == "trace_reset" || lowerArgs == "reset" || lowerArgs == "trace reset" ||
            lowerArgs == "--trace-reset")
        {
...
CommandResult handleAuditExportReport(const CommandContext& ctx)
{
    ...
    if (f)
    {
        ...
        fprintf(f, "Repairs: %llu  Anomalies: %llu\n", (unsigned long long)oStats.repairsAttempted,
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 9601, "gauntlet.export");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 9701, "voice.ptt");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 9703, "voice.joinRoom");
...
{
    if (ctx.isGui && ctx.idePtr)
        return routeToIde(ctx, 9709, "voice.modeDisabled");
...
{
    return routeToIde(ctx, 9801, "qw.shortcutReset");
}
CommandResult handleQwBackupCreate(const CommandContext& ctx)
{
    ...
    GetLocalTime(&st);
    char dir[128];
    snprintf(dir, sizeof(dir), "backups\\%04d%02d%02d_%02d%02d%02d", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute,
             st.wSecond);
...
CommandResult handleQwBackupPrune(const CommandContext& ctx)
{
    ...
    char buf[64];
    snprintf(buf, sizeof(buf), "  Pruned %d backup(s).\n", pruned);
    ctx.output(buf);
    return CommandResult::ok("qw.backupPrune");
}
#endif
...
CommandResult handleTelemetryDashboard(const CommandContext& ctx)
{
    ...
    oss << "=== Telemetry Dashboard ===\n"
        << "  Hotpatches:  " << stats.totalOperations.load() << " (M:" << stats.memoryPatchCount.load()
        << " B:" << stats.bytePatchCount.load() << " S:" << stats.serverPatchCount.load() << ")\n"
