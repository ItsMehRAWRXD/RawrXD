...
// IDETheme is fully defined via editor_engine.h → IDETheme.h

// MonacoCore tab width (in spaces)
#ifndef MC_TAB_SIZE
...
private:
    ...
    std::vector<uint32_t>       m_lineOffsetCache;  // Byte offsets per line for O(1) lookup
    ...
    // Allow ensureWindowClass() to access WndProc for WNDCLASS registration
    friend void ensureWindowClass(HINSTANCE hInst);
};
...
void ensureWindowClass(HINSTANCE hInst) {
...
bool MonacoCoreEngine::onChar(WCHAR ch) {
    ...
    if (ch == '\t') {
        ...
        char spaces[9] = "        ";  // 8 spaces + null terminator
...
uint32_t MonacoCoreEngine::lineStartOffset(int line) const {
    // Use cached line offsets for O(1) lookup if cache is populated
    if (!m_lineOffsetCache.empty() && line < (int)m_lineOffsetCache.size()) {
        return m_lineOffsetCache[line];
    }
    
    // Fallback: calculate by scanning newlines
    if (line <= 0) return 0;
