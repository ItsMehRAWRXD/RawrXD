...
void NativePDBParser::unload() {
    ...
    // ---- Phase 29.2: Free GSI hash table and TPI parser ----
    if (m_gsiHashTable) {
        delete static_cast<GSIHashTable*>(m_gsiHashTable);
        m_gsiHashTable = nullptr;
    }
    if (m_tpiParser) {
        delete static_cast<TPIStreamParser*>(m_tpiParser);
        m_tpiParser = nullptr;
    }
    if (m_symRecordData) {
...
PDBResult NativePDBParser::findPublicSymbol(const char* name, uint64_t* rvaOut) const {
    ...
    // ---- Phase 29.2: Try GSI hash table for O(1) lookup ----
    if (m_gsiHashTable && m_symRecordData && m_symRecordSize > 0) {
        const GSIHashTable* gsi = static_cast<const GSIHashTable*>(m_gsiHashTable);
        uint32_t nameLen = static_cast<uint32_t>(strlen(name));
        uint32_t symOffset = gsi->findSymbolOffset(name, nameLen);

        if (symOffset != UINT32_MAX && symOffset + 14 < m_symRecordSize) {
            // Extract RVA from the found CVPubSym32 record
            // Record layout: [recLen:2][recTyp:2][pubSymFlags:4][off:4][seg:2][name...]
            const CVPubSym32* pub = reinterpret_cast<const CVPubSym32*>(
                m_symRecordData + symOffset);
            *rvaOut = sectionOffsetToRVA(pub->seg, pub->off);
            return PDBResult::ok("Symbol found (GSI hash)");
        }
        // GSI miss — fall through to linear scan
    }

    // ---- Phase 29 v1 fallback: Linear scan via MASM kernel ----
...
PDBResult NativePDBParser::parsePublicSymbolStream() {
    ...
    if (!readStreamRange(m_symRecordStream, 0, m_symRecordData, symStreamSize)) {
        free(m_symRecordData);
        m_symRecordData = nullptr;
        return PDBResult::error("Failed to read symbol record stream", PDB_ERR_STREAM_OOB);
    }
