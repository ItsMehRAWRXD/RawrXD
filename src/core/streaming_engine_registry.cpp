...
void StreamingEngineRegistry::registerBuiltinEngines() {
    // Community engines — always available
    ...
    // Enterprise-gated engines — require valid license
    if (EnterpriseLicense::isFeatureEnabled(LicenseFeature::DualEngine800B)) {
        registerRawr1024DualEngine();
        std::cout << "[EngineRegistry] Enterprise: 800B Dual-Engine registered" << std::endl;
    } else {
        std::cout << "[EngineRegistry] Community mode: 800B Dual-Engine not available "
                  << "(upgrade to Enterprise for 800B+ model support)" << std::endl;
    }
    
    if (EnterpriseLicense::isFeatureEnabled(LicenseFeature::FlashAttention)) {
        registerFlashAttentionEngine();
        std::cout << "[EngineRegistry] Enterprise: Flash-Attention AVX-512 registered" << std::endl;
    }
}
...
void StreamingEngineRegistry::registerFlashAttentionEngine() {
    // Initialize the AVX-512 Flash Attention engine with license gating
    m_flashAttention.Initialize();  // Handles license + CPUID check internally

    StreamingEngineDescriptor desc;

    if (m_flashAttention.IsReady()) {
        // AVX-512 kernel available (Pro tier + hardware capable)
        desc.name = "FlashAttention-AVX512";
        desc.description = "AVX-512 Flash-Attention v2 — tiled O(N) memory, online softmax, "
                          "causal masking, GQA support. 2.4x over AVX2 C intrinsics.";
        desc.version = "2.0.0";
        desc.sourceFile = "src/asm/FlashAttention_AVX512.asm";
        desc.capabilities = EngineCapability::FlashAttention | EngineCapability::AVX512_DMA
                          | EngineCapability::Quantization;
    } else {
        // Fallback: register descriptor without active kernel
    ...
    desc.loaded = m_flashAttention.IsReady();
    registerEngine(desc);
}
