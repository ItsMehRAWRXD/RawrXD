...
void PrintEnterpriseBanner() {
    ...
    if (state == LicenseState::ValidTrial) {
        uint64_t remainingSec = lic.GetTrialRemainingSeconds();
        uint64_t remainingDays = (remainingSec + 86399ULL) / 86400ULL;
        std::ostringstream trial;
        trial << remainingDays << " days remaining";
        std::cout << "║ Trial:         " << std::left << std::setw(56)
                  << trial.str() << " ║\n";
    }

    {
        std::string aadStatus = lic.IsAzureADConfigured() ? "Configured" : "Not configured";
        std::string authority = lic.GetAzureADAuthority();
        if (!authority.empty() && lic.IsAzureADConfigured()) {
            aadStatus += " (" + authority + ")";
        }
        std::cout << "║ Azure AD:      " << std::left << std::setw(56)
                  << aadStatus << " ║\n";
    }
...
public:
    static void Show() {
        // Win32: native dialog with license status can be added here; console fallback for now.
...
public:
    static void Show() {
        // Linux: GTK/native dialog can be added here; console fallback for now.
        PrintLicenseDashboard();
    }
};
