...
// ================================================
// Result type (used by make_result and API)
// ================================================
enum class CheckpointStatus { Ok = 0, Error, NotFound };
constexpr CheckpointStatus CHECKPOINT_SUCCESS = CheckpointStatus::Ok;
constexpr CheckpointStatus CHECKPOINT_ERROR = CheckpointStatus::Error;

struct CheckpointResult {
    CheckpointStatus status = CheckpointStatus::Error;
    char message[256] = {};
};
...
// Passthrough compression (no-op copy; optional LZ4 can be linked for real compression)
...
// ================================================
// CheckpointManager class (include/checkpoint_manager.h) bridge to global state
// ================================================
...
bool CheckpointManager::initialize(const std::string& checkpointDir, int maxCheckpoints) {
    ...
    CheckpointResult r = checkpoint_initialize(checkpointDir.empty() ? nullptr : checkpointDir.c_str());
    return r.status == CHECKPOINT_SUCCESS;
}

std::vector<CheckpointManager::CheckpointIndex> CheckpointManager::listCheckpoints() const {
    std::vector<CheckpointIndex> out;
    std::lock_guard<std::mutex> lock(g_checkpoint_mutex);
    const size_t n = g_checkpoint_count.load();
    for (size_t i = 0; i < n; ++i) {
        const CheckpointEntry& e = g_checkpoints[i];
        CheckpointIndex idx;
        idx.checkpointId = e.checkpoint_id;
        idx.filePath = e.file_path;
        idx.metadata.timestamp = static_cast<int64_t>(e.header.timestamp);
        idx.checkpointNumber = static_cast<int>(i);
...
void CheckpointManager::show() {
    if (m_showCb) m_showCb(m_showCtx, listCheckpoints());
}
