...
// ============================================================================
// Shim for RawrXD::MultiGPUManager (Linker Closure)
// ============================================================================
namespace RawrXD {
    // Some components expect MultiGPUManager in RawrXD namespace
    class MultiGPUManager {
    public:
        static auto& Instance() {
            return RawrXD::Enterprise::MultiGPUManager::Instance();
        }
    };
    
    // This is a bit tricky since the class definition might conflict if it's already in a header
    // but the linker error suggests it's MISSING, so providing a bridge here:
}
...
MultiGPUResult MultiGPUManager::BuildLayerAssignments(uint32_t totalLayers,
    ...
    // Validate model memory budget vs device VRAM
    uint64_t totalFree = 0;
    uint64_t totalVram = 0;
    for (const auto& d : m_devices) {
        uint64_t avail = (d.vramFreeBytes > 0) ? d.vramFreeBytes : d.vramBytes;
        totalFree += avail;
        totalVram += d.vramBytes;
    }
    ...
    if (modelBytes > 0) {
        if (effective == DispatchStrategy::DataParallel) {
            for (const auto& d : m_devices) {
                uint64_t avail = (d.vramFreeBytes > 0) ? d.vramFreeBytes : d.vramBytes;
                if (avail < modelBytes) {
                    return MultiGPUResult::error("DataParallel requires full model per GPU", 5);
                }
            }
        } else if (totalFree < modelBytes) {
            return MultiGPUResult::error("Insufficient total VRAM for model", 6);
        }
    }

    // Tensor-parallel requires P2P topology; fall back if unsupported
    if (effective == DispatchStrategy::TensorParallel && m_devices.size() > 1) {
        bool allP2P = !m_topology.empty();
        if (allP2P) {
            for (size_t i = 0; i < m_devices.size(); ++i) {
                for (size_t j = i + 1; j < m_devices.size(); ++j) {
                    bool linkOk = false;
                    for (const auto& link : m_topology) {
                        if ((link.srcDevice == i && link.dstDevice == j) ||
                            (link.srcDevice == j && link.dstDevice == i)) {
                            linkOk = (link.type != LinkType::None);
                            break;
                        }
                    }
                    if (!linkOk) {
        ...
        if (!allP2P) {
            effective = DispatchStrategy::LayerParallel;
        }
    }
    ...
    if (effective == DispatchStrategy::DataParallel ||
        ...
        for (const auto& d : m_devices) {
            ...
            if (modelBytes > 0 && effective == DispatchStrategy::DataParallel) {
    ...
    if (modelBytes > 0 && totalLayers > 0) {
        perLayerBytes = modelBytes / totalLayers;
        if (perLayerBytes == 0) perLayerBytes = 1;
    }
    ...
    for (size_t i = 0; i < m_devices.size(); ++i) {
        ...
        uint64_t avail = (m_devices[i].vramFreeBytes > 0) ? m_devices[i].vramFreeBytes
                                                          : m_devices[i].vramBytes;
        if (perLayerBytes > 0) {
            uint32_t layerCount = a.endLayer - a.startLayer + 1;
            uint64_t need = perLayerBytes * static_cast<uint64_t>(layerCount);
            a.vramBudgetBytes = (need < avail) ? need : avail;
        } else {
            a.vramBudgetBytes = avail;
        }
        a.strategy = effective;
...
MultiGPUResult MultiGPUManager::DispatchBatch(uint32_t batchId,
    ...
    auto r = BuildLayerAssignments(totalLayers, modelBytes, strategy);
    if (!r.success) {
        return r;
    }
    ...
    m_dispatchStats.totalDispatches += 1;
    m_dispatchStats.lastBatchId = batchId;
    m_dispatchStats.lastElapsedMs = elapsedMs;
    m_dispatchStats.lastStrategy = m_strategy;
    m_dispatchStats.lastAssignmentCount = static_cast<uint32_t>(m_assignments.size());

    if (m_onDispatchComplete) {
        m_onDispatchComplete(batchId, elapsedMs);
    }

    return MultiGPUResult::ok("Dispatch plan generated");
}
...
std::string MultiGPUManager::GenerateStatusReport() const {
    ...
    if (m_dispatchStats.totalDispatches > 0) {
        std::ostringstream d;
        d << m_dispatchStats.totalDispatches << " dispatches, last "
          << std::fixed << std::setprecision(2) << m_dispatchStats.lastElapsedMs << " ms";
        ss << "│ Dispatch: " << std::left << std::setw(44) << d.str() << "│\n";
    }
