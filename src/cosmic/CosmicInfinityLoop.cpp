#include "CosmicInfinityLoop.hpp"
...
std::atomic<bool> CosmicInfinityLoop::s_paused{false};
std::atomic<bool> CosmicInfinityLoop::s_shouldStop{false};

CosmicInfinityLoopConfig CosmicInfinityLoop::s_config;
CosmicInfinityLoopMetrics CosmicInfinityLoop::s_metrics;

std::thread CosmicInfinityLoop::s_tickThread;
std::thread CosmicInfinityLoop::s_renderThread;

std::mutex CosmicInfinityLoop::s_tickCallbackMutex;
std::mutex CosmicInfinityLoop::s_frameCallbackMutex;
std::mutex CosmicInfinityLoop::s_renderCallbackMutex;
std::mutex CosmicInfinityLoop::s_updateCallbackMutex;
std::mutex CosmicInfinityLoop::s_metricsMutex;

std::vector<CosmicInfinityLoop::TickCallback> CosmicInfinityLoop::s_tickCallbacks;
std::vector<CosmicInfinityLoop::FrameCallback> CosmicInfinityLoop::s_frameCallbacks;
std::vector<CosmicInfinityLoop::RenderCallback> CosmicInfinityLoop::s_renderCallbacks;
std::vector<CosmicInfinityLoop::UpdateCallback> CosmicInfinityLoop::s_updateCallbacks;

bool CosmicInfinityLoop::Init(const CosmicInfinityLoopConfig& config) {
...
bool CosmicInfinityLoop::IsInitialized() {
...
void CosmicInfinityLoop::Pause() {
    s_paused.store(true);
}

void CosmicInfinityLoop::Resume() {
    s_paused.store(false);
}

bool CosmicInfinityLoop::IsPaused() {
    return s_paused.load();
}

void CosmicInfinityLoop::SetConfig(const CosmicInfinityLoopConfig& config) {
    s_config = config;
}

CosmicInfinityLoopConfig CosmicInfinityLoop::GetConfig() {
    return s_config;
}

CosmicInfinityLoopMetrics CosmicInfinityLoop::GetMetrics() {
    std::lock_guard<std::mutex> lock(s_metricsMutex);
    return s_metrics;
}

float CosmicInfinityLoop::GetCurrentTPS() {
    std::lock_guard<std::mutex> lock(s_metricsMutex);
...
int64_t CosmicInfinityLoop::GetFrameCount() {
    std::lock_guard<std::mutex> lock(s_metricsMutex);
    return s_metrics.totalFrames;
}

void CosmicInfinityLoop::RegisterTickCallback(TickCallback callback) {
    std::lock_guard<std::mutex> lock(s_tickCallbackMutex);
    s_tickCallbacks.push_back(callback);
}

void CosmicInfinityLoop::UnregisterTickCallback(TickCallback callback) {
    std::lock_guard<std::mutex> lock(s_tickCallbackMutex);
    // Note: Cannot reliably compare std::function instances
}

void CosmicInfinityLoop::RegisterFrameCallback(FrameCallback callback) {
    std::lock_guard<std::mutex> lock(s_frameCallbackMutex);
    s_frameCallbacks.push_back(callback);
}

void CosmicInfinityLoop::UnregisterFrameCallback(FrameCallback callback) {
    std::lock_guard<std::mutex> lock(s_frameCallbackMutex);
    // Note: Cannot reliably compare std::function instances
}

void CosmicInfinityLoop::RegisterRenderCallback(RenderCallback callback) {
    std::lock_guard<std::mutex> lock(s_renderCallbackMutex);
    s_renderCallbacks.push_back(callback);
}

void CosmicInfinityLoop::UnregisterRenderCallback(RenderCallback callback) {
    std::lock_guard<std::mutex> lock(s_renderCallbackMutex);
    // Note: Cannot reliably compare std::function instances
}

void CosmicInfinityLoop::RegisterUpdateCallback(UpdateCallback callback) {
    std::lock_guard<std::mutex> lock(s_updateCallbackMutex);
    s_updateCallbacks.push_back(callback);
}

void CosmicInfinityLoop::UnregisterUpdateCallback(UpdateCallback callback) {
    std::lock_guard<std::mutex> lock(s_updateCallbackMutex);
    // Note: Cannot reliably compare std::function instances
}

void CosmicInfinityLoop::Tick(float deltaTime) {
    {
        std::lock_guard<std::mutex> lock(s_tickCallbackMutex);
...
void CosmicInfinityLoop::TickLoop() {
...
void CosmicInfinityLoop::RenderLoop() {
    using namespace std::chrono;
...
void CosmicInfinityLoop::UpdateMetrics(float tickTimeMs, float frameTimeMs) {
    std::lock_guard<std::mutex> lock(s_metricsMutex);
    s_metrics.averageTickTimeMs = s_metrics.averageTickTimeMs * 0.95f + tickTimeMs * 0.05f;
    s_metrics.averageFrameTimeMs = s_metrics.averageFrameTimeMs * 0.95f + frameTimeMs * 0.05f;
}
