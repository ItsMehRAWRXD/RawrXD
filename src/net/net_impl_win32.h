...
public:
    ...
    RawrXD::Expected<HttpResponse, NetError> get(
        const std::string& url,
        const std::unordered_map<std::string, std::string>& headers = {}
    );
    
    RawrXD::Expected<HttpResponse, NetError> post(
        const std::string& url,
        const std::string& body,
        const std::unordered_map<std::string, std::string>& headers = {}
    );
    
    RawrXD::Expected<HttpResponse, NetError> postJson(
        const std::string& url,
    ...
    // Real streaming
    RawrXD::Expected<void, NetError> streamGet(
        const std::string& url,
...
private:
    ...
    // Real implementation
    RawrXD::Expected<HttpResponse, NetError> executeRequest(
    ...
    RawrXD::Expected<void, NetError> addAuthHeaders(
        std::unordered_map<std::string, std::string>& headers
    );
    
    RawrXD::Expected<void, NetError> handleCompression(
        HttpResponse& response
    );
    
    RawrXD::Expected<void, NetError> handleRedirects(
        HttpResponse& response,
        const std::string& url = "",
        int redirects = 0
    );
    ...
    // Retry logic
    RawrXD::Expected<HttpResponse, NetError> executeWithRetry(
...
public:
    ...
    RawrXD::Expected<void, NetError> connect(
        const std::string& url,
        const std::unordered_map<std::string, std::string>& headers = {}
    );
    ...
    void setMessageHandler(std::function<void(const WebSocketFrame&)> handler);

    // Send messages
    RawrXD::Expected<void, NetError> sendText(const std::string& message);
    RawrXD::Expected<void, NetError> sendBinary(const std::vector<uint8_t>& data);
    
    // Receive message
    RawrXD::Expected<WebSocketFrame, NetError> receiveFrame(
        std::chrono::milliseconds timeout = std::chrono::milliseconds(5000)
    );

    // Ping/Pong
    RawrXD::Expected<void, NetError> ping();
    RawrXD::Expected<void, NetError> pong();
...
private:
    ...
    // Real implementation
    RawrXD::Expected<void, NetError> performHandshake(
        const std::string& host,
        const std::unordered_map<std::string, std::string>& headers
    );
    
    RawrXD::Expected<void, NetError> sendFrame(
        WebSocketFrame::Opcode opcode,
        const std::vector<uint8_t>& payload,
        bool isFinal = true
    );
    
    RawrXD::Expected<WebSocketFrame, NetError> parseFrame(
...
public:
    ...
    RawrXD::Expected<void, NetError> initialize(const NetworkConfig& config);
    void shutdown();
