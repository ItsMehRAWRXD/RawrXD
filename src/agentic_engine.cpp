...
void AgenticEngine::initialize() {
    ...
    if (!compression_provider_) {
        qWarning() << "[AgenticEngine] No compression provider available";
    }
...
// ---- compression helpers ----
bool AgenticEngine::compressData(const QByteArray& in, QByteArray& out)
{
    if (!compression_provider_) return false;
    std::vector<uint8_t> in_vec(in.begin(), in.end());
    std::vector<uint8_t> out_vec;
    if (!compression_provider_->Compress(in_vec, out_vec)) return false;
    out = QByteArray(reinterpret_cast<const char*>(out_vec.data()), out_vec.size());
    return true;
}

bool AgenticEngine::decompressData(const QByteArray& in, QByteArray& out)
{
    if (!compression_provider_) return false;
    std::vector<uint8_t> in_vec(in.begin(), in.end());
    std::vector<uint8_t> out_vec;
    if (!compression_provider_->Decompress(in_vec, out_vec)) return false;
    out = QByteArray(reinterpret_cast<const char*>(out_vec.data()), out_vec.size());
    return true;
}

QString AgenticEngine::getCompressionStats() const
{
    if (!compression_provider_) return "No provider";
    auto stats = compression_provider_->GetStats();
    return QString("kernel=%1 calls=%2 ratio=%3%")
        .arg(stats.active_kernel.c_str())
        .arg(stats.total_calls)
        .arg(stats.avg_ratio * 100.0, 0, 'f', 1);
}

bool AgenticEngine::optimizeCompressionForModel(const QString& path)
{
    if (!compression_provider_) return false;
    qint64 sz = QFileInfo(path).size();
    uint8_t lvl = (sz < 1ULL<<30) ? 3 : (sz < 10ULL<<30 ? 6 : 9);
    if (auto d = std::dynamic_pointer_cast<DeflateWrapper>(compression_provider_))
        d->SetCompressionLevel(lvl);
    qInfo() << "[Agentic] Auto-set compression level" << lvl << "for" << path;
    return true;
}
