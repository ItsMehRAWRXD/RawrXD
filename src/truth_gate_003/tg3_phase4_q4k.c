/*
 * Truth Gate 003 - Phase 4: Full Transformer with Q4_K Support
 * 
 * Complete transformer with attention, RoPE, SwiGLU, KV cache
 * Now with Q4_K dequantization for output layer
 */
...
/* Dequantize Q4_K block - simplified version */
void dequantize_q4_k(const block_q4_K *block, float *out, int n) {
    ...
    /* scales[0-7] and mins[0-7] are packed as 4-bit values */
    for (int i = 0; i < 8; i++) {
        /* Scale is in first 8 nibbles */
        int scale_byte = i / 2;
        int scale_nibble = (i % 2 == 0) ? (block->scales[scale_byte] & 0x0F) 
                                        : ((block->scales[scale_byte] >> 4) & 0x0F);
        
        /* Min is in next 8 nibbles (bytes 4-7) */
        int min_byte = 4 + i / 2;
...
int sample_token(float *logits, int n_vocab, float temperature) {
    if (temperature == 0.0f) {
        /* Greedy - argmax */
        int best = 0;
...
int generate(TransformerCtx *ctx, int *tokens, int n_tokens, int max_new,
    ...
    for (int gen = 0; gen < max_new; gen++) {
        ...
        if (output_weight) {
            /* Use Q4_K dequantization for output layer */
            matvec_dequantized(ctx->tensor_base, output_weight, normed, logits, n_vocab, n_embd);
        } else {
        ...
        /* Sample next token */
        int next_token = sample_token(logits, n_vocab, temperature);
...
int main(int argc, char **argv) {
    ...
    printf("Truth Gate 003 - Phase 4: Full Transformer with Q4_K\n");
    printf("========================================================\n\n");
    ...
    for (uint64_t i = 0; i < n_tensors; i++) {
        ...
        p += name_len;
        tensors[i].offset = (p - (uint8_t*)mapped);
        uint64_t data_size = *(uint64_t*)p; p += 8;
        p += data_size;
    }
