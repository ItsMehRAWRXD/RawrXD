/*
 * Truth Gate 003 - TG3-G4: Multi-Token Generation
 * 
 * Purpose: Generate multiple tokens autoregressively
 * Acceptance: Generate 5+ tokens deterministically
 */
...
/* Compute logits for a single token */
int compute_logits(const uint8_t *tensor_base, TensorInfo *output_weight, 
                   const float *embedding, float *logits, int n_vocab, int embd_dim) {
    if (output_weight->type == 2) { /* Q4_0 */
        int blocks_per_row = embd_dim / 32;
        
        for (int tok = 0; tok < n_vocab && tok < 1000; tok++) { /* Sample first 1000 */
...
int main(int argc, char **argv) {
    printf("Truth Gate 003 - TG3-G4: Multi-Token Generation\n");
    printf("================================================\n\n");
    
    if (argc < 2) {
        printf("Usage: %s <model.gguf> [prompt] [num_tokens]\n", argv[0]);
        printf("  Default prompt: 'The capital of France is'\n");
        printf("  Default num_tokens: 5\n");
        return 1;
    }
    
    const char *model_path = argv[1];
    const char *prompt = (argc > 2) ? argv[2] : "The capital of France is";
    int num_tokens_to_generate = (argc > 3) ? atoi(argv[3]) : 5;
    ...
    float *logits = calloc(1000, sizeof(float)); /* Sample first 1000 */
    
    /* Get last token from prompt as starting point */
    int current_token = tokens[n_tokens - 2]; /* Last content token before EOS */
    
    printf("\nGeneration:\n");
    printf("  Starting from token %d: '%s'\n", current_token, vocab[current_token]);
    
    for (int gen = 0; gen < num_tokens_to_generate; gen++) {
        ...
        /* Compute logits */
        int n_logits = compute_logits(tensor_base, output_weight, norm_embedding, logits, n_vocab, embd_dim);
        
        /* Find argmax */
        int next_token = argmax(logits, n_logits);
        
        printf("  [%d] Token %d: '%s' (logit=%.4f)\n", 
               gen + 1, next_token, vocab[next_token], logits[next_token]);
        
        /* Validate */
        if (isnan(logits[next_token]) || isinf(logits[next_token])) {
    ...
    printf("\n====================================\n");
    printf("TG3-G4 Status: PASS\n");
    printf("====================================\n");
    printf("Generated %d tokens successfully!\n", num_tokens_to_generate);
    printf("\nNext: TG3-G5 (Temperature Sampling)\n");
