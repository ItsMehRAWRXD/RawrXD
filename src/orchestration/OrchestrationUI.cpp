    ...
    , m_orchestrator(orchestrator)
{
    ...
    if (m_orchestrator) {
        ...
        connect(m_orchestrator, &TaskOrchestrator::taskProgress,
                this, &OrchestrationUI::onTaskProgress);
...
QWidget* OrchestrationUI::createMemorySettingsSection()
{
    QGroupBox* memoryGroup = new QGroupBox("Memory Management");
    QVBoxLayout* layout = new QVBoxLayout(memoryGroup);
    
    // Memory profile selection
    QHBoxLayout* profileLayout = new QHBoxLayout();
    QLabel* profileLabel = new QLabel("Memory Profile:");
    m_memoryProfileCombo = new QComboBox();
    ...
    QLabel* strategyLabel = new QLabel("Allocation Strategy:");
    m_memoryStrategyCombo = new QComboBox();
    ...
    QHBoxLayout* usageLayout = new QHBoxLayout();
    m_memoryUsageLabel = new QLabel("Memory Usage: 0 B / 1 GB");
    ...
    // Connect signals
    connect(m_memoryProfileCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &OrchestrationUI::onMemoryProfileChanged);
    connect(m_memoryStrategyCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &OrchestrationUI::onMemoryStrategyChanged);
...
{
    if (m_orchestrator) {
        QString profile = m_memoryProfileCombo->itemData(index).toString();
...
{
    if (m_orchestrator) {
        QString strategy = m_memoryStrategyCombo->itemData(index).toString();
...
void OrchestrationUI::updateMemoryUsage()
{
    ...
    qint64 used = m_orchestrator->getTotalMemoryUsage();
    qint64 available = m_orchestrator->getAvailableMemory();
    
    QString usedStr = formatMemorySize(used);
    QString availableStr = (available == LLONG_MAX) ? "Unlimited" : formatMemorySize(available);
    
    m_memoryUsageLabel->setText(QString("Memory Usage: %1 / %2").arg(usedStr).arg(availableStr));
...
QString OrchestrationUI::formatMemorySize(qint64 bytes) const
{
