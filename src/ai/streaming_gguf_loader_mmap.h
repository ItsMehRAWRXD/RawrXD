...
/**
 * Memory-mapped streaming GGUF loader (Win32 + STL + optional Vulkan VRAM upload).
 * No Qt dependency. Supports large models without loading the entire file into RAM.
 */
...
class StreamingGGUFLoaderMmap
{
  private:
    ...
    template <typename T> bool readValue(T& value, size_t offset);
  ...
  private:
    ...
    std::unordered_map<std::string, VramBuffer> vramTensors;
    bool m_vramResident = false;

  public:
    bool LoadVramResident();
    bool IsVramResident() const { return m_vramResident; }
    void* GetTensorVramPtr(const std::string& name);
};

// Template implementation
template <typename T> inline bool StreamingGGUFLoaderMmap::readValue(T& value, size_t offset)
{
