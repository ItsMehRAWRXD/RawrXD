...
class MemoryMappedFile
{
  ...
  public:
     ...
     */
    template <typename T> const T* GetStruct(size_t offset) const
    {
        if (offset > fileSize || sizeof(T) > fileSize - offset)
            return nullptr;
        if (!EnsureMappedRange(offset, sizeof(T)))
            return nullptr;
    ...
    const void* GetRegion(size_t offset, size_t size) const
    {
        if (offset > fileSize || size > fileSize - offset)
            return nullptr;
        if (!EnsureMappedRange(offset, size))
            return nullptr;
    ...
    std::string GetString(size_t offset, size_t length) const
    {
        if (offset > fileSize || length > fileSize - offset)
            return "";
        if (!EnsureMappedRange(offset, length))
            return "";
    ...
    MemoryStats GetStats() const { return {fileSize, mappedViewSize, IsOpen(), filePath}; }
};
