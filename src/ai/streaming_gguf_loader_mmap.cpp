<<<<<<< HEAD:src/ai/streaming_gguf_loader_qt.cpp
...
std::string GGUFMetadataValue::AsString() const
{
    if (type != GGUFValueType::STRING || data.empty())
        return "";
    return std::string(reinterpret_cast<const char*>(data.data()), data.size());
}
...
uint32_t GGUFMetadataValue::AsUInt32() const
{
    if (type != GGUFValueType::UINT32 || data.size() < sizeof(uint32_t))
        return 0;
    uint32_t value;
    std::memcpy(&value, data.data(), sizeof(uint32_t));
    return value;
...
int32_t GGUFMetadataValue::AsInt32() const
{
    if (type != GGUFValueType::INT32 || data.size() < sizeof(int32_t))
        return 0;
    int32_t value;
    std::memcpy(&value, data.data(), sizeof(int32_t));
    return value;
...
uint64_t GGUFMetadataValue::AsUInt64() const
{
    if (type != GGUFValueType::UINT64 || data.size() < sizeof(uint64_t))
        return 0;
    uint64_t value;
    std::memcpy(&value, data.data(), sizeof(uint64_t));
    return value;
...
int64_t GGUFMetadataValue::AsInt64() const
{
    if (type != GGUFValueType::INT64 || data.size() < sizeof(int64_t))
        return 0;
    int64_t value;
    std::memcpy(&value, data.data(), sizeof(int64_t));
    return value;
...
float GGUFMetadataValue::AsFloat32() const
{
    if (type != GGUFValueType::FLOAT32 || data.size() < sizeof(float))
        return 0.0f;
    float value;
    std::memcpy(&value, data.data(), sizeof(float));
    return value;
...
double GGUFMetadataValue::AsFloat64() const
{
    if (type != GGUFValueType::FLOAT64 || data.size() < sizeof(double))
        return 0.0;
    double value;
    std::memcpy(&value, data.data(), sizeof(double));
    return value;
...
bool GGUFMetadataValue::AsBool() const
{
    if (type != GGUFValueType::BOOL || data.empty())
        return false;
    return data[0] != 0;
}

// StreamingGGUFLoaderMmap implementation
...
void StreamingGGUFLoaderMmap::setError(const std::string& error)
{
    lastError = error;
}

bool StreamingGGUFLoaderMmap::loadModel(const std::string& filePath)
{
...
void StreamingGGUFLoaderMmap::close()
{
    mappedFile.Close();
    metadata.clear();
    tensors.clear();
...
bool StreamingGGUFLoaderMmap::parseHeader()
{
    // Read GGUF header (20 bytes total for v3)
    if (!readValue(header.magic, currentFileOffset))
    {
    ...
    {
        std::ostringstream oss;
        oss << "Invalid GGUF magic: 0x" << std::hex << header.magic << " (expected 0x" << GGUF_MAGIC << ")";
        setError(oss.str());
        return false;
    }
    ...
    if (header.version != GGUF_VERSION_3)
    {
        setError("Unsupported GGUF version: " + std::to_string(header.version) + " (only version 3 supported)");
        return false;
    }
...
bool StreamingGGUFLoaderMmap::readString(std::string& str, size_t offset, size_t& newOffset)
{
    // Read string length (uint64_t in GGUF v3)
    uint64_t length;
    if (!readValue(length, offset))
    ...
    if (length > 100 * 1024 * 1024)
    {
        setError("String length too large: " + std::to_string(length) + " (possible corruption or parsing error)");
        return false;
    }
...
bool StreamingGGUFLoaderMmap::parseMetadata()
{
    ...
    for (uint64_t i = 0; i < header.metadata_count; ++i)
    {
        ...
        if (key.empty())
        {
            setError("Empty metadata key at index " + std::to_string(i) + "; possible file corruption");
            return false;
        }
        ...
        if (valueTypeRaw > 12)
        {  // GGUF spec limit on value types
            setError("Invalid metadata value type " + std::to_string(valueTypeRaw) + " for key: " + key +
        ...
        switch (valueType)
        {
            ...
            default:
                setError("Unsupported metadata value type: " + std::to_string(valueTypeRaw) + " for key: " + key);
                return false;
        }
...
bool StreamingGGUFLoaderMmap::parseTensorInfo()
{
    ...
    for (uint64_t i = 0; i < header.tensor_count; ++i)
    {
        ...
        for (uint32_t d = 0; d < nDims; ++d)
        {
            ...
            {
                setError("Tensor " + tensor.name + " dimension " + std::to_string(d) + " exceeds max int32 (" +
                         std::to_string(tensor.shape[d]) + "); likely file corruption");
                return false;
            }
            currentFileOffset += sizeof(uint64_t);
        }
...
bool StreamingGGUFLoaderMmap::LoadVramResident()
{
    if (!isLoaded)
    {
    ...
    auto& vmm = RawrXD::Core::VulkanMemoryManager::instance();
    if (!vmm.Initialize())
    {
        setError("Vulkan init failed");
        return false;
    }
    ...
    {
        VramBuffer vb;
        vb.size = t.size_bytes;

        vb.buffer = vmm.CreateBuffer(vb.size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                                     VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vb.memory);

        if (vb.buffer == VK_NULL_HANDLE)
        {
            setError("Failed to allocate VRAM for tensor: " + t.name);
            return false;
        }
        ...
        VkBuffer stagingBuf =
            vmm.CreateBuffer(vb.size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                             VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingMem);

        void* mapped = vmm.MapVRAM(stagingMem, vb.size);
        const void* src = mappedFile.GetRegion(t.offset, vb.size);
        if (!src)
        {
            setError("Failed to map file region for tensor: " + t.name);
            vkDestroyBuffer(vmm.GetDevice(), stagingBuf, nullptr);
            vkFreeMemory(vmm.GetDevice(), stagingMem, nullptr);
            return false;
        }
        std::memcpy(mapped, src, vb.size);
        vmm.UnmapVRAM(stagingMem);

        vmm.CopyBuffer(stagingBuf, vb.buffer, vb.size);

        // Cleanup staging
        vkDestroyBuffer(vmm.GetDevice(), stagingBuf, nullptr);
        vkFreeMemory(vmm.GetDevice(), stagingMem, nullptr);

        vramTensors[t.name] = vb;
    }
...
void StreamingGGUFLoaderMmap::buildTensorIndex()
{
    tensorNameMap.clear();
    for (size_t i = 0; i < tensors.size(); ++i)
    {
...
size_t StreamingGGUFLoaderMmap::getTensorDataSize(GGMLType type, const std::vector<uint64_t>& shape) const
{
    // Calculate total elements
    size_t elements = 1;
    for (uint64_t dim : shape)
...
size_t StreamingGGUFLoaderMmap::getTypeSize(GGMLType type) const
{
    switch (type)
    {
        case GGMLType::F32:
        ...
        case GGMLType::F16:
            return 2;
        case GGMLType::Q4_0:
            return 18;  // Block size for Q4_0
        case GGMLType::Q4_1:
            return 20;  // Block size for Q4_1
        case GGMLType::Q5_0:
            return 22;  // Block size for Q5_0
        case GGMLType::Q5_1:
            return 24;  // Block size for Q5_1
        case GGMLType::Q8_0:
            return 34;  // Block size for Q8_0
        case GGMLType::Q8_1:
            return 36;  // Block size for Q8_1
        case GGMLType::Q2_K:
            return 84;  // Block size for Q2_K
        case GGMLType::Q3_K:
            return 110;  // Block size for Q3_K
        case GGMLType::Q4_K:
            return 144;  // Block size for Q4_K
        case GGMLType::Q5_K:
            return 176;  // Block size for Q5_K
        case GGMLType::Q6_K:
            return 210;  // Block size for Q6_K
        default:
            return 1;  // Fallback
    }
}
...
std::vector<std::vector<uint8_t>> StreamingGGUFLoaderMmap::getMultipleTensors(
    const std::vector<std::string>& tensorNames)
{
    std::vector<std::vector<uint8_t>> results;
    results.reserve(tensorNames.size());
...
bool StreamingGGUFLoaderMmap::hasMetadata(const std::string& key) const
{
    return metadata.find(key) != metadata.end();
}

GGUFMetadataValue StreamingGGUFLoaderMmap::getMetadata(const std::string& key) const
{
    auto it = metadata.find(key);
    if (it != metadata.end())
    {
...
std::vector<std::string> StreamingGGUFLoaderMmap::getAllMetadataKeys() const
{
    std::vector<std::string> keys;
    keys.reserve(metadata.size());
    for (const auto& pair : metadata)
...
std::string StreamingGGUFLoaderMmap::getModelName() const
{
    if (hasMetadata("general.name"))
    {
        return getMetadata("general.name").AsString();
...
std::string StreamingGGUFLoaderMmap::getModelArchitecture() const
{
    if (hasMetadata("general.architecture"))
    {
        return getMetadata("general.architecture").AsString();
...
uint64_t StreamingGGUFLoaderMmap::getModelContextLength() const
{
    if (hasMetadata("llama.context_length"))
    {
        return getMetadata("llama.context_length").AsUInt64();
...
StreamingGGUFLoaderMmap::MemoryStats StreamingGGUFLoaderMmap::getMemoryStats() const
{
    MemoryStats stats;
    stats.totalFileSize = mappedFile.GetFileSize();
    stats.loadedTensorsCount = 0;
...
>>>>>>> origin/main:src/ai/streaming_gguf_loader_mmap.cpp
