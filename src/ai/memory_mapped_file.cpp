...
MemoryMappedFile::MemoryMappedFile(MemoryMappedFile&& other) noexcept
    : fileHandle(other.fileHandle), mappingHandle(other.mappingHandle), mappedViewBase(other.mappedViewBase),
...
bool MemoryMappedFile::EnsureMappedRange(size_t offset, size_t size) const
{
    ...
    void* view = MapViewOfFile(mappingHandle, FILE_MAP_READ, static_cast<DWORD>(alignedStart >> 32),
    ...
    // Warm the OS page cache for this view asynchronously.  PrefetchVirtualMemory
    // issues the async I/O requests immediately, so by the time the caller reads
    // sequentially the pages are already in flight.  Available since Windows 8.
#if defined(_WIN32_WINNT) && _WIN32_WINNT >= 0x0602
    WIN32_MEMORY_RANGE_ENTRY prefetchRange;
    prefetchRange.VirtualAddress = view;
    prefetchRange.NumberOfBytes  = static_cast<SIZE_T>(alignedSize);
    PrefetchVirtualMemory(GetCurrentProcess(), 1, &prefetchRange, 0);
#endif
...
bool MemoryMappedFile::Open(const std::string& path)
{
    ...
    // FILE_FLAG_SEQUENTIAL_SCAN tells the OS readahead/cache manager we will
    // traverse the file front-to-back (typical for model loading), enabling
    // aggressive async prefetch at the OS read-ahead tier.
    fileHandle = CreateFileA(path.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
