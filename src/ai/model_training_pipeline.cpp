...
// TrainingModelVersion JSON serialization
QJsonObject TrainingModelVersion::toJson() const {
    QJsonObject obj;
...
void TrainingModelVersion::fromJson(const QJsonObject& json) {
...
bool ModelTrainingPipeline::validateTrainingConfig(const QString& configId) {
    ...
    return !cfg.configId.isEmpty() && !cfg.dataset.datasetId.isEmpty();
}
...
bool ModelTrainingPipeline::cancelTraining(const QString& trainingId) {
    ...
    m_activeTrainings.remove(trainingId);
...
TrainingModelVersion ModelTrainingPipeline::getModelVersion(const QString& versionId) const {
    QReadLocker locker(&m_dataLock);
    auto it = m_modelVersions.find(versionId);
    if (it != m_modelVersions.end()) {
        return it->second;
    }
    return TrainingModelVersion();
}
...
QString ModelTrainingPipeline::compareModelVersions(const QString& version1Id, const QString& version2Id) const {
    TrainingModelVersion v1 = getModelVersion(version1Id);
    TrainingModelVersion v2 = getModelVersion(version2Id);
...
void ModelTrainingPipeline::executeTraining(const QString& trainingId) {
    ...
    if (it != m_trainingProgress.end()) {
        ...
        it->second.lastUpdateTime = QDateTime::currentDateTime();
    }
}
...
QJsonObject ModelTrainingPipeline::generateModelReport(const QString& versionId) const {
    TrainingModelVersion mv = getModelVersion(versionId);
    return mv.toJson();
}
