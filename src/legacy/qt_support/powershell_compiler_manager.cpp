...
#include "../../include/legacy_qt/powershell_compiler_manager.h"
...
PowerShellCompilerManager::PowerShellCompilerManager(QObject* parent)
    : QObject(parent), m_currentProcess(nullptr), m_timeoutTimer(nullptr), m_totalCompilations(0),
      m_successfulCompilations(0), m_failedCompilations(0), m_defaultTimeoutMs(30000)  // 30 seconds default timeout
{
    initialize();
}
                                                         ...
                                                         const QString& outputPath, const QJsonObject& options)
{
    ...
    connect(m_currentProcess, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this,
            &PowerShellCompilerManager::onProcessFinished);
    connect(m_currentProcess, &QProcess::errorOccurred, this, &PowerShellCompilerManager::onProcessError);

    // Set timeout
    int timeout = options.contains("timeout") ? options.value("timeout").toInt() : compiler.timeoutMs;
    if (timeout <= 0)
...
void PowerShellCompilerManager::onProcessFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (exitStatus == QProcess::NormalExit && exitCode == 0)
    {
        ...
        {
            // Simple heuristic to extract file path (Qt6 uses QRegularExpression)
            QRegularExpression filePattern(R"([A-Za-z]:\\[^\n]+\.(exe|dll|so|a|obj|o))");
            QRegularExpressionMatch match = filePattern.match(output);
            if (match.hasMatch())
            {
                result.outputFile = match.captured(0);
            }
        }
...
bool PowerShellCompilerManager::validateCompilerScript(const QString& scriptPath)
{
    ...
    return content.contains("param(") || content.contains("function") || content.contains("Write-Output") ||
           content.contains("Write-Error");
}
                                                          ...
                                                          const QString& outputPath, const QJsonObject& options)
{
    ...
    QString escapedScriptPath = compiler.scriptPath;
    escapedScriptPath.replace("'", "''");
    command += QString("& '%1'").arg(escapedScriptPath);

    // Add source file parameter
    QString escapedSource = source;
    escapedSource.replace("'", "''");
    command += QString(" -SourceFile '%1'").arg(escapedSource);

    // Add output path if specified
    if (!outputPath.isEmpty())
    {
        QString escapedOutput = outputPath;
        escapedOutput.replace("'", "''");
        command += QString(" -OutputPath '%1'").arg(escapedOutput);
    }
