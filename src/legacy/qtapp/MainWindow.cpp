...
#include "../../ai/streaming_gguf_loader_mmap.h"
...
inline bool safeWidgetCall(WidgetType* widget, Func&& func, const QString& operation)
{
    if (!widget)
    {
        RawrXD::Integration::logWarn("MainWindow", "widget_call", QString("%1 failed: widget is null").arg(operation));
        return false;
    }
    try
    {
...
template <typename Func> inline auto retryWithBackoff(Func&& func, int maxRetries = 3, int initialDelayMs = 100)
{
    int attempt = 0;
    ...
    while (attempt < maxRetries)
    {
        ...
        catch (const std::exception& e)
        {
            ...
            {
                RawrXD::Integration::logError(
                    "MainWindow", "retry_exhausted",
...
template <typename Func> inline QFuture<void> runAsync(Func&& func, const QString& operation)
{
    return QtConcurrent::run(
        [func = std::forward<Func>(func), operation]()
        {
...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent)
{
    ...
    connect(m_ggufServer, &GGUFServer::serverStarted, this,
            [this](quint16 port)
            {
                statusBar()->showMessage(tr("GGUF Server running on port %1").arg(port), 5000);
                qDebug() << "GGUF Server started on port" << port;
            });
    connect(m_ggufServer, &GGUFServer::error, this,
            [](const QString& err) { qWarning() << "GGUF Server error:" << err; });
    ...
    connect(commandPaletteShortcut, &QShortcut::activated, this,
            [this]()
            {
                if (m_commandPalette)
    ...
    if (!ggufEnv.isEmpty())
    {
        ...
        QMetaObject::invokeMethod(m_inferenceEngine, "loadModel", Qt::QueuedConnection, Q_ARG(QString, ggufEnv));
    }
...
void MainWindow::createVSCodeLayout()
{
    ...
    sidebarHeader->setStyleSheet(
        "QLabel { color: #e0e0e0; background-color: #2d2d30; padding: 8px; font-weight: bold; }");
    ...
    searchInput->setStyleSheet(
        "QLineEdit { background-color: #3c3c3c; color: #e0e0e0; border: 1px solid #555; padding: 5px; }");
    searchLayout->addWidget(searchInput);
    QLineEdit* replaceInput = new QLineEdit(m_primarySidebar);
    replaceInput->setPlaceholderText("Replace...");
    replaceInput->setStyleSheet(
        "QLineEdit { background-color: #3c3c3c; color: #e0e0e0; border: 1px solid #555; padding: 5px; }");
    ...
    startDebugBtn->setStyleSheet("QPushButton { background-color: #0e639c; color: white; padding: 6px; border: none; } "
    ...
    editorTabs_->setStyleSheet("QTabBar { background-color: #252526; }"
                               "QTabBar::tab { background-color: #1e1e1e; color: #e0e0e0; padding: 8px; margin: 0px; "
                               "border: 1px solid #3e3e42; }"
                               "QTabBar::tab:selected { background-color: #252526; border-bottom: 2px solid #007acc; }"
                               "QTabWidget::pane { border: none; }");

    codeView_ = new QTextEdit(editorFrame);
    codeView_->setStyleSheet("QTextEdit { background-color: #1e1e1e; color: #e0e0e0; font-family: 'Consolas', "
    ...
    {
        btn->setStyleSheet("QPushButton { background-color: transparent; color: #e0e0e0; border: none; padding: 8px; }"
    ...
    terminalView->setStyleSheet("QPlainTextEdit { background-color: #1e1e1e; color: #0dff00; font-family: 'Consolas', "
    ...
    outputView->setStyleSheet("QPlainTextEdit { background-color: #1e1e1e; color: #e0e0e0; font-family: 'Consolas', "
                              "monospace; font-size: 10pt; }");
    ...
    debugConsole->setStyleSheet("QPlainTextEdit { background-color: #1e1e1e; color: #e0e0e0; font-family: 'Consolas', "
                                "monospace; font-size: 10pt; }");
    ...
    m_hexMagConsole->setStyleSheet("QPlainTextEdit { background-color: #1e1e1e; color: #0dff00; "
    ...
    if (m_activityBar)
    {
        connect(m_activityBar, &ActivityBar::viewChanged, this,
                [this](ActivityBar::ViewType view)
                {
    ...
    connect(panelCloseBtn, &QPushButton::clicked, this, [this]() { m_bottomPanel->hide(); });

    connect(panelMinBtn, &QPushButton::clicked, this,
            [this]() { m_bottomPanel->setFixedHeight(m_bottomPanel->height() > 50 ? 35 : 200); });
...
void MainWindow::setAppState(std::shared_ptr<void> state)
{
    ...
    if (m_hexMagConsole)
    {
        m_hexMagConsole->appendPlainText(QString("[STATE] Application state updated at %1")
...
void MainWindow::setupMenuBar()
{
    ...
    newMenu->addAction(tr("New &Window"), this, &MainWindow::handleNewWindow,
    ...
    fileMenu->addAction(tr("Open &Folder..."), this, &MainWindow::handleAddFolder,
    ...
    recentMenu->addAction(tr("Clear Recent Files"), this,
                          [this]()
                          {
                              QSettings settings("RawrXD", "IDE");
    ...
    fileMenu->addAction(tr("Save A&ll"), this, &MainWindow::handleSaveAll,
                        QKeySequence(Qt::CTRL | Qt::Key_K, Qt::Key_S));
    ...
    fileMenu->addAction(tr("Close &All Editors"), this, &MainWindow::handleCloseAllEditors,
                        QKeySequence(Qt::CTRL | Qt::Key_K, Qt::CTRL | Qt::Key_W));
    ...
    fileMenu->addSeparator();
    fileMenu->addAction(
        tr("Pre&ferences..."), this, [this]() { toggleSettings(true); }, QKeySequence(Qt::CTRL | Qt::Key_Comma));
    ...
    editMenu->addAction(tr("Find in Fi&les..."), this, &MainWindow::handleFindInFiles,
    ...
    editMenu->addAction(tr("Go to S&ymbol..."), this, &MainWindow::handleGoToSymbol,
                        QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_O));
    editMenu->addAction(tr("Go to &Definition"), this, &MainWindow::handleGoToDefinition, QKeySequence(Qt::Key_F12));
    editMenu->addAction(tr("Go to &References"), this, &MainWindow::handleGoToReferences,
                        QKeySequence(Qt::SHIFT | Qt::Key_F12));

    editMenu->addSeparator();

    // Code editing
    editMenu->addAction(tr("Toggle &Comment"), this, &MainWindow::handleToggleComment,
                        QKeySequence(Qt::CTRL | Qt::Key_Slash));
    editMenu->addAction(tr("Format &Document"), this, &MainWindow::handleFormatDocument,
    ...
    viewMenu
        ->addAction(tr("Command Palette..."), this, &MainWindow::toggleCommandPalette,
    ...
    QAction* projExplAct = explorerMenu->addAction(tr("Project Explorer"), this, &MainWindow::toggleProjectExplorer,
    ...
    scmMenu
        ->addAction(tr("Version Control"), this, &MainWindow::toggleVersionControl,
    ...
    buildDebugMenu
        ->addAction(tr("Run && Debug"), this, &MainWindow::toggleRunDebug,
    ...
    compilerMenu->addAction(
        tr("Compile Current File"), this, [this]() { toggleCompileCurrentFile(); },
        QKeySequence(Qt::CTRL | Qt::Key_F7));
    compilerMenu->addAction(tr("Build Project"), this, [this]() { toggleBuildProject(); }, QKeySequence(Qt::Key_F7));
    compilerMenu->addAction(tr("Clean Build"), this, [this]() { toggleCleanBuild(); });
    compilerMenu->addSeparator();
    compilerMenu->addAction(tr("Compiler Settings..."), this, [this]() { toggleCompilerSettings(); });
    compilerMenu->addAction(tr("Compiler Output"), this, [this]() { toggleCompilerOutput(); })->setCheckable(true);

    // ----- AI & Agent Section -----
    QMenu* aiViewMenu = viewMenu->addMenu(tr("&AI && Agent"));
    QAction* aiChatAct = aiViewMenu->addAction(
        tr("AI Chat Panel"), this,
        [this](bool checked)
        {
    ...
    QAction* orchestrationAct = aiViewMenu->addAction(tr("Task Orchestration"), this,
                                                      [this](bool checked)
                                                      {
    ...
    connect(monAct, &QAction::toggled, this,
            [this](bool on)
            {
    ...
    QAction* layerQuantAct = modelViewMenu->addAction(tr("Layer Quantization"), this,
                                                      [this](bool checked)
                                                      {
                                                          if (m_layerQuantDock)
    ...
    QAction* interpretabilityAct = modelViewMenu->addAction(tr("Model Interpretability"), this,
                                                            [this](bool checked)
                                                            {
    ...
    if (m_interpretabilityPanelDock)
    {
        ...
        connect(m_interpretabilityPanelDock, &QDockWidget::visibilityChanged, interpretabilityAct,
                &QAction::setChecked);
    }
    ...
    terminalMenu
        ->addAction(tr("Terminal Cluster"), this, &MainWindow::toggleTerminalCluster,
                    QKeySequence(Qt::CTRL | Qt::Key_Backtick))
        ->setCheckable(true);
    ...
    QAction* masmAct = editorFeaturesMenu->addAction(tr("MASM Editor"), this,
                                                     [this](bool checked)
                                                     {
                                                         if (m_masmEditorDock)
    ...
    editorFeaturesMenu->addAction(tr("Language Server"), this, &MainWindow::toggleLanguageClientHost)
        ->setCheckable(true);
    editorFeaturesMenu->addSeparator();
    QAction* hotpatchAct = editorFeaturesMenu->addAction(tr("Hotpatch Panel"), this,
                                                         [this](bool checked)
                                                         {
    ...
    appearanceMenu->addAction(tr("Toggle Full Screen"), this, &MainWindow::handleFullScreen, QKeySequence::FullScreen)
        ->setCheckable(true);
    appearanceMenu->addAction(tr("Toggle Zen Mode"), this, &MainWindow::handleZenMode)->setCheckable(true);
    appearanceMenu->addSeparator();
    appearanceMenu
        ->addAction(tr("Toggle Side Bar"), this, &MainWindow::handleToggleSidebar, QKeySequence(Qt::CTRL | Qt::Key_B))
        ->setCheckable(true);
    ...
    toolsMenu->addAction(tr("Command Palette..."), this, &MainWindow::toggleCommandPalette,
                         QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_P));
    ...
    runMenu->addAction(tr("Run &Without Debugging"), this, &MainWindow::handleRunNoDebug,
    ...
    runMenu->addAction(tr("&Restart Debugging"), this, &MainWindow::handleRestartDebug,
    ...
    termMenu->addAction(tr("&New Terminal"), this, &MainWindow::handleNewTerminal,
                        QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_Backtick));
    ...
    connect(streamAct, &QAction::toggled, this,
            [this](bool on)
            {
                m_streamingMode = on;
                statusBar()->showMessage(on ? tr("Streaming inference ON") : tr("Streaming inference OFF"), 2000);
            });
    ...
    connect(agentModeGroup, &QActionGroup::triggered, this,
            [this](QAction* action) { changeAgentMode(action->data().toString()); });
    ...
    struct BackendOption
    {
        const char* id;
        const char* label;
    } backendOptions[] = {{"local", "Local GGUF"}, {"ollama", "Remote Ollama"}, {"custom", "Custom Backend"}};
    ...
    helpMenu->addAction(tr("Show All &Commands"), this, &MainWindow::toggleCommandPalette,
                        QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_P));
    helpMenu->addSeparator();
    helpMenu->addAction(tr("&Keyboard Shortcuts"), this, &MainWindow::handleShowShortcuts,
    ...
    helpMenu->addAction(tr("Toggle &Developer Tools"), this, &MainWindow::handleDevTools,
                        QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_I));
    helpMenu->addSeparator();
...
void MainWindow::setupToolBars()
{
    ...
    connect(m_modelSelector, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this](int idx) {
        if (idx <= 0)
            return;  // Skip "No model loaded" and separators
        ...
        if (modelData.startsWith("ollama:"))
        {
            ...
            QString gguf;
            QStringList searchDirs = {QString("D:/OllamaModels"), QDir::homePath() + "/models",
            ...
            for (const QString& dirPath : searchDirs)
            {
                ...
                if (!d.exists())
                    continue;
                QStringList matches =
                    d.entryList(QStringList() << QString("*%1*.gguf").arg(modelName), QDir::Files, QDir::Name);
                if (!matches.isEmpty())
                {
                    gguf = d.filePath(matches.first());
                    break;
                }
            }
            if (!gguf.isEmpty())
...
connect(m_agentModeSwitcher, &QComboBox::currentTextChanged, this,
        [this](const QString&)
        {
...
void MainWindow::refreshModelSelector()
{
    ...
    QStringList searchDirs = {QDir::currentPath() + "/models", QDir::homePath() + "/models",
    ...
    for (const auto& key : settings.allKeys())
    {
        ...
        if (!modelId.isEmpty())
        {
            ...
            {
                QString tooltip =
                    QString("<b>%1</b><br/>Provider: Claude<br/>Model: %2").arg(modelId, QString("Claude"));
    ...
    for (const auto& key : settings.allKeys())
    {
        ...
        if (!modelId.isEmpty())
        {
            ...
            {
                QString tooltip =
                    QString("<b>%1</b><br/>Provider: OpenAI<br/>Model: %2").arg(modelId, QString("OpenAI"));
...
{
    if (m_modelTooltipCache.contains(filePath))
        return m_modelTooltipCache[filePath];
    ...
    try
    {
        StreamingGGUFLoaderMmap loader;
...
void MainWindow::loadGGUFModel(const QString& ggufPath)
{
    ...
    if (!m_loadingProgressDialog)
    {
        ...
        connect(m_loadingProgressDialog, &QProgressDialog::canceled, this,
                [this]()
                {
    ...
    m_modelLoaderThread->setProgressCallback(
    ...
    m_modelLoaderThread->setCompleteCallback(
        [this](bool success, const std::string& err)
        {
            QMetaObject::invokeMethod(
                this, [this, success, err]() { onModelLoadFinished(success, err); }, Qt::QueuedConnection);
        });
    ...
    if (!m_loadProgressTimer)
    {
        ...
        connect(m_loadProgressTimer, &QTimer::timeout, this,
                [this]()
                {
...
void MainWindow::handleBackendSelection(QAction* action)
{
    ...
    if (backendId.isEmpty() || backendId == m_currentBackend)
        return;
...
void MainWindow::setupStatusBar()
{
    ...
    if (codeView_)
    {
        connect(codeView_, &QTextEdit::cursorPositionChanged, this,
                [this, lineColLabel]()
                {
    ...
    if (m_modelSelector)
    {
        connect(m_modelSelector, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                [this, modelLabel]()
                {
...
void MainWindow::initSubsystems()
{
    ...
    initSubsystem("InterpretabilityPanel",
                  [this]()
                  {
...
void MainWindow::handleGoalSubmit()
{
    ...
    if (!m_actionExecutor)
    {
        ...
        connect(m_actionExecutor, &ActionExecutor::actionStarted, this, &MainWindow::onActionStarted);
        connect(m_actionExecutor, &ActionExecutor::actionCompleted, this, &MainWindow::onActionCompleted);
        connect(m_actionExecutor, &ActionExecutor::planCompleted, this, &MainWindow::onPlanCompleted);
    }
...
void MainWindow::handleArchitectChunk(const QString& chunk)
{
    ...
    if (chatHistory_)
    {
        ...
        if (!chatHistory_->currentItem() || !chatHistory_->currentItem()->text().startsWith("Architect:"))
        {
...
{
    QString msg = success ? tr("✓✓✓ Workflow completed successfully!") : tr("✗ Workflow failed - check logs");
    ...
    if (mockStatusBadge_)
    {
        ...
        mockStatusBadge_->setStyleSheet(success ? "QLabel { color: #00ff00; }" : "QLabel { color: #ff0000; }");
    }
...
void MainWindow::saveEditorState()
{
    ...
    try
    {
        ...
        RawrXD::Integration::logInfo(
            "MainWindow", "editor_state_saved",
...
void MainWindow::saveTabState()
{
    ...
    try
    {
        ...
        RawrXD::Integration::logInfo("MainWindow", "tab_state_saved", QString("Saved %1 tabs").arg(tabCount));
    }
    catch (const std::exception& e)
    {
...
void MainWindow::restoreTabState()
{
    ...
    try
    {
        ...
        RawrXD::Integration::logInfo("MainWindow", "tab_state_restored", QString("Restored tab state"));
    }
    catch (const std::exception& e)
    {
...
void MainWindow::trackEditorScrollPosition()
{
    ...
    void MainWindow::clearRecentFiles()
    {
        ...
        RawrXD::Integration::logInfo("MainWindow", "recent_files_cleared", QString("Cleared all recent files"));
    }
    ...
    void MainWindow::addCommandToHistory(const QString& command)
    {
        ...
        try
        {
            ...
            QString timestampedCmd =
                QString("[%1] %2").arg(QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss")).arg(command);
    ...
    {
        QString filePath =
            QFileDialog::getOpenFileName(this, tr("Add File to Project"), QString(), tr("All Files (*.*)"));
    ...
    void MainWindow::handleAddFolder()
    {
        QString folderPath = QFileDialog::getExistingDirectory(
            this, tr("Add Folder to Project"), QString(), QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
    ...
    void MainWindow::handleAddSymbol()
    {
        bool ok;
        QString symbol =
            QInputDialog::getText(this, tr("Add Symbol"), tr("Symbol name:"), QLineEdit::Normal, QString(), &ok);
    ...
    {
        if (!editorTabs_ || index < 0 || index >= editorTabs_->count())
            return;
        ...
        if (editor && !editor->toPlainText().isEmpty())
        {
            QMessageBox::StandardButton reply = QMessageBox::question(
                this, tr("Close Tab"), tr("Close '%1'? Unsaved changes will be lost.").arg(editorTabs_->tabText(index)),
    ...
    void MainWindow::clearDebugLog()
    {
        ...
        {
            QMessageBox::StandardButton reply = QMessageBox::question(
                this, tr("Clear Debug Log"), tr("Are you sure you want to clear %1 lines of log data?").arg(lineCount),
                QMessageBox::Yes | QMessageBox::No);
        ...
        m_hexMagConsole->appendPlainText(
            QString("=== Log cleared at %1 ===").arg(QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss")));
    ...
    void MainWindow::saveDebugLog()
    {
        ...
        {
            QMessageBox::information(this, tr("Save Debug Log"), tr("Debug log is empty. Nothing to save."));
            return;
        }

        // Generate default filename with timestamp
        QString defaultName =
            QString("rawrxd_debug_%1.log").arg(QDateTime::currentDateTime().toString("yyyyMMdd_HHmmss"));

        QString filePath =
            QFileDialog::getSaveFileName(this, tr("Save Debug Log"), QDir::homePath() + "/" + defaultName,
    ...
    void MainWindow::filterLogLevel(const QString& level)
    {
        ...
        qInfo() << "[DEBUG_LOG] Applied filter. Showing" << filteredLines.size() << "lines, hidden" << filteredCount
                << "lines";
    }
    void MainWindow::showEditorContextMenu(const QPoint& pos)
    {
        ...
        QAction* deleteAction =
            contextMenu.addAction(QIcon(), tr("Delete"), [this, cursor]() mutable { cursor.removeSelectedText(); });
        ...
        if (hasSelection)
        {
            contextMenu.addAction(tr("🔍 Find '%1'").arg(selectedText.left(20)),
                                  [this, selectedText]()
                                  {
    ...
    void MainWindow::fixCode()
    {
        ...
        {
            QString prompt =
                tr("Find and fix any bugs or issues in this code:\n\n```\n%1\n```\n\nProvide the corrected code.")
                    .arg(sel);
            m_aiChatPanel->addUserMessage(prompt);
            onAIChatMessageSubmitted(prompt);
    ...
    void MainWindow::refactorCode()
    {
        ...
        {
            QString prompt =
                tr("Refactor this code to be more efficient, readable, and follow best practices:\n\n```\n%1\n```")
                    .arg(sel);
            m_aiChatPanel->addUserMessage(prompt);
            onAIChatMessageSubmitted(prompt);
    ...
    void MainWindow::generateTests()
    {
        ...
        if (m_aiChatPanel)
        {
            QString prompt = tr("Generate comprehensive unit tests for this code:\n\n```\n%1\n```\n\nInclude edge "
                                "cases and error handling tests.")
                                 .arg(sel);
            m_aiChatPanel->addUserMessage(prompt);
            onAIChatMessageSubmitted(prompt);
    ...
    void MainWindow::generateDocs()
    {
        ...
        if (m_aiChatPanel)
        {
            QString prompt = tr("Generate comprehensive documentation for this code:\n\n```\n%1\n```\n\nInclude "
                                "function descriptions, parameter docs, and usage examples.")
                                 .arg(sel);
            m_aiChatPanel->addUserMessage(prompt);
            onAIChatMessageSubmitted(prompt);
    ...
    void MainWindow::onProjectOpened(const QString& path)
    {
        ...
        {
            RawrXD::Integration::logWarn("MainWindow", "project_open",
                                         "Project Explorer feature is disabled in safe mode");
        ...
        if (!info.exists())
        {
            ...
            QMessageBox::warning(this, tr("Invalid Path"), tr("The specified path does not exist:\n%1").arg(path));
            return;
        }
    ...
    void MainWindow::onBuildStarted()
    {
        ...
        {
            RawrXD::Integration::logWarn("MainWindow", "build_started",
                                         "Build System feature is disabled in safe mode");
            return;
        }
    ...
    void MainWindow::onBuildFinished(bool success)
    {
        ...
        else
        {
            ...
            QMessageBox::warning(this, tr("Build Failed"), tr("Build process failed. Check console for details."));
        }
    ...
    void MainWindow::onVcsStatusChanged()
    {
        ...
        {
            RawrXD::Integration::logWarn("MainWindow", "vcs_status",
                                         "Version Control feature is disabled in safe mode");
            return;
        }
    ...
    void MainWindow::onTestRunStarted()
    {
        ...
        {
            notificationCenter_->notify(tr("Test Run Started"), tr("Test execution has begun"),
    ...
    void MainWindow::onTestRunFinished()
    {
        ...
        if (notificationCenter_)
        {
            notificationCenter_->notify(tr("Test Run Completed"),
    ...
    void MainWindow::onImageEdited(const QString& path)
    {
        ...
        if (!info.exists())
        {
            RawrXD::Integration::logError("MainWindow", "image_edit",
                                          QString("Image file does not exist: %1").arg(path));
            return;
        }
    ...
    void MainWindow::onDesignImported(const QString& file)
    {
        ...
        if (!info.exists())
        {
            RawrXD::Integration::logError("MainWindow", "design_import",
                                          QString("Design file does not exist: %1").arg(file));
            return;
        }
    ...
    void MainWindow::onDiffMerged()
    {
        ...
        RawrXD::Integration::logInfo("MainWindow", "diff_merged", QString("Diff merged (total: %1)").arg(mergeCount),
    ...
    void MainWindow::onColorPicked(const QColor& c)
    {
        ...
        {
            m_hexMagConsole->appendPlainText(
                QString("[COLOR] %1 (R:%2 G:%3 B:%4)").arg(c.name()).arg(c.red()).arg(c.green()).arg(c.blue()));
        }
    ...
    void MainWindow::scanProjectForTodos()
    {
        ...
        QStringList extensions = {"*.cpp", "*.h",  "*.hpp", "*.c",    "*.py", "*.js",
                                  "*.ts",  "*.rs", "*.go",  "*.java", "*.asm"};
                    ...
                    for (const QString& pattern : todoPatterns)
                    {
                        ...
                        if (idx != -1)
                        {
                            ...
                            if (comment.length() > 100)
                                comment = comment.left(100) + "...";

                            // Create list item with file:line info
                            QString itemText =
                                QString("%1:%2 - %3").arg(QFileInfo(filePath).fileName()).arg(lineNum).arg(comment);
    ...
    void MainWindow::onLSPDiagnostic(const QString& file, const QJsonArray& diags)
    {
        ...
        statusBar()->showMessage(tr("Diagnostics: %1 (%2 issues)").arg(QFileInfo(file).fileName()).arg(diagCount),
                                 2000);
    ...
    void MainWindow::onTimeEntryAdded(const QString& task)
    {
        ...
        {
            m_hexMagConsole->appendPlainText(
                QString("[TIME] %1 @ %2").arg(task).arg(QDateTime::currentDateTime().toString("hh:mm:ss")));
        }
    ...
    void MainWindow::onPomodoroTick(int remaining)
    {
        ...
        statusBar()->showMessage(
            tr("Pomodoro: %1m %2s").arg(remaining / 60).arg(remaining % 60, 2, 10, QLatin1Char('0')), 1000);
...
#define IMPLEMENT_TOGGLE(Func, Member, Type)                                                                           \
    ...
    void MainWindow::toggleProjectExplorer(bool visible)
    {
        if (visible)
        {
            if (!projectExplorer_)
            {
                ...
                connect(projectExplorer_, &RawrXD::ProjectExplorerWidget::fileDoubleClicked, this,
    ...
    void MainWindow::dropEvent(QDropEvent * event)
    {
        ...
        for (const QUrl& u : mime->urls())
        {
            ...
            if (og.open(QIODevice::WriteOnly))
            {
                og.write(gz);
                og.close();
                statusBar()->showMessage(tr("Compressed %1 → %2  (ratio %3%)")
    ...
    QWidget* MainWindow::createGoalBar()
    {
        ...
        try
        {
            ...
            goalBar->setStyleSheet(
                "QWidget#GoalBarWidget { background-color: #252526; border-bottom: 1px solid #3e3e42; }");
            ...
            goalInput_->setStyleSheet("QLineEdit#GoalInput { "
            ...
            submitBtn->setStyleSheet("QPushButton#SubmitButton { "
    ...
    QWidget* MainWindow::createAgentPanel()
    {
        ...
        try
        {
            ...
            agentSelector_->setStyleSheet(
                ...
                "QComboBox QAbstractItemView { background-color: #252526; color: #e0e0e0; selection-background-color: "
                "#007acc; }");
            layout->addWidget(agentSelector_);

            connect(agentSelector_, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
                    [this](int index)
                    {
            ...
            mockStatusBadge_->setStyleSheet("QLabel#StatusBadge { "
                                            "background-color: #3c3c3c; "
                                            "color: #4ec9b0; "
    ...
    QWidget* MainWindow::createProposalReview()
    {
        ...
        try
        {
            ...
            chatHistory_->setStyleSheet("QListWidget#ProposalList { "
            ...
            acceptBtn->setStyleSheet("QPushButton { background-color: #0e7a0d; color: white; border: none; padding: "
                                     "6px 12px; border-radius: 3px; } "
                                     "QPushButton:hover { background-color: #0c5c0b; }");
            btnLayout->addWidget(acceptBtn);

            QPushButton* rejectBtn = new QPushButton("Reject", panel);
            rejectBtn->setStyleSheet("QPushButton { background-color: #a1260d; color: white; border: none; padding: "
                                     "6px 12px; border-radius: 3px; } "
    ...
    QWidget* MainWindow::createEditorArea()
    {
        ...
        try
        {
            ...
            editorTabs_->setStyleSheet("QTabWidget::pane { border: none; background-color: #1e1e1e; } "
            ...
            codeView_->setStyleSheet("QTextEdit#CodeEditor { "
                                     "background-color: #1e1e1e; "
                                     "color: #d4d4d4; "
    ...
    QWidget* MainWindow::createQShellTab()
    {
        ...
        try
        {
            ...
            qshellOutput_->setStyleSheet("QTextEdit#QShellOutput { "
                                         "background-color: #1e1e1e; "
                                         "color: #0dff00; "
                                         "font-family: 'Consolas', 'Courier New', monospace; "
                                         "font-size: 10pt; "
            ...
            qshellInput_->setStyleSheet("QLineEdit#QShellInput { "
                                        "background-color: #1e1e1e; "
                                        "color: #0dff00; "
                                        "font-family: 'Consolas', 'Courier New', monospace; "
                                        "font-size: 10pt; "
            ...
            executeBtn->setStyleSheet("QPushButton { background-color: #007acc; color: white; border: none; padding: "
                                      "5px 15px; border-radius: 3px; } "
    ...
    QWidget* MainWindow::createTerminalPanel()
    {
        ...
        try
        {
            ...
            pwshOutput_->setStyleSheet("QPlainTextEdit { "
            ...
            pwshPrompt->setStyleSheet(
                "QLabel { color: #00ff00; font-family: 'Consolas', monospace; font-weight: bold; }");
            pwshInputLayout->addWidget(pwshPrompt);

            pwshInput_ = new QLineEdit(pwshTab);
            pwshInput_->setStyleSheet("QLineEdit { background-color: #012456; color: #eeedf0; font-family: 'Consolas', "
                                      "monospace; border: none; }");
            ...
            cmdOutput_->setStyleSheet("QPlainTextEdit { "
            ...
            cmdPrompt->setStyleSheet(
                "QLabel { color: #ffffff; font-family: 'Consolas', monospace; font-weight: bold; }");
            cmdInputLayout->addWidget(cmdPrompt);

            cmdInput_ = new QLineEdit(cmdTab);
            cmdInput_->setStyleSheet("QLineEdit { background-color: #0c0c0c; color: #cccccc; font-family: 'Consolas', "
                                     "monospace; border: none; }");
    ...
    QWidget* MainWindow::createDebugPanel()
    {
        ...
        try
        {
            ...
            debugOutput->setStyleSheet("QPlainTextEdit#DebugOutput { "
                                       "background-color: #1e1e1e; "
                                       "color: #cccccc; "
                                       "font-family: 'Consolas', monospace; "
                                       "font-size: 9pt; "
            ...
            qDebug() << "[createDebugPanel] Debug panel created successfully with" << debugOutput->blockCount()
                     << "initial log entries";
            return debugWidget;
        }
        catch (const std::exception& e)
    ...
    void MainWindow::setupDockWidgets()
    {
        ...
        try
        {
            ...
            if (!buildWidget_)
            {
                ...
                buildDock->setFeatures(QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable |
                                       QDockWidget::DockWidgetClosable);
            ...
            if (!vcsWidget_)
            {
                ...
                vcsDock->setFeatures(QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable |
                                     QDockWidget::DockWidgetClosable);
            ...
            if (!debugWidget_)
            {
                ...
                debugDock->setFeatures(QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable |
                                       QDockWidget::DockWidgetClosable);
            ...
            if (!testWidget_)
            {
                ...
                testDock->setFeatures(QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable |
                                      QDockWidget::DockWidgetClosable);
            ...
            if (!profilerWidget_)
            {
                ...
                profilerDock->setFeatures(QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable |
                                          QDockWidget::DockWidgetClosable);
            ...
            if (!database_)
            {
                ...
                dbDock->setFeatures(QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable |
                                    QDockWidget::DockWidgetClosable);
            ...
            if (!docker_)
            {
                ...
                dockerDock->setFeatures(QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable |
                                        QDockWidget::DockWidgetClosable);
    ...
    void MainWindow::setupSystemTray()
    {
        ...
        try
        {
            ...
            // Show notification
            trayIcon_->showMessage("RawrXD IDE", "Application is running in the system tray",
    ...
    void MainWindow::onRunScript() \n
    {
        ...
        void MainWindow::runInference()
        {
            ...
            {
                QMessageBox::warning(this, tr("No Model"), tr("Please load a GGUF model first."));
                return;
            }

            bool ok;
            QString prompt =
                QInputDialog::getMultiLineText(this, tr("Run Inference"), tr("Enter your prompt:"), QString(), &ok);
            ...
            QMetaObject::invokeMethod(m_inferenceEngine, "request", Qt::QueuedConnection, Q_ARG(QString, prompt),
                                      Q_ARG(qint64, reqId));
        }

        void MainWindow::loadGGUFModel()
        {
            QString filePath = QFileDialog::getOpenFileName(this, tr("Select GGUF Model"), QString(),
            ...
            QMetaObject::invokeMethod(m_inferenceEngine, "loadModel", Qt::QueuedConnection, Q_ARG(QString, filePath));
        }
        ...
        void MainWindow::onModelLoadedChanged(bool loaded, const QString& modelName)
        {
            ...
            if (loaded)
            {
                ...
                if (!devCmd.isEmpty() && !names.isEmpty())
                {
                    ...
                    if (m_hexMagConsole)
                        m_hexMagConsole->appendPlainText(QString("Auto-set %1 -> %2").arg(target, quant));
        ...
        {
            QString dir =
                QFileDialog::getExistingDirectory(this, tr("Select GGUF Folder"), QString(),
        ...
        void MainWindow::setupAgentSystem()
        {
            ...
            connect(m_agentModeGroup, &QActionGroup::triggered, this,
                    [this](QAction* action)
                    {
        ...
        void MainWindow::setupHotpatchPanel()
        {
            ...
            m_hotpatchPanelDock->setFeatures(QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable |
                                             QDockWidget::DockWidgetClosable);
            ...
            connect(m_hotReload, &HotReload::reloadFailed, this,
                    [this](const QString& error) { m_hotpatchPanel->logEvent("Reload Failed", error, false); });

            // Connect manual reload button in hotpatch panel to onHotReload
            connect(m_hotpatchPanel, &HotpatchPanel::manualReloadRequested, this,
                    [this](const QString& quantType)
                    {
            ...
            connect(toggleHotpatchAction, &QAction::triggered, this,
                    [this](bool visible) { toggleHotpatchPanel(visible); });
        }
        ...
        void MainWindow::setupMASMEditor()
        {
            ...
            m_masmEditorDock->setFeatures(QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable |
                                          QDockWidget::DockWidgetClosable);
            ...
            connect(m_masmEditor, &MASMEditorWidget::contentModified, this,
                    [this](int index)
                    {
                        QString modified = m_masmEditor->isModified(index) ? " *" : "";
                        statusBar()->showMessage(
                            tr("Modified: %1%2").arg(m_masmEditor->getTabName(index)).arg(modified), 1000);
                    });
            ...
            connect(toggleMASMAction, &QAction::triggered, this, [this](bool visible) { toggleMASMEditor(visible); });
        }
        ...
        void MainWindow::setupAIChatPanel()
        {
            ...
            m_aiChatPanelDock->setFeatures(QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable |
                                           QDockWidget::DockWidgetClosable);
            ...
            connect(m_aiChatPanel, &AIChatPanel::messageSubmitted, this, &MainWindow::onAIChatMessageSubmitted);
            connect(m_aiChatPanel, &AIChatPanel::quickActionTriggered, this, &MainWindow::onAIChatQuickActionTriggered);

            // Keep the main model selector in sync with agent breadcrumb updates (Ollama async fetch)
            if (m_aiChatPanel && m_aiChatPanel->getBreadcrumb())
            {
                connect(m_aiChatPanel->getBreadcrumb(), &AgentChatBreadcrumb::ollamaModelsUpdated, this,
                        &MainWindow::refreshModelSelector);
            }

            // Connect inference engine responses to chat panel
            connect(m_inferenceEngine, &InferenceEngine::streamToken, this,
                    [this](qint64, const QString& token)
                    {
                        if (m_aiChatPanel)
                            m_aiChatPanel->updateStreamingMessage(token);
                    });
            connect(m_inferenceEngine, &InferenceEngine::streamFinished, this,
                    [this](qint64)
                    {
            ...
            connect(toggleChatAction, &QAction::triggered, this,
                    [this](bool visible)
                    {
                        if (m_aiChatPanelDock)
                        {
            ...
            try
            {
                ...
                if (m_inferenceEngine && m_inferenceEngine->isModelLoaded())
                {
                    ...
                    QMetaObject::invokeMethod(m_inferenceEngine, "request", Qt::QueuedConnection,
                                              Q_ARG(QString, message), Q_ARG(qint64, reqId), Q_ARG(bool, true));
                }
                else
                {
        ...
        void MainWindow::setupLayerQuantWidget()
        {
            ...
            m_layerQuantDock->setFeatures(QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable |
                                          QDockWidget::DockWidgetClosable);
            ...
            connect(m_layerQuantWidget, &LayerQuantWidget::quantModeChanged, this, &MainWindow::onQuantModeChanged);
        ...
        void MainWindow::setupQuantizationMenu(QMenu * aiMenu)
        {
            ...
            connect(quantGroup, &QActionGroup::triggered, this,
                    [this](QAction* action)
                    {
        ...
        void MainWindow::setupSwarmEditing()
        {
            ...
            if (!m_swarmSocket)
            {
                ...
                connect(m_swarmSocket, &QWebSocket::connected, this,
                        [this]()
                        {
                ...
                // Connection closed
                connect(m_swarmSocket, &QWebSocket::disconnected, this,
                        [this]()
                        {
                ...
                // Error handling
                connect(m_swarmSocket, QOverload<QAbstractSocket::SocketError>::of(&QWebSocket::error), this,
                        [this](QAbstractSocket::SocketError error)
                        {
                            qWarning() << "[Swarm] Socket error:" << error << m_swarmSocket->errorString();
                            statusBar()->showMessage(tr("Swarm connection error: %1").arg(m_swarmSocket->errorString()),
                                                     5000);
                        });
            }
        ...
        void MainWindow::setupInterpretabilityPanel()
        {
            ...
            m_interpretabilityPanelDock->setFeatures(QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable |
                                                     QDockWidget::DockWidgetClosable);
            ...
            connect(m_interpretabilityPanel, &InterpretabilityPanelEnhanced::anomalyDetected, this,
                    [this](const QString& description)
                    {
                        // Show warning in status bar
                        statusBar()->showMessage(QString("⚠️ Model Anomaly: %1").arg(description), 10000);

                        // Log to console
                        if (m_hexMagConsole)
                        {
                            m_hexMagConsole->appendPlainText(QString("[INTERPRETABILITY] %1: %2")
                                                                 .arg(QDateTime::currentDateTime().toString("HH:mm:ss"))
                                                                 .arg(description));
                        }
            ...
            connect(m_interpretabilityPanel, &InterpretabilityPanelEnhanced::diagnosticsCompleted, this,
                    [this](const QJsonObject& diagnostics_json)
                    {
                        // Update status with diagnostics summary from JSON
                        QStringList issues;
                        if (diagnostics_json["has_vanishing_gradients"].toBool())
                            issues << "Vanishing Gradients";
                        if (diagnostics_json["has_exploding_gradients"].toBool())
                            issues << "Exploding Gradients";
                        if (diagnostics_json["has_dead_neurons"].toBool())
                            issues << "Dead Neurons";
                        if (diagnostics_json["average_sparsity"].toDouble() > 0.5)
                            issues << "High Sparsity";
                        if (diagnostics_json["attention_entropy_mean"].toDouble() < 1.0)
                            issues << "Low Attention Entropy";

                        if (!issues.isEmpty())
                        {
                            statusBar()->showMessage(
                                QString("🔍 Diagnostics: %1 issue(s) - %2").arg(issues.size()).arg(issues.join(", ")),
                                8000);
                        }
                        else
                        {
            ...
            connect(m_interpretabilityPanel, &InterpretabilityPanelEnhanced::exportRequested, this,
            ...
            if (m_inferenceEngine)
            {
                ...
                connect(m_inferenceEngine, &InferenceEngine::modelLoadedChanged, this,
                        [this](bool success)
                        {
                            if (success)
                            {
                                m_interpretabilityPanelDock->show();
                                statusBar()->showMessage(QString("📊 Interpretability Panel enabled"), 3000);
                            }
                        });

                // Connect attention data stream to interpretability panel
                connect(m_inferenceEngine, &InferenceEngine::attentionDataAvailable, this,
                        [this](const QJsonArray& attentionData)
                        {
                            if (!m_interpretabilityPanel)
                                return;
                ...
                connect(m_inferenceEngine, &InferenceEngine::gradientDataAvailable, this,
                        [this](const QJsonArray& gradientData)
                        {
                            if (!m_interpretabilityPanel)
                                return;
                ...
                connect(m_inferenceEngine, &InferenceEngine::activationDataAvailable, this,
                        [this](const QJsonArray& activationData)
                        {
                            if (!m_interpretabilityPanel)
                                return;
                ...
                connect(m_inferenceEngine, &InferenceEngine::layerContributionAvailable, this,
                        [this](const QJsonArray& layerData)
                        {
                            if (!m_interpretabilityPanel)
                                return;
                ...
                connect(m_inferenceEngine, &InferenceEngine::tokenLogitsAvailable, this,
                        [this](int tokenIdx, const QJsonArray& logits)
                        {
                            if (!m_interpretabilityPanel)
                                return;
        ...
        static const QVector<LanguageCompilerInfo> g_languageRegistry = {
            // Assembly Languages
            {"Assembly (x64)",
             {"asm", "s", "nasm", "masm", "yasm", "fasm", "gas"},
             "ml64",
             "asm_compiler_from_scratch",
             "-f win64",
             true,
             true},
        ...
        static QString targetArchToString(TargetArch a)
        {
            switch (a)
            {
                ...
                case TargetArch::MIPS64:
                    return "mips64";
                case TargetArch::MIPS32:
                    return "mips";
                case TargetArch::PowerPC64:
                    return "ppc64";
                case TargetArch::PowerPC32:
                    return "ppc";
                case TargetArch::SPARC64:
                    return "sparc64";
                case TargetArch::WebAssembly32:
                    return "wasm32";
                case TargetArch::WebAssembly64:
                    return "wasm64";
                case TargetArch::AVR:
                    return "avr";
                case TargetArch::ESP32:
                    return "xtensa";
                case TargetArch::STM32:
                    return "thumbv7em";
                default:
                    return "native";
            }
        ...
        void MainWindow::toggleCompileCurrentFile()
        {
            ...
            QLabel* headerLabel = new QLabel(
                tr("🔧 Compiling <b>%1</b> file: <code>%2</code>").arg(compilerInfo->language).arg(fi.fileName()));
            ...
            rawrxdRadio->setToolTip(
                tr("Pure assembly implementation - maximum performance, full cross-platform support"));
            rawrxdRadio->setChecked(true);

            QRadioButton* systemRadio = new QRadioButton(
                tr("🔧 System Compiler (%1)")
            ...
            QObject::connect(platformCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
                             [&](int idx)
                             {
            ...
            if (useRawrXDCompiler)
            {
                ...
                if (debugBuild)
                    compilerArgs << "--debug";
                if (optimizeBuild)
                    compilerArgs << "--optimize" << "3";
                if (staticLink)
                    compilerArgs << "--static";
                if (stripSymbols && !debugBuild)
                    compilerArgs << "--strip";
            ...
            connect(compiler, &QProcess::finished, this,
                    [this, compiler, fi, outputPath, compilerName, targetPlatform, targetArch](int exitCode)
                    {
        ...
        void MainWindow::toggleBuildProject()
        {
            ...
            // Monitor output
            connect(compiler, &QProcess::readyReadStandardOutput, this,
                    [this, compiler]()
                    {
            ...
            connect(compiler, &QProcess::readyReadStandardError, this,
                    [this, compiler]()
                    {
            ...
            connect(
                compiler, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this,
                [this, compiler, sourceFiles, projectDir](int exitCode, QProcess::ExitStatus exitStatus)
                {
                    if (exitCode == 0 && exitStatus == QProcess::NormalExit)
                    {
                        ...
                        statusBar()->showMessage(
                            tr("✅ Build successful: %1/%2 files compiled").arg(successCount).arg(sourceFiles.size()),
                            5000);
                        ...
                        if (m_hexMagConsole)
                        {
                            m_hexMagConsole->appendPlainText("\n========== BUILD SUCCESS ==========");
                            m_hexMagConsole->appendPlainText(
                                QString("Compiled: %1 of %2 files").arg(successCount).arg(sourceFiles.size()));
                            m_hexMagConsole->appendPlainText("===================================\n");
                        }

                        qInfo() << "[Build] Build successful -" << successCount << "of" << sourceFiles.size()
                                << "files";
        ...
        void MainWindow::toggleCleanBuild()
        {
            ...
            QDir dir(projectDir);
            QStringList cleanPatterns = {"*.exe", "*.obj", "*.o",  "*.lib", "*.dll",   "*.ir", "*.s",
                                         "*.out", "*.a",   "*.so", "*.dll", "*.dylib", "*.ll"};
            ...
            if (notificationCenter_)
            {
                notificationCenter_->notify(tr("Clean Complete"), tr("Cleaned %1 build artifacts").arg(cleaned),
                                            NotificationLevel::Info);
            }
        ...
        void MainWindow::toggleCompilerSettings()
        {
            ...
            if (outputIndex >= 0)
                outputCombo->setCurrentIndex(outputIndex);
            ...
            optCombo->addItems({"O0 - No optimization (Fastest compile, slower code)", "O1 - Basic optimization",
            ...
            if (optIndex >= 0)
                optCombo->setCurrentIndex(optIndex);
            else
                optCombo->setCurrentIndex(2);
            ...
            parallelLayout->addWidget(
                new QLabel(tr("Number of parallel compilation threads (System: %1 cores):").arg(maxThreads)));
            ...
            // === Info Section ===
            QLabel* infoLabel = new QLabel(tr("<b>RawrXD Compiler Information:</b><br>"
                                              "• Version: 1.0<br>"
            ...
            connect(buttons->button(QDialogButtonBox::Ok), &QPushButton::clicked, dialog,
                    [this, dialog, targetCombo, outputCombo, optCombo, debugCheck, warningsCheck, verboseCheck,
                     threadsSpinner]()
                    {
                        // Save settings
                        QSettings settings("RawrXD", "Compiler");
            ...
            connect(
                buttons->button(QDialogButtonBox::RestoreDefaults), &QPushButton::clicked, dialog,
                [this, targetCombo, outputCombo, optCombo, debugCheck, warningsCheck, verboseCheck, threadsSpinner]()
                {
        ...
        void MainWindow::toggleCompilerOutput()
        {
            ...
            if (!m_compilerOutputDock)
            {
                ...
                m_compilerOutputDock->setFeatures(QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable |
                                                  QDockWidget::DockWidgetClosable);
                ...
                connect(saveAction, &QAction::triggered, this,
                        [this]()
                        {
                            ...
                            QString fileName = QFileDialog::getSaveFileName(
                                this, tr("Save Compiler Output"), defaultPath,
                            ...
                            if (!fileName.isEmpty())
                            {
                                ...
                                if (file.open(QIODevice::WriteOnly | QIODevice::Text))
                                {
                                    ...
                                    file.close();
                                    statusBar()->showMessage(
                                        tr("✅ Output saved: %1").arg(QFileInfo(fileName).fileName()), 3000);
                ...
                m_compilerOutput->setStyleSheet("QPlainTextEdit {"
                                                "  background-color: #1e1e1e;"
                                                "  color: #dcdcdc;"
                                                "  selection-background-color: #264f78;"
                                                "  border: none;"
                ...
                connect(wrapCheck, &QCheckBox::toggled, this,
                        [this](bool checked)
                        {
                            if (m_compilerOutput)
                            {
                ...
                connect(m_compilerOutput, &QPlainTextEdit::textChanged, this,
                        [statusLabel, this]()
                        {
        ...
        void MainWindow::setupOrchestrationSystem()
        {
            ...
            if (!m_orchestrationDock)
            {
                ...
                connect(m_taskOrchestrator, &RawrXD::TaskOrchestrator::taskSplitCompleted, this,
                        [this](const QList<RawrXD::TaskDefinition>& tasks)
                        { statusBar()->showMessage(tr("Task split into %1 subtasks").arg(tasks.size()), 3000); });

                connect(
                    m_taskOrchestrator, &RawrXD::TaskOrchestrator::orchestrationCompleted, this,
                    [this](const QList<RawrXD::OrchestrationResult>& results)
                    {
                        int successCount = 0;
                        ...
                        statusBar()->showMessage(
                            tr("Orchestration completed: %1/%2 tasks successful").arg(successCount).arg(results.size()),
                            5000);
                    });
            }
        ...
        void MainWindow::setupCommandPalette()
        {
            ...
            m_commandPalette->setStyleSheet(
                ...
                "QLineEdit { background: #252526; color: #cccccc; border: 1px solid #3c3c3c; border-radius: 4px; "
            ...
            commands << tr("File: New File") << tr("File: Open File") << tr("File: Save") << tr("File: Save As")
                     ...
                     << tr("Build: Build Project") << tr("Build: Run") << tr("Build: Debug") << tr("Git: Commit")
                     << tr("Git: Push") << tr("Git: Pull") << tr("Git: Branch") << tr("AI: Send to Chat")
                     << tr("AI: Explain Code") << tr("AI: Generate Tests") << tr("Settings: Open Settings")
                     << tr("Settings: Keyboard Shortcuts") << tr("Settings: Theme");

            commandList->addItems(commands);

            // Filter on search
            connect(searchInput, &QLineEdit::textChanged, this,
                    [commandList, commands](const QString& text)
                    {
                        commandList->clear();
            ...
            connect(commandList, &QListWidget::itemActivated, this,
        ...
        void MainWindow::onAgentWishReceived(const QString& wish)
        {
            ...
            {
                RawrXD::Integration::logWarn("MainWindow", "agent_wish",
                                             "Agent System feature is disabled in safe mode");
                return;
            }
            ...
            RawrXD::Integration::logInfo(
                "MainWindow", "agent_wish_received",
        ...
        void MainWindow::onAgentPlanGenerated(const QString& planSummary)
        {
            ...
            RawrXD::Integration::logInfo(
                "MainWindow", "agent_plan_generated",
        ...
        void MainWindow::onAgentExecutionCompleted(bool success)
        {
            ...
            if (m_agentChatPane)
            {
                QString result = success ? tr("✅ Execution completed successfully") : tr("❌ Execution failed");
                m_agentChatPane->appendMessage("system", result);
            }

            MetricsCollector::instance().incrementCounter(success ? "agent_executions_successful"
            ...
            if (notificationCenter_)
            {
                notificationCenter_->notify(success ? tr("Agent Execution Successful") : tr("Agent Execution Failed"),
            ...
            RawrXD::Integration::logInfo(
                "MainWindow", "agent_execution_completed",
        ...
        void MainWindow::handleFind()
        {
            ...
            bool ok;
            QString searchText =
                QInputDialog::getText(this, tr("Find"), tr("Search for:"), QLineEdit::Normal, QString(), &ok);
        ...
        void MainWindow::onExplorerItemDoubleClicked(QTreeWidgetItem * item, int column)
        {
            ...
            RawrXD::Integration::logInfo(
                "MainWindow", "explorer_item_double_clicked",
                QString("Explorer item double-clicked: %1 (type: %2, total: %3)")
                    .arg(info.fileName(), info.isDir() ? "dir" : "file")
                    .arg(doubleClickCount),
        ...
        void MainWindow::onAIChatCodeInsertRequested(const QString& code)
        {
            ...
            {
                RawrXD::Integration::logWarn("MainWindow", "ai_chat_code_insert",
                                             "AI Chat feature is disabled in safe mode");
                return;
            }
            ...
            if (!editorObj)
            {
                ...
                QMessageBox::information(this, tr("No Active Editor"), tr("Please open an editor tab to insert code."));
                return;
            }
            ...
            if (notificationCenter_)
            {
                notificationCenter_->notify(tr("Code Inserted"),
                                            tr("AI-generated code has been inserted into the editor."),
                                            NotificationLevel::Success);
            }

            RawrXD::Integration::logInfo(
                "MainWindow", "ai_chat_code_inserted",
