...
bool InferenceEngine::hot_swap_model(const std::string& new_model_path)
{
    qInfo() << "[InferenceEngine] Initiating hot swap to model:" << QString::fromStdString(new_model_path);
    QString qPath = QString::fromStdString(new_model_path);

    // Thread-safe swap process:
    {
        // 1. Lock instance to pause active requests and generation tasks safely
        QMutexLocker lock(&m_mutex);
        
        qInfo() << "[InferenceEngine] Mutex locked. Unloading current model context and VRAM...";
        
        // Pause any ongoing processing state
        m_isProcessingInference = false;

        // 2. Clear VRAM / Tensors / Context cache immediately (Gracefully unload active llama_context/model internals)
        if (m_loader) {
            delete m_loader;
            m_loader = nullptr;
        }
        
        // Reset local memory management structures
        m_tensorCache.clear();
        m_systemPromptTokens.clear();
        m_kvCacheReady = false;
        m_modelPath.clear();
        
        // Cleanse transformer weights/state
        m_transformer = TransformerInference();
        
        qInfo() << "[InferenceEngine] Active model context successfully unloaded and VRAM cleared.";
        
        // Signal that the model is no longer loaded
        QMetaObject::invokeMethod(this, "modelLoadedChanged", Qt::QueuedConnection,
            Q_ARG(bool, false), Q_ARG(QString, QString()));
            
        // Lock releases here before loadModel to prevent deadlocks
    }

    // 3. Re-initialize the new model context without tearing down the whole engine
    qInfo() << "[InferenceEngine] Re-initializing new model context safely...";
    return loadModel(qPath);
}
