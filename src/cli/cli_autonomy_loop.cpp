...
std::string CLIAutonomyLoop::runSSALift(const std::string& binaryPath,
    ...
    // Gate: RAWRXD_AGENT_TOOL_AUTHORITY_001 — route SSA lift through the
    // single tool-dispatch authority (CLI surface) instead of calling the
    // decision tree's internal lifter directly.
    std::ostringstream addrStr;
    addrStr << "0x" << std::hex << functionAddr;

    RawrXD::Agent::AgentToolRequest req;
    req.tool_name = "ssa_lift";
    req.caller_surface = "cli";
    req.args = {
        {"binary_path", binaryPath},
        {"function_start", addrStr.str()}
    };

    auto result = RawrXD::Agent::AgentToolAuthority::Dispatch(req);
    if (result.success) {
        return result.output;
    }

    // Fallback: legacy decision-tree path (records bypass for gate honesty)
    RawrXD::Agent::AgentToolAuthority::IncrementBypass(
        RawrXD::Agent::AgentToolSurface::CLI);
