...
bool cli_headless_init(AgenticEngine* engine, SubAgentManager* subAgentMgr)
{
    ...
    g_historyRecorder->setLogCallback(
    ...
    g_historyRecorder->record(
        "session_init", "system", "", "Headless CLI systems initialized", "", "", true, 0, "",
        "{\"phase\":\"20\",\"systems\":\"safety,confidence,replay,governor,history,policy,explain,multi_response\"}");
...
void cli_headless_shutdown()
{
    ...
    {
        g_historyRecorder->record("session_shutdown", "system", "", "Headless CLI shutting down", "", "", true, 0, "",
                                  "");
    ...
    delete g_explainEngine;
    g_explainEngine = nullptr;
    delete g_policyEngine;
    g_policyEngine = nullptr;
    delete g_historyRecorder;
    g_historyRecorder = nullptr;
    delete g_multiResponse;
    g_multiResponse = nullptr;
...
bool cli_gate_check(int actionClass, int riskTier, float confidence, const std::string& description)
{
    // Safety contract check
    auto& safety = AgentSafetyContract::instance();
    auto safetyResult = safety.checkAndConsume(static_cast<ActionClass>(actionClass),
                                               static_cast<SafetyRiskTier>(riskTier), description, confidence);
    ...
    auto confResult = gate.evaluateAndRecord(confidence, static_cast<ActionClass>(actionClass),
                                             static_cast<SafetyRiskTier>(riskTier), description);

    if (confResult.decision == ConfidenceDecision::Abort)
    {
        std::cout << "🎯 Confidence ABORT: " << confResult.reason << " (confidence=" << confResult.rawConfidence
    ...
    {
        std::cout << "⚠️  Confidence ESCALATE (auto-approved in headless): " << confResult.reason << "\n";
    }

    // Record in replay journal
    auto& replay = ReplayJournal::instance();
    replay.recordSafetyEvent(ReplayActionType::SafetyCheck, description,
...
void cli_record_action(int actionType, const std::string& category, const std::string& action, const std::string& input,
                       const std::string& output, int exitCode, float confidence, double durationMs)
{
    auto& replay = ReplayJournal::instance();
    replay.recordAction(static_cast<ReplayActionType>(actionType), category, action, input, output, exitCode,
...
void cmd_safety(const std::string& args)
{
    ...
    else if (sub == "violations")
    {
        ...
        {
            std::cout << "  [" << v.id << "] " << violationTypeToString(v.type) << " — "
                      << actionClassToString(v.attemptedAction) << " (risk=" << safetyRiskTierToString(v.riskTier)
                      << ")\n";
            std::cout << "       " << v.description << "\n";
            if (v.wasEscalated)
            {
                std::cout << "       Escalated → " << (v.userApproved ? "approved" : "denied") << "\n";
            }
        }
    }
    else if (sub == "block")
    {
        ...
        if (rest == "ReadFile")
            ac = ActionClass::ReadFile;
        else if (rest == "WriteFile")
            ac = ActionClass::WriteFile;
        else if (rest == "EditFile")
            ac = ActionClass::EditFile;
        else if (rest == "CreateFile")
            ac = ActionClass::CreateFile;
        else if (rest == "DeleteFile")
            ac = ActionClass::DeleteFile;
        else if (rest == "RunCommand")
            ac = ActionClass::RunCommand;
        else if (rest == "RunBuild")
            ac = ActionClass::RunBuild;
        else if (rest == "RunTest")
            ac = ActionClass::RunTest;
        else if (rest == "InstallPackage")
            ac = ActionClass::InstallPackage;
        else if (rest == "NetworkRequest")
            ac = ActionClass::NetworkRequest;
        else if (rest == "ModifyModel")
            ac = ActionClass::ModifyModel;
        else if (rest == "PatchMemory")
            ac = ActionClass::PatchMemory;
        else if (rest == "ProcessKill")
            ac = ActionClass::ProcessKill;
        else if (rest == "ServerRestart")
            ac = ActionClass::ServerRestart;
        else if (rest == "SearchCode")
            ac = ActionClass::SearchCode;
        else if (rest == "QueryModel")
            ac = ActionClass::QueryModel;
        else
        {
    ...
    else if (sub == "risk")
    {
        ...
        if (rest == "None")
            tier = SafetyRiskTier::None;
        else if (rest == "Low")
            tier = SafetyRiskTier::Low;
        else if (rest == "Medium")
            tier = SafetyRiskTier::Medium;
        else if (rest == "High")
            tier = SafetyRiskTier::High;
        else if (rest == "Critical")
            tier = SafetyRiskTier::Critical;
        else
        {
...
void cmd_confidence(const std::string& args)
{
    ...
    if (sub == "status" || sub.empty())
    {
        ...
        std::cout << "  Avg confidence:    " << std::fixed << std::setprecision(3) << stats.avgConfidence << "\n";
    ...
    else if (sub == "policy")
    {
        ...
        if (rest == "strict")
            pol = GatePolicy::Strict;
        else if (rest == "normal")
            pol = GatePolicy::Normal;
        else if (rest == "relaxed")
            pol = GatePolicy::Relaxed;
        else if (rest == "disabled")
            pol = GatePolicy::Disabled;
        else
        {
    ...
    else if (sub == "threshold")
    {
        ...
        std::cout << "✅ Thresholds set: execute=" << exec_t << " escalate=" << esc_t << " abort=" << abort_t << "\n";
    }
    else if (sub == "history")
    {
        ...
        std::cout << std::setw(6) << "Seq" << std::setw(10) << "Conf" << std::setw(12) << "Decision" << std::setw(16)
                  << "Action" << "\n";
        std::cout << std::string(44, '-') << "\n";
        for (const auto& h : history)
        {
            std::cout << std::setw(6) << h.sequenceId << std::setw(10) << std::fixed << std::setprecision(3)
                      << h.confidence << std::setw(12) << confidenceDecisionToString(h.decision) << std::setw(16)
                      << actionClassToString(h.actionClass) << "\n";
        }
        std::cout << "\n";
    }
    else if (sub == "trend")
...
void cmd_replay(const std::string& args)
{
    ...
    else if (sub == "last")
    {
        ...
        {
            std::cout << "[" << std::setw(6) << r.sequenceId << "] " << std::setw(20) << std::left
                      << replayActionTypeToString(r.type) << " " << std::setw(10) << std::left << r.category << " ";
            if (!r.action.empty())
            {
    ...
    else if (sub == "sessions")
    {
        ...
        {
            std::cout << "  [" << s.sessionId << "] " << s.sessionLabel << " (" << s.actionCount << " actions, "
    ...
    else if (sub == "filter")
    {
        ...
        if (rest == "AgentQuery")
            filter.filterType = ReplayActionType::AgentQuery;
        else if (rest == "AgentResponse")
            filter.filterType = ReplayActionType::AgentResponse;
        else if (rest == "AgentToolCall")
            filter.filterType = ReplayActionType::AgentToolCall;
        else if (rest == "CommandExecution")
            filter.filterType = ReplayActionType::CommandExecution;
        else if (rest == "FileWrite")
            filter.filterType = ReplayActionType::FileWrite;
        else if (rest == "ModelInference")
            filter.filterType = ReplayActionType::ModelInference;
        else if (rest == "SafetyCheck")
            filter.filterType = ReplayActionType::SafetyCheck;
        else if (rest == "SafetyDenial")
            filter.filterType = ReplayActionType::SafetyDenial;
        else if (rest == "GovernorSubmit")
            filter.filterType = ReplayActionType::GovernorSubmit;
        else if (rest == "GovernorTimeout")
            filter.filterType = ReplayActionType::GovernorTimeout;
        else if (rest == "Checkpoint")
            filter.filterType = ReplayActionType::Checkpoint;
        else if (rest == "ModelHotpatch")
            filter.filterType = ReplayActionType::ModelHotpatch;
        else
        {
        ...
        for (const auto& r : records)
        {
            ...
            if (r.durationMs > 0)
                std::cout << " (" << formatDuration(r.durationMs) << ")";
            std::cout << "\n";
        }
        std::cout << "\n";
    }
...
void cmd_governor(const std::string& args)
{
    ...
    else if (sub == "run")
    {
        ...
        if (!cli_gate_check(static_cast<int>(ActionClass::RunCommand), static_cast<int>(SafetyRiskTier::High), 0.8f,
                            "Governor: " + command))
        {
            return;
        }
        ...
        auto taskId = governor.submitCommand(command, timeout, GovernorRiskTier::Medium, "CLI: " + command);

        // Record in replay
        auto& replay = ReplayJournal::instance();
        replay.recordGovernorEvent(ReplayActionType::GovernorSubmit, "CLI governor run: " + command, taskId);
        ...
        if (result.timedOut)
        {
            ...
            replay.recordGovernorEvent(ReplayActionType::GovernorTimeout, "Timed out: " + command, taskId);
        }
        ...
        else
        {
            ...
            replay.recordGovernorEvent(ReplayActionType::GovernorComplete, "Completed: " + command, taskId);
        }
        ...
        std::cout << "Duration: " << formatDuration(result.durationMs) << " (" << result.bytesRead
                  << " bytes captured)\n";
        std::cout << "───────────────────────\n\n";
        ...
        if (g_historyRecorder)
        {
            g_historyRecorder->record("command_execution", "governor", "", command, result.output, "",
                                      result.exitCode == 0 && !result.timedOut, static_cast<int>(result.durationMs),
    ...
    else if (sub == "kill")
    {
        ...
        std::cout << (ok ? "✅ Task killed" : "❌ Task not found or already complete") << " (id=" << taskId << ")\n";
    }
    ...
    else if (sub == "wait")
    {
        ...
        std::cout << "Exit: " << result.exitCode << " Duration: " << formatDuration(result.durationMs) << "\n";
        if (!result.output.empty())
            std::cout << result.output << "\n";
    }
    else
...
void cmd_multi_response(const std::string& args)
{
    ...
    if (sub == "templates")
    {
        ...
        {
            std::cout << "  [" << static_cast<int>(t.id) << "] " << t.name << " " << (t.enabled ? "✅" : "❌") << "\n";
        }
        std::cout << "\n";
    }
    else if (sub == "stats")
    ...
    else if (sub == "compare")
    {
        ...
        std::cout << "\n" << std::string(80, '=') << "\n";
        std::cout << "Multi-Response Comparison — Prompt: \"" << session->prompt.substr(0, 60) << "\"\n";
        std::cout << std::string(80, '=') << "\n";
        ...
        {
            const auto& pref = session->responses[session->preferredIndex];
            const auto& tmpl = g_multiResponse->getTemplate(pref.templateId);
    ...
    else if (!sub.empty())
    {
        ...
        // Gate check
        if (!cli_gate_check(static_cast<int>(ActionClass::QueryModel), static_cast<int>(SafetyRiskTier::Low), 0.9f,
                            "Multi-response: " + prompt.substr(0, 60)))
        {
            return;
        }
        ...
        if (g_historyRecorder)
        {
            g_historyRecorder->record("multi_response", "system", "",
                                      "Generated " + std::to_string(maxResp) + " responses", prompt, "", true, 0, "",
                                      "");
        }
    }
    else
...
void cmd_history(const std::string& args)
{
    ...
    if (sub == "show" || sub.empty())
    {
        ...
        {
            std::cout << "[" << std::setw(6) << e.id << "] " << std::setw(20) << std::left << e.eventType
                      << (e.success ? "✅" : "❌") << " ";
            if (!e.agentId.empty())
    ...
    else if (sub == "session")
    {
        ...
        {
            std::cout << "  [" << e.id << "] " << e.eventType << (e.success ? " ✅" : " ❌") << " "
                      << e.description.substr(0, 50) << "\n";
        }
        std::cout << "\n";
    }
    else if (sub == "agent")
    ...
    else if (sub == "type")
    {
        ...
        for (const auto& e : events)
        {
            std::cout << "  [" << e.id << "] " << (e.success ? "✅" : "❌") << " " << e.description.substr(0, 60)
                      << "\n";
        }
        std::cout << "\n";
    }
...
void cmd_explain(const std::string& args)
{
    ...
    if (sub == "agent")
    {
        ...
        std::cout << "  Nodes: " << trace.nodeCount << " Failures: " << trace.failureCount
                  << " Policies: " << trace.policyFireCount << " Duration: " << trace.totalDurationMs << "ms\n";
        ...
        {
            std::cout << "    [" << node.eventId << "] " << node.eventType << " " << (node.success ? "✅" : "❌") << " "
    ...
    else if (sub == "failures")
    {
        ...
        {
            std::cout << "  [" << f.failureEventId << "] " << f.failureType << " agent=" << f.agentId << "\n";
            ...
            {
                std::cout << "    Historical success rate: " << std::fixed << std::setprecision(1)
                          << (f.historicalSuccessRate * 100) << "% (" << f.historicalOccurrences << " occurrences)\n";
            }
        }
        std::cout << "\n";
    }
    else if (sub == "policies")
    {
        ...
        for (const auto& p : attrs)
        {
            ...
            if (!p.triggerPattern.empty())
                std::cout << " pattern=" << p.triggerPattern;
            if (p.triggerFailureRate > 0)
                std::cout << " failRate=" << p.triggerFailureRate;
            std::cout << "\n";
            std::cout << "    Effect: " << p.effectDescription << "\n";
            std::cout << "    Applied " << p.policyAppliedCount << " times"
                      << " est.improvement=" << std::fixed << std::setprecision(1) << (p.estimatedImprovement * 100)
                      << "%\n";
        }
        std::cout << "\n";
    }
...
void cmd_policy(const std::string& args)
{
    ...
    if (sub == "list" || sub.empty())
    {
        ...
        {
            std::cout << "  " << (p.enabled ? "✅" : "❌") << " [" << p.id.substr(0, 8) << "] " << p.name << " (v"
                      << p.version << ", priority=" << p.priority << ", applied=" << p.appliedCount << ")\n";
    ...
    else if (sub == "show")
    {
        ...
        if (!p)
        {
            ...
            {
                if (pol.id.find(rest) == 0)
                {
                    p = g_policyEngine->getPolicy(pol.id);
                    break;
                }
            }
        }
    ...
    else if (sub == "enable")
    {
        if (rest.empty())
        {
            std::cout << "Usage: !policy enable <id>\n";
            return;
        }
    ...
    else if (sub == "disable")
    {
        if (rest.empty())
        {
            std::cout << "Usage: !policy disable <id>\n";
            return;
        }
    ...
    else if (sub == "remove")
    {
        if (rest.empty())
        {
            std::cout << "Usage: !policy remove <id>\n";
            return;
        }
    ...
    else if (sub == "heuristics")
    {
        ...
        std::cout << std::setw(25) << std::left << "Key" << std::setw(8) << "Total" << std::setw(8) << "Pass"
                  << std::setw(8) << "Fail" << std::setw(10) << "Rate" << std::setw(10) << "Avg ms" << "\n";
        std::cout << std::string(69, '-') << "\n";
        for (const auto& h : heuristics)
        {
            std::cout << std::setw(25) << std::left << h.key << std::setw(8) << h.totalEvents << std::setw(8)
                      << h.successCount << std::setw(8) << h.failCount << std::setw(10) << std::fixed
                      << std::setprecision(1) << (h.successRate * 100) << "%" << std::setw(10)
                      << static_cast<int>(h.avgDurationMs) << "\n";
        }
        std::cout << "\n";
    }
    else if (sub == "suggest")
    {
        ...
        for (const auto& s : suggestions)
        {
            ...
            std::cout << "    Est. improvement: " << std::fixed << std::setprecision(1)
                      << (s.estimatedImprovement * 100) << "% (" << s.supportingEvents << " supporting events)\n";
    ...
    else if (sub == "accept")
    {
        if (rest.empty())
        {
            std::cout << "Usage: !policy accept <suggestion_id>\n";
            return;
        }
        bool ok = g_policyEngine->acceptSuggestion(rest);
        std::cout << (ok ? "✅ Suggestion accepted and policy activated" : "❌ Suggestion not found") << "\n";
    }
    else if (sub == "reject")
    {
        if (rest.empty())
        {
            std::cout << "Usage: !policy reject <suggestion_id>\n";
            return;
        }
    ...
    else if (sub == "export")
    {
        if (rest.empty())
        {
            std::cout << "Usage: !policy export <filepath>\n";
            return;
        }
    ...
    else if (sub == "import")
    {
        if (rest.empty())
        {
            std::cout << "Usage: !policy import <filepath>\n";
            return;
        }
    ...
    {
        std::cout << "Usage: !policy "
                     "<list|show|enable|disable|remove|heuristics|suggest|accept|reject|pending|export|import|stats>\n";
    }
}
...
void cmd_model(const std::string& args)
{
    ...
    if (sub == "load" || sub == "model_load")
    {
        ...
        if (!cli_gate_check(static_cast<int>(ActionClass::ReadFile), static_cast<int>(SafetyRiskTier::Low), 0.95f,
                            "Analyze GGUF model: " + rest))
        {
            return;
        }
        ...
        if (ok)
        {
            ...
            std::cout << "  Parameters: " << std::fixed << std::setprecision(1) << (totalParams / 1e9) << "B\n";
            std::cout << "  Total size: " << (totalSize / (1024ULL * 1024ULL)) << " MB\n";
            std::cout << "  Size tier: " << hotpatcher.getModelSizeTier() << "\n";

            cli_record_action(static_cast<int>(ReplayActionType::AgentToolCall), "model", "analyze", rest,
                              "Analyzed: " + std::to_string(layers.size()) + " layers, " + std::to_string(totalParams) +
                                  " params",
                              0, 1.0f, 0.0);
        }
        else
    ...
    else if (sub == "plan" || sub == "model_plan")
    {
        ...
        for (const auto& d : plan)
        {
            ...
            if (d.qualityImpact > maxQualityImpact)
                maxQualityImpact = d.qualityImpact;
        }
        ...
        std::cout << "  Max quality impact: " << std::fixed << std::setprecision(2) << (maxQualityImpact * 100.0f)
                  << "%\n";
    ...
    else if (sub == "surgery" || sub == "model_surgery")
    {
        ...
        if (!cli_gate_check(static_cast<int>(ActionClass::ModifyModel), static_cast<int>(SafetyRiskTier::Critical),
        ...
        hotpatcher.setSurgeryProgressCallback(
            [](SurgeryOp op, uint32_t layerIdx, uint32_t totalLayers, float progress, void* /*userData*/)
            {
                const char* opStr = "Unknown";
                switch (op)
                {
                    case SurgeryOp::RequantizeLayer:
                        opStr = "Requantize";
                        break;
                    case SurgeryOp::RequantizeRange:
                        opStr = "RequantRange";
                        break;
                    case SurgeryOp::RequantizeAll:
                        opStr = "RequantAll";
                        break;
                    case SurgeryOp::EvictLayer:
                        opStr = "Evict";
                        break;
                    case SurgeryOp::ReloadLayer:
                        opStr = "Reload";
                        break;
                    case SurgeryOp::SplitLayer:
                        opStr = "Split";
                        break;
                    case SurgeryOp::MergeShards:
                        opStr = "Merge";
                        break;
                    case SurgeryOp::CompressKVCache:
                        opStr = "Compress";
                        break;
                }
                std::cout << "\r  [" << opStr << "] Layer " << layerIdx << "/" << totalLayers << " " << std::fixed
                          << std::setprecision(1) << (progress * 100.0f) << "%   " << std::flush;
            },
            nullptr);
        ...
        if (result.success)
        {
            ...
            cli_record_action(static_cast<int>(ReplayActionType::AgentToolCall), "model", "surgery", "",
                              "Surgery completed: " + std::to_string(result.layersAffected) + " layers", 0, 1.0f,
                              elapsed);
        }
        else
        {
    ...
    else if (sub == "pressure" || sub == "pressure_auto")
    {
        ...
        std::cout << "    Pressure:      " << static_cast<int>(budget.pressure) << " ("
                  << (budget.pressure == VRAMPressure::Low        ? "Low"
                      ...
                      : budget.pressure == VRAMPressure::Critical ? "Critical"
                                                                  : "Emergency")
                  << ")\n";
    ...
    else if (sub == "layers")
    {
        ...
        for (const auto& l : layers)
        {
            ...
            std::cout << "  " << state << " [" << std::setw(3) << l.index << "] " << std::setw(30) << std::left
                      << l.name << " Q" << static_cast<int>(l.quantType) << " " << std::setw(8) << std::right
                      << (l.sizeBytes / 1024) << " KB"
                      << (l.loadedInVRAM  ? " [GPU]"
                          : l.loadedInRAM ? " [RAM]"
                                          : " [DISK]")
...
void cmd_swarm_orchestrator(const std::string& args)
{
    ...
    if (sub == "join" || sub == "swarm_join")
    {
        ...
        if (!cli_gate_check(static_cast<int>(ActionClass::NetworkRequest), static_cast<int>(SafetyRiskTier::Medium),
        ...
        cli_record_action(static_cast<int>(ReplayActionType::AgentToolCall), "swarm", "join", rest, r.detail,
                          r.success ? 0 : 1, 0.9f, 0.0);
    }
    ...
    else if (sub == "distribute" || sub == "swarm_distribute")
    {
        ...
        if (!cli_gate_check(static_cast<int>(ActionClass::ModifyModel), static_cast<int>(SafetyRiskTier::High), 0.85f,
                            "Distribute model across swarm: " + modelPath))
        {
            return;
        }
        ...
        cli_record_action(static_cast<int>(ReplayActionType::AgentToolCall), "swarm", "distribute",
                          modelPath + " " + std::to_string(totalLayers), r.detail, r.success ? 0 : 1, 0.85f, 0.0);
    }
    else if (sub == "rebalance" || sub == "swarm_rebalance")
    {
        ...
        // Gate check
        if (!cli_gate_check(static_cast<int>(ActionClass::ModifyModel), static_cast<int>(SafetyRiskTier::Medium), 0.90f,
                            "Rebalance swarm layer shards"))
        {
            return;
        }
    ...
    else if (sub == "nodes" || sub == "swarm_nodes")
    {
        ...
        for (const auto& n : nodes)
        {
            ...
            switch (n.state)
            {
                case RawrXD::Swarm::NodeState::Offline:
                    stateStr = "OFFLINE";
                    break;
                case RawrXD::Swarm::NodeState::Joining:
                    stateStr = "JOINING";
                    break;
                case RawrXD::Swarm::NodeState::Active:
                    stateStr = "ACTIVE";
                    break;
                case RawrXD::Swarm::NodeState::Overloaded:
                    stateStr = "OVERLOADED";
                    break;
                case RawrXD::Swarm::NodeState::Failed:
                    stateStr = "FAILED";
                    break;
            }
            std::cout << "  " << (n.state == RawrXD::Swarm::NodeState::Active ? "🟢" : "🔴") << " " << n.nodeId << " ["
                      << stateStr << "]"
                      << " VRAM=" << (n.freeVRAM / (1024ULL * 1024ULL)) << "/" << (n.totalVRAM / (1024ULL * 1024ULL))
                      << "MB"
                      << " GPU=" << (n.gpuAccel ? "YES" : "NO") << " layers=" << n.hostedLayers.size() << "\n";
        }
    }
    else if (sub == "shards" || sub == "swarm_shards")
    {
        ...
        {
            std::cout << "  " << (s.resident ? "🟢" : "⚪") << " Layers " << s.layerStart << "-" << s.layerEnd << " → "
                      << s.nodeId << " Q" << static_cast<int>(s.quant) << " " << (s.sizeBytes / (1024ULL * 1024ULL))
                      << "MB"
                      << " refs=" << s.refCount << "\n";
        }
    }
