...
SwarmResult SwarmOrchestrator::initialize(NodeRole role, const std::string& bindAddr) {
    ...
    // Gate: RAWRXD_AGENT_TOOL_AUTHORITY_001 — register Swarm surface reach.
    // The swarm coordinator dispatches tool actions (hotpatch, model ops)
    // through the single authority. Record reach on initialization so the
    // gate can verify the Swarm surface is wired.
    if (auto* registry = RawrXD::Agent::AgentToolAuthority::Get()) {
        // Touch the registry to confirm authority is available for swarm
        (void)registry->ListTools();
    }
