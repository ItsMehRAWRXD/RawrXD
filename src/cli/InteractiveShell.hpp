...
╚═══════════════════════════════════════════════════════════════╝
    ...
    std::string hotPatch(const std::string& file, const std::string& function) {
        if (file.empty() || function.empty()) {
            return "[HotPatch] Usage: /hotpatch <file> <function>";
        }

        std::ifstream in(file, std::ios::binary);
        if (!in) {
            return "[HotPatch] File not found: " + file;
        }

        in.seekg(0, std::ios::end);
        std::streamoff endPos = in.tellg();
        if (endPos < 0) {
            return "[HotPatch] Failed to read file: " + file;
        }
        in.seekg(0, std::ios::beg);

        std::string content;
        content.resize(static_cast<size_t>(endPos));
        if (!content.empty()) {
            in.read(&content[0], static_cast<std::streamsize>(content.size()));
            if (!in.good() && !in.eof()) {
                return "[HotPatch] Failed to load file data: " + file;
            }
        }
        in.close();

        const size_t symbolPos = content.find(function);
        if (symbolPos == std::string::npos) {
            return "[HotPatch] Symbol not found: " + function;
        }

        const std::string marker = "// [RawrXD HotPatch] target=" + function + "\n";
        ...
        if (content.compare(lineStart, marker.size(), marker) == 0) {
            return "[HotPatch] Patch already present for " + function;
        }

        std::string backupPath = file + ".rawrxd.bak";
        std::ofstream backup(backupPath, std::ios::binary | std::ios::trunc);
        if (!backup) {
            return "[HotPatch] Failed to create backup: " + backupPath;
        }
        if (!content.empty()) {
            backup.write(content.data(), static_cast<std::streamsize>(content.size()));
        }
        backup.close();

        std::string patched = content;
        patched.insert(lineStart, marker);
        std::ofstream out(file, std::ios::binary | std::ios::trunc);
        if (!out) {
            return "[HotPatch] Failed to open file for patching: " + file;
        }
        if (!patched.empty()) {
        ...
        return "[HotPatch] Applied patch marker for " + function + " (backup: " + backupPath + ")";
    }
