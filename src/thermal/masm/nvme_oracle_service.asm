...
OFF_DEBUG_DRIVE               EQU 152      ; 0x98
OFF_DEBUG_BYTES               EQU 156      ; 0x9C
OFF_DEBUG_RAW                 EQU 160      ; 0xA0 (64 bytes)
OFF_DEBUG_STAGE               EQU 224      ; 0xE0
OFF_DEBUG_ERROR               EQU 228      ; 0xE4
OFF_DEBUG_IOCTL               EQU 232      ; 0xE8

; Magic Values
SIGNATURE_SOVE                EQU 45564F53h  ; "SOVE" little-endian
VERSION_VALUE                 EQU 1
MAX_DRIVES                    EQU 16
TEMP_INVALID                  EQU -273
TEMP_DESC_SIZE                EQU 512
...
STORAGE_TEMPERATURE_INFO STRUCT 1
    Index                     dd ?
    Temperature               dd ?      ; In 0.1 Kelvin units
    OverThreshold             dd ?
    UnderThreshold            dd ?
...
.DATA
    ...
    ; MMF name (Global namespace - shared with harness/installer)
    ...
    szDbgDriveTemp    db "[NVMeOracle] Drive %d RawTemp = %d (0.1K)",13,10,0
    szDbgDriveFail    db "[NVMeOracle] Drive %d: IOCTL failed",13,10,0
    szDbgDriveRawHex  db "[NVMeOracle] Drive %d Raw[%02X]: %08X %08X %08X %08X %08X %08X %08X %08X",13,10,0

    ; Debug log path
    szDbgLogDir       db "C:\\ProgramData\\Sovereign",0
    szDbgLogPath      db "C:\\ProgramData\\Sovereign\\nvme_debug.log",0
    szDbgLogDirAlt    db "C:\\Sovereign",0
    szDbgLogPathAlt   db "C:\\Sovereign\\nvme_debug.log",0
    ...
    tempDescPadding   db (TEMP_DESC_SIZE - sizeof STORAGE_TEMPERATURE_DATA_DESCRIPTOR) dup(0)
    bytesReturned     dq 0
    logBytesWritten   dq 0
    g_dbgStage         dd 0
    g_dbgLastError     dd 0
    g_dbgIoctlResult   dd 0
...
; ═══════════════════════════════════════════════════════════════════════════════
; LogAppend - Append debug string to nvme_debug.log
; RCX = pointer to null-terminated string
; ═══════════════════════════════════════════════════════════════════════════════
LogAppend PROC FRAME
    push    rbx
    .pushreg rbx
    push    rdi
    .pushreg rdi
    ...
    ; CreateDirectoryA(szDbgLogDir, NULL)
    lea     rcx, szDbgLogDir
    xor     rdx, rdx
    call    CreateDirectoryA

    ; Try primary log path
    lea     rcx, szDbgLogPath
    mov     edx, GENERIC_WRITE OR FILE_APPEND_DATA
    ...
    ; CreateDirectoryA(szDbgLogDirAlt, NULL)
    lea     rcx, szDbgLogDirAlt
    xor     rdx, rdx
    call    CreateDirectoryA

    ; Try alternate log path
    lea     rcx, szDbgLogPathAlt
    mov     edx, GENERIC_WRITE OR FILE_APPEND_DATA
    mov     r8d, FILE_SHARE_READ OR FILE_SHARE_WRITE
    xor     r9d, r9d
    mov     dword ptr [rsp+32], OPEN_ALWAYS
...
@GotLogHandle:
    ...
    ; WriteFile(hFile, buffer, len, &logBytesWritten, NULL)
    mov     rcx, rdi
    mov     rdx, rbx
    mov     r8d, eax
    lea     r9, logBytesWritten
...
; ECX = drive index (0-9)
; Returns: EAX = temperature in 0.1K units, or TEMP_INVALID on failure
; ═══════════════════════════════════════════════════════════════════════════════
QueryDriveTemp PROC FRAME
    ...
    cmp     rax, INVALID_HANDLE_VALUE
    je      @CreateFailed
    mov     r12, rax                    ; Save device handle
    ...
    ; Clear tempDesc buffer to avoid stale data
    lea     rdi, tempDesc
    mov     ecx, TEMP_DESC_SIZE / 8
    xor     eax, eax
    rep     stosq
    ...
    mov     qword ptr [rsp+40], TEMP_DESC_SIZE
    lea     rax, bytesReturned
    mov     qword ptr [rsp+48], rax
    mov     qword ptr [rsp+56], 0
    mov     qword ptr [bytesReturned], 0
    ...
    mov     rax, qword ptr [bytesReturned]
    cmp     rax, 32                     ; Header (16) + one info entry (16)
    jl      @QueryFailed
    
    ; Check TemperatureInfoCount > 0
    lea     rbx, tempDesc
    mov     eax, dword ptr [rbx+8]      ; TemperatureInfoCount
    test    eax, eax
    jz      @QueryFailed
    ...
    sub     rsp, 80h
    lea     rcx, dbgBuffer
    lea     rdx, szDbgDriveRawHex
    mov     r8d, esi                    ; Drive ID
    mov     r9d, dword ptr [bytesReturned]
    mov     eax, dword ptr [tempDesc]
    mov     dword ptr [rsp+20h], eax
    mov     eax, dword ptr [tempDesc+4]
    mov     dword ptr [rsp+28h], eax
    mov     eax, dword ptr [tempDesc+8]
    mov     dword ptr [rsp+30h], eax
    mov     eax, dword ptr [tempDesc+0Ch]
    mov     dword ptr [rsp+38h], eax
    mov     eax, dword ptr [tempDesc+10h]
    mov     dword ptr [rsp+40h], eax
    mov     eax, dword ptr [tempDesc+14h]
    mov     dword ptr [rsp+48h], eax
    mov     eax, dword ptr [tempDesc+18h]
    mov     dword ptr [rsp+50h], eax
    mov     eax, dword ptr [tempDesc+1Ch]
    ...
    ; Get raw temperature from first info entry (0.1 Kelvin units)
    mov     eax, dword ptr [rbx+14h]
    test    eax, eax
    jz      @QueryFailed
    
    ; Sanity range check (0.1K units)
    cmp     eax, 5000
    jg      @QueryFailed
    cmp     eax, 1000
    jl      @QueryFailed
    
    ; Debug output: Drive N raw temp
...
@CreateFailed:
    mov     dword ptr [g_dbgStage], 1
    call    GetLastError
    mov     dword ptr [g_dbgLastError], eax
    mov     dword ptr [g_dbgIoctlResult], 0
    jmp     @QueryFailed

@IoctlFailed:
    mov     dword ptr [g_dbgStage], 2
    mov     dword ptr [g_dbgIoctlResult], eax
    call    GetLastError
    mov     dword ptr [g_dbgLastError], eax
    jmp     @QueryFailed
...
UpdateDrives PROC FRAME
    ...
    mov     r11d, dword ptr [rdi + OFF_COUNT]
    test    r11d, r11d
    jnz     @DriveCountOk
    mov     r11d, MAX_DRIVES
@DriveCountOk:
    cmp     r11d, MAX_DRIVES
    jbe     @DriveCountClamped
    mov     r11d, MAX_DRIVES
@DriveCountClamped:
...
@DriveLoop:
    ...
    ; Drive ID is the loop index (PhysicalDriveN)
    mov     ecx, r12d
    
    ; Query temperature for this drive
    call    QueryDriveTemp
    mov     r10d, eax                   ; Preserve result for MMF temp write

    ; Debug: store last raw descriptor in MMF tail
    mov     dword ptr [rdi + OFF_DEBUG_DRIVE], r12d
    mov     eax, dword ptr [bytesReturned]
    mov     dword ptr [rdi + OFF_DEBUG_BYTES], eax
    mov     eax, dword ptr [g_dbgStage]
    mov     dword ptr [rdi + OFF_DEBUG_STAGE], eax
    mov     eax, dword ptr [g_dbgLastError]
    mov     dword ptr [rdi + OFF_DEBUG_ERROR], eax
    mov     eax, dword ptr [g_dbgIoctlResult]
    mov     dword ptr [rdi + OFF_DEBUG_IOCTL], eax
    lea     rsi, tempDesc
    lea     rbx, [rdi + OFF_DEBUG_RAW]
...
ServiceEntry PROC FRAME
    sub     rsp, 88             ; Alloc 88 bytes (align 16 byte boundary)
                                ; Shadow (32) + Array (32) + Padding (24)
    .allocstack 88
    .endprolog
    ...
    ; Using offset 32 (after shadow space)
