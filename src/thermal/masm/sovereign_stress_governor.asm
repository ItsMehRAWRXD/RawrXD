...
.data
    ...
    szNoDrives          db "[ERROR] No valid drives available!", 13, 10, 0
    lenNoDrives         equ $ - szNoDrives
    
    szDriveOpenFail     db "[ERROR] Drive open failed!", 13, 10, 0
    lenDriveOpenFail    equ $ - szDriveOpenFail
    
    szRankResult        db "[DEBUG] Rank result: ", 0
    lenRankResult       equ $ - szRankResult
    ...
    ; Formatting
    szDriveTemp         db "[THERMAL] Hottest: ", 0
    lenDriveTemp        equ $ - szDriveTemp
    szTempSuffix        db " C", 13, 10, 0
    lenTempSuffix       equ $ - szTempSuffix
    szBestDrive         db "[GOVERNOR] Selected best drive: ", 0
    lenBestDrive        equ $ - szBestDrive
...
.data?
    ...
    ; I/O Buffer (use VirtualAlloc at runtime for 4K alignment)
    ioBuffer            dq ?            ; Pointer to 64KB aligned buffer (allocated at runtime)
...
Governor_StressCycle PROC FRAME
    ...
    lea rcx, [szBestDrive]
    mov edx, lenBestDrive
    call ConsolePrint
    
    ; Print drive number
    mov ecx, r15d
    lea rdx, [scratchBuf]
    mov [rsp+48h], rdx          ; Save buffer address
    call IntToStr
    mov rcx, [rsp+48h]          ; Restore buffer address  
    mov edx, eax                ; Length in EAX
    call ConsolePrint
    ...
    ; Skip physical drive access for demo - go straight to cycle
...
@no_timeout:
    ...
    ; Print current hottest temp every second (simulated via loop)
    inc qword ptr [cycleCount]
    mov rax, [cycleCount]
    and eax, 3Fh                ; Every 64 iterations
    jnz @skip_print
    
    lea rcx, [szDriveTemp]
    mov edx, lenDriveTemp
    call ConsolePrint
    
    mov ecx, r14d               ; Hottest temp
    lea rdx, [scratchBuf]
    mov [rsp+48h], rdx          ; Save buffer address on stack
    call IntToStr
    mov rcx, [rsp+48h]          ; Restore buffer address
    mov edx, eax                ; Length from IntToStr
    call ConsolePrint
    
    lea rcx, [szTempSuffix]
    mov edx, lenTempSuffix
    call ConsolePrint
