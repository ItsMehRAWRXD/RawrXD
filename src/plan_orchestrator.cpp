...
PlanningResult PlanOrchestrator::generatePlan(const QString& prompt, 
    ...
    for (int i = 0; i < tasksArray.size(); ++i) {
        ...
        if (!QFileInfo(task.filePath).isAbsolute()) {
            task.filePath = QDir(workspaceRoot).absoluteFilePath(task.filePath);
        }
        
        // Check if file exists (skip if not)
        if (!QFileInfo::exists(task.filePath)) {
            qWarning() << "[PlanOrchestrator] File does not exist, skipping:" << task.filePath;
            continue;
        }
