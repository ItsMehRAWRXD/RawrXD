...
Scheduler_WaitForTask PROC FRAME
    ...
    .endprolog
    mov r12, rcx                    ; task_id
    mov r13d, edx                   ; timeout_ms

    lea rbx, g_task_scheduler

    ; Linear search for task by ID in global history
    ; Walk global queue first, then check completed tasks
    mov rsi, [rbx].TASK_SCHEDULER.global_queue
...
search_workers:
    ; Check each worker's local queue and current_task
    xor edi, edi                    ; worker index

check_worker:
    cmp edi, [rbx].TASK_SCHEDULER.worker_count
    jge task_not_found
    ...
    mov rax, [rsi].WORKER_CONTEXT.current_task
    test rax, rax
    jz check_worker_queue
    cmp [rax].TASK_NODE.task_id, r12
    je found_task_direct

check_worker_queue:
    mov rax, [rsi].WORKER_CONTEXT.local_queue_head

scan_worker_q:
    test rax, rax
    jz next_worker_check
    cmp [rax].TASK_NODE.task_id, r12
    je found_task_wait
    mov rax, [rax].TASK_NODE.next
    jmp scan_worker_q
...
found_task_direct:
    mov rsi, rax                    ; RSI = task node

found_task_wait:
    ; RSI = TASK_NODE* for the matching task
    ; Check if already completed
    cmp [rsi].TASK_NODE.state, 2    ; Done?
    je task_completed

    ; If task has a completion event, wait on it
    mov rcx, [rsi].TASK_NODE.completion_event
    test rcx, rcx
    jnz wait_on_event

    ; No event — create one and attach
    xor ecx, ecx
    xor edx, edx                    ; Auto-reset
    xor r8d, r8d                    ; Not signaled
    xor r9d, r9d
    call CreateEventW
    mov [rsi].TASK_NODE.completion_event, rax
    mov rcx, rax

wait_on_event:
    mov edx, r13d                   ; timeout_ms
    call WaitForSingleObject

    cmp eax, 0                      ; WAIT_OBJECT_0
    jne wait_task_timeout

task_completed:
    ; Return task result
    mov rax, [rsi].TASK_NODE.result
    jmp wait_task_done

task_not_found:
wait_task_timeout:
    xor eax, eax                    ; Failure / timeout

wait_task_done:
    pop r14 r13 r12 rdi rsi rbx
    ret
...
detect_loop:
    ...
    ; Full periodic scan: iterate all resources for waiting tasks
    lea rcx, [r15].CONFLICT_DETECTOR.graph_lock
    call AcquireSRWLockShared

    xor r12d, r12d                  ; Resource index

scan_resources:
    cmp r12d, [r15].CONFLICT_DETECTOR.resource_count
    jge scan_done

    mov rsi, [r15].CONFLICT_DETECTOR.resources
    mov eax, r12d
    imul eax, SIZEOF RESOURCE_ENTRY
    lea rsi, [rsi + rax]

    ; Only check resources that have waiters
    cmp [rsi].RESOURCE_ENTRY.wait_count, 0
    je next_resource_scan

    ; Must have an owner to form a cycle
    mov rcx, [rsi].RESOURCE_ENTRY.owner_task
    test rcx, rcx
    jz next_resource_scan

    ; Release shared lock for CheckDeadlock (needs write path)
    push r12 rsi
    lea rcx, [r15].CONFLICT_DETECTOR.graph_lock
    call ReleaseSRWLockShared

    ; Run DFS cycle detection from this owner task
    mov rcx, [rsi].RESOURCE_ENTRY.owner_task
    call ConflictDetector_CheckDeadlock
    test eax, eax
    jz no_cycle_found
    ...
    ; Deadlock detected
    lock inc [r15].CONFLICT_DETECTOR.deadlocks_detected

    ; Call deadlock callback if registered
    mov rax, [r15].CONFLICT_DETECTOR.on_deadlock
    test rax, rax
    jz no_cycle_found
    mov ecx, r12d                   ; resource index
    mov rdx, [rsi].RESOURCE_ENTRY.owner_task
    call rax
...
recv_loop:
    ...
    ; Validate minimum packet size (32 bytes)
    cmp rbx, 32
    jb recv_error

    ; Extract sender node_id from packet offset 16
    mov r13d, DWORD PTR [rsp + 128 + 16]

    ; Get current timestamp
    call GetHighResTick
    mov r12, rax                    ; Current time

    ; Find node by node_id and update last_heartbeat
    xor ecx, ecx                    ; node index
    mov edi, [r15].HEARTBEAT_MONITOR.node_count

find_sender_node:
    cmp ecx, edi
    jge recv_node_not_found
    ...
    cmp [rsi].HEARTBEAT_NODE.node_id, r13d
    je recv_update_node

    inc ecx
    jmp find_sender_node

recv_update_node:
    ; Update last heartbeat timestamp
    mov [rsi].HEARTBEAT_NODE.last_heartbeat, r12

    ; Mark healthy if not failed
    cmp [rsi].HEARTBEAT_NODE.status, 3  ; Failed
    je @F
    mov [rsi].HEARTBEAT_NODE.status, 1  ; Healthy
@@:

    ; Increment version and consecutive_ok
    inc [rsi].HEARTBEAT_NODE.version
    inc [rsi].HEARTBEAT_NODE.consecutive_ok
    mov [rsi].HEARTBEAT_NODE.missed_beats, 0

    ; Extract sender status from packet offset 20
    mov eax, DWORD PTR [rsp + 128 + 20]
    mov [rsi].HEARTBEAT_NODE.remote_status, eax

recv_node_not_found:
    ; Update global stats
    lock inc [r15].HEARTBEAT_MONITOR.beats_received
    lock add [r15].HEARTBEAT_MONITOR.bytes_received, rbx
...
GPU_WaitForDMA PROC FRAME
    ...
    ; Convert timeout_ns to ms (/ 1000000)
    mov rax, r13
    xor edx, edx
    mov ecx, 1000000
    div rcx
    mov r14, rax                    ; timeout_ms
    test r14, r14
    jnz @F
    mov r14, 1                      ; minimum 1ms
@@:

    ; Read expected fence value
    mov rbx, [r12].QUAD_SLOT_FULL.dma_fence

wait_dma_loop:
    ; Check if fence is signaled (completed value >= expected)
    mov rax, [r12].QUAD_SLOT_FULL.dma_fence_completed
    cmp rax, rbx
    jge dma_complete
...
Tensor_QuantizedMatMul PROC FRAME
    ...
    ; AVX-512 tiled matmul: C[M,N] = A[M,K] * B[K,N]
    ; Outer loops: rows of A (M), columns of B (N)
    xor ecx, ecx                    ; i = 0 (row of A / row of C)
...
row_loop_512:
    ...
    push rcx
    xor edx, edx                    ; j = 0 (col of B / col of C)

col_loop_512:
    cmp edx, ebx                    ; j < N
    jge col_loop_512_done

    push rdx

    ; Accumulator for C[i,j] — 16-wide FP32
    vpxorq zmm0, zmm0, zmm0        ; acc = 0

    ; Inner loop: dot product over K dimension, 16 elements per iteration
    xor eax, eax                    ; k = 0
...
k_loop_512:
    ...
    imul r8d, esi                   ; i * K
    add r8d, eax                    ; i * K + k
    lea r9, [r12 + r8]             ; A is INT8 quantized
    ...
    push rdx
    add r10d, edx                   ; k * N + j
    lea r11, [r13 + r10]           ; B is INT8 quantized

    ; Determine elements to process this iteration
    mov r8d, esi
    sub r8d, eax                    ; remaining = K - k
    cmp r8d, 16
    jbe @F
    mov r8d, 16                     ; min(remaining, 16)
@@:

    ; Load A[i, k..k+15] — sign-extend INT8 to INT32
    vpmovsxbd zmm4, XMMWORD PTR [r9]
    vcvtdq2ps zmm4, zmm4            ; Convert INT32 -> FP32

    ; Load B[k..k+15, j] — sign-extend INT8 to INT32
    vpmovsxbd zmm5, XMMWORD PTR [r11]
    vcvtdq2ps zmm5, zmm5            ; Convert INT32 -> FP32
...
k_done_512:
    ; Horizontal sum of zmm0 to get scalar C[i,j]
    ...
    imul r8d, ebx                   ; i * N
    add r8d, edx                    ; i * N + j
    vmovss DWORD PTR [r14 + r8*4], xmm0
...
no_avx512:
    ; AVX2 fallback: scalar tiled matmul
    xor ecx, ecx                    ; i = 0
...
; =============================================================================
; HELPER PROCEDURES - PREVIOUSLY MISSING IMPLEMENTATIONS
; =============================================================================

; High-resolution timestamp via QueryPerformanceCounter
...
; RCX = ticks
TicksToMicroseconds PROC FRAME
    sub rsp, 16
    .endprolog
    mov r8, rcx                     ; Save ticks

    lea rcx, [rsp]
    call QueryPerformanceFrequency
    mov rcx, QWORD PTR [rsp]       ; freq

    ; us = ticks * 1000000 / freq
    mov rax, r8
    mov rdx, 1000000
    mul rdx                         ; RDX:RAX = ticks * 1000000
    div rcx                         ; RAX = microseconds

    add rsp, 16
    ret
TicksToMicroseconds ENDP
...
; CRC32 computation using hardware CRC32 instruction (SSE4.2)
...
crc_tail:
    ...
    mov rcx, r9
    and rcx, 7                      ; remaining = length & 7

crc_loop_1:
    test rcx, rcx
    jz crc_done
    crc32 eax, BYTE PTR [r8]
    inc r8
    dec rcx
    jmp crc_loop_1
...
; Allocate page-aligned DMA buffer via VirtualAlloc
; RCX = size in bytes
...
; Lock INFINITY stream status with exclusive SRW lock
; Assumes RBX = stream pointer (INFINITY_STREAM_FULL*)
...
; Unlock INFINITY stream status exclusive SRW lock
; Assumes RBX = stream pointer (INFINITY_STREAM_FULL*)
...
PUBLIC TicksToMilliseconds
PUBLIC CalculateCRC32
PUBLIC AllocateDMABuffer
PUBLIC Infinity_LockStatusExclusive
PUBLIC Infinity_UnlockStatusExclusive
