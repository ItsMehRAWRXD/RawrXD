...
#define CHECK_MGR() do { if (!s_manager) return ToolCallResult::Error("Collaboration manager not initialized", ToolOutcome::ExecutionError); } while(0)
...
ToolCallResult CollabToolHandlers::HostSession(const nlohmann::json& args) {
    ...
    if (s_manager->hostSession(port, name)) {
        ...
        return ToolCallResult::Ok("Live Share session hosted on port " + std::to_string(port) +
                                  ". Token: " + s_manager->getSessionToken(), meta);
    }
...
ToolCallResult CollabToolHandlers::ShareFile(const nlohmann::json& args) {
    ...
    if (path.empty()) return ToolCallResult::Error("path is required", ToolOutcome::ValidationFailed);
...
ToolCallResult CollabToolHandlers::SetPermission(const nlohmann::json& args) {
    ...
    if (userId.empty()) return ToolCallResult::Error("userId is required", ToolOutcome::ValidationFailed);
...
ToolCallResult CollabToolHandlers::SendChat(const nlohmann::json& args) {
    ...
    if (message.empty()) return ToolCallResult::Error("message is required", ToolOutcome::ValidationFailed);
...
ToolCallResult CollabToolHandlers::TerminalInput(const nlohmann::json& args) {
    ...
    if (id.empty()) return ToolCallResult::Error("terminalId is required", ToolOutcome::ValidationFailed);
...
ToolCallResult CollabToolHandlers::ReviewCode(const nlohmann::json& args) {
    ...
    if (file.empty()) return ToolCallResult::Error("filePath is required", ToolOutcome::ValidationFailed);
...
nlohmann::json CollabToolHandlers::GetAllSchemas() {
    ...
    schemas.push_back(tool("collab_host_session",
        "Start hosting a Live Share collaboration session",
        {{"type", "object"}, {"properties", {
            {"port", {{"type", "integer"}, {"description", "WebSocket port (default 9876)"}}},
            {"hostName", {{"type", "string"}, {"description", "Display name for the host"}}}
        }}}));

    schemas.push_back(tool("collab_join_session",
        "Join an existing Live Share session",
        {{"type", "object"}, {"properties", {
    ...
    schemas.push_back(tool("collab_leave_session",
        "Leave the current collaboration session",
        {{"type", "object"}, {"properties", {}}}));

    schemas.push_back(tool("collab_share_file",
        "Share a file for collaborative editing in the Live Share session",
        {{"type", "object"}, {"properties", {
    ...
    schemas.push_back(tool("collab_edit_file",
        "Make an edit to a shared file (CRDT-synchronized)",
        {{"type", "object"}, {"properties", {
    ...
    schemas.push_back(tool("collab_list_participants",
        "List all participants in the current session",
        {{"type", "object"}, {"properties", {}}}));

    schemas.push_back(tool("collab_send_chat",
        "Send a chat message to all session participants",
        {{"type", "object"}, {"properties", {
            {"message", {{"type", "string"}, {"description", "Chat message text"}}}
        }}, {"required", {"message"}}}));

    schemas.push_back(tool("collab_create_terminal",
        "Create a shared terminal visible to all participants",
        {{"type", "object"}, {"properties", {
    ...
    schemas.push_back(tool("collab_terminal_input",
        "Write input to a shared terminal",
        {{"type", "object"}, {"properties", {
    ...
    schemas.push_back(tool("collab_terminal_scrollback",
        "Get the scrollback buffer of a shared terminal",
        {{"type", "object"}, {"properties", {
            {"terminalId", {{"type", "string"}, {"description", "Terminal identifier"}}}
        }}, {"required", {"terminalId"}}}));

    schemas.push_back(tool("collab_list_terminals",
        "List all shared terminals in the session",
        {{"type", "object"}, {"properties", {}}}));

    schemas.push_back(tool("collab_pair_start",
        "Activate pair programming mode with AI assistance",
        {{"type", "object"}, {"properties", {}}}));

    schemas.push_back(tool("collab_pair_swap_roles",
        "Swap driver/navigator roles between two participants",
        {{"type", "object"}, {"properties", {
            {"userA", {{"type", "string"}, {"description", "First user ID"}}},
            {"userB", {{"type", "string"}, {"description", "Second user ID"}}}
        }}, {"required", {"userA", "userB"}}}));

    schemas.push_back(tool("collab_ai_suggest",
        "Request an AI pair programming suggestion for a user",
        {{"type", "object"}, {"properties", {
            {"userId", {{"type", "string"}, {"description", "Target user ID"}}}
        }}, {"required", {"userId"}}}));

    schemas.push_back(tool("collab_review_code",
        "AI-assisted code review at a specific location",
        {{"type", "object"}, {"properties", {
    ...
    schemas.push_back(tool("collab_kick",
        "Remove a participant from the session",
        {{"type", "object"}, {"properties", {
            {"userId", {{"type", "string"}, {"description", "User ID to remove"}}}
        }}, {"required", {"userId"}}}));

    schemas.push_back(tool("collab_status",
        "Get full collaboration diagnostics (session, terminals, pair programming)",
        {{"type", "object"}, {"properties", {}}}));
