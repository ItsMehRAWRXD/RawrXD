...
std::string ChatPanelIntegration::callGithubCopilotAPI(const std::string& message) {
    ...
    // Fallback: token check. GitHub does not expose a public REST API for Copilot Chat (only metrics/user management).
    // 100.1% parity: extension path above is full parity; when extension absent, user must use extension or local-agent.
    ...
    return "[GitHub Copilot] Token set. For chat, install the Copilot extension (Tools > Extensions) or switch to 'local-agent'. GitHub does not provide a public Chat REST API.";
}
...
std::string ChatPanelIntegration::callAmazonQAPI(const std::string& message) {
    ...
    // Fallback: credentials check. Direct Bedrock InvokeAgent (SigV4) is Phase 2 when extension absent.
    // 100.1% parity: extension path above is full parity; when extension absent, user must use extension or local-agent.
    ...
    (void)region;
    return "[Amazon Q] AWS credentials set. For chat, install the Amazon Q extension (Tools > Extensions) or switch to 'local-agent'. Direct Bedrock API (Phase 2).";
}
