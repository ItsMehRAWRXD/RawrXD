...
QVector<CoverageReport> CoverageAnalyzer::analyzeProjectCoverage(const QString& projectPath, const QString& testResultsDir)
{
    ...
    QDirIterator dirIt(projectPath, filters, QDir::Files, QDirIterator::Subdirectories);
    while (dirIt.hasNext()) {
        QString filePath = dirIt.next();
        CoverageReport report = analyzeCoverage(filePath, testResultsDir);
...
int TestCoordinator::generateAllTests()
{
    ...
    QDirIterator dirIt(m_projectPath, filters, QDir::Files, QDirIterator::Subdirectories);
    
    while (dirIt.hasNext()) {
        QString filePath = dirIt.next();
        auto unitTests = m_generator->generateUnitTests(filePath);
...
#include "moc_test_generation_engine.cpp"
