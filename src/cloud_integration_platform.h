...
public:
    ...
    void addProvider(CloudProviderType provider, const CloudConfig& config);

    // Multi-Cloud Operations
    bool deployMultiCloud(const DeploymentConfig& config, const QVector<CloudProviderType>& providers);
...
signals:
    ...
    void deploymentSucceeded(QString applicationName, CloudProviderType provider);
    void deploymentFailed(QString applicationName, CloudProviderType provider, QString error);
    void failoverTriggered(QString applicationName);
    void metricsCollected(QJsonObject aggregatedMetrics);

private:
    QMap<CloudProviderType, std::unique_ptr<QObject>> m_providers;
    ...
    CloudProviderType m_activeProvider;
};
...
class CloudUtils {
public:
    static QString getProviderString(CloudProviderType provider);
    static CloudProviderType getProviderFromString(const QString& providerString);
    static QString getRegionFromLatLong(double lat, double lon);
    static QVector<QString> getAvailableRegions(CloudProviderType provider);
    static QString selectOptimalRegion(CloudProviderType provider, const QVector<QString>& requirements);
};
