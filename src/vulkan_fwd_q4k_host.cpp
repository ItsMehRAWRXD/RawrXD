// vulkan_fwd_q4k_host.cpp — host-IO packed Q4_K GEMV (logits / cert)
#ifdef _WIN32
#include <windows.h>
#endif
...
bool VulkanCompute::DispatchGEMVPacked(const void* packed, size_t bytes,
                                       ...
                                       uint64_t pinKey) {
    if (!EnsureQ4kPipeline() || !packed || !input || !output || bytes == 0)
        return false;
    ++gemv_attempts_;
    ...
    if (reuseIn) {
        ...
        void* mappedUp = nullptr;
        vkMapMemory(device_, gemv_in_mem_, 0, inB, 0, &mappedUp);
        std::memcpy(mappedUp, input, inB);
    ...
    if (!BindGemvStorage(wbuf, bytes, gemv_in_buf_, inB, gemv_out_buf_, outB,
                         q4k_pipe_, rows, cols, (rows + 63u) / 64u))
        return false;
    void* mapped = nullptr;
