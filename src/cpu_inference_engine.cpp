...
static void DequantizeTensor(const std::vector<uint8_t>& raw, float* out, size_t count, TensorType type) {
    DequantizeTensorPtr(raw.data(), raw.size(), out, count, type);
}
...
bool CPUInferenceEngine::loadModel(const std::string& path) { return LoadModel(path); }
...
void CPUInferenceEngine::InitKVCache() {
    if (m_kv_cache.size() == m_numLayers) return; 
    
    m_kv_cache.resize(m_numLayers);
    size_t layer_size = (size_t)m_maxSeqLen * m_embeddingDim;
    for (auto& layer : m_kv_cache) {
        layer.keys.resize(layer_size);
        layer.values.resize(layer_size);
...
void CPUInferenceEngine::FeedForward(const float* input, float* output, int layer_idx) {
    ...
    auto LoadLoad = [&](const std::string& suffix, std::vector<float>& w_out, TensorType& type_out) -> bool {
         ...
         if (!m_loader->GetTensorData(name, raw)) return false;
...
void CPUInferenceEngine::MultiHeadAttention(const float* query, const float* key, const float* value,
    ...
    auto Project = [&](const std::string& suffix, float* out_buf) {
        std::string name = prefix + suffix;
        std::vector<uint8_t> raw;
        if (m_loader->GetTensorData(name, raw)) {
...
void CPUInferenceEngine::GenerateStreaming(const std::vector<int32_t>& input_tokens,
    ...
    for (int step = 0; step < max_tokens; ++step) {
        ...
        {
             std::vector<uint8_t> raw;
             if (m_loader->GetTensorData("output_norm.weight", raw)) {
...
std::string CPUInferenceEngine::infer(const std::string& prompt, int max_tokens) {
    ...
    std::promise<void> done;
    auto future = done.get_future();
    // Use overloaded max_tokens
    GenerateStreaming(tokens, max_tokens, [&](const std::string& s) { result += s; }, [&]() { done.set_value(); });
    future.wait();
    return result;
}
