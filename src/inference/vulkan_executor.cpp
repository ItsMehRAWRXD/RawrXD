...
bool VulkanExecutor::ExecuteRMSNormFP16(const std::vector<float>& input, std::vector<float>& output,
    ...
    std::cout << "[VulkanExecutor] RMSNorm: size=" << size << ", eps=" << eps << "\n";
    ...
    if (it == pipelines_.end()) {
        std::cerr << "[VulkanExecutor] RMSNorm pipeline not found\n";
        return false;
    }
    ...
    // Download result
    std::vector<uint16_t> output_fp16(size);
    ...
    std::cout << "[VulkanExecutor] RMSNorm complete\n";
    return true;
}

bool VulkanExecutor::ExecuteSoftmaxFP16(const std::vector<float>& input, std::vector<float>& output,
    ...
    std::cout << "[VulkanExecutor] Softmax: rows=" << rows << ", cols=" << cols << "\n";
    ...
    if (it == pipelines_.end()) {
        std::cerr << "[VulkanExecutor] Softmax pipeline not found\n";
        return false;
    }
    ...
    std::cout << "[VulkanExecutor] Softmax complete\n";
    return true;
}
...
bool VulkanExecutor::LoadShaders() {
    ...
    // Load RMSNorm shader for transformer normalization layers
    if (!CreateComputePipeline("rms_norm_fp16", RMS_NORM_FP16_SPV, RMS_NORM_FP16_SPV_SIZE * sizeof(uint32_t))) {
        std::cerr << "[VulkanExecutor] Failed to load rms_norm_fp16\n";
        return false;
    }
    std::cout << "[VulkanExecutor] Loaded rms_norm_fp16\n";
    
    // Load Softmax shader for attention layers
    if (!CreateComputePipeline("softmax_fp16", SOFTMAX_FP16_SPV, SOFTMAX_FP16_SPV_SIZE * sizeof(uint32_t))) {
        std::cerr << "[VulkanExecutor] Failed to load softmax_fp16\n";
        return false;
    }
    std::cout << "[VulkanExecutor] Loaded softmax_fp16\n";
