...
public:
    ...
    // PascalCase API (used by .cpp implementation)
    bool IsGGUFFile(const std::string& path);
    bool ContainsGGUF(const std::string& blob_path, uint64_t& offset);
    std::vector<GGUFBlobInfo> DetectAllGGUFBlobs(const std::string& directory);
...
public:
    ...
    // PascalCase API (used by .cpp implementation)
    bool ParseGGUFHeader(const std::string& blob_path, uint64_t offset, GGUFHeader& header);
    bool ExtractGGUFData(const std::string& blob_path, uint64_t offset, const std::string& output_path);
    std::vector<std::string> ExtractMetadataKeys(const std::string& blob_path, uint64_t offset);
...
public:
    ...
    // PascalCase API (used by .cpp implementation)
    std::string FindOllamaModelsDirectory();
    std::vector<OllamaModel> FindAllModels();
    std::vector<OllamaModel> FindModelsInDirectory(const std::string& directory);
...
public:
    ...
    OllamaBlobStreamAdapter(const std::string& blob_path, uint64_t offset);
