...
static bool emit_vmovups_load(std::vector<uint8_t>& out, uint8_t dstZmm, uint8_t baseReg)
{
    ...
    const uint8_t bytes[] = {0x62, 0xF1, 0x7C, 0x48, 0x10, make_modrm(0, dstZmm, baseReg)};
...
static bool emit_vmovups_store(std::vector<uint8_t>& out, uint8_t baseReg, uint8_t srcZmm)
{
    ...
    const uint8_t bytes[] = {0x62, 0xF1, 0x7C, 0x48, 0x11, make_modrm(0, srcZmm, baseReg)};
    out.insert(out.end(), bytes, bytes + sizeof(bytes));
    return true;
}
...
static bool emit_vfmadd231ps(std::vector<uint8_t>& out, uint8_t dstZmm, uint8_t src1Zmm, uint8_t memBaseReg)
{
    ...
    const uint8_t bytes[] = {0x62, 0xF2, p1, 0x48, 0xB8, make_modrm(0, dstZmm, memBaseReg)};
    out.insert(out.end(), bytes, bytes + sizeof(bytes));
    return true;
}
...
static bool build_avx512_fma_probe_kernel(std::vector<uint8_t>& out)
{
    ...
    if (!emit_vmovups_load(out, 0, 1))
        return false;  // zmm0 <- [rcx]
    if (!emit_vmovups_load(out, 1, 2))
        return false;  // zmm1 <- [rdx]
    if (!emit_vfmadd231ps(out, 0, 1, 2))
        return false;  // zmm0 += zmm1 * [rdx]
    if (!emit_vmovups_store(out, 1, 0))
        return false;  // [rcx] <- zmm0
    emit_vzeroupper(out);
    emit_ret(out);
    return true;
}
...
// Accelerator table lives in Win32IDE_Accelerators.cpp (TranslateAccelerator / production message loop).
// Do not duplicate createAcceleratorTable() here — it caused confusion and risks ODR violations if this TU is linked.
    ...
    std::thread(
        [this]()
        {
            if (m_toolchain->discoverToolchain())
            {
                ...
                {
                    SendMessageA(m_hwndStatusBar, SB_SETTEXTA, 0, (LPARAM) "[+] Toolchain: VS2022 ready");
                }
            }
            else
            {
                if (IsWindow(m_hwndMain))
                {
                    SendMessageA(m_hwndStatusBar, SB_SETTEXTA, 0, (LPARAM) "[!] Toolchain: Not found");
                }
            }
        })
        .detach();
...
void Win32IDE::onBuildProject()
{
    ...
    m_toolchain->buildProject(
        ...
        [this](RawrXD::BuildPhase phase, const std::string& detail)
        {
            ...
            switch (phase)
            {
                case RawrXD::BuildPhase::Discovering:
                    status = "Detecting toolchain...";
                    break;
                case RawrXD::BuildPhase::Assembling:
                    status = "MASM: " + detail;
                    break;
                case RawrXD::BuildPhase::Compiling:
                    status = "C++: " + detail;
                    break;
                case RawrXD::BuildPhase::Linking:
                    status = "Link: " + detail;
                    break;
                case RawrXD::BuildPhase::Done:
                    status = "Build OK: " + detail;
                    break;
                case RawrXD::BuildPhase::Failed:
                    status = "BUILD FAILED: " + detail;
                    break;
                default:
                    status = detail;
                    break;
        ...
        [this](RawrXD::BuildPhase phase, const std::string& detail)
        {
            if (IsWindow(m_hwndMain))
            {
                std::string s = (phase == RawrXD::BuildPhase::Done)     ? "Rebuild OK"
                                : (phase == RawrXD::BuildPhase::Failed) ? "REBUILD FAILED"
                                                                        : detail;
...
void Win32IDE::onBuildRun()
{
    ...
    if (requestJitProbe)
    {
        ...
        {
            appendToOutput("[JIT] In-process AVX-512 kernel executed (no .exe emitted). out[0]=" + std::to_string(out0),
                           "Build", OutputSeverity::Info);
            if (IsWindow(m_hwndStatusBar))
            {
                SendMessageA(m_hwndStatusBar, SB_SETTEXTA, 0, (LPARAM) "JIT probe OK: in-process AVX-512 execution");
            }
        }
        else
        {
            appendToOutput("[JIT] In-process AVX-512 probe unavailable: " + reason, "Build", OutputSeverity::Warning);
            if (IsWindow(m_hwndStatusBar))
            {
                SendMessageA(m_hwndStatusBar, SB_SETTEXTA, 0, (LPARAM) "JIT probe skipped (see output)");
            }
        }
        return;
    }
...
void Win32IDE::onBuildAsmCurrent()
{
    ...
    if (ext != ".asm")
    {
        ...
        if (r.success)
        {
            appendToOutput("[OK] Compiled in " + std::to_string((int)r.elapsedMs) + "ms", "Build",
                           OutputSeverity::Info);
        }
        else
        {
...
void Win32IDE::onBuildSetTarget()
{
    ...
    if (showBuildTargetInputDialog(prompt.c_str(), newTarget, sizeof(newTarget)))
    {
        ...
        if (!targetStr.empty() && targetStr != m_toolchain->getTarget())
        {
            ...
            SendMessageA(m_hwndStatusBar, SB_SETTEXTA, 0, (LPARAM)(std::string("Target: ") + targetStr).c_str());
        }
    }
}
...
void Win32IDE::onBuildSetConfig(RawrXD::BuildConfig config)
{
    ...
    SendMessageA(m_hwndStatusBar, SB_SETTEXTA, 0, (LPARAM)(std::string("Config: ") + name).c_str());
}
...
bool Win32IDE::showBuildTargetInputDialog(const char* prompt, char* buffer, size_t bufferSize)
{
    ...
    HWND hwndDialog = CreateWindowExA(WS_EX_DLGMODALFRAME, "STATIC", "Set Build Target",
                                      WS_POPUP | WS_CAPTION | WS_SYSMENU | WS_VISIBLE, 200, 200, 400, 150, m_hwndMain,
    ...
    // Add prompt text
    CreateWindowExA(0, "STATIC", prompt, WS_CHILD | WS_VISIBLE, 10, 10, 380, 40, hwndDialog, nullptr, m_hInstance,
                    nullptr);

    // Add edit control
    HWND hwndEdit = CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", buffer, WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL, 10, 55,
                                    380, 25, hwndDialog, nullptr, m_hInstance, nullptr);

    // Add buttons
    HWND hwndOK = CreateWindowExA(0, "BUTTON", "OK", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON, 200, 90, 80, 30,
                                  hwndDialog, (HMENU)IDOK, m_hInstance, nullptr);
    HWND hwndCancel = CreateWindowExA(0, "BUTTON", "Cancel", WS_CHILD | WS_VISIBLE, 290, 90, 80, 30, hwndDialog,
                                      (HMENU)IDCANCEL, m_hInstance, nullptr);
