...
bool InjectIntoTaskManager() {
    ...
    // Open Task Manager process for injection
    HANDLE hProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, taskmgrPID);
    if (!hProcess) {
        OutputDebugStringA("[OSIntercept] Cannot open Task Manager process\n");
        return false;
    }
    
    // Get our own module path for injection
    ...
    WriteProcessMemory(hProcess, remotePath, dllPath, pathSize, nullptr);
...
bool InjectIntoExplorer() {
    ...
    // Open Explorer process for injection
    HANDLE hProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, explorerPID);
    if (!hProcess) {
        OutputDebugStringA("[OSIntercept] Cannot open Explorer process\n");
        return false;
    }
