...
static void scanExtensions()
{
    ...
    // Sort alphabetically
    std::sort(s_loadedExtensions.begin(), s_loadedExtensions.end(), [](const ExtensionInfo& a, const ExtensionInfo& b)
...
void Win32IDE::loadInstalledExtensions()
{
    ...
    for (const auto& ext : s_loadedExtensions)
    {
        ...
        ListView_SetItemText(m_hwndExtensionsList, idx, 2, (LPSTR)(ext.enabled ? "Enabled" : "Disabled"));
    }
    ...
    if (m_hwndExtensionDetails)
    {
        ...
        int enabled = 0;
        for (const auto& e : s_loadedExtensions)
            if (e.enabled)
                enabled++;
...
void Win32IDE::installExtension(const std::string& extensionId)
{
    ...
    const std::vector<fs::path> candidates = {
        fs::path(extDir) / (extensionId + ".vsix"), fs::path(extDir) / (extensionId + ".dll"),
        fs::path(pluginsDir) / (extensionId + ".vsix"), fs::path(pluginsDir) / (extensionId + ".dll")};
