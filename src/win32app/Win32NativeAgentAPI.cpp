...
bool MemoryManager::DestroyHeap(HANDLE hHeap) {
    if (!hHeap || hHeap == GetProcessHeap()) {
        return false; // Cannot destroy default process heap
    }
    std::lock_guard<std::mutex> lock(m_mutex);
    BOOL result = ::HeapDestroy(hHeap);
    if (result) {
        auto it = std::find(m_heaps.begin(), m_heaps.end(), hHeap);
        if (it != m_heaps.end()) {
            m_heaps.erase(it);
        }
    }
...
bool MemoryManager::UnmapViewOfFile(void* address) {
    if (!address) {
        return false;
    }
    std::lock_guard<std::mutex> lock(m_mutex);
    BOOL result = ::UnmapViewOfFile(address);
    if (result) {
        auto it = std::find(m_mappedViews.begin(), m_mappedViews.end(), address);
        if (it != m_mappedViews.end()) {
            m_mappedViews.erase(it);
        }
    }
    return result == TRUE;
}
