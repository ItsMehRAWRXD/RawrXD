...
static void buildFeatureText(std::wstringstream& ss) {
    ...
    uint64_t trialRemainingSec = lic.GetTrialRemainingSeconds();
    if (trialRemainingSec > 0) {
        uint64_t trialDays = (trialRemainingSec + 86399ULL) / 86400ULL;
        ss << L"Trial remaining: " << trialDays << L" days\r\n";
    }

    std::string storedPath = lic.GetStoredLicensePath();
    if (!storedPath.empty()) {
        ss << L"Last license path: " << utf8ToWide(storedPath) << L"\r\n";
    }

    std::string aadStatus = lic.IsAzureADConfigured() ? "configured" : "not configured";
    std::string aadAuthority = lic.GetAzureADAuthority();
    ss << L"Azure AD: " << utf8ToWide(aadStatus);
    if (!aadAuthority.empty() && lic.IsAzureADConfigured()) {
        ss << L" (" << utf8ToWide(aadAuthority) << L")";
    }
    ss << L"\r\n\r\n";
        ...
        case WM_CREATE: {
            ...
            if (ide) {
                if (!ide->m_featureRegistryPanel) {
                    ...
                    FeatureRegistryPanel::setConsoleOutputCallback(
                        [ide](const std::string& s) {
                            ide->appendToOutput(s, "FeatureRegistry", OutputSeverity::Info);
                        });
                }
