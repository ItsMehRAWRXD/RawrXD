...
static std::string dispatchTool(const ToolCall& tc, IOutputSink* sink, bool allowShell) {
    // Gate: RAWRXD_AGENT_TOOL_AUTHORITY_001 — Headless surface.
    // Route ALL tool calls through the single AgentToolAuthority instead
    // of the local toolReadFile/toolWriteFile/toolRunShell/toolListDir
    // implementations. This ensures the Headless reach counter is
    // incremented and the gate can verify authority.
    using RawrXD::Agent::AgentToolAuthority;
    using RawrXD::Agent::AgentToolRequest;
    using RawrXD::Agent::AgentToolSurface;

    // Map the workflow tool names to registry tool names
    std::string registryName = tc.name;
    json args;
    if (tc.name == "read_file") {
        args["path"] = extractJsonField(tc.args, "path");
    } else if (tc.name == "write_file") {
        args["path"] = extractJsonField(tc.args, "path");
        args["content"] = extractJsonField(tc.args, "content");
    } else if (tc.name == "run_shell") {
        if (!allowShell) return "ERROR: shell execution disabled by policy";
        registryName = "execute_command";
        args["command"] = extractJsonField(tc.args, "cmd");
    } else if (tc.name == "list_dir") {
        registryName = "list_directory";
        args["path"] = extractJsonField(tc.args, "path");
    } else {
    ...
    req.tool_name = registryName;
    req.args = args;
    req.caller_surface = "headless";

    auto result = AgentToolAuthority::Dispatch(req);
    if (sink) {
        std::string dbg = "[TOOL:" + registryName + "] " +
            (result.success ? "OK" : "FAIL");
        sink->appendOutput(dbg.c_str(), OutputSeverity::Debug);
    }
