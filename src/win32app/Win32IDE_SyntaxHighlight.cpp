...
COLORREF Win32IDE::getTokenColor(TokenType type) const {
    // Per-language palette override: check if the current theme defines
    // a language-specific palette for the active syntax language.
    // Non-zero fields in the palette override the theme-global color.
    auto langIt = m_currentTheme.languagePalettes.find((int)m_syntaxLanguage);
    if (langIt != m_currentTheme.languagePalettes.end()) {
        const auto& lp = langIt->second;
        COLORREF langColor = 0;
        switch (type) {
            case TokenType::Keyword:      langColor = lp.keywordColor;      break;
            case TokenType::BuiltinType:  langColor = lp.typeColor;         break;
            case TokenType::String:       langColor = lp.stringColor;       break;
            case TokenType::Comment:      langColor = lp.commentColor;      break;
            case TokenType::Number:       langColor = lp.numberColor;       break;
            case TokenType::Preprocessor: langColor = lp.preprocessorColor; break;
            case TokenType::Operator:     langColor = lp.operatorColor;     break;
            case TokenType::Function:     langColor = lp.functionColor;     break;
            case TokenType::Bracket:      langColor = lp.bracketColor;      break;
            case TokenType::Default:      langColor = lp.textColor;         break;
            default: break;
        }
        if (langColor != 0) return langColor;
        // Fall through to theme-global palette when the language palette
        // field is zero (unset).
    }

    // Theme-global palette with hardcoded Dark+ fallback for zero/uninitialized fields
    switch (type) {
...
void Win32IDE::applySyntaxColoring() {
    ...
    // Tokenize all visible lines and batch tokens by color to reduce
    // CHARFORMAT2A reinitializations.  Each unique color gets one cf struct
    // initialization, then all tokens of that color are applied sequentially.
    // This reduces the per-token overhead from ~120 bytes of struct init +
    // 2 SendMessage calls to just the 2 SendMessage calls.

    // Phase 1: collect all tokens for the visible range
    struct BatchToken {
        int start;
        int end;
        COLORREF color;
    };
    std::vector<BatchToken> allTokens;
    allTokens.reserve(256); // Pre-allocate for typical visible range
    ...
    for (int lineIdx = rangeStart; lineIdx <= rangeEnd; lineIdx++) {
        ...
        for (const auto& tok : lineTokens) {
            ...
            if (tokEnd > textLen) tokEnd = textLen;
            allTokens.push_back({tok.start, tokEnd, getTokenColor(tok.type)});
        }
    }

    // Phase 2: sort tokens by color so we can reuse the CHARFORMAT2A struct
    // across all tokens sharing the same color (avoids re-init per token)
    std::sort(allTokens.begin(), allTokens.end(),
              [](const BatchToken& a, const BatchToken& b) {
                  return a.color < b.color;
              });

    // Phase 3: apply batched — one CHARFORMAT2A per color group
    COLORREF currentBatchColor = (COLORREF)-1;
    CHARFORMAT2A cf = {};
    cf.cbSize = sizeof(cf);
    cf.dwMask = CFM_COLOR;
    cf.dwEffects = 0;

    for (const auto& bt : allTokens) {
        if (bt.color != currentBatchColor) {
            currentBatchColor = bt.color;
            cf.crTextColor = currentBatchColor;
        }
        CHARRANGE cr;
        cr.cpMin = bt.start;
        cr.cpMax = bt.end;
