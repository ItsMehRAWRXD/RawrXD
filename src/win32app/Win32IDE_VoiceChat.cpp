...
void Win32IDE::initVoiceChat()
{
    ...
    OutputDebugStringA("[INFO] Voice Chat subsystem initialized\n");
}
...
void Win32IDE::shutdownVoiceChat()
{
    ...
    OutputDebugStringA("[INFO] Voice Chat subsystem shut down\n");
}
...
void Win32IDE::createVoiceChatPanel(HWND hwndParent)
{
    ...
    OutputDebugStringA("[INFO] Voice Chat panel created\n");
}
...
void Win32IDE::cmdVoiceSpeak()
{
    ...
    if (m_hwndEditor) {
        ...
        if (end > start && (end - start) < sizeof(buf)) {
            // Get text from editor and extract selection
            int len = GetWindowTextLengthA(m_hwndEditor);
            if (len > 0 && len < 65536) {
                std::vector<char> allText(len + 1, 0);
                GetWindowTextA(m_hwndEditor, allText.data(), len + 1);
                size_t selLen = end - start;
                if (selLen < sizeof(buf) && start < static_cast<DWORD>(len)) {
                    memcpy(buf, allText.data() + start, selLen);
                    buf[selLen] = '\0';
                }
            }
        }
...
void Win32IDE::cmdVoiceShowMetrics()
{
    ...
    snprintf(buf, sizeof(buf),
        ...
        (long long)m.recordingCount, (long long)m.playbackCount,
        (long long)m.transcriptionCount, (long long)m.ttsCount,
        (long long)m.errorCount, (long long)m.bytesRecorded,
        (long long)m.bytesPlayed, (long long)m.vadSpeechEvents,
        (long long)m.relayMessagesSent, (long long)m.relayMessagesRecv,
...
// ============================================================================
// Phase 34: Voice UX Polish — Global Hotkeys
// ============================================================================
// Hotkey IDs (application-scope via RegisterHotKey)
#define VOICE_HOTKEY_TOGGLE_PTT    0xA001
#define VOICE_HOTKEY_TOGGLE_PANEL  0xA002
#define VOICE_HOTKEY_STOP          0xA003
...
// ============================================================================
// Phase 34: Voice UX Polish — Status Bar Integration
// ============================================================================
void Win32IDE::updateVoiceStatusBar()
{
...
// ============================================================================
// Phase 34: Voice UX Polish — Preferences Persistence
// ============================================================================
// Preferences stored in: %LOCALAPPDATA%\RawrXD\voice_prefs.json
// Format: compact JSON — no Qt, no external dependencies
// Fields: inputDevice, pushToTalk, autoTranscribe, sensitivity, locale

static std::string voicePrefsPath()
{
    ...
    if (SUCCEEDED(SHGetFolderPathA(nullptr, CSIDL_LOCAL_APPDATA, nullptr, 0, appData))) {
        ...
        return dir + "\\voice_prefs.json";
    }
    return "";
}

void Win32IDE::voiceSavePreferences()
{
    std::string path = voicePrefsPath();
    if (path.empty()) return;

    FILE* f = fopen(path.c_str(), "wb");
    if (!f) {
        OutputDebugStringA("[Voice UX] Failed to save preferences\n");
        return;
    }

    // Gather current settings from VoiceChat engine
    bool ptt = true;
    ...
    if (g_voiceChat) {
        ptt = (g_voiceChat->getMode() == VoiceChatMode::PushToTalk);
        VoiceChatConfig cfg = g_voiceChat->getConfig();
        autoTranscribe = !cfg.apiEndpoint.empty();  // transcription enabled if endpoint set
        sensitivity = cfg.vadThreshold;
#ifdef _WIN32
    ...
    fprintf(f, "{\n");
    fprintf(f, "  \"pushToTalk\": %s,\n", ptt ? "true" : "false");
    fprintf(f, "  \"autoTranscribe\": %s,\n", autoTranscribe ? "true" : "false");
    fprintf(f, "  \"sensitivity\": %.2f,\n", sensitivity);
    fprintf(f, "  \"inputDeviceId\": %d,\n", deviceId);
    fprintf(f, "  \"panelVisible\": %s\n", m_voicePanelVisible ? "true" : "false");
    fprintf(f, "}\n");
    fclose(f);

    OutputDebugStringA("[Voice UX] Preferences saved\n");
}

void Win32IDE::voiceLoadPreferences()
{
    std::string path = voicePrefsPath();
    if (path.empty()) return;

    FILE* f = fopen(path.c_str(), "rb");
    if (!f) return;  // No prefs yet — use defaults
    ...
    // Minimal JSON parsing (no Qt, no third-party)
    auto parseBool = [&](const char* key, bool def) -> bool {
    ...
    auto parseFloat = [&](const char* key, float def) -> float {
        const char* pos = strstr(buf, key);
        if (!pos) return def;
        pos += strlen(key);
        pos = strchr(pos, ':');
        ...
        while (*pos == ' ' || *pos == '\t') pos++;
        float val;
        if (sscanf(pos, "%f", &val) == 1) return val;
        return def;
    };

    auto parseInt = [&](const char* key, int def) -> int {
        const char* pos = strstr(buf, key);
        if (!pos) return def;
        pos += strlen(key);
        pos = strchr(pos, ':');
        ...
        while (*pos == ' ' || *pos == '\t') pos++;
        int val;
        if (sscanf(pos, "%d", &val) == 1) return val;
        return def;
    };

    bool ptt           = parseBool("\"pushToTalk\"", true);
    bool autoTranscribe = parseBool("\"autoTranscribe\"", true);
    float sensitivity  = parseFloat("\"sensitivity\"", 0.5f);
    int deviceId       = parseInt("\"inputDeviceId\"", 0);
    bool panelVisible  = parseBool("\"panelVisible\"", false);

    // Apply to VoiceChat engine
    if (g_voiceChat) {
        g_voiceChat->setMode(ptt ? VoiceChatMode::PushToTalk : VoiceChatMode::Continuous);
        VoiceChatConfig cfg = g_voiceChat->getConfig();
        cfg.vadThreshold = sensitivity;
        // autoTranscribe is an endpoint-level config — keep existing
        g_voiceChat->configure(cfg);
        g_voiceChat->selectInputDevice(deviceId);
    }

    m_voicePanelVisible = panelVisible;
    if (g_hwndVoicePanel) {
        ShowWindow(g_hwndVoicePanel, panelVisible ? SW_SHOW : SW_HIDE);
    }

    char msg[256];
    snprintf(msg, sizeof(msg),
        "[Voice UX] Preferences loaded: PTT=%d, autoTranscribe=%d, sensitivity=%.2f, device=%d",
        ptt, autoTranscribe, sensitivity, deviceId);
