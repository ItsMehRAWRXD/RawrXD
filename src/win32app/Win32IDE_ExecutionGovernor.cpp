...
// ── Local copies of utility functions (same as in Win32IDE_LocalServer.cpp) ──
...
void Win32IDE::cmdGovernorSubmitCommand() {
    ...
    if (safetyResult.verdict == ContractVerdict::Denied) {
        appendToOutput("[SAFETY DENIED] " + safetyResult.reason + "\n" + safetyResult.suggestion);
    ...
    if (confEval.decision == ConfidenceDecision::Abort) {
        appendToOutput("[CONFIDENCE ABORT] " + confEval.reason);
        return;
    }
    ...
    GovernorTaskId taskId = ExecutionGovernor::instance().submitCommand(
        ...
        [this, command](const GovernorCommandResult& result) {
            std::string msg = "Governor Task Complete:\n"
                              ...
                              "  Output:\n" + result.output;
            PostMessage(m_hwndMain, WM_USER + 200, 0, 0);
        });

    appendToOutput("[Governor] Task " + std::to_string(taskId) + " submitted: " + command);
...
void Win32IDE::cmdGovernorKillAll() {
    ...
    if (result == IDYES) {
        ...
        appendToOutput("[Governor] All tasks killed");
        ReplayJournal::instance().recordGovernorEvent(
...
void Win32IDE::cmdSafetyResetBudget() {
    ...
    if (result == IDYES) {
        ...
        appendToOutput("[Safety] Intent budgets reset to default");
        ReplayJournal::instance().recordSafetyEvent(
...
void Win32IDE::cmdSafetyRollbackLast() {
    ...
    if (success) {
        appendToOutput("[Safety] Last action rolled back successfully");
        ReplayJournal::instance().recordSafetyEvent(
            ReplayActionType::SafetyRollback, "Rollback last action");
    } else {
        appendToOutput("[Safety] No rollback available");
    }
}
...
void Win32IDE::cmdReplayExportSession() {
    ...
    if (sessionId == 0) {
        appendToOutput("[Replay] No active session to export");
        return;
    }
    ...
    if (success) {
        appendToOutput("[Replay] Session " + std::to_string(sessionId) +
                        " exported to " + filename);
    } else {
        appendToOutput("[Replay] Export failed — no records found");
    }
}

void Win32IDE::cmdReplayCheckpoint() {
    // Generate a timestamp-based checkpoint label
    SYSTEMTIME st;
    GetLocalTime(&st);
    char labelBuf[128];
    snprintf(labelBuf, sizeof(labelBuf), "checkpoint-%04d%02d%02d-%02d%02d%02d",
             st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
    std::string label(labelBuf);

    uint64_t seqId = ReplayJournal::instance().recordCheckpoint(label);
    appendToOutput("[Replay] Checkpoint '" + label + "' recorded (seq " +
                    std::to_string(seqId) + ")");
}
...
void Win32IDE::cmdConfidenceSetPolicy() {
    ...
    appendToOutput("[Confidence] Policy changed: " +
