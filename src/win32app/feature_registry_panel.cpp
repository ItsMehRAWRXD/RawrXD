...
void FeatureRegistryPanel::output(const std::string& s) {
    if (s_consoleOutputCb) s_consoleOutputCb(s);
    else std::cout << s;
}
...
// ============================================================================
// Console Output (for non-GUI / CLI contexts). Uses output() to route to IDE when set.
// ============================================================================
void FeatureRegistryPanel::printFeatureTable() {
    auto& lic = EnterpriseLicenseV2::Instance();
    std::ostringstream oss;
    oss << "\n" << std::left << std::setw(6) << "ID" << std::setw(40) << "Feature"
        << std::setw(15) << "Tier" << std::setw(8) << "Enabled" << std::setw(8) << "Impl"
        << std::setw(8) << "Wired" << std::setw(8) << "Tested" << "\n"
        << std::string(93, '-') << "\n";
    for (uint32_t i = 0; i < TOTAL_FEATURES; i++) {
        const FeatureDefV2& def = g_FeatureManifest[i];
        bool enabled = lic.isFeatureEnabled(def.id);
        oss << std::left << std::setw(6) << i << std::setw(40) << (def.name ? def.name : "?")
            << std::setw(15) << tierName(def.minTier) << std::setw(8) << (enabled ? "YES" : "no")
...
void FeatureRegistryPanel::printLicenseStatus() {
    ...
    oss << "  Tier:                " << tierName(lic.currentTier()) << "\n"
        << "  Enabled Features:    " << lic.enabledFeatureCount() << " / " << TOTAL_FEATURES << "\n"
        << "  Implemented:         " << lic.countImplemented() << " / " << TOTAL_FEATURES << "\n"
        << "  Wired to UI:         " << lic.countWiredToUI() << " / " << TOTAL_FEATURES << "\n"
        << "  Tested:              " << lic.countTested() << " / " << TOTAL_FEATURES << "\n"
        << "  Audit Entries:       " << lic.getAuditEntryCount() << " / " << lic.MAX_AUDIT_ENTRIES << "\n"
        << "  HWID:                " << hwid << "\n\n";
    output(oss.str());
}
...
void FeatureRegistryPanel::printAuditReport() {
    ...
    for (size_t i = count - show; i < count; i++) {
        ...
        oss << "  [" << std::setw(4) << i << "] " << (e.granted ? "ALLOW" : "DENY ")
...
void FeatureRegistryPanel::printEnforcementStatus() {
    ...
    output(len > 0 ? std::string(buf) + "\n" : "  (License enforcement not initialized — load license or start full IDE.)\n");
}
...
void FeatureRegistryPanel::printGapsReport() {
    ...
    for (uint32_t i = 0; i < TOTAL_FEATURES; i++) {
        const FeatureDefV2& def = g_FeatureManifest[i];
        if (!def.implemented) {
            oss << "    [X] " << (def.name ? def.name : "?")
                << "  (" << (def.sourceFile ? def.sourceFile : "?") << ")\n";
            notImpl++;
        }
    }
    if (notImpl == 0) oss << "    (none)\n";
    oss << "\n  NOT WIRED TO ENFORCEMENT:\n";
    for (uint32_t i = 0; i < TOTAL_FEATURES; i++) {
        const FeatureDefV2& def = g_FeatureManifest[i];
        if (def.implemented && !def.wiredToUI) {
            oss << "    [~] " << (def.name ? def.name : "?")
                << "  (" << (def.sourceFile ? def.sourceFile : "?") << ")\n";
            notWired++;
        }
    }
    if (notWired == 0) oss << "    (none)\n";
    oss << "\n  NOT TESTED:\n";
    for (uint32_t i = 0; i < TOTAL_FEATURES; i++) {
        const FeatureDefV2& def = g_FeatureManifest[i];
...
void FeatureRegistryPanel::printFullDashboard() {
    output("\n=================================================================\n"
           "         RawrXD Enterprise License Dashboard v2.0\n"
           "=================================================================\n\n");
    output("-----------------------------------------------------------------\n  LICENSE STATUS\n-----------------------------------------------------------------\n");
    printLicenseStatus();
    output("\n-----------------------------------------------------------------\n  FEATURE MATRIX\n-----------------------------------------------------------------\n");
    ...
    oss << "-----------------------------------------------------------------\n  RUNTIME FEATURE FLAGS\n-----------------------------------------------------------------\n"
        << "  Enabled: " << ff.enabledCount() << "  Overridden: " << ff.overriddenCount()
