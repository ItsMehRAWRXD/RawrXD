...
#define ID_BTN_SIGNIN_COPILOT     7406   // Opens GitHub Copilot sign-in (gets to login area)
#define ID_BTN_SIGNIN_AMAZONQ     7407   // Opens Amazon Q / AWS Toolkit login wizard
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            CreateWindowW(L"BUTTON", L"Install Amazon Q",
                          ...
                          10, 10, 130, 30, hwnd, (HMENU)ID_BTN_INSTALL_AMAZONQ,
                          GetModuleHandle(nullptr), nullptr);

            CreateWindowW(L"BUTTON", L"Sign In → Amazon Q",
                          WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                          150, 10, 140, 30, hwnd, (HMENU)ID_BTN_SIGNIN_AMAZONQ,
                          GetModuleHandle(nullptr), nullptr);

            CreateWindowW(L"BUTTON", L"Install GitHub Copilot",
                          WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                          300, 10, 160, 30, hwnd, (HMENU)ID_BTN_INSTALL_COPILOT,
                          GetModuleHandle(nullptr), nullptr);

            CreateWindowW(L"BUTTON", L"Sign In → Copilot",
                          WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                          470, 10, 130, 30, hwnd, (HMENU)ID_BTN_SIGNIN_COPILOT,
                          GetModuleHandle(nullptr), nullptr);

            CreateWindowW(L"BUTTON", L"Sync Marketplace",
                          WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                          610, 10, 130, 30, hwnd, (HMENU)ID_BTN_SYNC_MARKETPLACE,
                          GetModuleHandle(nullptr), nullptr);

            CreateWindowW(L"BUTTON", L"Select All",
                          WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                          750, 10, 90, 30, hwnd, (HMENU)ID_BTN_SELECT_ALL,
                          GetModuleHandle(nullptr), nullptr);

            CreateWindowW(L"BUTTON", L"Deselect All",
                          WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                          850, 10, 90, 30, hwnd, (HMENU)ID_BTN_DESELECT_ALL,
                          GetModuleHandle(nullptr), nullptr);

            CreateWindowW(L"BUTTON", L"Save Settings",
                          WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                          950, 10, 100, 30, hwnd, (HMENU)ID_BTN_SAVE_SETTINGS,
                          GetModuleHandle(nullptr), nullptr);
        ...
        case WM_COMMAND: {
            ...
            else if (wmId == ID_BTN_SIGNIN_AMAZONQ) {
                auto& api = vscode::VSCodeExtensionAPI::instance();
                if (api.isInitialized()) {
                    auto r = api.executeCommand("aws.toolkit.login", nullptr);
                    if (!r.success) {
                        r = api.executeCommand("aws.amazonq.configure", nullptr);
                    }
                    if (!r.success && r.detail && strlen(r.detail) > 0) {
                        MessageBoxA(hwnd, r.detail, "Sign In Amazon Q", MB_OK | MB_ICONWARNING);
                    }
                } else {
                    MessageBoxW(hwnd, L"Extension API not ready. Install Amazon Q first, then try again.", L"Sign In Amazon Q", MB_OK | MB_ICONINFORMATION);
                }
                return 0;
            }
            ...
            else if (wmId == ID_BTN_SIGNIN_COPILOT) {
                auto& api = vscode::VSCodeExtensionAPI::instance();
                if (api.isInitialized()) {
                    auto r = api.executeCommand("github.copilot.signIn", nullptr);
                    if (!r.success && r.detail && strlen(r.detail) > 0) {
                        MessageBoxA(hwnd, r.detail, "Sign In GitHub Copilot", MB_OK | MB_ICONWARNING);
                    }
                } else {
                    MessageBoxW(hwnd, L"Extension API not ready. Install GitHub Copilot first, then try again.", L"Sign In GitHub Copilot", MB_OK | MB_ICONINFORMATION);
                }
                return 0;
            }
