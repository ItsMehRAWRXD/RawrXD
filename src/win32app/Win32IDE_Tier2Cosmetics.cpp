...
void Win32IDE::addTerminalTab(int profileIndex) {
    ...
    // Git Bash and WSL use CommandPrompt pipe mode with custom executable
    if (profile.shellPath.find("bash") != std::string::npos ||
        profile.shellPath.find("wsl") != std::string::npos) {
        shellType = Win32TerminalManager::CommandPrompt;
    }
