...
bool Win32IDE::handleTier1Command(int commandId)
{
    ...
    if (commandId == IDM_T1_SMOOTH_SCROLL_TOGGLE)
    {
        ...
        appendToOutput(m_smoothScroll.enabled ? "[Tier1] Smooth scroll enabled\n" : "[Tier1] Smooth scroll disabled\n");
        return true;
    }
    // Minimap Enhanced (12010–12019)
    if (commandId == IDM_T1_MINIMAP_TOGGLE)
    {
        toggleMinimap();
        return true;
    }
    if (commandId == IDM_T1_MINIMAP_HIGHLIGHT)
    {
        m_minimapHighlightCursor = !m_minimapHighlightCursor;
        updateMinimap();
        return true;
    }
    ...
    if (commandId == IDM_T1_FUZZY_PALETTE)
    {
        showFuzzyCommandPalette();
        return true;
    }
    if (commandId == IDM_T1_FUZZY_FILES)
    {
        showFuzzyFileFinder();
        return true;
    }
    if (commandId == IDM_T1_FUZZY_SYMBOLS)
    {
        showFuzzySymbolSearch();
        return true;
    }

    // Settings GUI (12040–12049)
    if (commandId == IDM_T1_SETTINGS_GUI)
    {
        showSettingsGUIDialog();
        return true;
    }
    if (commandId == IDM_T1_SETTINGS_RESET)
    {
    ...
    // Welcome Page (12050–12059)
    if (commandId == IDM_T1_WELCOME_SHOW)
    {
        showWelcomePage();
        return true;
    }
    if (commandId == IDM_T1_WELCOME_CLONE)
    {
        handleWelcomeCloneRepo();
        return true;
    }
    if (commandId == IDM_T1_WELCOME_OPEN_FOLDER)
    {
        handleWelcomeOpenFolder();
        return true;
    }
    if (commandId == IDM_T1_WELCOME_NEW_FILE)
    {
        handleWelcomeNewFile();
        return true;
    }

    // File Icon Theme (12060–12069)
    if (commandId == IDM_T1_ICON_THEME_SET)
    {
        showFileIconThemeSelector();
        return true;
    }
    if (commandId == IDM_T1_ICON_THEME_SETI)
    {
        setFileIconTheme("seti");
        return true;
    }
    if (commandId == IDM_T1_ICON_THEME_MATERIAL)
    {
        setFileIconTheme("material");
        return true;
    }
    ...
    // Split Editor (12080–12089)
    if (commandId == IDM_T1_SPLIT_VERTICAL)
    {
        splitEditorVertical();
        return true;
    }
    if (commandId == IDM_T1_SPLIT_HORIZONTAL)
    {
        splitEditorHorizontal();
        return true;
    }
    if (commandId == IDM_T1_SPLIT_GRID_2X2)
    {
        splitEditorGrid2x2();
        return true;
    }
    if (commandId == IDM_T1_SPLIT_CLOSE)
    {
        closeSplitEditor();
        return true;
    }
    if (commandId == IDM_T1_SPLIT_FOCUS_NEXT)
    {
        focusNextSplitPane();
        return true;
    }
    ...
    // Auto-Update (12090–12099)
    if (commandId == IDM_T1_UPDATE_CHECK)
    {
        checkForUpdates();
        return true;
    }
    if (commandId == IDM_T1_UPDATE_INSTALL)
    {
        installUpdate();
        return true;
    }
    if (commandId == IDM_T1_UPDATE_DISMISS)
    {
        dismissUpdateNotification();
        return true;
    }
    if (commandId == IDM_T1_UPDATE_RELEASE_NOTES)
    {
        showReleaseNotes();
        return true;
    }
...
{
    if (!m_smoothScroll.enabled || !m_hwndEditor)
        return false;
...
void Win32IDE::initMinimapEnhanced()
{
    ...
    if (!m_hwndMinimap && m_hwndMain)
    {
        ...
        m_hwndMinimap =
            CreateWindowExA(0, "RawrXD_Minimap", "", WS_CHILD | (m_minimapVisible ? WS_VISIBLE : 0), 0, 0,
                            MINIMAP_ENHANCED_WIDTH, 400, m_hwndMain, (HMENU)(UINT_PTR)12011, m_hInstance, nullptr);
...
void Win32IDE::paintMinimapEnhanced(HDC hdc, const RECT& rect)
{
    ...
    float scale = (totalLines > maxVisibleLines) ? (float)totalLines / (float)maxVisibleLines : 1.0f;
    ...
    HFONT miniFont = CreateFontA(-2, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
...
LRESULT CALLBACK Tier1EnhancedMinimapWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        case WM_PAINT:
        {
            ...
            if (ide)
                ide->paintMinimapEnhanced(memDC, rc);
        ...
        case WM_MOUSEMOVE:
        {
            if (!(wParam & MK_LBUTTON) || !ide || !ide->m_hwndEditor)
                break;
...
// Full implementation now lives in Win32IDE_Breadcrumbs.cpp (integrated into build).
// Linker stub removed — see Win32IDE_Breadcrumbs.cpp::updateBreadcrumbsForCursor().
...
static int fuzzyScore(const char* query, const char* target, std::vector<int>& positions)
{
    ...
    for (int ti = 0; ti < tLen && qi < qLen; ti++)
    {
        ...
        prevWasSep = (target[ti] == ' ' || target[ti] == '.' || target[ti] == '_' || target[ti] == ':' ||
                      target[ti] == '/' || target[ti] == '-');
    }
...
void Win32IDE::showFuzzyPaletteWindow()
{
        ...
        wc.lpfnWndProc = [](HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) -> LRESULT
        {
            ...
            switch (msg)
            {
                case WM_CREATE:
                {
                    ...
                    // Search input
                    HWND input = CreateWindowExA(0, "EDIT", "", WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL | WS_BORDER, 8,
                                                 8, 580, 28, hwnd, (HMENU)1, nullptr, nullptr);
                    SetWindowLongPtr(input, GWLP_ID, 1);

                    // Set dark theme colors
                    HFONT font =
                        CreateFontA(-16, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                    ...
                    CreateWindowExA(0, "LISTBOX", "",
                                    WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOTIFY | LBS_NOINTEGRALHEIGHT, 8, 42, 580,
                                    350, hwnd, (HMENU)2, nullptr, nullptr);
                    ...
                    if (LOWORD(wParam) == 1 && HIWORD(wParam) == EN_CHANGE && ide)
                    {
                        ...
                        if (strlen(query) == 0)
                        {
                            ...
                            for (auto& cmd : ide->m_fuzzyCommandLabels)
                            {
                                ...
                                SendMessageA(list, LB_SETITEMDATA, SendMessageA(list, LB_GETCOUNT, 0, 0) - 1,
                                             cmd.commandId);
                            }
                        }
                        else
                        {
                            ...
                            {
                                char display[256];
                                snprintf(display, sizeof(display), "[%s] %s (score:%d)", m.category, m.label, m.score);
    ...
    HWND palette = CreateWindowExA(WS_EX_TOPMOST | WS_EX_TOOLWINDOW, "RawrXD_FuzzyPalette", "Command Palette",
                                   WS_POPUP | WS_BORDER, x, y, w, h, m_hwndMain, nullptr, m_hInstance, this);
...
static int createSettingRow(HWND parent, int y, const char* label, const char* ctrlClass, DWORD ctrlStyle, int ctrlId,
                            const char* value, int rowHeight = 28)
{
    CreateWindowExA(0, "STATIC", label, WS_CHILD | WS_VISIBLE | SS_RIGHT, 10, y + 4, 200, 20, parent, nullptr, nullptr,
                    nullptr);

    HWND ctrl = CreateWindowExA(0, ctrlClass, value ? value : "", WS_CHILD | WS_VISIBLE | ctrlStyle | WS_BORDER, 220, y,
                                250, rowHeight - 4, parent, (HMENU)(UINT_PTR)ctrlId, nullptr, nullptr);

    HFONT font = CreateFontA(-14, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                             CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH, "Segoe UI");
...
LRESULT CALLBACK Tier1LegacySettingsGUIProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        case WM_CREATE:
        {
            ...
            HFONT headerFont =
                CreateFontA(-18, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                            CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH, "Segoe UI");
            HFONT labelFont =
                CreateFontA(-14, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                            CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH, "Segoe UI");

            // Search bar at top
            HWND search = CreateWindowExA(0, "EDIT", "", WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL | WS_BORDER, 10, 10,
                                          460, 28, hwnd, (HMENU)SC_SEARCH, nullptr, nullptr);
            SendMessageA(search, WM_SETFONT, (WPARAM)labelFont, TRUE);
            SendMessageW(search, EM_SETCUEBANNER, TRUE, (LPARAM)L"Search settings...");

            // Scrollable area
            HWND scrollArea = CreateWindowExA(0, "STATIC", "", WS_CHILD | WS_VISIBLE | SS_OWNERDRAW, 0, 45, 700, 500,
            ...
            // ── General ──
            HWND secHdr = CreateWindowExA(0, "STATIC", "  General", WS_CHILD | WS_VISIBLE, 10, y, 460, 24, hwnd,
                                          nullptr, nullptr, nullptr);
            SendMessage(secHdr, WM_SETFONT, (WPARAM)headerFont, TRUE);
            y += 30;

            // AutoSave checkbox
            HWND chk = CreateWindowExA(0, "BUTTON", "Auto Save", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 220, y, 200,
                                       22, hwnd, (HMENU)SC_AUTOSAVE, nullptr, nullptr);
            ...
            chk = CreateWindowExA(0, "BUTTON", "Line Numbers", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 220, y, 200, 22,
                                  hwnd, (HMENU)SC_LINENUMBERS, nullptr, nullptr);
            SendMessage(chk, WM_SETFONT, (WPARAM)labelFont, TRUE);
            if (s_ide->m_settings.lineNumbersVisible)
                SendMessage(chk, BM_SETCHECK, BST_CHECKED, 0);
            y += 28;

            chk = CreateWindowExA(0, "BUTTON", "Word Wrap", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 220, y, 200, 22,
                                  hwnd, (HMENU)SC_WORDWRAP, nullptr, nullptr);
            SendMessage(chk, WM_SETFONT, (WPARAM)labelFont, TRUE);
            if (s_ide->m_settings.wordWrapEnabled)
                SendMessage(chk, BM_SETCHECK, BST_CHECKED, 0);
            y += 28;

            chk = CreateWindowExA(0, "BUTTON", "Minimap", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 220, y, 200, 22,
                                  hwnd, (HMENU)SC_MINIMAP, nullptr, nullptr);
            SendMessage(chk, WM_SETFONT, (WPARAM)labelFont, TRUE);
            if (s_ide->m_settings.minimapEnabled)
                SendMessage(chk, BM_SETCHECK, BST_CHECKED, 0);
            y += 28;

            chk = CreateWindowExA(0, "BUTTON", "Syntax Coloring", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 220, y, 200,
                                  22, hwnd, (HMENU)SC_SYNTAXCOLORING, nullptr, nullptr);
            SendMessage(chk, WM_SETFONT, (WPARAM)labelFont, TRUE);
            if (s_ide->m_settings.syntaxColoringEnabled)
                SendMessage(chk, BM_SETCHECK, BST_CHECKED, 0);
            y += 28;

            chk = CreateWindowExA(0, "BUTTON", "Ghost Text", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 220, y, 200, 22,
                                  hwnd, (HMENU)SC_GHOSTTEXT, nullptr, nullptr);
            SendMessage(chk, WM_SETFONT, (WPARAM)labelFont, TRUE);
            if (s_ide->m_settings.ghostTextEnabled)
                SendMessage(chk, BM_SETCHECK, BST_CHECKED, 0);
            y += 28;

            chk = CreateWindowExA(0, "BUTTON", "Breadcrumbs", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 220, y, 200, 22,
                                  hwnd, (HMENU)SC_BREADCRUMBS, nullptr, nullptr);
            SendMessage(chk, WM_SETFONT, (WPARAM)labelFont, TRUE);
            ...
            // ── Editor ──
            secHdr = CreateWindowExA(0, "STATIC", "  Editor", WS_CHILD | WS_VISIBLE, 10, y, 460, 24, hwnd, nullptr,
                                     nullptr, nullptr);
            SendMessage(secHdr, WM_SETFONT, (WPARAM)headerFont, TRUE);
            y += 30;

            // Font Size
            CreateWindowExA(0, "STATIC", "Font Size:", WS_CHILD | WS_VISIBLE | SS_RIGHT, 10, y + 4, 200, 20, hwnd,
                            nullptr, nullptr, nullptr);
            char buf[64];
            snprintf(buf, sizeof(buf), "%d", s_ide->m_settings.fontSize);
            HWND edit = CreateWindowExA(0, "EDIT", buf, WS_CHILD | WS_VISIBLE | ES_NUMBER | WS_BORDER, 220, y, 80, 24,
                                        hwnd, (HMENU)SC_FONTSIZE, nullptr, nullptr);
            SendMessage(edit, WM_SETFONT, (WPARAM)labelFont, TRUE);
            y += 32;

            // Font Name
            CreateWindowExA(0, "STATIC", "Font Name:", WS_CHILD | WS_VISIBLE | SS_RIGHT, 10, y + 4, 200, 20, hwnd,
                            nullptr, nullptr, nullptr);
            edit = CreateWindowExA(0, "EDIT", s_ide->m_settings.fontName.c_str(), WS_CHILD | WS_VISIBLE | WS_BORDER,
                                   220, y, 200, 24, hwnd, (HMENU)SC_FONTNAME, nullptr, nullptr);
            SendMessage(edit, WM_SETFONT, (WPARAM)labelFont, TRUE);
            y += 32;

            // Tab Size
            CreateWindowExA(0, "STATIC", "Tab Size:", WS_CHILD | WS_VISIBLE | SS_RIGHT, 10, y + 4, 200, 20, hwnd,
                            nullptr, nullptr, nullptr);
            snprintf(buf, sizeof(buf), "%d", s_ide->m_settings.tabSize);
            edit = CreateWindowExA(0, "EDIT", buf, WS_CHILD | WS_VISIBLE | ES_NUMBER | WS_BORDER, 220, y, 80, 24, hwnd,
                                   (HMENU)SC_TABSIZE, nullptr, nullptr);
            SendMessage(edit, WM_SETFONT, (WPARAM)labelFont, TRUE);

            chk = CreateWindowExA(0, "BUTTON", "Use Spaces", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 310, y + 2, 120,
                                  22, hwnd, (HMENU)SC_USESPACES, nullptr, nullptr);
            SendMessage(chk, WM_SETFONT, (WPARAM)labelFont, TRUE);
            ...
            // Encoding
            CreateWindowExA(0, "STATIC", "Encoding:", WS_CHILD | WS_VISIBLE | SS_RIGHT, 10, y + 4, 200, 20, hwnd,
                            nullptr, nullptr, nullptr);
            HWND combo = CreateWindowExA(0, "COMBOBOX", "", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL, 220,
                                         y, 200, 200, hwnd, (HMENU)SC_ENCODING, nullptr, nullptr);
            ...
            // EOL Style
            CreateWindowExA(0, "STATIC", "EOL Style:", WS_CHILD | WS_VISIBLE | SS_RIGHT, 10, y + 4, 200, 20, hwnd,
                            nullptr, nullptr, nullptr);
            combo = CreateWindowExA(0, "COMBOBOX", "", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST, 220, y, 200, 150, hwnd,
                                    (HMENU)SC_EOLSTYLE, nullptr, nullptr);
            SendMessage(combo, WM_SETFONT, (WPARAM)labelFont, TRUE);
            ...
            // ── AI Settings ──
            secHdr = CreateWindowExA(0, "STATIC", "  AI / Model", WS_CHILD | WS_VISIBLE, 10, y, 460, 24, hwnd, nullptr,
                                     nullptr, nullptr);
            SendMessage(secHdr, WM_SETFONT, (WPARAM)headerFont, TRUE);
            y += 30;

            CreateWindowExA(0, "STATIC", "Temperature:", WS_CHILD | WS_VISIBLE | SS_RIGHT, 10, y + 4, 200, 20, hwnd,
                            nullptr, nullptr, nullptr);
            snprintf(buf, sizeof(buf), "%.2f", s_ide->m_settings.aiTemperature);
            edit = CreateWindowExA(0, "EDIT", buf, WS_CHILD | WS_VISIBLE | WS_BORDER, 220, y, 80, 24, hwnd,
                                   (HMENU)SC_TEMPERATURE, nullptr, nullptr);
            SendMessage(edit, WM_SETFONT, (WPARAM)labelFont, TRUE);
            y += 32;

            CreateWindowExA(0, "STATIC", "Top P:", WS_CHILD | WS_VISIBLE | SS_RIGHT, 10, y + 4, 200, 20, hwnd, nullptr,
                            nullptr, nullptr);
            snprintf(buf, sizeof(buf), "%.2f", s_ide->m_settings.aiTopP);
            edit = CreateWindowExA(0, "EDIT", buf, WS_CHILD | WS_VISIBLE | WS_BORDER, 220, y, 80, 24, hwnd,
                                   (HMENU)SC_TOPP, nullptr, nullptr);
            SendMessage(edit, WM_SETFONT, (WPARAM)labelFont, TRUE);
            y += 32;

            CreateWindowExA(0, "STATIC", "Max Tokens:", WS_CHILD | WS_VISIBLE | SS_RIGHT, 10, y + 4, 200, 20, hwnd,
                            nullptr, nullptr, nullptr);
            snprintf(buf, sizeof(buf), "%d", s_ide->m_settings.aiMaxTokens);
            edit = CreateWindowExA(0, "EDIT", buf, WS_CHILD | WS_VISIBLE | ES_NUMBER | WS_BORDER, 220, y, 100, 24, hwnd,
                                   (HMENU)SC_MAXTOKENS, nullptr, nullptr);
            SendMessage(edit, WM_SETFONT, (WPARAM)labelFont, TRUE);
            y += 32;

            CreateWindowExA(0, "STATIC", "Ollama URL:", WS_CHILD | WS_VISIBLE | SS_RIGHT, 10, y + 4, 200, 20, hwnd,
                            nullptr, nullptr, nullptr);
            edit = CreateWindowExA(0, "EDIT", s_ide->m_settings.aiOllamaUrl.c_str(), WS_CHILD | WS_VISIBLE | WS_BORDER,
                                   220, y, 250, 24, hwnd, (HMENU)SC_OLLAMAURL, nullptr, nullptr);
            SendMessage(edit, WM_SETFONT, (WPARAM)labelFont, TRUE);
            y += 38;

            // ── Local Parity (no API key) ──
            secHdr = CreateWindowExA(0, "STATIC", "  Local Parity (no API key)", WS_CHILD | WS_VISIBLE, 10, y, 460, 24,
                                     hwnd, nullptr, nullptr, nullptr);
            SendMessage(secHdr, WM_SETFONT, (WPARAM)headerFont, TRUE);
            y += 30;

            chk = CreateWindowExA(0, "BUTTON", "Use local GGUF for completion/chat (no API key)",
                                  WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 220, y, 350, 22, hwnd,
                                  (HMENU)SC_LOCALPARITY_ENABLED, nullptr, nullptr);
            SendMessage(chk, WM_SETFONT, (WPARAM)labelFont, TRUE);
            if (s_ide->m_settings.localParityEnabled)
                SendMessage(chk, BM_SETCHECK, BST_CHECKED, 0);
            y += 28;

            CreateWindowExA(0, "STATIC", "Local model path (GGUF):", WS_CHILD | WS_VISIBLE | SS_RIGHT, 10, y + 4, 200,
                            20, hwnd, nullptr, nullptr, nullptr);
            edit = CreateWindowExA(0, "EDIT", s_ide->m_settings.localParityModelPath.c_str(),
                                   WS_CHILD | WS_VISIBLE | WS_BORDER, 220, y, 200, 24, hwnd,
                                   (HMENU)SC_LOCALPARITY_MODELPATH, nullptr, nullptr);
            SendMessage(edit, WM_SETFONT, (WPARAM)labelFont, TRUE);

            // Browse button
            HWND browseBtn = CreateWindowExA(0, "BUTTON", "...", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 425, y, 45, 24,
                                             hwnd, (HMENU)13070, nullptr, nullptr);
            ...
            CreateWindowExA(0, "STATIC", "Update manifest URL (public):", WS_CHILD | WS_VISIBLE | SS_RIGHT, 10, y + 4,
                            200, 20, hwnd, nullptr, nullptr, nullptr);
            edit = CreateWindowExA(0, "EDIT", s_ide->m_settings.updateManifestUrl.c_str(),
                                   WS_CHILD | WS_VISIBLE | WS_BORDER, 220, y, 250, 24, hwnd,
                                   (HMENU)SC_UPDATEMANIFEST_URL, nullptr, nullptr);
            SendMessage(edit, WM_SETFONT, (WPARAM)labelFont, TRUE);
            y += 40;

            // ── Buttons ──
            HWND btn = CreateWindowExA(0, "BUTTON", "Save", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 480, y, 90, 32, hwnd,
                                       (HMENU)SC_SAVE_BTN, nullptr, nullptr);
            SendMessage(btn, WM_SETFONT, (WPARAM)labelFont, TRUE);

            btn = CreateWindowExA(0, "BUTTON", "Cancel", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 580, y, 90, 32, hwnd,
                                  (HMENU)SC_CANCEL_BTN, nullptr, nullptr);
            SendMessage(btn, WM_SETFONT, (WPARAM)labelFont, TRUE);

            btn = CreateWindowExA(0, "BUTTON", "Reset Defaults", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 340, y, 130, 32,
                                  hwnd, (HMENU)SC_RESET_BTN, nullptr, nullptr);
            SendMessage(btn, WM_SETFONT, (WPARAM)labelFont, TRUE);
        ...
        case WM_COMMAND:
        {
            ...
            if (cmd == SC_SAVE_BTN)
            {
                ...
                s_ide->m_settings.autoSaveEnabled =
                    (SendMessageA(GetDlgItem(hwnd, SC_AUTOSAVE), BM_GETCHECK, 0, 0) == BST_CHECKED);
                s_ide->m_settings.lineNumbersVisible =
                    (SendMessageA(GetDlgItem(hwnd, SC_LINENUMBERS), BM_GETCHECK, 0, 0) == BST_CHECKED);
                s_ide->m_settings.wordWrapEnabled =
                    (SendMessageA(GetDlgItem(hwnd, SC_WORDWRAP), BM_GETCHECK, 0, 0) == BST_CHECKED);
                s_ide->m_settings.minimapEnabled =
                    (SendMessageA(GetDlgItem(hwnd, SC_MINIMAP), BM_GETCHECK, 0, 0) == BST_CHECKED);
                s_ide->m_settings.syntaxColoringEnabled =
                    (SendMessageA(GetDlgItem(hwnd, SC_SYNTAXCOLORING), BM_GETCHECK, 0, 0) == BST_CHECKED);
                s_ide->m_settings.ghostTextEnabled =
                    (SendMessageA(GetDlgItem(hwnd, SC_GHOSTTEXT), BM_GETCHECK, 0, 0) == BST_CHECKED);
                s_ide->m_settings.breadcrumbsEnabled =
                    (SendMessageA(GetDlgItem(hwnd, SC_BREADCRUMBS), BM_GETCHECK, 0, 0) == BST_CHECKED);
                s_ide->m_settings.smoothScrollEnabled =
                    (SendMessageA(GetDlgItem(hwnd, SC_SMOOTHSCROLL), BM_GETCHECK, 0, 0) == BST_CHECKED);
                s_ide->m_smoothScroll.enabled = s_ide->m_settings.smoothScrollEnabled;
                s_ide->m_settings.failureDetectorEnabled =
                    (SendMessageA(GetDlgItem(hwnd, SC_FAILUREDETECTOR), BM_GETCHECK, 0, 0) == BST_CHECKED);
                s_ide->m_settings.useSpaces =
                    (SendMessageA(GetDlgItem(hwnd, SC_USESPACES), BM_GETCHECK, 0, 0) == BST_CHECKED);

                char buf[256];
                GetWindowTextA(GetDlgItem(hwnd, SC_FONTSIZE), buf, sizeof(buf));
                s_ide->m_settings.fontSize = atoi(buf);
                if (s_ide->m_settings.fontSize < 6)
                    s_ide->m_settings.fontSize = 6;
                if (s_ide->m_settings.fontSize > 72)
                    s_ide->m_settings.fontSize = 72;
                ...
                s_ide->m_settings.localParityEnabled =
                    (SendMessageA(GetDlgItem(hwnd, SC_LOCALPARITY_ENABLED), BM_GETCHECK, 0, 0) == BST_CHECKED);
            ...
            if (cmd == SC_CANCEL_BTN)
                DestroyWindow(hwnd);
            if (cmd == SC_RESET_BTN)
            {
                if (MessageBoxW(hwnd, L"Reset all settings to defaults?", L"Reset", MB_YESNO | MB_ICONQUESTION) ==
                    IDYES)
                {
...
// ============================================================================
// Displays a "Get Started" page as a quick-start dialog on first launch.
// Shows: keyboard shortcuts, feature highlights, quick actions.
// ============================================================================

// Welcome HTML content removed — using native dialog instead of WebView2.
// WebView2Container wraps Monaco and does not support raw URL navigation.

[[maybe_unused]] static const char* WELCOME_HTML_UNUSED = R"HTML(
<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
...
{
    // Show welcome/onboarding page
    m_welcomePageShown = true;

    // Show welcome in a styled dialog with quick actions
    int result = MessageBoxA(m_hwndMain,
                             ...
                             "Features:\n"
                             "  - Smooth scroll & cursor animation\n"
                             "  - Code minimap with syntax colors\n"
                             "  - fzf-style fuzzy command palette\n"
                             "  - Drag-and-drop tab reordering\n"
                             "  - Split editor (vertical/horizontal/grid)\n"
                             "  - File icon themes (Seti/Material)\n\n"
...
void Win32IDE::handleWelcomeCloneRepo()
{
    ...
    char url[512] = {};
    // Simple input dialog
    HWND input = CreateWindowExA(WS_EX_DLGMODALFRAME, "EDIT", "", WS_OVERLAPPEDWINDOW | WS_VISIBLE | ES_AUTOHSCROLL,
...
static const FileIconMapping g_setiIcons[] = {
    {".cpp", 0, RGB(86, 156, 214), "C+"},       {".c", 1, RGB(86, 156, 214), "C"},
    {".h", 2, RGB(86, 156, 214), "H"},          {".hpp", 3, RGB(86, 156, 214), "H+"},
    {".py", 4, RGB(55, 118, 171), "Py"},        {".js", 5, RGB(241, 224, 90), "JS"},
    {".ts", 6, RGB(49, 120, 198), "TS"},        {".html", 7, RGB(227, 76, 38), "HT"},
    {".css", 8, RGB(86, 61, 124), "CS"},        {".json", 9, RGB(241, 224, 90), "{ }"},
    {".xml", 10, RGB(227, 76, 38), "XM"},       {".md", 11, RGB(66, 165, 245), "MD"},
    {".txt", 12, RGB(150, 150, 150), "Tx"},     {".asm", 13, RGB(206, 145, 120), "AS"},
    {".rs", 14, RGB(222, 165, 132), "Rs"},      {".go", 15, RGB(0, 173, 216), "Go"},
    {".java", 16, RGB(176, 114, 25), "Jv"},     {".cs", 17, RGB(104, 33, 122), "C#"},
    {".rb", 18, RGB(204, 52, 45), "Rb"},        {".php", 19, RGB(119, 123, 180), "PH"},
    {".lua", 20, RGB(0, 0, 128), "Lu"},         {".sh", 21, RGB(137, 224, 81), "Sh"},
    {".bat", 22, RGB(137, 224, 81), "Ba"},      {".ps1", 23, RGB(1, 36, 86), "PS"},
    {".sql", 24, RGB(227, 76, 38), "SQ"},       {".yaml", 25, RGB(203, 23, 30), "YM"},
    {".yml", 25, RGB(203, 23, 30), "YM"},       {".toml", 26, RGB(150, 150, 150), "TM"},
    {".ini", 27, RGB(150, 150, 150), "IN"},     {".cmake", 28, RGB(6, 152, 154), "CM"},
    {".gitignore", 29, RGB(240, 80, 50), "Gi"}, {".dockerfile", 30, RGB(56, 151, 239), "Dk"},
    {".svg", 31, RGB(255, 180, 0), "SV"},       {".png", 32, RGB(42, 136, 42), "Im"},
    {".jpg", 33, RGB(42, 136, 42), "Im"},       {".gif", 34, RGB(42, 136, 42), "Im"},
...
void Win32IDE::initFileIconTheme()
{
    ...
    if (!m_fileIconImageList)
    {
        ...
        for (int i = 0; i < ICON_COUNT; i++)
        {
            ...
            HFONT font = CreateFontA(-10, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                                     CLIP_DEFAULT_PRECIS, NONANTIALIASED_QUALITY, FIXED_PITCH, "Consolas");
            HFONT oldFont = (HFONT)SelectObject(memDC, font);
            DrawTextA(memDC, g_setiIcons[i].iconChar, -1, &rc, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
...
int Win32IDE::getFileIconIndex(const std::string& filename) const
{
    ...
    if (dot == std::string::npos)
        return ICON_COUNT - 1;  // default icon
    ...
    {
        if (ext == g_setiIcons[i].extension)
            return i;
    }
...
void Win32IDE::initTabDragDrop()
{
    ...
    if (m_hwndTabBar)
    {
        m_originalTabBarProc = (WNDPROC)SetWindowLongPtr(m_hwndTabBar, GWLP_WNDPROC, (LONG_PTR)TabBarDragProc);
    ...
    switch (msg)
    {
        ...
        case WM_PAINT:
        {
            ...
            if (ide->m_tabDragging && ide->m_dragInsertIndex >= 0)
            {
                ...
                int markerX = (ide->m_dragInsertIndex > ide->m_dragTabIndex) ? tabRect.right : tabRect.left;
...
void Win32IDE::reorderTab(int fromIndex, int toIndex)
{
    ...
    if (fromIndex < (int)m_editorTabs.size() && toIndex < (int)m_editorTabs.size())
    {
        ...
        if (insertAt > (int)m_editorTabs.size())
            insertAt = (int)m_editorTabs.size();
...
HWND Win32IDE::createEditorPane(HWND parent, const RECT& bounds)
{
    ...
    HWND pane = CreateWindowExA(WS_EX_CLIENTEDGE, RICHEDIT_CLASSA, "",
                                WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | ES_MULTILINE | ES_AUTOVSCROLL |
                                    ES_AUTOHSCROLL | ES_WANTRETURN,
                                bounds.left, bounds.top, bounds.right - bounds.left, bounds.bottom - bounds.top, parent,
...
void Win32IDE::splitEditorVertical()
{
    ...
    MoveWindow(m_hwndEditor, editorRect.left, editorRect.top, midX - editorRect.left - 2,
               editorRect.bottom - editorRect.top, TRUE);

    // Create right pane
    RECT rightBounds = {midX + 2, editorRect.top, editorRect.right, editorRect.bottom};
...
void Win32IDE::splitEditorHorizontal()
{
    ...
    MoveWindow(m_hwndEditor, editorRect.left, editorRect.top, editorRect.right - editorRect.left,
               midY - editorRect.top - 2, TRUE);

    // Create bottom pane
    RECT bottomBounds = {editorRect.left, midY + 2, editorRect.right, editorRect.bottom};
...
void Win32IDE::splitEditorGrid2x2()
{
    ...
    MoveWindow(m_hwndEditor, editorRect.left, editorRect.top, midX - editorRect.left - 2, midY - editorRect.top - 2,
               TRUE);
...
{
    if (!m_splitEditorActive || m_splitPanes.empty())
        return;
...
{
    if (!m_splitEditorActive || !m_hwndEditor)
        return;
...
void Win32IDE::checkForUpdates()
{
    ...
    auto checkThread = std::thread(
        [this]()
        {
            ...
            // Fetch latest release version from GitHub API via WinHTTP
            HINTERNET hSession = WinHttpOpen(L"RawrXD-AutoUpdate/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
            ...
            if (hSession)
            {
                ...
                {
                    HINTERNET hRequest =
                        WinHttpOpenRequest(hConnect, L"GET", L"/repos/RawrXD/RawrXD-Shell/releases/latest", nullptr,
                    ...
                    if (hRequest)
                    {
                            ...
                            WinHttpReceiveResponse(hRequest, nullptr))
                        {
                            ...
                            {
                                size_t valStart = response.find('"', tagPos + 10);
                                if (valStart != std::string::npos)
                                {
                                    ...
                                    {
                                        latestVersion = response.substr(valStart, valEnd - valStart);
            ...
            // Fallback: check local marker file if API was unreachable
...
{
    if (!m_updateAvailable || m_updateDismissed)
        return;

    // Show tray balloon tip
    m_trayIconData.uFlags = NIF_INFO;
    snprintf(m_trayIconData.szInfo, sizeof(m_trayIconData.szInfo),
             "RawrXD IDE v%s is available!\nClick to view release notes.", m_updateVersion.c_str());
    strncpy(m_trayIconData.szInfoTitle, "Update Available", sizeof(m_trayIconData.szInfoTitle) - 1);
    ...
    {
        char updateText[128];
        snprintf(updateText, sizeof(updateText), "Update v%s available", m_updateVersion.c_str());
...
LRESULT CALLBACK MultiFileSearchDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        case WM_CREATE:
        {
            ...
            SetWindowLongPtr(hwnd, GWLP_USERDATA, (LONG_PTR)ide);
            CreateWindowExA(0, "STATIC", "Find in files (workspace):", WS_CHILD | WS_VISIBLE, 12, 12, 280, 18, hwnd,
                            nullptr, nullptr, nullptr);
            HWND edit = CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", "", WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL, 12, 34,
                                        320, 24, hwnd, (HMENU)1, nullptr, nullptr);
            CreateWindowExA(0, "BUTTON", "Search", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 12, 66, 80, 26, hwnd,
                            (HMENU)2, nullptr, nullptr);
            CreateWindowExA(0, "BUTTON", "Close", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 98, 66, 80, 26, hwnd, (HMENU)3,
                            nullptr, nullptr);
            SetFocus(edit);
            return 0;
        }
...
LRESULT CALLBACK CICDSettingsDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        case WM_CREATE:
            ...
            CreateWindowExA(0, "BUTTON", "Add job", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 12, 56, 80, 26, hwnd,
                            (HMENU)10, nullptr, nullptr);
            CreateWindowExA(0, "LISTBOX", "", WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOTIFY, 12, 90, 420, 220, hwnd,
                            (HMENU)11, nullptr, nullptr);
            CreateWindowExA(0, "BUTTON", "Close", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 340, 318, 92, 26, hwnd,
                            (HMENU)12, nullptr, nullptr);
            return 0;
        case WM_COMMAND:
...
LRESULT CALLBACK ModelRegistryDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        case WM_CREATE:
        {
            ide = (Win32IDE*)((CREATESTRUCTA*)lParam)->lpCreateParams;
            SetWindowLongPtr(hwnd, GWLP_USERDATA, (LONG_PTR)ide);
            CreateWindowExA(0, "STATIC", "Model Registry — registered trained models.", WS_CHILD | WS_VISIBLE, 12, 12,
                            380, 18, hwnd, nullptr, nullptr, nullptr);
            CreateWindowExA(0, "LISTBOX", "", WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOTIFY, 12, 36, 420, 240, hwnd,
                            (HMENU)20, nullptr, nullptr);
            CreateWindowExA(0, "BUTTON", "Set active", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 12, 284, 80, 26, hwnd,
                            (HMENU)21, nullptr, nullptr);
            CreateWindowExA(0, "BUTTON", "Close", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 352, 284, 80, 26, hwnd,
                            (HMENU)22, nullptr, nullptr);
            if (ide)
            {
