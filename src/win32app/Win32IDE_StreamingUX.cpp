...
namespace
{
static std::wstring agentStatusUtf8ToWide(const std::string& utf8)
{
    if (utf8.empty())
        return {};
    int len =
...
void Win32IDE::showModelProgressBar(const std::string& operation)
{
    ...
    {
        m_hwndModelProgressContainer = CreateWindowExA(0, "STATIC", "", WS_CHILD | SS_OWNERDRAW, 0, barY, mainRC.right,
                                                       barHeight, m_hwndMain, nullptr, m_hInstance, nullptr);

        SetPropA(m_hwndModelProgressContainer, "IDE_PTR", (HANDLE)this);
    }

    // Position and resize container
    SetWindowPos(m_hwndModelProgressContainer, HWND_TOP, 0, barY, mainRC.right, barHeight, SWP_SHOWWINDOW);

    // Create progress bar control (use Win32 PROGRESS_CLASS)
    if (!m_hwndModelProgressBar)
    {
        m_hwndModelProgressBar =
            CreateWindowExA(0, PROGRESS_CLASSA, "", WS_CHILD | WS_VISIBLE | PBS_SMOOTH, 8, 4, mainRC.right - 180, 24,
    ...
    {
        SetWindowPos(m_hwndModelProgressBar, nullptr, 8, 4, mainRC.right - 180, 24, SWP_NOZORDER | SWP_SHOWWINDOW);
    }
    ...
    {
        m_hwndModelProgressLabel =
            CreateWindowExA(0, "STATIC", operation.c_str(), WS_CHILD | WS_VISIBLE | SS_LEFT | SS_CENTERIMAGE, 8, 4,
                            mainRC.right - 180, 24, m_hwndModelProgressContainer, nullptr, m_hInstance, nullptr);
    ...
    SetWindowPos(m_hwndModelProgressLabel, HWND_TOP, 12, 4, mainRC.right - 184, 24, SWP_SHOWWINDOW);

    // Create cancel button
    if (!m_hwndModelCancelBtn)
    {
        m_hwndModelCancelBtn =
            CreateWindowExA(0, "BUTTON", "Cancel", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, mainRC.right - 160, 4, 80, 24,
    ...
    {
        SetWindowPos(m_hwndModelCancelBtn, nullptr, mainRC.right - 160, 4, 80, 24, SWP_NOZORDER | SWP_SHOWWINDOW);
    }
...
void Win32IDE::showAgentActivityStatus(const std::string& text, int durationMs)
{
    if (!m_hwndStatusBar)
        return;
    std::wstring w = agentStatusUtf8ToWide(text);
    if (w.size() > 220)
        w.resize(220);
    SendMessageW(m_hwndStatusBar, SB_SETTEXTW, 0, reinterpret_cast<LPARAM>(w.c_str()));
    if (durationMs > 0 && m_hwndMain)
        SetTimer(m_hwndMain, 42, static_cast<UINT>(durationMs), nullptr);
}
...
void Win32IDE::updateSubAgentProgress(int completedTasks, int totalTasks, const std::string& currentTask)
{
    ...
    std::string status =
        "🔀 " + currentTask + " [" + std::to_string(completedTasks) + "/" + std::to_string(totalTasks) + "]";
...
void Win32IDE::showSwarmStatus()
{
    ...
    if (!todos.empty())
    {
        ...
        for (const auto& item : todos)
        {
            ...
            switch (item.status)
            {
                case TodoItem::Status::NotStarted:
                    icon = "⬜";
                    break;
                ...
                case TodoItem::Status::Completed:
                    icon = "✅";
                    break;
                case TodoItem::Status::Failed:
                    icon = "❌";
                    break;
            }
            out << icon << " [" << item.id << "] " << item.title << " — " << item.statusString() << "\n";
    ...
    if (!agents.empty())
    {
        ...
        for (const auto& agent : agents)
        {
            ...
            switch (agent.state)
            {
                case SubAgent::State::Pending:
                    icon = "⏳";
                    break;
                case SubAgent::State::Running:
                    icon = "🔄";
                    break;
                case SubAgent::State::Completed:
                    icon = "✅";
                    break;
                case SubAgent::State::Failed:
                    icon = "❌";
                    break;
                case SubAgent::State::Cancelled:
                    icon = "🚫";
                    break;
            }
            out << icon << " " << agent.id << " [" << agent.stateString() << "] " << agent.description << " ("
                << agent.elapsedMs() << "ms)\n";
        }
        out << "\n";
    }
    ...
    if (!chainSteps.empty())
    {
        ...
        for (const auto& step : chainSteps)
        {
            ...
            out << icon << " Step " << (step.index + 1) << ": " << step.promptTemplate.substr(0, 80)
...
void Win32IDE::updateContextSliderLabel()
{
    ...
    static const struct
    {
        ...
        {4096, "4K"},     {32768, "32K"},   {65536, "64K"},  {131072, "128K"},
...
{
    if (category == "system")
        m_contextUsage.systemTokens = tokens;
    else if (category == "tools")
        m_contextUsage.toolDefTokens = tokens;
    else if (category == "user")
        m_contextUsage.userContextTokens = tokens;
    else if (category == "messages")
        m_contextUsage.messageTokens = tokens;
    else if (category == "results")
        m_contextUsage.toolResultTokens = tokens;
    updateContextWindowDisplay();
}
...
void Win32IDE::updateContextWindowDisplay()
{
    ...
    std::string label = "Ctx: " + formatTokenCount(total) + "/" + formatTokenCount(m_contextUsage.maxTokens) + "  " +
    ...
    if (result.success)
    {
        ...
        // Update UI status
        {
            ...
            if (!result.alternatives.empty())
            {
                ...
                {
                    if (i > 0)
                        appendToOutput(", ", "ModelLoader", OutputSeverity::Info);
