...
#endif
#ifndef IDM_VIEW_FILE_EXPLORER
#define IDM_VIEW_FILE_EXPLORER 2025
#endif
#ifndef IDM_AUDIT_SHOW_DASHBOARD
#define IDM_AUDIT_SHOW_DASHBOARD 2026
#endif
#ifndef IDM_AGENT_BOUNDED_LOOP
#define IDM_AGENT_BOUNDED_LOOP 2027
#endif
...
// ============================================================================
Win32IDE::~Win32IDE() {
    // Minimal destructor — OS cleans up process resources on exit
}
...
LRESULT Win32IDE::handleMessage(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    default:
        ...
        // HexMag ask completion (WM_APP+220 — must not collide with WM_COPILOT_*).
        if (uMsg == (WM_APP + 220)) {
        ...
        if (uMsg == WM_NATIVE_AI_ERROR) {
            onNativeAIError(wParam, lParam);
            return 0;
        }
        if (uMsg == WM_NATIVE_AI_PROGRESS) {
            onNativeAIProgress(wParam, lParam);
            return 0;
        }
        break;
    }
...
void Win32IDE::onCreate(HWND hwnd) {
    ...
    {
        char buf[512];
        sprintf_s(buf, sizeof(buf), "HWND audit: Main=%p Editor=%p Sidebar=%p "
