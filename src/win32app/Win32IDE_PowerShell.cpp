...
namespace
{
bool resolvePwshOrWindowsPowerShell(std::string& outQuotedExe)
{
    char buf[MAX_PATH]{};
    if (SearchPathA(nullptr, "pwsh.exe", nullptr, MAX_PATH, buf, nullptr))
    {
        outQuotedExe = std::string("\"") + buf + "\"";
        return true;
    }
    outQuotedExe = "powershell.exe";
    return false;
}

std::string escapeForSingleQuotedPowerShellPath(const std::string& path)
{
    std::string o;
...
std::string Win32IDE::runProcessCaptureWithTimeout(const std::string& fullCommandLine, const char* cwdOverride)
{
    std::string cwdStore;
    const char* cwd = cwdOverride;
    if (!cwd)
        cwd = preferredIntegratedTerminalWorkingDirectory(cwdStore);

    HANDLE hStdOutRead = nullptr;
    HANDLE hStdOutWrite = nullptr;
    SECURITY_ATTRIBUTES sa = {sizeof(SECURITY_ATTRIBUTES), NULL, TRUE};
    if (!CreatePipe(&hStdOutRead, &hStdOutWrite, &sa, 0))
        return "ERROR: CreatePipe failed for subprocess capture";
    ...
    std::vector<char> cmdBuf(fullCommandLine.begin(), fullCommandLine.end());
    cmdBuf.push_back('\0');

    PROCESS_INFORMATION pi = {};
    const BOOL ok = CreateProcessA(nullptr, cmdBuf.data(), nullptr, nullptr, TRUE, CREATE_NO_WINDOW, nullptr,
    ...
    if (!ok)
    {
        ...
        return "ERROR: CreateProcess failed for subprocess capture";
    }
    ...
    while (ReadFile(hStdOutRead, buffer, kMaxChunk, &bytesRead, nullptr) && bytesRead > 0)
    {
    ...
    const bool isAgentic =
        (RawrXD::AgenticAutonomousConfig::instance().getOperationMode() != RawrXD::AgenticOperationMode::Ask);
    const std::string hint = RawrXD::AgenticAutonomousConfig::instance().getRecommendedTerminalRequirementHint();
    const DWORD timeoutMs = static_cast<DWORD>(IDEConfig::getInstance().getTerminalTimeoutMs(isAgentic, hint));

    const DWORD wr = WaitForSingleObject(pi.hProcess, timeoutMs);
    DWORD exitCode = 0;
    ...
    if (wr == WAIT_TIMEOUT)
    {
        output += "\r\n[Exit: timeout after " + std::to_string(timeoutMs) +
                  "ms — last GetExitCodeProcess=" + std::to_string(exitCode) + "]\r\n";
    }
    else
    {
...
std::string Win32IDE::runPowerShellSubprocessCapture(const std::string& command)
{
    std::string shellExe;
    resolvePwshOrWindowsPowerShell(shellExe);
    const std::string fullCmd = shellExe + " -NoProfile -Command \"" + escapePowerShellString(command) + "\"";
    return runProcessCaptureWithTimeout(fullCmd, nullptr);
}

void Win32IDE::schedulePowerShellCommandQueueDrain()
{
    if (!m_hwndMain || !IsWindow(m_hwndMain) || m_psCommandQueue.empty() || isShuttingDown())
        return;
    SetTimer(m_hwndMain, RAWRXD_IDT_PS_QUEUE_DRAIN, 1, nullptr);
}

void Win32IDE::drainPowerShellCommandQueue()
{
    ...
    if (m_psCommandQueue.empty())
        return;

    PSCommand cmd = std::move(m_psCommandQueue.front());
    m_psCommandQueue.erase(m_psCommandQueue.begin());

    const std::string out = runPowerShellSubprocessCapture(cmd.command);
    if (cmd.callback)
        cmd.callback(out);
    else
    {
        appendToOutput(std::string("[Async PowerShell #") + std::to_string(cmd.id) + "]\n" + out, "Output",
                       OutputSeverity::Info);
    }

    if (!m_psCommandQueue.empty() && !isShuttingDown())
        SetTimer(m_hwndMain, RAWRXD_IDT_PS_QUEUE_DRAIN, 1, nullptr);
}
...
std::string Win32IDE::executePowerShellScript(const std::string& scriptPath, const std::vector<std::string>& args)
{
    std::string shellExe;
    resolvePwshOrWindowsPowerShell(shellExe);
    std::string command = shellExe + " -NoProfile -ExecutionPolicy Bypass -File '" +
                          escapeForSingleQuotedPowerShellPath(scriptPath) + "'";
    ...
    return runProcessCaptureWithTimeout(command, nullptr);
}
...
std::string Win32IDE::executePowerShellCommand(const std::string& command, bool async)
{
    if (async)
    {
        ...
        schedulePowerShellCommandQueueDrain();
        return "Command queued: " + std::to_string(psCmd.id);
    }

    // Programmatic PowerShell: always isolated subprocess (pwsh or Windows PowerShell).
    // The integrated terminal receives input only via executeCommand() → writeInput, not here.
    return runPowerShellSubprocessCapture(command);
}
...
{
    std::string output =
        executePowerShellCommand("Get-Module -ListAvailable | Select-Object Name | ConvertTo-Json", false);
...
bool Win32IDE::importPowerShellModule(const std::string& moduleName)
{
    ...
    bool success = (result.find("ERROR") == std::string::npos && result.find("Exception") == std::string::npos);
...
std::string Win32IDE::invokePowerShellFunction(const std::string& functionName, const std::vector<std::string>& args)
{
...
{
    std::string output =
        executePowerShellCommand("Get-ChildItem function: | Select-Object Name | ConvertTo-Json", false);
...
{
    std::string psCommand = "Invoke-Command -ComputerName '" + computerName + "' -ScriptBlock { " + command + " }";
    return executePowerShellCommand(psCommand, false);
}
...
std::string Win32IDE::selectPowerShellObject(const std::string& inputObject, const std::vector<std::string>& properties)
{
...
{
    std::string command = "New-PSDrive -Name '" + name + "' -PSProvider '" + provider + "' -Root '" + root + "'";
    std::string result = executePowerShellCommand(command, false);
...
bool Win32IDE::removePowerShellJob(int jobId)
{
    ...
    m_psJobs.erase(it);
    m_psState.activeJobs.erase(std::remove(m_psState.activeJobs.begin(), m_psState.activeJobs.end(), jobId),
...
bool Win32IDE::startPowerShellTranscript(const std::string& path)
{
    ...
    bool success =
        (result.find("Transcript started") != std::string::npos || result.find("ERROR") == std::string::npos);
...
bool Win32IDE::registerPowerShellEvent(const std::string& sourceIdentifier, const std::string& eventName,
                                       const std::string& action)
{
    std::string command =
        "Register-EngineEvent -SourceIdentifier '" + sourceIdentifier + "' -Action { " + action + " }";
...
std::string Win32IDE::formatPowerShellTable(const std::string& data, const std::vector<std::string>& properties)
{
...
std::string Win32IDE::invokeRawrXDFunction(const std::string& functionName, const std::vector<std::string>& args)
{
    if (!m_rawrXDModuleLoaded)
    {
...
{
    std::vector<std::string> args = {"-ModelPath", modelPath, "-MaxZoneMB", std::to_string(maxZoneMB)};
...
{
    std::vector<std::string> args = {"-Prompt", prompt, "-MaxTokens", std::to_string(maxTokens)};
...
std::string Win32IDE::getRawrXDPowerShellPath()
{
    ...
    std::vector<std::string> searchPaths = {base + "RawrXD.ps1", base + "scripts\\RawrXD.ps1", ".\\RawrXD.ps1",
                                            "..\\RawrXD.ps1",    "..\\..\\RawrXD.ps1",         "scripts\\RawrXD.ps1"};
