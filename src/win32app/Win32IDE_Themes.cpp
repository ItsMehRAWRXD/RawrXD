...
{
    if (themeId < IDM_THEME_DARK_PLUS || themeId > IDM_THEME_ABYSS)
        return;
    ...
    if (m_hMenu)
    {
        for (int id = IDM_THEME_DARK_PLUS; id <= IDM_THEME_ABYSS; id++)
        {
            CheckMenuItem(m_hMenu, id, MF_BYCOMMAND | (id == themeId ? MF_CHECKED : MF_UNCHECKED));
        }
    }
...
#undef DIFF_COLOR
    ...
    {
        LOG_DEBUG("Theme diff: \"" + before.name + "\" -> \"" + candidate.name + "\" — " + std::to_string(changeCount) +
                  " field(s) changed:\n" + diff);
    }
}
...
void Win32IDE::applyThemeToAllControls()
{
    ...
    LOG_DEBUG("applyThemeToAllControls: \"" + t.name + "\" (darkMode=" + std::to_string(t.darkMode) + ")");
    ...
    {
        if (m_actBarBackgroundBrush)
            DeleteObject(m_actBarBackgroundBrush);
        if (m_actBarHoverBrush)
            DeleteObject(m_actBarHoverBrush);
        if (m_actBarActiveBrush)
            DeleteObject(m_actBarActiveBrush);
    ...
    {
        if (m_sidebarContentBrush)
            DeleteObject(m_sidebarContentBrush);
    ...
    if (m_hwndEditor)
    {
        COLORREF editorBg = t.backgroundColor;
        COLORREF editorFg = t.textColor;

        auto luma = [](COLORREF c) -> int
        { return (299 * (int)GetRValue(c) + 587 * (int)GetGValue(c) + 114 * (int)GetBValue(c)) / 1000; };

        // Prevent black-on-black (or near-identical) editor text during theme transitions.
        if (std::abs(luma(editorBg) - luma(editorFg)) < 70)
        {
            editorFg = t.darkMode ? RGB(212, 212, 212) : RGB(24, 24, 24);
        }

        SendMessage(m_hwndEditor, EM_SETBKGNDCOLOR, 0, editorBg);
        CHARFORMAT2A cf = {};
        cf.cbSize = sizeof(cf);
        cf.dwMask = CFM_COLOR;
        cf.crTextColor = editorFg;
        cf.dwEffects = 0;
    ...
    {
        // VS Code / Explorer tab strip — use system visual style so tabs track dark/light chrome.
        if (t.darkMode)
            SetWindowTheme(m_hwndPanelTabs, L"Explorer", nullptr);
        else
            SetWindowTheme(m_hwndPanelTabs, nullptr, nullptr);
    ...
    {
        if (t.darkMode)
            SetWindowTheme(m_hwndOutputTabs, L"Explorer", nullptr);
        else
            SetWindowTheme(m_hwndOutputTabs, nullptr, nullptr);
        InvalidateRect(m_hwndOutputTabs, nullptr, TRUE);
    }

    // -- Integrated terminal shell tabs (pwsh / cmd / Agent) --
    if (g_rawrxdIntegratedTerminalTabs && IsWindow(g_rawrxdIntegratedTerminalTabs))
    {
        if (t.darkMode)
            SetWindowTheme(g_rawrxdIntegratedTerminalTabs, L"Explorer", nullptr);
        else
            SetWindowTheme(g_rawrxdIntegratedTerminalTabs, nullptr, nullptr);
        InvalidateRect(g_rawrxdIntegratedTerminalTabs, nullptr, TRUE);
    }
    ...
    for (auto& pane : m_terminalPanes)
    {
        if (!pane.hwnd)
            continue;
        const COLORREF termBg = (pane.kind == TerminalPaneKind::AgentReadOnly)
                                    ? blendColor(t.panelBg, RGB(235, 111, 146), 0.10f)
                                    : t.panelBg;
        SendMessage(pane.hwnd, EM_SETBKGNDCOLOR, 0, termBg);
        CHARFORMAT2A tcf = {};
        tcf.cbSize = sizeof(tcf);
        tcf.dwMask = CFM_COLOR;
        tcf.crTextColor = (pane.kind == TerminalPaneKind::AgentReadOnly)
                              ? blendColor(t.panelFg, RGB(235, 188, 200), 0.35f)
                              : t.panelFg;
        tcf.dwEffects = 0;
    ...
    {
        if (m_secondarySidebarBrush)
            DeleteObject(m_secondarySidebarBrush);
    ...
    if (m_hwndMain)
    {
        if (m_mainWindowBrush)
            DeleteObject(m_mainWindowBrush);
...
void Win32IDE::setWindowTransparency(BYTE alpha)
{
    ...
    if (m_hMenu)
    {
        int checks[] = {IDM_TRANSPARENCY_100, IDM_TRANSPARENCY_90, IDM_TRANSPARENCY_80, IDM_TRANSPARENCY_70,
                        IDM_TRANSPARENCY_60,  IDM_TRANSPARENCY_50, IDM_TRANSPARENCY_40};
        BYTE values[] = {255, 230, 204, 178, 153, 128, 102};
        for (int i = 0; i < 7; i++)
        {
            CheckMenuItem(m_hMenu, checks[i], MF_BYCOMMAND | (alpha == values[i] ? MF_CHECKED : MF_UNCHECKED));
        }
    }
    ...
    if (m_hwndStatusBar)
    {
        ...
        if (pct == 100)
            msg = "Transparency: Off (100% opaque)";
        SendMessageA(m_hwndStatusBar, SB_SETTEXT, 0, (LPARAM)msg.c_str());
    }
}
...
INT_PTR CALLBACK TransparencyDlgProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg)
    {
        case WM_INITDIALOG:
        {
            ...
            HWND hSlider = CreateWindowExA(0, TRACKBAR_CLASSA, "", WS_CHILD | WS_VISIBLE | TBS_HORZ | TBS_AUTOTICKS, 20,
                                           50, 350, 35, hwndDlg, (HMENU)1001, nullptr, nullptr);
            ...
            // Label
            HWND hLabel = CreateWindowExA(0, "STATIC", "Window Opacity:", WS_CHILD | WS_VISIBLE, 20, 20, 200, 20,
                                          hwndDlg, (HMENU)1002, nullptr, nullptr);
            (void)hLabel;

            // Value label
            char buf[32];
            snprintf(buf, sizeof(buf), "%d%%", currentPct);
            CreateWindowExA(0, "STATIC", buf, WS_CHILD | WS_VISIBLE | SS_CENTER, 160, 85, 60, 20, hwndDlg, (HMENU)1003,
                            nullptr, nullptr);
...
void Win32IDE::buildThemeMenu(HMENU hParentMenu)
{
    ...
    // Mark current transparency level
    {
        ...
        for (int i = 0; i < 7; i++)
        {
            CheckMenuItem(hTransMenu, checks[i],
                          MF_BYCOMMAND | (m_windowAlpha == values[i] ? MF_CHECKED : MF_UNCHECKED));
        }
    }
...
INT_PTR CALLBACK ThemePickerDlgProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg)
    {
        case WM_INITDIALOG:
        {
            ...
            // -- Theme listbox --
            CreateWindowExA(0, "STATIC", "Color Theme:", WS_CHILD | WS_VISIBLE, 14, 10, 200, 18, hwndDlg, nullptr,
                            nullptr, nullptr);

            HWND hList = CreateWindowExA(WS_EX_CLIENTEDGE, "LISTBOX", "",
                                         WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOTIFY | LBS_NOINTEGRALHEIGHT, 14, 30,
                                         290, 260, hwndDlg, (HMENU)(UINT_PTR)IDP_THEME_LIST, nullptr, nullptr);
            ...
            // -- Transparency slider --
            CreateWindowExA(0, "STATIC", "Window Opacity:", WS_CHILD | WS_VISIBLE, 14, 300, 120, 18, hwndDlg, nullptr,
                            nullptr, nullptr);

            HWND hSlider = CreateWindowExA(0, TRACKBAR_CLASSA, "", WS_CHILD | WS_VISIBLE | TBS_HORZ | TBS_AUTOTICKS, 14,
                                           320, 230, 28, hwndDlg, (HMENU)(UINT_PTR)IDP_TRANSPARENCY, nullptr, nullptr);
            SendMessage(hSlider, TBM_SETRANGE, TRUE, MAKELPARAM(30, 100));
            ...
            CreateWindowExA(0, "STATIC", pctBuf, WS_CHILD | WS_VISIBLE | SS_CENTER, 250, 324, 50, 18, hwndDlg,
                            (HMENU)(UINT_PTR)IDP_TRANSPARENCY_LBL, nullptr, nullptr);

            // -- OK / Cancel buttons --
            CreateWindowExA(0, "BUTTON", "OK", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON, 120, 360, 80, 28, hwndDlg,
                            (HMENU)(UINT_PTR)IDP_BTN_OK, nullptr, nullptr);
            CreateWindowExA(0, "BUTTON", "Cancel", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 210, 360, 80, 28, hwndDlg,
                            (HMENU)(UINT_PTR)IDP_BTN_CANCEL, nullptr, nullptr);
        ...
        case WM_COMMAND:
        {
            ...
            if (ctrl == IDP_THEME_LIST && notif == LBN_SELCHANGE)
            {
                ...
                if (idx >= 0 && idx < g_themePickerCount && s_pickerInstance)
                {
                    ...
                    LOG_DEBUG("Theme picker: live preview of \"" + s_pickerInstance->m_currentTheme.name + "\"");
                }
                return TRUE;
            }
            ...
            if (ctrl == IDP_BTN_OK)
            {
                ...
                if (s_pickerInstance)
                {
                    ...
                    if (s_pickerInstance->m_hMenu)
                    {
                        for (int id = IDM_THEME_DARK_PLUS; id <= IDM_THEME_ABYSS; id++)
                        {
                            CheckMenuItem(s_pickerInstance->m_hMenu, id,
                                          MF_BYCOMMAND |
            ...
            if (ctrl == IDP_BTN_CANCEL)
            {
                if (s_pickerInstance)
                {
                    ...
                    LOG_INFO("Theme picker: cancelled — restored \"" + s_pickerInstance->m_currentTheme.name + "\"");
                }
                EndDialog(hwndDlg, IDCANCEL);
                return TRUE;
            }
