...
MainWindow::MainWindow()
    : m_hwnd(nullptr), m_editorHwnd(nullptr), m_terminalHwnd(nullptr), m_overclockHwnd(nullptr),
      m_floatingPanel(nullptr), m_problemsPanelHwnd(nullptr), m_findPanelHwnd(nullptr), m_findEditHwnd(nullptr),
      m_replaceEditHwnd(nullptr), m_findNextBtnHwnd(nullptr), m_replaceBtnHwnd(nullptr), m_replaceAllBtnHwnd(nullptr),
      m_statusBarHwnd(nullptr), m_tabBarHwnd(nullptr), m_tabScrollHwnd(nullptr), m_terminalRunning(false),
      m_psInWrite(nullptr), m_psOutRead(nullptr), m_terminalReaderActive(false), m_floatingPanelVisible(false),
...
#endif
    // Theme profiles
    m_themes.push_back({"dark", (unsigned)RGB(30, 30, 30), (unsigned)RGB(212, 212, 212), (unsigned)RGB(86, 156, 214),
                        (unsigned)RGB(181, 206, 168), (unsigned)RGB(212, 212, 212), (unsigned)RGB(206, 145, 120),
                        (unsigned)RGB(106, 153, 85)});
    m_themes.push_back({"light", (unsigned)RGB(255, 255, 255), (unsigned)RGB(0, 0, 0), (unsigned)RGB(0, 0, 160),
...
#ifdef _WIN32
    m_findPanelHwnd =
        CreateWindowEx(WS_EX_TOPMOST | WS_EX_TOOLWINDOW, "STATIC", "Find/Replace", WS_POPUP | WS_VISIBLE | WS_BORDER,
                       650, 10, 320, 120, m_hwnd, nullptr, GetModuleHandle(nullptr), nullptr);

    m_findEditHwnd = CreateWindowEx(0, "EDIT", nullptr, WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, 10, 30, 140,
                                    22, m_findPanelHwnd, nullptr, GetModuleHandle(nullptr), nullptr);
    SendMessage(m_findEditHwnd, WM_SETFONT, (WPARAM)GetStockObject(DEFAULT_GUI_FONT), TRUE);

    m_replaceEditHwnd = CreateWindowEx(0, "EDIT", nullptr, WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, 160, 30,
                                       140, 22, m_findPanelHwnd, nullptr, GetModuleHandle(nullptr), nullptr);
    SendMessage(m_replaceEditHwnd, WM_SETFONT, (WPARAM)GetStockObject(DEFAULT_GUI_FONT), TRUE);

    m_findNextBtnHwnd = CreateWindowEx(0, "BUTTON", "Find Next", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 10, 70, 90, 28,
                                       m_findPanelHwnd, (HMENU)1, GetModuleHandle(nullptr), nullptr);

    m_replaceBtnHwnd = CreateWindowEx(0, "BUTTON", "Replace", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 110, 70, 90, 28,
                                      m_findPanelHwnd, (HMENU)2, GetModuleHandle(nullptr), nullptr);

    m_replaceAllBtnHwnd = CreateWindowEx(0, "BUTTON", "Replace All", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 210, 70, 90,
                                         28, m_findPanelHwnd, (HMENU)3, GetModuleHandle(nullptr), nullptr);
#endif
}
...
void MainWindow::createMenuBar()
{
    ...
    AppendMenuA(selMenu, MF_STRING | (m_columnSelectionMode ? MF_CHECKED : 0), IDM_SEL_COLUMN_MODE,
    ...
    AppendMenuA(appearanceMenu, MF_STRING | (m_activityBarVisible ? MF_CHECKED : 0), IDM_VIEW_ACTIVITY_BAR,
                "Activity Bar");
    AppendMenuA(appearanceMenu, MF_STRING | (m_primarySidebarVisible ? MF_CHECKED : 0), IDM_VIEW_PRIMARY_SIDEBAR,
                "Primary Side Bar\tCtrl+B");
    AppendMenuA(appearanceMenu, MF_STRING | (m_secondarySidebarVisible ? MF_CHECKED : 0), IDM_VIEW_SECONDARY_SIDEBAR,
                "Secondary Side Bar");
    ...
    AppendMenuA(appearanceMenu, MF_STRING | (m_zenModeEnabled ? MF_CHECKED : 0), IDM_VIEW_ZEN_MODE,
                "Zen Mode\tCtrl+K Z");
    ...
    AppendMenuA(editorLayoutMenu, MF_STRING | (m_wordWrapEnabled ? MF_CHECKED : 0), IDM_VIEW_WORD_WRAP,
                "Word Wrap\tAlt+Z");
    AppendMenuA(editorLayoutMenu, MF_STRING | (m_lineNumbersEnabled ? MF_CHECKED : 0), IDM_VIEW_LINE_NUMBERS,
                "Line Numbers");
...
void MainWindow::handleMenuCommand(WORD cmdId)
{
    ...
    switch (cmdId)
    {
        ...
        case IDM_EDIT_CUT:
            if (m_editorHwnd)
                SendMessage(m_editorHwnd, WM_CUT, 0, 0);
            break;
        case IDM_EDIT_COPY:
            if (m_editorHwnd)
                SendMessage(m_editorHwnd, WM_COPY, 0, 0);
            break;
        case IDM_EDIT_PASTE:
            if (m_editorHwnd)
                SendMessage(m_editorHwnd, WM_PASTE, 0, 0);
            break;
        case IDM_EDIT_FIND:
            if (m_findPanelHwnd)
        ...
        case IDM_SEL_ALL:
            if (m_editorHwnd)
                SendMessage(m_editorHwnd, EM_SETSEL, 0, -1);
            break;
                    ...
                    if (!needle.empty())
                    {
                        if (cmdId == IDM_SEL_ADD_NEXT_OCCURRENCE)
                        {
                            ...
                            if (next != std::string::npos)
                            {
                                ...
                                SendMessageA(m_editorHwnd, EM_SETSEL, (WPARAM)p,
                                             (LPARAM)(p + static_cast<long>(needle.size())));
                            }
                        }
                        else
                        {
        ...
        case IDM_VIEW_PRIMARY_SIDEBAR:
            ...
            if (m_fileBrowserHwnd)
                ShowWindow(m_fileBrowserHwnd, m_primarySidebarVisible ? SW_SHOW : SW_HIDE);
            if (m_hwnd)
            {
                RECT rc;
                GetClientRect(m_hwnd, &rc);
        ...
        case IDM_VIEW_PANEL:
            ...
            if (m_terminalHwnd)
                ShowWindow(m_terminalHwnd, m_panelVisible ? SW_SHOW : SW_HIDE);
            break;
        case IDM_VIEW_STATUS_BAR:
            ...
            if (m_statusBarHwnd)
                ShowWindow(m_statusBarHwnd, m_statusBarVisible ? SW_SHOW : SW_HIDE);
            break;
        ...
        case 14002:  // IDC_AGENT_STATUS_LABEL
        {
            ...
            st << "Agent: " << (m_chatBusy ? "Busy" : "Idle")
               << " | Model: " << (m_modelLoaded ? "Loaded" : "Not loaded") << " | Turns: " << m_chatHistory.size();
            if (m_statusBarHwnd)
            {
        ...
        case 14006:  // IDC_AGENT_SUMMARY_EDIT
        {
            ...
            summary << "Summary: tabs=" << m_tabs.size() << ", active=" << (m_tabs.empty() ? 0 : (m_currentTab + 1))
                    << ", problems=" << m_problems.size() << ", breakpoints=" << g_breakpointLines.size()
        ...
        case 2109:  // IDC_DEBUGGER_STATUS_TEXT
        {
            ...
            if (g_debuggerTabIndex == 1)
                tabName = "watch";
            else if (g_debuggerTabIndex == 2)
                tabName = "variables";
            else if (g_debuggerTabIndex == 3)
                tabName = "stack";
            std::ostringstream st;
            st << "Debugger | tab=" << tabName << " | breakpoints=" << g_breakpointLines.size()
        ...
        case 2111:  // IDC_DEBUGGER_WATCH_LIST
            if (m_editorHwnd)
            {
                ...
                if (selEnd > selStart)
                {
                    ...
                    if (!buf.empty())
                    {
                        const bool exists = std::find(g_debugWatchExpressions.begin(), g_debugWatchExpressions.end(),
                                                      buf) != g_debugWatchExpressions.end();
                        if (!exists)
                        {
        ...
        case IDM_VIEW_ZEN_MODE:
            ...
            if (m_zenModeEnabled)
            {
                ...
                if (m_fileBrowserHwnd)
                    ShowWindow(m_fileBrowserHwnd, SW_HIDE);
                if (m_terminalHwnd)
                    ShowWindow(m_terminalHwnd, SW_HIDE);
                if (m_statusBarHwnd)
                    ShowWindow(m_statusBarHwnd, SW_HIDE);
                // Go fullscreen
            ...
            else
            {
                ...
                SetMenu(m_hwnd, m_menuBar);
                if (m_fileBrowserHwnd && m_primarySidebarVisible)
                    ShowWindow(m_fileBrowserHwnd, SW_SHOW);
                if (m_terminalHwnd && m_panelVisible)
                    ShowWindow(m_terminalHwnd, SW_SHOW);
                if (m_statusBarHwnd && m_statusBarVisible)
                    ShowWindow(m_statusBarHwnd, SW_SHOW);
        ...
        case 1017:  // IDC_SEVERITY_FILTER
        {
            ...
            const char* mode = "all";
            if (g_severityFilterMode == 1)
                mode = "error";
            else if (g_severityFilterMode == 2)
                mode = "warning";
            else if (g_severityFilterMode == 3)
                mode = "info";
        ...
        case IDM_RUN_TOGGLE_BREAKPOINT:
            if (m_editorHwnd)
            {
                ...
                if (!filePath.empty() && filePath.find(".ps1") != std::string::npos)
                {
                    ...
                    {
                        ps << "Get-PSBreakpoint | Where-Object { $_.Script -eq '" << escaped << "' -and $_.Line -eq "
                           << line << " } | Remove-PSBreakpoint\n";
                    }
                    sendToTerminal(ps.str());
                }

                std::ostringstream status;
                status << "# Breakpoint " << (adding ? "set" : "cleared") << " at line " << line
                       << " (tracked=" << g_breakpointLines.size() << ")\n";
        ...
        case IDM_TERM_SPLIT:
            ...
            if (m_terminalRunning && m_terminalProcess.hProcess)
            {
                ...
                if (m_psInWrite)
                {
                    CloseHandle(m_psInWrite);
                    m_psInWrite = nullptr;
                }
                if (m_psOutRead)
                {
                    CloseHandle(m_psOutRead);
                    m_psOutRead = nullptr;
                }
            }
            if (m_terminalHwnd)
                SetWindowTextA(m_terminalHwnd, "");
            createTerminal();
        ...
        case IDM_HELP_WELCOME:
            MessageBoxA(m_hwnd, "Welcome to RawrXD IDE!\n\nA lightweight, fast IDE for PowerShell and C++ development.",
                        "Welcome", MB_OK | MB_ICONINFORMATION);
            break;
        case IDM_HELP_DOCS:
        ...
        case IDM_HELP_RELEASE_NOTES:
            MessageBoxA(m_hwnd,
                        "RawrXD IDE v1.0\n\nRelease Notes:\n- Initial release\n- Full menu bar\n- Multi-tab support\n- "
                        "Integrated terminal\n- AI chat integration",
                        "Release Notes", MB_OK | MB_ICONINFORMATION);
            break;
...
int MainWindow::exec()
{
    ...
    if (m_terminalRunning)
    {
        ...
        if (m_psInWrite)
        {
            ...
            DWORD written = 0;
            WriteFile(m_psInWrite, exitCmd, (DWORD)strlen(exitCmd), &written, nullptr);
        }
...
void MainWindow::createWindow()
{
    ...
    m_hwnd = CreateWindowEx(0, className, m_windowTitle.c_str(), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT,
                            1200, 700, nullptr, nullptr, GetModuleHandle(nullptr), this);
    if (m_hwnd)
    {
        createMenuBar();  // Create the menu bar first
        createTabBar();
        createEditor();
        m_statusBarHwnd = CreateWindowExA(0, "STATIC", "", WS_CHILD | WS_VISIBLE | SS_LEFT, 10, 450, 580, 20, m_hwnd,
...
void MainWindow::createEditor()
{
    ...
    m_editorHwnd = CreateWindowExA(0, "RICHEDIT50W", nullptr,
    ...
    if (m_editorHwnd)
    {
        HFONT hFont = CreateFontA(11, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
...
void MainWindow::createTabBar()
{
    ...
    if (!m_hwnd)
        return;
    m_tabBarHwnd = CreateWindowExA(0, "STATIC", "", WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN, 10, 10, 580, 24, m_hwnd,
                                   nullptr, GetModuleHandle(nullptr), nullptr);
    m_tabScrollHwnd = CreateWindowExA(0, "SCROLLBAR", nullptr, WS_CHILD | WS_VISIBLE | SBS_HORZ, 10, 34, 580, 14,
                                      m_hwnd, (HMENU)1988, GetModuleHandle(nullptr), nullptr);
    refreshTabBar();
#endif
}
...
#ifdef _WIN32
    if (m_aiMetricsHwnd)
        SetWindowTextA(m_aiMetricsHwnd, ss.str().c_str());
#endif
}
...
void MainWindow::runPesterTests()
{
    ...
    std::string testCommand =
        "powershell -Command \"Invoke-Pester -Path . -OutputFormat NUnitXml -OutputFile TestResults.xml\"";
...
void MainWindow::publishToGallery()
{
    ...
    std::string publishCommand =
        "powershell -Command \"Publish-Script -Path script.ps1 -NuGetApiKey $env:NUGET_API_KEY\"";
...
void MainWindow::setEditorFont(const std::string& fontName, int fontSize)
{
    ...
    {
        HFONT hFont =
            CreateFontA(fontSize, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                        CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, FIXED_PITCH | FF_MODERN, fontName.c_str());
        SendMessage(m_editorHwnd, WM_SETFONT, (WPARAM)hFont, TRUE);
    }
#endif
}
...
void MainWindow::createProblemsPanel()
{
    ...
    m_problemsPanelHwnd =
        CreateWindowExA(0, "EDIT", "Problems Panel\r\n",
                        WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_MULTILINE | ES_READONLY | ES_AUTOVSCROLL, 0, panelY,
                        clientRect.right, panelHeight, m_hwnd, nullptr, GetModuleHandle(nullptr), nullptr);

    if (m_problemsPanelHwnd)
    {
        HFONT hFont = CreateFontA(10, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                                  CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, FIXED_PITCH | FF_MODERN, "Consolas");
...
{
    std::sort(m_problems.begin(), m_problems.end(),
        ...
        for (const auto& p : m_problems)
        {
            if (type == "all" || p.severity == type)
            {
                ...
                std::string problemText =
                    icon + " " + p.file + ":" + std::to_string(p.line) + " - " + p.message + "\r\n";
...
void MainWindow::logAITelemetry(const std::string& model, bool success)
{
    ...
    if (log.is_open())
    {
        ...
        log << std::put_time(std::localtime(&now_c), "%F %T") << " [AI_REQUEST] Model=" << safeModel
...
void MainWindow::exportMetrics(const std::string& format)
{
    ...
    {
        MessageBoxA(m_hwnd, "Could not read ai_usage.log or create export file.", "Export Failed",
                    MB_OK | MB_ICONERROR);
    }
}
...
void MainWindow::initUnitTesting()
{
    ...
    handleCommand("powershell -Command \"if (!(Get-Module -ListAvailable -Name Pester)) { Install-Module -Name Pester "
    ...
    if (window)
    {
        switch (uMsg)
        {
            ...
            case WM_COMMAND:
                ...
                {
                    window->addTab("Untitled");
                    return 0;
                }
                if (LOWORD(wParam) >= 2000 && LOWORD(wParam) < 2100)
                {
                    size_t idx = LOWORD(wParam) - 2000;
                    window->switchTab(idx);
                    return 0;
                }
                if (LOWORD(wParam) == 3001)
                {
                    int sel = (int)SendMessage(window->m_commandPaletteHwnd, LB_GETCURSEL, 0, 0);
                    window->executePaletteSelection(sel);
                ...
                if (HIWORD(wParam) == LBN_DBLCLK && window->m_fileBrowserHwnd &&
                    (HWND)lParam == window->m_fileBrowserHwnd)
                {
                ...
                // Route all menu bar commands to handleMenuCommand
                {
                    ...
                    if (cmdId >= 100 && cmdId < 800)
                    {  // Menu ID range 100-799
            ...
            case WM_SIZE:
            {
                ...
                {
                    MoveWindow(window->m_editorHwnd, 10, 56, std::max(120, rc.right - 20),
                               std::max(120, rc.bottom - 106), TRUE);
                }
                if (window->m_statusBarHwnd)
                {
                    MoveWindow(window->m_statusBarHwnd, 10, std::max(0, rc.bottom - 24), std::max(120, rc.right - 20),
                               20, TRUE);
                }
                ...
                if (window->m_splitLayout)
                {
                    ...
                    if (window->m_userChatInputHwnd && window->m_userChatSendBtn)
                    {
                        ...
                        if (window->m_terminalHwnd)
                        {
                            ...
                            MoveWindow(window->m_userChatSendBtn, chatX + chatW - btnW - pad,
                                       chatY + chatH - btnH - pad, btnW, btnH, TRUE);
            ...
            case WM_MOUSEWHEEL:
            {
                ...
                if (!overTabArea && window->m_tabScrollHwnd && GetWindowRect(window->m_tabScrollHwnd, &slider) &&
                    PtInRect(&slider, pt))
                {
                    overTabArea = true;
                }
            ...
            case WM_KEYDOWN:
                ...
                {
                    if (window->m_findPanelHwnd)
                        ShowWindow(window->m_findPanelHwnd, SW_SHOW);
                    return 0;
                }
                ...
                {
                    if (window->m_tabs.size() > 1)
                        window->closeTab(window->m_currentTab);
                    return 0;
                }
                if ((GetKeyState(VK_CONTROL) & 0x8000) && wParam == VK_TAB)
                {
                    size_t n = window->m_tabs.size();
                    if (n > 1)
                        window->switchTab((window->m_currentTab + 1) % n);
                    return 0;
                }
                ...
                {
                    window->handleMenuCommand(IDM_FILE_NEW);
                    return 0;
                }
                if ((GetKeyState(VK_CONTROL) & 0x8000) && wParam == 'O')
                {
                    window->handleMenuCommand(IDM_FILE_OPEN);
                    return 0;
                }
                if ((GetKeyState(VK_CONTROL) & 0x8000) && wParam == 'S')
                {
                    if (GetKeyState(VK_SHIFT) & 0x8000)
                        window->handleMenuCommand(IDM_FILE_SAVEAS);
                    else
                        window->handleMenuCommand(IDM_FILE_SAVE);
                    return 0;
                }
                if ((GetKeyState(VK_CONTROL) & 0x8000) && wParam == 'B')
                {
                    window->handleMenuCommand(IDM_VIEW_PRIMARY_SIDEBAR);
                    return 0;
                }
                if ((GetKeyState(VK_CONTROL) & 0x8000) && wParam == 'J')
                {
                    window->handleMenuCommand(IDM_VIEW_PANEL);
                    return 0;
                }
                if ((GetKeyState(VK_CONTROL) & 0x8000) && wParam == 'G')
                {
                    window->handleMenuCommand(IDM_EDIT_GOTO_LINE);
                    return 0;
                }
                if ((GetKeyState(VK_CONTROL) & 0x8000) && wParam == 'A')
                {
                    window->handleMenuCommand(IDM_EDIT_SELECTALL);
                    return 0;
                }
                if ((GetKeyState(VK_CONTROL) & 0x8000) && wParam == VK_OEM_3)
                {  // Ctrl+` (backtick)
                ...
                if (wParam == VK_F5)
                {
                    if (GetKeyState(VK_CONTROL) & 0x8000)
                        window->handleMenuCommand(IDM_RUN_WITHOUT_DEBUG);
                    else
                        window->handleMenuCommand(IDM_RUN_START_DEBUG);
                    return 0;
                }
                if (wParam == VK_F9)
                {
                    window->handleMenuCommand(IDM_RUN_TOGGLE_BREAKPOINT);
                    return 0;
                }
                if (wParam == VK_F10)
                {
                    window->handleMenuCommand(IDM_RUN_STEP_OVER);
                    return 0;
                }
                if (wParam == VK_F11)
                {
                    if (GetKeyState(VK_SHIFT) & 0x8000)
                        window->handleMenuCommand(IDM_RUN_STEP_OUT);
                    else
                        window->handleMenuCommand(IDM_RUN_STEP_INTO);
                    return 0;
                }
                if (wParam == VK_F12)
                {
...
void MainWindow::createLayoutPanes()
{
    ...
    m_fileBrowserHwnd = CreateWindowExA(WS_EX_CLIENTEDGE, "LISTBOX", "",
                                        WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOINTEGRALHEIGHT | LBS_NOTIFY, 0, 0,
                                        100, 100, m_hwnd, (HMENU)5000, GetModuleHandle(nullptr), nullptr);
    ...
    m_topChatHwnd = CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", "",
                                    WS_CHILD | WS_VISIBLE | ES_MULTILINE | ES_AUTOVSCROLL | ES_READONLY | WS_VSCROLL, 0,
                                    0, 100, 100, m_hwnd, nullptr, GetModuleHandle(nullptr), nullptr);
    ...
    HWND userChatContainer = CreateWindowExA(WS_EX_CLIENTEDGE, "STATIC", "", WS_CHILD | WS_VISIBLE, 0, 0, 100, 100,
                                             m_hwnd, nullptr, GetModuleHandle(nullptr), nullptr);

    m_userChatInputHwnd = CreateWindowExA(WS_EX_CLIENTEDGE, "EDIT", "",
                                          WS_CHILD | WS_VISIBLE | ES_MULTILINE | ES_AUTOVSCROLL | WS_VSCROLL, 0, 0, 100,
                                          100, m_hwnd, (HMENU)5001, GetModuleHandle(nullptr), nullptr);
    SendMessageA(m_userChatInputHwnd, WM_SETFONT, (WPARAM)GetStockObject(DEFAULT_GUI_FONT), TRUE);

    m_userChatSendBtn = CreateWindowExA(0, "BUTTON", "Send", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0, 0, 80, 28,
                                        m_hwnd, (HMENU)5002, GetModuleHandle(nullptr), nullptr);
...
void MainWindow::createTerminal()
{
    ...
    if (!m_terminalRunning)
    {
        ...
        if (CreateProcessA(nullptr, cmdLine, nullptr, nullptr, TRUE, CREATE_NO_WINDOW, nullptr, nullptr, &si,
                           &m_terminalProcess))
        {
...
void MainWindow::startChatRequest(const std::string& prompt)
{
    ...
    if (m_chatBusy)
    {
        appendTopChat("System", "Chat busy. Please wait.");
        return;
    }
    m_chatBusy = true;
        ...
        [self, prompt]()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    RawrXD::Backend::NativeInferenceResponse resp = self->m_ollama.chatSync(req);
...
{
    if (!m_editorHwnd || searchText.empty())
        return;
    ...
    if (pos == std::string::npos)
        pos = text.find(searchText, 0);  // wrap
    if (pos != std::string::npos)
    {
...
void MainWindow::startTerminalReader()
{
    ...
    m_terminalReaderThread = std::thread(
        [this]()
        {
            char buffer[512];
            DWORD readBytes = 0;
...
void MainWindow::stopTerminalReader()
{
    ...
    if (m_terminalReaderThread.joinable())
    {
        if (m_terminalReaderThread.get_id() == std::this_thread::get_id())
        {
            m_terminalReaderThread.detach();
        }
        else
        {
            m_terminalReaderThread.join();
        }
    }
    if (m_psOutRead)
    {
...
void MainWindow::applyEdit(size_t pos, size_t eraseLen, std::string_view insertText)
{
    ...
    if (eraseLen)
        currentBuffer().erase(pos, eraseLen);
    if (!insertText.empty())
        currentBuffer().insert(pos, insertText);
    // Coalescing: merge sequential inserts at same advancing position within 400ms
    uint64_t now = GetTickCount64();
    bool canCoalesce = m_lastWasInsert && eraseLen == 0 && insertText.size() == 1 && pos == m_lastEditPos &&
                       (now - m_lastEditTick) < 400;
    ...
    m_lastEditTick = now;
    m_lastEditPos = pos + insertText.size();
    m_lastWasInsert = insertText.size() > 0 && eraseLen == 0;
    ...
    if (m_tabs.size())
    {
        m_tabs[m_currentTab].dirty = true;
    }
    refreshTabBar();
    updateStatusBar();
}
...
void MainWindow::performUndo()
{
    ...
    if (!cmd.inserted.empty())
        currentBuffer().erase(cmd.pos, cmd.inserted.size());
    if (!cmd.removed.empty())
        currentBuffer().insert(cmd.pos, cmd.removed);
    // Apply incrementally
    ...
    SendMessage(m_editorHwnd, EM_SETSEL, (WPARAM)(cmd.pos + cmd.removed.size()),
                (LPARAM)(cmd.pos + cmd.removed.size()));
}
...
void MainWindow::performRedo()
{
    ...
    if (!cmd.removed.empty())
        currentBuffer().erase(cmd.pos, cmd.removed.size());
    if (!cmd.inserted.empty())
        currentBuffer().insert(cmd.pos, cmd.inserted);
    ...
    SendMessage(m_editorHwnd, EM_SETSEL, (WPARAM)(cmd.pos + cmd.inserted.size()),
                (LPARAM)(cmd.pos + cmd.inserted.size()));
}
#ifdef _WIN32
void MainWindow::retokenizeAndApplyColors()
{
    ...
    m_engine.tokenize(text, tokens);
    COLORREF kwColor = RGB(86, 156, 214), numColor = RGB(181, 206, 168), identColor = RGB(212, 212, 212),
             defColor = RGB(212, 212, 212);
    if (m_currentTheme < m_themes.size())
    {
    ...
    for (const auto& tk : tokens)
    {
        ...
        cf.dwMask = CFM_COLOR;
        if (tk.type == 5)
            cf.crTextColor = cmtColor;
        else if (tk.type == 4)
            cf.crTextColor = strColor;
        else if (tk.type == 3)
            cf.crTextColor = kwColor;
        else if (tk.type == 1)
            cf.crTextColor = numColor;
        else if (tk.type == 2)
            cf.crTextColor = identColor;
        else
            cf.crTextColor = defColor;
        SendMessageA(m_editorHwnd, EM_SETCHARFORMAT, SCF_SELECTION, (LPARAM)&cf);
    }
    DWORD selStart = 0, selEnd = 0;
    SendMessage(m_editorHwnd, EM_GETSEL, (WPARAM)&selStart, (LPARAM)&selEnd);
...
void MainWindow::switchTab(size_t index)
{
    if (index >= m_tabs.size())
        return;
    m_currentTab = index;
    selectLanguageForFile(m_tabs[index].filename);
    syncEditorFromBuffer();
    refreshTabBar();
}
...
void MainWindow::closeTab(size_t index)
{
    ...
    if (m_currentTab >= m_tabs.size())
        m_currentTab = m_tabs.empty() ? 0 : m_tabs.size() - 1;
    refreshTabBar();
    syncEditorFromBuffer();
}
...
void MainWindow::refreshTabBar()
{
    ...
    for (size_t i = 0; i < m_tabs.size(); ++i)
    {
        ...
        HWND btn = CreateWindowExA(0, "BUTTON", label.c_str(), WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, x, 0, btnWidth,
                                   height, m_tabBarHwnd, (HMENU)(2000 + i), inst, nullptr);
        m_tabButtons.push_back(btn);
        x += btnWidth + spacing;
    }
    CreateWindowExA(0, "BUTTON", "+", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, x, 0, 24, height, m_tabBarHwnd,
                    (HMENU)1999, inst, nullptr);
}
...
{
    if (endsWith(filename, ".cpp") || endsWith(filename, ".hpp") || endsWith(filename, ".h") ||
        endsWith(filename, ".c"))
    {
...
{
#ifdef _WIN32
    if (!m_statusBarHwnd || !m_editorHwnd || m_tabs.empty())
        return;
    DWORD selStart = 0, selEnd = 0;
    SendMessage(m_editorHwnd, EM_GETSEL, (WPARAM)&selStart, (LPARAM)&selEnd);
    ...
    if (fname.ends_with(".cpp") || fname.ends_with(".hpp") || fname.ends_with(".h") || fname.ends_with(".c"))
        lang = "cpp";
    else if (fname.ends_with(".ps1") || fname.ends_with(".psm1"))
        lang = "powershell";
    std::stringstream sb;
    sb << fname << (m_tabs[m_currentTab].dirty ? "*" : "") << "  Ln " << (line + 1) << " Col " << col << "  " << lang;
...
void MainWindow::saveAllDirtyTabs()
{
    for (size_t i = 0; i < m_tabs.size(); ++i)
        if (m_tabs[i].dirty)
            saveTab(i);
...
void MainWindow::loadGGUFModel()
{
    ...
    try
    {
        ...
        if (m_ggufLoader && m_ggufLoader->isOpen())
        {
            ...
            {
                sendToTerminal(
                    "[Model Loader] Warning: Inference Engine failed to load weights. Chat will not work.\n");
...
void MainWindow::reloadCurrentModel()
{
    ...
    // Brief yield so unload teardown can finish (keep UI responsive)
    Sleep(32);
...
{
#ifdef _WIN32
    if (m_commandPaletteHwnd)
        return;
    m_commandPaletteHwnd =
        CreateWindowExA(WS_EX_TOPMOST | WS_EX_TOOLWINDOW, "LISTBOX", "", WS_POPUP | WS_BORDER | LBS_NOTIFY, 620, 120,
                        240, 190, m_hwnd, (HMENU)3001, GetModuleHandle(nullptr), nullptr);
...
void MainWindow::populateCommandPalette()
{
    ...
    try
    {
        ...
        bool isCpp =
            fname.ends_with(".cpp") || fname.ends_with(".hpp") || fname.ends_with(".h") || fname.ends_with(".c");
    ...
    catch (const std::exception& e)
    {
        ...
        allTools = {"read_file", "write_file", "replace_in_file", "execute_command", "search_code", "get_diagnostics"};
    }
    ...
    std::sort(displayList.begin(), displayList.end(), [](const auto& a, const auto& b) { return a.second < b.second; });
...
void MainWindow::toggleCommandPalette()
{
    ...
    {
        populateCommandPalette();
        ShowWindow(m_commandPaletteHwnd, SW_SHOW);
    }
#endif
}
...
void MainWindow::executePaletteSelection(int index)
{
    ...
    if (index < 0 || index >= (int)m_paletteTool.size() || !m_commandPaletteHwnd)
        return;
    ...
    if (toolName.find("Quick:") == 0)
    {
        ...
        else if (toolName == "Quick: Toggle Header/Source")
        {
            if (!m_tabs.empty())
            {
                ...
                std::string alt;
                namespace fs = std::filesystem;
                if (cur.size() > 4 && cur.substr(cur.size() - 4) == ".cpp")
                    alt = cur.substr(0, cur.size() - 4) + ".h";
                else if (cur.size() > 2 && cur.substr(cur.size() - 2) == ".h")
                    alt = cur.substr(0, cur.size() - 2) + ".cpp";
                ...
                {
                    if (cur.size() > 4 && cur.substr(cur.size() - 4) == ".cpp")
                        alt = cur.substr(0, cur.size() - 4) + ".hpp";
                    if (!alt.empty() && fs::exists(alt))
                        addTab(alt);
                }
            }
        ...
        else if (toolName == "Quick: Format Script")
        {
            if (!m_tabs.empty())
            {
                ...
                {
                    handleCommand("powershell -NoProfile -Command \"& { if (Get-Command Invoke-ScriptAnalyzer "
                                  "-ErrorAction SilentlyContinue) { Invoke-ScriptAnalyzer -Path '" +
                                  fn + "' -Fix } else { Write-Host 'Install PSScriptAnalyzer for formatting' } }\"");
                }
            }
        }
...
void MainWindow::loadSettings()
{
    ...
    auto findVal = [&](const std::string& key) -> std::string
    {
        ...
        if (c == std::string::npos)
            return "";
        auto q = content.find_first_of(",}", c + 1);
        return content.substr(c + 1, q - (c + 1));
    };
    std::string theme = findVal("theme");
    if (!theme.empty())
    {
    ...
    std::string fontSize = findVal("fontSize");
    if (!fontSize.empty())
        m_fontSize = atoi(fontSize.c_str());
    std::string tabSz = findVal("tabSize");
    if (!tabSz.empty())
        m_tabSize = atoi(tabSz.c_str());
    std::string secondarySidebarVisible = findVal("secondarySidebarVisible");
    if (!secondarySidebarVisible.empty())
    {
        m_secondarySidebarVisible = (secondarySidebarVisible.find("true") != std::string::npos ||
                                     secondarySidebarVisible.find("1") != std::string::npos);
    }
    std::string statusBarVisible = findVal("statusBarVisible");
    if (!statusBarVisible.empty())
    {
        m_statusBarVisible =
            (statusBarVisible.find("true") != std::string::npos || statusBarVisible.find("1") != std::string::npos);
    }
    ...
    {
        g_agentDiffPanelVisible = (agentDiffPanelVisible.find("true") != std::string::npos ||
                                   agentDiffPanelVisible.find("1") != std::string::npos);
    }
    std::string debuggerContainerVisible = findVal("debuggerContainerVisible");
    if (!debuggerContainerVisible.empty())
    {
        g_debuggerContainerVisible = (debuggerContainerVisible.find("true") != std::string::npos ||
                                      debuggerContainerVisible.find("1") != std::string::npos);
    }
    ...
    if (watchKey != std::string::npos)
    {
        ...
        if (lb != std::string::npos && rb != std::string::npos && rb > lb)
        {
            ...
            while (pos < raw.size())
            {
                ...
                if (!item.empty() && item.size() <= 256 && g_debugWatchExpressions.size() < 128)
                    g_debugWatchExpressions.push_back(item);
                pos = q2 + 1;
            }
        }
    }
...
void MainWindow::createFloatingPanel()
{
    ...
    wc.hCursor = LoadCursor(nullptr, IDC_ARROW);
    RegisterClass(&wc);
    m_floatingPanel = CreateWindowEx(WS_EX_TOPMOST | WS_EX_TOOLWINDOW, "RawrXDFloatPanel", "RawrXD Panel",
                                     WS_POPUP | WS_BORDER | WS_SYSMENU, 820, 10, 260, 260, m_hwnd, nullptr,
