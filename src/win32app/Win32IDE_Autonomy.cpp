...
AutonomyManager::AutonomyManager(AgenticBridge* bridge)
    : m_bridge(bridge), m_running(false), m_autoLoop(false), m_maxActionsPerMinute(30), m_actionsThisWindow(0)
{
    // Default no-op output callback — prevents silent drop when setOutputCallback() hasn't
...
std::string AutonomyManager::planNextAction()
{
    ...
    if (m_memory.empty())
    {
        ...
        return "prompt: You are an autonomous agent. Your Goal: " + m_goal +
               ...
               "- TOOL:hexmag_swarm:{\"prompts\":[\"task1\",\"task2\"],\"strategy\":\"concatenate\",\"maxParallel\":4} "
               "(parallel fan-out)\n"
    ...
    if (lastObs.rfind("ANSWER:", 0) == 0)
    {
        ...
        return "prompt: Please format your next step as a single tool command starting with 'tool:'. If done, say "
               "'TASK_COMPLETE'.";
    }
