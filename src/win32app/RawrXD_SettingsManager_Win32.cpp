 ...
 * Pure Win32 replacement for Qt QSettings (integrated into Win32 IDE)
...
// RawrXD Settings and Default Shell Implementation
...
public:
    ...
    bool GetString(const wchar_t* key, wchar_t* buffer, size_t bufSize) {
        if (!m_rootKey || !buffer || bufSize == 0) return false;
...
#else
            RegDeleteKeyW(HKEY_CURRENT_USER, m_appKey);
#endif
