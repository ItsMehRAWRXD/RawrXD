...
// Member function wrapper — delegates to the file-local static helper
bool Win32IDE::asmIsAsmFile(const std::string& filePath) const {
    size_t dot = filePath.rfind('.');
    if (dot == std::string::npos) return false;
    std::string ext = asmToLower(filePath.substr(dot));
    return (ext == ".asm" || ext == ".s" || ext == ".nasm" ||
            ext == ".masm" || ext == ".inc" || ext == ".mac");
}
