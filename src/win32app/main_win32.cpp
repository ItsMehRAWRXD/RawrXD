...
static int WinMainImpl(HINSTANCE hInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    for (const std::string& name : quickPhases)
    {
        ...
        if (!runPhase(name, ide, hInstance, lpCmdLine))
        {
            ...
            // Graceful fallback: if window creation failed, do NOT call MessageBoxW
            // unconditionally — a non-interactive window station (headless terminal,
            // service context, SSH session, etc.) will cause MessageBoxW to corrupt
            // the stack and trigger /GS abort (0xC0000409).
            // Instead: log to stderr and exit cleanly for all contexts.
            if (name == "createWindow")
            {
                fprintf(stderr, "[main_win32] createWindow failed — no interactive window station. "
                                "Run with --headless for terminal/server mode.\n");
                return 2; // distinct exit code for headless-required
            }
