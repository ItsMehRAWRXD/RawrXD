...
#include "../core/ide_hybrid_opt_in.hpp"
...
void Win32IDE::appendText(HWND hwnd, const std::string& text)
{
    ...
    SETTEXTEX st;
    st.flags = ST_SELECTION;  // append at caret — ST_DEFAULT replaces entire buffer
...
bool Win32IDE::loadGGUFModel(const std::string& filepath)
{
    ...
    info += RawrXD::IdeHybrid::composeStatusLine(true, m_nativeEngine != nullptr) + "\n\n";
...
void Win32IDE::generateResponseAsync(const std::string& prompt, std::function<void(const std::string&, bool)> callback)
{
    ...
    appendToOutput(
        RawrXD::IdeHybrid::composeStatusLine(m_ggufLoader != nullptr, m_nativeEngine != nullptr) + "\n",
        "Output", OutputSeverity::Info);
