...
{
    if (m_lineEndingSelectorInitialized)
        return;
...
bool Win32IDE::handleLineEndingCommand(int commandId)
{
    ...
    if (content.empty() && commandId != IDM_LINEENDING_DETECT)
        return true;
    ...
    else if (commandId == 12202)
    {
        newEol = "\n";
        mode = 1;
    }
    else if (commandId == 12203)
    {
        newEol = "\r";
        mode = 2;
    }
    if (!newEol)
        return true;  // other IDs in range: no convert
...
bool Win32IDE::handleDebugWatchCommand(int commandId)
{
    ...
    if (commandId == IDM_DBGWATCH_SHOW)
    {
        ...
        {
            std::string msg = "[Debug Watch] " + std::to_string(m_watchList.size()) +
                              " watch expression(s). Attach debugger to see Watch panel.\n";
...
{
    if (m_callStackSymbolsInitialized)
        return;
...
bool Win32IDE::handleCallStackCommand(int commandId)
{
    ...
    if (commandId == IDM_CALLSTACK_RESOLVE)
        oss << " (symbols resolved)";
    oss << "\n";
