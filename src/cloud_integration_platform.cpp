...
void CloudOrchestrator::addProvider(CloudProviderType provider, const CloudConfig& config)
{
    switch (provider) {
        case AWS:
...
bool CloudOrchestrator::deployMultiCloud(const DeploymentConfig& config, const QVector<CloudProviderType>& providers)
{
    qInfo() << "[CloudOrchestrator] Deploying to" << providers.count() << "cloud providers";
    emit deploymentInitiated(config.applicationName);
    
    for (CloudProviderType provider : providers) {
...
QString CloudUtils::getProviderString(CloudProviderType provider)
{
    switch (provider) {
        case AWS: return "AWS";
...
CloudProviderType CloudUtils::getProviderFromString(const QString& providerString)
{
...
QVector<QString> CloudUtils::getAvailableRegions(CloudProviderType provider)
{
...
QString CloudUtils::selectOptimalRegion(CloudProviderType provider, const QVector<QString>& requirements)
{
