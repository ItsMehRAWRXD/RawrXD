...
bool CodebaseAuditSystem::enable_continuous_monitoring(const std::string& project_path,
                                                       std::chrono::minutes monitoring_interval) {
    if (!initialized_.load() || project_path.empty()) return false;
    std::lock_guard<std::mutex> lock(monitoring_mutex_);
    monitored_projects_[project_path] = monitoring_interval;
    last_monitor_run_[project_path] = std::chrono::system_clock::time_point{};
    if (!continuous_monitoring_enabled_.load()) {
        continuous_monitoring_enabled_ = true;
        if (monitoring_thread_.joinable()) monitoring_thread_.join();
        monitoring_thread_ = std::thread(&CodebaseAuditSystem::monitoring_thread_function, this);
    }
    std::cout << "[CodebaseAudit] Monitoring enabled for " << project_path << " (interval " << monitoring_interval.count() << " min)" << std::endl;
    return true;
}

void CodebaseAuditSystem::disable_continuous_monitoring(const std::string& project_path) {
    std::lock_guard<std::mutex> lock(monitoring_mutex_);
    if (project_path.empty()) {
        monitored_projects_.clear();
        last_monitor_run_.clear();
        project_signature_.clear();
    } else {
        monitored_projects_.erase(project_path);
        last_monitor_run_.erase(project_path);
        project_signature_.erase(project_path);
    }
}

uint64_t CodebaseAuditSystem::compute_project_signature(const std::string& project_path) const {
        ...
        for (const auto& entry : fs::recursive_directory_iterator(root, fs::directory_options::skip_permission_denied)) {
            if (entry.is_regular_file()) {
                ...
                try {
                    ...
                    uint64_t t = static_cast<uint64_t>(mtime.time_since_epoch().count());
                    if (t > max_time) max_time = t;
                } catch (const fs::filesystem_error&) { /* skip */ }
            }
        }
    } catch (const fs::filesystem_error&) {
        return 0;
    }
    return (count << 32) | (max_time & 0xFFFFFFFFu);
}

bool CodebaseAuditSystem::has_project_changed(const std::string& project_path) {
    uint64_t current = compute_project_signature(project_path);
    std::lock_guard<std::mutex> lock(monitoring_mutex_);
    auto it = project_signature_.find(project_path);
    if (it != project_signature_.end() && it->second == current)
        return false;
    return true;
}

void CodebaseAuditSystem::monitor_project(const std::string& project_path) {
    if (!initialized_.load() || shutting_down_.load()) return;
    if (!has_project_changed(project_path)) return;
    try {
        ProjectAuditResult result = audit_full_project(project_path, "");
        update_statistics(result);
        uint64_t sig = compute_project_signature(project_path);
        std::lock_guard<std::mutex> lock(monitoring_mutex_);
        project_signature_[project_path] = sig;
    } catch (const std::exception& e) {
        std::cerr << "[CodebaseAudit] Monitor audit failed for " << project_path << ": " << e.what() << std::endl;
    }
}

void CodebaseAuditSystem::monitoring_thread_function() {
    const auto kPollInterval = std::chrono::minutes(1);
    while (continuous_monitoring_enabled_.load()) {
        std::this_thread::sleep_for(kPollInterval);
        if (shutting_down_.load()) break;
        std::map<std::string, std::chrono::minutes> copy;
        {
            std::lock_guard<std::mutex> lock(monitoring_mutex_);
            if (monitored_projects_.empty()) continue;
            copy = monitored_projects_;
        }
        auto now = std::chrono::system_clock::now();
        for (const auto& [path, interval] : copy) {
            if (shutting_down_.load()) break;
            std::chrono::system_clock::time_point last;
            {
                std::lock_guard<std::mutex> lock(monitoring_mutex_);
                auto it = last_monitor_run_.find(path);
                last = (it != last_monitor_run_.end()) ? it->second : std::chrono::system_clock::time_point{};
            }
            if (last.time_since_epoch().count() == 0 || (now - last) >= interval) {
                monitor_project(path);
                std::lock_guard<std::mutex> lock(monitoring_mutex_);
                last_monitor_run_[path] = now;
            }
        }
    }
}
...
bool CodebaseAuditSystem::is_cache_valid(const std::string& file_path) {
    ...
    if (std::chrono::system_clock::now() - timestamp_it->second > cache_ttl)
        return false;
    try {
