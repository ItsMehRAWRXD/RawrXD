...
public:
    std::string infer(const AgentRequest& req) override {
        // ---- Validate model state ----
        if (!m_loaded || !m_loader || m_loader->tensors.empty()) {
            return "[Sovereign-Small] Error: No model loaded. Call load_model() first.";
        }
        ...
        if (embed_dim == 0) {
            for (auto& t : m_loader->tensors) {
                if (!t.dims.empty()) {
                    size_t last = (size_t)t.dims.back();
                    if (last > embed_dim && last <= 8192) embed_dim = last;
                }
            }
        }
        if (embed_dim == 0) embed_dim = 2048; // smaller default for lightweight model
        ...
        // ---- Forward pass (lightweight — fewer layers for speed) ----
        size_t layers_processed = 0;
        for (auto& t : m_loader->tensors) {
            if (!t.data || t.size == 0) continue;
            if (t.name == "token_embd.weight") continue;
        ...
        if (out_norm && out_norm->data) {
            ...
            dequant_tensor(out_norm, norm_w.data(), embed_dim);
            rms_norm(activations.data(), embed_dim);
            for (size_t i = 0; i < embed_dim; i++) activations[i] *= norm_w[i];
        } else {
            rms_norm(activations.data(), embed_dim);
        }
        ...
        // ---- Decode — conservative settings for small model ----
        ...
        std::string prefix = "[Sovereign-Small]";

        if (decoded.empty()) {
            char diag[512];
            snprintf(diag, sizeof(diag),
                     "%s Inference complete: %zu tensors, %zu layers, "
                     "embed_dim=%zu, %.1fms. No decodable output.",
                     prefix.c_str(), m_loader->tensors.size(),
                     layers_processed, embed_dim, elapsed_ms);
            return std::string(diag);
        }
