...
PyreTensor PyreTensor::allocate(PyreDataType dtype, uint64_t d0, uint64_t d1, uint64_t d2, uint64_t d3)
{
    PyreTensor t{};
    t.dtype = dtype;
    t.dims[0] = d0;
...
PyreTensor PyreTensor::wrap(void* rawData, PyreDataType dtype, uint64_t d0, uint64_t d1, uint64_t d2, uint64_t d3)
{
    PyreTensor t{};
    t.data = rawData;
    t.dtype = dtype;
...
static void gemm_fp32(const float* A, const float* B, float* C, uint32_t M, uint32_t N, uint32_t K)
{
...
// GEMV: y[M] = A[M×K] × x[K]
static void gemv_fp32(const float* A, const float* x, float* y, uint32_t M, uint32_t K)
{
...
static void rmsnorm(const float* input, const float* weight, float* output, uint32_t dim, float eps)
{
...
static void rope(float* data, uint32_t seqLen, uint32_t headDim, uint32_t seqOffset, float theta)
{
...
static void embedding_lookup(const float* table, const uint32_t* ids, float* output, uint32_t count, uint32_t dim)
{
    for (uint32_t i = 0; i < count; i++)
    {
...
PyreGraph::PyreGraph()
    : m_hFile(INVALID_HANDLE_VALUE), m_hMapping(NULL), m_pMappedView(nullptr), m_mappedSize(0), m_initialized(false),
      m_modelLoaded(false), m_graphBuilt(false)
{
...
PatchResult PyreGraph::shutdown()
{
    std::lock_guard<std::mutex> lock(m_mutex);
    if (!m_initialized)
        return PatchResult::ok("PyreGraph not initialized");
...
PatchResult PyreGraph::loadModel(const char* filepath)
{
    std::lock_guard<std::mutex> lock(m_mutex);
    if (!m_initialized)
        return PatchResult::error("PyreGraph not initialized", -1);
    if (!filepath || filepath[0] == '\0')
        return PatchResult::error("Empty model path", -1);

    // Open file with memory mapping
    m_hFile = CreateFileA(filepath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
                          FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
    if (m_hFile == INVALID_HANDLE_VALUE)
    {
        char msg[512];
        snprintf(msg, sizeof(msg), "Failed to open model file: %s (error %lu)", filepath, GetLastError());
    ...
    {
        char msg[256];
        snprintf(msg, sizeof(msg), "Invalid Pyre magic: expected 0x%08X, got 0x%08X", PYRE_MAGIC, m_header.magic);
        UnmapViewOfFile(m_pMappedView);
        m_pMappedView = nullptr;
        CloseHandle(m_hMapping);
        m_hMapping = NULL;
    ...
    for (uint32_t i = 0; i < m_header.numTensors; i++)
    {
        ...
        PyreTensor tensor =
            PyreTensor::wrap(tensorData, entry.dtype, entry.dims[0], entry.dims[1], entry.dims[2], entry.dims[3]);
    ...
    // Track peak memory
    uint64_t totalAlloc = m_hiddenState.byteSize + m_residual.byteSize + m_normOut.byteSize + m_attnOut.byteSize +
                          m_ffnGate.byteSize + m_ffnUp.byteSize + m_ffnDown.byteSize + m_qBuf.byteSize +
    ...
    // Optional layer offload (separate mmap of the same weights file). Enable with
    // RAWRXD_ENABLE_LAYER_OFFLOAD=1 or set RAWRXD_SPILL_ROOT for spill staging.
    {
        auto& om = RawrXD::LayerOffloadManager::instance();
        if (!om.isInitialized())
        {
            const char* spill = std::getenv("RAWRXD_SPILL_ROOT");
            const char* en = std::getenv("RAWRXD_ENABLE_LAYER_OFFLOAD");
            const bool want = (spill && spill[0]) ||
                              (en && (en[0] == '1' || en[0] == 'y' || en[0] == 'Y' || en[0] == 't' || en[0] == 'T'));
            if (want)
            {
                RawrXD::OffloadConfig cfg =
                    RawrXD::OffloadConfig::withEnvironmentOverrides(RawrXD::OffloadConfig::defaultConfig());
                (void)om.initialize(filepath, m_config, cfg);
            }
        }
    }

    char msg[512];
    snprintf(msg, sizeof(msg), "Pyre model loaded: %u tensors, %u layers, hidden=%u, heads=%u, vocab=%u, seq=%u",
             m_header.numTensors, m_config.numLayers, m_config.hiddenDim, m_config.numHeads, m_config.vocabSize,
             m_config.maxSeqLen);
...
bool PyreGraph::isModelLoaded() const
{
    return m_modelLoaded;
}
const PyreLayerConfig& PyreGraph::getConfig() const
{
    return m_config;
}
...
PatchResult PyreGraph::buildGraph()
{
    std::lock_guard<std::mutex> lock(m_mutex);
    if (m_ops.empty())
        return PatchResult::error("No ops in graph", -1);
    ...
    for (const auto& op : m_ops)
    {
        ...
        {
            char msg[256];
            snprintf(msg, sizeof(msg), "Op '%s' references invalid output tensor %u", op.debugName ? op.debugName : "?",
                     op.outputIndex);
            return PatchResult::error(msg, -1);
        }
    }
...
PatchResult PyreGraph::execMatMul(PyreTensor& out, const PyreTensor& A, const PyreTensor& B)
{
    ...
    if (rc != 0)
        return PatchResult::error("ASM GEMM failed", rc);
#else
...
PatchResult PyreGraph::execRMSNorm(PyreTensor& out, const PyreTensor& input, const PyreTensor& weight, float eps)
{
...
{
    if (!inout.isValid())
        return PatchResult::error("SiLU: invalid tensor", -1);
...
{
    if (!inout.isValid())
        return PatchResult::error("Softmax: invalid tensor", -1);
...
{
    if (!inout.isValid())
        return PatchResult::error("RoPE: invalid tensor", -1);
...
PatchResult PyreGraph::execEmbeddingLookup(PyreTensor& out, const PyreTensor& table, const uint32_t* ids,
                                           uint32_t count)
{
...
PatchResult PyreGraph::forward(const uint32_t* tokenIds, uint32_t seqLen, float* logitsOut)
{
    ...
    if (!m_initialized)
        return PatchResult::error("PyreGraph not initialized", -1);
    if (!m_modelLoaded)
        return PatchResult::error("No model loaded", -1);
...
PatchResult PyreGraph::forwardEmbedding(const uint32_t* tokenIds, uint32_t seqLen)
{
    ...
    if (!embedWeight)
        return PatchResult::error("Missing embedding weight tensor", -1);
...
PatchResult PyreGraph::forwardTransformerLayer(uint32_t layerIdx)
{
    ...
    if (wQ && wK && wV)
    {
        ...
        {
            // RoPE on Q head
            PyreTensor qHead = PyreTensor::wrap(m_qBuf.fp32() + h * headDim, PyreDataType::FP32, seqLen, headDim);
            r = execRoPE(qHead, 0, theta);
            if (!r.success)
                return r;

            // RoPE on K head
            PyreTensor kHead = PyreTensor::wrap(m_kBuf.fp32() + h * headDim, PyreDataType::FP32, seqLen, headDim);
...
PatchResult PyreGraph::forwardFinalNorm()
{
    ...
    if (!normW)
        return PatchResult::ok("No final norm weight — skipped");
...
PatchResult PyreGraph::forwardLogits(float* logitsOut, uint32_t seqLen)
{
    ...
    if (!lmHead)
    {
        ...
        if (!lmHead)
            return PatchResult::error("Missing lm_head / embed_tokens weight for logits", -1);
    }
...
size_t PyreGraph::dumpDiagnostics(char* buffer, size_t bufferSize) const
{
    ...
    int written = snprintf(buffer, bufferSize,
                           ...
                           "  RMSNorm EPS:      %.2e\n"
                           "  RoPE Theta:       %.1f\n",
                           m_initialized ? "YES" : "NO", m_modelLoaded ? "YES" : "NO", m_tensors.size(), m_ops.size(),
                           m_graphBuilt ? "YES" : "NO", m_stats.totalForwardPasses, m_stats.totalOpsExecuted,
                           m_stats.totalGemmCycles, m_stats.totalNormCycles, m_stats.totalActivationCycles,
                           m_stats.totalSoftmaxCycles, m_stats.peakMemoryBytes,
                           static_cast<double>(m_stats.peakMemoryBytes) / (1024.0 * 1024.0), m_stats.currentMemoryBytes,
                           static_cast<double>(m_stats.currentMemoryBytes) / (1024.0 * 1024.0), m_stats.lastForwardMs,
                           m_stats.avgTokensPerSec, m_config.hiddenDim, m_config.numHeads, m_config.numKVHeads,
                           m_config.headDim, m_config.intermediateSize, m_config.vocabSize, m_config.maxSeqLen,
