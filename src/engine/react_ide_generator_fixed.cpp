...
std::string ReactIDEGenerator::GenerateViteConfig() {
    return R"RAW(import { defineConfig } from 'vite'
...
std::string ReactIDEGenerator::GenerateMainTsx() {
    return R"RAW(import React from 'react'
...
std::string ReactIDEGenerator::GenerateEngineBridge() {
    return R"RAW(import { create } from 'zustand'
...
std::string ReactIDEGenerator::GenerateVSIXLoaderPanel() {
    return R"RAW(import React, { useState } from 'react';
import { useEngineStore } from '@/lib/engine-bridge';
...
std::string ReactIDEGenerator::GenerateAppTsx(const ReactIDETemplate& tmpl) {
    std::stringstream imports;
    std::stringstream components;

    imports << "import React, { useEffect, useState } from 'react';\n";
    imports << "import { useEngineStore } from '@/lib/engine-bridge';\n";
    imports << "import { Terminal, Play, Save } from 'lucide-react';\n";
    imports << "import Editor from '@monaco-editor/react';\n";
    ...
    // Detect features
    for (const auto& f : tmpl.features) {
        if (f == "Memory Manager") hasMemory = true;
        if (f == "Hot Patcher") hasPatch = true;
        if (f == "VSIX Loader") hasVSIX = true;
        if (f == "Model Inference") hasModel = true;
        if (f == "Chat Interface") hasChat = true;
    }

    if (hasMemory) imports << "import { MemoryPanel } from '@/components/MemoryPanel';\n";
    if (hasPatch) imports << "import { HotPatchPanel } from '@/components/HotPatchPanel';\n";
    if (hasVSIX) imports << "import { VSIXLoaderPanel } from '@/components/VSIXLoaderPanel';\n";
    if (hasModel) imports << "import { ModelLoaderPanel } from '@/components/ModelLoaderPanel';\n";
    if (hasChat) imports << "import { ChatInterface } from '@/components/ChatInterface';\n";

    return imports.str() + R"RAW(
function App() {
  const { connect, isConnected, logs, executeCommand } = useEngineStore();
  const [code, setCode] = useState('// RawrXD IDE Ready\n\n#include <iostream>\n\nint main() {\n    std::cout << "Hello World" << std::endl;\n    return 0;\n}');
  ...
  const handleRun = async () => {
    await executeCommand(`!run_code ${btoa(code)}`);
  };
  ...
  return (
    <div className="h-screen w-screen bg-background text-foreground flex overflow-hidden">
      ...
      <div className="w-16 flex-none bg-secondary border-r border-border flex flex-col items-center py-4 space-y-4">
        <div className="w-10 h-10 rounded-lg bg-primary flex items-center justify-center font-bold text-white shadow-lg shadow-primary/20">RX</div>
        <div className="flex-1" />
        <div className={`w-3 h-3 rounded-full ${isConnected ? 'bg-green-500' : 'bg-red-500'} transition-colors duration-500`} title={isConnected ? "Connected" : "Disconnected"} />
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-12 border-b border-border flex items-center px-4 justify-between bg-card">
          <div className="flex items-center gap-4">
             ...
             <div className="flex items-center gap-2">
                 <button onClick={handleRun} className="flex items-center gap-1 bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded text-xs font-medium transition-colors">
                     <Play className="w-3 h-3" /> Run
                 </button>
                 <button className="p-1 hover:bg-secondary rounded"><Save className="w-4 h-4 text-muted-foreground" /></button>
             </div>
          </div>
        ...
        <main className="flex-1 overflow-hidden grid grid-cols-12 auto-rows-fr">
          ...
          <div className="col-span-8 border-r border-border bg-[#1e1e1e] flex flex-col">
             <Editor
               height="100%"
               defaultLanguage="cpp"
               defaultValue="// RawrXD Engine Ready"
               theme="vs-dark"
               value={code}
             ...
             )" + (hasChat ? "<div className=\"h-[40%] border-t border-border\"><ChatInterface /></div>" : "") + R"RAW(
          </div>

          {/* Right Panel - Tools */}
          <div className="col-span-4 flex flex-col bg-background">
            <div className="flex-1 overflow-y-auto border-b border-border">
              )" + (hasMemory ? "<MemoryPanel />\n              <div className=\"h-px bg-border my-2\" />\n" : "") + 
                   (hasPatch ? "<HotPatchPanel />\n              <div className=\"h-px bg-border my-2\" />\n" : "") +
                   (hasVSIX ? "<VSIXLoaderPanel />\n              <div className=\"h-px bg-border my-2\" />\n" : "") +
                   (hasModel ? "<ModelLoaderPanel />\n" : "") + R"RAW(
            </div>
...
bool ReactIDEGenerator::GenerateIDE(const std::string& name, const std::string& template_name,
    ...
    WriteFile(project_path / "tsconfig.node.json", R"RAW({
  "compilerOptions": {
    "composite": true,
    "skipLibCheck": true,
    "module": "ESNext",
  ...
  "include": ["vite.config.ts"]
    ...
    WriteFile(project_path / "src" / "App.tsx", GenerateAppTsx(tmpl));
    WriteFile(project_path / "src" / "index.css", R"RAW(@tailwind base;
@tailwind components;
@tailwind utilities;
...
@layer base {
  * {
  ...
  body {
    ...
    // Feature Components
    for (const auto& f : tmpl.features) {
        if (f == "Memory Manager") WriteFile(project_path / "src" / "components" / "MemoryPanel.tsx", GenerateMemoryPanel());
        if (f == "Hot Patcher") WriteFile(project_path / "src" / "components" / "HotPatchPanel.tsx", GenerateHotPatchPanel());
        if (f == "VSIX Loader") WriteFile(project_path / "src" / "components" / "VSIXLoaderPanel.tsx", GenerateVSIXLoaderPanel());
        if (f == "Model Inference") WriteFile(project_path / "src" / "components" / "ModelLoaderPanel.tsx", GenerateModelLoaderPanel());
        if (f == "Chat Interface") WriteFile(project_path / "src" / "components" / "ChatInterface.tsx", GenerateChatInterface());
    }
    
    // Always generate Settings (future use)
    WriteFile(project_path / "src" / "components" / "SettingsPanel.tsx", GenerateSettingsPanel());

    // UI Components (Minimal shadcn abstraction)
    WriteFile(project_path / "src" / "components" / "ui" / "card.tsx", R"RAW(import * as React from "react"
...
export { Card, CardHeader, CardTitle, CardContent }
    ...
    // Utils
    WriteFile(project_path / "src" / "lib" / "utils.ts", R"RAW(import { type ClassValue, clsx } from "clsx"
...
std::string ReactIDEGenerator::GenerateChatInterface() {
    return R"RAW(import React, { useState, useRef, useEffect } from 'react';
import { useEngineStore } from '@/lib/engine-bridge';
