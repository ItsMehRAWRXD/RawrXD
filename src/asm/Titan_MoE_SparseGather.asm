...
;   Out: RAX = context handle (= config pointer) or NULL if invalid
; ============================================================================
SparseGather_Initialize PROC EXPORT
    ; Validate config pointer
    test    rcx, rcx
    jz      SG_init_fail
    ; Validate weights_base (must be non-null)
    mov     rax, QWORD PTR [rcx + MOECONFIG_WEIGHTS_BASE]
    test    rax, rax
    jz      SG_init_fail
...
;   Completely safe version - just returns zeros without touching memory
; ============================================================================
SparseGather_GetStats PROC EXPORT FRAME
    .endprolog
    ; Just return - don't touch any memory to avoid AV
    xor     rax, rax
    ret
