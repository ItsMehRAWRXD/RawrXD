...
void MultiFileSearchWidget::addResultToTree(const MultiFileSearchResult& result) {
    ...
    static thread_local std::wstring wdisplay;
    {
        int len = MultiByteToWideChar(CP_UTF8, 0, display.c_str(), (int)display.size(), nullptr, 0);
        if (len > 0) {
            wdisplay.resize((size_t)len);
            MultiByteToWideChar(CP_UTF8, 0, display.c_str(), (int)display.size(), wdisplay.data(), len);
        } else {
            wdisplay.clear();
        }
    }
    tvis.item.pszText = const_cast<LPWSTR>(wdisplay.c_str());
