...
RawrXD::Expected<CompetitiveResult, AuditError> IDEAuditor::compareWithGitHubCopilot(
    ...
    if (!m_network || !m_network->isInitialized()) {
        return RawrXD::unexpected(AuditError::NetworkUnavailable);
    }
    ...
    for (const auto& testCase : testCases) {
        ...
        // Get our result
        auto futureRes = m_swarm->submitTask(testCase, "");
        auto val = futureRes.get();
        
        if (val.consensus.empty()) {
            continue;
        }
        ...
        auto ourTokens = m_tokenizer->encode(val.consensus);
        ...
        // Calculate accuracy
        float accuracy = calculateSemanticSimilarity(theirResult, val.consensus);
        result.theirResult.codeCompletionAccuracy += accuracy;
        result.ourResult.codeCompletionAccuracy += val.confidence;
    }
...
RawrXD::Expected<CodeMetrics, AuditError> IDEAuditor::analyzeCodebase(const std::string& path) {
    ...
    if (!fs::exists(path)) {
        return RawrXD::unexpected(AuditError::ComponentNotFound);
    }
