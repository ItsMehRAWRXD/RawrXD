...
struct ToolMetadata {
    ...
    // json inputSchema; // Legacy/Alternative
    // json outputSchema;
    std::vector<ToolArgument> arguments; // Powerful argument validation
...
struct ToolResult {
    bool success = false;
    // json data; // Legacy
    std::string output; // Aligned with cpp usage
    std::string error;
    int statusCode = 0;
    double durationMs = 0.0;
    ...
    int retryCount = 0;
    json data; // Keep for now
};

using ToolExecutor = std::function<ToolResult(const std::map<std::string, std::string>&, const ToolContext&)>;

struct ToolDefinition {
    ToolMetadata metadata;
    ToolConfig config;
    ToolHandler handler; // Legacy
    ToolExecutor executor; // Strong type
...
// Hook definitions
using PreExecutionHook = std::function<void(const std::string&, const std::map<std::string, std::string>&, const ToolContext&)>;
...
public:
    ...
    std::vector<ToolMetadata> getAllToolMetadata() const;
