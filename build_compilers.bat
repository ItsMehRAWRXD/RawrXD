...
REM Assemble
"%ML64%" /c /Fo"%OBJ_FILE%" "%ASM_FILE%" >nul 2>&1
if errorlevel 1 (
    echo   [ASM FAIL]
    set /a FAIL+=1
    goto :eof
...
REM Link
"%LINK%" /SUBSYSTEM:CONSOLE /ENTRY:start "%OBJ_FILE%" "%SDK_LIB%" /OUT:"%EXE_FILE%" >nul 2>&1
if errorlevel 1 (
    echo   [LINK FAIL]
    set /a FAIL+=1
    goto :eof
