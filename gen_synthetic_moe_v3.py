...
# Metadata (tightly packed, NO alignment between values)
metadata = [
...
# Tensor info table (tightly packed)
tensors = []

def add_tensor(name, shape, dtype):
    tensors.append((name, shape, dtype))

add_tensor('token_embd.weight', [V, H], 0)
...
# Compute data start position (after tensor info)
