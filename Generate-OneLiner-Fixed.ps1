# RawrXD One-Liner Generator - Fixed Version
...
param(
    ...
    [Parameter(Mandatory=$false)]
    [string]$OutputFile = "D:\lazy init ide\OneLiner-AutonomousAgent-Fixed.txt",
...
Write-Host "║                                                                   ║" -ForegroundColor Magenta
Write-Host "║         RawrXD ONE-LINER GENERATOR - FIXED                      ║" -ForegroundColor Magenta
Write-Host "║                    Version 3.0.1 - Production Ready               ║" -ForegroundColor Magenta
Write-Host "║                                                                   ║" -ForegroundColor Magenta
Write-Host "╚═══════════════════════════════════════════════════════════════════╝" -ForegroundColor Magenta
Write-Host ""
...
# Modify script to use $args instead of param block
Write-Host "Modifying script to use `$args..." -ForegroundColor Cyan
try {
    # Remove the param block
    $scriptContent = $scriptContent -replace '(?s)^param\([^\)]+\)\s*', ''
...
Write-Host "Generate-OneLiner-Fixed.ps1" -ForegroundColor Green -NoNewline
