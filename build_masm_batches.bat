...
:: RawrXD MASM64 Batch Build - CMD Version
...
set "SUCCESS_LIST=%TEMP%\rawrxd_success.txt"
...
for /f "usebackq delims=" %%f in ("%FILE_LIST%.filtered") do (
    set /a FILE_INDEX+=1
    set /a FILE_IN_BATCH=((FILE_INDEX-1) %% 15) + 1
    set /a CURRENT_BATCH=(FILE_INDEX-1) / 15 + 1
    
    if !FILE_IN_BATCH! equ 1 (
        if !BATCH_NUM! gtr 1 (
            echo.
            echo Batch !BATCH_NUM! completed - !BATCH_SUCCESS!/15 successful
        )
        set BATCH_NUM=!CURRENT_BATCH!
        set BATCH_SUCCESS=0
        echo.
        echo ========================================
    ...
    <nul set /p =[!BATCH_NUM!/%BATCHES%] (!FILE_IN_BATCH!/15) Compiling: !FILE_NAME! 
