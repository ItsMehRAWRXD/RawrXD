#=============================================================================
# Build_Amphibious_Hybrid.ps1 — Unified CLI: try Ollama first, fallback to simulation
# Links RawrXD_InferenceAPI.asm (hybrid real/sim) instead of RawrXD_ML_Runtime.asm.
# Requires: winhttp.lib for WinHttpSetOption/connect timeout.
#
# Output: build\amphibious\RawrXD_CLI_Hybrid.exe
# Run: .\build\amphibious\RawrXD_CLI_Hybrid.exe
#   - If Ollama is running on 127.0.0.1:11434 → [MODE] real, streams tokens.
#   - If not → [MODE] simulation, emits fallback message (no hang).
#=============================================================================
...
$cliExe = Join-Path $outDir 'RawrXD_CLI_Hybrid.exe'

Write-Host 'Linking hybrid CLI (InferenceAPI + winhttp)' -ForegroundColor Yellow
& $link /NOLOGO /SUBSYSTEM:CONSOLE /ENTRY:Main /OUT:$cliExe $cliObj $commonObjs `
    kernel32.lib user32.lib msvcrt.lib legacy_stdio_definitions.lib ucrt.lib winhttp.lib
if ($LASTEXITCODE -ne 0) { throw 'CLI link failed' }

Write-Host ''
Write-Host 'Amphibious hybrid build complete.' -ForegroundColor Green
Write-Host "  RawrXD_CLI_Hybrid.exe: $cliExe" -ForegroundColor Green
Write-Host '  Tries Ollama (127.0.0.1:11434) with 5s timeout; falls back to simulation if unavailable.' -ForegroundColor Gray
Write-Host '  Run: .\build\amphibious\RawrXD_CLI_Hybrid.exe' -ForegroundColor White
