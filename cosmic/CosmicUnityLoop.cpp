#include "CosmicUnityLoop.hpp"
#include <algorithm>
#include <cmath>

namespace CosmicUnity {

CosmicUnityLoop& g_cosmicUnityLoop = CosmicUnityLoop::GetInstance();

CosmicUnityLoop& CosmicUnityLoop::GetInstance() {
    static CosmicUnityLoop instance;
    return instance;
}

void CosmicUnityLoop::Init() {
    std::lock_guard<std::mutex> lock(mutex_);
    config_ = CosmicUnityConfig();
    targetFrameDuration_ = std::chrono::duration<double, std::milli>(1000.0 / config_.maxFPS);
    ResetMetrics();
}

void CosmicUnityLoop::Shutdown() {
    Stop();
}

void CosmicUnityLoop::Start() {
    {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    tickThread_ = std::make_unique<std::thread>(&CosmicUnityLoop::TickThreadFunc, this);

    if (config_.enableMultiLayerSynchronization) {
        syncThread_ = std::make_unique<std::thread>(&CosmicUnityLoop::SyncThreadFunc, this);
    }

    if (config_.enableCrossLayerHarmonyHarmonization) {
        harmonyThread_ = std::make_unique<std::thread>(&CosmicUnityLoop::HarmonyThreadFunc, this);
    }
}

void CosmicUnityLoop::Stop() {
    shouldStop_ = true;
...
void CosmicUnityLoop::Pause() {
    metrics_.isPaused = true;
}

void CosmicUnityLoop::Resume() {
    metrics_.isPaused = false;
    lastTickTime_ = std::chrono::steady_clock::now();
    lastFrameTime_ = std::chrono::steady_clock::now();
}

void CosmicUnityLoop::SetConfig(const CosmicUnityConfig& config) {
    std::lock_guard<std::mutex> lock(mutex_);
    config_ = config;
    targetFrameDuration_ = std::chrono::duration<double, std::milli>(1000.0 / config_.maxFPS);
}

CosmicUnityConfig CosmicUnityLoop::GetConfig() const {
    std::lock_guard<std::mutex> lock(mutex_);
    return config_;
}

CosmicUnityMetrics CosmicUnityLoop::GetMetrics() const {
    return metrics_;
}

void CosmicUnityLoop::ResetMetrics() {
...
void CosmicUnityLoop::SetTickCallback(CosmicUnityTickCallback callback) {
    std::lock_guard<std::mutex> lock(mutex_);
    tickCallback_ = callback;
}

void CosmicUnityLoop::TickThreadFunc() {
...
void CosmicUnityLoop::SyncThreadFunc() {
    using namespace std::chrono;
...
void CosmicUnityLoop::HarmonyThreadFunc() {
    using namespace std::chrono;
...
void CosmicUnityLoop::CalculateTPS() {
    using namespace std::chrono;
...
void CosmicUnityLoop::CalculateFPS() {
    using namespace std::chrono;
    auto now = steady_clock::now();
...
void CosmicUnityLoop::RenderFrame() {
