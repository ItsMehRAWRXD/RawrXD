...
function Invoke-ShellTool {
    ...
    try {
        ...
        foreach ($cmdlet in $safeCmdlets) {
            $iss.Commands.Add((Get-Command $cmdlet))
        }
        ...
        if ($handle.AsyncWaitHandle.WaitOne(30000)) {
        ...
        $result = if ($output) { ($output | Out-String).Trim() } else { "ok" }
        if ($errors) {
            $errorText = ($errors | ForEach-Object { $_.Excepti