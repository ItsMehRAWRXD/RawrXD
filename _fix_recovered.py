...
LRESULT CALLBACK ActivityBarProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: {
        ...
        SidebarView view = SidebarView::None;
        if (id == IDC_ACTIVITY_EXPLORER) view = SidebarView::Explorer;
        else if (id == IDC_ACTIVITY_SEARCH) view = SidebarView::Search;
        else if (id == IDC_ACTIVITY_SCM) view = SidebarView::SourceControl;
        else if (id == IDC_ACTIVITY_DEBUG) view = SidebarView::RunDebug;
        else if (id == IDC_ACTIVITY_EXTENSIONS) view = SidebarView::Extensions;
        // Show the selected view window
        if (g_hwndSidebarContent) {
            ...
            // Simple mapping: use command id + 1000 as child window id
...
extern "C" void Win32IDE_Sidebar_Create(HWND hwndParent, HINSTANCE hInstance) {
    ...
    // Create placeholder view windows (hidden)
    for (const auto& btn : buttons) {
    ...
    g_currentView = SidebarView::Explorer;
...
std::vector<MCPMessage> MCPBridgeManager::GetRecentMessages(size_t count) const {
    ...
    size_t toReturn = std::min(count, m_messages.size());
...
base = r'f:\~dev\rawrxd\src\win32app'
write_file(os.path.join(base, 'Win32IDE_Commands.cpp'), commands_cpp)
write_file(os.path.join(base, 'Win32IDE_Sidebar.cpp'), sidebar_cpp)
write_file(os.path.join(base, 'Win32IDE_MCPHooks.cpp'), mcphooks_cpp)
print('Done')
