...
function Create-ExplorerContextMenu {
    ...
    # Open in pop-out editor
    $openPopOutItem = New-Object System.Windows.Forms.ToolStripMenuItem
    $openPopOutItem.Text = "Open in Pop-Out Editor"
    $openPopOutItem.add_Click({
        $node = $global:RawrXD.Components.FileExplorer.SelectedNode
        if ($node -and $node.Tag.Type -eq "File") {
            # Check if function exists (from main RawrXD.ps1)
            if (Get-Command Open-FileInPopOutEditor -ErrorAction SilentlyContinue) {
                Open-FileInPopOutEditor -FilePath $node.Tag.FullPath
            }
            else {
                Write-RawrXDLog "Open-FileInPopOutEditor function not available" -Level WARNING -Component "FileExplorer"
            }
        }
    })
    ...
    $contextMenu.Items.AddRange(@($openItem, $openPopOutItem, $openExternalItem, $separator1, $copyPathItem, $propertiesItem))
    return $contextMenu
}
