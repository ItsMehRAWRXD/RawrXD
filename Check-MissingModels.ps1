...
# Debug: Show first few installed models
Write-Host "   Sample installed models:" -ForegroundColor DarkGray
$installedModels | Select-Object -First 5 | ForEach-Object { Write-Host "     - $_" -ForegroundColor DarkGray }
...
foreach ($guideModel in $guideModels) {
    ...
    if ($installedModels -contains $guideModel.Name) {
        ...
        # Try fuzzy matching
        $guideModelName = $guideModel.Name

        # Special case: dolphin-llama alternative name
        if ($guideModel.Name -eq "dolphin-llama3.1:8b-q5_0") {
            foreach ($installed in $installedModels) {
                if ($installed -like "*dolphin*" -and $installed -like "*llama*3.1*") {
                    $found = $true
            ...
            # Extract key parts from guide model name
            # Remove hf.co/bartowski/ prefix
            $modelPart = $guideModelName -replace '^hf\.co/bartowski/', ''
            # Extract model name without quantization
            $modelBase = $modelPart -replace ':[^:]+$', '' -replace '-GGUF$', ''

            # Try to match against installed models
            foreach ($installed in $installedModels) {
                # Check if installed model name contains the guide model base name
                if ($installed -like "*$modelBase*") {
                    $found = $true
                    $matchedName = $installed
                    break
                }
                # Also check if guide model base contains installed model parts
                $installedBase = $installed -replace '^hf\.co/bartowski/', '' -replace ':[^:]+$', '' -replace '-GGUF$', ''
                if ($modelBase -eq $installedBase -or $installedBase -like "*$modelBase*" -or $modelBase -like "*$installedBase*") {
                    $found = $true
                    $matchedName = $installed
                    break
                }
