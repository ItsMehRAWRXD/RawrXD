# Generate and Build All Compilers
...
Write-Host "GENERATING AND BUILDING COMPILERS" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
foreach ($compiler in $compilers) {
    ...
    $asmFile = Join-Path $OUTPUT_DIR "$baseName.asm"
...
"@.Replace('`$','$')
    ...
    # Write assembly file
    $asmContent | Set-Content -Path $asmFile -Encoding ASCII
    
    # Assemble
    & $ML64 /c /Fo$objFile $asmFile | Out-Null
