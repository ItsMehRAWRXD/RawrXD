#!/usr/bin/env python3
"""
Gate 6: Multi-Layer Forward Pass (Fast Version)
Validates: Forward pass through multiple transformer layers

Acceptance Criteria:
- Input: token embeddings
- Process: Multiple transformer layers (5 layers for speed)
...
class Gate6Validator:
    ...
    def __init__(self, model_path, num_layers=5):
        ...
        self.layers = []
        self.total_layers = 0
        self.num_layers_to_test = num_layers
    ...
    def validate(self):
        """Run all validations"""
        print("=" * 60)
        print("Gate 6: Multi-Layer Forward Pass (Fast)")
        print("=" * 60)
    ...
    def parse_gguf(self):
        ...
        try:
            ...
            self.total_layers = self.parser.metadata.get('llama.block_count', 22)
    ...
    def load_layers(self):
        ...
        try:
            num_to_load = min(self.num_layers_to_test, self.total_layers)
            print(f"\nLoading {num_to_load} transformer layers...")
    ...
    def run_multi_layer_forward(self):
        """Run forward pass through all loaded layers"""
        try:
            ...
            for i, layer in enumerate(self.layers):
                x = layer.forward(x)
                if (i + 1) % 2 == 0 or i == len(self.layers) - 1:
    ...
    def generate_report(self):
        ...
        print("=" * 60)
        print(f"Model:    {self.model_path}")
        print(f"Layers:   {len(self.layers)} / {self.total_layers}")
        print("-" * 60)
        ...
        if failed == 0:
            ...
            print(f"\nNote: Tested {len(self.layers)} of {self.total_layers} layers")
            print(f"Full model would take ~{self.forward_time*1000*self.total_layers/len(self.layers):.0f}ms")
        else:
            print(f"\nResult: FAILED")
...
def main():
    ...
    # Test 5 layers for speed (full model has 22)
    validator = Gate6Validator(model_path, num_layers=5)
